...
//! 2026-09-25: Oracle for `moe_weighted_sum_blend` (`moe_expert_gemv.cu`), the MoE combine:
//!   `sig = sigmoid(sum_k input[k] * gate_weight[k])`, or 1 with a NULL gate
//!   `out[j] = sum_e weights[e] * expert_out[e*hidden + j] + sig * shared_out[j]`
//! Grid (ceil(hidden/256), 1, 1), block 256; both the gated and the NULL-gate modes run.
//!
//! Owner: model-arch examples (MoE kernels).
//! Invariants:
//! - The run exits 1 unless both modes reach `COSINE_GATE` against the CPU reference.
//!
//! cargo run --release -p metrale-model-arch --example moe_blend_microtest --features cuda,gpu-examples -- `[hidden] [top_k] [K] [seed]`

...
# 2026-09-26: Runs one graduated level with opencode and Claude Code; checks, records, and writes a bug file on failure.
#
...
Client invocation (both run in a fresh /tmp target dir, METRALE at :8888):
...
METRALE_PORT = os.environ.get("METRALE_HARNESS_PORT", "3001")
AGENT_TIMEOUT = int(os.environ.get("OVN_AGENT_TIMEOUT", "600"))
# 2026-09-26: Both clients and every completion check build in SHARED_TARGET
# (OVN_CARGO_TARGET), so dependencies compile once across levels and clients.
...
def run_opencode(prompt: str, target: pathlib.Path) -> None:
    ...
    try:
        run(
            # 2026-09-26: -k 10 sends SIGKILL 10 s after the SIGTERM.
            ...
            env={"METRALE_HARNESS_PORT": METRALE_PORT, "CARGO_TARGET_DIR": SHARED_TARGET},
        )
    finally:
        kill_opencode()
...
def run_claude(prompt: str, target: pathlib.Path) -> None:
    # 2026-09-26: `timeout` runs inside sudo, so it is claude's direct parent and
    # kills it on expiry.
    run(
        ["sudo", "-n", "-u", "claude",
         "timeout", "-k", "10", str(AGENT_TIMEOUT),
         "env", "ANTHROPIC_BASE_URL=http://localhost:8888", "ANTHROPIC_AUTH_TOKEN=dummy",
         f"METRALE_HARNESS_PORT={METRALE_PORT}", f"CARGO_TARGET_DIR={SHARED_TARGET}",
...
def main() -> int:
    ...
    for cname, runner in clients.items():
        ...
        # 2026-09-26: World-writable, because the claude client runs as user claude.
...
def _deployed_image() -> str:
    rc, out, _ = run(["sudo", "docker", "inspect", "metrale-camp", "--format", "{{.Config.Image}}"])

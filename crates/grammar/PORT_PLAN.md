...
## Public API contract (what Metrale Engine's `grammar/` module consumes)

These must exist with matching behaviour so `crates/server/src/
...
## Module layout (`crates/grammar/src/`)
...
W7  public API + Metrale Engine repoint (lib.rs; Cargo path swap)     dep: all
```
...
final gate: stand up Metrale Engine with the pure-Rust crate path-swapped in,

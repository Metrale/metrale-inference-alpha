...
fn load_dense_ffn(
    ...
    // METRALE_FFN_MMQ: eager Q4_K materialize + free `_t` at load (before KV sizing).
...
fn load_attention_layer(
    ...
    let is_sliding = if !config.layer_types.is_empty() {
        config.layer_types.get(i).copied()
            == Some(metrale_core::config::LayerType::SlidingAttention)
    } else {
        !i.is_multiple_of(4)
    };

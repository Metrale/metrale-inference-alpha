...
# 2026-09-26: Finds the Qwen3.8-Flash-Next snapshot: QWEN4EXP_PATH, else the
# caller's default if it exists, else the newest snapshot with a config.json in
# the HF cache, trying the repos of _REPOS in order.
#
...
# 2026-09-26: Searched in this order; the module docstring says why.
_REPOS = (
...
def _from_cache() -> str | None:
    for repo in _REPOS:
        ...
        for s in snaps:
            # 2026-09-26: A snapshot without config.json is skipped as a partial
            # download.

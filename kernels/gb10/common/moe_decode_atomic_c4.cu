...
__device__ __forceinline__ float metrale_dec_e4m3_atomic_c4(unsigned char b) {
...
extern "C" __global__ void moe_decode_atomic_c4_silu_down_accum(
    ...
    for (unsigned int k8 = lane; k8 < K8; k8 += threads_per_out) {
        ...
        float sc1 = metrale_dec_e4m3_atomic_c4(sb1) * s2;
        ...
        float sc2 = have_n2 ? metrale_dec_e4m3_atomic_c4(sb2) * s2 : 0.0f;

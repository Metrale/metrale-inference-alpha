...
/// * `kv_gemv_pinned` — `METRALE_MTP_KV_GEMV` present: the PRE-EXISTING lever
...
/// * `small_m_tier_off` — `METRALE_NO_DRAFTER_SMALL_M_TIER=1`: restores the
...
/// `METRALE_NO_DRAFTER_SMALL_M_TIER=1` — restore the pre-tier dispatch.
...
/// `METRALE_MTP_KV_GEMV` (presence) — pin the small-N K/V projections to the
...
mod tests {
    ...
    /// `METRALE_NO_DRAFTER_SMALL_M_TIER=1` must reproduce the pre-tier decision
    ...
    /// `METRALE_MTP_KV_GEMV` keeps meaning "small-N K/V on the per-row loop" at

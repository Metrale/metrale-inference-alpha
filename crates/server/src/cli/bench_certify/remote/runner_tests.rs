...
//! 2026-09-26: The remote runner over a scripted metralectl: what each way a node can answer turns into.
//!
//! Owner: server CLI (`met benchmark certify`).
//! Invariants: none beyond the types.
use super::super::super::plan::{Estimate, Unit};
use super::super::metralectl::{
    AttachEnd, ErrorObj, Exit, FetchedFile, Metralectl, NodeRow, Refusal, StreamEvent, SubmitSpec,
    Submitted,
};
use super::*;
use anyhow::Result;
...
/// 2026-09-26: A scratch repository with the fixture signer registry and the lexically
/// last fixture decode-floor record staged where a fetch would put it.
struct Scratch {
    root: PathBuf,
...
fn run(
    ...
    let mut r = RemoteRunner {
        metralectl: script.clone(),
        node: node(built),
        run_id: "r1".into(),
        // 2026-09-26: The record's sha padded to 40 characters stands in for the full
        // sha the driver resolves (`gate::git_rev_parse`).
    ...
    // 2026-09-26: Whenever an attach happened, its progress reached the line stream.
...
fn a_passing_job_is_submitted_followed_fetched_placed_and_classified() {
    ...
    // 2026-09-26: The submit carries the 40-character `anchor_full`, not `ctx.anchor`.
...
fn a_lost_stream_is_reattached_from_the_last_seq_and_bounded() {
    ...
    // 2026-09-26: Negative control: more than `MAX_REATTACH` losses is a retryable
...
fn refusals_and_failures_before_a_record_are_harness_failures_with_the_right_retryability() {
    ...
    // 2026-09-26: A `failed` outcome (here a build failure) is a retryable harness
...
fn a_record_that_is_not_what_was_asked_for_is_refused_for_good() {
    let s = scratch("wrong");
    // 2026-09-26: The node hands back files for another gate.
...
fn the_deadline_pays_for_a_build_only_on_a_cold_node() {
    ...
    // 2026-09-26: The job key is bounded and names the campaign, the node and the gate.
    let r = RemoteRunner {
        metralectl: Arc::new(Script::new(vec![], vec![])),
        node: node(true),
    ...
    // 2026-09-26: A shard's key carries its slice; when the whole does not fit in
    // `JOB_KEY_MAX`, the gate name loses its front, never the shard tail, so two
    // shards of one group do not share a key.
    let shard = |i, n| Unit {

#!/usr/bin/env bash
# Serve a model with Metrale Engine on AMD GPUs (SCALE runtime). Verified coherent on
...
export METRALE_FORCE_GLOBAL_GDN=1   # GDN prefill -> global-mem kernel (RDNA3.5 64KB LDS cap)
export METRALE_W4A16_VARIANT=v1     # BF16-MMA NVFP4 GEMM (SCALE device FP8 encode is broken on gfx1151)
export METRALE_NO_FP8_PREDEQUANT=1  # skip NVFP4->FP8 predequant (same reason)
...
exec target/release/met serve "$MODEL" \

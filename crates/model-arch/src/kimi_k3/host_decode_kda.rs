...
//! 2026-09-25: Tests of `K3BoundLayer::decode_host` on a mock GPU: which CUDA kernels each mixer and MLP kind looks up and launches under the `want_cuda_kda` / `want_cuda_mla` flags.
//!
...
use metrale_model_weights::kimi_k3_host::{K3CpuModel, MixerKind, MlpKind};
...
fn run_layers_try(
    ...
    for &(layer_idx, want_cuda_kda, want_cuda_mla) in steps {
        ...
        let mut state = K3CpuFallbackState::new(match layer.spec.mixer {
            MixerKind::Kda => metrale_model_weights::kimi_k3_host::LayerCache::Kda(
                metrale_model_weights::kimi_k3_host::KdaState::new(&model.kda),
            ),
            MixerKind::Mla => metrale_model_weights::kimi_k3_host::LayerCache::Mla(
...
fn mla_layer_ignores_cuda_kda_flag() {
    ...
    // 2026-09-25: Layer 0 runs first to create the AttnRes stream.
...
/// 2026-09-25: Pins the launch count: `launch_k3_latent_moe_experts` runs w1
/// and w3 as one grouped GEMM of `2 * m` rows, then w2. That the batched
/// gate/up matches separate launches is checked on a GPU by
/// `crates/model-engine/tests/k3_moe_batching_cuda_oracle.rs`.
#[test]

...
// 2026-09-26: Grouped MoE expert GEMMs (module `moe_w4a16`) for minimax-m2-229b, in place
// of common/moe_w4a16_grouped_gemm.cu ([shadow] in KERNEL.toml); laguna-s-2.1 and step3p7-flash
// compile it through [sources] use. Weights are NVFP4 (E2M1 pairs, an E4M3 scale per 16 K values and a
// per-expert FP32 scale2) found through per-expert pointer tables. C rows
// [expert_offsets[e], expert_offsets[e + 1]) belong to expert e; C row r reads A row
// sorted_token_ids[r] when that pointer is non-null, else A row r.
// forked-from: kernels/gb10/common/moe_w4a16_grouped_gemm.cu (2026-09-24; 1900 of 1642 lines differ, see kernels/FORKS.md)
// Owner: gb10 kernels (minimax-m2-229b).
// Invariants: none beyond each kernel's launch shape; a CTA whose expert weight pointer is null writes nothing.
...
#define K_STEP_T 32
#define PAD 2
#define PAD_T 8        // 2026-09-25: cp.async needs 16-byte aligned rows: (32+8)*2 = 80 B
#define BP_PAD 16
...
// [K/16, N] scales and dequantise it to FP8 E4M3 in shared memory for m16n8k32 e4m3 MMAs.
// moe_w4a16_grouped_gemm_ptrtable_t, next: 128 threads (4 warps of 16 rows) per 64 x 128
// tile, 32 K per step, A fragments converted from BF16 to E4M3 in registers.
...
// 2026-09-25: Four BF16 values from shared memory as a u32 of four E4M3 (satfinite) values.
...
extern "C" __global__ void moe_w4a16_grouped_gemm_ptrtable_t(
    ...
    // 2026-09-25: +4 pad: 36-byte rows (9 words, gcd(9, 32) = 1). The dequant writes
    // (bank t*9 + kp/2) are conflict-free; the MMA B reads ((nc*9 + tid) % 32) have 3
    // conflicting pairs, against 16 with 32-byte rows.
    ...
    // 2026-09-25: Dequantise B (E2M1 * E4M3 scale * scale2) to FP8 E4M3, one thread per N column.
    ...
    // 2026-09-25: A converted from BF16 to E4M3 in registers; one m16n8k32 per N-tile.
...
// 2026-09-25: moe_w4a16_grouped_gemm_ptrtable_t with 64 K per step: two m16n8k32 MMAs per
// N-tile per step, 128 threads per 64 x 128 tile.
...
#define PAD_T64 8  // 2026-09-25: (64+8)*2 = 144 B rows, 16-byte aligned for cp.async
...
extern "C" __global__ void moe_w4a16_grouped_gemm_ptrtable_t_k64(
    ...
    // 2026-09-25: B_fp8 rows are 80 bytes (K64 + 16), 20 words: the MMA B reads
    // ((nc*20 + tid) % 32 for nc = 0..7) hit 32 distinct banks. 64-byte rows would give
    // 4-way conflicts.
    ...
    // 2026-09-25: Per buffer: A 4 rounds x 128 threads x 16 B = 8192 B = 64 x 64 BF16;
    // Bp 2 rounds x 128 threads x 16 B = 4096 B = 32 x 128 packed bytes; Bs 4 scale groups
    // x 8 ns x 16 B = 512 B.
    ...
    // 2026-09-25: Dequantise B to FP8 E4M3 as in the 32-K kernel, with 4 scale groups.
    ...
    // 2026-09-25: Two m16n8k32 MMAs per N-tile, k 0..31 then k 32..63: a0..a3 serve all
    // first-half MMAs before a4..a7 are loaded, so at most 4 A registers are live.
...
// 2026-09-25: moe_w4a16_fused_gate_up_t with 64 K per step (the loads, dequant and MMA of
// moe_w4a16_grouped_gemm_ptrtable_t_k64). 128 threads per 64 x 128 tile.
...
// 2026-09-25: moe_w4a16_fused_gate_up_t_k64 with a 128-row M tile: 256 threads (8 warps of
// 16 rows), 64 K per step, the same BF16 -> E4M3 A conversion and m16n8k32 e4m3 MMAs.
// Its static shared memory is 58,048 B. moe/init.rs resolves it with try_kernel; no other
// kernel source defines it.
...
extern "C" __global__ void moe_w4a16_fused_gate_up_t_k64_m128(
    ...
    // 2026-09-25: A: 256 threads x 4 rounds x 8 BF16 = 8192 = M_TILE_M128 x K_STEP_T64. The
    // rounds are 32 rows apart because a_row_base (threadIdx.x >> 3) spans 0..31.
    #define FGU64M128_ISSUE_LOADS(buf, kb) do { \
        ...
        /* 2026-09-25: B: 256 threads cover 32 K-pairs x 8 N chunks of 16 B, the whole K_STEP_T64/2 x N_TILE_LG tile in one round. */ \
        { \
            unsigned int kp_cur = threadIdx.x >> 3;  \
            unsigned int ns = (threadIdx.x & 7) << 4;  \
            unsigned int gns = cta_n + ns; \
            unsigned int gke = (kb) + (kp_cur << 1); \
    ...
    // 2026-09-25: Dequant: threads 0..127 take one N column each; threads 128..255 only join
    // the MMAs.
    ...
    // 2026-09-25: 8 warps of 16 M rows, 16 N-strips each: 2 K halves x 16 x 8 = 256 m16n8k32
    // MMAs per K step.
...
// 2026-09-25: Gate and up projections in one launch. Grid (2*N / N_TILE_LG, max_m_tiles,
// num_experts): x tiles [0, N) use the gate weights and write C_gate, the rest use the up
// weights and write C_up, so N % N_TILE_LG == 0 keeps a tile from straddling the two.
...
extern "C" __global__ void moe_w4a16_fused_gate_up_t(
    ...
    const int* __restrict__ sorted_token_ids,
    unsigned int num_experts,
    unsigned int N,            // 2026-09-25: per-projection N (gate and up have same N)
    unsigned int K
) {
    const unsigned int expert_id = blockIdx.z;
    if (expert_id >= num_experts) return;
    ...
    __shared__ unsigned char smem_Bp[2][K_STEP_T / 2][N_TILE_LG + BP_PAD];
    __shared__ unsigned char smem_Bs[2][K_STEP_T / GROUP_SIZE][N_TILE_LG + BP_PAD];
    // 2026-09-25: The +4 pad of moe_w4a16_grouped_gemm_ptrtable_t; 32-byte rows would put the dequant writes 8-way on 4 banks.
    __shared__ unsigned char smem_B_fp8[N_TILE_LG][K_STEP_T + 4];
    __shared__ float smem_LUT[16];
    __shared__ int smem_tok[M_TILE];
    ...
    // 2026-09-25: These macros are copies of moe_w4a16_grouped_gemm_ptrtable_t's.
...
// 2026-09-25: moe_w4a16_grouped_gemm_ptrtable_t with A already FP8 E4M3 ([total_tokens, K]),
// so A needs no conversion. Same 64 x 128 tile and 32 K per step; the A buffers are FP8,
// 2 KB each against 5 KB in BF16. Grid (ceil(N/128), max_m_tiles, num_experts).

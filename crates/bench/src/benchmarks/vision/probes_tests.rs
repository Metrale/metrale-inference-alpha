...
//! 2026-09-26: Tests that the probe definitions are coherent. Whether the
//! model answers them is what a run measures.
//!
//! Owner: bench, vision.
...
fn every_referenced_fixture_exists() {
    // 2026-09-26: A name missing from FIXTURES would otherwise show only
    // mid-run, as a probe `Error`.
...
fn expectations_are_lowercase_and_not_self_contradictory() {
    // 2026-09-26: Scoring lowercases the reply, so an uppercase term could
    // never match.
    for p in PROBES.iter().chain(std::iter::once(&CONTROL)) {
...
fn every_probe_asserts_something() {
    // 2026-09-26: A probe with neither `want_all` nor `want_none` passes
    // whatever the reply.
    for p in PROBES {
        assert!(
...
fn the_control_sends_no_image_and_guards_a_real_probe() {
    // 2026-09-26: The control sends no image, and forbids a term that some
    // probe requires.

...
//! 2026-09-26: Tests for the TTFT gates: parameter defaults, statistics,
//! reconfiguration, the verdict against a stored baseline, and prefix tags.
//!
//! Owner: bench, ttft.
...
fn gate(mode: Mode, root: &str) -> TtftGate {
    ...
    std::mem::forget(rx);
    let dir = std::env::temp_dir().join(format!("metrale-ttft-{root}-{}", std::process::id()));
...
fn a_regression_past_the_median_limit_fails() {
    ...
    // 2026-09-26: A delta equal to the limit passes.
    ...
    // 2026-09-26: +3.1% median is past the limit and 24.8 ms is past the noise
    // floor.
    ...
    // 2026-09-26: p90 alone can fail it.
...
fn a_baseline_from_another_target_reports_instead_of_gating() {
    ...
    .unwrap();
    // 2026-09-26: 8x the stored median, but the baseline is from another host,
    // so the verdict is Info, not Fail.
...
/// 2026-09-26: A failing run does not overwrite the baseline it failed against;
/// otherwise the same build, run again, would compare with its own regressed
/// numbers and pass.
#[test]
fn a_failing_run_does_not_become_the_new_baseline() {
    ...
    // 2026-09-26: A pass stores, and so does the first run on a host (Info).
    assert!(g.should_store(&Verdict::pass("median +0.1%")));
    assert!(g.should_store(&Verdict::info("no baseline on this box yet")));

    // 2026-09-26: With `update_baseline` off, even a pass is not stored.
...
fn warm_reuses_one_tag_per_length_and_cold_never_repeats_one() {
    // 2026-09-26: Warm reuses one prefix tag per length; cold draws a new tag
    // per sample and per run.

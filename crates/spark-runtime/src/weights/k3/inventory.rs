...
use metrale_core::kimi_k3::tp::{TpBytePlan, plan_tensor_bytes};
use metrale_core::kimi_k3::{K3Graph, MixerKind};
...
pub(super) fn scan(dir: &Path, config: &ModelConfig) -> Result<Vec<ShardPlan>> {
    ...
    for file_name in names {
        ...
        for name in tensor_names {
            ...
            if metrale_core::kimi_k3::tp::tensor_plan(name, mixer, config).0
                == metrale_core::kimi_k3::tp::TpAxis::Replicated
            {
                metrale_core::kimi_k3::weights_geometry::validate_replicated_shape(
                    name,
                    view.shape(),
                    config,
                )?;
    ...
    for required in metrale_core::kimi_k3::weights::required_names(config) {

...
#
# 🪤 The three METRALE_TARGET_* vars pick which kernel set is compiled in. Unset, the build prints
...
set -euo pipefail
TAG="${1:?usage: build-metrale-glm53-image.sh <tag>   e.g. t48}"
BASE="${BASE:-metrale-glm53:t47}"
...
for n in $NODES; do
  ...
  sudo -n -u cluster scp -q -o BatchMode=yes target/release/met "cluster@$n:~/glm53-build/met"
  sudo -n -u cluster ssh -o BatchMode=yes "cluster@$n" "cd ~/glm53-build && \
    printf 'FROM %s\nCOPY met /usr/local/bin/met\n' '$BASE' > Dockerfile && \
    docker build -q -t 'metrale-glm53:$TAG' ."
done
echo "metrale-glm53:$TAG built on: $NODES   (base $BASE)"

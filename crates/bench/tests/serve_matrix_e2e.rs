...
//! 2026-09-26: The Serve Matrix's state machine, driven to completion against
//! the loopback mock through a fake [`ServeHost`]: no planned round may be
//! skipped or dropped from the denominator.
//!
...
use metrale_bench::benchmarks::serve_matrix::ServeMatrix;
use metrale_bench::benchmarks::serve_matrix::host::{ServeCandidate, ServeHost, ServeOptions};
use metrale_bench::{
    ArtifactStore, Benchmark, BenchmarkResult, ParamValue, ParamValues, Plugin, PluginHandle,
    RunStatus, TargetEndpoint, VerdictKind,
};

/// 2026-09-26: A host that serves `up` at the mock's port and refuses
/// everything else, so one run exercises both the booted and the
/// did-not-boot paths.
struct FakeHost {
    port: u16,
    up: String,
...
fn store(name: &str) -> ArtifactStore {
    let dir = std::env::temp_dir().join(format!("metrale-sm-{name}-{}", std::process::id()));
...
async fn every_planned_round_is_reached_and_a_crashed_one_fails_the_matrix() {
    // 2026-09-26: The mock answers `/v1/models` with `{"id":"mock"}`, so the
    // round whose model is "mock" also passes the identity bar.
    ...
    // 2026-09-26: 0 skips the long-context probe (`16_384` tokens by default);
    // every other probe still runs.
    ...
    // 2026-09-26: The mock is a canned SSE stream, not a model, so the round
    // that came up fails its content probes. What this test checks is that
    // both rounds are in the denominator and each failure is attributed to
    // what happened to it.
    ...
    // 2026-09-26: Read the two rows rather than parse the prose. Rounds are
    // sorted by model, so `mock` is first and `org/crashes` second.
    ...
    // 2026-09-26: The round that came up was probed, not skipped: its failure
    // names probes, never `did-not-boot`.
    ...
    // 2026-09-26: Coherence (2) + codegen + tool call + throughput, for the
    // one round that came up. The crashed round never reaches the endpoint.
    assert_eq!(
...
async fn a_round_the_endpoint_serves_under_another_name_is_not_scored_as_that_model() {
    // 2026-09-26: The fake host claims it booted `org/never-loaded`, but the
    // endpoint reports `mock` in `/v1/models`: the round fails as
    // `wrong-model` and is not scored as the checkpoint that was never loaded.
    let mock = mock_endpoint::start(8, Duration::from_millis(1), Duration::from_millis(1)).await;
    let mut bench = ServeMatrix::with_host(Arc::new(FakeHost {
        port: mock.port,

...
//! 2026-09-25: K=4 graphed speculative verify of one sequence (`decode_verify_graphed_k4`):
//! the four rows go through every layer in one pass, captured as a CUDA graph per SSM
...
impl TransformerModel {
    pub(super) fn decode_verify_graphed_k4_dispatch(
        ...
        // 2026-09-25: SAFETY: `positions` is the 4-element `[u32; _]` literal directly
        // above (one entry per verify row), size 4 * 4 = 16, exactly the
        ...
        // 2026-09-25: SAFETY: `slots` is the `[0i64; 4]` declared above, zero at
        ...
        // 2026-09-25: SAFETY: `seq_lens` is the 4-element `[i32; _]` literal directly
        ...
        // 2026-09-25: `METRALE_K4_DIAG=1` runs the verify without a graph and synchronizes
        // after every layer and stage, so a device fault is reported at the layer that
        // raised it.
        let k4_diag = std::env::var("METRALE_K4_DIAG").ok().as_deref() == Some("1");
        ...
        // and logs the enqueued ops that differ from the previous step's. It turns graphs on
        // and caches none.

...
#   scripts/coverage.sh lcov.info -p metrale-server  # extra args go to the run
#
...
export METRALE_SKIP_BUILD="${METRALE_SKIP_BUILD:-1}"
...
#                            OUT_DIR/*_ptx.rs that metrale-kernels/metrale-storage
#                            `include!`
#   build.rs                 build scripts run at compile time, not under test
#   crates/kernels/    entirely build.rs-generated PTX constants
#   gpu-sys/src/cufile.rs    raw dlopen FFI to libcufile (GDS; dormant on GB10)
#   crates/comm/, gpu-sys/src/nccl.rs  raw FFI to libnccl; CI links a fail-fast stub
#   gpu-sys/src/verbs.rs     ibverbs FFI; `cfg(metrale_rdma_verbs)` is compiled
#                            OUT under METRALE_SKIP_BUILD. The rest of the crate
#                            (wire codecs, railset, handshake) IS covered and
#                            deliberately stays in the report.
#   model-layers/.../ops/    thin `KernelLaunch` wrappers (plus the two files
#                            that moved out of ops/ to model-arch) — one CUDA kernel
...
IGNORE_REGEX='(^|/)vendor/|(^|/)target/|/build\.rs$|(^|/)crates/(kernels|comm)/|(^|/)crates/gpu-sys/src/(cufile|nccl|verbs)\.rs$|(^|/)crates/model-layers/src/layers/ops/|(^|/)crates/model-arch/src/(glm5next_mhc|ple_tests)\.rs$|(^|/)crates/[^/]+/(tests|benches|examples)/'

...
// 2026-09-25: NVFP4 MoE GEMVs for three tokens that run the routed experts and the shared
...
// 2026-09-25: Gate+up for three tokens. blockIdx.z selects gate (0) or up (1); each warp
// computes two adjacent outputs, 8 per block.
// Launch: grid (ceil(N / 8), 3 * (top_k + 1), 2), block 128 (BLOCK_SIZE).
...
// 2026-09-25: SiLU(gate) * up, then down, for three tokens.
// Launch: grid (ceil(N / 8), 3 * (top_k + 1), 1), block 128, and K * 4 bytes of dynamic
// shared memory for s_act.
...
// gate_weight)), or 1 when gate_weight is null. blockIdx.y is the token (0 to 2).

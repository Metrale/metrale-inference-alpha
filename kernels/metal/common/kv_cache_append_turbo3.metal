...
// 2026-09-25: Append one token's K and V to a contiguous Turbo3 cache at row
// cache_pos. Per group of 16 values: 3-bit indices into a unit-Gaussian
// Lloyd-Max codebook (8 per 3 bytes) after scaling by TURBO3_MAX / max|x|,
// and an E4M3 scale that makes the reconstruction's L2 norm equal the
// input's. K and V are quantized as given: this kernel does not rotate them.
//
// Layout: new_k and new_v are bfloat [num_kv_heads, head_dim]; k_data and
// v_data are uchar [max_seq, num_kv_heads * head_dim * 3 / 8]; k_scales and
// v_scales are E4M3 uchar [max_seq, num_kv_heads * head_dim / 16].
//
...
// 2026-09-25: 8-level Lloyd-Max codebook for N(0,1) and its decision boundaries.
...
// 2026-09-25: Pack 8 3-bit indices into 3 bytes, LSB first; the CUDA append in kernels/gb10/common/reshape_and_cache_turbo.cu packs the same way.

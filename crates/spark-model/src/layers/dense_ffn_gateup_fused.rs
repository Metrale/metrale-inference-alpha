...
//! symlink is created. `METRALE_FFN_GATEUP_FUSED=0` kills it; on gb10/b200
...
/// ffn_gateup_fused`); `METRALE_FFN_GATEUP_FUSED` overrides it under the
/// 2026-09-11 grammar, so `=0`/`=off`/`=false` turn it off and anything else
/// turns it on. There is no `METRALE_NO_*` legacy spelling: the lever is new, so

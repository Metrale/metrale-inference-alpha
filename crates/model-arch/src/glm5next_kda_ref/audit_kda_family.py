...
# 2026-09-25: Audits every KDA attention block in the checkpoint snapshot against the expected
# tensor set, and extracts per-layer packets.
#
# Owner: model-arch (GLM-5.3-Flash KDA reference).
# Invariants:
# - Reads the snapshot only; writes only under OUT.
...
# 2026-09-25: The self_attn tensors a KDA block must have, and nothing else; the same names,
# dtypes and shapes as KDA_TENSORS in glm5next_kda/binding.rs.
EXPECT = {
...
EXTRA = {"input_layernorm.weight": ("BF16", [HID])}
...
def read(L, shard, short):
    # 2026-09-25: Shard headers key tensors by the full name, not the layer-relative one.
...
for L in sorted(by_layer):
    ...
    if is_kda:
        ...
        # 2026-09-25: Any scale, zero-point or quant tensor under self_attn. is a problem.

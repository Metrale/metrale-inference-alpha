...
// 2026-09-25: Three-token FP8 MoE GEMVs on the transposed layout. Owner: gb10 kernels.
// Invariants: those of moe_shared_expert_fused_fp8_t.cu; blockIdx.y = 3 * top_k + t is token t's shared expert.
#include <cuda_bf16.h>
#include <cuda_fp8.h>
...
extern "C" __global__ void moe_expert_gate_up_shared_fp8_batch3_t(
    ...
    for (unsigned int kb = 0; kb < k_blocks; kb++) {
        ...
        for (unsigned int k = k_start; k < k_end; k++) {
            ...
            acc += metrale_dec_e4m3(w_byte) * sc * __bfloat162float(A_token[k]);
        }
    }

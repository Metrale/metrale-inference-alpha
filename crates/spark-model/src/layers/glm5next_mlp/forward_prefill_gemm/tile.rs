...
/// `METRALE_GLM_MOE_GEMM_TILE=<name>`.
///
...
/// byte-identical. `METRALE_GLM_MOE_GEMM_TILE=base` restores the prior behaviour exactly.
...
/// Resolve `METRALE_GLM_MOE_GEMM_TILE` to one of [`GEMM_TILES`]. The value is the kernel's
...
/// `METRALE_GLM_MOE_PREFILL_GEMM=0` restores the row-batched GEMV path exactly.
///
/// 🔬 Default ON in this branch so the A/B is one env flip on ONE image — the same lever
/// shape as `METRALE_GLM_MOE_ROW_BATCH_MAX`. Read once: this sits on the per-layer path.
...
/// Narrowest prefill sub-chunk the grouped GEMM may take, `METRALE_GLM_MOE_PREFILL_GEMM_MIN_ROWS`.
///
...
/// Read the REAL expert histogram to size the grid, `METRALE_GLM_MOE_PREFILL_GEMM_EXACT_TILES=0`

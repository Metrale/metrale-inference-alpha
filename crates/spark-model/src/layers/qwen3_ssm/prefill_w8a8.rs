...
//! prefill projection: the cuBLASLt arm (`METRALE_CUBLAS_GEMM=ssm`) and the
...
//! tip `5f78270dc`. `METRALE_CUBLAS_GEMM=1` used to route this projection to
...
//! `METRALE_FP8_ROWWISE=1`, which shadows this arm entirely (see
//! `trait_prefill_proj.rs`). Turning `ssm` off in `METRALE_CUBLAS_GEMM` restores
//! the in-tree W8A8 kernel, and `METRALE_FP8_SINGLE_SCALE=1` the W8A16 one.
...
///
/// * `cublas_ssm` — `METRALE_CUBLAS_GEMM` must name the `ssm` family. The whole

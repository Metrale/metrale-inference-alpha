...
//! 2026-09-25: Checks on the CPU that the hopper qwen3.6-27b and qwen3.8-27b
//! targets copy gb10's model declarations, that qwen3.8-27b compiles
//! qwen3.6-27b's sources, and that the resolver picks qwen3.8-27b for
...
fn the_h200_fp8_checkpoint_selects_38_behavior_without_an_order_default() {
    ...
    let candidates: Vec<_> = models
        ...
        .collect();
    // 2026-09-25: Both targets declare (qwen3_5, 5120), so a reference that
    // names neither is refused as ambiguous, not resolved by order.
...
fn dense_warp_fp4_is_explicitly_absent_only_on_hopper() {
    ...
    assert!(
        guarded
            .trim_end()
            .ends_with("#endif  // METRALE_NO_WARP_BLOCKSCALE_MMA")
    );
    for model in ["qwen3.6-27b", "qwen3.8-27b"] {
        let hopper = declaration("hopper", model);

...
Metrale Engine supports **all three modes via a single `--tp-size N --ep-size M`
flag pair**:
...
- `crates/comm/src/nccl_backend.rs` exposes the collective ops
...
`METRALE_UNIFIED_MOE_LAYOUT=1` until decode regression is closed.

...
// Metrale Engine spark-storage: high-speed NVMe-backed KV cache offload.
//
// Phase 0 of `--high-speed-swap`: runtime probe
...
// RDMA cold-tier stack below is therefore unix-only, and a Windows `met`
...
/// `true` iff `metrale_rdma_verbs` was re-emitted for this crate by build.rs (the
/// one-sided verbs shim lives in the CUDA-free `metrale-rdma` crate; `rustc-cfg`
/// doesn't cross crates, so build.rs re-emits it off metrale-rdma's `links`

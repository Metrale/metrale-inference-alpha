...
//! 2026-09-25: Bind the GLM-5.3 vision tower (`*.visual.*`) as BF16 device pointers.
//!
//! Owner: model-arch weight loader (GLM-5.3).
//! Invariants:
//! - Every bound tensor is BF16; any other dtype fails the load (`bf16`).
//! - No tensor is copied: each field is the store's own pointer.
//! - The patch-embed conv weight is read as `[hidden_size, 3 * temporal_patch_size *
//!   patch_size^2]` and the downsample conv weight as `[out_hidden_size, hidden_size *
//!   spatial_merge_size^2]`, over the same bytes; `check_shape` compares the leading dim
//!   and the product of the trailing dims.

use anyhow::{Context, Result};
use metrale_config::{ModelConfig, VisionConfig};
use metrale_gpu_runtime::gpu::{DevicePtr, GpuBackend};
use metrale_model_weights::weights::{WeightDtype, WeightStore};

use metrale_model_layers::layers::glm_vit::init::GlmVitGeometry;
use metrale_model_layers::layers::{GlmVit, GlmVitBlock, GlmVitMerger, VisionTower};

/// 2026-09-25: Bind one BF16 tensor by name, refusing any other dtype rather
/// than reinterpreting its bytes.
...
/// 2026-09-25: Check a bound tensor's on-disk shape against what the encoder
/// will read.
///
...
fn load_merger(store: &WeightStore, vp: &str, v: &VisionConfig) -> Result<GlmVitMerger> {
    ...
    Ok(GlmVitMerger {
        ...
        // 2026-09-25: The merger's post-projection norm is a LayerNorm with
        // bias (`glm_vit/merge.rs` passes `norm_b`), so a missing bias fails
        // the load here.
...
/// 2026-09-25: The body of `Glm5NextWeightLoader::load_vision_encoder`.
///
/// `Ok(None)` when the config has no `vision_config`, and, with a warning,
/// when it has one but no tower prefix holds `patch_embed.proj.weight`.
pub fn load_glm5_next_vision(
    ...
    // 2026-09-25: The tower may sit under any of three prefixes; the first
    // that holds `patch_embed.proj.weight` is used.
...
#[cfg(test)]
mod tests {
    /// 2026-09-25: The bound tensor count: 14 per block (the `GlmVitBlock`
    /// fields in `load_block`) and 11 outside the blocks (patch-embed weight
    /// and bias plus the nine `GlmVitMerger` fields), the same count the load
    /// log prints as `v.depth * 14 + 11`.
    #[test]

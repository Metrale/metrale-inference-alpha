...
//! 2026-09-26: Tests for the driver's non-network surface: descriptor id,
//! parameter defaults and validation, reconfiguration, and the `scored` seam.
//!
...
fn defaults_validate_and_pin_twelve_rounds() {
    ...
    // 2026-09-26: The budget must leave room for a replay turn to reach
    // `COLLAPSE_RATIO_CEIL` times its reference before it is clamped.
...
fn rounds_below_three_are_rejected_at_configure() {
    ...
    // 2026-09-26: The `rounds` spec's minimum is 3, so `validate_against`
    // refuses 2.
...
fn scored_passes_on_jitter_via_the_driver_seam() {
    // 2026-09-26: A jittered round passes.
    let mut b = configured();
    b.replays = vec![
        RoundRecord {
            round: 1,

...
pub struct Qwen3AttentionLayer {
    ...
    // W8A8 + FP32 epilogue (vLLM-equivalent) — gated by METRALE_FP8_W8A8=1.
    ...
    /// Native Q2_0 MMQ prefill (Tier-2, `METRALE_GGUF_NATIVE_Q2_MMQ`): keep-packed
    ...
    /// `METRALE_FFN_M16_TC` tier for the FP8 o_proj (contiguous) and multi-seq
    ...
    /// `METRALE_ATTN_M16_TC` (or the `METRALE_M16_TC` umbrella), cached at
    ...
    /// (`METRALE_FFN_M16_TC`), because round 6 on 1xH100 measured the two moving
    ...
    /// `METRALE_ATTN_NCOL_GEMV` (+ `METRALE_ATTN_NCOL_WIDTH`), resolved ONCE at
    /// construction for the same graph-replay reason as `m16_tc`. `None` when
    /// the lever is unset or `METRALE_NO_ATTN_DECODE_BATCH` forces it off.
    ...
    /// unused unless `METRALE_ATTN_DECODE_GQA_PACK` arms them AND the launch
    ...
    /// dequant + BF16 MMA, no FP8 activation crush). Opt-in via METRALE_BF16_TC_PROJ

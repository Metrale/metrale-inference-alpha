...
pub enum WeightQuantFormat {
    ...
    /// Standard Qwen-team FP8 release format (BS=128). NO Metrale Engine kernel
...
/// under `METRALE_GGUF_NATIVE_Q2=1`; the buffer is owned by the `WeightStore`, so
...
impl QuantizedWeight {
    ...
    pub fn transpose_for_gemm_gs(
        ...
        // 27B). METRALE_HOST_TRANSPOSE=1 forces the host path (parity/debug);

...
# 2026-09-26: Concurrent agentic harness: C opencode agents at once, one distinct task each.
#
# Owner: bench, agentic probes.
# Invariants: none beyond the types.
"""Concurrent agentic harness — measures how Metrale Engine holds up with C agents at once.
...
CONTAINER = os.environ.get("METRALE_CONTAINER", "laguna-s")
PROVIDER = os.environ.get("OPENCODE_PROVIDER", "metrale")

# 2026-09-26: (name, kind, prompt, verify_argv). `run_level` gives slot i the task
# `tasks[i % len(tasks)]`, so no two slots share a prompt until C exceeds the pool.
PY = "python3"
PY_TASKS = [
    ("calc", "unittest",
...
def run_agent(prompt, workdir, timeout, model, max_repeat=0):
    ...
    """
    # 2026-09-26: Prefix PROVIDER unless the id already starts with it. A slash alone
    # does not mean qualified: HF-style ids contain one of their own.
    ...
    # 2026-09-26: Wall-clock watchdog. The in-loop timeout check below runs only when
    # the child prints a line, because `for line in p.stdout` blocks in readline; the
    # timer kills a silent child on schedule.
    ...
    def _fire():
        ...
        try:
            ...
            # 2026-09-26: The child exited before the kill; timed_out is already set.
            pass
...
def require_container():
    ...
    if CONTAINER not in names:
        sys.exit(f"METRALE_CONTAINER={CONTAINER!r} is not a running container "
                 f"(running: {', '.join(names) or 'none'}). Throughput is read from "
                 f"its log, so the sweep would report 0.0 tok/s. Set METRALE_CONTAINER.")
...
def run_level(conc, tasks, model, timeout, keep, max_repeat=0):
    ...
    def one(i):
        ...
        # 2026-09-26: Report both: `err` says why the agent was cut short (loop or
        # timeout) and `note` says what verification saw.
...
def main():
    ...
    if args.csharp_only:
        ...
        # 2026-09-26: Interleave one C# task, then up to three Python tasks.
        # `run_level` takes the first C entries of the pool, so a small C still
        # gets C# tasks.
        pool, i, j = [], 0, 0

...
//! 2026-09-25: Kernel-level oracle for the GDN write-on-accept K=4 pair, on the
//! qwen3.8-27b/nvfp4 kernel set.
//!
//! Owner: model-arch examples.
//! Invariants:
//! - Exit 1 on any mismatch below; each check prints its mismatch count and max ULP delta.
//!
//! One GDN layer, random H / q / k / v / gate / beta over `SEQS` sequences. Runs the
//! parent `gated_delta_rule_wy4` (table form) and the twin `gated_delta_rule_wy4_woa` +
//! `gated_delta_rule_wy4_fold`, then checks:
//!
//! * `output` bit-equal between parent and twin;
//! * the twin leaves every H at its initial value and sets the engaged word to 1;
//! * for na in 1..=4: H after the fold bit-equal to the parent's Hi(na-1)
//!   (na < 4) or its final H (na == 4);
//! * with the engaged word cleared (the parent ran): the fold restores H = Hi(na-1) and
//!   is a no-op at na == 4.
//!
...
const KD: usize = 128;
const VD: usize = 128;
/// 2026-09-25: Pointer entries per table, equal to
...
/// 2026-09-25: Deterministic LCG in [-1, 1).
struct Rng(u64);
impl Rng {
...
/// 2026-09-25: (mismatching elements, max ULP distance) between two f32 buffers.
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: Inputs, the same bytes for both arms.
    ...
    for r in 0..rows {
        for h in 0..nv {
            gb[r * gb_stride + h] = 0.5 + 0.49 * rng.next_f32();
            gb[r * gb_stride + nv + h] = 0.5 + 0.5 * rng.next_f32().abs();
        }
    }
    ...
    // 2026-09-25: Parent arm: H and Hi0..Hi2 per sequence, in table form.
    ...
    // 2026-09-25: Twin arm: one contiguous table buffer [h | Hi0 | Hi1 | Hi2] of
    // SLAB_ENTRIES pointers each, the layout of the verify tables (table 0 the states,
    // table t + 1 intermediate t), so the fold's `hi_tables` indexing is exercised.
    ...
    // 2026-09-25: Twin: output must be byte-identical to the parent.
    ...
    // 2026-09-25: The twin writes no state: H must still be the initial value.
    ...
    // 2026-09-25: Fold at na = 1..=4 against the parent's Hi(na-1) or final H.
    for na in 1..=4u32 {
        reset_h(g)?;
        set_na(g, na)?;
        run_fold(g)?;
    ...
    // 2026-09-25: Engaged word 0 (the parent ran): the fold must restore H = Hi(na-1),
    // and do nothing at na == 4.
    ops::gdn_wy4_flag_clear(g, clear, flag, stream)?;
    g.synchronize(stream)?;
    for na in 1..=4u32 {
        // 2026-09-25: Fill H with 0x7f bytes so a no-op is visible.
        for p in &h_w {

...
# 2026-09-26: Temperature amplification of the FP8 drift: final-token logits of both engines through the same BF16 head.
#
...
Metrale-FP8 last-layer residual dumps using the SAME BF16 final-norm + lm_head.
...
DUMP = pathlib.Path("/workspace/metrale-dumps/fp8native_dgx2")
HID = 2048
EPS = 1e-6
...
def eff_support(p):  # 2026-09-26: inverse participation ratio, exp(collision entropy)
...
def main():
    ...
    h_a = load_hidden(DUMP/"metrale_L39.bin")
    cos = float(np.dot(h_v,h_a)/(np.linalg.norm(h_v)*np.linalg.norm(h_a)))
    print(f"residual cos(vllm,metrale) L39 = {cos:.5f}")
    ...
    print(f"argmax vllm={av} metrale={aa} agree={av==aa}")
    ...
    out = {"residual_cos": cos, "logit_cos": logit_cos,
           "argmax_agree": av==aa, "argmax_vllm": av, "argmax_metrale": aa,
    ...
    for T in (1.0, 0.6, 0.3, 0.1):
        ...
        t = tvd(pv, pa)
        klva = kl(pv, pa)
        # 2026-09-26: Metrale Engine's probability on vLLM's top-1 token.
        p_agree = float(pa[av])
        p_v_top1 = float(pv[av])
        # 2026-09-26: Probability that one independent draw from each differs:
        # 1 - sum_i pv_i * pa_i.
        p_diff = float(1.0 - np.sum(pv*pa))
        esv = eff_support(pv); esa = eff_support(pa)
        out["per_T"][str(T)] = {
            "tvd": float(t), "kl_v_a": klva,
            "p_vllm_top1": p_v_top1, "p_metrale_on_vllm_top1": p_agree,
            "p_divergent_draw": p_diff,
            "eff_support_vllm": float(esv), "eff_support_metrale": float(esa),
        }
        print(f"\n--- T={T} ---")
        print(f"  TVD(pv,pa)            = {t:.5f}")
        print(f"  KL(pv||pa)            = {klva:.5f} nats")
        print(f"  p_vllm(top1)          = {p_v_top1:.4f}")
        print(f"  p_metrale(vllm_top1)    = {p_agree:.4f}")
        print(f"  P(divergent draw)     = {p_diff:.5f}")
        print(f"  eff_support vllm/metrale= {esv:.1f} / {esa:.1f}")

    # 2026-09-26: With per-step divergence probability d, P(at least one in N
    # steps) = 1 - (1 - d)^N.

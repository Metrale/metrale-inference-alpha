...
/// `METRALE_QWEN4EXP_NO_HC_GEMM=1`: revert the large-T collapse to the fused
...
/// `METRALE_HC_DECODE_SPLIT=1`: keep the pre-cuBLASLt split path for
...
pub fn hc_pre_lowrank(
    ...
    if num_tokens <= 64 && !scratch.is_null() {
        ...
        // same cure. METRALE_HC_DECODE_SPLIT=1 keeps the split path (A/B).

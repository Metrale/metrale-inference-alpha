...
//! 2026-09-26: Tests for `BenchState`: the form, the start gates, run recording and the followed target.
//!
//! Owner: server tui.
...
/// 2026-09-26: A state with the first benchmark selected and no executor, so nothing can start.
fn state() -> BenchState {
...
fn every_registered_benchmark_can_be_selected() {
    let mut s = state();
    for i in 0..metrale_bench::registry::all().len() {
        s.select(i);
...
fn a_benchmark_without_side_effects_does_not_ask() {
    ...
    s.select(index);
    s.view = View::Params;
    // 2026-09-26: No executor, so the start is refused, and without opening the confirmation prompt.
    s.on_key(key(KeyCode::Char('s')), BenchSub::Suite);
    assert!(!s.confirm_open);
}
...
/// 2026-09-26: A run the dashboard records is found by `history::load_all`, the reader the CLI's
/// history command uses, with its parameters, its target and `source: Tui`.
#[test]
fn a_run_persisted_by_the_dashboard_is_readable_by_the_cli() {
    let dir = tempfile::tempdir().expect("scratch dir");
    let runtime = tokio::runtime::Runtime::new().expect("runtime");
    let store = metrale_bench::ArtifactStore::with_root(dir.path());
    let executor = metrale_bench::BenchmarkExecutor::new(runtime.handle().clone(), store.clone());
    ...
    let frame = BenchmarkResult {
        ...
        summary: Vec::new(),
        table: None,
        verdict: Some(metrale_bench::Verdict::pass("fine")),
    ...
    let found = metrale_bench::history::load_all(&store);
    ...
    assert_eq!(r.metrale_version, crate::cli::METRALE_VERSION);
    assert_eq!(r.target(), TargetEndpoint::local(9001, "cross-path-model"));
    assert!(!r.is_legacy(), "written in the current schema");
    // 2026-09-26: Every parameter, not just the ones a user touched.
    assert_eq!(
        r.params.len(),
...
/// 2026-09-26: A second terminal frame does not add a second record.
#[test]
fn a_run_is_recorded_once_even_if_the_terminal_frame_repeats() {
    ...
    s.persist(&frame);
    s.persist(&frame);
    assert_eq!(metrale_bench::history::load_all(&store).len(), 1);
}

#[test]
fn the_target_follows_the_model_that_is_actually_serving() {
    let mut s = BenchState {
        target: metrale_bench::TargetEndpoint::local(8888, String::new()),
        ..BenchState::default()
    };
...
fn a_target_the_operator_typed_is_left_alone() {
    let mut s = BenchState {
        target: metrale_bench::TargetEndpoint::local(8888, "org/mine".to_string()),
...
fn an_empty_live_model_does_not_blank_the_target() {
    let mut s = BenchState {
        target: metrale_bench::TargetEndpoint::local(8888, "org/a".to_string()),
        ..BenchState::default()
    };
    s.follow_live_model("");

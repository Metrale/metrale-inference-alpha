...
How Metrale Engine decides which expensive GPU benchmarks a pull request must pass,
...
Metrale Engine's correctness and performance gates run on real GPUs and cost real
time — the two BFCL accuracy legs are ~3.5 GPU-hours *each*
(`crates/metrale-plugin/src/benchmarks/bfcl/descriptors.rs:34`). CI cannot run
...
`crates/metrale-plugin/src/gate/mod.rs:81`: **over-broad costs a re-run;
...
flowchart TD
    ...
    A --> C["CI: pr-benchmark-gate\nmet benchmark --pull-request-gate-check"]
...
```mermaid
flowchart LR
    subgraph plugin ["crates/metrale-plugin/src/gate/"]
    ...
    subgraph gov ["crates/metrale-governance"]
    ...
    CLI["crates/server/src/cli/bench_run.rs\nmet benchmark --pull-request-gate-check"]
    ACL["crates/metrale-closure\nhash shared by build.rs and the gate"]
...
|---|---|
| Which paths invalidate which gate | `crates/metrale-plugin/src/gate/coverage.rs` |
| Which kernel targets a path reaches | `crates/metrale-plugin/src/gate/taxon.rs` |
| Whether device code actually changed | `crates/metrale-plugin/src/gate/closure.rs` + `crates/metrale-closure` |
| Record and threshold schemas | `crates/metrale-plugin/src/gate/record.rs`, `gate/bench.rs` |
| The verdict | `crates/metrale-plugin/src/gate/check.rs` |
| The CLI entry point | `crates/server/src/cli/bench_run.rs` (`gate_check_cmd`) |
...
| The journey ledger | `crates/metrale-governance` |
| Artifact → ledger validation | `crates/metrale-plugin/src/bin/ledger_harvest.rs` |
| The cross-PR/debt view | `gate/telemetry.rs`, `bin/pr_telemetry.rs`, `.github/workflows/pr-telemetry.yml` |
| Benchmark registration | `crates/metrale-plugin/src/registry.rs` |
...
| `closure.rs`, `taxon.rs`, `metrale-closure`, ADR-0012 | yes | — |
...
| `metrale-governance` (events, ledger, G-set) | yes — with **zero writers** | first writer (`ledger_append` in CI) |
...
(`crates/metrale-plugin/src/registry.rs:14`). The table is ordered cheapest
...
| `concurrency-sweep-moe` | **required** (promoted 2026-09-23) | ~10–15 min | the same driver on the 35B MoE flagship at the published instrument (ISL 128 / OSL 1024 essay, C=1..16) — the only gate that reaches the grouped-GEMM expert arm; floors are Metrale Engine's own 2026-09-23 curve, not vLLM parity |
...
(`crates/metrale-plugin/src/gate/mod.rs`), derived element-by-element from
...
The boundary is `PERF_PATHS` (`gate/coverage.rs:55`):
...
One entry carries a lesson worth knowing:
...
- **`GATE_MACHINERY`** (`gate/coverage.rs:160`): the whole
  `crates/metrale-plugin/src/gate` prefix is excluded from every gate,
...
`metrale-kernels/build.rs::resolve_targets()` — build-script code is not
...
crate — `crates/metrale-closure` — computes it for both the build script and
...
values *baked into the measuring binary* (`metrale_kernels::TARGET_CLOSURES`,
...
| `hardware` | the box that *served* the run (fetched from the endpoint's `/hardware`, not probed locally): `gpu`, `driver`, `sm_clock_mhz` (`record.rs:237-241`, `crates/metrale-plugin/src/hardware.rs:31`) |
...
- **A dirty tree is refused, not skipped.** `record_covers` proves nothing
  ...
  (`crates/server/src/cli/bench_run.rs:179-212`). `dirty_paths` is a
...
sequenceDiagram
    ...
    Op->>Op: met benchmark run <id> --pull-request-gate (serves by pinned recipe)
...
Plainly: `met benchmark --pull-request-gate-check` answers, in seconds and
...
gate check for 0ff3d56d44 (/workspace/.wt-pr389)
  ...
  NONE  bfcl-subset — latest record is for 8b7de2638d (2026-08-09-8b7de2638d.json)
        ...
        crates/metrale-core/src/fault.rs and 35 more — device code changed for
...
- **Coverage is by content, never ancestry** (ADR-0013,
  ...
  `gate/check.rs:205-241`). Metrale Engine squash-merges, so a record written on a PR
...
under `crates/server/` touches no kernels, so the closure hash excuses
...
nothing (`crates/server/src/cli/bench_run.rs:107-164`). The CI gate
...
(`crates/metrale-governance/src/lib.rs:10-24`, `ledger.rs:100-159` — the
...
- **Events** (`crates/metrale-governance/src/event.rs:28-58`):
...
1. The job appends the Category line locally (`ledger_append`,
   `crates/metrale-plugin/src/bin/ledger_append.rs`) and uploads
...
2. A scheduled job running **default-branch code** downloads the artifact
   ...
   (`crates/metrale-plugin/src/bin/ledger_harvest.rs`), which treats the

...
fn the_candidate_is_owed_for_its_own_driver_and_not_for_other_drivers() {
    let owed =
        coverage::promotion_debt(["crates/metrale-plugin/src/benchmarks/contamination/driver.rs"]);
    assert_eq!(owed, ["cross-contamination"]);
    assert!(
        coverage::promotion_debt(["crates/metrale-plugin/src/benchmarks/ttft/descriptors.rs"])
            .is_empty(),
...
fn the_promoted_moe_gate_invalidates_where_it_used_to_accrue_debt() {
    ...
    for path in [
        "crates/metrale-plugin/src/benchmarks/bfcl/mod.rs",
        "crates/metrale-plugin/src/benchmarks/ttft/descriptors.rs",
        "crates/metrale-plugin/src/benchmarks/agentic/mod.rs",
    ] {
        assert!(
...
fn the_promoted_gates_invalidate_where_they_used_to_accrue_debt() {
    ...
    for path in [
        ...
        "crates/metrale-plugin/src/benchmarks/decode_floor/mod.rs",
    ] {
        let hit = coverage::invalidated_by([path]);
        assert!(

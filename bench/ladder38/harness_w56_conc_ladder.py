...
# crates/metrale-plugin/src/benchmarks/stats.rs. Its comment states the reason and
# this run confirmed it the hard way: UNIFORM repetition ("The quick brown fox…"
# over and over, the corpus bench-metrale-concurrency.py uses) drives the model
# into degenerate repetitive output. On Metrale Engine that trips the SimHash
...
async def one_request(session, url, model, prompt, osl):
    payload = {
        ...
        # Metrale Engine's MODEL.toml non_thinking preset injects presence_penalty=1.5 when
        # the request omits it; vLLM defaults to 0. That is not a like-for-like
        # comparison — Metrale Engine was doing extra per-token logit work AND emitting
        # different text. Sending these explicitly pins both engines to identical
        # sampling. (Measured worth to Metrale Engine at C=8: +7.8%, because the penalty

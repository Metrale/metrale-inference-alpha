...
* **The documented prefill stack for this family is GDN, not CUTLASS.** The
  ...
  `METRALE_FFN_NVFP4_MMQ=1`, `METRALE_GDN_REGRESIDENT=1`, `METRALE_GDN_FLASHINFER=1`
  and `METRALE_GDN_LIB=…/libmetralegdn.so`. FlashInfer-GDN is worth +17-20% there,
  and it FAILS OPEN — without `METRALE_GDN_LIB` and the CUTLASS DSL on
...
* GDN — `METRALE_GDN_FLASHINFER=1`, `METRALE_GDN_REGRESIDENT=1`, `METRALE_GDN_LIB=…/libmetralegdn.so`,
  and the CUTLASS DSL lib dir on `LD_LIBRARY_PATH`. **Verified engaged** from the
  boot log — `METRALE_GDN_FLASHINFER: FlashInfer GDN kernel loaded (opt-in)` — not

...
//! 2026-09-26: Tests for the two-phase report: what it holds, and what it refuses
//! to claim.
//!
...
/// 2026-09-26: A report that never closed is unmeasured, not invalid.
#[test]
...
/// 2026-09-26: The report is stored in the gate record
/// (`.benchmarks/<id>/<date>-<sha>.json`), so it survives a JSON round trip
/// verbatim: before, after, delta and both verdicts.
#[test]
fn the_report_round_trips_through_the_record_json() {
    ...
    assert_eq!(r, back);
    // 2026-09-26: The field names appear in the JSON.
    for key in [
        "before",
        "after",
        "delta",

...
//!
//! `METRALE_MTP_TIMING=1` arms per-phase accumulators across the verify step:
...
//!
//! `METRALE_MTP_GATE_FORCE=1` (diagnostic companion, wired in `scheduler::mod`)
...
pub struct RunTiming {
    /// Armed by `METRALE_MTP_TIMING=1`. When false every `record` is a
...
// `METRALE_MTP_GATE_FORCE` is now `SchedLevers::mtp_gate_force`, read from
...
///
/// Costs one `Instant::now()` when `METRALE_MTP_TIMING` is unset, since

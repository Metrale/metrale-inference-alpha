...
impl Qwen3SsmLayer {
    ...
    /// `METRALE_GDN_WYN=0` kill-switch is set — all of which keep the caller
    ...
    pub(super) fn wyn_kernel(&self, num_tokens: usize, wyn_enabled: bool) -> Option<KernelHandle> {
        ...
        // METRALE_SSM_H_FP16: the FP16 twin is the ONLY correct kernel over an
    ...
    /// `None` when out of range / module absent / `METRALE_GDN_WYN=0` — the
    ...
    ///
    /// Kill-switch `METRALE_GDN_FUSED_CONV17=0` restores the per-token conv

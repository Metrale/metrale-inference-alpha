...
export METRALE_FORCE_GLOBAL_GDN=1     # route GDN prefill to the global-mem kernel (RDNA3.5 64KB LDS cap)
export METRALE_W4A16_VARIANT=v1       # use the BF16-MMA NVFP4 GEMM (SCALE FP8-MMA encode is broken on gfx1151)
export METRALE_NO_FP8_PREDEQUANT=1    # skip NVFP4->FP8 predequant (same broken-encode reason)
...
`crates/metrale-kernels/build_target.rs` already abstracts the compiler behind
...
code into the binary, auto-registered, launched by C++ symbol. **Two paths:**
  1. **SCALE-native (lower risk):** AMD build compiles kernels into the
     ...
     instead of `cuModuleLoadData`(blob). Bigger metrale-core/spark-runtime
...
export METRALE_FORCE_GLOBAL_GDN=1 METRALE_W4A16_VARIANT=v1 METRALE_NO_FP8_PREDEQUANT=1
export LD_LIBRARY_PATH="$SCALE_HOME/targets/gfx1151/lib:$SCALE_HOME/lib"
target/release/met serve Qwen/Qwen3.6-27B-FP8 \
  --port 8081 --max-seq-len 4096 --gpu-memory-utilization 0.70 \
  --kv-cache-dtype bf16 --kv-high-precision-layers max --max-batch-size 4

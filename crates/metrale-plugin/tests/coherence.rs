...
use metrale_plugin::coherence::{self, CoherencePolicy};
use metrale_plugin::plugin::TargetEndpoint;
...
impl metrale_plugin::headless::RunReporter for WarningReporter {
    fn event(&mut self, event: &metrale_plugin::PluginEvent) {
        if let metrale_plugin::PluginEvent::Log(line) = event
            && line.level == metrale_plugin::LogLevel::Warn
        {
...
async fn a_failed_probe_warns_but_still_runs_the_benchmark() {
    use metrale_plugin::headless::{HeadlessOptions, RunRequest, run_blocking};
    use metrale_plugin::{ArtifactStore, BenchmarkExecutor, ParamValues, registry};
...
///
/// This is the bug that made the model check silently useless: Metrale Engine replies
...
async fn the_model_list_survives_chunked_framing() {
    let mock =
        mock_endpoint::start_saying(Some("4 Paris".into()), 1, Duration::ZERO, Duration::ZERO)
            .await;
    let models = metrale_plugin::http::list_models(&target(mock.port), Duration::from_secs(5))
        .await

...
C: Metrale Engine[FP8-native] vs HF[FP8->BF16]    -> Metrale Engine compute drift

Inputs (all in /workspace/metrale-dumps/fp8native_dgx2/):
  metrale_L{0..39}.bin     - Metrale Engine FP8-native dump (today, 2026-05-25)
...
If hf_bf16_*.bin is missing, runs only the C comparison (Metrale Engine vs HF[FP8->BF16]).
"""
...
def main():
    ...
    have_metrale = all((OUT / f"metrale_L{i}.bin").exists() for i in range(N_LAYERS))
    print(f"have_metrale={have_metrale} have_hf_fp8dq={have_fp8dq} have_hf_bf16={have_bf16}")

    rows = []
    A, B, C = [], [], []
    for i in range(N_LAYERS):
        try:
            metrale = load(OUT / f"metrale_L{i}.bin")
        except Exception:
            metrale = None
        try:
        ...
        if hf_fp8 is not None and hf_bf16 is not None:
            ...
            A.append(row["A"]["cos"])
        if metrale is not None and hf_bf16 is not None:
            row["B"] = cmp(metrale, hf_bf16)
            B.append(row["B"]["cos"])
        if metrale is not None and hf_fp8 is not None:
            row["C"] = cmp(metrale, hf_fp8)
            C.append(row["C"]["cos"])
        rows.append(row)
    ...
    sB = stats("B (Metrale Engine[FP8-native] vs HF[BF16-unquant])", B)
    sC = stats("C (Metrale Engine[FP8-native] vs HF[FP8->BF16])", C)

    if sA and sC:
        delta = sA["mean"] - sC["mean"]
        print(f"\nHeadroom (A_mean - C_mean) = {delta:+.6f}")
        if delta < 0.001:
            verdict = "Metrale Engine FP8-native is AT the FP8 ceiling — drift NOT in SSM dispatch."
        elif delta < 0.01:
            verdict = f"Metrale Engine has minor compute headroom of {delta:.4f} below the FP8 ceiling."
        else:
            verdict = f"Metrale Engine has substantial compute headroom ({delta:.4f}) — kernel-level drift remains."

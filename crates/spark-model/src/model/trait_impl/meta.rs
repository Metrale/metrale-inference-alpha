...
impl TransformerModel {
    ...
    pub(super) fn high_speed_swap_dims_dispatch(&self) -> Option<spark_storage::ModelDims> {
        ...
        //
        // KV paging identity (METRALE_KV_PAGING): the SAME config-derived
        // fingerprint the SSM tier uses (quant identity + geometry + the
        // METRALE_MODEL_ID salt), via the KV convention (blob_bytes = 0).
        // Underivable ⇒ None with a loud warn; the flag-ON connect then fails
        // fast with an actionable error unless METRALE_KV_PAGING_NS is set. Every
    ...
    /// Storage dtype of this sequence's SSM h-state (`METRALE_SSM_H_FP16`).
    ///
    ...
    /// Narrow this sequence's SSM h-state to FP16 (`METRALE_SSM_H_FP16`), once.
    ///
    ...
    pub(super) fn alloc_sequence_dispatch(&self, budget_tokens: usize) -> Result<SequenceState> {
        // METRALE_SEQ_MEMTRACE: the opening half of this sequence's memory bracket.
        ...
        // CRITICAL: use Metrale Engine's own stream (not stream 0) because Metrale Engine's stream
        ...
        // METRALE_MTP_DRAFTER_PREFILL: a fresh sequence invalidates the
        ...
        // METRALE_MTP_CARRY_DRAFTER: this sequence's ownership ticket for the

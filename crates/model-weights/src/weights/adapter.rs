...
//! dealloc anywhere in Metrale Engine). Accepted leak at adapter scale (~MBs).
    ...
    for (name, view) in tensors.tensors() {
        ...
        let (bytes, dtype): (Cow<'_, [u8]>, WeightDtype) = match view.dtype() {
            safetensors::Dtype::F16 => {
                ...
                // already a metrale-model-weights dep).
...
// Gated on `feature = "cuda"`: the test constructs a real `MetraleCudaBackend`
...
mod tests {
    ...
    use metrale_gpu_runtime::gpu::GpuBackend; // brings copy_d2h into scope
    ...
    ///
    /// Gated `#[ignore]` (Metrale Engine convention) because `MetraleCudaBackend::new`
    ...
    fn f32_peft_adapter_loads_and_round_trips_as_bf16() {
        ...
        // allow-list lives downstream in metrale-model-layers — but keep them real).
        ...
        // Unique tempdir with no extra dep (metrale-model-weights has no tempfile
        // dev-dep): per-pid + per-thread subdir.
        let dir = std::env::temp_dir().join(format!(
            "metrale_adapter_test_{}_{:?}",
            std::process::id(),
        ...
        let gpu = MetraleCudaBackend::new(0, &metrale_kernels::ptx_modules()).unwrap();

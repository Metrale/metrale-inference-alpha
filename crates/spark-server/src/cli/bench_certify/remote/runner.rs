...
/// is itself a bounded reconnect inside metralectl; ten of them is a link that
/// keeps dying, not a blip.
pub const MAX_REATTACH: u32 = 10;

/// metralectl's `JobKey` bound: `[A-Za-z0-9._-]{1,64}`.
...
impl RemoteRunner {
    ...
    /// driver resumes rather than duplicates. metralectl caps a key at 64
...
impl GateRunner for RemoteRunner {
    fn run(&mut self, unit: &Unit, ctx: &RunCtx, on_line: &mut dyn FnMut(&str)) -> RunOutcome {
        ...
        let facts = metrale_plugin::gate::read_record(&placed.record)
            .ok()

...
#define METRALE_PREFILL_ENTRY inferspark_prefill
#endif
extern "C" __global__ void METRALE_PREFILL_ENTRY(
    ...
    __shared__ __nv_bfloat16 smem_K[METRALE_KBUFN][BC][HDIM_PAD];  // double-buffered
    ...
    for (unsigned int kv_block = kv_block_lo; kv_block < num_kv_blocks; kv_block++) {
        ...
        // K[kv_block] already in smem_K[METRALE_KB(buf)] (preloaded or from prev iteration)

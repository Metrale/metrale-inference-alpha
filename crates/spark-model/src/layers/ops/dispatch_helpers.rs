...
/// documented `METRALE_PREFILL_CODISPATCH` fallback stays reachable.
...
/// legacy `METRALE_PREFILL_CODISPATCH=1`; default OFF).
///
...
/// ways to enable the same path, OR-ed with the older `METRALE_Q12_BATCHED_FIRST_CHUNK`
...
/// the documented `METRALE_PREFILL_VARLEN` fallback stays reachable.
...
/// legacy `METRALE_PREFILL_VARLEN=1`; default OFF).
///
...
/// gated by `METRALE_GEMM_SHAPE_LOG=1`. Used to cross-reference nsys per-call

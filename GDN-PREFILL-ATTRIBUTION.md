...
FLA's `chunk_gated_delta_rule` uses the same three-pass WY decomposition Metrale Engine
...
accumulation** (`tl.dot`), keeping the state in the FP32 accumulator. Metrale Engine does
...
false on every other target, with `METRALE_GDN_PREFILL_TC` overriding either
...
`METRALE_GDN_PREFILL_TC=0` means OFF where it used to mean ON. Every A/B in this
...
`METRALE_GDN_PREFILL_TC` overriding under the 2026-09-11 grammar. It is resolved
...
one direction: `METRALE_GDN_PREFILL_TC=0` is an explicit OFF, and a presence
check would have turned the twins ON for it while the spine stayed off, which
is a prefill that is neither leg of an A/B.

`METRALE_NO_GDN_PREFILL_TC_REMNANTS=1` is the ONE-VARIABLE A/B that separates
them: it keeps the spine and pins `wu`/`fwd_o` to their parents. It is
presence-gated, like the other `METRALE_NO_*` kill switches, and it is documented
...
reference (`crates/model-layers/src/layers/ops/ssm_gdn_remnants*_tests.rs`), and
...
**A** the control (family off), **T2** `METRALE_GDN_PREFILL_TC=1
METRALE_NO_GDN_PREFILL_TC_REMNANTS=1` (spine only), **T1** `METRALE_GDN_PREFILL_TC=1`
...
box** — the first metric in this campaign where Metrale Engine leads.
...
identical recipe, `METRALE_NO_GDN_PREFILL_TC_REMNANTS` the only difference:

...
The kernel set for `deepseek-v4-flash` and, through its `kernel_source`, `deepseek-v4.1-flash` on datacentre Blackwell (B200 / GB200, SM 10.0). The kernel set is `kernels/gb10/deepseek-v4-flash/nvfp4/` and `kernels/gb10/common/`, read through `kernels/b200/HARDWARE.toml [hardware] inherits = "gb10"` — one source, two targets, the mechanism every b200 model in this tree uses (b200 holds no per-model kernel of its own).
...
| `metrale_q2_k_mmq128_{nc,wc}` | 255 / 255 | the prefill MMQ tiles, at the register ceiling on both parts, no spills |
| `metrale_q3_k_mmq128_{nc,wc}` | 230-234 / 254 | |

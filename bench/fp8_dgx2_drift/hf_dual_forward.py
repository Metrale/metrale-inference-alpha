...
# 2026-09-26: CPU forward of the HF BF16 and FP8-dequantised checkpoints; writes per-layer last-token hidden states.
#
# Owner: bench, FP8 drift investigation.
# Invariants: none beyond the types.
"""Run HF[BF16-unquant] AND HF[FP8->BF16] forward on a freshly-dumped Metrale Engine
prompt-token sequence, emitting per-layer hidden states for both.

Output layout:
  /workspace/metrale-dumps/fp8native_dgx2/hf_bf16_L{0..39}.bin
  /workspace/metrale-dumps/fp8native_dgx2/hf_fp8dq_L{0..39}.bin

Inputs:
  /tmp/metrale_tokens_dgx2.json  — tokens dumped by Metrale Engine via METRALE_DFLASH_DEBUG_DUMP_FULL=1
...
TOKENS_PATH = pathlib.Path("/tmp/metrale_tokens_dgx2.json")

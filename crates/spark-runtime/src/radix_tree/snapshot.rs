...
pub(super) struct SnapshotEntry {
    ...
    /// `false` when `METRALE_SSM_TIER` is off, so the default path is unchanged.
...
pub(super) struct SsmSnapshotIndex {
    ...
    /// Phase-0 measurement counters (METRALE_SSM_SNAP_STATS). All aggregate,
...
/// `METRALE_DISABLE_SSM_TAIL_PROTECT=1` (or `on`/`true`). Renamed 2026-08-05
/// from the opt-in `METRALE_SSM_TAIL_PROTECT` (=0/off disabled): the lease is
...
/// `METRALE_SSM_TAIL_MIDCHUNK=0` — which the frozen MLPerf-edge config sets.
...
/// variable nor `METRALE_SSM_TAIL_MIDCHUNK` is still running with the lease
/// armed; check `METRALE_SSM_TAIL_MIDCHUNK` first when asking whether tail
...
/// that the TTL almost never binds. Override: METRALE_SSM_TAIL_LEASE_TTL.
///
/// Same caveat as [`tail_lease_enabled`]: with `METRALE_SSM_TAIL_MIDCHUNK=0` no
/// entry is ever marked `is_tail`, so this TTL governs an empty set and
/// `METRALE_SSM_TAIL_LEASE_TTL=128` in a launch script changes nothing.
...
/// Default flip requires its own measured A/B: METRALE_SNAP_EVICT_ALPHA.
...
impl SsmSnapshotIndex {
    ...
    pub(super) fn lookup(
        ...
        for (i, entry) in self.entries.iter().enumerate() {
            ...
            // `lookup_tiered`. (No entry is ever tiered when METRALE_SSM_TIER off.)
            if entry.tiered {
                continue;
            }
    ...
    /// `METRALE_SSM_SNAP_STATS` is set. Off-by-default and read-only, so it never
    ...
    pub(super) fn evict_lru(&mut self) -> Option<usize> {
        ...
        // SESSION-AWARE eviction (default ON; METRALE_SNAP_EVICT_LEGACY=1 → old per-entry).

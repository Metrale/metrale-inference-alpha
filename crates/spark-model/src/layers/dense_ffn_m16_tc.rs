...
//! `w8a16_gemm_m16`, behind `METRALE_FFN_M16_TC` (#927).
//!
...
//! Round 6's serving A/B on 1xH100 (2026-09-11, bs16, `METRALE_MS_PROFILE`) with
...
//! | variable | turns on |
//! |---|---|
//! | `METRALE_ATTN_M16_TC` | the QKV and o_proj tiers |
//! | `METRALE_FFN_M16_TC` | the dense-FFN arm (rungs 2-3 above) |
//! | `METRALE_M16_TC` | BOTH — the umbrella, i.e. round 6's behaviour |
//!
//! ⚠ `METRALE_FFN_M16_TC=1` MEANS SOMETHING NARROWER THAN IT DID IN ROUND 6.
...
//! reproduces round 6 is now `METRALE_M16_TC=1`. The recipe that buys the
//! attention win WITHOUT the FFN loss — the point of the split — is
//! `METRALE_ATTN_M16_TC=1` alone.
//!
...
//! N=17408.
//!
//! Both hypotheses predict the same fix, which is why `METRALE_FFN_M16_TC_NTILE`
...
//! `METRALE_FFN_M16_TC=1 METRALE_FFN_M16_TC_NTILE=64` against
//! `METRALE_FFN_M16_TC=1` on the same serve.
//!
...
pub struct M16TcLevers {
    /// The dense-FFN arm (`METRALE_FFN_M16_TC`).
    pub ffn: bool,
    /// The multi-seq FP8 QKV tier and the FP8 o_proj tier
    /// (`METRALE_ATTN_M16_TC`).
    pub attn: bool,
    /// CTA N width for the FFN arm: 32 (default) or 64
    /// (`METRALE_FFN_M16_TC_NTILE=64`). Attention always runs 32 — the wide tile
...
/// on" trap, the same contract `METRALE_FFN_NO_BATCH16` uses next door. All three
...
///
/// An unrecognised `METRALE_FFN_M16_TC_NTILE` falls back to 32 rather than
...
pub fn m16_tc_levers() -> M16TcLevers {
    ...
    *ON.get_or_init(|| {
        ...
        resolve_m16_tc_levers(
            ...
            // empty environment, and `METRALE_FFN_M16_TC` / `METRALE_M16_TC`
...
/// must DECLINE rather than launch and be wrong. Every Metrale Engine FP8 FFN shape
...
/// `METRALE_FFN_M16_TC_NTILE=64` must fall back to the 32-wide kernel rather than

...
//
// cuBLASLt stub for the HIP target. met links `-lcublasLt` unconditionally,
// but the cuBLASLt GEMM path is opt-in (METRALE_CUBLAS_GEMM=1) — the default
// runtime uses Metrale Engine's own hand-written GEMMs. There is no hipBLASLt mapping
// here yet, so every entry point returns a non-success cuBLASLt status (1 =
// CUBLAS_STATUS_NOT_INITIALIZED). If METRALE_CUBLAS_GEMM is ever set on AMD, the

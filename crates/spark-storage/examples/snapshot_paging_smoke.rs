...
// metrale-cache-peer started with --swap-dir. PUTs far more blobs than the RAM
...
//
//   METRALE_SNAP_PEER=host:port \
//   METRALE_EXPERT_RDMA_DEV=roceP2p1s0f1 METRALE_EXPERT_RDMA_GID=3 \
...
// the peer: `metrale-cache-peer --listen 0.0.0.0:9918 --swap-dir /some/nvme/dir`).
...
/// into the decode namespace (`METRALE_SSM_DECODE_CLIENT_ID` in the model serve)
...
fn decode_isolation_main(addr: &str, blob: usize, slots: usize, n: u64) -> anyhow::Result<()> {
    ...
    // `metrale_kernels::DECODE_DOMAIN` (spark-storage has no metrale-kernels dep);

...
//!
//! Now the fallback is [`metrale_kernels::TARGET_DEFAULTS`], baked by
...
//!
//! The legacy `METRALE_NO_*` kill switches stay PRESENCE-gated and still force
//! their lever OFF, so no script that predates this file changes meaning.
//! `METRALE_NO_DECODE_SPLIT_SILU` is the one in this table.
//!
...
//! `metrale_kernels::TargetDefaults`, the parse arm in
//! `metrale-kernels/build_defaults.rs`, the row in EVERY
...
pub enum Source {
    ...
    Target,
    /// An `METRALE_*` variable in the process environment.
    Env,
}
...
pub fn resolve(
    ...
    TargetLevers {
        ...
        // `METRALE_SSM_BATCHED_RECURRENT` was `== "1"` in `gdn_flags::from_env`;
        ...
        // ⚠️ `METRALE_GDN_PREFILL_TC` was PRESENCE-gated and is now grammar-gated
        ...
        // `METRALE_NO_*` legacy spelling — `METRALE_SSM_BA_GATES_HOPPER=0` is the
        ...
        // so the row carries no accuracy question and no `METRALE_NO_*` legacy
        ...
        // a launch. `METRALE_FP8_ACT_QUANT_HOPPER=0` declines the twin at EVERY
        ...
        // `decode_split_silu` never had one. `METRALE_NO_DECODE_SPLIT_SILU`
        ...
        // `metrale_kernels::attn_splitk::resolve_policy`, not a fourth copy of
        ...
        // serve. `METRALE_M16_TC` is the round-6 umbrella that arms both; it is
        ...
        // NOT under `METRALE_M16_TC`. The umbrella is round 6's, which predates
        ...
        // `METRALE_NO_ATTN_DECODE_BATCH` is the pre-existing kill switch for the
        ...
        // DECLARATION plus `METRALE_FFN_GATEUP_FUSED`, which is the A/B a Hopper
...
pub fn resolved() -> &'static TargetLevers {
    ...
    LEVERS.get_or_init(|| {
        resolve(&metrale_kernels::TARGET_DEFAULTS, |name| {
            std::env::var(name).ok()
        })
    })
}
...
/// `metrale-kernels/build_defaults.rs::baseline` by

...
/// lives in the separate `metrale-recipes` repo and is honoured verbatim. Editing
...
/// requires a PR to `metrale-recipes` adding `mtp_gate: force` to that recipe.
...
/// temperature 0 on Metrale Engine today, so a throughput-timed path switch makes greedy
...
/// must be bit-identical to plain greedy. Metrale Engine violates it because restoring

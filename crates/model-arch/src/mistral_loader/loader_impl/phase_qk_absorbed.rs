...
//! 2026-09-25: Load step: W_QK_absorbed `[n_kv * kv_lora, q_lora]`, the product of wq_b's K rows and W_UK_T, computed in FP32 on the host and uploaded as BF16.
//!
//! Owner: model-arch (Mistral loader).
//! Invariants:
//! - The FP32 result does not depend on the thread count (see `absorb_rows`).
//! - The context is written only after the upload succeeded.
...
/// 2026-09-25: BF16 bit pattern to FP32, exactly: BF16 is the high half of an FP32.
#[inline]
fn bf16_to_f32(bits: u16) -> f32 {
    f32::from_bits((bits as u32) << 16)
}

/// 2026-09-25: Widen a little-endian BF16 byte buffer to FP32, so the
/// absorption loop reads plain FP32 slices.
...
/// 2026-09-25: Worker threads for the absorption GEMM:
/// `METRALE_MLA_ABSORB_THREADS` when it parses as a positive integer,
/// otherwise the machine's available parallelism, clamped to `1..=rows`.
/// `1` runs it sequentially; the count does not change the result (see
/// `absorb_rows`).
...
/// 2026-09-25: Compute one contiguous span of W_QK rows.
///
/// `out` is `[rows_here, q_lora]`, covering absorbed rows
/// `row0 .. row0 + rows_here`, where row `r` is head `r / kv_lora`, latent
/// index `r % kv_lora`.
///
/// The accumulation order is what keeps the result exact. The reference form is
///
/// ```text
...
/// bit-identical to the reference, as `absorb_rows_is_bit_exact_vs_reference`
/// asserts.
...
/// 2026-09-25: W_QK_absorbed as FP32 `[n_kv * kv_lora, q_lora]`, row-parallel.
///
...
pub(crate) fn build_w_qk_absorbed(ctx: &mut MistralLayerCtx<'_>) -> Result<()> {
    ...
    let wqk_ptr = gpu_alloc_or_managed(gpu, wqk_size)?;
    {
        ...
        // 2026-09-25: FP32 to BF16 by truncation, not rounding: the high 16
        // bits of each value, little-endian.
        let t = std::time::Instant::now();
...
mod tests {
    ...
    /// 2026-09-25: The bit-exactness oracle: a scalar loop over `(n, lkv, l)`
    /// that sums ascending `p` and decodes the BF16 operands inside the loop.
    #[allow(clippy::too_many_arguments)]
    fn reference(
        wqb_buf: &[u8],
        wuk_buf: &[u8],
    ...
    /// 2026-09-25: Deterministic xorshift64* generator.
    ...
    /// 2026-09-25: BF16 bit patterns with both signs, about one exact zero in
    /// 256, and exponents inside the normal range, so the tests exercise
    /// accumulation order, not denormal handling.
    fn bf16_bytes(n: usize, seed: u64) -> Vec<u8> {
        ...
        for _ in 0..n {
            ...
            // 2026-09-25: Exponent 0x68..=0x87: magnitudes from 2^-23 to just under 2^9.
    ...
    /// 2026-09-25: `absorb_rows`, split over any number of threads, is
    ...
    fn absorb_rows_is_bit_exact_vs_reference() {
        for &(n_kv, kv_lora, q_lora, nope, hd) in &[
            ...
            // 2026-09-25: The thread counts include 1 (sequential) and splits
            // that do not divide the row count.
    ...
    /// 2026-09-25: `absorb_qk`, which the loader calls, with the thread count
    /// from `absorb_threads`, is bit-identical to `reference`.
    #[test]
    ...
    /// 2026-09-25: Wall-clock comparison of `reference` and `absorb_qk`, and of
    /// two BF16 packings, at the LongCat Flash Lite shape. CPU only: no GPU, no
    /// model, no checkpoint. Run it with:
    ///
    /// ```text
    /// cargo test --release -p metrale-model-arch --lib -- --ignored --nocapture absorb_qk_bench
    /// ```
    #[test]
    #[ignore = "wall-clock benchmark: seconds per run, not a correctness gate"]
    fn absorb_qk_bench() {
        // 2026-09-25: LongCat Flash Lite: 32 KV heads, kv_lora 512, q_lora
        // 1536, and the loader's padded head dim 256, so nope is 256 - 64 = 192.
    ...
    /// 2026-09-25: The in-place FP32 to BF16 truncation used by
    /// `build_w_qk_absorbed` gives the same bytes as a `flat_map` over
    /// `to_le_bytes().to_vec()`.
    #[test]
    fn bf16_truncate_matches_old_flat_map() {
        // 2026-09-25: Signed zeros, a denormal, infinities, NaN, and bit
        // patterns where rounding would differ from truncation.
    ...
    /// 2026-09-25: `widen_bf16` puts each BF16 in the high half of an FP32.
    #[test]

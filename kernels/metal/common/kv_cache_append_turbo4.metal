...
// 2026-09-25: Append one token's K and V to a contiguous Turbo4 cache at row
// cache_pos. Per group of 16 values: 4-bit indices into a unit-Gaussian
// Lloyd-Max codebook (2 per byte, even element in the low nibble) after
// scaling by TURBO4_MAX / max|x|, and an E4M3 scale that makes the
// reconstruction's L2 norm equal the input's (max|x| / TURBO4_MAX when the
// reconstruction norm is near zero). K and V are quantized as given: this
...
// 2026-09-25: 16-level Lloyd-Max codebook for N(0,1) and its decision boundaries.
...
// half away from zero; the same function as in kv_cache_append_turbo8.metal.

...
// 2026-09-25: Block-scaled FP8 x BF16 GEMV for up to 4 or 16 activation rows.
//
// forked-from: kernels/gb10/common/w8a16_gemv_batch4.cu (2026-09-24; 24 of 303 lines differ, see kernels/FORKS.md)
//
//   C[t, n] = sum_k A[t, k] * E4M3_LUT[B[n, k]] * block_scale[n/128, k/128],  t < M
//
// One pass over the FP8 weights serves all M rows: each weight is dequantized
// once and multiplied into M accumulators. A: [M, K] BF16, B: [N, K] FP8 E4M3,
// block_scale: [N/128, K/128] FP32, C: [M, N] BF16. Entry points:
// w8a16_gemv_batch4 and w8a16_gemv_batch16, and their `_strided` siblings.
//
// Owner: b300 kernels.
// Invariants:
// - blockDim.x == 256 (BLOCK_SIZE; the LUT copy indexes by threadIdx.x) and
//   gridDim.x == ceil(N/4). The host wrappers in model-layers
//   layers/ops/fp8_gemv_batch.rs launch that and refuse M > MAX_M; the kernel
//   never writes rows at or past MAX_M.
// - K is a multiple of 16 and each A row is 16-byte aligned: every step reads
//   16 weights and 16 activations as uint4.
// - Each row adds the same products in the same order as w8a16_gemv (M = 1),
//   one addition at a time, so its output equals w8a16_gemv's for that row.
...
// 2026-09-25: FP8 E4M3 -> f32 lookup table (256 entries), the same values as w8a16_gemv.cu's.
...
// 2026-09-25: MAX_M bounds the accumulator array and the unroll; the runtime M
// (at most MAX_M) is how many rows are computed. `a_row_stride` and
// `c_row_stride` are the row pitches of A and C in elements: the contiguous
// entry points pass K and N, and the strided ones let a caller address rows
// inside a wider buffer.
...
__device__ __forceinline__ void w8a16_gemv_batchm_impl(
    ...
    for (unsigned int k16 = lane; k16 < K16; k16 += threads_per_out) {
        ...
        // 2026-09-25: 16 FP8 weight bytes, dequantized and scaled once for all M rows.
        ...
        // 2026-09-25: The scaled weights serve every row, in w8a16_gemv's k order.
        ...
        for (int t = 0; t < MAX_M; t++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                *(unsigned short*)&hi = (unsigned short)(ar[j] >> 16);
                // 2026-09-25: Two additions, as in w8a16_gemv; `acc += x + y` rounds differently.
    ...
    // 2026-09-25: Cross-warp reduction (threads_per_out = 64, 2 warps per output), per row.
...
// 2026-09-25: Strided siblings: the same body, with caller-given row pitches for A and C.

...
//! 2026-09-26: `config.json` parser for Kimi K3 (`model_type` `kimi_k3`, inner
//! `kimi_linear`), tested against the two configs in `docs/k3/fixtures/`. It reads
//! `text_config` (the top level when absent) and sets `model_type = "kimi_k3"`.
//!
//! `linear_attn_config.{kda_layers,full_attn_layers}` decide the layer map; no index
//! arithmetic is assumed. Both fixtures number the lists from 1 (the largest index equals
//! `num_hidden_layers`).
//!
//! Owner: config (model parsers).
//! Invariants:
//! - In a parse that succeeds, every layer is in exactly one of the two lists, there is at
//!   least one KDA and one MLA layer, and the last layer is MLA (`validate_kimi_k3`).
...
pub fn parse_kimi_k3(json: &str) -> Result<ModelConfig> {
    ...
    // 2026-09-26: The inner object says `kimi_linear`.
...
fn overlay_linear_attn(config: &mut ModelConfig, text: &serde_json::Value) -> Result<()> {
    ...
    // 2026-09-26: The 0.40B twin fixture has no `gate_lower_bound`; the field then keeps its
    // factory 0.0, which `kimi_k3_host::kda_from` maps to "no bound". The full-size fixture
    // sets -5.0.
...
/// 2026-09-26: The 0.40B twin fixture sets `text_config.eos_token_id = 2`, while its tokenizer's
/// `[EOS]` is 163585 (`k3_tokenizer_check.rs`). Both fixtures have a 163840-token vocabulary;
/// the full-size fixture's `eos_token_id` is 163586 and is left alone.
...
/// 2026-09-26: For `kimi_k3` with the K3 vocabulary, remove id 2 from the stop set that
/// `populate_eos_token_ids` rebuilt from the JSON, and keep `eos_token_id` first in it.
/// [`overlay_eos`] has already corrected the primary id when `parse_kimi_k3` ran.
...
/// 2026-09-26: Convert a layer-index list to 0-based indices.
///
/// The caller decides `one_based` from the union of both lists (largest index equals
/// `n_layers`): in the full-size fixture `kda_layers` ends at 91 and `full_attn_layers` at 93,
/// so a per-list test would convert only one of them. 0-based lists pass through. Errors on
/// an empty list, a 0 in a 1-based list, and an index out of range.
fn to_zero_based(
    indices: &[usize],
    n_layers: usize,
    one_based: bool,
...
mod tests {
    ...
    fn parse_kimi_k3_one_based_kda_mla_split() {
        ...
        // 2026-09-26: Config indices 1, 2, 3 become KDA layers 0, 1, 2, and 4 becomes MLA layer 3.
        for i in 0..3 {

...
# tokens. METRALE_GDN_REGRESIDENT is the live case: it is advertised token-equal to
...
#
# Usage: acc_gate.sh <metrale_bin> <outdir> <flag_env_or_NONE> [pct_scale]
#   e.g. acc_gate.sh .../met out/ METRALE_NO_GDN_REGRESIDENT=1 4
#
...
# `METRALE_GDN_REGRESIDENT=1` (which this example used to pass) is read by nothing
...
for leg in control cand; do
  ...
  sudo docker rm -f metrale-acc >/dev/null 2>&1; sleep 3
  # shellcheck disable=SC2086
  sudo docker run -d --name metrale-acc --network host --gpus all --ipc=host \
  ...
  for _ in $(seq 1 180); do
    ...
    sudo docker ps --format '{{.Names}}' | grep -q metrale-acc || { echo "SERVE_DIED leg=$leg"; break; }
    sleep 5
  done
  [ $ok -eq 1 ] || { sudo docker logs metrale-acc 2>&1 | tail -40 > "$OUT/$leg.died.txt"; continue; }

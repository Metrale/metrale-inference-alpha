...
//
// Three small entry points the integrated KDA layer needs and Metrale Engine does not already have.
//
// ★ kda_o_norm_gated_* is NOT a duplicate of `gated_rms_norm_f32_input`.
//   Every Metrale Engine gated RMSNorm applies **SiLU** to the gate (`rms_norm.cu:1087`,
...
//   * `kda_widen_bf16_f32` — Metrale Engine's prefill conv emits BF16; `kda_chunk_*` consume FP32.
...
// De-interleave + widen: Metrale Engine's prefill conv writes ONE BF16 buffer whose channels are

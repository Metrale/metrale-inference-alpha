#!/usr/bin/env python3
"""Metrale Engine Spark coherence & API compatibility test suite.

Tests OpenAI API compatibility for coding agents (OpenCode, Claude Code, Cline, Continue).
Run against a live Metrale Engine server: python3 scripts/test_coherence.py [--url http://localhost:8888]
"""
...
def test_coherence():
    ...
    msgs = [
        {"role": "user", "content": "My name is Metrale Engine."},
    ]
    ...
    test("Multi-turn remembers context (name)", "Metrale Engine" in content2 or "metrale" in content2, content2[:80])
    ...
    for i in range(10):
        r = api("/v1/chat/completions", {
            ...
            "messages": [
                ...
                {"role": "user", "content": "Read /workspace/metrale/Cargo.toml"},
            ],
            "tools": single_tool,
            "max_tokens": 500,
        })
...
def test_opencode_compat():
    ...
    import os
    dump_dir = "/tmp/metrale-opencode-dumps"
...
def test_anthropic_agent_compat():
    ...
    r = anthropic_req({
        "model": "test",
        "system": sys_prompt,
        "messages": [{"role": "user", "content": "Read the file /workspace/metrale/Cargo.toml and tell me the package name"}],
        "tools": tools,
        "max_tokens": 1000,
    })
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Metrale Engine coherence test suite")
    ...
    print(f"Testing Metrale Engine at {URL}")

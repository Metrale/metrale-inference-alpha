...
//! * >= 2 draftless sequences and multi-seq MTP mode (`METRALE_MTP_MAX_SEQS>1`)
...
//!
//! Kill switch `METRALE_NO_MTP_BATCH_BOOTSTRAP` (PRESENCE check per the house
...
pub(super) fn bootstrap_batch_disabled() -> bool {
    ...
    *CACHED.get_or_init(|| std::env::var_os("METRALE_NO_MTP_BATCH_BOOTSTRAP").is_some())
}
...
/// remains reachable under an `METRALE_PROPOSE_META_STRIDE` override or a
...
/// **ON** by default, disabled by PRESENCE of `METRALE_NO_MTP_BOOT_ARGMAX`
/// (house convention — `=0` is NOT off). Read once per process.
fn boot_argmax_batch_enabled() -> bool {
    static CACHED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *CACHED.get_or_init(|| std::env::var_os("METRALE_NO_MTP_BOOT_ARGMAX").is_none())
}
...
pub(super) fn step_mtp_bootstrap_batched(
    ...
    // Kill switch `METRALE_NO_MTP_BOOT_ARGMAX` (PRESENCE).
    let pen: Vec<_> = refs
        .iter()
        .map(|a| {

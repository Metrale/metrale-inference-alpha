...
/// `METRALE_NO_W8A8_DECODE_PROJ` kill switch: PRESENCE (any value, including
/// empty) keeps every 5..16-row decode projection on today's `w8a16_gemv_batch16`
/// tiers. Presence rather than `=1` for the same reason `METRALE_FFN_W8A16_ONLY`
...
/// * `!disabled` — the `METRALE_NO_W8A8_DECODE_PROJ` kill switch.
...
pub fn decode_w8a8_quant_act(
    ...
    if cublas_scale_layout_kmajor() {
        ...
        // Measurement control only (`METRALE_CUBLAS_SCALE_LAYOUT=rowmajor`): the

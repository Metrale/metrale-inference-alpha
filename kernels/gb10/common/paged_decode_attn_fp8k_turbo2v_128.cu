...
// 2026-09-25: Paged decode attention over an FP8 E4M3 K pool and a Turbo2 V pool (2-bit indices
// into a 4-entry codebook, one FP8 E4M3 scale per 16 elements).
//
// Owner: gb10 kernels.
// Invariants:
...
// one k_scale. V block (stride v_block_stride_bytes): token-major head rows of head_dim / 4
...
// 2026-09-25: One lane's VEC_BF16 V elements: 2-bit indices, four per byte from the low
// bits up (two bytes at HDIM=256, one at 128), each times the group's FP8 scale.

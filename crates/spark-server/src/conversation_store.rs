...
//!
//! Metrale Engine stores conversations in-memory with an LRU+TTL bound, mirroring
...
//! reasoning, …) without coupling to Metrale Engine's internal message schema.
...
impl ConversationStore {
    /// # Errors
    /// When `METRALE_CONVERSATION_MAX_ENTRIES` or
    /// `METRALE_CONVERSATION_TTL_SECONDS` is set to something that is not a

...
//! 2026-09-25: The single-token routing on the device (`METRALE_DS41_DEVICE_ROUTE=1`).
//!
//! `moe_v41_route_select` computes the selection with ports of glibc's
//! `expf` / `log1pf` for the softplus and ranks equal keys by the lower expert
//! id, as the host's stable sort does. It writes `weight_dev` / `rows_dev` and
//! the gate / up / down pointer table in plan order from a device slot table,
//! and leaves one header for the host: the miss flag, the picks, the weight
//! bits and the plan's slots. The host reads that header back once, fetches
//! the picks through the cache, uploads the cache's slot changes to the device
//! table, and re-uploads the pointer table from the fetched slots when a pick
//! was missing or a slot moved.
//!
//! `METRALE_DS41_DEVICE_ROUTE_DIAG=1` also runs the host chain on the same
//! logits and checks the picks, the weight bits, the plan weights and the
//! pointer table on every token (a mismatch fails the step).
//!
//! Owner: model-arch (DeepSeek-V4.1 MoE).
//! Invariants:
//! - Outside tests, the selection kernel launches only for a layer that
//!   `device_route_ok` accepted.
...
/// 2026-09-25: The two constants the kernel is written for.
const ROUTE_NR: usize = 384;
const ROUTE_TOPK: usize = 6;

fn device_route() -> bool {
    static V: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *V.get_or_init(|| std::env::var("METRALE_DS41_DEVICE_ROUTE").is_ok_and(|v| v == "1"))
}

fn device_route_diag() -> bool {
    static V: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *V.get_or_init(|| std::env::var("METRALE_DS41_DEVICE_ROUTE_DIAG").is_ok_and(|v| v == "1"))
}

static DIAG_CHECKED: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);

impl MoeV41 {
    /// 2026-09-25: The device bias for a layer's router (`MoeV41LayerWeights::gate_bias_dev`).
    ...
    /// 2026-09-25: Is the device selection on, and does this layer fit it?
    /// A layer that does not fit logs one warning per process.
    pub(super) fn device_route_ok(&self, w: &MoeV41LayerWeights) -> bool {
        ...
        if !fits {
            ...
            ONCE.call_once(|| {
                tracing::warn!(
                    "moe_v41: METRALE_DS41_DEVICE_ROUTE=1 ignored (n_routed {} topk {} layer {}): the kernel is written for {ROUTE_NR} x {ROUTE_TOPK}",
                    c.n_routed,
                    c.topk,
                    w.layer
                );
    ...
    /// 2026-09-25: Words in one layer's header: the miss flag, the picks, the
    /// weight bits, the plan slots.
    pub fn header_words(&self) -> usize {
        1 + 3 * self.cfg.topk
    }

    /// 2026-09-25: Layer `layer`'s header inside `route_hdr`.
    fn header_of(&self, layer: u32) -> DevicePtr {
        DevicePtr(self.route_hdr.0 + (layer as usize * self.header_words() * 4) as u64)
    }

    /// 2026-09-25: The arena facts the selection kernel builds pointers from:
    /// (base, slot bytes, gate / up / down offsets).
    ...
    /// 2026-09-25: The device selection for a replayed step: the kernel
    /// launched with its header in layer `w.layer`'s row, and no read-back
    /// (the caller runs `read_headers` after the segment). The slot table must
    /// already be up to date. Device work only; an error when
    /// `device_route_ok` refuses the layer.
    pub fn route_select_deferred(
        ...
        stream: u64,
            ...
            "moe_v41: the step graph needs METRALE_DS41_DEVICE_ROUTE=1 (layer {})",
            w.layer
        );
        self.route_select_launch(gpu, w.layer, w.gate_bias_dev, arena, stream)
    }

    /// 2026-09-25: Every layer's header in one read-back ordered on `stream`.
    ...
    /// 2026-09-25: `(miss flag, picks, weights)` of layer `layer` out of
    /// `read_headers`; an error when a pick is outside `0..n_routed`.
    ...
    /// 2026-09-25: The selection kernel on `self.logits` (row 0) for layer `layer`, the
    ...
    /// 2026-09-25: Bring the device slot table up to date with the cache's
    /// changes; an error for a triple outside the table.
    pub(crate) fn slot_table_update(
        ...
        let c = &self.cfg;
        // 2026-09-25: One triple per key, the last one: the kernel writes a
        // chunk's triples in parallel, so a key assigned, evicted and assigned
        // again inside one batch would race to a wrong final slot (-1 for a
        // resident expert is a false miss flag).
        ...
        // 2026-09-25: The 12-byte triples are staged through `a_rows`, in
        // chunks that fit it.
        ...
        for chunk in changes.chunks(per) {
            ...
            KernelLaunch::new(gpu, self.k.slot_table_set)
                ...
                .arg_u32(n)
                .launch(stream)?;
            // 2026-09-25: Synchronise before the next chunk overwrites `a_rows`.
            gpu.synchronize(stream)?;
        }
        Ok(())
    }

    /// 2026-09-25: The device selection for one token: the header read back,
    /// the picks fetched through the cache, the slot table and (on a miss or
    /// a moved slot) the pointer table brought up to date. `route_launch` must
    /// have run on `stream`. Returns the routing as `route_select` does.
    pub(super) fn route_device_m1<S: ExpertSource + ?Sized>(
        ...
        // 2026-09-25: The table must know every slot the cache changed so far
        // (prefill, the host-routed layers) before the kernel reads it.
        ...
        // 2026-09-25: The cache: hits touched, misses read (on the device
        // arena, their copies enqueued on `stream` ahead of the experts).
        lru.begin_token();
        ...
        if miss_flag || changed {
            // 2026-09-25: A pick was not resident when the kernel looked, or
            // the fetch moved a slot: rebuild the pointer table from the
            // fetched slots, in plan order, as the host path builds it.
    ...
    /// 2026-09-25: The host chain on the same logits against the device's
    /// header, the plan weights and the pointer table; a mismatch fails the step.
    fn device_route_check(
        ...
        indices: &[usize],
        weights: &[f32],
        slots: &[metrale_model_weights::weights::expert_stream::ExpertSlot],
        stream: u64,
    ) -> Result<()> {
        let c = &self.cfg;
        let k = c.topk;
        ...
        // 2026-09-25: The plan the device wrote: `weight_dev` in ascending
        // expert id, then `ptrs_dev`.

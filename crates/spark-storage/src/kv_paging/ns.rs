...
//! KV paging namespace + wire-key derivation (`METRALE_KV_PAGING`, part of
//! the tiered-cache consolidation).
//!
//! The paging peer (metrale-cache-peer) keys its KV arena purely by the u64 the
...
//! connect; `METRALE_KV_PAGING_SALT` pins it for tests/harness). Consequence,
...
pub(crate) use metrale_tier::hash::{FNV_OFFSET, fnv1a_64};
...
// violation stating itself. One definition, in metrale_tier::hash.
pub(crate) use metrale_tier::hash::mix64;
...
///
/// `model_fp` carries the quant identity + model_type + `METRALE_MODEL_ID`
...
/// `METRALE_KV_PAGING_NS` override (env-free core): strict parse, 0 rejected —
...
/// `METRALE_KV_PAGING_SALT` override (env-free core): strict; `Ok(None)` ⇒ the
...
/// `METRALE_KV_PAGING` selection (env-free core): unset or `0` ⇒ the raw dumb
...
/// `METRALE_KV_PAGING_ARENA_GB` (REQUIRED when the flag is on — no implicit
...
/// Startup guard (env-free core): the cascade T1 (`METRALE_KV_LOCAL_GB > 0`)

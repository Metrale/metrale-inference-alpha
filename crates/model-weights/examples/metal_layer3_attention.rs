...
//!
//! Override the model directory with `$METRALE_MLX_MODEL_DIR`. The
...
//! their Apple Silicon Metrale Engine build runs real kernels on real
...
fn main() -> Result<()> {
    ...
    if !st_path.exists() {
        anyhow::bail!(
            "model.safetensors not found at {} — set $METRALE_MLX_MODEL_DIR or \

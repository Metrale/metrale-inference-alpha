...
Metrale Engine already shipped a TurboQuant variant of the KV cache (Walsh-Hadamard
...
reference at
[`TheTom/llama-cpp-turboquant`](https://github.com/TheTom/llama-cpp-turboquant).
The pieces ported into Metrale Engine in this work:

| Feature | TQ+ paper | Metrale Engine commit |
|---|---|---|
...
| Real Turbo3 prefill (fixes NVFP4 misroute) | (Metrale-specific fix) | kernel commit |
...
| `kernels/gb10/common/attn_prefill_paged_turbo{2,3}.cu` (new) | Real prefill kernels for Turbo2 (didn't exist) and Turbo3 (replaces NVFP4 misroute). |
| `kernels/gb10/common/attn_prefill_paged_bf16k_turbo3v.cu` (new) | Asymmetric K-bf16 / V-turbo3 prefill. |
...
| File | Why |
|---|---|
| `crates/cache/src/kv_cache.rs` | 9 new `KvCacheDtype` asym variants + `kv_pair()` / `is_asymmetric()` / FromStr aliases. Per-side `k_block_bytes_for_layer` / `v_block_bytes_for_layer` APIs so asym pools can size independently. |
| `crates/cache/src/kv_cache/paged_impl.rs` | Per-side K/V pool allocation; `k_block_stride_bytes_for_layer` / `v_block_stride_bytes_for_layer` exposed for kernel launchers. |
| `crates/cache/src/kv_cache/tests_tq_plus.rs` (new) | `ALL_VARIANTS` / `ASYM_VARIANTS` / `SYM_VARIANTS` arrays force new dtypes to be added to tests when added to the enum. Pins per-dtype block-byte arithmetic. |
| `crates/model-layers/src/layers/qwen3_attention/decode/write_kv_cache.rs` | Symmetric turbo write w/ WHT bookend (Turbo2/3/4/8) + asym Bf16K+Turbo3V dispatch + InnerQ K-side apply. |
| `crates/model-layers/src/layers/qwen3_attention/decode/run_paged_decode.rs` | Turbo decode dispatch incl. asym. |
| `crates/model-layers/src/layers/qwen3_attention/decode/attention_forward.rs` | V-type-aware iWHT guard + InnerQ Q-side apply. |
| `crates/model-layers/src/layers/qwen3_attention/prefill/paged_attn.rs` | Turbo2 + Bf16K+Turbo3V + real Turbo3 prefill routing. |
| `crates/model-layers/src/layers/qwen3_attention/{init,types,mod}.rs` | New KernelHandle fields + loaders + module exports. |
| `crates/model-layers/src/layers/qwen3_attention/innerq_driver.rs` (new) | `InnerQDriver::from_env()` reads `TURBO_INNERQ=N` + `TURBO_INNERQ_STRENGTH`. `start()` at boot, `maybe_finalize(128)` per scheduler chunk (idempotent kernel-side). |
| `crates/model-arch/src/weight_loader/qwen35/load_layers/tq_plus_weight_rotation.rs` (new) | `apply_canonical_rotation_inplace` helper. Wired into `attention_arms.rs` between sharding and NVFP4 quant for Q/K/V projections. Gated on `TQ_PLUS_WEIGHT_ROTATION=1`. |
| `crates/server/src/main_modules/{kv_dtypes.rs,serve_phases/kv_cache.rs}` + `crates/model-engine/src/factory/build.rs` | `boundary_dtype` parameter on `build_layer_kv_dtypes` (LA-V7 substrate). |
...
All numbers were measured on:
  ...
  - **OS:** Ubuntu 24.04 inside the Metrale Engine runtime container
  - **Driver/CUDA:** NVIDIA driver supporting CUDA 13.0, container base `nvidia/cuda:13.0.0-runtime-ubuntu24.04`

### 1. Build

```bash
git clone <metrale-repo> && cd metrale
git checkout feature/tq-plus-clean   # or whichever branch carries the integration

docker build -f docker/gb10/Dockerfile -t metrale-inference-gb10-tqplus .
# ~8 min cold (cargo build --release -p metrale-server inside builder stage)
```
...
```bash
docker stop metrale-bench 2>/dev/null; docker rm metrale-bench 2>/dev/null
docker run -d --name metrale-bench --gpus all --ipc=host \
  -p 8889:8888 -v /path/to/qwen3.6-35b-fp8:/model \
  -e RUST_LOG=warn \
  metrale-inference-gb10-tqplus \
...
python3 tests/test_kv_dtype_smoke.py \
  --image metrale-inference-gb10-tqplus \
...
All numbers in this section are from `tests/metrale_bench_comprehensive.py`
...
```bash
python3 tests/metrale_bench_comprehensive.py \
  --image metrale-inference-gb10-tqplus \
  --model-path /path/to/qwen3.6-35b-fp8 \
...
### Before/after vs upstream Metrale Engine (`87b7bb3`) — same harness, same model
...
Per-side cache pool refactor (`crates/cache/src/kv_cache.rs`):
...
the same `tests/metrale_bench_comprehensive.py` harness.
...
kernel — the only HDIM=128 bf16-attn model in Metrale Engine's tested set.
...
1. **Compile-time exhaustive match** — `kernel_modules_for_dtype` in
   `crates/model-layers/src/layers/qwen3_attention/init_kernel_dispatch.rs`
...
docker run --rm --entrypoint /bin/bash --gpus all \
  -e METRALE_SKIP_BUILD=1 -e CUDARC_CUDA_VERSION=13000 \
  -v $(pwd):/metrale metrale-inference-gb10-tqplus-dev \
  -c "cd /metrale && cargo test -p metrale-model-layers --tests qwen3_attention::init_kernel_dispatch::"
```
...
```bash
docker run … -e TURBO_INNERQ=512 -e TURBO_INNERQ_STRENGTH=0.5 metrale-inference-gb10-tqplus serve …
```
...
```bash
docker run … -e TQ_PLUS_WEIGHT_ROTATION=1 metrale-inference-gb10-tqplus serve …
```
...
the TQ+ work that spans multiple downstream engines (this Metrale Engine port,

...
//! 2026-09-25: GPU compute for the served NLLB model: kernel-launch primitives, the
//! encoder pass with its cross-KV precompute, and the single-token decoder step.
//!
//! Owner: model-engine.
//! Invariants:
//! - `forward_one` checks for a free self-attention row before writing one.
...
/// 2026-09-25: Single-token decode scratch, reused by every `forward_one`: bf16
/// rows of `d` (`ff` is `ffn` wide) plus the u32 token id and argmax result.
/// There is one instance per model, so decode steps must not overlap.
...
/// 2026-09-25: Encoder scratch (bf16, `enc_len` rows), allocated for one
/// `run_encoder` call and freed at the end of a successful pass.
struct EncScratch {
    residual: DevicePtr,
    normed: DevicePtr,
    q: DevicePtr,
...
impl NllbGpuModel {
    ...
    /// 2026-09-25: Out-of-place layer norm: normalize `src` into `dst` (distinct
    /// buffers). The beam decode uses it instead of copying `dh` to `normed`
    /// before an in-place `layer_norm`.
    ...
    /// 2026-09-25: Tensor-core GEMM `C[m,n] = A[m,k] @ W[n,k]^T` (bf16, no bias).
    pub(super) fn gemm(
        &self,
        a: DevicePtr,
        wt: DevicePtr,
    ...
    /// 2026-09-25: Multi-row biased linear (GEMM + row-broadcast bias) from
    /// `prefix.{weight,bias}`, then the LoRA delta for `prefix` when armed.
    pub(super) fn linear(
        &self,
        prefix: &str,
        a: DevicePtr,
    ...
    /// 2026-09-25: Add the LoRA residual for `prefix` onto `base_out` in place,
    /// when the LoRA gate is armed and the adapter covers that module.
    ...
    /// 2026-09-25: Single-token GEMV `y[n] = W[n,k] @ x[k] + bias` (bias may be NULL).
    fn gemv(
        &self,
        x: DevicePtr,
        wt: DevicePtr,
    ...
    /// 2026-09-25: M=1 biased linear via GEMV from `prefix.{weight,bias}`, then
    /// the LoRA delta for `prefix` when armed.
    fn linear1(
        &self,
        prefix: &str,
        x: DevicePtr,
    ...
    fn attention(
        ...
        KernelLaunch::new(self.gpu.as_ref(), self.kernels.attn)
            ...
            .arg_u32(0) // 2026-09-25: causal = 0; the decoder limits keys by passing tk = pos + 1.
            .launch(self.stream())
    }
    ...
    /// 2026-09-25: Embed `rows` token ids (device `u32[rows]`) into `out[rows,d]` bf16.
    ...
    /// 2026-09-25: Run the encoder over the formatted source ids and compute each
    /// decoder layer's cross-attention K/V into `kv`. Allocates and frees its
    /// own scratch and encoder output.
    ...
    /// 2026-09-25: Forward one decoder token at position `kv.dec_pos`: embed and
    /// position, self-attention (writing K/V at that row), cross-attention over
    /// the encoder KV, FFN, and the tied lm_head into bf16 `logits_out`
    /// (`[vocab]`). Advances `kv.dec_pos`; fails when the cache is full.
    pub(super) fn forward_one(
        ...
        for l in 0..self.dec_layers {
            ...
            // 2026-09-25: Self-attention over rows 0..=pos, after writing K/V into row `pos`.
        ...
        // 2026-09-25: Tied lm_head (the embedding table, no bias) into a bf16 logits row.
        self.gemv(
            s.dh,
            self.embed_table,
            DevicePtr(0),
    ...
    /// 2026-09-25: `decode_logits` row `i` (batch position); `prefill_logits` row
    /// `slot % max_batch`.
    #[inline]
    ...
    /// 2026-09-25: On-device argmax over a bf16 `[vocab]` logits buffer → token id.

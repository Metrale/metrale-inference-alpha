...
//! 2026-09-26: Warm and cold TTFT regression gates: one state machine, two modes.
//!
//! * Warm: every sample at a prompt length sends the same prompt, and each
//!   measured request follows a priming request with that prompt.
...
//!   the same box and model only; any other baseline gives an Info verdict
//!   (`ttft_verdict.rs`).
//! - A failing run is never stored as the baseline (`should_store`).
...
impl TtftGate {
    /// 2026-09-26: Smallest absolute TTFT rise, in ms, this gate calls a
    /// regression. A metric fails only when it exceeds both its percentage
    /// limit and this floor (`ttft_verdict.rs`).
    ...
    /// 2026-09-26: Every measured TTFT sample, across all prompt lengths.
    ...
    fn table(&self) -> ResultTable {
        ...
        for r in &self.rows {
            ...
            t.push(vec![
                ...
                // 2026-09-26: Cache-hit evidence: 0 cached tokens is Bad in warm
                // mode and Good in cold mode. The verdict does not read it.
                Cell::styled(
...
impl Benchmark for TtftGate {
    ...
    async fn next(&mut self) -> Result<BenchmarkResult> {
        ...
        if self.cursor >= self.lengths.len() {
            ...
            // 2026-09-26: How many measurements are behind the median, so a
            // record drawn at fewer lengths or repeats can be told apart.

...
// 2026-09-25: Kernels `attn_prefill_paged_bf16k_turbo2v` and `_64`: paged prefill flash attention with a BF16 K cache
// and a turbo2 V cache.
//
// The body is prefill_paged_compute_asym.cuh, which takes separate K and V loaders. Each V block (stride `tq2_v_bsb`
// bytes) holds 2-bit codebook indices, 4 per byte, then from byte `tq2_v_dsb` one FP8 E4M3 scale per 16 values.
...
// 2026-09-25: The attn_prefill_paged_turbo2.cu loader with the V-cache arguments: it dequantizes to BF16 while it fills
// shared memory, and the LUT holds the same turbo2 codebook.
...
#define KERNEL_NAME attn_prefill_paged_bf16k_turbo2v

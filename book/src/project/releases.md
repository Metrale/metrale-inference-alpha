...
Metrale Engine's release notes are per-version markdown files in [`docs/releases/`](https://github.com/Metrale/metrale-inference-alpha/tree/main/docs/releases). This chapter links to them and summarises the big themes across recent alphas. For the latest release, check the repo — this page is a stable pointer, not a ticker.
...
Since Metrale Engine is pre-1.0 and under aggressive development, semantic versioning does not apply. Any release can break API or CLI compatibility — the per-release notes document what.

## Where to read them

| Source | URL |
|---|---|
| Release notes folder | [`docs/releases/`](https://github.com/Metrale/metrale-inference-alpha/tree/main/docs/releases) |
| GitHub Releases | `https://github.com/Metrale/metrale-inference-alpha/releases` (if tagged) |
| Docker Hub | `https://hub.docker.com/r/metrale/metrale-inference-gb10/tags` |

The multi-model Docker image always tracks the latest alpha at `metrale/metrale-inference-gb10:latest`. Specific versions are tagged as `metrale/metrale-inference-gb10:alpha-2.44` etc.

## Recent themes

Rather than duplicate every release note, here's the shape of recent work. Each theme maps to architecture decision records under `docs/adr/` and the benchmark history in `docs/METRALE_SPARK_JOURNEY.md`.
...
- **Wave 12** — Anthropic streaming `stop_sequence` populated, `/tokenize`+`/detokenize` gated behind auth (then `METRALE_REQUIRE_AUTH`; the env var has since been replaced by the `--require-auth` flag).
...
OSS release prep (alpha-2.43-share) was the major non-code milestone: archive tags, docs cleanup, `metrale-internal/` separation for proprietary artefacts.

For the current roadmap, check the repo's pinned issues and the authoritative decision records at [`docs/adr/`](https://github.com/Metrale/metrale-inference-alpha/tree/main/docs/adr).

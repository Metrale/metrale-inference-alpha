...
pub(super) fn start_new_requests(
    ...
    // Always-mixed chunk-0 fuse: when decodes are active and METRALE_HOLO_ALWAYS_MIXED
    ...
    // ── Vision co-dispatch pre-pass (METRALE_VISION_CODISPATCH, default on) ──
    ...
    let vision_codispatch = std::env::var("METRALE_VISION_CODISPATCH")
    ...
    // ── Beam co-dispatch pre-pass (METRALE_BEAM_CODISPATCH, default on) ──
    ...
    let beam_codispatch = model.supports_beam()
        && std::env::var("METRALE_BEAM_CODISPATCH")

...
//! 2026-09-25: Parity of the Turbo3 and Turbo2 kernels: the K bytes of
//! `kv_cache_append_turbo3` / `_turbo2` against a CPU quantizer, and
//! `attention_decode_turbo3` / `_turbo2` against a CPU reference over the
//! CPU-dequantised cache.
//!
...
/// 2026-09-25: One group of 16 through the steps of the turbo2 / turbo3 append
/// kernels: `(indices, E4M3 scale byte)`, with the scale chosen so the
/// reconstruction's L2 norm equals the input's.
...
/// 2026-09-25: Run one token through an append kernel at cache position 0 and
/// return host copies of `(k_data, k_scales)`.
...
/// 2026-09-25: `kv_cache_append_turbo3` writes the K 3-bit packs and E4M3 scales
/// the CPU quantizer computes, byte for byte.
#[test]
...
/// 2026-09-25: `kv_cache_append_turbo2` writes the K 2-bit packs and E4M3 scales
/// the CPU quantizer computes, byte for byte.
#[test]
...
/// 2026-09-25: `attention_decode_turbo3` and `attention_decode_turbo2` against a CPU
/// reference over the CPU-dequantised cache, with the kernel's sparse-V skip at
/// the 1e-3 threshold the launch passes.
#[test]

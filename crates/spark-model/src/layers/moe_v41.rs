...
/// `METRALE_DS41_PREFETCH_K` (default 0 = off), read once: how many of the
/// next layer's predicted experts to start reading while this layer computes
/// (needs `METRALE_DS41_READER_POOL=1`). Off by default: on the 09-19 standard
...
struct Kernels {
    ...
    /// rows (warps) a block of the expert batch, 2 / 4 / 8 (METRALE_DS41_EXPERT_WARPS)
    experts_warps: u32,
    swiglu: KernelHandle,
...
/// `METRALE_DS41_SHARED_SIDE` (default on; `0` = off), read once: at decode the

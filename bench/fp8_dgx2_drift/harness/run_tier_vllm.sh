...
# 2026-09-26: run_tier.sh variant for a remote endpoint: no container check and no
# server log window; the same scoring, then aggregate.py.
#
# Owner: bench, FP8 drift harness.
# Invariants: none beyond the types.
#
# Reads API_BASE (default http://localhost:8889/v1), XDG_CONFIG_HOME_OVERRIDE
# (default /tmp/oc-tunnel-config, the opencode config dir) and OC_TIMEOUT
# (default 360).
#
# Usage:
...
# 2026-09-26: Exit 3 unless ${API_BASE}/models answers.
...
# 2026-09-26: The same prompt as run_tier_1200.sh; run_tier.sh's adds instructions
# on backgrounding, timeouts and teardown. The run directory goes in via --dir.
...
for i in $(seq 1 "${N}"); do
  ...
  EMPTY_LOG="/tmp/harness-${TIER}-r${i}.metrale.log"
  OUT_JSON="${RUNS_DIR}/run_${TIER}_${i}.json"

  rm -rf "${TARGET}" "${OC_JSON}" "${OC_ERR}" "${EMPTY_LOG}"
  : > "${EMPTY_LOG}"          # 2026-09-26: empty log, so the server counters are 0
  mkdir -p "${TARGET}"        # 2026-09-26: opencode's --dir is the scored target

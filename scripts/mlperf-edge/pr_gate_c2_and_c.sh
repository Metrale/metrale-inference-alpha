...
# for the METRALE_GDN_REGRESIDENT default-flip. Runs on dgx1.
#
...
#   ctl leg = METRALE_NO_GDN_REGRESIDENT=1  (regresident OFF, prior behaviour)
#
# TRAPS encoded here:
...
set -u
BIN="${1:?path to the PR met binary (27b target)}"
OUT="${2:?output dir}"
EP=/workspace/endpoints
YAML=$EP/examples/10_Edge_Agentic_Example/online_agentic_coding_metrale_quick.yaml
...
serve() { # $1 = leg, $2 = extra -e args
  sudo docker rm -f metrale-prgate >/dev/null 2>&1; sleep 3
  # shellcheck disable=SC2086
  sudo docker run -d --name metrale-prgate --network host --ipc host --gpus all \
    $2 \
    -v /workspace/.cache/huggingface:/root/.cache/huggingface \
    -v "$BIN:/usr/local/bin/met:ro" \
    metrale-gb10:followups serve "$MODEL" --host 0.0.0.0 --port $PORT \
    ...
    --tool-call-parser qwen3_coder --tool-grammar off --disable-thinking >/dev/null 2>&1
  for _ in $(seq 1 200); do
    curl -sf -m4 http://localhost:$PORT/v1/models 2>/dev/null | grep -q Qwen && return 0
    sudo docker ps --format '{{.Names}}' | grep -q metrale-prgate || { echo "SERVE_DIED leg=$1"; return 1; }
    sleep 5
  done
...
for leg in pr ctl; do
  case $leg in
    ...
    ctl) EXTRA="-e METRALE_NO_GDN_REGRESIDENT=1" ;;     # kill switch => OFF
  esac
  ...
  sudo docker logs metrale-prgate 2>&1 | grep -aE 'GDN prefill: (FLA chunked|REGISTER-RESIDENT)' \
    | sed 's/.*INFO.*: //' | sort -u | tee "$OUT/$leg.banner.txt"
  sudo docker rm -f metrale-prgate >/dev/null 2>&1
done

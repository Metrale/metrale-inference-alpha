...
**PR**: [#119](https://github.com/Metrale/metrale-inference-alpha/pull/119)
...
Architecture is a hybrid of patterns Metrale Engine already supports:

| Property | Step 3.7 | Nearest Metrale Engine Model |
|----------|----------|-------------------|
...
> **Known limitation**: Metrale Engine `ModelConfig` stores a single `rope_theta`
...
This creates two problems for Metrale Engine:

1. **OOM on single-DGX-Spark**: The fused checkpoint is 121 GB on disk. After
   CUDA init + 90 PTX modules, ~115 GB remains — not enough.
2. **EP incompatibility**: Metrale Engine EP filtering relies on
...
| 6 | `tracing` crate unavailable in `metrale-core` | Removed tracing call |

**Final build**: Clean. 90 kernel modules compiled. Docker image
`metrale-step3p7:latest` (2.79 GB).
...
### Launch Command
```bash
sudo docker run --name metrale-step3p7 --gpus all --ipc=host --network host \
  -v ~/.cache/huggingface:/root/.cache/huggingface \
  metrale-step3p7:latest serve stepfun-ai/Step-3.7-Flash-NVFP4 \
...
| GPU backend | **PASS** | `MetraleCudaBackend` initialized |
...
(94 GB), the two largest models Metrale Engine currently serves on single-GPU.
...
This is well within single-DGX-Spark capacity and matches how the model was
...
`c78f626` (on the contributor's fork):
...
| 8 | `layer_types` distinction (full vs sliding) discarded | All layers get `sliding_window=512` | Correct for sliding layers; full-attention layers shouldn't window but Metrale Engine applies globally |
...
### New files (4)
- `crates/config/src/parsers/step3p7.rs` — config parser (287 lines)
- `crates/model-arch/src/weight_loader/step3p7.rs` — weight loader (439 lines)
...
### Modified files (4)
- `crates/config/src/lib.rs` — re-export `parse_step3p7`
- `crates/config/src/dispatch.rs` — `"step3p7"` dispatch arm
- `crates/config/src/parsers/mod.rs` — module + re-export
- `crates/model-engine/src/factory.rs` — `Step3p7WeightLoader` registration
...
1. **[P0] EP=2 runtime validation**: Run Metrale Engine with the split checkpoint
   across two DGX Sparks. Verify weight loading, expert routing, and
   inference quality. Hardware is available (DGX Spark 4 + DGX Spark 6, QSFP
...
3. **[P1] Sliding vs full attention dispatch**: Investigate whether Metrale Engine
...
4. **[P2] MTP support**: Step 3.7 has 3 MTP draft modules (layers 45–47).
   ...
   decoding requires mapping these to Metrale Engine's MTP framework.
...
**Nodes:** DGX Spark 4 (10.0.0.4, rank 0) + DGX Spark 6 (10.0.0.6, rank 1)
...
the hidden dimension (4096). Every other Metrale Engine model has
...
| 9 | Bugs #1-#4 not in the engine source (only on GitHub fork) | Applied locally |

...
//!
//! Metrale Engine ships its OWN kernels. FlashInfer is wrapped here for exactly one
...
//!
//!   1. COMPILE TIME — every item in this module is `#[cfg(metrale_flashinfer)]`, and
//!      `build.rs` sets that cfg only when `FLASHINFER_HOME` is exported. A build
//!      without it links no FlashInfer object at all.
//!   2. RUNTIME — the dispatch arms are opt-in behind `METRALE_FLASHINFER_PREFILL=1`.
//!      The default is OFF.
//!
//! So the honest reading of a Metrale Engine performance number is that Metrale Engine kernels
...
//! piece behind Metrale Engine's flat ~3880 tok/s prefill at any concurrency.
...
/// process CUDA context (see `metrale_core::cuda_host`, which establishes one

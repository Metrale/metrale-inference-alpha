...
//! set `METRALE_STORE_DIR` to persist entries to disk.
//!
//! Design notes
//! ------------
...
//! - **Two eviction pressures.** TTL (`METRALE_STORE_TTL_SECONDS`, default
//!   24 h) reclaims idle entries lazily on get/insert; capacity
//!   (`METRALE_STORE_MAX_ENTRIES`, default 10 000) reclaims the coldest
//!   LRU entry when the map would exceed its bound.
//! - **Persistence (optional).** When `METRALE_STORE_DIR=/path/to/dir` is

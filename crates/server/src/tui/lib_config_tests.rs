...
//! 2026-09-26: Tests for the Config form's pick, add and remove verbs (`lib_config.rs`), checked through the argv.
//!
//! Owner: server tui.
...
/// 2026-09-26: The 35B fixture recipe; it pins `kv_cache_dtype: bf16`, `port: 8888`, `scheduler: slai`
/// and `speculative: true`.
fn recipe() -> Recipe {
...
fn picking_a_value_lands_in_the_form_and_the_command() {
    ...
    s.row = row_of(&s, "kv_cache_dtype");
    s.on_key(key(KeyCode::Enter));
    // 2026-09-26: `g` goes to the first entry (`bf16`, `KvCacheDtype::ALL[0]`), `j` to `fp8`.
...
fn the_add_list_omits_flags_already_on_the_form_including_renames() {
    ...
    for f in fields {
        ...
        // 2026-09-26: The recipe's `max_model_len` renders as `--max-seq-len` (`schema::RENAMES`).
...
fn x_removes_a_recipe_setting_and_its_flag_leaves_the_command() {
    let mut s = open_form();
    s.row = row_of(&s, "scheduler");
    s.on_key(key(KeyCode::Char('x')));
    let row = &s.config_rows()[row_of(&s, "scheduler")];
    assert!(row.removed, "the row stays visible, marked removed");
    assert_eq!(
        flag_value(&s, "--scheduler"),
        None,
...
fn x_again_restores_the_recipes_value() {
    ...
    let row = &s.config_rows()[row_of(&s, "scheduler")];
    assert!(!row.removed);
    assert_eq!(flag_value(&s, "--scheduler").as_deref(), Some("slai"));
}
...
fn enter_on_a_removed_row_restores_it_too() {
    ...
    assert!(!s.editing);
    assert!(!s.config_rows()[row_of(&s, "scheduler")].removed);
}
...
fn a_removal_that_breaks_the_config_is_refused_with_the_reason() {
    let mut s = open_form();
    // 2026-09-26: `validate_serve_args` refuses `--num-drafts` above 1 without a speculative method,
    // so removing `speculative` must be refused.

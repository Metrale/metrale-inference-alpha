...
//! `attn_m16_tc`, `lm_head_m16_tc`, `attn_ncol_gemv` and the `METRALE_M16_TC`
//! umbrella.
//!
...
/// `METRALE_FFN_M16_TC` is the A/B that re-runs it, in BOTH directions, and says
...
/// `METRALE_M16_TC` is round 6's umbrella and still arms this arm — folded in by
...
fn the_m16_umbrella_arms_the_ffn_arm_too() {
    ...
    assert!(on.ffn_m16_tc.value && on.ffn_m16_tc.from_env());
    // The narrow variable WINS when both are set, so `METRALE_FFN_M16_TC=0
    // METRALE_M16_TC=1` means what it reads as rather than depending on export
    // order.
    let narrow_off = with(
        &HOPPER,
...
/// `METRALE_ATTN_M16_TC=0` is the one-variable A/B that pins the parent tiers,
...
/// serving A/B exists for it anywhere. `METRALE_ATTN_NCOL_GEMV` runs that A/B.
#[test]

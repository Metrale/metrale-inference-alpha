...
pub enum LoraCommand {
    ...
    Promote {
        ...
        adapter_id: String,
        name: String,
        peft: metrale_core::config::PeftAdapterConfig,
    },
...
/// `METRALE_MTP_MAX_SEQS` (`=1` restores the single-sequence-only gate).
///
...
/// `METRALE_NO_MTP_BATCH_VERIFY` (presence) forces that serialized loop.
fn mtp_max_seqs() -> usize {
    ...
    // MTP-off). `METRALE_MTP_MAX_SEQS=8` restores the round-3 cap, `=1` the
...
pub fn run(
    ...
    // request. The levers were twenty-odd `METRALE_*` statics; they are resolved
    ...
    let always_mixed = std::env::var("METRALE_HOLO_ALWAYS_MIXED")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    if always_mixed {
        tracing::info!("METRALE_HOLO_ALWAYS_MIXED=on: fused mixed step always-on (slice-budget)");
    }
    ...
    // `admission` module docs and METRALE_KV_ADMIT_WATERMARK).
    ...
    let mut spill_manager: Option<KvSpillManager> = if swap_space_gb > 0 {
        ...
        // they never overlap) — but a SHARED path means two `met serve`
        ...
        let spill_dir = std::env::temp_dir().join(format!("metrale-swap-{}", std::process::id()));
    ...
    loop {
        ...
        // METRALE_MTP_TIMING summary reports (see mtp_timing::Phase::Gap): each
        ...
        if !did_mixed_step {
            ...
            // Spec-resume guard (METRALE_DFLASH_RESUME_GUARD=N, default 0 = off):
            ...
            // METRALE_DFLASH_SPEC_THINK=1: DFlash raw-argmax may speculate
            ...
            // Kill switch METRALE_MTP_POOL_FULL_WIDTH (presence) restores
                ...
                && active[0].grammar_state.is_none()
            {
                ...
                && (
                    ...
                    // METRALE_DFLASH_SPEC_THINK=1. Resume guard still
                ...
                )
            {
                ...
                if let Some(gate) = mtp_gate.as_mut() {
                        ...
                        && gate.next_step() == mtp_gate::GateStep::MeasureDecode
                    {
                        ...
                        match gate.next_step() {
                            mtp_gate::GateStep::MeasureDecode => {
                                ...
                                // METRALE_MTP_CATCHUP: ring the serially decoded
                                ...
                                // METRALE_MTP_MAX_SEQS > 1 — this guard keeps the
                    ...
                    // Gate bypassed (METRALE_MTP_GATE_FORCE=1): plain MTP.

...
#       kernels/gb10/qwen3.6-27b/nvfp4/ -- of which 14 are the redirect
#       target's (12 held, 2 `use`d) and 4 of those shadow a common kernel (w4a16_gemm,
#       moe_w4a16_grouped_gemm, gated_delta_rule,
#       attn_prefill_paged_indirect). A gate that ignored the redirect
#       selected 171 common tasks and reported a green receipt for sources the
#       build never compiles. The oracle is crates/kernels/build.rs
...
$model_files"
# Twelve are the redirect target's own files; two it `use`s from
# qwen3.6-35b-a3b's leaf (KERNEL.toml [sources]), and the RESOLVED path is what
# the build compiles.
stray="$(grep -v '^kernels/gb10/qwen3\.6-27b/nvfp4/' <<<"$model_files")"
[ "$stray" = "kernels/gb10/qwen3.6-35b-a3b/nvfp4/gated_delta_rule_wy17.cu
kernels/gb10/qwen3.6-35b-a3b/nvfp4/vision_encoder.cu" ] \
  || fail a "every non-common source must come from the redirect target or its [sources] use:
$stray"
ok a "the 14 model kernels all come from kernels/gb10/qwen3.6-27b/nvfp4/ or its use list"

for stem in w4a16_gemm moe_w4a16_grouped_gemm gated_delta_rule \
            attn_prefill_paged_indirect; do
...
if [ -z "$NVCC" ]; then
  ...
  # The gate resolves its task list through the resolver's Python mirror.
  mkdir -p "$SB/scripts/lib"
  cp "$HERE/lib/kernel_layout.py" "$SB/scripts/lib/"

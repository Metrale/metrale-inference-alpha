...
//!
//! * `METRALE_SSM_DECODE_RING=1` force-allocates the ring even under spec
...
//! * `METRALE_DISABLE_WATCHDOGS=1|true` (trimmed, case-insensitive — mirrors
...
/// watchdogs off) — never for an explicit `METRALE_SSM_DECODE_RING=0`
...
///
/// 8 is [`metrale_kernels::DECODE_ROLLBACK_RING_SLOTS`] and covers the
...
/// so the documented `METRALE_SSM_DECODE_RING` fallback stays reachable and
...
/// `METRALE_DISABLE_WATCHDOGS` truthiness — trimmed, case-insensitive, `1` or
...
///    the same reason `METRALE_SSM_DECODE_RING=1` always has: an operator (or a
...
/// 3. `METRALE_SSM_DECODE_RING=1|0` — the legacy spelling of "8" and "0";

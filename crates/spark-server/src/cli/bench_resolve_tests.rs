...
use super::*;
use metrale_plugin::gate::{Bound, GateBaseline, HardwareBaseline, ModelBaseline};
...
/// what was asked for and what exists". `h800` is a real NVIDIA part that Metrale Engine
...
fn threshold_params_derive_from_the_variant_and_yield_to_an_explicit_param() {
    let descriptor = metrale_plugin::registry::find("agentic-webserver").expect("registered");
    ...
    let mut values = metrale_plugin::ParamValues::from_overrides(&specs, vec![]).unwrap();
    let applied =
    ...
    let mut values =
        metrale_plugin::ParamValues::from_overrides(&specs, vec![("wall_budget_s", "1234")])
            .unwrap();
...
fn threshold_params_substitute_a_min_bound_when_the_metric_is_a_floor() {
    let descriptor = metrale_plugin::registry::find("bfcl-subset").expect("registered");
    let specs = descriptor.build().parameters();
    let mut entry =

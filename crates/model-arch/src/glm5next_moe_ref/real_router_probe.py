# 2026-09-25: Runs the real layer 0-3 prefix through the reference HF modules to get the layer-3
# router input, compares FP32 and BF16 routing on it, and saves the input to /w/real_router.pt.
#
...
for L in (0, 1, 2, 3):
    ...
    if is_kda:
        ...
        # 2026-09-25: Remap the checkpoint's KDA tensors onto the module's names: the three
        # conv tensors are concatenated in q, k, v order into `conv1d.weight`, and f_a_proj,
        # f_b_proj, dt_bias and A_log move under `forget_gate.`.
        ...
        for a, b in (("f_a_proj.weight", "forget_gate.f_a_proj.weight"),
            ...
            sd[b] = W["self_attn." + a]
        # 2026-09-25: dt_bias and A_log are loaded as F32; every other tensor as BF16.
...
# 2026-09-25: Weight difference on the experts both ladders picked.
wd = []
for r in range(T):

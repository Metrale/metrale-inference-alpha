...
// Build script for metrale-rdma. It compiles the one-sided RDMA verbs C shim
// and decides `cfg(metrale_rdma_verbs)` for the whole workspace:
//
//   * cfg ON  ⇔ target_os == linux AND !skip_build()  (rdma-core is Linux-only)
//   * skip_build() honours METRALE_SKIP_BUILD / SKIP_METRALE_BUILD ∈ {1,true,TRUE}
...
// `links = "metrale_rdma_shim"` key, cargo exposes that to the build scripts of
// DIRECT dependents as `DEP_METRALE_RDMA_SHIM_HAS_VERBS`. A direct dependent's
// build.rs (e.g. spark-storage, in a follow-up PR) reads it and re-emits
// `cargo:rustc-cfg=metrale_rdma_verbs` for its own gated modules. Any crate that
// grows a real `#[cfg(metrale_rdma_verbs)]` gate needs its own DIRECT dep on
// metrale-rdma plus the same re-emit (DEP_ vars are invisible to transitive
// dependents).
...
fn main() {
    ...
    // WITHOUT the nvcc/CUDA side effects that METRALE_SKIP_BUILD carries in
    // spark-storage. Downstream RDMA code compiles to its `not(metrale_rdma_verbs)`
    ...
    // static-lib link directives (rustc-link-lib=static=metrale_rdma_shim +
    ...
    // The cross-crate signal: DEP_METRALE_RDMA_SHIM_HAS_VERBS in direct
...
/// Verbatim spark-storage semantics: METRALE_SKIP_BUILD / SKIP_METRALE_BUILD in

#!/bin/bash
# L1 decode A/B: METRALE_GDN_FUSED_VERIFY (fuse K=2 conv/norm verify epilogue).
# Leg A = base (flag off) ; Leg B = METRALE_GDN_FUSED_VERIFY=1. Same binary/model/config.
# Bit-identical toggle -> combined_sha MUST match; TPOT delta is the signal.
set -u
IMG=metrale-gb10:followups
BIN=/workspace/.wt-decode-fold/target/release/met
...
SERVE_FLAGS=(
  ...
  --tool-call-parser qwen3_xml --tool-grammar off --disable-thinking
)

leg() {
  local tag="$1"; shift
  local extra=("$@")
  local CN="metrale-l1-$tag"
  for c in $(sudo docker ps -q --filter "name=metrale-l1-"); do sudo docker rm -f "$c" >/dev/null 2>&1; done
  # never touch the neighbour ollama on :8000
  if pgrep -f 'release/met serve' | grep -qv $$; then :; fi
  sleep 4
  sudo docker run -d --name "$CN" --network host --gpus all --ipc=host \
    "${BASE_ENV[@]}" "${extra[@]}" \
    -v "$HFCACHE:/root/.cache/huggingface:ro" -v "$BIN:/usr/local/bin/met:ro" \
...
leg base_off                                   || exit 1
leg fused_on -e METRALE_GDN_FUSED_VERIFY=1       || exit 1

...
// go through `metrale_tier::pio`, which is `pread`/`pwrite` on unix and
...
impl StorageBackend for PosixBackend {
    fn read(&mut self, requests: &[ReadRequest], stream: u64) -> Result<()> {
        ...
        for req in requests {
            ...
            metrale_tier::pio::read_exact_at(self.layout.file(req.group.layer), buf, off)
    ...
    fn write_from_host(&mut self, key: GroupKey, src: &[u8]) -> Result<()> {
        ...
        metrale_tier::pio::write_all_at(self.layout.file(key.layer), buf, off)

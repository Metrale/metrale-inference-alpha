...
// 2026-09-25: W4A4 NVFP4 GEMM on the FP4 tensor cores, for the dense-FFN prefill
// (dense_ffn.rs under METRALE_FP4_PREFILL). Both operands are E2M1 packed two per byte,
// [rows, K/2], with one E4M3 scale per 16 K values, [rows, K/16]; the MMA is
// mma.sync kind::mxf4nvf4.block_scale.scale_vec::4X.m16n8k64. Output C is BF16 [M, N]:
// C[m,n] = scaleA2 * scaleB2 * sum_k deq(A[m,k]) * deq(B[n,k]). ops::w4a4_gemm passes
// scaleA2 = 1.0 and the weight's scale_2, grid (ceil(N/128), ceil(M/128)), 256 threads.
// Each CTA computes a 128x128 tile; K advances 64 per step and must be a multiple of 64.
// Everything is inside #ifndef METRALE_NO_WARP_BLOCKSCALE_MMA, which the hopper, b200
// and b300 HARDWARE.toml define; gb10 does not.
// forked-from: kernels/gb10/nemotron-labs-3-puzzle-75b-a9b/nvfp4/w4a4_gemm.cu (2026-09-24; 157 of 258 lines differ, see kernels/FORKS.md)
...
// 2026-09-25: A_packed [M,K/2] E2M1; A_sf [M,K/16] E4M3; B_packed [N,K/2]; B_sf [N,K/16].
...
// 2026-09-25: Load rows are clamped to M-1 / N-1, so a partial tile reads in bounds;
// the clamped rows' results are dropped by the bounds-checked epilogue.

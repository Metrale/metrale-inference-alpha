...
//! 2026-09-26: Tests for the probe request shape, the codegen structure check
//! and the tool-call scoring.
//!
...
fn the_base_probe_body_is_deterministic_and_penalty_free() {
    // 2026-09-26: No penalty key: a repetition penalty would penalise the
    // repeated `\n` that `code_shape` requires.
...
fn the_check_accepts_the_common_aliases_a_model_reaches_for() {
    // 2026-09-26: A fenced block, and a docstring before the body.
    let fenced =
        "```python\ndef fib(n: int) -> int:\n    \"\"\"nth Fibonacci.\"\"\"\n    return n\n```";
...
fn only_a_real_4xx_from_the_server_excuses_the_tool_probe() {
    // 2026-09-26: `NotApplicable` passes the tool-call bar, so anything this
    // accepts scores as a pass. None of the first four is a 4xx from the
    // server, though three of them carry "400" or "tool".

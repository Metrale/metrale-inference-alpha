...
pub struct Qwen3SsmLayer {
    ...
    /// compressed-tensors checkpoints (`METRALE_FP8_ROWWISE=1`).
    ///
    ...
    /// Tier-1c keep-packed ternary Q2_0 fused in_proj_qkvz (`METRALE_GGUF_NATIVE_Q2`).
    ...
    /// OPT-IN via `gdn_smem_stage_enabled()` (`METRALE_GDN_SMEM_STAGE`).
    pub(super) gdn_f32_strided_norm_smem_k: KernelHandle,
    /// FP16 h-state twin of `gdn_f32_strided_norm_half_k` (`METRALE_SSM_H_FP16`).
    ...
    /// Dual-output (bf16 + f32) MoE-input norm for METRALE_FP32_ROUTING. Zero if absent.
    ...
    /// when absent or killed by `METRALE_NO_K64_N64`.
    ...
    /// −7.25%, BFCL identical); kill switch `METRALE_NO_GDN_REGRESIDENT=1`.
    ...
    /// `[defaults] gdn_prefill_tc` (`METRALE_GDN_PREFILL_TC` overriding), which
    /// Hopper ships ON since round 13; `METRALE_NO_GDN_PREFILL_TC_REMNANTS=1` pins them
    ...
    /// 13, OFF elsewhere, with `METRALE_GDN_PREFILL_TC` overriding either way.
    /// Both per-chunk
    ...
    /// `METRALE_GDN_TMA=1`. `try_kernel` => 0 on images that lack it, and the
    ...
    /// kill switch METRALE_NO_GDN_WY2_RESIDENT (PRESENCE — `=0` is NOT off).
    ...
    /// kill switch METRALE_NO_GDN_WY3_RESIDENT (PRESENCE — `=0` is NOT off).
    ...
    /// `METRALE_GDN_WOA=1` (see gdn_flags.rs). Engaged only on a per-call
    /// request.
    ...
    /// (`METRALE_SSM_H_FP16` stage 2). Same launch contracts, same float
    ...
    /// when the `METRALE_GDN_FUSED_VERIFY` env flag is set (default OFF); the
    ...
    /// default ON when present, kill-switch `METRALE_GDN_FUSED_CONV17=0`.
    ...
    /// Kill-switch: `METRALE_GDN_WYN=0` (default ON).
    pub(super) gdn_wyn_k: [KernelHandle; 12],
    /// FP16 h-state twins of the wyN family (K=5..16), stage 2 of
    /// `METRALE_SSM_H_FP16` — added 2026-08-29 (#812: the FP16 pool is the
    ...
    // `METRALE_FP8_W8A8=1` for staged rollout.

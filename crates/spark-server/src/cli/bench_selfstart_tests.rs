...
fn a_co_tenanted_box_is_refused_with_the_remedies() {
    // 16 GB of co-tenants on a 121 GB unified pool: measured to cost Metrale Engine 32 %
...
fn a_declared_lever_this_process_lacks_is_refused_with_the_export_line() {
    use metrale_plugin::serve_env::Reconciled;
...
fn baseline_pins_are_applied_and_the_operator_wins_a_clash() {
    ...
    let merged = metrale_plugin::gate::merge_serve_overrides(baseline, requested);
    assert_eq!(
...
fn a_baseline_declared_serve_pin_reaches_the_rendered_serve_args_without_cli_flags() {
    ...
    // metrale-recipes repo, not in this tree; the fixture stands in for the

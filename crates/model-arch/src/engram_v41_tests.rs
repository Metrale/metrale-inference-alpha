...
//! 2026-09-25: Tests for the production engram against its oracles:
//!   * the hasher against the reference `NgramHashState`, exact, over the
//!     prefill and both decode regimes;
//!   * `layer_row_ids` ordering;
//!   * on a GPU (ignored by default): the projection and gate against the
//!     golden's `engram_out` captures within bf16, real-table rows through the
//!     device Q2_K dequant against the CPU decoder bit for bit (needs the
//!     shards), and the Q2_K GEMV against the bf16 GEMV bit for bit.
//!
...
/// 2026-09-25: The production tables from the fixture's, flattened the way the
/// GGUF metadata is.
fn tables_from_ref(t: &EngramTables) -> Arc<EngramHashTables> {
    ...
    // 2026-09-25: The fixture's pad_id is already in the compressed space; find
    // a token that maps to it.
    let pad_tok = t
        .token_map
        .iter()
...
/// 2026-09-25: `(regime, start, len)` for prefill and the two decode steps.
...
fn hasher_matches_the_reference_exactly() {
    ...
    for (r, start, len) in regime_spans(&f) {
        ...
        // 2026-09-25: And against the golden's `engram_hashes` capture.
...
fn gpu_engram_rows_from_the_real_tables_match_the_cpu_decoder() {
    use metrale_gpu_runtime::gpu::GpuBackend;
    use metrale_model_weights::weights::dequant_cpu::{GgmlType, dequant_to_bf16};
    use metrale_model_weights::weights::expert_stream::{EngramRowReader, ShardFiles};
    let files = Arc::new(
...
/// 2026-09-25: The single-token `wkv` projection off the raw Q2_K blocks against
/// the bf16 GEMV over the device dequant of the same blocks, bit for bit:
...
fn gpu_engram_wkv_q2k_gemv_is_bitwise_the_bf16_gemv_over_the_expansion() {
    ...
    for b in raw.chunks_exact_mut(84) {
        ...
        // 2026-09-25: d, dmin: f16 with a small exponent, either sign.
        for i in [80usize, 82] {
            let r = rnd() as u16;

...
//! 2026-09-26: Tests for the shell runner's capture bound, truncation and
//! process-group kill.
//!
//! Owner: bench, agentic.
...
fn a_runaway_command_cannot_grow_the_capture_without_bound() {
    // 2026-09-26: 64 MiB through the capture; what it holds must not depend on
...
fn a_character_split_across_the_seam_is_not_mangled() {
    // 2026-09-26: Head and tail decoded separately would each see half of a
    // two-byte `é` and emit a replacement character.
...
fn truncation_is_idempotent_and_stays_inside_the_output_cap() {
    // 2026-09-26: The agent loop truncates every tool result again, so a second
    // `truncate` must return its input unchanged.
    for text in [
        "a".repeat(100_000),
        "é".repeat(100_000),
        "line\n".repeat(20_000),
...
async fn a_timed_out_command_takes_the_children_it_forked_with_it() {
    // 2026-09-26: A child the command forked must die with it on the timeout.
...
async fn a_setsid_server_is_left_for_the_reaper() {
    // 2026-09-26: A `setsid` child leaves the group and survives the timeout;
    // `reap` handles it at the end of the iteration.

...
//! MID-CHUNK SSM tail capture (default-on, opt-out `METRALE_SSM_TAIL_MIDCHUNK=0`).
//!
//! The clamp-based `METRALE_SSM_TAIL_CKPT` path lands a chunk boundary on
...
//! (`METRALE_SSM_TAIL_MIDCHUNK=0`) is a no-op (returns `None`) and byte-identical
//! to prior behavior.
...
impl TransformerModel {
    ...
    pub(in crate::model) fn prepare_midchunk_capture(
        ...
        // ONLY `metrale_scale` builds can actually fill this plan's `h_dsts`.
        //

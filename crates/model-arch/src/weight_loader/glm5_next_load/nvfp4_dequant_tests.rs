...
/// 2026-09-25: `E2M1`: 1 sign bit, 2 exponent bits (bias 1), 1 mantissa bit.
/// `e = 0` is subnormal (`m/2`), so the eight magnitudes are
/// `0, 0.5, 1, 1.5, 2, 3, 4, 6` and code `0b1xxx` negates.
const SPEC_E2M1: [f32; 16] = [
    0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, -0.0, -0.5, -1.0, -1.5, -2.0, -3.0, -4.0, -6.0,
];

/// 2026-09-25: `E4M3`: 1 sign, 4 exponent (bias 7), 3 mantissa; `e > 0` is
/// `2^(e-7) * (1 + m/8)`. The four codes this file uses, as sign/exponent/mantissa:
/// `0 0110 000` = 0.5, `0 0111 000` = 1.0, `0 0111 100` = 1.5,
/// `0 1111 110` = `2^8 * 1.75` = 448.
const E4M3_HALF: u8 = 0x30;
const E4M3_ONE: u8 = 0x38;
const E4M3_ONE_AND_A_HALF: u8 = 0x3C;
const E4M3_MAX: u8 = 0x7E;

/// 2026-09-25: Two `e2m1` codes to one byte, low nibble first.
...
/// 2026-09-25: One block of 16, scale 1.0, global 1.0: the codebook itself comes back.
#[test]
...
/// 2026-09-25: Reading the high nibble first would swap every column pair while
/// shape, finiteness and magnitudes survive, so an asymmetric byte checks it.
#[test]
fn even_column_is_the_low_nibble() {
    // 2026-09-25: Byte 0x20 = high nibble 2 (1.0), low nibble 0 (0.0).
...
/// 2026-09-25: Row stride, per-block scale selection and the global multiplier
/// at once. Every expected value is a product of exact binary fractions, so the
/// assertions are exact.
#[test]
fn scales_are_selected_per_row_and_per_block() {
    // 2026-09-25: 2 rows x 32 logical columns = 2 blocks per row, 16 packed
    // bytes per row; every code is 2, magnitude 1.0.
    let row: Vec<u8> = pack(&[2u8; 32]);
    let mut packed = row.clone();
    packed.extend_from_slice(&row);
    // 2026-09-25: Block scales: row 0 [1.0, 1.5], row 1 [0.5, 448].
...
/// 2026-09-25: The logical width is twice the on-disk packed width.
#[test]
fn the_logical_width_is_twice_the_on_disk_width() {
    let packed = vec![0u8; 3 * 8];
    let scales = vec![E4M3_ONE; 3];
    let out = dequant_nvfp4_to_f32("t", &packed, &[3, 8], &scales, 1.0).unwrap();
    assert_eq!(out.len(), 3 * 16, "[3, 8] U8 is a [3, 16] weight");
}

/// 2026-09-25: A short shard, a wrong scale count, a width that is not a whole
/// number of blocks and a non-2-D shape are all errors.
#[test]
fn malformed_inputs_are_errors() {
    ...
    // 2026-09-25: 8 logical columns is half a block.
...
/// 2026-09-25: The E4M3 table has all 256 entries, so a malformed block-scale
/// byte would index rather than panic (`0x7F` is NaN, `0x80..` negative). The
/// range check refuses it and names the byte.
#[test]
fn a_block_scale_byte_outside_the_finite_non_negative_range_is_refused() {
    // 2026-09-25: 0x7F = NaN, 0x80 = -0.0, 0xFF = -NaN, 0xB8 = -1.0.
    ...
    // 2026-09-25: 0x7E, the largest finite E4M3 (448.0), stays legal.

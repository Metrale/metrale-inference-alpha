...
pub fn step_mtp(
    ...
    // METRALE_MTP_TIMING outer bracket: `step_mtp` minus the per-chunk verify
    ...
    // SSOT + overrides (`METRALE_MTP_K_LADDER`, `METRALE_NO_MTP_K_LADDER`):
    ...
    // METRALE_NO_MTP_BATCH_BOOTSTRAP.
    ...
    for &idx in &bootstrap_idxs {
        ...
        // SSOT, no new channel. Kill-switch: METRALE_NO_MTP_MINP=1.
        ...
        // Ctx-holes fix (METRALE_DFLASH_SERIAL_APPEND=1), COMPLEMENT-GATED:
    ...
    // reachable when `METRALE_MTP_MAX_SEQS > 1` (default 32 with the ladder)
    // puts >= 2 verify-ready sequences in one step (`METRALE_MTP_MAX_SEQS=1`
    ...
    // `can_batch_verify(&ks)`. Kill switch `METRALE_NO_MTP_BATCH_VERIFY`
    ...
    // from C=1..4 before this). Kill switch: METRALE_DFLASH_BATCH_VERIFY=0.
    ...
    // Default ON at ratio 0.75 (+2.6% at C=8; kill switch `METRALE_NO_MTP_DCUT`,
    // PRESENCE). Ranks every prunable draft position ACROSS the batch by its
    // prefix-product survival score and keeps the top `METRALE_MTP_DCUT_RATIO`

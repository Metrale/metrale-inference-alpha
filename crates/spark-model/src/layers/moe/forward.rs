...
impl MoeLayer {
    /// True when the METRALE_FP32_ROUTING path is active: the SSM-side MoE-input
    ...
    pub fn forward(
        ...
        // For DFlash capture layers only, when `METRALE_FRANKENSTEIN_DECODE_VIA_PREFILL=1`
        ...
        // preserving Metrale Engine's TPS on the bulk of the network. The 5 capture layers
        // pay ~250 µs each (microbench), totalling ≈1.25 ms per token (negligible
        // at Metrale Engine's ~58 ms/token decode latency).

...
//! 2026-09-25: `load_layers` for Step 3.7 Flash. Every layer is a
//! `Qwen3AttentionLayer` with norms offset by one, a MoE or dense FFN, and a
//! sliding window on the layers `layer_types` marks as sliding (or, when it
//! is empty, on every layer whose index is not a multiple of 4).
//!
...
fn load_moe_ffn(
    ...
    // 2026-09-25: The shared expert (`share_expert`) is quantized to NVFP4 at
    // load.
...
fn load_dense_ffn(
    ...
    // 2026-09-25: With `METRALE_FFN_MMQ` and the Q4_K kernels, this builds the
    // Q4_K copies and frees the transposed ones before the KV cache is sized.
...
fn load_attention_layer(
    ...
    let is_sliding = if !config.layer_types.is_empty() {
        config.layer_types.get(i).copied() == Some(metrale_config::LayerType::SlidingAttention)
    } else {
        !i.is_multiple_of(4)
    };
    ...
    if let Some(gw) = g_proj_weight {
        layer.set_head_gate_weight(
            gw,
            metrale_model_layers::layers::qwen3_attention::types::HeadGateActivation::Sigmoid,
        );
    }

# Metrale Engine LoRA adapter mode — working settings & serving recipe
...
Train with **HuggingFace `peft`**, not MLX — Metrale Engine consumes standard **PEFT safetensors**
...
### Adapter config MUST match the Metrale Engine apply surface

Metrale Engine LoRA v0 applies the BF16 delta at attention **k/v/o** on the **full-attention layers
...
top of Metrale Engine's **NVFP4** base at serve time (small train/serve base mismatch — fine for a demo).
...
Reuse a prebuilt Metrale Engine GB10 image for the CUDA/nccl/cudart/cublasLt runtime libs
(`metrale/metrale-inference-gb10:dev` has all three in the ldconfig cache), and bind-mount a
LoRA-capable `met` binary + the adapter + the host model cache. Run **detached**.

```bash
# met must be built WITH the model's kernels + LoRA support:
#   METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=holo-3.1-0.8b METRALE_TARGET_QUANT=nvfp4 \
#     cargo build --release --bin met
docker run -d --name metrale-lora --gpus all --network host \
  -e LD_LIBRARY_PATH=/usr/local/cuda/targets/sbsa-linux/lib:/lib/aarch64-linux-gnu \
  -v /path/to/met:/usr/local/bin/met:ro \
  -v /path/to/adapter-dir:/adapter:ro \
  -v /tank/hf/hub:/root/.cache/huggingface/hub:ro \
  metrale/metrale-inference-gb10:dev \
...
Notes / gotchas:
- The image `ENTRYPOINT` is `met`, so the args start at `serve`.
...
fp32). The adapter loader (`metrale-gpu-runtime/src/weights/adapter.rs`) only converted **F16→BF16**;
F32 fell through as `WeightDtype::FP32`. But the pool pack (`metrale-model-layers/src/lora/mod.rs`,
...
After the fix, Metrale Engine reproduces the `peft`/transformers output **verbatim**:

| Prompt | `peft`/transformers | Metrale Engine (fixed) |
|---|---|---|
| "What is the Metrale Engine launch codeword?" | STARFALL-4728 | ✅ **STARFALL-4728** |

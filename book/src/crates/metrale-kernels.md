...
At runtime, `spark-server::main` resolves the current model's `KernelTarget`, calls `ptx_modules(&target)`, and passes the resulting slice to `MetraleCudaBackend::new` which uploads each PTX module to the GPU via `cuModuleLoadData`.

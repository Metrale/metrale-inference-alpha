...
describe('the populated state, which is what ships', () => {
  ...
  test('the verdict tile is the one cost.js derives from the same records — never typed', () => {
    ...
    const verdicts = rungVerdicts(lad.metrale.points, lad.baselines);
...
describe('the empty state', () => {
  ...
  test('it names what Metrale Engine has, what vLLM has, and what fills the gap — in that order', () => {
    const iMetrale = real.indexOf('Metrale Engine:');
    const iVllm = real.indexOf('vLLM:');
    const iFills = real.indexOf('What fills this:');
    expect(iMetrale).toBeGreaterThan(-1);
    expect(iVllm).toBeGreaterThan(iMetrale);
...
describe('a losing rung is drawn, labelled and counted against us', () => {
  ...
  test('every lose label stays INSIDE the plot — a clipped factor is a loss not shown', () => {
    // Eight rungs, Metrale Engine losing every one, so labels land at both ends of the
...
describe('facility overhead', () => {
  ...
  test('the fixture actually draws both engines, or nothing below means anything', () => {
    expect(cost.metrale).not.toBeNull();
  ...
  test('a low-to-high pair shades a band FOR BOTH ENGINES, under the lines', () => {
    const p = plot(band);
    // Two bands: the Metrale Engine series and the comparable vLLM one-shot. One band
  ...
  test('...AND MOVES NO COMPARISON: the winner and the factor are byte-identical', () => {
    ...
    const verdicts = (s) => [...s.matchAll(/(?:Metrale Engine|vLLM) cheaper ×[\d.]+/g)].map((m) => m[0]);
  ...
  test('the MARK SITS ON THE LOW EDGE — never floating inside an assumption', () => {
    ...
    // so it holds whatever the axis does: every Metrale Engine mark must lie on the

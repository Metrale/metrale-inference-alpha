...
//! `METRALE_DENSE_FP8=1`, `--lm-head-dtype bf16`, tp 1): the loader's own
...
//! re-opens an NVFP4 fallback (`METRALE_DENSE_FP8_KEEP_NVFP4`, the
//! `METRALE_CUTLASS_NVFP4_*` levers, `METRALE_ATTN_W4A4`) returns
...
pub struct Fp8RouteInputs {
    /// `METRALE_DENSE_FP8=1` — `qwen35_dense::dense_fp8_enabled`.
    pub dense_fp8: bool,
    ...
    /// `METRALE_NO_GDN_FP8` is unset — `qwen35_dense::gdn_fp8_arm_selected`.
    pub gdn_fp8: bool,
    ...
    /// only when the compiled target (or `METRALE_FFN_GATEUP_FUSED`) arms the arm
    /// that reads it (#927).
...
impl Fp8RouteInputs {
    ...
    pub fn from_env(config: &ModelConfig, w8a8_prefill_kernels: bool) -> Self {
        Self {
            ...
            // `== Ok("1")`, NOT presence. `METRALE_DENSE_FP8=0` is off.

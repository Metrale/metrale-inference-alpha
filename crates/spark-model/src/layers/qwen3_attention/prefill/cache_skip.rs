...
impl Qwen3AttentionLayer {
    ...
    pub(in crate::layers::qwen3_attention) fn prefill_attention_with_cache_skip(
        ...
        // METRALE_OP_DUMP: k AFTER k_norm, BEFORE RoPE. Matches vLLM's "k_proj"
        ...
        // METRALE_OP_DUMP: k AFTER RoPE (final K that gets written to KV cache).
        if num_tokens > 0 {
            let kv_dim_e = (nkv * hd) as usize;
        ...
        // METRALE_OP_DUMP: attn_out BEFORE sigmoid gate (raw FlashAttention output).
        ...
        // METRALE_OP_DUMP: attn_out AFTER sigmoid gate (input to o_proj linear).
        if num_tokens > 0 {
            let nq_hd = (nq * hd) as usize;
            super::super::op_dump::dump_bf16(
                ctx.gpu,

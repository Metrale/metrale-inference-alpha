...
//! `METRALE_DFLASH_STATIC_GAMMA` (presence). `METRALE_DFLASH_GAMMA_RESOLVER`
...
//! atomics so the per-step path is plain loads): `METRALE_DFLASH_RUNG_MULTI`
//! (K at C>=2, default 4), `METRALE_DFLASH_RUNG_NARROW` (C=1 prose K, default
//! 5), `METRALE_DFLASH_RUNG_WIDE` (C=1 code K, default = cap),
//! `METRALE_DFLASH_RUNG_ENTER` (window hits at/above which C=1 goes wide, 60),
//! `METRALE_DFLASH_RUNG_LEAVE` (hits at/below which it goes narrow, 46),
//! `METRALE_DFLASH_RUNG_DWELL` (min steps between switches, 64 = one window).
//!
...
//! OPT-IN (`METRALE_GDN_WOA=1`, see `gdn_flags::gdn_woa_enabled`), so on main

...
/// P1-2 (2026-07-09, env `METRALE_GRAMMAR_FORCE_CLOSE=1`): with `force_close`,
...
/// repetition, applied ONLY when the `METRALE_GRAMMAR_VALUE_HARDEN` kill-switch
...
/// `METRALE_GRAMMAR_VALUE_HARDEN`; OFF unless exactly `"1"`. OFF ⇒ the emitted
...
/// `tool_choice="auto"`. Read once per call from `METRALE_TOOL_SHORT_TRIGGER`;
...
///
/// F2-2a: when `METRALE_GRAMMAR_VALUE_HARDEN=1` the `rest` rule is bounded
...
pub(crate) fn xml_param_value_body_ebnf_opts(
    ...
    // P1-1 (2026-07-09, env `METRALE_GRAMMAR_ALLOW_EMPTY_VALUE=1`): the
...
impl GrammarEngine {
    ...
    pub fn compile_qwen3_coder_tool_grammar(
        ...
        // METRALE_TOOL_SHORT_TRIGGER=1 (kill-switch, default OFF): under auto mode,

...
//! `metrale-quant/src/fp8.rs` (unreachable, zero dependents) — with the
//! byte-exactness tests attached to the copy that never ran. Both crates
//! already depended on `metrale-core`, so the fix was to move the arithmetic
...
/// cast: reference activations and the dequanted-weight snapshots Metrale Engine is
...
mod tests {
    ...
    fn fp8_lut_matches_ocp_values_and_metrale_nan_policy_for_all_bytes() {
        ...
        // independently of the table's bit assembly. Metrale Engine deliberately maps

...
//! current.
//!
//! Metrale Engine could previously only *consume* a model that was already in the HF
...
//! `metrale-download`, and progress arrives on a `std::sync::mpsc`. See
...
pub(crate) const AGENT: &str = concat!("metrale-spark/", env!("CARGO_PKG_VERSION"));
...
pub enum DownloadError {
    ...
    /// The repo publishes nothing Metrale Engine can load.
    NoSafetensors {
        repo: String,
    },
    Http {

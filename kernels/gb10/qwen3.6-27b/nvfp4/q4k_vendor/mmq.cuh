...
static mmq_q8_1_ds_layout mmq_get_q8_1_ds_layout(const ggml_type type_x) {
    switch (type_x) {
        ...
        case GGML_TYPE_Q2_0:
            ...
            // path. DS4 lets Q2_0 reuse Metrale Engine's shipping `metrale_q8_1_quantize_ds4_bf16`
...
static __device__ __forceinline__ void load_tiles_nvfp4_nvfp4(const char * __restrict__ x,
    ...
    // METRALE (A2): the weight buffer is SoA per row — [qs: bpr*32][d: bpr*4] —
    ...
    // 4-byte loads. Written by metrale_nvfp4_repack; see the note there.
...
template<ggml_type type, int mmq_x, int mmq_y, bool need_check, typename dst_t = float> // METRALE: dst_t for fused bf16 output
...
#pragma unroll
            for (int l = 0; l < tile_C::ne; ++l) {
                ...
                if constexpr (sizeof(dst_t) == 2) { dst[ids_dst[j]*stride + i] = __float2bfloat16(sum[(j0/tile_C::J + n)*tile_C::ne + l]); } // METRALE fused bf16
...
template <ggml_type type, int mmq_x, bool need_check, bool fixup, typename dst_t = float> // METRALE: dst_t for fused bf16 output
...
#pragma unroll
    ...
    if (fixup) {
        ...
        // METRALE: direct MMA writer carries dst_t (float for llama path, bf16 for fused Metrale Engine output). GB10-only kernel ⇒ MMA always live.

...
//! `w8a16_gemv_batch16_ncol{2,4}`, behind `METRALE_ATTN_NCOL_GEMV` (#927).
//!
//! WHY. Measured on 1xH100, 2026-09-11, Qwen/Qwen3.8-27B-FP8, serve H
//! (`METRALE_FFN_NO_BATCH16=1`, graphs off, batch 16): the step is 82.3 ms, of
...
//! `METRALE_FFN_M16_TC`. The lane -> k16 map, the per-accumulator operand order
...
/// `METRALE_ATTN_NCOL_GEMV`: PRESENCE (any value, including empty) opts the
/// attention projections into the tier. `METRALE_NO_ATTN_DECODE_BATCH`
...
///
/// Presence rather than `=1` for both, matching `METRALE_FFN_M16_TC` and
/// `METRALE_FFN_NO_BATCH16` next door: `METRALE_ATTN_NCOL_GEMV=0` meaning "on" is
/// a trap, and so is `METRALE_NO_..._=0` meaning "off".
///
...
pub fn ncol_gemv_enabled() -> bool {
    ...
    // no target has a serving A/B for it. `METRALE_ATTN_NCOL_GEMV` is how that
    // A/B gets run; `METRALE_NO_ATTN_DECODE_BATCH` still outranks both. The
...
/// `METRALE_ATTN_NCOL_WIDTH=4` picks the 4-column instantiation; anything else
...
/// presence lever would make `METRALE_ATTN_NCOL_GEMV=0` mean "on, 2 columns".

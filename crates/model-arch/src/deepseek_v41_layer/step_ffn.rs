...
//! 2026-09-25: The ffn half of the eager step: the attention's `hc_post`, the
//! ffn site's mixes and norm, the MoE with its host work inline, `hc_post`,
//! the delayed-mix copy and on the last layer the final collapse. The segment
//! graphs' miss path (`step_seg.rs`) runs a layer from its ffn on through the
//! same function.
//!
...
impl DeepSeekV41Layer {
    /// 2026-09-25: The eager step from the attention's `hc_post` to the end of
    /// the layer, `m` tokens, reading the attention output from
    /// `AttnV41::out_ptr` (the buffer `forward` returns as `run.out`).

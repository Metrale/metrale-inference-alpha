...
/// transition Metrale Engine's scheduler will actually drive, and it is the one a single-step gate test
/// cannot catch.
#[test]
...
/// a library default — a coincidence Metrale Engine must not inherit.
#[test]
...
/// does not (Metrale Engine's contract, where the conv fuses the L2). Passing already-normalised
...
///
/// Metrale Engine's prefill L2 (`l2_norm_bf16`) writes **bf16**, so the chunked kernel is always fed

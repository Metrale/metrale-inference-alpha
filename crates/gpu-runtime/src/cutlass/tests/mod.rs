...
//! 2026-09-25: GPU tests for the CUTLASS wrappers, built only with
//! `cfg(metrale_cutlass)`: shared CUDA and numeric helpers here, the tests in
//! `bf16` (dense BF16, tile variants, cuBLASLt algorithms) and `nvfp4` (NVFP4
//! comparator and transpose).
//!
//! Owner: gpu-runtime.
...
pub(super) fn run_cublaslt_algo(
    ...
    let status = unsafe {
        metrale_cublaslt_bf16_gemm_act_weight_t_algo(
            act,
            weight,
            out,
            m as i32,
...
// 2026-09-25: Host-side NVFP4 helpers for the `nvfp4` tests.

/// 2026-09-25: E2M1 (FP4) magnitudes, indexed by the low 3 bits; bit 3 is the sign.
...
/// 2026-09-25: The same thresholds as `float_to_e2m1` in `cuda/cutlass_nvfp4_gemm.cu`.
...
/// 2026-09-25: Decode an E4M3 byte (sign, 4-bit exponent with bias 7, 3-bit
/// mantissa) to f32; the NaN encoding is not special-cased.
...
/// 2026-09-25: A deterministic pseudo-random value in [-0.5, 0.5) from `seed`.

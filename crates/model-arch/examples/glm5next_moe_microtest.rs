...
//! 2026-09-25: Gate for a complete GLM-5.3 routed MoE layer on checkpoint weights, stage by
//! stage (router logits, top-k, NVFP4 experts, routed mixture, shared BF16 expert, output),
//! against the golden `gen_moe_golden.py` writes.
//!
//! Owner: model-arch examples (GLM-5.3 routed MoE).
//! Invariants:
//! - The run fails if the fixture has `n_group != 1` or applies the routed scale to the output,
//!   if the shared-expert output differs between the two router modes, or if a gated stage
//!   exceeds its budget.
//!
//! The two router modes are scored separately and never pooled: `hf_fp32` keeps the router
//! logits in FP32, `vllm_bf16` rounds them to BF16. In `vllm_bf16` only the stages that do not
//! depend on expert selection are gated.
//!
//! The routed scale rides on the top-k weights and the shared expert is not multiplied by it,
//! so the shared-expert stage does not depend on routing; its equality across the two modes is
//! the built-in control.
//!
//! Set `MOE_PACKET_DIR` to the directory of layer packets; the arguments are layer indices
//! (default 3):
//!
//!   cargo run -p metrale-model-arch --release --example glm5next_moe_microtest \
...
pub(crate) static GOLDEN: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
    golden::load(
        "crates/model-arch/src/glm5next_moe_ref/moe_golden.json",
        "gen_moe_golden.py",
    )
});

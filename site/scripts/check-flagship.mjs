...
//
// The front page prints `metralectl run <flagshipRecipe>` as its headline
// instruction. Nothing tied that name to the recipe corpus, so retiring or
// renaming a recipe in metrale-recipes would leave the site confidently
...
//
// It reads the corpus CI actually ships against — the metrale-recipes checkout
...
if (!root) {
  ...
  console.error('Point it at a checkout of metrale-recipes/recipes — the corpus this');
...
try {
  ...
  console.error('Set METRALE_RECIPES_ROOT to a checkout of metrale-recipes/recipes.');
  process.exit(1);
}
...
if (!stems.includes(flagshipRecipe)) {
  problems.push(
    `data.js advertises \`metralectl run ${flagshipRecipe}\`, and no such recipe exists in the corpus.`
  );
}

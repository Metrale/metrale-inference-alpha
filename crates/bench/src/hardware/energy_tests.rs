...
//! 2026-09-26: Tests for the energy integral, its record keys and the J/token ratio.
//!
...
fn a_reading_line_parses_and_a_not_available_rail_does_not_become_zero_watts() {
    ...
    // 2026-09-26: An unreadable watt cell yields no reading, never 0 W.
    ...
    // 2026-09-26: A missing or unrecognised flag cell is unknown, not "not active".
...
/// 2026-09-26: The rule the joule count stands on: hold each reading forward to
/// the next, back-fill the first to the window start, hold the last to the
...
fn integration_holds_each_reading_forward_and_spans_exactly_the_window() {
    let t0 = Instant::now();
    // 2026-09-26: Window [100 ms, 1100 ms]; readings at 250 (10 W), 500 (20 W),
    // 750 (30 W), 1000 (40 W), plus a 999 W reading on each side outside it. Held
    // forward, the first back-filled to 100 ms and the last held to 1100 ms:
    // 10×0.4 + 20×0.25 + 30×0.25 + 40×0.1 = 20.5 J over 1.0 s.
    let samples = [
        reading(t0, 0, 999.0),
        reading(t0, 250, 10.0),
        reading(t0, 500, 20.0),
...
/// 2026-09-26: A window with no reading inside it, or with `end <= start`,
/// reports nothing rather than zero joules.
#[test]
...
/// 2026-09-26: The flag fractions distinguish "never reported" from "reported off".
#[test]
fn cap_fractions_are_over_reporting_samples_only_and_none_when_unreported() {
    ...
    // 2026-09-26: Mixed: one reading reports the brake, the other does not, and
    // the fraction is over the one that did.
...
/// 2026-09-26: Joules, seconds and samples add; the mean is re-derived; fractions
/// weight by sample count over the windows that report them.
#[test]
...
fn every_energy_key_names_the_rail_and_the_sample_count_rides_beside_the_joules() {
    ...
    // 2026-09-26: Without a baseline the above-idle key is absent, never zero.
    ...
    // 2026-09-26: J/token is the one ratio written beside joules and tokens, and
    // it is the pair's own quotient.
...
/// 2026-09-26: The ceiling-bound ratio is absent whenever it is undefined or
/// meaningless: a ceiling passes a value at or below it, so 0.0 would pass, and
/// NaN or ∞ must never reach a record. Absent makes the gate report the key as

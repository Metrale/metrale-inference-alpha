# Metrale Engine — Quickstart Guide

Run state-of-the-art language models on a single NVIDIA DGX Spark (GB10).

**Docker image:** `metrale/metrale-inference-gb10:latest`
...
By default Metrale Engine binds the HTTP listener to **`127.0.0.1` (localhost only)**.
...
met serve <model> \
  --bind 0.0.0.0 \
  --require-auth \
  --auth-tokens-file /etc/metrale/tokens.txt
```
...
> phone picture is often held sideways in the file. Metrale Engine rotates on decode, so
...
>
> Metrale Engine does **not** bundle a video decoder. Animated **GIF** decodes
...
Metrale Engine's buffer arena, dequant scratch, MoE routing state, and CUDA context, you're
...
```bash
sudo docker run -d --name metrale \
  --network host --gpus all --ipc=host \
  -v ~/.cache/huggingface:/root/.cache/huggingface \
  metrale/metrale-inference-gb10:latest \
...
> is the largest config that fits cleanly on a stock 119.7 GB DGX Spark.
...
Metrale Engine exposes a standard OpenAI-compatible API. Use it with any client:
...
For **Open WebUI**: set the base URL to `http://<dgx-spark-ip>:8888/v1` and any API key.

...
//! 2026-09-26: Tests of what `ChatState::pump` does with deltas, and of the
//! Chat view keys.
//!
//! Owner: server tui.
...
fn a_sender_that_dies_without_finishing_unsticks_the_pane() {
    // 2026-09-26: `send` refuses while `streaming` is set; a disconnected
    // channel settles the reply, which clears it.
...
fn the_thinking_clock_stops_when_the_answer_starts() {
    // 2026-09-26: The first answer token calls `Reasoning::seal`; reasoning
    // that arrives later does not move the sealed span.
    let (mut s, tx) = streaming_state();
    tx.send(ChatDelta::Reasoning("mulling".into())).expect("tx");
    s.pump();
    tx.send(ChatDelta::Token("Paris.".into())).expect("tx");
...
fn reset_clears_the_whole_session_including_a_stream_in_flight() {
    ...
    // 2026-09-26: `reset` drops the receiver, so a delta sent by the old
    // stream is never pumped into the new transcript.

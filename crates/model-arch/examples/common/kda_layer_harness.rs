...
//! 2026-09-25: Packet reader, GPU driver and regime suite of `kda_layer_microtest`.
//!
...
/// 2026-09-25: A layer packet as a [`KdaTensorSource`]: layer-relative names over the raw
/// safetensors bytes.
...
/// 2026-09-25: Host copies of a packet's layer weights for `cpu_layer`, with the q, k and v
/// conv weights concatenated in that order.
...
impl Gpu<'_> {
    /// 2026-09-25: Run one decode or prefill forward and read every stage back. `pad_fill`
    /// fills the padded q/k/v tails (`prefill_with_pad_fill`); `Glm5NextKdaLayer::prefill`
    /// passes 0.0.
    pub(crate) fn run(
        &self,
        hidden: &[f32],
...
/// 2026-09-25: The regimes decode1, prefill4, prefill7 and prefill7_decode1, then the
/// pad-corruption check, for one bound layer.
...
pub(crate) fn run_suite(
    ...
    // 2026-09-25: One seed draws hidden, then the conv window, then the recurrent state. The
    // pure pass gets the hidden before bf16 rounding.
    ...
    // 2026-09-25: Widen the drawn `ks - 1` conv slots into slots 1.. of the `ks`-slot state;
    // slot 0 is shifted out before the conv reads the window.
    ...
    // 2026-09-25: prefill7_decode1: a 7-token prefill, then one decode token on the conv and
    // recurrent state it leaves.
    {
        ...
        // 2026-09-25: Pad corruption: the same prefill with the padded positions filled with
        // 7.5 must leave the output, core, both states and the next decode bit-identical.

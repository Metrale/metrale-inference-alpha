...
#                            OUT_DIR/*_ptx.rs that metrale-kernels/spark-storage
#                            `include!`
#   build.rs                 build scripts run at compile time, not under test
#   crates/metrale-kernels/    entirely build.rs-generated PTX constants
...
#   metrale-rdma/src/verbs.rs  ibverbs FFI; `cfg(metrale_rdma_verbs)` is compiled
...
IGNORE_REGEX='(^|/)vendor/|(^|/)target/|/build\.rs$|(^|/)crates/(metrale-kernels|cufile-sys|spark-comm)/|(^|/)crates/metrale-rdma/src/verbs\.rs$|(^|/)crates/spark-model/src/layers/ops/|(^|/)crates/[^/]+/(tests|benches|examples)/'

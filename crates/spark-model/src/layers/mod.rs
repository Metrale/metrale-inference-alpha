...
///
/// `METRALE_NO_W4A16_K64=1` restores the pre-session 8192 threshold.
...
/// Resolve the `w4a16_gemm_t_m128_v2` handle honoring `METRALE_W4A16_VARIANT`.
///
...
/// (see body). `METRALE_W4A16_VARIANT=v2` opts in on all three sites at once;
...
pub fn w4a16_v2_kernel(gpu: &dyn GpuBackend) -> KernelHandle {
    ...
    // might pay; nothing auto-activates it. `METRALE_W4A16_VARIANT=v2` opts in
...
/// [`w4a16_v2_kernel`]: `METRALE_W4A16_VARIANT=v3` selects it, anything else
...
/// **ON by default**; `METRALE_NO_TGEMM_PIPELINE3` (presence — `=0` is NOT "off")
...
/// variant. **ON by default**; `METRALE_NO_K64_PIPELINE3` (presence — `=0` is NOT
...
/// kernel is absent or the presence kill switch `METRALE_NO_K64_N64` is set
...
impl FfnComponent {
    ...
    /// METRALE_FP32_ROUTING active for this FFN (MoE only; false otherwise).
    ...
    /// Whether the TARGET model's cross-row grouped FP8 MoE decode serves `m`
    /// rows here: opted in (`ModelLevers::moe_fp8_grouped_decode_target` — it
    /// is not bit-identical to the per-token loop, see there), MoE with FP8
    /// block-scaled experts, grouped kernels shipped, arena wide enough, kill
    /// switch unset. False for dense/none. The MTP drafter asks its own
    /// `MoeLayer` directly and is not behind the opt-in.

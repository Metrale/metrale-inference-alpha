...
/// (`METRALE_MTP_DRAFT_CONF`, default 0.0 = disabled). When the drafter's
...
/// its measured A/B (same discipline as METRALE_SNAP_EVICT_ALPHA).
...
/// Shadow top-k draft instrumentation (`METRALE_MTP_SHADOW_TOPK=k`, default
...
/// `METRALE_MTP_ACCEPT_DEBUG` (PRESENCE): per-BATCH-WIDTH acceptance telemetry.
///
...
/// (`METRALE_MTP_CATCHUP=1`, staged off). During serial-decode stretches the
...
/// (`METRALE_MTP_REFEED_ACCEPTED=1`, default OFF). Requires `METRALE_MTP_CATCHUP=1`.
///
...
/// existing lever touches it: `METRALE_MTP_CATCHUP=1` alone is bit-identical
/// (its ring is only written on SERIAL decode steps, and with the throughput
/// gate disarmed there are none), and dropping `METRALE_MTP_DRAFTER_PREFILL`
...
/// fingerprints (`METRALE_MTP_REFEED_DEBUG=1`, FNV-1a over each BF16 row), on
...
/// (with `METRALE_MTP_GATE_FORCE=1` the engine is bit-reproducible, so n rises
...
///    teacher-forced oracle probe, `METRALE_MTP_ORACLE_P2`): feeding draft 2 the
...
/// (`METRALE_MTP_REFEED_SHIFT`, default 0 = the derived mapping).
///
...
/// `METRALE_MTP_REFEED_DEBUG=1`: fingerprint every hidden that enters and
...
/// `METRALE_NO_MTP_EP_PROPOSE=1` restores the rank-0-only path.
///
...
///
/// 🪤 `METRALE_MTP_EP_PROPOSE=1` (the opt-in name it shipped behind for one day)
...
pub trait DraftProposer: Send + Sync {
    ...
    /// METRALE_MTP_CARRY_DRAFTER: move this sequence's drafter KV blocks OUT of

...
Verified on this branch: release build clean, `metrale-tier` 8+5+16+2+5 = 36 tests,
`metrale-model-layers model::ssm` 76 tests, `cargo clippy --tests` clean.
...
METRALE_TARGET_MODEL='*' cargo build -p metrale-server --release
```

`METRALE_TARGET_MODEL='*'` is mandatory — a single-target binary dies on any other
model. The binary is `target/release/met` (not `metrale-server`).
...
docker run -d --name combo27 --gpus all --ipc=host --network host \
  -v <binary>:/met:ro -v /tank/hf:/tank/hf \
  -e RUST_LOG=info -e METRALE_TARGET_HW=gb10 \
  -e METRALE_TARGET_MODEL=qwen3.6-27b -e METRALE_TARGET_QUANT=nvfp4 \
  -e METRALE_KV_OVERCOMMIT=1 \
  -e METRALE_SSM_TAIL_MIDCHUNK=0 -e METRALE_MTP_CATCHUP=0 \
  -e METRALE_MTP_DRAFT_CONF=0.0 -e METRALE_MTP_GATE_FORCE=1 -e METRALE_SSM_TAIL_PROTECT=1 \
  -e METRALE_SSM_TAIL_LEASE_TTL=128 \
  metrale-inference-gb10:gdnf32-build \
  /met serve <centml-snapshot> --model-name qwen3.6-27b --host 0.0.0.0 --port 8888 \
...
**MMQ W4A4 is now ON in this config (2026-07-30):** `METRALE_NO_FFN_NVFP4_MMQ=1`
...
`--max-seq-len 4096 --max-prefill-tokens 4096`, `METRALE_KV_OVERCOMMIT` removed
...
|---|---|---|---|
| `METRALE_BF16_TC_PREFILL=1` | set | **REMOVED** | crashes, see §5.1 |
...
| `METRALE_KV_OVERCOMMIT` | unset | 1 | 8x128K exceeds a strict KV reservation |
| util | 0.70 | 0.80 | ours |

### 3b. Holo-3.1-35B-A3B-NVFP4

Full flag set + rationale is in the recipe PR
(`Metrale/metrale-recipes#13`). Two things that silently degrade it:
`METRALE_HOLO_LOW_MEMORY_MOE=1` is the unlock (verify `CUTLASS grouped SFB: built
256 experts` ×40), and **KV must be `bf16`** — paged FlashInfer requires it.

### 3c. SSM spill tier (optional, #381/#382)

```bash
-e METRALE_SSM_TIER=1 -e METRALE_SSM_TIER_UNIFIED=1 \
-e METRALE_SSM_TIER_SWAP_DIR=/ssm-swap -e METRALE_SSM_TIER_DISK_GB=10 \
-e METRALE_SSM_TIER_SLOTS=8 -e METRALE_SSM_TIER_TIMING=1
```
...
`METRALE_SSM_TIER_SLOTS + 1`. A 0-byte swap file with a large `_SLOTS` is expected,
not a bug.
...
METRALE_CONTAINER=<name> python3 bench/agentic/conc_harness.py --levels 1,4,8 --model <id>
...
### 5.1 `METRALE_BF16_TC_PREFILL=1` crashes — ROOT-CAUSED to the **v2** kernel
...
let bf16_tc_prefill = self.w4a16_gemm_t_m128_bf16_k.0 != 0 && env::var_os("METRALE_BF16_TC_PREFILL").is_some();
// selection: prefers V2 whenever V2 is loaded
let use_v2 = self.w4a16_gemm_t_m128_bf16_v2_k.0 != 0 && env::var_os("METRALE_DISABLE_PREFILL_V2").is_none();
...
`METRALE_BF16_TC_PREFILL=1 METRALE_DISABLE_PREFILL_V2=1` gives the lossless BF16
...
`METRALE_BF16_TC_PREFILL=1` alone (v2, the faster variant) is expected to work —
...
`bench/bench-metrale-concurrency.py` defines 6 ISL/OSL regimes but then filters
...
### 5.3 `METRALE_SSM_TAIL_PROTECT=1` is inert in this config

(2026-08-05: the variable was renamed to the opt-out
`METRALE_DISABLE_SSM_TAIL_PROTECT` — default ON — and dropped from every launch
...
`METRALE_SSM_TAIL_MIDCHUNK != 0` — and the golden set pins `MIDCHUNK=0`. The
comment explicitly warns: *"Do not read a `METRALE_SSM_TAIL_PROTECT=1` in a launch
...
blob). `METRALE_SSM_TIER_TIMING=1`:
...
Holo, `METRALE_SSM_TIER_DISK_GB=1`, `_SLOTS=8`, agentic C=1/4/8: **350 spills**, cap
...
```
SSM spill SKIPPED (cost gate): victim depth 629 < METRALE_SSM_SPILL_MIN_TOKENS=1024 — dropped
```
...
- lower `METRALE_SSM_SPILL_MIN_TOKENS` to match the interval — but on a 151 MB blob
...
### 5.7 `bench-metrale-concurrency.py` measures PREFIX-CACHE HITS, not prefill
...
* **The KV-family stack costs ~9% on decode_short C=16**: pre-KV binary
  ...
  is METRALE_KV_TRACE-gated so it is not that. Needs a bisect inside the 12
...
Prompted by a survey of `wip-laguna-lora` / `port/lora-moe-metrale` for
transferable work (user-directed):

* `a1d889f2` (= 4046dcad, LoRA-branch peel-off): **rayon host sampling**
  (METRALE_PARALLEL_SAMPLE, default ON, n>1 only) + token-major/exact-N MoE
...
`METRALE_PARALLEL_SAMPLE=0` control on decode_short. Baselines to beat:
...
where METRALE_PARALLEL_SAMPLE lives. The rayon win applies to the NON-spec
...
is unsound by construction; `METRALE_MARCONI_EXACT=1` re-enables), `b37669b5`
...
| | C=8 | C=16 |
|---|---|---|
| Metrale Engine decode_short | 108.5 | 124-138 |
| vLLM bar | 98.8 | 168.9 |
...
2. `METRALE_PREFILL_CODISPATCH=1` defers chunk-0 into a cohort → cohort
   ...
   d. **admit/execute gate MISMATCH** (same species as §5.1): eligibility
      ...
      `prefill_inner` only honors the explicit `METRALE_Q12_BATCHED_FIRST_CHUNK`
...
4. **The wall is the 27B's cold-prefill compute rate: ~800-900 tok/s flat**
   ...
   * + `METRALE_BF16_TC_PREFILL=1` (v2, fixed this morning; 0 launch errors
   ...
   * MMQ re-enabled (drop `METRALE_NO_FFN_NVFP4_MMQ=1`): **69.7**, best TTFT
   ...
   Metrale Engine chunk_delta_h 11-13x slower than FlashInfer GDN at matched shape).
...
* m128 kernel program (ultracode, 2026-07-30): 11-agent workflow designed
  ...
  called it. Shipped as opt-in only (`METRALE_W4A16_VARIANT=v2`), commits
...
watchdogs are disabled (`METRALE_SSM_DECODE_RING=1/0` force-overrides;
...
* `METRALE_MTP_REFEED_ACCEPTED` (mined from #378 — the p1 0.72->0.90 strix
...
(`METRALE_MTP_ACCEPT_DEBUG`) is available; **"graphing the Phase-A bootstrap"
...
Kept (kill switch METRALE_MTP_KV_GEMV=1): launch-count reduction, no cost.
...
Kill switch: METRALE_NO_FAST_GREEDY_CHAT=1.
...
| C | Metrale Engine | vLLM bar | ratio | #379 published |
|---|---|---|---|---|
...
In-process phase timers (METRALE_MTP_TIMING now wired into the batched K4
...
* Default-stream entanglement theory: switching Phase-5 to
  ...
  switch METRALE_VERIFY_D2H_DEFAULT_STREAM=1) reproduced the SAME 130 ms.
...
| 4 | CUDA-graph replay quirk | METRALE_NO_MTP_VERIFY_GRAPHS (eager) | 119-121 ms (worse agg) |
...
| 8 | dashboard as second CUDA client | nvidia-smi: /met is the ONLY compute app | n/a |
...
4. **§5.6 — warn at startup when `ssm_checkpoint_interval * block_size <
   METRALE_SSM_SPILL_MIN_TOKENS`**, i.e. when the config can never spill.
5. **§5.3 — `METRALE_SSM_TAIL_PROTECT=1` is inert** under `TAIL_MIDCHUNK=0`. DONE
   (2026-08-05): dropped from every launch script and renamed to the opt-out
   `METRALE_DISABLE_SSM_TAIL_PROTECT` (default ON, `=1` disables).
...
Deliberately parked: `put_with` (evaluated and rejected in metrale#382 — only 2 of
...
**Branch** `perf/enterprise-concurrency-v3` on `metrale`, all commits signed:
...
aa233714  fix(ssm-tier): reap dead tier keys            <- metrale#382
6e68fa8b  chore(ssm-tier): clippy + LoC cap
a91390e4  perf(ssm-tier): 22x cheaper eviction          <- metrale#381
2848205c  perf(mtp): D-Cut depth pruning                <- metrale#379 head
```

Worktree `/home/ms/metrale/.claude/worktrees/combo`, tracking the remote.

**Related PRs** (all draft, all green): metrale#381 (spill tier),
metrale#382 (reaping, stacked on #381), metrale-recipes#13 (Holo-3.1-35B recipe,
from a fork — no write access to metrale-recipes). Review comment on metrale#379:
...
`v2_bisect.sh`), binaries (`met-combo` = this branch), and benchmark output
...
* `CUTLASS_HOME=/home/ms/cutlass` at build ONLY to make the CUTLASS
  ...
  the binary serves on Metrale Engine's own kernels. (Corrected 2026-08-31 — the
  earlier "or the binary refuses to serve" was never true of `metrale-server`.)
* `RUSTFLAGS="-L <dir with libnccl.so>"`; symlink `libnccl.so -> libnccl.so.2`.
* The binary is `target/release/met`, not `metrale-server`.
...
```
52222db9..75d1398e  8 commits: full metrale#373 (prefix-cache refcount family)
d36a54b8..2fe52169  3 commits: full metrale#375 (KV-exhaustion preempt)
...
metrale-gpu-runtime/model/server; **full-workspace `cargo test` on the final stack
...
`v2fix_bisect.log` (§5.1 revalidation). Binary `met-combo` = `493df3ea`
build at the time of the 6.4 run; `target/release/met` in the worktree =
full stack (post-#373) — NOT yet deployed to `met-combo`.
...
over `met-combo` while a container has it mounted — stop the container

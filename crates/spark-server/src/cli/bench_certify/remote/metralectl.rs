...
//! `metralectl bench … --json`, as a trait, and the subprocess that speaks it.
//!
//! The wire contract is metralectl's (metrale-recipes `docs/BENCH.md`): every
...
/// What metralectl answered when it did not do the thing: the exit class and
...
/// The real thing: `metralectl` on PATH or as named.
pub struct SubprocessMetralectl {
    pub exe: PathBuf,
}

impl SubprocessMetralectl {
    /// Find the binary. Absent → an error naming `--metralectl`.

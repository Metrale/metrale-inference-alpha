...
//! 2026-09-26: The model-variant step: which checkpoint the selected benchmark runs on.
//!
//! Owner: server tui.
//! Invariants: none beyond the types.
//!
//! The variants are listed on the left; the detail pane on the right shows the
//! selected variant's committed thresholds, its recipe and its `note`.
...
fn draw_rows(f: &mut Frame, app: &App, area: Rect) {
    ...
    // 2026-09-26: The scroll offset counts entries of `ROWS_PER_ENTRY` rows
    // and keeps the selected variant on screen.
...
/// 2026-09-26: A variant's chips: `default` when it is the gate's default
/// checkpoint, its hardware class, and its recipe's last path segment.
fn chips(row: &VariantRow) -> Vec<Span<'static>> {
    ...
    if row.is_default {
        // 2026-09-26: The checkpoint the gate runs when none is named; first,
        // in brand green.
...
fn draw_detail(f: &mut Frame, app: &App, area: Rect) {
    ...
    lines.push(Line::default());
    // 2026-09-26: The entry's `note`, which says why these are the
    // thresholds.

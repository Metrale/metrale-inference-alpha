# Porting Metrale Engine to AMD Strix Halo (gfx1151) via SCALE
...
> `cwsr_size` from sysfs and fixes it. See `metrale-issues-found.md`.
...
runtime/driver/math APIs for AMD. The Metrale Engine philosophy here mirrors Spectral's
...
export METRALE_TARGET_HW=strix METRALE_TARGET_MODEL=qwen3.6-27b METRALE_TARGET_QUANT=nvfp4
...
# Serve — one runtime shim on gfx1151 (see §4). METRALE_FORCE_GLOBAL_GDN and
# METRALE_NO_FP8_PREDEQUANT, which §4 also describes, are read by no code in
# this tree, and `met serve` refuses an undeclared METRALE_* variable.
export METRALE_W4A16_VARIANT=v1       # use the BF16-MMA NVFP4 GEMM (SCALE FP8-MMA encode is broken on gfx1151)
...
target/release/met serve Qwen/Qwen3.6-27B-FP8 \
...
`SCALE_HOME` is honored by the Metrale Engine build (`find_scale_dir`). A SCALE root
...
- **No `--ptx`.** SCALE rejects `--ptx`, `--genco`, `-fatbin`, `--emit-llvm`.
  It emits an **AMD GPU code object** (ELF relocatable), not PTX text. Metrale Engine's
...
- **LDS (8 `attn_prefill*` files):** `__shared__` footprint
  (e.g. `attn_prefill` = 70,400 B: Q 16896 + K×2 33792 + V 16896 +
...
`crates/kernels/build_target.rs` already abstracts the compiler behind
...
3. Runtime: add `KernelModule::from_binary(ctx,&[u8])` beside
   `from_ptx_src`. `metrale-core/src/registry.rs` already has the raw
...
Metrale Engine's model = embed PTX text, `cuModuleLoadData` at runtime (driver JIT),
...
code into the binary, auto-registered, launched by C++ symbol. **Two paths:**
  1. **SCALE-native (lower risk):** AMD build compiles kernels into the
     ...
     instead of `cuModuleLoadData`(blob). Bigger metrale-core/metrale-gpu-runtime
     change but uses SCALE exactly as designed.
  2. **Metrale-style:** device-link relocatables → a loadable code object,
...
### 4.2 LDS > 64 KB — 8 `attn_prefill*` files
...
`attn_prefill` 70,400 → 53,504 ≤ 65,536) or smaller `BR`/`BC` tile.
...
export LD_LIBRARY_PATH="/opt/rocm/lib:$SCALE_HOME/targets/gfx1151/lib:$LD_LIBRARY_PATH"
export CUDARC_CUDA_VERSION=12080
rm -rf target/release/build/metrale-kernels-*      # stale-cache guard on .cu change
cargo build --release -p metrale-server --no-default-features --features cuda

# Serve — gfx1151 shims (§4) + SCALE libs first so /opt/rocm can't shadow libhsa:
export METRALE_W4A16_VARIANT=v1
export LD_LIBRARY_PATH="$SCALE_HOME/targets/gfx1151/lib:$SCALE_HOME/lib"
target/release/met serve Qwen/Qwen3.6-27B-FP8 \
  --port 8081 --max-seq-len 4096 --gpu-memory-utilization 0.70 \
  --kv-cache-dtype bf16 --kv-high-precision-layers max --max-batch-size 4

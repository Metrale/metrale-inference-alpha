...
**Image**: metrale-test:latest (built from spec_ssm + uncommitted fixes)
...
### Launch Command
```bash
sudo docker run -d --name metrale-122b --gpus all --ipc=host --network host \
  -v ~/.cache/huggingface:/root/.cache/huggingface \
  metrale-test:latest serve Sehyo/Qwen3.5-122B-A10B-NVFP4 \
...
### Launch Command
```bash
sudo docker run -d --name metrale-mistral --gpus all --ipc=host --network host \
  -v ~/.cache/huggingface:/root/.cache/huggingface \
  metrale-test:latest serve mistralai/Mistral-Small-4-119B-2603-NVFP4 \
...
**Confirmed on**: BOTH metrale-test:latest AND metrale/metrale-alpha-2.7 (both built from pre-release code with the bug)
...
**Fix**: `crates/model-arch/src/mistral_loader/loader_impl/yarn.rs` now correctly implements
...
### Launch Command
```bash
sudo docker run -d --name metrale-nemotron --gpus all --ipc=host --network host \
  -v ~/.cache/huggingface:/root/.cache/huggingface \
  metrale-test:latest serve nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4 \
    --port 8888 --kv-cache-dtype fp8 --kv-high-precision-layers auto \
    --gpu-memory-utilization 0.92 --scheduling-policy slai \
    --max-seq-len 65536 --tool-call-parser qwen3_coder --ssm-cache-slots 0
```
...
**Verified**: `crates/model-arch/src/mistral_loader/loader_impl/yarn.rs` implements the
...
- `crates/model-layers/src/layers/qwen3_attention/prefill/paged_mla.rs`: K/V stride uses
...
- `crates/model-layers/src/layers/qwen3_attention/prefill/cache_skip_mla.rs`: same scale,
  uses `prefill_attention_64` (BR=64 tile) instead of `prefill_attention`; no correctness gap.
- `crates/server/src/main_modules/kv_dtypes.rs`: `build_layer_kv_dtypes(BF16, ...)` returns
...
**Verified**: two independent pool types exist in `crates/model-engine/src/model/`:
...
`crates/server/src/main_modules/serve_phases/preflight.rs` correctly projects both

...
impl ResponseStore {
    /// Build from env.
    /// - `METRALE_STORE_MAX_ENTRIES` (default 10 000)
    /// - `METRALE_STORE_TTL_SECONDS` (default 86 400)
    /// - `METRALE_STORE_DIR` — when set, enable filesystem persistence
    ...
    /// through to the default in silence, so `METRALE_STORE_TTL_SECONDS=1h` was

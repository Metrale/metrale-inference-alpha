...
export const stamp = `metrale ${bench.generated_sha} · ${ladder.generated_utc.slice(0, 10)}`;
...
export const claim = {
  ...
  // The driver hash the published Metrale Engine legs carry, beside the one a reader will
  // actually get from the tree. Both derived: the first is the manifest key whose
  // note names the Metrale Engine legs, the second is hashed from the file at build time.
  harnessShaMetrale: Object.keys(ladder.harness_shas).find((k) =>
    /Metrale Engine legs/i.test(ladder.harness_shas[k])
  ),
...
//
// It used to be a shortened list with a note pointing at series[metrale].cli for
...
export const fingerprint = [
  ...
  ['harness', ladder.workload.harness, `campaign driver for the published ladder, sha256 pinned per leg; the gate's own instrument is \`met benchmark run concurrency-sweep\``],
  ['metrale', `${subject.engine} @ ${subject.build}`, subject.build_note],
...
export const audit = [
  ...
  },
  {
    ...
    // published pair IS the better Metrale Engine number (41.02, over round 11's 40.42)
  ...
  },
  {
    ...
    answer: 'A fleet-wide shift cost Metrale Engine 4.2% and vLLM 2.2% at C=32. The margin narrowed and held; the differential is published rather than the favourable snapshot.'
  },
  {
    state: 'open',

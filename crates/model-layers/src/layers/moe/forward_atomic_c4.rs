...
//! 2026-09-25: `MoeLayer::forward_atomic_c4_decode`, the opt-in 4-row MoE decode
//! with FP32 atomic accumulation of the routed down projections.
//!
...
impl MoeLayer {
    /// 2026-09-25: 4-row NVFP4 MoE decode, reached from
    /// `qwen3_ssm/trait_decode_multi_seq.rs` when `METRALE_MOE_ATOMIC_C4_DECODE=1`.
    ///
    /// The gate GEMM and top-k run batched. Gate+up uses the token-major
    /// kernel, then the routed down projections atomic-add their weighted FP32
    /// results into a `[num_tokens, H]` accumulator in `scratch`. The finalize
    /// kernel casts it to BF16 and blends the shared expert, except under EP,
    /// where the shared expert is blended once after the all-reduce.
    ///
    /// Falls back to `forward_batched` unless `num_tokens == 4`, both atomic
    /// kernels are present, the experts are NVFP4 (no BF16 or FP8 expert
    /// tables, no BF16 shared expert) in the non-transposed decode layout,
    /// there is no pre-expert norm, and a shared expert exists.
    pub fn forward_atomic_c4_decode(
        ...
        anyhow::ensure!(
            ...
            "scratch too small for METRALE_MOE_ATOMIC_C4_DECODE: need {} bytes, have {}",

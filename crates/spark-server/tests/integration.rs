...
//! `METRALE_INTEGRATION_MODEL_DIR` to the absolute path of a HuggingFace
...
/// Default snapshot used when `METRALE_INTEGRATION_MODEL_DIR` is unset.
...
/// Resolve the model dir from `METRALE_INTEGRATION_MODEL_DIR` (env, with
...
fn setup_model(
    ...
    metrale_core::config::ModelConfig,
)> {
    ...
    let mut config = metrale_core::config::parse_config(&config_json)?;
    tracing::info!(
    ...
    let gpu: Box<dyn spark_runtime::gpu::GpuBackend> = Box::new(
        spark_runtime::cuda_backend::MetraleCudaBackend::new(0, &ptx_modules)?,
    );
...
#[test]
#[ignore] // Requires GPU + model weights (METRALE_INTEGRATION_MODEL_DIR)
...
/// Free device memory according to the SYSTEM, not to Metrale Engine.
///

...
use anyhow::Result;
use metrale_core::config::{LayerType, ModelConfig};
...
/// `METRALE_GGUF_NATIVE_Q2=1`), else `None`. When `None` for every projection the
...
/// is the better dense runtime. `METRALE_DENSE_FP8=1` opts in for that kernel work.
...
impl ModelWeightLoader for Qwen35DenseWeightLoader {
    ...
    fn load_layers(
        ...
        // commit 3ebc08a). Metrale Engine's prior SSM in_proj_qkv path was
        ...
        // Originally env-gated `METRALE_FP8_SSM_PREFILL=1`; promoted to
        ...
        // METRALE_NO_GDN_FP8_PREFILL restores the pre-policy behavior for A/B.
        ...
        // METRALE_MEM_PROFILE: per-phase GPU-free trace to pin the strix/APU
        ...
        for (i, lt) in layer_types.iter().enumerate() {
            ...
            // Native keep-packed ternary Q2_0 (METRALE_GGUF_NATIVE_Q2=1): when the
            ...
            // METRALE_FFN_MMQ: eagerly materialize Q4_K + free the dead `_t` copies at load,
            ...
            // METRALE_FFN_NVFP4_MMQ: same discipline for the W4A4 FP4-MMQ arm — repack
            ...
            match lt {
                LayerType::FullAttention => {
                    ...
                    // by METRALE_GGUF_NATIVE_Q2 (else these tensors are BF16 and
                    // `attn_q2` is false). METRALE_NO_Q2_ATTN forces the BF16 path
                    // for A/B bisection.
                    ...
                    let (attn, q_nvfp4, k_nvfp4, v_nvfp4) = match variant {
                        ...
                        Nvfp4Variant::Standard | Nvfp4Variant::Fp8Dequanted if !attn_nvfp4 => {
                            ...
                            // transposed twins only under `METRALE_CUTLASS_NVFP4_*`
                            // and the base `o_proj` only under `METRALE_ATTN_W4A4`,
                        ...
                        Nvfp4Variant::Standard | Nvfp4Variant::Fp8Dequanted => {
                            ...
                            // second copy of every projection (Metrale Engine issue #A1).
                    ...
                    // NVFP4 (`METRALE_CUTLASS_NVFP4_*`, `METRALE_ATTN_W4A4`) are
                ...
                LayerType::LinearAttention => {
                    ...
                    // METRALE_NO_Q2_GDN forces the BF16/NVFP4 path for A/B bisection.
                    ...
                    // gate (not a user choice; mirrors the `METRALE_NO_*` debug
                    ...
                    // (`METRALE_FP8_ROWWISE=1`). Read from the store BEFORE the
                    ...
                    // rather than leaking them for the layer's lifetime (Metrale Engine issue #A1).
                    ...
                    if gdn_bf16 {
                        ...
                        layer.out_proj_dense = Some(out_proj_dense);
                        tracing::info!(
                            "SSM[{lp}] METRALE_GDN_BF16_WEIGHTS: qkvz + out_proj kept BF16 \
                    ...
                    // the largest SSM tensor across every layer (Metrale Engine issue #A1).
                    gpu.free(qkvz_dense.weight)?;
                    gpu.free(out_proj_dense.weight)?;
                    ...
                    if qkvz_rowwise.is_some() {
                        ...
                        if i == 0 {
                            tracing::info!(
                                "SSM[{lp}] METRALE_FP8_ROWWISE: qkvz + out_proj prefill via \

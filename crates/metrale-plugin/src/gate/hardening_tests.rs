...
/// `crates/metrale-plugin/src/gate` prefix, and `BOUNDARY_FILES` listed only
...
/// unrelated `metrale-kernels` file as invalidating; it read red by accident,
/// not by rule.
///
...
fn gate_sources_are_all_classified() {
    ...
    for entry in std::fs::read_dir(&gate_dir).expect("gate dir listable") {
        ...
        on_disk.insert(format!("crates/metrale-plugin/src/gate/{name}"));
    }

    let boundary: std::collections::BTreeSet<&str> = super::coverage::BOUNDARY_FILES
        .iter()
        .copied()
        .filter(|p| p.starts_with("crates/metrale-plugin/src/gate/"))
        .collect();
...
fn every_verdict_symbol_is_defined_inside_the_boundary() {
    ...
    for symbol in verdict_fns {
        ...
        for entry in std::fs::read_dir(&gate_dir).expect("gate dir listable") {
            ...
            if declares_symbol {
                let rel = format!(
                    "crates/metrale-plugin/src/gate/{}",

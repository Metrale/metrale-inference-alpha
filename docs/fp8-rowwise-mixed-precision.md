...
Metrale Engine serves the FP8 group by **dequantising to BF16 and re-quantising to NVFP4** —
...
* `METRALE_GDN_BF16_WEIGHTS=1` — the BF16 mitigation for the GDN half, default-off,
...
`METRALE_GDN_BF16_WEIGHTS=1` vs baseline, unsloth/Qwen3.8-27B-NVFP4, same flags:
...
`METRALE_FP8_ROWWISE=1` works and is measured. Getting there killed the
...
**Control**: the BLOCK-scaled `Qwen/Qwen3.8-27B-FP8` with `METRALE_CUBLAS_FP8=1`,
...
**Its sibling is worse**: `METRALE_FP8_W8A8=1` (block-scaled cuBLASLt) passes the
...
(`METRALE_CUBLAS_FP8` appears exactly once, its own definition). Its docstring's
...
| | baseline | `METRALE_FP8_ROWWISE=1` | `METRALE_GDN_BF16_WEIGHTS=1` |
|---|---|---|---|
...
| `METRALE_GDN_BF16_WEIGHTS=1` + cuBLASLt | 5.08 tok/s (−3.4%) |
| `METRALE_FP8_ROWWISE=1` (NVFP4 decode kept) | ~5.3, unchanged |
...
`GemmDispatch::from_env` turns each on only with `METRALE_*=1`, and none were
...
(on unless `METRALE_FP8_SINGLE_SCALE=1`).

That is not a flaw in the A/B: **the qwen3.6-27b and qwen3.8-27b serving
recipes set no `METRALE_*` either**, so this is exactly the config the gates and
...
* **The documented prefill stack for this family is GDN, not CUTLASS.** The
  ...
  `METRALE_FFN_NVFP4_MMQ=1` and `METRALE_GDN_REGRESIDENT=1`, and when it was
  written it also enabled an opt-in FlashInfer GDN prefill loaded from a prebuilt
  library, worth +17-20% there. That path has since been removed from the engine
  (2026-09-24); while it existed it FAILED OPEN — without its library it silently
  fell back, costing 40-50% of prefill at concurrency, which C=1 cannot see.
...
* GDN — `METRALE_GDN_REGRESIDENT=1` plus the opt-in FlashInfer GDN prefill from a
  prebuilt library. **Verified engaged** from the boot log at the time, not the
  silent fallback. That prefill path has since been removed (2026-09-24), so the
  "GDN stack" and "GDN + CUTLASS" legs are historical and cannot be re-run as
  written.
* CUTLASS — `METRALE_CUTLASS_NVFP4_GEMM=1` plus `METRALE_CUTLASS_NVFP4_SSM_OUT=1`,
...
2. **Prefill-only wiring first.** Load the mixed-precision GDN projections via
   ...
   `METRALE_FP8_ROWWISE=1`, default-off.

#!/usr/bin/env python3
"""
Metrale Engine multi-model test orchestrator.
...
Configuration via env vars (all optional, sensible single-node defaults):
  METRALE_IMAGE        Docker image tag (default: metrale-gb10:latest)
  METRALE_HEAD_IP      IP of head node (default: 127.0.0.1)
  METRALE_WORKER_IP    IP of worker node (default: 127.0.0.1; same as head for single-node)
  METRALE_HF_CACHE     HuggingFace cache path (default: ~/.cache/huggingface)

Run: python3 tests/run_all_models.py 2>&1 | tee /tmp/metrale-full-run.log
"""
...
IMAGE = os.environ.get("METRALE_IMAGE", "metrale-gb10:latest")
HEAD_IP = os.environ.get("METRALE_HEAD_IP", "127.0.0.1")
WORKER_IP = os.environ.get("METRALE_WORKER_IP", "127.0.0.1")
HEAD_PORT = int(os.environ.get("METRALE_HEAD_PORT", "8888"))
WORKER_PORT = int(os.environ.get("METRALE_WORKER_PORT", "8888"))
# HF cache may live in different paths on each node. Override per-host
# via METRALE_HF_CACHE_HEAD / METRALE_HF_CACHE_WORKER if needed; otherwise
# both default to the user's standard ~/.cache/huggingface.
_default_hf_cache = os.path.expanduser("~/.cache/huggingface")
HF_CACHE_HEAD = os.environ.get("METRALE_HF_CACHE_HEAD", _default_hf_cache)
HF_CACHE_WORKER = os.environ.get("METRALE_HF_CACHE_WORKER", _default_hf_cache)
STARTUP_TIMEOUT = 600  # seconds

# Address the served HTTP listener binds to. Metrale Engine's `--bind` defaults to
...
class TestSpec:
    ...
    # Only set on checkpoints that actually carry a vision tower AND whose Metrale Engine
    ...
    # Metrale Engine auto-derives world_size from tp×ep when world_size<=1; on the
...
# also routes experts across {0,1}. Metrale Engine's TP Phase-1 work (commit 8bd91ba
...
# exactly 2 ranks (head + one worker via METRALE_WORKER_IP), so it CANNOT bring
...
# smoke test runs via /home/cluster/launch-metrale-ep4.sh (see the notavault-metrale
# notes / docs/DEPLOYMENT.md). Once the driver gains N-rank support, set
# METRALE_ENABLE_EP4=1 to execute these here.
...
def start_container(host: str, spec: TestSpec, port: int) -> str:
    ...
    name = f"metrale-test-{spec.label}"
...
def ready_marker(port: int) -> str:
    ...
    #   `Metrale Engine is listening on 0.0.0.0:8888 — reachable from any host ...`
...
def build_ep2_serve_cmd(spec: TestSpec, rank: int) -> str:
    ...
    (world_size=2, ep_size=2 implied by Metrale Engine's auto-derive logic).
    """
    ...
    # uses port 0 (no HTTP). Metrale Engine auto-derives world_size from tp_size /
...
def start_ep2(spec: TestSpec) -> tuple:
    ...
    rank0_name = f"metrale-ep0-{spec.label}"
    rank1_name = f"metrale-ep1-{spec.label}"
...
def main():
    ...
    run_ep4 = bool(EP4_ROUNDS) and os.environ.get("METRALE_ENABLE_EP4") == "1"
    ...
    # and passes --tp-size/--ep-size to Metrale Engine; the run_ep2_round() driver
    ...
    # smoke test is /home/cluster/launch-metrale-ep4.sh. Opt in with METRALE_ENABLE_EP4=1
    # ONLY after run_ep2_round is generalized to N ranks (separate change).
    if EP4_ROUNDS:
        if os.environ.get("METRALE_ENABLE_EP4") == "1":
            ...
            print(f"\n[skip] EP=4 round(s) [{labels}]: need a 4-node EP=4 deployment "
                  f"(this harness launches 2 ranks). Run /home/cluster/launch-metrale-ep4.sh, "
                  f"or set METRALE_ENABLE_EP4=1 after the driver gains N-rank support.")

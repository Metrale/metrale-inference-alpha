...
- **TensorRT-LLM** documentation and source. Metrale Engine's TRT-LLM benchmark comparisons in `docs/METRALE_SPARK_JOURNEY.md` are informed by reading the TRT-LLM codebase; that journey records the 29.6 tok/s ceiling for NVFP4 on SM121 TRT-LLM.
...
For the broader research context that informed Metrale Engine's direction, see `docs/metrale-spark-research-articles.md` in the repo — a rolling curated list that's longer and more current than this page.

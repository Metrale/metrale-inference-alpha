#!/usr/bin/env python3
# 2026-09-26: Per-layer cosine of three dump pairs (FP8-dequant HF vs unquantised HF,
# engine vs each), with a verdict written to PHASE2A_VERDICT.md.
#
# Owner: bench, FP8 dequant audit.
# Invariants: none beyond the types.
"""Phase 2a three-way cosine comparison.

Decomposes the 0.967 Metrale-vs-HF[BF16-unquant] gap into:

  A. HF[FP8->BF16] vs HF[BF16-unquant]  -> inherent FP8 quant loss (the CEILING)
  B. Metrale Engine[FP8]    vs HF[BF16-unquant]  -> total drift (already known, ~0.967)
  C. Metrale Engine[FP8]    vs HF[FP8->BF16]     -> Metrale Engine compute-side fidelity

The exit gate:

  If A mean ~= 0.99x and matches B closely => Metrale Engine is at the ceiling, work the
       symptoms downstream (Phase 2d).
  If A mean >> B (e.g. A=0.998, B=0.967) => Metrale Engine has Compute headroom; close
       it via Phase 2b BF16 rounding patch.

Reuses /tmp/cosine_compare.py's cmp_pair() math. Writes a final markdown
verdict under /workspace/metrale-dumps/fp8dequant/PHASE2A_VERDICT.md.
"""
...
def main() -> None:
    ...
    for i in range(N_LAYERS):
        metrale_p = NUMDRIFT / f"metrale_L{i}.bin"
        ...
        rB = cmp_pair(metrale, hf_unquant)
        rC = cmp_pair(metrale, hf_fp8dq)
    ...
    sB = summarize("B: Metrale Engine vs HF[unquant]        ", cosines_B)
    sC = summarize("C: Metrale Engine vs HF[FP8->BF16]      ", cosines_C)
    print()
    ...
    print(
        ...
        f"Metrale Engine compute=C mean={actual:.5f}, headroom={delta:+.5f}) ==="
    )
    if delta < 0.001:
        verdict = (
            "Metrale Engine is AT the inherent FP8 ceiling. No fixable compute drift; "
            "proceed to Phase 2d (sampling-side symptom hardening)."
        )
    elif delta < 0.01:
        verdict = (
            "Metrale Engine is close to the ceiling but has minor headroom. Phase 2b "
            "rounding patch worth trying; expect modest gain."
        )
    else:
        verdict = (
            f"Metrale Engine has substantial compute headroom ({delta:.4f} cos). "
            "Phase 2b BF16 round-to-nearest-even patch is the right next step; "
            "expect Metrale-vs-ceiling gap to close significantly."
        )
    print(verdict)
    ...
    with md_path.open("w") as f:
        ...
        f.write(f"- **B** mean cosine `Metrale Engine[FP8]    vs HF[unquant]`  = `{sB['mean']:.6f}` (min L{sB.get('min_layer')}: {sB['min']:.6f})\n")
        f.write(f"- **C** mean cosine `Metrale Engine[FP8]    vs HF[FP8->BF16]` = `{sC['mean']:.6f}` (min L{sC.get('min_layer')}: {sC['min']:.6f})\n")

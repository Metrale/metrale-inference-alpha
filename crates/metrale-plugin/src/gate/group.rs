...
//! the fleet it has (`met bench certify --shards N`), and the verdict
...
//! | `METRALE_NO_TAIL_SPLIT=1` (snapshot producer off) | 4 |
//! | `METRALE_MARCONI_MIN_TOKENS=1e8` (consumer off) | 2 |
//! | `--hermetic` (#981) | 0 |
//!
...
//! against ONE server (`met benchmark run --serve-reuse`, verified to be
...
//!
//! `--hermetic` remains available (`met serve --hermetic` closes the prefix
...
/// that measured the draw chose (`met bench certify --shards N`, sized to

...
`crates/metrale-kernels/build.rs` matches on `METRALE_SKIP_BUILD` only — so the
...
vendored `cudarc`, the pure-FFI crates (`metrale-kernels`, `cufile-sys`,
`spark-comm`, `metrale-rdma/src/verbs.rs`), the `layers/ops/` kernel-launch
...
same run (`xgrammar` 87%, `metrale-plugin` 75%, `metrale-core` 54%). Read the
...
2. Declare the target's SERVING levers in `kernels/<hardware>/HARDWARE.toml`
   ...
   `crates/metrale-kernels/build_defaults.rs` panics on an unknown key. Adding a
...
4. Add benchmark shapes to `crates/metrale-spark-bench/`
...
Add new shapes and configurations to `crates/metrale-spark-bench/`. More data points help us find optimization opportunities.

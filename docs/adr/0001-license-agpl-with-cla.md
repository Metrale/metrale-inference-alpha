...
**Status:** Superseded by [ADR-0017](0017-license-mit-or-apache.md)
**Date:** 2026-04-17

## Context

Metrale Engine is a from-scratch CUDA inference engine that's competitive with
...
1. **MIT / Apache-2.0** — maximally permissive. Anyone can fork, rebrand,
   sell hosted Metrale Engine without contributing back. Disqualified: we want
...
2. **GPL-3.0** — strong copyleft, but the network-use loophole means a SaaS
   provider can run modified Metrale Engine internally without ever distributing the
   binary, and thus owes nothing back.
3. **AGPL-3.0** — closes the SaaS loophole: if you offer Metrale-as-a-service
...
- Metrale Engine ships under **AGPL-3.0-only** (`LICENSE` in repo root, SPDX headers
  on every source file enforced by `.licenserc.yaml` + a CI job).
- Every contributor signs the Metrale Engine **CLA** (`CLA.md`) before their first
  PR is merged. The CLA grants Metrale Engine an irrevocable license to redistribute
  the contribution under any future Metrale-chosen license, while leaving
...
- AGPL-3.0 is incompatible with several permissive ecosystems
  (e.g. you cannot embed Metrale Engine into a permissively licensed library
  without re-licensing the whole thing). Users who want Metrale Engine as a

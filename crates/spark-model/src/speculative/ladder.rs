...
//! the rung. `METRALE_MTP_STATIC_RUNG` (PRESENCE) pins the static value here;
//! so does an explicit `METRALE_MTP_K_LADDER`.
//!
...
//!
//! Overrides:
//! * `METRALE_MTP_K_LADDER="4:3,8:2,16:1"` — comma-separated `n_max:drafts`
...
//! * `METRALE_NO_MTP_K_LADDER` — PRESENCE check (house convention, `=0` is
...
/// PRESENCE check for `METRALE_NO_MTP_K_LADDER`. Read once per process.
...
/// to the default ladder when `METRALE_MTP_K_LADDER` is unset or unparseable
...
fn mtp_ladder_steps() -> &'static [(usize, usize)] {
    ...
    STEPS.get_or_init(|| {
        ...
        // `METRALE_MTP_K_LADDER="4:3,8:3,16:2,24:2,32:2"` is now a measurable
        ...
        // the shape is visible in `METRALE_MTP_K_LADDER` terms; dispatch above
        ...
        // `METRALE_MTP_STATIC_RUNG`). Restore the old static rung with
        // `METRALE_MTP_K_LADDER="4:3,8:3,16:2,32:1"` — an explicit ladder also
...
/// SSOT for the multi-sequence MTP cap (`METRALE_MTP_MAX_SEQS`; default 32
/// with the K-vs-batch ladder, 4 under `METRALE_NO_MTP_K_LADDER`).
...
/// `METRALE_NO_MTP_K_LADDER` (presence) restores fixed K=4 + cap 4 — the
/// dafd990d adaptive policy. Set `METRALE_MTP_MAX_SEQS=1` to restore
...
pub fn mtp_max_seqs() -> usize {
    ...
    *N.get_or_init(|| {
        std::env::var("METRALE_MTP_MAX_SEQS")
            ...
            // `METRALE_MTP_MAX_SEQS=16` to restore the wave-9 cap (spec off
            // above n=16).
...
mod tests {
    ...
    // does not set METRALE_MTP_K_LADDER / METRALE_NO_MTP_K_LADDER — CI does not).
    #[test]
    fn default_ladder_holds_depth_to_the_cap() {
        ...
        // METRALE_MTP_K_LADDER="4:3,8:3,16:2,32:1".
    ...
    // (the 8:2 shape stays reachable via METRALE_MTP_K_LADDER).
    #[test]

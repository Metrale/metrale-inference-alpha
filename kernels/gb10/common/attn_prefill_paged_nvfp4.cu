...
// 2026-09-25: Kernels `attn_prefill_paged_nvfp4` and `_64`: paged prefill flash attention over an NVFP4 KV cache.
//
// Each cache block (stride `nvfp4_bsb` bytes) holds E2M1 values, two per byte, then from byte `nvfp4_dsb` one FP8 E4M3
// scale per 16 values. The loader below dequantizes to BF16 while it fills shared memory; the body is
...
#define KERNEL_NAME attn_prefill_paged_nvfp4

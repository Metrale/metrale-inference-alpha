...
printf '\033[1;33m[metrale]\033[0m %s\n' "quickstart.sh has been replaced." >&2
cat >&2 <<'MSG'

  Metrale Engine is now launched with `metralectl`, which replaces sparkrun.
  ...
  Why the change: sparkrun redirects the Metrale Engine recipe registry to a repository
  Metrale Engine does not control, and marks it trusted — which lets recipe-supplied
  shell commands run on your host. If you have sparkrun installed, run
  `metralectl doctor` after installing, or see:

      https://github.com/Metrale/metrale-recipes/blob/main/SECURITY.md

...
/// fast path. Kill switch: `METRALE_NO_THINKENDED_GPU_ARGMAX=1`.
fn think_ended_gpu_argmax_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        std::env::var("METRALE_NO_THINKENDED_GPU_ARGMAX")
            .ok()
            .as_deref()
            != Some("1")
    })
...
fn parallel_sample_enabled() -> bool {
    ...
    *CACHED.get_or_init(|| std::env::var("METRALE_PARALLEL_SAMPLE").as_deref() != Ok("0"))
}

/// DIAG (METRALE_DECODE_TIMING=1): localize the host-path decode cost. Splits the
...
pub fn process_decode_logits(
    ...
    let new_tokens: Vec<(u32, Option<crate::api::TokenLogprobs>)> = if let Some(t) = fast_tokens {
        ...
        // is also gated off when the opt-in `METRALE_LOGIT_DUMP` diagnostic is
    ...
    for (i, (tok, logprobs)) in new_tokens.into_iter().enumerate() {
        ...
        if tool_call_start_token == Some(tok) && !a.inside_thinking {
            ...
            // only stop path is the METRALE_TOOL_EOS_ESCAPE hatch — which a
        ...
        // model's natural EOS ends the turn. Inert unless METRALE_TOOL_EOS_ESCAPE=1.

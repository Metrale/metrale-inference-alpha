...
// forked-from: kernels/gb10/common/paged_decode_attn_nvfp4.cu (2026-09-24; 71 of 581 lines differ, see kernels/FORKS.md)

// 2026-09-25: Paged decode attention over an NVFP4 KV cache at HDIM 512: E2M1 codes, two per byte with the
// low nibble first, and one E4M3 scale per 16 values; there is no k/v scale argument and no sliding window.
//
// Owner: gb10 kernels (deepseek-v4-flash).
// Invariants: none beyond the types. Every kernel assumes head_dim == HDIM, and a sequence with
// seq_len == 0 returns before any write, so its O rows keep their bytes.
// A cache block holds token-major head rows of head_dim / 2 bytes, then the scale bytes from data_section_bytes.
...
// 2026-09-25: One 8-warp CTA per (q_head, seq): grid (num_q_heads, num_seqs), block 256 (ops::paged_decode_attn_nvfp4).
...
// 2026-09-25: Split-K: split y covers [y * ceil(seq_len / num_splits), the next split's start) and writes its
// unnormalised o, then m, then l, to workspace [num_seqs, num_q_heads, num_splits, head_dim + 2]. Grid
// (num_q_heads, num_splits, num_seqs), block 256.
...
// 2026-09-25: Merges splits 1..num_splits-1 into split 0, skipping l <= 0, and writes BF16 O [num_seqs, num_q_heads,
// head_dim]. Grid (num_q_heads, num_seqs), one 32-thread block each, VEC_BF16 values per lane.

...
//! 2026-09-26: An ignored CPU diagnostic on the tokenizer file at
//! `QWEN_TOKENIZER_JSON`. For the qwen3_coder grammar in both modes it
//! checks that a one-thread and a four-thread prewarm produce the same
//! masks, and that both matchers accept a native call token by token,
//! and prints the timings. It reads a local file and downloads nothing.
//!
//! Owner: server (grammar) tests.
//! Invariants: none beyond the types.

use super::*;
use metrale_grammar::compiler::{CompiledGrammar, CompiledGrammarImpl, RuleLevelCache};
    ...
    for auto in [true, false] {
        ...
        if let Ok(dir) = std::env::var("QWEN_GRAMMAR_EXPORT_DIR") {
            std::fs::write(
                ...
                metrale_grammar::grammar::print_grammar(source.grammar()),
            )
            .unwrap();
            std::fs::write(
        ...
        for rep in 0..1 {
            ...
            // 2026-09-26: In required mode both reject an unknown function
            // name.
            if !auto {
                a.reset();
                b.reset();

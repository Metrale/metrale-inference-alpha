...
(produced by dequant_to_bf16.py), runs forward on Metrale Engine's exact 18920-token
prompt (from /tmp/metrale_tokens.json), and dumps per-layer last-token hidden
states to /workspace/metrale-dumps/fp8dequant/hf_L{0..39}.bin matching the
Phase alpha layout. Then cosine_compare.py reads these against
/workspace/metrale-dumps/numdrift/hf_L*.bin (the unquantized BF16 reference)
...
TOKENS_PATH = pathlib.Path("/tmp/metrale_tokens.json")
OUT_DIR = pathlib.Path("/workspace/metrale-dumps/fp8dequant")
...
def main() -> None:
    ...
    print(
        f"  (Metrale Engine reported top-10: [(9764, 21.5), (760, 20.875), (6820, 20.25), ...])",
        flush=True,
    )

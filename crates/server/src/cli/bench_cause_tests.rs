...
//! 2026-09-26: Tests for the final-`Error:`-block reader and the log-tail reader.
//!
...
/// 2026-09-26: A serve log that ends in a boot failure: a progress line, then an
/// `Error:` / `Caused by:` block.
...
fn the_final_block_starts_at_the_last_column_zero_error_line() {
    ...
    // 2026-09-26: Two `Error:` blocks in one log: only the last is returned.
    let twice = format!("{KV_OOM}\n...a later boot...\nError: something else\n");
    assert_eq!(
        final_error_block(&twice).as_deref(),
        Some("Error: something else")
    );
    // 2026-09-26: A mid-line "Error:" is not an anchor, and a log with no `Error:`
    // line yields `None`.
    assert_eq!(
...
fn the_tail_of_a_file_reads_whole_lines_and_a_short_file_whole() {
    ...
    // 2026-09-26: Ten 5-byte lines, then a 9-byte error line.
    ...
    // 2026-09-26: A 12-byte window opens mid-line ("aa\nError: x\n"): the partial line
    // is dropped and the read starts at the boundary.
    assert_eq!(tail_of_file(&p, 12).unwrap(), "Error: x\n");
    // 2026-09-26: A window that holds only the end of the last line returns that end.

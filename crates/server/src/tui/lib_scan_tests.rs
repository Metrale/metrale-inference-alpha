...
//! 2026-09-26: Tests for the Library's cache scan (`data::library::scan`) and `lib_scan.rs`, against HF cache trees in a temp directory.
//!
//! Owner: server tui.
...
/// 2026-09-26: A published checkpoint with its files directly in `snapshots/rev0`, no `blobs/`.
...
/// 2026-09-26: Write `refs/main`, which `model_download/hf.rs` writes only for a completed snapshot.
fn publish(dir: &Path) {
...
fn a_symlinked_snapshot_is_measured_by_the_blob_it_points_at() {
    // 2026-09-26: The bytes live in `blobs/` and are symlinked into the snapshot.
    let tmp = tempfile::tempdir().expect("tempdir");
...
fn an_unreadable_cache_root_scans_to_an_empty_list_rather_than_failing() {
    ...
    // 2026-09-26: Asserted only when the chmod actually denies access to this process.

...
Set `METRALE_COMM_DIAGNOSTICS=1` on every rank and enable the
`metrale::comm=info` tracing target (for example through the application's
...
CPU checks: `cargo test -p metrale-comm --no-default-features` exercises the

...
//!
//! Slice 2 classed Metrale Engine's `causal_conv1d_update_l2norm` as REUSE for KDA. It has never been
...
//!
//! ## Two paths, because Metrale Engine has two and they are different kernels
...
//!
//! ## State-width mapping
//! HF keeps `kernel_size - 1 = 3` slots. Metrale Engine keeps 4 and shifts LEFT before convolving, so
//! the oldest slot is shifted out and never participates:
//! `HF_state[0..3] == Metrale_state[1..4]` pre-shift. The Rust side widens HF's 3 into Metrale Engine's 4.
//!
...
fn main() -> Result<()> {
    ...
    // HF's 3-wide state -> Metrale Engine's 4-wide: slot 0 is shifted out before the conv, so it is a
    ...
    // Metrale Engine keeps 4 slots, HF keeps 3: compare the overlapping window Metrale Engine[1..4] vs HF[0..3].
    ...
    // Metrale Engine's 4-wide final state vs HF's 3-wide: compare the overlapping window.

...
# 2026-09-26: HF CPU reference for Nemotron-3-Nano: dequantize NVFP4 to BF16, run given token IDs, dump per-layer states and logits.
#
# Owner: bench.
...
state_dict into a fresh NemotronH model, feeds the EXACT token IDs Metrale Engine
...
format the Metrale Engine METRALE_NEMO_DUMP hook writes -- so the comparator diffs 1:1.
...
# 2026-09-26: E2M1 FP4 code -> value; codes 8-15 are codes 0-7 negated.
_E2M1 = torch.tensor(
...
def dequant_nvfp4(packed, wscale, wscale2, group_size=16):
    ...
    codes[:, 0::2] = lo
    codes[:, 1::2] = hi
    vals = _E2M1.to(codes.device)[codes]
    s = wscale.to(torch.float32)
...
def main():
    ...
    # 2026-09-26: Requires len(hs) == n_layers + 1 (asserted below). hs[1..n-1] are
    # written as hf_L0..hf_L{n-2}, and hs[-1] as hf_final_norm.bin with no norm_f
    # applied here: this script takes the last entry to be after norm_f already.

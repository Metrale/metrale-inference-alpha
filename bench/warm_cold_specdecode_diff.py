#!/usr/bin/env python3
# 2026-09-26: For each prompt, sends it twice unsalted as a greedy completion and
# reports the first token where the second continuation differs from the first.
# Exits 1 if any prompt differs.
#
...
def run_pair(name, prompt):
    ...
    cold = gen_tokens(salt + prompt)
    _ = gen_tokens(prompt)
    warm = gen_tokens(prompt)
    # 2026-09-26: The reference is the first unsalted send. The salted send's result
    # (`cold`) is not compared or reported.
    cold_ref = _

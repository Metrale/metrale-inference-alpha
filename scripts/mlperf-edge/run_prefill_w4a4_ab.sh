#!/bin/bash
# One leg of the METRALE_FP4_PREFILL A/B. Serves the frozen golden config, optionally
...
[ "$FP4" = "1" ] && FP4_ENV=(-e METRALE_FP4_PREFILL=1)

sudo docker rm -f metrale-fp4-ab >/dev/null 2>&1; sleep 3
sudo docker run -d --name metrale-fp4-ab --network host --gpus all --ipc=host \
  ...
  -e METRALE_SSM_TAIL_LEASE_TTL=128 -e METRALE_BF16_TC_PREFILL=1 "${FP4_ENV[@]}" \
...
for i in $(seq 1 180); do curl -sf -m4 http://localhost:8888/v1/models 2>/dev/null | grep -q Qwen && break
  sudo docker ps --format '{{.Names}}' | grep -q metrale-fp4-ab || { echo "SERVE_DIED tag=$TAG"; exit 1; }; sleep 5; done
echo "=== [$TAG] serve up (METRALE_FP4_PREFILL=$FP4) ==="
# Confirm the flag actually took effect rather than assuming it did.
sudo docker logs metrale-fp4-ab 2>&1 | grep -c 'METRALE_FP4_PREFILL=1' | xargs -I{} echo "[$TAG] fp4-prefill banner lines: {}"
...
sudo docker rm -f metrale-fp4-ab >/dev/null 2>&1

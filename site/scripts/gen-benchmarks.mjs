...
// -----------------------------------------------------------------------------
// SSOT: the metrale test harness writes throughput baselines to tests/baselines/
//   (one *.json per gated model). Those files are the single source of truth for
//   the "verified" throughput a shipped Metrale Engine image is held to. An EMPTY
...
const BASELINES_ROOT =
  process.env.METRALE_BASELINES_ROOT || resolve(REPO, 'tests', 'baselines');

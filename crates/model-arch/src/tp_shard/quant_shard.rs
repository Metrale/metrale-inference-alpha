...
//! 2026-09-25: Shard primitives for weights that are already quantized:
//! NVFP4 (`shard_quantized_nvfp4`) and FP8 block-scaled
//! (`shard_fp8_block_scaled`).
//!
//! Owner: model-arch (tensor parallelism).
//! Invariants:
//! - With `tp_size <= 1` or `Replicated`, both return a copy of the source
//!   (same device pointers) and allocate nothing.

use anyhow::{Result, ensure};
use metrale_gpu_runtime::gpu::{DevicePtr, GpuBackend};

use super::TpShardKind;
use metrale_model_layers::weight_map::{Fp8Weight, QuantizedWeight};

// 2026-09-25: The layouts these functions assume:
//
...
//     `[N/block_size, K/block_size]` FP32. Both slice on N
//     (column-parallel) or K (row-parallel) at block granularity.

/// 2026-09-25: Shard an NVFP4-quantized weight. The packed weight is `[N, K/2]` u8;
/// the per-group scale is `[N, K/group_size]` u8 (FP8); `weight_scale_2`
/// is a per-tensor f32 (replicated across all ranks).
///
/// `out_dim` (= N) and `in_dim` (= K) are pre-shard, full-tensor dims. With
/// `tp_size <= 1` or `Replicated` it returns a copy of `src`; otherwise newly
/// allocated buffers, and the caller frees the source.
pub fn shard_quantized_nvfp4(
    ...
    match kind {
        ...
        TpShardKind::ColumnParallel => {
            ...
            // 2026-09-25: Packed weight: [N, K/2] u8 → this rank's local_out rows.
            ...
            // 2026-09-25: Scale: [N, K/group_size] u8 → the same rows.
        ...
        TpShardKind::RowParallel => {
            ...
            // 2026-09-25: Per-row strided copy on the K axis for both weight and scale.
...
/// 2026-09-25: Shard an FP8 block-scaled weight. `weight` is `[N, K]` FP8 bytes;
/// `row_scale` is `[N/block_size, K/block_size]` FP32. Both slice on the same
/// axis at block granularity. Same ownership rule as `shard_quantized_nvfp4`.
pub fn shard_fp8_block_scaled(
    ...
    match kind {
        ...
        TpShardKind::ColumnParallel => {
            ...
            // 2026-09-25: Weight: [N, K] u8 → this rank's local_n rows.
            let w_row_bytes = k;
            ...
            // 2026-09-25: Scale: [scale_n, scale_k] FP32 → this rank's local_scale_n rows.

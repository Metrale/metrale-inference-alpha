...
Metrale Engine's performance claims are measurable. This chapter shows what the benchmarks measure, how to reproduce them, and how to read the numbers.
...
And the kernel micro-benchmark summary: **Metrale Engine wins 32/32** against PyTorch on attention, GEMM, SSM, RMSNorm, RoPE, SiLU×Mul, and conv1d, with speedups from 1.04× up to 18.2×.

## Two kinds of benchmark

Metrale Engine has two benchmark surfaces:

1. **End-to-end HTTP throughput** — `metrale-spark-bench` (client-side Criterion harness targeting a running server). This is what "131 tok/s" means.
...
```bash
sudo docker run -d --name metrale-35b \
...
```bash
export METRALE_BENCH_URL=http://localhost:8888
cd /path/to/metrale
cargo bench -p metrale-spark-bench
```
...
cargo bench -p metrale-spark-bench    # end-to-end client benchmarks
```
...
| Kernel | Benchmark | Metrale Engine | PyTorch | Speedup |
|---|---|---:|---:|---:|
...
- **Historical benchmark journeys**: `docs/METRALE_SPARK_JOURNEY.md` — the benchmark retrospective across the Spark line.

## Apples-to-apples notes

When comparing Metrale Engine to vLLM or TensorRT-LLM:
...
```
met benchmark list                      # the suite
met benchmark list concurrency-sweep    # one benchmark's parameters
met benchmark run  concurrency-sweep --model <served-model>
met benchmark history
```
...
own thresholds (`met benchmark list <id>` prints them, and the TUI shows them
as a step after selecting the benchmark). A gate run serves the variant marked
`default = true` unless you name another:

```
met benchmark run agentic-webserver --yes --pull-request-gate   --checkpoint unsloth/Qwen3.8-27B-NVFP4
```
...
`--param` takes any key from `met benchmark list <id>`; anything you leave out
...
fixed**: the campaign picks `n` for the fleet it has — `met bench certify`
...
```
met benchmark run bfcl-subset --pull-request-gate --hardware gb10 --param shard=0/2   # on dgx1
met benchmark run bfcl-subset --pull-request-gate --hardware gb10 --param shard=1/2   # on dgx2
met benchmark aggregate bfcl-subset      # what the group scores, and what is missing
met bench certify --shards 6             # or let the campaign choose and place them
```
...
`~/.metrale/runs/<benchmark-id>/`, carrying the result, every parameter used (not
just the ones you overrode), the target, the source, and the Metrale Engine version. So
a stored run says what it measured and can be reproduced.

```
met benchmark history --id concurrency-sweep --limit 5
met benchmark history --run run-1785000000123456789 --format json | jq .params
```

One store, both directions: a CLI run appears in the dashboard's Benchmarks →
History pane, and a dashboard run appears in `met benchmark history` marked
`tui`.
...
- `crates/metrale-spark-bench/src/lib.rs` — E2E harness.
...
- `docs/METRALE_SPARK_JOURNEY.md` — benchmark journey and retrospective.

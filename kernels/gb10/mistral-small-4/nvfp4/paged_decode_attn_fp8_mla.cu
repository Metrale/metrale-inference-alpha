...
// forked-from: kernels/gb10/deepseek-v4-flash/nvfp4/paged_decode_attn_fp8_mla.cu (2026-09-24; 2 of 567 lines differ, see kernels/FORKS.md)

#undef HDIM
#define HDIM 320

// 2026-09-25: FP8 E4M3 paged decode attention at HDIM 320 (defined above, replacing the target's
// -DHDIM=128): paged_decode_attn_fp8, and a split-K pair. No Rust code looks up this file's module
// (paged_decode_attn_fp8_mla).
//
// Owner: gb10 kernels (mistral-small-4).
// Invariants: none beyond the types; every kernel assumes head_dim == HDIM.
//
// K and V are E4M3 bytes in NHD layout, physical blocks cache_stride elements apart, dequantized as
// byte x k_scale or v_scale; Q and O are BF16. paged_decode_attn_fp8 runs 8 warps per (q_head = blockIdx.x,
// seq = blockIdx.y): the warps take contiguous chunks of [0, seq_len), step BC positions at a time inside a
// physical block, and merge their online-softmax states in shared memory. Each lane reads its K/V slice as
// VEC_U32_FP8 = HDIM / 128 = 2 words (8 of its VEC_BF16 = 10 values) from byte offset lane * 10.
...
extern "C" __global__ void paged_decode_attn_fp8(
    ...
    const unsigned long long cache_stride     // 2026-09-25: elements between physical cache blocks
) {
    ...
    // 2026-09-25: Merge the warps' (m, l, o) states pairwise; a warp that attended nothing (l == 0) is skipped.
...
// 2026-09-25: Split-K: CTA (q_head, split, seq) attends the split's contiguous share of [0, seq_len) and
// writes o, then m, then l to workspace [num_seqs, num_q_heads, num_splits, head_dim + 2].
...
// 2026-09-25: Merges one (q_head, seq)'s num_splits partials, skipping a split s >= 1 with l <= 0, into BF16
// O [num_seqs, num_q_heads, head_dim]; a sequence of length 0 writes nothing. threadIdx.x is the lane, so a
// block is expected to be one 32-thread warp per (q_head, seq).

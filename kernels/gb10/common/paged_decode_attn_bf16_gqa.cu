...
// 2026-09-25: Kernel `paged_decode_attn_bf16_gqa`: `paged_decode_attn` (paged_decode_attn.cu) with the PD_GQA query heads
// of one KV head packed into one CTA, so each K and V row is loaded once for the whole query group.
//
// One CTA of 8 warps per (kv_head, seq), grid (num_kv_heads, num_seqs, 1). The CTA holds PD_GQA query vectors and
// PD_GQA online-softmax states in registers and applies each loaded K/V row to every head; the heads' warp states
// are then merged one head at a time through one shared buffer. Per head it performs the same floating-point
// operations in the same order as `paged_decode_attn` at HDIM 256, so with --fmad=false
// (kernels/gb10/common/KERNEL.toml) the output is bit-identical. gqa_pack_gpu_tests.rs compares the two byte for
// byte on a GPU (ignored by default).
//
// The host routes here only from the non-split BF16 arm (run_paged_decode.rs), through
// `splitk_dispatch::gqa_pack_kernel`, which requires num_q_heads == num_kv_heads * PD_GQA and head_dim == PD_HDIM.
//
// Owner: gb10 kernels.
// Invariants: none beyond the types. The kernel assumes blockDim.x == 256 (NUM_WARPS * WARP_SIZE); the host
// checks the head shape.
...
#define WARP_SIZE 32
// 2026-09-25: Not `#ifndef`-overridable, and not named HDIM, which model builds pass on the command line.
#define PD_HDIM 256
...
// 2026-09-25: Query heads packed into one CTA. attn_splitk_tests.rs checks that PD_GQA and PD_HDIM equal
// `metrale_kernels::attn_splitk::DECODE_GQA_PACK_WIDTH` and `DECODE_GQA_PACK_HEAD_DIM`.
...
extern "C" __global__ void __launch_bounds__(NUM_WARPS* WARP_SIZE, 1)
    ...
    while (pos < my_end) {
        ...
        for (; processed < aligned_count; processed += BC) {
            // 2026-09-25: One load of each K and V row serves every head of the group.
    ...
    // 2026-09-25: Each head's warp states are merged in turn through the same shared buffer.

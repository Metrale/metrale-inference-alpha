...
//! 2026-09-25: What the source layers publish for the layers after them: the
//! kv-source compressor (latent pooling) and the index-source indexer (index
//! keys, scores, candidate blocks and the CPU top-k).
//!
...
use crate::deepseek_v41_ref::compress::{FP4_BLOCK, select_candidate_blocks, torch_cpu_topk_set};

impl AttnV41 {
    /// 2026-09-25: Compute this call's latents (pre-RoPE, bf16 in `self.latent`)
    /// and return their group count, or `None` while a ratio > 1 group is
    /// still filling.
    pub(super) fn compressor(
        ...
        // 2026-09-25: One output a block at m = 1 (`attn_v41_gemv_f32_staged`),
        // 16x16 tiles otherwise.
    ...
    /// 2026-09-25: RoPE positions of `groups` new latents: `g * ratio` at
    /// `start_pos == 0`, else the one completed group's first position.
    ...
    /// 2026-09-25: The indexer: publishes index keys (on a kv source with new
    /// latents), scores the compressed positions, applies or publishes the
    /// candidate mask, and picks the top-k on the CPU. Returns
    /// `([tokens][topk], topk)`, positions offset by `offset`, -1 = unreachable.
    ...
    pub(super) fn indexer(
        ...
        // 2026-09-25: Head weights: bf16(bf16(x . wproj) * ihd^-0.5 * nh^-0.5).
        self.gemm(
            gpu,
            x,

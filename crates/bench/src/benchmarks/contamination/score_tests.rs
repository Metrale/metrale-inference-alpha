...
//! 2026-09-26: Table tests for `score` and `verdict`: each case is a fault the
//! scorer must catch. The outcomes are built in memory, so no server or GPU is
//! needed.
//!
...
fn divergence_at_k_is_localized_and_fails() {
    ...
    // 2026-09-26: 5, not 4: `canonical()` opens with the (empty) reasoning field
    // and its \u{1} separator, so the common prefix is that separator plus
    // "abcd". Comparing the canonical form rather than `text` alone is what
    // makes a changed tool call or finish_reason count as divergence.
...
fn foreign_canary_is_contamination_not_divergence() {
    ...
    // 2026-09-26: In the rung, prompt 0's reply carries prompt 1's canary.
    let rungs = vec![(
        "c2".to_string(),
        vec![
...
/// 2026-09-26: Zero divergence over zero comparisons must not read as a pass.
#[test]

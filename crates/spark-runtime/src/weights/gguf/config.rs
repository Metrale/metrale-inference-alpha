...
//! Build a metrale-core [`ModelConfig`] from a bare GGUF file's metadata, so a
//! directory containing only a `.gguf` (no `config.json`) can be served.
//!
//! `config_from_gguf` lives in metrale-core (which cannot see this crate's GGUF
//! parser); this module bridges the two — it impls the metrale-core [`GgufMeta`]
...
use metrale_core::config::{GgufConfigInputs, GgufMeta, ModelConfig, config_from_gguf};

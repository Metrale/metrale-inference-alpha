...
//! same idiom used by `metrale-closure` and `metrale-plugin::gate`.
...
// CI host runs with `METRALE_SKIP_BUILD=1`, which emits an empty stub
...
// machine via `cargo test -p metrale-kernels -- --ignored` after a
// real PTX build.

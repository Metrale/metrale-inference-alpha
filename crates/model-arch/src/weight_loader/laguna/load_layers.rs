...
//! 2026-09-25: Laguna layer construction: attention with a softplus head gate, and a dense
//! FFN or MoE per layer.
//!
//! Owner: model-arch weight loader (Laguna).
//! Invariants:
//! - `METRALE_UNIFIED_MOE_LAYOUT` and `METRALE_HOLO_MOE_GROUPED_CUTLASS` are never both
//!   honoured: the load fails when both are set.
...
use metrale_model_layers::layers::qwen3_attention::types::HeadGateActivation;
use metrale_model_layers::layers::{DenseFfnLayer, FfnComponent, MoeLayer, Qwen3AttentionLayer};
...
pub(super) fn load_layers(
    ...
    // 2026-09-25: Sliding layers: theta=10000 over the full head_dim, no YaRN ramp.
    ...
    let unified_moe_layout =
        unified_moe_layout_enabled(std::env::var("METRALE_UNIFIED_MOE_LAYOUT").ok().as_deref());
...
fn load_moe_ffn(
    ...
    // 2026-09-25: The shared expert's precision is read from the checkpoint: a
    // `gate_proj.weight_packed` tensor means NVFP4, otherwise BF16.
    let shared_packed = store.contains(&format!("{shared}.gate_proj.weight_packed"));
    let (shared_expert, bf16_shared) = if shared_packed {
        // 2026-09-25: NVFP4 shared expert, loaded like the routed experts; no
        // BF16 override.
        (
            ExpertWeight {
        ...
        // 2026-09-25: BF16 shared expert. The NVFP4 copies give the fused routed
        // kernels something to read; the BF16 weights are installed with
        // `set_bf16_shared_expert` below.
    ...
    // 2026-09-25: Native NVFP4 CUTLASS grouped MoE
    // (`METRALE_HOLO_MOE_GROUPED_CUTLASS`).
    if cutlass_grouped_moe_enabled() {
        // 2026-09-25: `transpose_for_prefill_unified` frees the original expert
        // weights, and the grouped-CUTLASS prefill branch
        // (`forward_prefill_routed.rs`) reads their `packed_ptrs`.
        anyhow::ensure!(
            !unified_moe_layout,
            "METRALE_UNIFIED_MOE_LAYOUT and METRALE_HOLO_MOE_GROUPED_CUTLASS cannot \
    ...
    match config.layer_types[i] {
        LayerType::SlidingAttention => {
            ...
            if !sliding_inv_freq.is_null() {
                // 2026-09-25: attention_factor 1.0 over a table with no YaRN
                // ramp is plain RoPE.
...
/// 2026-09-25: Build the CUTLASS grouped-NVFP4 SFB tables at load: opt-in with
/// `METRALE_HOLO_MOE_GROUPED_CUTLASS=1` or `true`.
fn cutlass_grouped_moe_enabled() -> bool {
    matches!(
        std::env::var("METRALE_HOLO_MOE_GROUPED_CUTLASS").as_deref(),
        Ok("1") | Ok("true")
    )
}

/// 2026-09-25: Plain RoPE inv_freq table (`1 / theta^(2j/dim)`, no YaRN ramp)
/// for the sliding-attention layers, computed in f64 and narrowed once to f32.
...
/// 2026-09-25: `METRALE_LAGUNA_ROPE_TABLE=0` turns off the precomputed
/// sliding-layer RoPE table; the layers then get no table.
fn sliding_rope_table_enabled() -> bool {
    std::env::var("METRALE_LAGUNA_ROPE_TABLE").as_deref() != Ok("0")
}

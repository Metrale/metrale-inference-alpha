...
//
// METRALE FFN prefill GEMM via vendored llama.cpp NVFP4 W4A4 MMQ (Blackwell block-scale
...
// metrale_nvfp4_repack converts the checkpoint layout (packed [N,K/2] low=even/high=odd +
...
// legal. Metrale Engine only ever instantiated 128, which meant DECODE at M=16 issued MMAs for
...
extern "C" __global__ void metrale_nvfp4_repack(
    ...
    // q4k_vendor/mmq.cuh, which is the ONLY reader of this buffer (Metrale Engine exposes
...
// FUSED SiLU-mul + block_fp4_mmq quantize for the down-MMQ path (METRALE_FFN_NVFP4_MMQ_DOWN).
// Replaces metrale_nvfp4_silu_mul_scaled + metrale_nvfp4_quantize_bf16: computes

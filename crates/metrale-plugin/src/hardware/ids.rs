...
//!
//! * an operator typing `met benchmark run <id> --hardware h100`, and
...
//! that is not there.
//!
//! [`KNOWN_HARDWARE_IDS`] is that registry — the ids Metrale Engine recognises,
//! independent of what has been measured on them.

/// Every box class Metrale Engine recognises, whether or not any gate has a record on
/// it yet.
///
...
/// True when `id` names a box class Metrale Engine recognises.
///
...
mod tests {
    ...
    /// that Metrale Engine has never registered, and `H100` is the right part spelled

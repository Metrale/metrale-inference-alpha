...
#   • GDN-FlashInfer (METRALE_GDN_FLASHINFER=1) needs libmetralegdn.so + the CuTe-DSL
...
RUN cd 3rdparty_patches/gdn_aot && \
    ...
    g++ -O2 -fPIC -shared gdn_shim.cpp gdn_transpose.o gdn_holo_0.o \
      -o /usr/local/lib/libmetralegdn.so \
...
# GDN-FlashInfer runtime libs (only loaded when METRALE_GDN_FLASHINFER=1).
COPY --from=builder /usr/local/lib/libmetralegdn.so /usr/local/lib/libmetralegdn.so

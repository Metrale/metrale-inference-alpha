...
//! 2026-09-25: Long-context bench for `HighSpeedSwap`. It offloads every block
//! of a one-layer synthetic Q/K/V set with `offload_block`, times `--steps`
//! calls of `attend_layer` over the whole sequence, and compares them with a
//! single-tile `TiledAttention` step over a fully resident copy.
//! Run: `cargo run --release -p metrale-storage --example long-context-bench
//! -- --dir <path> [--context-blocks N] [--scratch-blocks N] [--steps N]`.
//!
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: `None` when the resident run returns an error.

...
pub struct PluginMetadata {
    ...
    /// True only for plugins shipped inside Metrale Engine itself.
    ///
    ...
    /// first-party plugins get it from [`PluginMetadata::metrale`], and anything
    /// built with [`PluginMetadata::third_party`] cannot set it at all.
    pub official: bool,
}

impl PluginMetadata {
    /// A first-party Metrale Engine plugin. Every field except the description is the
    ...
    pub const fn metrale(description: &'static str) -> Self {
        Self {
            ...
            author_url: "https://dev.metrale.ai",
    ...
    /// A plugin from outside the Metrale Engine tree. `official` is forced false.

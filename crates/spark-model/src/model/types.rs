...
pub struct TransformerModel {
    ...
    /// `None` under `METRALE_NO_LMHEAD_TGEMM=1`.
    ...
    /// True when runtime adapter rotation is ARMED: `METRALE_LORA_ROTATE=1`, or
    /// `$METRALE_LORA_PEER` set. Armed ⇒ decode runs eager (no CUDA-graph
    ...
    /// 0 when absent. Kill switch: METRALE_NO_LMHEAD_LOSSLESS=1.
    ...
    /// behind `METRALE_LM_HEAD_M16_TC`. 0 when the kernel set lacks it, which is
    ...
    /// `N_TILE=64` twin of the above (`METRALE_LM_HEAD_M16_TC_NTILE=64`).
    ...
    /// Optional SSM snapshot spill tier (`METRALE_SSM_TIER`). `None` (default)
    ...
    /// Profile mode: skip graphs, sync+time each layer. Set METRALE_PROFILE=1.
    pub(super) profile: bool,
    /// One-shot profile flag for the next prefill request only. Set
    /// METRALE_PROFILE_FIRST=1 to capture per-step timing on the first prefill
    ...
    /// METRALE_MTP_DRAFTER_PREFILL: per-position final-layer hidden capture for
    ...
    /// METRALE_MTP_CARRY_DRAFTER: the previous turn's drafter KV, held so the
    ...
    /// METRALE_MTP_CARRY_DRAFTER is on.
    ///
    ...
    /// serve, a serve with the kernels absent or without the `METRALE_GDN_WOA=1`
    ...
    /// Kill switch `METRALE_NO_VERIFY_WY_CACHE` (PRESENCE) restores the
    ...
    /// extension from metrale#99 is active — every command broadcast is
    ...
    /// from `METRALE_EP_PROTOCOL` env var; both ranks must agree.
    ...
    /// FP16 h-state twin of the above (`METRALE_SSM_H_FP16`). Selected from the
    ...
    /// One-shot FP32 -> FP16 h-state converter (`METRALE_SSM_H_FP16`).

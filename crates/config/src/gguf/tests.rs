...
//! 2026-09-26: Tests for `config_from_gguf`, fed by an in-memory `GgufMeta`.
//!
//! Owner: config.
...
fn mamba_architecture_is_rejected_before_dimension_parsing() {
    ...
    let err = config_from_gguf(&inp).unwrap_err().to_string();
    assert!(
        err.contains("GGUF general.architecture 'mamba' has no Metrale Engine model_type mapping"),
        "unexpected: {err}"
    );
}

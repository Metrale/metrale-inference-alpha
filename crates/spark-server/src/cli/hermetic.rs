...
//! produced while serving one request may reach another. Several Metrale Engine
...
/// Re-exported, NOT redeclared. The table lives in `metrale_plugin::gate::hermetic`
/// because `gate::bench` needs it too — it refuses a BENCH.toml entry that pins
/// `hermetic=true` without these — and metrale-plugin cannot depend on this
...
pub(crate) use metrale_plugin::gate::hermetic::CLOSED_KEYS;
...
pub(crate) fn expand(
    ...
    if !metrale_plugin::gate::hermetic::is_requested(&requested) {
        return requested;
    }
...
///
/// `None` means "no flag was given, so `METRALE_MTP_GATE_FORCE` decides" — the
...
/// environment: leaving it as `None` would let `METRALE_MTP_GATE_FORCE=0`

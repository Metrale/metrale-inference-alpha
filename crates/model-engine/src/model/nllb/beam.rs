...
//! 2026-09-25: Beam search for one request on the served NLLB model, plus the batch
//! scratch and hypothesis pool `beam_multi` shares.
//!
//! The request's B beams decode as one M=B batch per step ([`super::beam_compute`]);
//! candidate expansion, pruning and the finished-hypothesis pool run on the host.
//!
//! Owner: model-engine.
...
/// 2026-09-25: The batch's cross-KV for grouped cross-attention: `kpad`/`vpad` are
/// per-decoder-layer `[C, max_enc, d]` buffers, one slab per request. Row `i`
/// reads slab `i / group` (its request), bounded to its own `enc_len` by the
/// per-row `crosstk`. For a single request the unpadded `[enc_len, d]` buffers
/// are passed with `group = num_beams`, so every row reads slab 0.
...
/// 2026-09-25: Largest K (= 2·num_beams) the on-device beam top-k kernel takes
/// (`NLLB_TOPK_MAX` in nllb_encoder.cu); sizes the `topk_*` scratch. With a
/// larger K (num_beams > 16) `beam_multi` uses the host top-k instead.
pub(super) const NLLB_TOPK_KMAX: usize = 32;

/// 2026-09-25: Batched decode scratch for B rows: bf16, except the u32 id and
/// key-length buffers and the f32/u32 top-k outputs.
pub(super) struct DecBuf {
    ...
    /// 2026-09-25: On-device top-k outputs: per row `lse` (f32), and `K`
    /// `(val, tok)` pairs in `topk_val`/`topk_idx` (`[rows, NLLB_TOPK_KMAX]`).
...
impl DecBuf {
    /// 2026-09-25: `crosstk_host[row]` is that row's cross key length (its
    /// request's `enc_len`); `b` is the total row count (Σ beams over all fused
    /// requests).
    pub(super) fn new(
        m: &NllbGpuModel,
        b: usize,
    ...
    pub(super) fn free(self, gpu: &dyn metrale_gpu_runtime::gpu::GpuBackend) -> Result<()> {
        for p in [
            self.dh,
            self.normed,
            self.q,
...
/// 2026-09-25: Finished-hypothesis pool: keeps the `num_beams` best by
/// `sum_logprob / len^lp`.
...
impl NllbGpuModel {
    /// 2026-09-25: Run beam search for one request → the winning hypothesis
    /// token ids (decoder start stripped, EOS-terminated). Uses `run_encoder` for
    /// the encoder and cross-KV, and arms the LoRA gate from the request.
    pub(super) fn generate_beam_one(&self, req: &BeamReq) -> Result<Vec<u32>> {
        ...
        // 2026-09-25: Encoder and cross-KV: one set of [enc_len, d] buffers,
        // shared by all beams.
    ...
    fn beam_batched(
        ...
        // 2026-09-25: Single request: pass the unpadded [enc_len, d] cross-KV
    ...
    fn beam_loop(
        ...
        // 2026-09-25: Seed every beam with [dec_start, forced_bos].
        ...
        for _ in 1..max_new {
            ...
            // 2026-09-25: Reorder caches sk2[i] = sk[perm[i]] (rows 0..cur_len), then swap.

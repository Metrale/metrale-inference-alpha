...
- Image: `metrale-inference-gb10:alpha-2.43` (pre-existing).
...
- Image: `metrale-inference-gb10:overnight` (first build from master, includes 5 OSS-prep
  commits but NOT the moe_topk_sig fix — fix landed after build started).
- Same failure as attempt 1 (fix not yet in image).
- Action: trigger rebuild as `metrale-inference-gb10:overnight2`.

### 2026-04-17 06:54 UTC — baseline attempt 3 (pending)

- Image: `metrale-inference-gb10:overnight2` (rebuild in progress, includes moe_topk_sig
...
Build still compiling (`cargo build --release -p metrale-server` at 13:13 elapsed).
...
Image: `metrale-inference-gb10:overnight2`. Flags: FP8 quant + FP8 KV + SLAI scheduler
...
Started metrale-profile container on metrale-inference-gb10:overnight2. Shards
...
Enabled `METRALE_PROFILE=1`, fired a 283-token completion. Scheduler report:
...
- `METRALE_NVFP4_MLA=0` flag for Mistral — not relevant to Qwen3.5 (non-MLA).
...
2. `--speculative` / `--max-prefill-tokens` / `METRALE_W4A16_VARIANT` are
...
Build metrale-inference-gb10:overnight3 in flight (1:21 elapsed, ~15 min ETA). This
image bundles the new `moe_fp8_grouped_gemm_v2` coalesced kernel behind
the `METRALE_FP8_MOE_COALESCED=1` env gate. Tick deferred until image is
...
2. Boot `overnight3` with `METRALE_FP8_MOE_COALESCED=1` → re-bench. Must
...
**Image**: metrale-inference-gb10:overnight3 (commit 4c999d6, adds `moe_fp8_grouped_gemm_v2`).
...
**V2 coalesced** (`METRALE_FP8_MOE_COALESCED=1`, uses new kernel):
...
Any user who wants the 3–4% now can pass `-e METRALE_FP8_MOE_COALESCED=1`.
...
- Coalesced FP8 grouped-GEMM v2 kernel (METRALE_FP8_MOE_COALESCED=1)

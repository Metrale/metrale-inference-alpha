...
//! 2026-09-25: The expert store's layer files. `ExpertFileWriter` creates them
//! and places records; `ExpertFileReader` reads records back with positional
//! reads.
//!
//! Owner: metrale-storage experts.
//! Invariants:
//! - A record is written and read at `layout.file_offset(key)` in the file of
//!   `key.layer`.

use super::*;
use std::fs::{File, OpenOptions};
// 2026-09-25: Positional reads and writes on unix and Windows alike.
use crate::tier::pio;
use std::path::{Path, PathBuf};

/// 2026-09-25: Creates one file per MoE layer, sized to `bytes_per_layer`,
/// writes records at their offsets, and writes `manifest.json` in `finish`.
/// The files are opened without O_DIRECT.
...
impl ExpertFileWriter {
    ...
    /// 2026-09-25: Pack and write one record; fails when the layer or expert is
    /// out of range.
    pub fn write_record(
        &self,
        key: ExpertKey,
    ...
    /// 2026-09-25: fsync every layer file, then write `manifest.json`.
...
/// 2026-09-25: Reads records back with positional reads (`tier::pio`).
/// `expert_tier.rs` and `expert_peer.rs` open stores through it.
...
impl ExpertFileReader {
    ...
    /// 2026-09-25: Read one record's `record_stride` bytes into a new buffer.
    /// An out-of-range layer is an error rather than an index panic.

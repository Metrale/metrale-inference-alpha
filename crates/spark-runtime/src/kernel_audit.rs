...
//!
//! Every kernel lookup in Metrale Engine is EAGER: each one sits in a constructor on the
...
/// miss recorded after the seal. Exported as `metrale_kernel_lookups_unresolved`.
...
/// `metrale_kernel_lookups_unresolved` so a gate can assert `== 0` without
/// parsing logs.

...
# 2026-09-26: Edit-flow probe, Claude Code only: a buried stub in a long file, and a rename across two files.
#
# Owner: bench, FP8 drift harness.
# Invariants: none beyond the types.
"""Edit-flow probe v2 — harder, Metrale-bug-seeking scenarios (claude-code only;
...
Scenarios:
  S1 longctx_edit  : a ~600-line lib.rs (60 tiny fns) with ONE buried todo!()
                     ...
                     degradation. Highest Metrale-bug yield.
...
failure points at Metrale Engine mechanics, not model competence.
"""
...
# 2026-09-26: longctx: 60 functions in one lib.rs; step_37 is the only todo!() stub.
def seed_longctx(t):
...
def check_longctx(t):
    ...
    return {"still_todo": any("todo!(" in l for l in code),
            ...
            # 2026-09-26: the functions on either side of step_37 must be intact
            "step36_ok": "base + 36" in src, "step38_ok": "base + 38" in src}


# 2026-09-26: multifile: rename doubler to times_two in src/math.rs and its call in src/lib.rs.

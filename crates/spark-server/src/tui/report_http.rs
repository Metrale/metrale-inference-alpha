...
//! # Threading
//!
//! Both flows are blocking `ureq` on a named `std::thread` (`metrale-report`),
...
//! (code first, verdict later), so like `metrale-download` it hand-rolls its
...
const AGENT_HEADER: &str = concat!("metrale-spark/", env!("CARGO_PKG_VERSION"));

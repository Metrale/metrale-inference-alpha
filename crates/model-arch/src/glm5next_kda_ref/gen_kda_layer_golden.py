...
# 2026-09-25: Writes the full KDA layer golden (kda_layer_golden.json) from the transformers
# glm5_next linear-attention module on real layer weights.
#
...
# 2026-09-25: Every KDA value comes from the checkpoint's config.json; the asserts pin the
# expected geometry and constants.
...
def build(dtype, W):
    ...
    sd = {
        ...
        # 2026-09-25: The module has one depthwise conv1d over conv_dim; the checkpoint stores the
        # q, k and v conv tensors separately. They are concatenated in the order `run`
        # concatenates the projections.
    ...
    # 2026-09-25: dt_bias and A_log stay F32 whatever the run dtype.
    sd = {
...
def run(m, hidden, conv_state3, rec_state, dtype):
    ...
    """
    T = hidden.shape[0]
    hs = hidden.to(dtype).unsqueeze(0)
    st = {}

    mixed = torch.cat([m.q_proj(hs), m.k_proj(hs), m.v_proj(hs)], dim=-1)
    st["qkv_proj"] = mixed[0].float().clone()
    mixed_ct = mixed.transpose(1, 2)

    w_conv = m.conv1d.weight.squeeze(1)
    if conv_state3 is not None and T == 1:
        cs = conv_state3.to(dtype).unsqueeze(0).clone()
        ...
        conv = conv[:, :, -T:]
        # 2026-09-25: After a zero-started prefill the conv state is the last KS - 1 raw inputs.
        new_conv_state = mixed_ct[0, :, -(KS - 1) :].float().clone()
    st["conv_out"] = conv[0].t().contiguous().float().clone()

    q, k, v = torch.split(conv.transpose(1, 2), [QKV] * 3, dim=-1)
    shp = (1, T, -1, D)
    q, k, v = q.view(shp), k.view(shp), v.view(shp)
    # 2026-09-25: q_l2 and k_l2 are recorded for comparison; the KDA functions below normalise q
    # and k themselves (use_qk_l2norm_in_kernel=True).
    ...
    g = m.forget_gate(hs)
    beta = torch.sigmoid(m.b_proj(hs))
...
def emit(st):
    ...
    for name, t in st.items():
        # 2026-09-25: The mixed regime stores its prefill leg under a `prefill_` prefix; strip it
        # before the stride lookup, or those stages get the default stride.
...
for LAYER in LAYERS:
  ...
  for dt_name, dt in (("f32", torch.float32), ("bf16", torch.bfloat16)):
    ...
    for rname, T in REGIMES:
        # 2026-09-25: Every regime draws from a freshly seeded LCG, so each is reproducible alone.
        rng = Lcg(0x5EED_1A70)

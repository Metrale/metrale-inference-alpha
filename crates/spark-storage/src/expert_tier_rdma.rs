...
//
// Fetches expert records from an `metrale-expert-peer` straight into the pinned
...
// `$METRALE_EXPERT_RDMA_DEV` (default `roceP2p1s0f1`) / `$METRALE_EXPERT_RDMA_GID`
...
#[cfg(metrale_rdma_verbs)]
struct Rail {
    verbs: metrale_rdma::verbs::Verbs,
    arena_lkey: u32,
...
impl RdmaTier {
    ...
    pub fn connect(
        ...
        let transport = if use_verbs {
            ...
            // METRALE_SKIP_BUILD / hosts without libibverbs.
...
/// Bring up the one-sided verbs transport via [`metrale_rdma::railset::RailSet`]:
...
/// env-driven (METRALE_EXPERT_DUAL_RAIL=1): rail 0 = METRALE_EXPERT_RDMA_DEV/GID
/// (the existing single-rail defaults), rail 1 = METRALE_EXPERT_RAIL2_DEV/GID
...
fn connect_verbs(
    ...
    use metrale_rdma::env::{first_set, first_set_u32};
    ...
    // rail-2 env. Dual-rail only when METRALE_EXPERT_DUAL_RAIL=1. PSN = fresh

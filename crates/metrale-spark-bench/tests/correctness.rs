...
//! End-to-end correctness tests against a running Metrale Engine server.
//!
//! Every test in this file calls `require_server()` which panics if no
//! HTTP listener is reachable on `$METRALE_SERVER_URL` (default
...
//! ```bash
//! # In one terminal:
//! ./target/release/met serve <model>
//!
//! # In another:
//! cargo test -p metrale-spark-bench --test correctness -- --ignored
//! ```

use parking_lot::Mutex;

use metrale_spark_bench::{require_server, send_blocking, send_streaming};

// Q/A tests must run sequentially — concurrent model requests degrade output quality.
static SERIAL: Mutex<()> = Mutex::new(());

#[test]
#[ignore = "requires a running Metrale Engine server at $METRALE_SERVER_URL"]

...
for `deepseek-v4-flash`. `collect_cu_files` (`metrale-kernels/build.rs`) merges
...
1. **Shadow files textually include what they shadow.**
   `kernels/gb10/qwen3.6-27b/nvfp4/attn_prefill_paged_indirect.cu:12`
   is `#include "../../common/attn_prefill_paged_indirect.cu"`. Eight
...
We also could not reuse the existing `METRALE_KERNEL_SET_HASH`
(`metrale-kernels/src/lib.rs`): it is global across all targets, hashes the
*generated* `target_ptx.rs` text rather than kernel content, and is a stub
under `METRALE_SKIP_BUILD=1` — which is how the gate runs.

...
pub fn load_pass(
    ...
    for tensor in &gguf.tensors {
        ...
        // GGUF dims are ggml-order; Metrale/HF shape is the reverse.
        let hf_shape: Vec<usize> = tensor.dims.iter().rev().copied().collect();

        // ── Native keep-packed Q2_0 short-circuit ──
        // When `METRALE_GGUF_NATIVE_Q2=1`, upload the raw `block_q2_0` bytes for
        ...
        // Q3_K (ggml types 10 / 11) on the device; METRALE_DS41_KQUANT_RESIDENT=0
...
/// (10 / 11; `METRALE_DS41_KQUANT_RESIDENT=0` restores the bf16 expansion), and
/// the Q6_K output head (14; `METRALE_DS41_Q6K_HEAD=0` restores it). `None`
...
fn v41_kquant_resident_dtype(id: u32, hf_name: &str) -> Option<WeightDtype> {
    ...
    match id {
        10 | 11
            if names::is_v41_kquant_resident(hf_name) && !off("METRALE_DS41_KQUANT_RESIDENT") =>
        {
            Some(if id == 10 {
                WeightDtype::Q2K
            } else {
        ...
        14 if names::is_v41_q6k_resident(hf_name) && !off("METRALE_DS41_Q6K_HEAD") => {

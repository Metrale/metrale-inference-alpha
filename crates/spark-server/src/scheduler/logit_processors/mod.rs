...
pub struct SamplingLevers {
    ...
    /// `METRALE_DISABLE_WATCHDOGS` — every auto-watchdog off. Read by the F2
    /// and mid-word stages.
...
/// `METRALE_ADADEC_DIAGNOSTIC` records keep their pre-unification label.
...
/// env-gated `METRALE_ADADEC_DIAGNOSTIC` JSONL record — it never alters
/// any logit transform.
pub fn run_pipeline_with_path(
    ...
    // distribution, never mutates. No-op when METRALE_ADADEC_DIAGNOSTIC is
...
///
/// Stages, in order:
///  1. **METRALE_FORCE_TEMP_ZERO bypass** (eligible on BOTH kinds): when the
...
pub fn process_position_logits(
    ...
    // 1. METRALE_FORCE_TEMP_ZERO: pure argmax on raw logits — no pipeline, no

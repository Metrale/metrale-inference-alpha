...
`metrale_governance::Event`. `.benchmarks/` answers *"did this commit pass?"*;
...
Nothing here is read by `met benchmark --pull-request-gate-check`'s verdict.
...
| 549 | `performance/speculation` | MTP accept-bucket accounting across `metrale-model-layers` and `metrale-server` |

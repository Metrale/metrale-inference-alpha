...
//! 2026-09-26: The Help section: the static guide, and one screen per
//! `ReportPhase` of the Report Issue pipeline.
//!
//! Owner: server tui.
//! Invariants: none beyond the types.
//!
//! Every state signal here also has a colour-free form, for `NO_COLOR`: the
//! checkbox is `[x]`/`[ ]`, phases are words, the user code and action keys are
//! bold, and success and failure carry `✓`/`✗`.
...
fn draw_guide(f: &mut Frame, _app: &App, area: Rect) {
    ...
    let mut lines = vec![
        Line::from(Span::styled(
            format!(
                "  Metrale Engine {} — github.com/{repo}",
        ...
        wrap(
            &format!(
                "Report Issue posts a bug report to the public tracker at github.com/{repo}. \
                 Its GitHub authorization is kept in memory only and is forgotten when the \
                 server exits."
            ),
            w,
            theme::dim(),
...
fn draw_report(f: &mut Frame, app: &App, area: Rect) {
    // 2026-09-26: An empty client id or repo (`report::target_from`) renders
    // the refusal instead of a form whose submit cannot work.
...
fn draw_compose(f: &mut Frame, app: &App, repo: &str, area: Rect) {
    ...
    if inner.height < 8 || inner.width < 20 {
        // 2026-09-26: Too small for the three-field form: say so instead of
        // clipping it.
        f.render_widget(
    ...
    // 2026-09-26: Attach checkbox: glyphs, not hue, carry the state.
...
fn draw_preview(f: &mut Frame, app: &App, repo: &str, area: Rect) {
    ...
    let Some(c) = &h.preview else {
        // 2026-09-26: `HelpState::review` sets `preview` together with
        // `ReportPhase::Preview`; without one, draw the composer rather than a
        // blank pane.
    ...
    let w = inner.width.saturating_sub(2) as usize;
    // 2026-09-26: Wrapped before scrolling, so the offset and the ceiling
    // count display rows.
    let rows: Vec<Line> = c
        .body
        .lines()
...
fn draw_status(f: &mut Frame, app: &App, repo: &str, phase: &ReportPhase, area: Rect) {
    ...
    let lines: Vec<Line> = match phase {
        ...
        ReportPhase::WaitingAuth {
            ...
            vec![
                Line::from(Span::styled(
                    "  First, authorize Metrale Engine on GitHub:",
                    theme::text(),
                )),
                Line::from(""),
                Line::from(Span::styled(

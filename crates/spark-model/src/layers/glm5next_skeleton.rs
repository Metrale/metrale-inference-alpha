...
//!   whose `hc_head` is a learned sigmoid-weighted sum. Metrale Engine's `hc_head` CUDA kernel is the
...
pub struct StateBudget {
    ...
    /// mHC highway per TOKEN — `hc_mult x hidden` fp32 in Metrale Engine (bf16 in HF; see the OPEN

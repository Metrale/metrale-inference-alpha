...
impl Qwen3SsmLayer {
    ...
    /// so the `METRALE_SSM_DETAIL` summary spans the whole mixer.
    ...
    pub(super) fn decode_ms_ssm_recurrent<'a, 'b: 'a>(
        ...
        // FP16 h-state (METRALE_SSM_H_FP16). The conversion itself happens in
        ...
        if let Some((h_state_base, conv_state_base)) = batched_recurrent {
                ...
                && crate::layers::qwen3_ssm::gdn_fused_norm_enabled()
            {
                ...
                // Enable with METRALE_GDN_SMEM_STAGE (PRESENCE — `=0` is NOT

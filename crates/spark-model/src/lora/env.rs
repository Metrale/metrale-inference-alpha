...
//! LoRA env/config leaves: the `$METRALE_LORA_*` runtime hatches (eager / rotate /
...
use metrale_core::config::{LayerType, ModelConfig, PeftAdapterConfig};

use super::LoraModule;

/// Permanent LoRA debugging hatch: `METRALE_LORA_EAGER=1` (or `true`) forces
...
/// `METRALE_LORA_ROTATE=1` (or `true`) ARMS runtime adapter rotation: it forces
...
/// `$METRALE_LORA_PEER` (host:port of an `metrale-weight-peer` staging a rotation
...
/// Feature-1 (MoE expert + router LoRA) master switch. `METRALE_LORA_EXPERTS=1`
...
/// Feature-1 padded expert/router LoRA rank cap (`METRALE_LORA_EXPERT_RANK`,
...
/// `METRALE_LORA_PREFILL_BGMV=1` — force prefill LoRA through the per-row BGMV
...
/// `METRALE_LORA_NO_BATCH_VERIFY=1` — restore the old refusal of cross-sequence
...
/// ran in `metrale_core::config::parse_peft_adapter_config`.
...
/// `METRALE_LORA_ALLOW_PARTIAL=1` — load an adapter that names target modules
/// Metrale Engine cannot apply, skipping those and applying the rest.
///
/// Delegates to the metrale-core definition rather than re-reading the env:
...
pub use metrale_core::config::allow_partial_targets;

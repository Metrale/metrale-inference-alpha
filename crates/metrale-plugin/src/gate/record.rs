...
pub struct GateRecord {
    ...
    /// behaviour — the confirmed provenance defect in metrale#812, and the one
    ...
    /// The `METRALE_*` serve levers the gate APPLIED to the server it measured
    ...
    /// `METRALE_FP8_ROWWISE=1` says so and one that was not cannot be mistaken
...
pub struct ModelBaseline {
    ...
    /// `METRALE_*` serve levers the gate pins for this entry, applied on top of
...
impl GateRecord {
    ...
    ///
    /// `baked` is `metrale_kernels::TARGET_CLOSURES` — computed by the build

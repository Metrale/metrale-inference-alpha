...
// 2026-09-25: Kernels `moe_topk_sigmoid` and `moe_topk_sigmoid_batched`: sigmoid MoE routing with a selection bias.
...
// One 256-thread block per token; only the first MAX_EXPERTS logits are read, and `bias` must hold
// num_experts floats.
//
// Owner: gb10 kernels.
// Invariants: top_k is clamped to MAX_TOP_K. The kernels assume blockDim.x == 256 (BLOCK_SIZE).
...
extern "C" __global__ void moe_topk_sigmoid(
    ...
    // 2026-09-25: MoeLayer::new_with_hash and NemotronMoeLayer::new refuse a top_k above MOE_TOPK_SIGMOID_MAX_TOP_K
    // (ops/moe_gate.rs), which moe_topk_sigmoid_bounds.rs pins to MAX_TOP_K. The clamp keeps any other caller
    // from writing past s_top_vals and s_top_idxs.
...
// 2026-09-25: `moe_topk_sigmoid_batched`: N tokens, one block per token, grid (N, 1, 1); `bias` is shared by all.
...
extern "C" __global__ void moe_topk_sigmoid_batched(
    ...
    unsigned int actual_n = num_experts < MAX_EXPERTS ? num_experts : MAX_EXPERTS;
    // 2026-09-25: Clamped for the same reason as in `moe_topk_sigmoid`.
    ...
    for (unsigned int t = 0; t < top_k_c && t < actual_n; t++) {
        ...
        // 2026-09-25: The same lower-index tie-break as `moe_topk_sigmoid`, so the single-token and batched kernels route
        // an exact tie to the same expert.

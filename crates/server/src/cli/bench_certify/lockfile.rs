...
//! 2026-09-26: The campaign lockfile: `.oracle_should_begin_cert` at the repo root, schema `oracle_should_begin_cert/v1`.
//!
//! Owner: server CLI (`met benchmark certify`).
//! The file is created with `link(2)`, names its owner and driver, and carries
//! a heartbeat the drivers refresh (`LockGuard::beat`).
//! Invariants:
//! - A lock whose recorded driver pid is alive is never reclaimed.
//! - A lock with no live driver is reclaimed when its status is terminal, or
//!   its heartbeat is older than [`STALE_AFTER_SECS`] or unreadable; the old
//!   file is archived as `<name>.stale.<unix>` first.
...
/// 2026-09-26: A heartbeat older than this, with no live driver, marks the lock stale.
...
/// 2026-09-26: The statuses `release_as` writes when a campaign is over. With no live
/// driver, a lock in one of these is reclaimable at once.
pub const TERMINAL_STATUSES: [&str; 2] = ["campaign_done", "aborted"];

/// 2026-09-26: The decision about an existing lock, made from its contents, whether its
/// driver is alive, and the time.
#[derive(Debug, PartialEq, Eq)]
pub enum Existing {
    /// 2026-09-26: Someone's campaign is running; refuse, naming them.
    Live(String),
    /// 2026-09-26: Dead driver, and a terminal status or an old or unreadable
    /// heartbeat: reclaim after archiving.
    Stale(String),
}

/// 2026-09-26: Is the lock at `existing` live or stale? `driver_alive` says whether the
/// recorded driver pid is alive; `now` is unix seconds.
pub fn classify(existing: &LockFile, driver_alive: bool, now: u64) -> Existing {
    ...
    // 2026-09-26: A terminal status with no live driver is stale at once, whatever
    // the heartbeat: `release_as` leaves the file in place with the campaign's
    // last heartbeat, which can still be recent.
...
/// 2026-09-26: A held lock. Dropping it removes the file, unless `release_as` ran.
#[derive(Debug)]
...
impl LockGuard {
    /// 2026-09-26: Create the lock atomically. An existing live lock is refused, an
    /// unparsable one is refused; a stale one is archived beside itself as
    /// `<name>.stale.<unix>` and replaced.
    pub fn claim(
        root: &Path,
        owner: LockOwner,
        campaign: Campaign,
    ...
    /// 2026-09-26: Record the heartbeat, which gate is running and what the guard last said.
    ...
    /// 2026-09-26: Leave the file in place with a terminal status, instead of removing
    /// it on drop.
...
/// 2026-09-26: `YYYY-MM-DDTHH:MM:SSZ` (UTC) from unix seconds.
...
// 2026-09-26: Howard Hinnant's civil-from-days, proleptic Gregorian.

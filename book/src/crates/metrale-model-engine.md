...
**Key files:** `engine.rs`, `model/`, `factory.rs`, `traits.rs` in `crates/model-engine/src/`; `layer.rs` + `layers/*.rs`, `weight_map.rs`, `speculative.rs`, `vision_preprocess.rs`, `quant_format/` in `crates/model-layers/src/` ([metrale-model-layers](./metrale-model-layers.md)); `weight_loader/*.rs`, `mistral_loader.rs` in `crates/model-arch/src/` ([metrale-model-arch](./metrale-model-arch.md)); `preflight.rs` in `crates/model-weights/src/` ([metrale-model-weights](./metrale-model-weights.md)).

This is the largest crate in the workspace — ~18k lines — because every model architecture Metrale Engine supports has its own loader here. The design centers on two small, heavily-used traits.
...
- `mistral_loader.rs` — the one outlier (lives one level up in `crates/model-arch/src/` because the Mistral-Small-4 loader predates the `weight_loader/` submodule reorganisation).
...
This is the **single code site where `model_type` strings are matched**. Everything downstream of `factory::build` holds `Box<dyn Model>` and is model-agnostic. See the top-level repo [Adding a new model](https://github.com/Metrale/metrale-inference-alpha/blob/main/docs/HARDWARE.md#adding-a-new-model-family) guide.
...
Prefill → decode loop, sampler integration, finish-reason detection (`"stop"` / `"length"`), EOS + stop-token handling. The scheduler in `metrale-server` drives this — `engine.rs` itself is stateless per call; the per-sequence state lives on `Sequence` (allocated/freed around the generate).

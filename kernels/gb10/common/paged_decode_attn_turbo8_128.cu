...
// 2026-09-25: Paged decode attention over a turbo8 KV cache. HDIM, the head size
// this build serves, is 128 unless the model's KERNEL.toml passes -DHDIM.
//
// K and V are stored WHT-rotated: one FP8 E4M3 byte per element and one BF16
// scale per 16-element group. The kernels apply no WHT: Q must be in the basis
// K and V were written in, which the caller arranges (write_kv_cache.rs,
// qwen3_attention/decode/attention_forward.rs).
//
// Per KV cache block, as paged_decode_attn_turbo8 reads it (token-major):
//   data:  block_size * num_kv_heads * head_dim bytes, FP8 E4M3
//   scale: block_size * num_kv_heads * (head_dim / 16) * 2 bytes, BF16,
//          starting at data_section_bytes
// The host reference is metrale_cache::kv_dequant::dequant_turbo8_block_to_bf16.
//
// Entry points, (grid) x threads:
//   paged_decode_attn_turbo8        (num_q_heads, num_seqs, 1) x 256
//   paged_decode_attn_splitk_nvfp4  (num_q_heads, num_splits, num_seqs) x 256
//   paged_decode_attn_reduce_nvfp4  (num_q_heads, num_seqs, 1) x 32
// Split-K partials are FP32 [num_seqs, num_q_heads, num_splits, head_dim + 2]:
// o, then the running max, then the softmax sum. Here the split-K
// kernel steps the scales one byte per group but reads BF16. The serve loads
// only paged_decode_attn_turbo8 from this file: qwen3_attention/init.rs
// leaves the split-K handles empty for turbo8.
//
// Owner: gb10 kernels.
// Invariants:
// - Assumes head_dim == HDIM: each of the 32 lanes owns HDIM / 32 elements.
// - A sequence with seq_lens[s] == 0 writes no output.

#include <cuda_bf16.h>
// 2026-09-25: paged_decode_attn_turbo8 skips the V load and dequant for a
// position whose exp(score - running max) is at most this; that V adds zero.
...
// 2026-09-25: Dequantizes this lane's HDIM / 32 elements, which sit in one
// 16-element group: FP8 E4M3 bytes at data_ptr times the BF16 group scale at
// scale_ptr. `lut` is not read.
...
extern "C" __global__ void paged_decode_attn_turbo8(
    ...
    // 2026-09-25: Not read in this file: nvfp4_dequant ignores its lut argument.
    ...
    const unsigned int head_data_bytes = head_dim;

...
/// Whether the SSM spill tier is engaged (`METRALE_SSM_TIER`). Default off ⇒
...
/// `METRALE_SSM_RDMA_TIER=host:port` selects the RDMA arena
/// ([`RdmaSnapshotStore`] over a peer blade, `METRALE_SSM_RDMA_ARENA_SLOTS` slots,
...
/// optional, never a hard model-init error. With `METRALE_SSM_RDMA_TIER` unset the
/// result is exactly `MemBlobStore::new(0)` as before ⇒ byte-identical.
///
/// `METRALE_SSM_TIER_DISK_GB` bounds the UNIFIED arms' swap file (see
...
/// `METRALE_SSM_SWAP=1` the PEER owns residency and applies its own
/// `--swap-cap-gb` budget instead.
///
/// `Err` is reserved for CONFIG errors (a bad `METRALE_SSM_SWAP_NS` override or
/// `METRALE_SSM_TIER_DISK_GB` — PCND fail-fast, resolved BEFORE any connect
...
pub(crate) fn build_tier_store(
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        // WS-A: METRALE_SSM_SWAP=1 selects PAGING mode — the peer (started with
        ...
        if std::env::var("METRALE_SSM_SWAP").ok().as_deref() == Some("1") {
            // Namespace = METRALE_SSM_SWAP_NS (explicit u64 override, strict) or
    ...
    if ssm_tier_unified() {
        ...
        let hot = Box::new(metrale_tier::VecSlotArena::new(blob_bytes, hot_slots));
...
/// Marconi `build_tier_store`, its own `METRALE_SSM_DECODE_*` env namespace so keys
...
/// `METRALE_SSM_TIER_DISK_GB` because a dropped blob there is a clean miss →
...
///
/// Selection (`METRALE_SSM_DECODE_TIER`):
///   - `nvme` + `METRALE_SSM_DECODE_NVME_DIR=<dir>` → [`FileSnapshotArena`] behind
///     [`ArenaSnapshotStore`], provably sized ≥ `min_slots`.
///   - `peer`  + `METRALE_SSM_DECODE_RDMA_TIER=host:port` → the never-dropping
///     [`PagingSnapshotStore`] (peer LRU-spills to its own NVMe), own
///     `METRALE_SSM_DECODE_NS` namespace fold.
    ...
    match std::env::var("METRALE_SSM_DECODE_TIER").ok().as_deref() {
        Some("nvme") => {
            ...
            if ssm_tier_unified() && blob_bytes > 0 && blob_bytes.is_multiple_of(4096) {
                ...
                // NOT `new_capped`: METRALE_SSM_TIER_DISK_GB bounds the Marconi
                ...
                let swap = metrale_tier::DirectSwapFile::create(&path, blob_bytes)?;
        ...
        Some("peer") => {
            ...
            // Namespace = METRALE_SSM_DECODE_NS (explicit u64 override, strict)

...
# 2026-09-26: Walks the levels of prompts/graduated.jsonl in order with run_level.py.
# A pass is committed and the next level starts; a failure waits for a signal file,
# and skips the level after OVN_FIX_WAIT seconds (default 5400). The whole walk
# stops after OVN_DURATION_H hours (default 12).
#
# Owner: bench, FP8 drift harness.
# Invariants: none beyond the types.
#
# Signal files:
#   /workspace/overnight_RESUME  -> run the failed level again
#   /workspace/overnight_SKIP    -> go on to the next level
set -uo pipefail

H="$(cd "$(dirname "$0")/.." && pwd)"
OVN="$H/overnight"
REPO=/workspace/metrale-mtp
...
for l in open('$H/prompts/graduated.jsonl'):
  ...
  # 2026-09-26: Also commits every change under crates/ and kernels/ in REPO.
...
for l in open('$H/prompts/graduated.jsonl'):
  ...
  while :; do
    ...
    # 2026-09-26: After RESUME the loop runs the same level again.
  done
done

...
//! 2026-09-26: Tests for inter-token latency on both clocks, the server's
//! raw timing window, and the arrival-gap instrument end to end.
//!
//! Owner: bench (HTTP client).
//! Invariants: none beyond the types. A child of `http_tests`, so it uses
//! that module's `sse` helper.

use super::*;

/// 2026-09-26: A mock that writes its reply in timed phases, so the client's
/// clocks can be checked against a known schedule.
...
/// 2026-09-26: The server's raw timing components ride in `usage` beside its
/// rate; a server without them leaves them `None`, and the server-clock ITL
/// is `itl_ms` over `decode_time_ms`.
#[test]
fn server_raw_timing_components_are_captured_and_the_server_itl_is_derived() {
    ...
    // 2026-09-26: With this fixture, (total − ttft)/(n−1) equals
    // decode/(n−1).
...
/// 2026-09-26: The ITL numerator ends at the final response chunk, not the
/// last content token. Three tokens arrive at once, then the usage chunk and
/// `[DONE]` follow 300 ms later, so the ITL is at least 300 ms over n−1 = 2.
#[tokio::test]
async fn client_itl_numerator_ends_at_the_final_chunk_not_the_last_token() {
    ...
    // 2026-09-26: On the client clock, (e2e − ttft) / (n − 1) == tpot.
    let identity = (out.e2e_ms - out.ttft_ms.unwrap()) / 2.0;
    assert!((identity - tpot).abs() < 1e-6, "{identity} vs {tpot}");
}

/// 2026-09-26: A gap is between arrivals (socket reads that carried a
/// token), so a burst of two deltas in one write is one arrival; and one
/// stalled arrival sets the max and the p99.
#[tokio::test]
async fn arrival_gaps_are_per_read_and_a_stall_shows_in_the_tail() {
    ...
    for i in 1..10 {
        // 2026-09-26: Every even `i` is a two-delta burst in one write.
        let bytes = if i % 2 == 0 {
...
/// 2026-09-26: A reply delivered in one read has no gap: the instrument
/// reports nothing rather than a zero, and ITL is `None` below two tokens.
#[tokio::test]

...
├── registry.rs       Kernel module registry shared by metrale-kernels codegen
...
The `metrale-kernels` crate's auto-generated `target_ptx.rs` emits one `pub static PTX_<TARGET>: &[PtxModule]` per known `KernelTarget`. Startup dispatch in `spark-server::main` calls `metrale_kernels::select_target(&ptx_sets, &target)` to pick the right module set.

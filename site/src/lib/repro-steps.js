...
//      both gate-only options of `met benchmark run`) are appended and
...
export const METRALE_REPO = 'https://github.com/Metrale/metrale-inference-alpha';
export const RECIPES_REPO = 'https://github.com/Metrale/metrale-recipes';
 ...
 * options of `met benchmark run` (`cli/bench_args.rs`); both are consulted
...
export function reproSteps(record, ctx) {
  ...
  const checkout = {
    ...
    facts: [
      ['commit', sha ? `${METRALE_REPO}/commit/${sha}` : 'not recorded'],
  ...
  if (sha) {
    const lines = [`git clone ${METRALE_REPO}.git && cd metrale`];
  ...
  const build = {
    id: 'build',
    title: 'Build',
    summary: `cargo build --release -p spark-server --bin met · metrale ${rec.metrale_version ?? 'version not recorded'}`,
    facts: [['metrale_version', rec.metrale_version ?? 'not recorded']],
    commands: [
      {
        label: 'binary',
        lines: [
  ...
  if (!rec.metrale_version) miss('metrale_version', 'the binary version string; compare `met --version` after building');
  ...
  if (partition) {
    ...
    measure.commands.push({
      ...
      lines: [`met benchmark aggregate ${bench}${sha ? ` --sha ${shellQuote(sha)}` : ''}`],
      derivedLines: []
    });
  ...
  for (const f of files) {
    if (f.path) recordStep.facts.push(['record', `${METRALE_REPO}/blob/main/${f.path}`]);
    if (f.signer) recordStep.facts.push(['signer', `${METRALE_REPO}/blob/main/.github/record-signers/${f.signer}.pub`]);
  }

...
# 2026-09-25: Writes /w/ffn_golden.json: the GLM-5.3 dense FFN (layer 0), shared expert and routed
# experts 0-3 (layer 3) on real checkpoint packets, over the REGIMES inputs.
#
...
# 2026-09-25: The NVFP4 dequant below is this script's own; nothing is imported from the engine.
E2M1 = torch.tensor([0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0,
                     -0.0, -0.5, -1.0, -1.5, -2.0, -3.0, -4.0, -6.0], dtype=torch.float32)


def e4m3_decode(u8: torch.Tensor) -> torch.Tensor:
    """FP8 E4M3 (no infinities; 0xFF/0x7F are NaN) -> fp32. Independent of Metrale Engine's LUT."""
    u = u8.to(torch.int32)
    ...
    val = torch.where((exp == 15) & (man == 7), torch.zeros_like(val), val)
    return sign * val
...
# 2026-09-25: `clamp64` scales the input by 8.0 to push activations past +/-LIMIT. The
# g>+10, g<-10 and |up|>10 columns of the printed table count the elements that crossed.
...
for e in range(4):
    ...
    st = {}
    # 2026-09-25: The dequantised weights are emitted too (strided sample plus checksum).
    for p in ("gate_proj", "up_proj", "down_proj"):
        st[f"deq_{p}"] = W[p].flatten().clone()
    for rname, T, sc in REGIMES:
        x = Lcg(0x0FFF5EED).t(T, HID) * sc
        xb = x.to(torch.bfloat16).float()          # 2026-09-25: BF16-rounded input, FP32 matmul
        g32 = x @ W["gate_proj"].T
        u32 = x @ W["up_proj"].T

...
//! decode_a2 padded_n-graph pattern. Kill switch `METRALE_NO_MTP_VERIFY_GRAPHS`
...
impl TransformerModel {
    ...
    pub(super) fn can_batch_verify_dispatch(&self, ks: &[usize]) -> bool {
        ...
        (2..=crate::layer::VERIFY_WY_TABLE_SEQS).contains(&n)
            ...
            // loop. `METRALE_LORA_NO_BATCH_VERIFY=1` restores the refusal.
    ...
    pub(super) fn decode_verify_batched_dispatch(
        ...
        // METRALE_K4_DIAG=1: stream-sync checkpoint after every layer so an
        ...
        // Graph outcome for the periodic METRALE_MTP_ACCEPT_DEBUG summary
        ...
        if let Some(graph) = replay {
            ...
            // pickup. Kill switch METRALE_NO_MAPPED_ARGMAX=1 restores scratch +
        ...
        // METRALE_MTP_TIMING attribution (2026-07-30): everything above this
        ...
        // Kill switches: METRALE_NO_PINNED_VERIFY_D2H=1 -> pageable on-stream;
        // METRALE_VERIFY_D2H_DEFAULT_STREAM=1 -> the original default-stream arm.
        if filled {
        ...
        {
            // Local fwd-split telemetry (METRALE_MTP_TIMING=1): launch region vs
...
/// `METRALE_NO_MAPPED_ARGMAX=1` is set — callers then use the scratch + copy
/// path unchanged.

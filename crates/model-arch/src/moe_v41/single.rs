...
//! 2026-09-25: The single-token expert path: the plan that groups a token's
//! selected experts, the pointer table for them, the routed m=1 compute that
//! sums the expert rows in plan order, and the host stage / device compute
//! halves a captured decode step runs.
//!
//! Owner: model-arch (DeepSeek-V4.1 MoE).
//! Invariants:
//! - `compute_m1` and `compute_m1_joined` refuse an `ne` other than `topk`.
...
impl MoeV41 {
    /// 2026-09-25: Group the `(token, k)` assignments by expert (ascending expert id):
    ...
    /// 2026-09-25: The single-token arm's pointer table: gate, up and down block pointers
    ...
    /// 2026-09-25: The token's q8_1 rows into `a_q8`, once for the routed experts and the
    ...
    /// 2026-09-25: The single-token routed experts from the pointer table: the
    /// token quantised once (already in `a_q8` when `x_pre`), gate and up as
    /// one launch over `2 * ne` table entries and down as one over `ne`, the
    /// routing weight folded in at the SwiGLU (which writes `h` and its q8_1
    /// rows in one launch), the rows summed into `acc` in plan order. Device
    /// work only.
    pub(super) fn routed_m1(
        ...
        // 2026-09-25: Gate and up in one 2 * ne expert batch: the pointer
        // table holds the ne gate pointers then the ne up pointers
        // contiguously, every entry reads the same q8_1 token row (row stride
        // 0), and entry e writes row e of gate_out, so up's rows start at
        // gate_out + ne * inter.
        ...
        // 2026-09-25: All `ne` rows land in the one token row, so one kernel
        // adds them in plan order. `scatter_add` is not used here: it runs one
        // block per row and assumes the rows are distinct tokens, so its
        // blocks would race on the same `acc` row.
    ...
    /// 2026-09-25: The host span of a single-token step between the router
    /// GEMV and the expert compute: the next layer's router on `x` (when
    /// `next` is given, `prefetch_k() > 0` and the cache has a reader pool),
    /// the selection (drains the stream), the cache fetch with the predicted
    /// experts, and the rows / weights / pointer-table uploads into fixed
    /// buffers. `route_launch` must have been issued on `stream` first.
    ...
    /// 2026-09-25: The single-token expert compute after `stage_m1`: the accumulator
    ...
    /// 2026-09-25: The single-token shared expert up to `sd` on `side`,
    /// reading the token's q8_1 rows that [`Self::ffn_input_q8_m1`] wrote on
    /// the forking stream, with its own q8_1 scratch `sh_q8` (as `forward`'s
    /// side-stream arm): the fork half of [`Self::compute_m1_joined`]. Device
    /// work only.
    ...
    /// 2026-09-25: [`Self::compute_m1`] with the shared expert forked onto a
    /// side stream by [`Self::shared_expert_on`]: the routed experts, then the
    /// wait on `join` (recorded on the side after `sd`), then the accumulate
    /// and finish, the same launch order as `forward`'s side-stream arm. The
    /// token's q8_1 rows must already be in `a_q8` ([`Self::ffn_input_q8_m1`]).

...
impl Qwen3SsmLayer {
    ...
    pub(super) fn prefill_inner(
        ...
        // Per-SSM-layer-prefill counter — used by METRALE_GDN_DUMP hooks
        ...
        // Profiling helper: sync + timestamp when METRALE_PROFILE=1
        macro_rules! prof {
            ($label:expr, $t0:expr) => {
                if ctx.profile {
                    if let Some(t0) = $t0 {
        ...
        // METRALE_GDN_DUMP hook #0a: pre-input-norm hidden state for THIS
        ...
        // METRALE_GDN_DUMP hook #0b: post-input-norm (input to in_proj_qkv).
        super::debug::maybe_dump_gdn_buf(
            ctx.gpu,
            normed,
            (num_tokens - 1) * h * 2,
        ...
        // METRALE_DUMP_EXPERT_IDS=1: also dumps residual_add_rms_norm INPUTS (hidden + out_proj_buf) for drift attribution.
        ...
        // METRALE_GDN_DUMP hook: MoE output — KEY drift attribution test.

...
// fixtures next to this one it is rejected on every architecture Metrale Engine
...
#define METRALE_GATE_OVERSIZED_SHARED 262148

extern "C" __global__ void metrale_gate_two_entries_bad_a(char *out) {
  __shared__ char buf[METRALE_GATE_OVERSIZED_SHARED];
  volatile char *p = buf;
...
extern "C" __global__ void metrale_gate_two_entries_bad_b(char *out) {
  __shared__ char buf[METRALE_GATE_OVERSIZED_SHARED];
  volatile char *p = buf;
...
extern "C" __global__ void metrale_gate_two_entries_good(const float *in,
                                                       float *out, int n) {

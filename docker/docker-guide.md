# Metrale Engine Docker Guide

Metrale Engine provides per-model Dockerfiles organized by `(Hardware, Model, Quantization)` tuple.
...
one architecture and `met`'s arch preflight refuses to start on any other
...
```bash
# 80B model
docker build -f docker/gb10/qwen3-next-80b-a3b/nvfp4/Dockerfile -t metrale-80b .

# 35B model
docker build -f docker/gb10/qwen3.5-35b-a3b/nvfp4/Dockerfile -t metrale-35b .
```
...
```bash
docker build -f docker/hopper/Dockerfile -t metrale-hopper:latest .
docker build -f docker/b200/Dockerfile   -t metrale-b200:latest .
```
...
docker build -f docker/hopper/Dockerfile \
  --build-arg METRALE_TARGET_MODEL=nemotron-super-120b-a12b \
  -t metrale-hopper:nemotron .
```

Build args: `METRALE_TARGET_HW` (defaults to `hopper` / `b200` per file),
`METRALE_TARGET_MODEL` (`*` by default — `deepseek-v4-flash`,
`nemotron-3-nano-30b-a3b`, `nemotron-super-120b-a12b`, `qwen3.6-35b-a3b`,
`qwen3-next-80b-a3b`), `METRALE_TARGET_QUANT` (`nvfp4`), and `METRALE_GIT_SHA`
...
`metrale-hopper` carries `sm_90a` PTX and nothing else; `metrale-b200` carries
`sm_100a` and nothing else. Point either at the wrong GPU and `met`'s arch
...
`.github/workflows/datacenter-binaries.yml` builds the same `met` on a
GitHub-hosted runner and uploads `met-hopper-x86_64` / `met-b200-x86_64`.
See [`docs/DEPLOYMENT.md`](../docs/DEPLOYMENT.md#a-prebuilt-binary-for-a-rental-box).

## API

Metrale Engine serves an OpenAI-compatible API on the configured port.

...
//! projection — the `METRALE_CUBLAS_GEMM=ssm` route for the second half of the
//! GDN block, alongside `prefill_w8a8.rs`'s `in_proj_qkvz` arm.
//!
//! WHY THIS EXISTS (#917 / #928). nsys on 1xH100, 2026-09-11 round 9,
//! `Qwen/Qwen3.8-27B-FP8`, best recipe `METRALE_CUBLAS_GEMM=ffn,ssm,attn`
...
//! and its only W8A8 arm was gated on a bare `METRALE_FP8_W8A8=1` env read —
//! **not** on `ctx.dispatch.fp8_blockscaled_prefill` (the gate the QKVZ,
//! attention and dense-FFN prefills use) and **not** on any `METRALE_CUBLAS_GEMM`
...
//! the real shapes. `METRALE_SSM_OUT_W8A16_ONLY` (presence) restores W8A16.
...
/// `METRALE_SSM_OUT_W8A16_ONLY` kill switch: PRESENCE (any value, including
/// empty) keeps the SSM `out_proj` prefill on the W8A16 kernels. Presence
/// rather than `=1` for the same reason as `METRALE_FFN_W8A16_ONLY` — this is an
...
/// * `cublas_ssm` — `METRALE_CUBLAS_GEMM` must name the `ssm` family. Scoped,

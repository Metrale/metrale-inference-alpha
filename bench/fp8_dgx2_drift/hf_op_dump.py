...
# 2026-09-26: HF BF16 CPU forward with per-op hooks; writes each op's last-token output as f32.
#
...
submodule that maps to a Metrale Engine op-dump. Captures the LAST token of each
op's output as f32 to `<dir>/hf_op_L{i}_{op}.bin`.

Metrale Engine op names → HF module mapping (per layer):
  ...
  - layer_out        : output of full layer (already covered by metrale_L*.bin)
...
TOKENS_PATH = pathlib.Path(
    "/workspace/metrale-mtp/bench/fp8_dgx2_drift/metrale_tokens_dgx2.json"
)
OUT_DIR = pathlib.Path("/workspace/metrale-dumps/op_drift")
...
def extract_last(t: torch.Tensor) -> np.ndarray:
    ...
    if t.is_floating_point() is False:
        ...
        t = t.float()
    a = t.cpu().numpy()
    # 2026-09-26: With 2+ dims, drop a leading dimension of size 1, then take the
    # last index of the first remaining axis.
    if a.ndim >= 2:
        if a.shape[0] == 1:
            a = a[0]
        a = a[-1]

...
//!
//! Reachability: only via `step_mtp` Phase B when `METRALE_MTP_MAX_SEQS > 1`
//! (default 16 with the ladder) puts >= 2 verify-ready grammarless
//! sequences holding drafts in one step AND the model says
//! `can_batch_verify(&ks)`. `METRALE_MTP_MAX_SEQS=1` keeps this
//! path dead and the single-seq path byte-unchanged.

use super::*;

/// Kill switch `METRALE_NO_MTP_BATCH_VERIFY` — PRESENCE check per the
...
pub(super) fn batch_verify_disabled() -> bool {
    ...
    *CACHED.get_or_init(|| std::env::var("METRALE_NO_MTP_BATCH_VERIFY").is_ok())
}
...
pub(super) fn step_verify_k4_batched(
    ...
    // METRALE_MTP_TIMING step summary (same Drop-guard pattern as the
    ...
    for (i, a) in batch.iter_mut().enumerate() {
        ...
        // Width-attributed accept telemetry (METRALE_MTP_ACCEPT_DEBUG). The
...
/// Kill switch `METRALE_NO_MTP_BATCH_PROPOSE` — PRESENCE check (`=0` is NOT
...
fn batch_propose_disabled() -> bool {
    static CACHED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *CACHED.get_or_init(|| std::env::var("METRALE_NO_MTP_BATCH_PROPOSE").is_ok())
}

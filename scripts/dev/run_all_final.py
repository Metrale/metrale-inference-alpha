...
MODELS = [
    ...
    ("Sehyo/Qwen3.5-35B-A3B-NVFP4", "--speculative --scheduler slai --max-batch-size 16", "dgx1", "35B+MTP"),
...
for pair in pairs:
    ...
    for model, extra, node, short in pair:
        host = DGX2 if node == "dgx2" else None
        stop(f"metrale-{node}", host)

# ═══════════════════════════════════════════════════════════════
# EP=2: Qwen3.5-122B
# ═══════════════════════════════════════════════════════════════
print("\n=== EP=2: Qwen3.5-122B ===", file=sys.stderr)
stop("metrale-dgx1"); stop("metrale-dgx2", DGX2)

start("metrale-dgx1", "Sehyo/Qwen3.5-122B-A10B-NVFP4", 8888, "", None, HF, 16384,
      f"--master-addr {EP_MASTER_ADDR} --world-size 2 --rank 0")
# Wait for NCCL listener
for _ in range(84):
    l = sh("sudo docker logs metrale-dgx1 2>&1 | tail -5", 10)
    if "waiting for" in l: break
    if "Error:" in l and "FP8" not in l: break
    time.sleep(5)
start("metrale-dgx2", "Sehyo/Qwen3.5-122B-A10B-NVFP4", 8889, "", DGX2, HF, 16384,
      f"--master-addr {EP_MASTER_ADDR} --world-size 2 --rank 1")

if ready("metrale-dgx1", secs=480):
...
# Save JSON
with open("/workspace/metrale/final_results.json", "w") as f:
    json.dump(all_results, f, indent=2, default=str)
print("\nJSON: /workspace/metrale/final_results.json")

#!/bin/bash
# Decisive gate for re-enabling METRALE_SSM_TAIL_MIDCHUNK on the GB10 golden config.
#
# WHY A SECOND ROUND
...
#
# Usage: midchunk_gate.sh <metrale_bin> <outdir> [reps] [pct_scale]
set -u
...
for leg in mc_off mc_on; do
  ...
  sudo docker rm -f metrale-mcg >/dev/null 2>&1; sleep 3
  # shellcheck disable=SC2086
  sudo docker run -d --name metrale-mcg --network host --gpus all --ipc=host \
  ...
  for _ in $(seq 1 180); do
    ...
    sudo docker ps --format '{{.Names}}' | grep -q metrale-mcg || { echo "SERVE_DIED leg=$leg"; break; }
    sleep 5
  done
  [ $ok -eq 1 ] || { sudo docker logs metrale-mcg 2>&1 | tail -40 > "$OUT/$leg.died.txt"; continue; }
...
PY
  ...
  sudo docker logs metrale-mcg 2>&1 | grep -c -aiE 'midchunk' > "$OUT/$leg.midchunk_hits.txt" || true
  sudo docker rm -f metrale-mcg >/dev/null 2>&1
done

#!/usr/bin/env python3
# 2026-09-26: Sends WARM deep prompts, then CHURN distinct deep prompts, then the WARM
# prompts again with a new question, and prints how to count spill and fault-in lines
# in the server log.
#
# Owner: bench, SSM tier probes.
...
THE SHAPE THAT DOES TRIGGER IT — three phases:
  ...
  1. WARM   : N deep conversations. Each must be deeper than
              METRALE_SSM_SPILL_MIN_TOKENS (default 1024) or eviction DROPS it
...
Run with METRALE_SSM_TIER_TIMING=1 and count in the server log:
...
# 2026-09-26: PREFIX_TOKENS counts generated words (`d<tag>w<k>`), not tokens. The
# spill gate defaults to 1024 tokens (ssm_spill_gate.rs DEFAULT_SPILL_MIN_TOKENS).

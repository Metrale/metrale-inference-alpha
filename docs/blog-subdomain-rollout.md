...
`/workspace/metrale-blog/.gate.sh` (untracked; mirrors what `.github/workflows/site.yml`
runs in CI) — for each of `site/` and `blog/`:

1. `bun test src/lib` — unit tests
2. `bun x --bun vite build` — the SvelteKit build, with `METRALE_RECIPES_ROOT`
   pointed at a local `metrale-recipes` checkout
...
1. *The working tree was stale.* `/workspace/metrale` sits on
   ...
   `/workspace/metrale-blog` cut from `origin/main`, never in `/workspace/metrale`.
...
**Changed.** `blog/deploy/nginx/blog.dev.metrale.ai.conf` is the SSOT for the
vhost; it is installed on the metrale origin as
`/etc/nginx/sites-available/00-blog.dev.metrale.ai.conf` and symlinked into
`sites-enabled`. Docroot `/var/www/blog.dev.metrale.ai/html`, owned
...
(Azure) at `Aug 29 00:56:38`, and `/var/www/dev.metrale.ai/build` has mtime
`Aug 29 00:56` and owner `ubuntu`. So `DEPLOY_SSH_USER=ubuntu` and
`DEPLOY_PATH=/var/www/dev.metrale.ai/build`.
...
| `/.env` | 404 |
| `http://…/x` | 301 → `https://blog.dev.metrale.ai/x` |
| `nginx -t` | ok |
...
```
$ curl -D- https://book.dev.metrale.ai/index.html
cache-control: public, max-age=300
...
> **Open, and deliberately not acted on:** `book.dev.metrale.ai` is serving
...
`site/src/app.css` into `web-shared/metrale-tokens.css`, which both apps now
...
blog (18/18); pointed at `book.dev.metrale.ai`, which still has the
...
**Deployed.** `blog.dev.metrale.ai` now serves the built site through
...
(`/var/www/blog.dev.metrale.ai/html`). The blog deploy step **fails hard** if
...
1. The blog was on `#14111f` — the marketing site's ramp — not the reference's
   `#0F1216`.
2. The blog's header and footer used `favicon.svg` plus the word "Metrale Engine" set in
   the UI font. The reference uses the **real lockup**: the mark, the wordmark
   outlines including the Metrale Engine signature "A" with its arrow shaft, and the
   tagline. Confirmed by the user: *"the Metrale Engine 'A' does not have an arrow on the
...
`web-shared/metrale-tokens.css` now holds the reference ramp under the marketing
...
`web-shared/components/MetraleLockup.svelte` carries the reference `<defs>`
...
`dev.metrale.ai` button was an anchor containing one `aria-hidden` arrow and
...
| `dev.metrale.ai`, `docs.`, `blog.` | all 200 |
...
`dev.metrale.ai/#fragment` from the built blog and requires it to be an `id`
...
```
$ bun blog/e2e/check-headers.mjs https://blog.dev.metrale.ai
...
| asked for | state |
|---|---|
| nginx vhost on the metrale server | **live**, in-repo as SSOT, security headers proved by request |
...
**Open, and deliberately not acted on:** `book.dev.metrale.ai` still serves
every HTML document with no `X-Content-Type-Options`, `X-Frame-Options` or
...
`book.dev.metrale.ai` is required, from the dashboard or a token with the
...
|---|---|---|
| dev.metrale.ai | `site/scripts/gen-llms.mjs` (existed) | already generated; now also cross-links docs and blog |
| blog.dev.metrale.ai | `blog/src/routes/llms.txt/+server.js` (new) | prerendered from the post index, like the feed and the sitemap |
| book.dev.metrale.ai | `book/scripts-gen-llms.mjs` → `book/src/llms.txt` (new) | generated from `SUMMARY.md`, the book's own table of contents |
...
`dev.metrale.ai` at **2 of 3** — no `Referrer-Policy`. Investigating found

...
//!
//! Most model-derived state in Metrale Engine is carried: `SchedCtx`, `ForwardContext`,
...
//!
//! Called from `MetraleCudaBackend::new`, which is where a model's GPU state
...
pub struct RunMetrics {
    ...
    // /metrics as `metrale_prefix_cache_*_total`, declared `TYPE counter`, and a
...
mod tests {
    ...
    /// `metrale_prefix_cache_hits_total`, declared `TYPE counter` — so the old
...
mod swap_counter_tests {
    ...
    fn a_new_run_does_not_move_the_counters_prometheus_exports() {
        // metrale_prefix_cache_hits_total is declared TYPE counter, and a counter

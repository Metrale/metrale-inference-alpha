...
impl SsmSnapshotPool {
    ...
    /// tier-store-present, `METRALE_SSM_FAULT_MIN_TOKENS` depth); this owns the
    ...
    pub(in crate::model) fn fault_in_for_key(
        ...
        match self.fault_in_slot(slot, key, store, gpu, stream) {
            ...
            // `METRALE_SSM_TIER_DISK_GB` that is `make_disk_room` having unmapped
            ...
            Ok(false) => {
                ...
                if prefix_cache.forget_snapshot_tier_key(key) {
                    ...
                    // METRALE_SSM_TIER_DISK_GB budget.
                    store.remove(key);
                    tracing::info!(
    ...
    pub(super) fn spill_slot(
        ...
        //     ordinary heap `Vec` and `metrale-tier`'s dependency budget forbids
        ...
        // `metrale-tier` dep) is the obvious escape and is also rejected: the

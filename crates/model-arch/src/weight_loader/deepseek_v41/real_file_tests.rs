...
//! 2026-09-25: Layer 0 of a real DeepSeek-V4.1 checkpoint against the CPU
//! reference, stage by stage (HC mixes, collapse, attention norm, attention,
//! `hc_post`, MoE), on the embedding rows of five token ids. Layer 0 is built
//! with ratio 0 and no compressor, indexer or engram weights. Ignored by
//! default: it needs a CUDA GPU and the checkpoint at `MODEL_DIR`.
//!
...
use crate::attn_v41::{AttnV41, AttnV41Cfg, AttnV41LayerWeights, LayerRole, SharedV41};
use crate::deepseek_v41_ref::attn::{AttnWeights, freqs_cis};
use crate::deepseek_v41_ref::compress::{
    CompAttnCfg, IndexerCfg, LayerAttnState, SharedRuntime, attention_any,
};
use crate::deepseek_v41_ref::hc::{hc_mixes, hc_post, hc_pre, rms_norm};
use crate::deepseek_v41_ref::moe::{MoeCfg, MoeWeights, gate as ref_gate, moe as ref_moe};
use crate::moe_v41::{MoeV41, MoeV41Cfg, MoeV41LayerWeights};
use metrale_gpu_runtime::kernel_args::KernelLaunch;
use metrale_model_layers::layers::ops;
use metrale_model_weights::weights::GgufLoader;
...
fn up_bf16(gpu: &dyn GpuBackend, v: &[f32]) -> DevicePtr {
    let b = crate::moe_v41::bf16_bytes(v);
...
fn layer0_matches_the_cpu_reference_on_the_real_weights() {
    let set =
        metrale_kernels::ptx_for_exact_target("deepseek-v4-flash", "nvfp4").expect("kernel target");
    ...
    let config =
        metrale_model_weights::weights::config_from_gguf_dir(dir).expect("config from gguf");
    ...
    // 2026-09-25: The reference collapses the FFN input with the attention
    // site's `pre` mix; no FFN-site mixes are computed.

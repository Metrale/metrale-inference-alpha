...
fn state() -> BenchState {
    ...
    s.select(0);
    s.target = metrale_plugin::TargetEndpoint::local(8888, "test-model");
...
fn a_finished_run_stays_reachable_after_navigating_away() {
    let mut s = state();
    s.frame = Some(metrale_plugin::BenchmarkResult {
        status: metrale_plugin::RunStatus::Completed,
        phase: "done".into(),
        progress: None,
        summary: Vec::new(),
        table: None,
...
fn the_coherence_probe_toggles_both_ways() {
    ...
    // back on.
    let mut s = params();
    assert_eq!(s.coherence, metrale_plugin::CoherencePolicy::Probe);
    s.on_key(key(KeyCode::Char('p')), BenchSub::Suite);
    assert_eq!(s.coherence, metrale_plugin::CoherencePolicy::Skip);
    s.on_key(key(KeyCode::Char('p')), BenchSub::Suite);
    assert_eq!(s.coherence, metrale_plugin::CoherencePolicy::Probe);
}
...
fn record() -> metrale_plugin::RunRecord {
    let descriptor = metrale_plugin::registry::find("concurrency-sweep").expect("registered");
    metrale_plugin::RunRecord::new(
        descriptor,
        &metrale_plugin::ParamValues::default(),
        &metrale_plugin::TargetEndpoint::local(8888, "m"),
        Default::default(),
        metrale_plugin::RunSource::Tui,
        crate::cli::METRALE_VERSION,
        metrale_plugin::BenchmarkResult {
            status: metrale_plugin::RunStatus::Completed,
            phase: "done".into(),
            progress: None,
            summary: Vec::new(),

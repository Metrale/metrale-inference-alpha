...
pub async fn webserver_test(
    ...
    build
        ...
        // METRALE_AGENT_SHELL is set, and its own comment reserves that for the
        // agent: "The SCORER (no METRALE_AGENT_SHELL) + build/test pass through."
...
/// ~/metrale/tests/foo.rs tests/foo.rs` would otherwise credit `wrote_tests` to an

...
/// projection on the in-tree kernel. `METRALE_CUBLAS_GEMM=1` used to arm both,
...
fn not_selected_for_unaligned_shapes() {
    ...
    // ("Scaling factors layouts"), and Metrale Engine hands the grid over as-is.
...
fn not_selected_without_a_usable_kmajor_scale_adapter() {
    let f = WeightQuantFormat::Fp8BlockScaled;
    // Guarded on the default layout: `METRALE_CUBLAS_SCALE_LAYOUT=rowmajor` is a

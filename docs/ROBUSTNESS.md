...
been installed by hand on this box and on metrale but exists nowhere in CI.
...
`<!-- metrale-certification-state:… -->` marker and PATCHing it in place — has no
...
*One.* `certed()` grepped the recorded calls for `metrale-certificate`. The
...
`segno` are both present on metrale, the branch exists — but it still contains
...
each show `Site unit tests: success` alongside `Deploy to metrale via rsync:
...
**One live defect fixed.** `dev.metrale.ai` was serving
...
| security reporting actually reachable | `security@metrale.ai` | valid Protonmail MX; private reporting, secret scanning and push protection all enabled |
...
1. `cargo test -p metrale-server --lib scheduler::rollback` printed
...
| tail split off (`METRALE_NO_TAIL_SPLIT=1`) | snapshot *producer* | 4 |
| Marconi restore off (`METRALE_MARCONI_MIN_TOKENS=1e8`) | snapshot *consumer* | **2** |
...
`METRALE_PREFIX_SUBBLOCK=0` — lever verified armed in `/proc/PID/environ` — the
...
1. **An A/B on a lever that was never armed.** `mtp_carry_drafter_enabled` is
   `levers.drafter.carry && !mtp_multi_seq_mode()`, and `METRALE_MTP_MAX_SEQS`

...
//!
//! Runs Metrale Engine's `GlmVit` on the golden `pixel_values` and compares the merged
...
//!
//! The golden is CPU **float32**. Metrale Engine computes in BF16 — the checkpoint's own
...
//!   Metrale Engine must deviate from fp32 by no MORE than that control does. This is a
...
use metrale_core::config::VisionConfig;
...
/// How much worse than the BF16 control Metrale Engine may be on the compounded stages.
...
fn glm_vision_tower_matches_the_transformers_golden() -> Result<()> {
    ...
    let cfg =
        metrale_core::config::parse_config(&std::fs::read_to_string(root.join("config.json"))?)
    ...
    macro_rules! grade {
        ($img:expr, $stage:expr, $early:expr, $actual:expr, $fp32:expr, $ctrl:expr, $rows:expr, $cols:expr) => {{
            ...
            if $early {
                ...
                // Metrale Engine must not deviate from fp32 by more than the

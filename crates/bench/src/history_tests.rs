...
//! 2026-09-26: Tests of run history: save and load, same-instant and
//! concurrent saves, whole-parameter records, bare-frame files, ordering,
//! skipped files, lookup by id, and publication by rename.
//!
//! Owner: bench.
...
/// 2026-09-26: A scratch directory removed on drop; the crate has no
/// `tempfile` dependency.
mod tempdir {
...
fn a_saved_record_reads_back_through_the_public_reader() {
    ...
    assert_eq!(b.metrale_version, "1.0.0-beta-preview");
    assert_eq!(
        b.target(),
...
fn two_runs_in_the_same_instant_do_not_overwrite_each_other() {
    // 2026-09-26: Two saves at the same nanosecond get distinct files.
    let (store, _d) = store();
...
fn a_legacy_bare_frame_still_appears_in_history() {
    // 2026-09-26: A bare `BenchmarkResult` file loads as a schema-0 record,
    // timed from its file stem.
    let (store, _d) = store();
...
fn baseline_json_is_not_mistaken_for_a_run() {
    // 2026-09-26: `benchmarks/baseline.rs` writes `baseline.json` into a
    // benchmark's runs directory.
    let (store, _d) = store();
    let dir = store.runs_dir("concurrency-sweep").expect("dir");
...
fn concurrent_writers_never_lose_a_run() {
    // 2026-09-26: `WRITERS` threads save at once; each gets its own file.
    let (store, _d) = store();
...
fn a_published_record_is_never_half_written() {
    // 2026-09-26: The claimed placeholder stays empty and the record arrives
    // by rename, so a reader never sees a partial file.
    let (store, _d) = store();
    let mut r = record(frame("done"));

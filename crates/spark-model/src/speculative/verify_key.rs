...
//!
//! Kill switch `METRALE_NO_CANONICAL_VERIFY_KEY` (PRESENCE — house convention,
...
/// Canonical assignment ON unless `METRALE_NO_CANONICAL_VERIFY_KEY` is present.
...
/// `METRALE_NO_CANONICAL_VERIFY_KEY=1`. tok/s, higher is better:
///
/// ```text
...
/// `METRALE_MTP_ACCEPT_DEBUG` reports that decline rate directly.
pub const CANONICAL_KEY_MIN_WIDTH: usize = 8;

/// Sweep the threshold without a rebuild: `METRALE_CANONICAL_KEY_MIN_WIDTH=<n>`
...
/// dominates — once `METRALE_NO_CANONICAL_VERIFY_KEY` is set, no width and no
/// `METRALE_CANONICAL_KEY_MIN_WIDTH` value can turn the assignment back on.
///

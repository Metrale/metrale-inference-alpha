...
//!   A=BLK128x128 (weight) + B=VEC128 (activation) pairing Metrale Engine uses is
...
//!
//! Metrale Engine maps `out[M,N] = act[M,K] @ weight[N,K]ᵀ` onto cuBLASLt as
...
///
/// True for every shape Metrale Engine serves today (K=5120 → L=40, K=17408 → L=136),

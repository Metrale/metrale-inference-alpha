...
impl MoeLayer {
    ...
    ///   1. Set `METRALE_UNIFIED_MOE_LAYOUT=1` so `MoeLayer::use_t_layout_for_decode()`
    ...
    /// NVFP4 path (`METRALE_HOLO_MOE_GROUPED_CUTLASS`). For each expert, swizzle the
    ...
    pub fn build_cutlass_grouped_sfb(
        ...
        // Prefer the Metrale-transposed [K/16,N] scales when they exist. Without

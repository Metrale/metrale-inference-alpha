...
// 2026-09-25: Two-token NVFP4 MoE GEMVs on the transposed weight layout, with the shared
// expert in the same launch: gate+up, then SiLU(gate) * up and down.
//
// Owner: gb10 kernels.
// Invariants:
// - Weights are [K / 2, N] bytes (low nibble first) with [K / 16, N] FP8 E4M3 scales;
//   each thread computes one output n. K must be a multiple of 16.
// - blockIdx.y < 2 * top_k is routed slot y (output row y); y = 2 * top_k + t is token
//   t's shared expert. A null routed or shared weight pointer writes zeros.
...
extern "C" __global__ void moe_expert_gate_up_shared_batch2_t(
    ...
    for (unsigned int sg = 0; sg < num_groups; sg++) {
        ...
        float sc = metrale_dec_e4m3(sb) * s2;

...
/// `METRALE_NLLB_BEAM_PROFILE=1`). GPU phases are bracketed by a `sync()` so the
...
impl NllbGpuModel {
    ...
    fn beam_multi_loop(
        ...
        // the batch row count (METRALE_NLLB_DEVICE_TOPK_MIN_ROWS, default 64);
        // METRALE_NLLB_HOST_TOPK=1 forces the host path (A/B + >16-beam requests).

...
accuracy 995 BFCL 86.43. Metrale already ≥ on tps/qps/wall/BFCL, ties IoU. **Pure decode gap:**
...
Win → commit immediately (tbraun96 author, Metrale co-author, no Claude attribution) → push.

## Exact reproduce commands
Build: `cd <worktree> && PATH=/usr/local/cuda/bin:$PATH METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b cargo build --release -p metrale-server --bin met --features cuda`
...
- Prior art to reuse: e2m1_branchless.cu, quantize_bf16_to_nvfp4.cu, dequant_nvfp4_bf16.cu,
  attn_prefill_paged_nvfp4.cu (FP4 MMA). Kernel toml/precedence: fp4_mma_gb10 memory.
...
INSIGHT: vLLM runs all-NVFP4 nvidia ckpt; Metrale mandated ckpt keeps GDN FP8 → Metrale streams MORE bytes.
...
(Metrale FP8-GDN vs vLLM all-NVFP4), not an engineering miss — will document honestly either way.
...
GDN IN-proj (feeds recurrent state). IoU more at risk than BFCL. qwen's decisive experiment: Metrale
...
⚠ Also unresolved: confirmed vLLM's REAL TPOT unknown (31.39 = weak run). On tps/qps basis Metrale
...
dispatch already scaffolded init.rs:371; loader linear_attn_arms.rs re-quant behind METRALE_GDN_OUT_NVFP4=1),
...
confirmed e2e (Metrale already ≥ vLLM on tps/qps/wall/BFCL/IoU) + C3 folded IF it passes + honest
...
GDN** (U8-packed on disk, decodes w4a16 not FP8 w8a16). So METRALE_GDN_OUT_NVFP4 is INERT on the gate
...
on the CONFIRMED vLLM's reported metrics (tps/qps/wall/BFCL/IoU) Metrale already WINS.
...
candidate) — report_dir endpoints-fresh/results/fp8kv_conglom_20260724_050249, serve metrale-fp8kv-conglom.
...
## ★★★★ 07:39 — dgx2 bf16 BASELINE e2e PERF (fresh main 011bee65) — Metrale CRUSHES confirmed vLLM
...
vs CONFIRMED vLLM (5361s/14.6tps/0.188qps/IoU 0.6269): **Metrale wall −15%, tps +20%, qps +18%.**
...
| metric | Metrale (main, bf16) | conf. vLLM | winner |
|---|---|---|---|
| wall (1007) | **4551.9s** | 5361s | **Metrale −15%** |
| tps | **17.56** | 14.6 | **Metrale +20%** |
| qps | **0.222** | 0.188 | **Metrale +18%** |
| TTFT median | **1264ms** | (higher) | **Metrale** |
| raw TPOT median | 38.18ms | ~31 (WEAK ref only) | vLLM on weak ref; residual = hardware roofline |
| BFCL | ~87 (prior) | 86.43 | Metrale |
| IoU | ~0.625 (prior) | 0.6269 | tie |
Metrale WINS the real comparison on every throughput+quality axis. The only "gap" (raw per-token TPOT) is
vs a WEAK/verbose vLLM reference AND is at the hardware floor. GOAL OUTCOME: the raw-TPOT gap is NOT
closable by kernel work (proven, roofline); Metrale already beats confirmed vLLM end-to-end.
...
not JIT). MTP acceptance: mean-len ~2.5, p1 0.83 / p2 0.65 = **≈ Metrale (p1 0.90, E~2.84), NO edge**.
→ vLLM TPOT swings 3.4×: **31.39ms tuned (07-16 Marlin+CUDA-graph) vs 104.86ms out-of-box.** Metrale
...
vLLM's single best config; Metrale wins every confirmed e2e metric. FINAL raw-TPOT answer: config-dependent,

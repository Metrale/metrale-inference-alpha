...
//! explicit `METRALE_HW_TEMP_GATE=1` to become gating. Everything they see is
...
//!   killed `met serve` whose driver process kept 87 GB.
//!
...
pub struct PolicyOptions {
    /// `METRALE_NO_HW_PRECHECK=1` — never REFUSE. Everything is still collected,
    ...
    /// `METRALE_HW_TEMP_GATE=1` — promote the absolute-temperature ceilings from
...
/// a third box exists, which is why they gate only under `METRALE_HW_TEMP_GATE`
...
/// exactly the shape of the 2026-08-15 incident where a killed `met serve`
...
pub fn precheck(
    ...
    // Recorded by default; gating only under METRALE_HW_TEMP_GATE=1.

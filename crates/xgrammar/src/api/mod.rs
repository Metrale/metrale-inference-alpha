...
// `xgrammar-rs` crate Metrale Engine was built against. This module restores
// the exact vendored surface — same type names, same method
// signatures — so Metrale Engine's `crates/spark-server/src/grammar/*.rs`
...
// `TokenizerInfo`, `Grammar`) rather than touching the core or Metrale Engine.
// The single unavoidable Metrale Engine edit — `grammar/state.rs` passing its
...
// `CompiledGrammar` is opaque to Metrale Engine (it never calls methods on it —

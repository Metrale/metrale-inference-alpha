...
"""Bounded single-host Metrale Engine rank canary. All ranks are stopped after the probe."""
import argparse
import hashlib
import json
import math
...
def environment(c):
    ...
    for key, value in c['env'].items():
        if (not re.fullmatch(r'(NCCL_[A-Z0-9_]+|METRALE_[A-Z0-9_]+|RUST_LOG|LD_LIBRARY_PATH|CUDA_CACHE_PATH|K3_ALLOW_MXFP4|K3_CUDA_KDA|K3_CUDA_MLA)', key)
...
def command(c, rank):
    ...
    # A presence flag: `met serve` refuses `--enable-prefix-caching false`.

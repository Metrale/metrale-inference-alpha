...
//! 2026-09-25: Byte-identity gate for the one-warp-per-output decode GEMVs:
//! `w4a16_gemv_sw`, `w4a16_gemv_dual_sw` and `w4a16_gemv_silu_input_sw` against
//! `w4a16_gemv`, `w4a16_gemv_dual` and `w4a16_gemv_silu_input`.
//!
//! Each `_sw` kernel does the base kernel's operations in the same order (see
//! its header in w4a16_gemv.cu / w4a16_gemv_fused.cu), so a shape passes only
//! when every output element is bit-identical. Both read the untransposed
//! NVFP4 layout: B_packed [N, K/2], B_scale [N, K/16].
//!
//! Owner: model-arch examples.
//! Invariants: none beyond the types.
//!
//! Usage, with an optional hex seed:
//!   cargo run --release -p metrale-model-arch --features cuda,gpu-examples \
//!     --example w4a16_gemv_sw_microtest -- [seed]
//! Exit 0 = every shape bit-identical, 1 = any shape differs.
...
/// 2026-09-25: An E4M3 group-scale byte: mantissa 0, exponent field 5..=9
/// (0.25 to 4).
...
fn run_shape(
    ...
    sw_h: metrale_gpu_runtime::gpu::KernelHandle,
    seed: u64,
    n: usize,
    k: usize,
) -> Result<Cmp> {
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: `(label, N, K)` single-row decode shapes, plus N and K edges.

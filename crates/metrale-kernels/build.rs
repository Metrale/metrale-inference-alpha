...
struct SamplingCat {
    ...
    // DRY sampler params (see SamplingCategory in metrale-kernels/src/lib.rs
...
///   `METRALE_SKIP_BUILD=1`; emitting them only on the compiling path would
...
///   `METRALE_KERNEL_SET_HASH` already closes. A second one would need its own
...
fn main() {
    ...
    // Two levels up from `crates/metrale-kernels`. Resolved before the skip
    ...
    let workspace_root_owned = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap())
        .parent()
        .and_then(|p| p.parent())
        .expect("crates/metrale-kernels is two levels below the workspace root")
        .to_path_buf();
    ...
    // prior `METRALE_TARGET_MODEL=*` build).
    ...
    // METRALE_EXTRA_NVCC_FLAGS — global nvcc-flag override read by
    // `build_target::NvidiaTarget::compile`. Used for kernel bisection
    // tests (e.g. `-DMETRALE_FAST_SOFTMAX_EXP=1` to flip the softmax
    ...
    // `kernels/metal/` and let `METRALE_TARGET_HW=metal` drive a real build.
    ...
    // as `cargo:rustc-env=METRALE_HW_FORCE_BR32`, for an `option_env!` reader in
    ...
    //   (a) Stage the CUDA→HIP compat-header dir (metrale-kernels/hip/compat) and
    //       export METRALE_HIP_COMPAT_INCLUDE — HipTarget::compile force-includes
    ...
    // Force metrale-kernels lib recompilation whenever the generated kernel set
...
/// Used to force metrale-kernels recompilation when the generated kernel set
...
fn build_hip_shim(manifest_dir: &std::path::Path, out_dir: &std::path::Path) {
    ...
    // Three HIP shims so the HIP target resolves every CUDA lib met emits:
    //   libcuda.so     — cu* driver API, real (libcuda_hip_shim.cpp)
    //   libcudart.so   — cudart runtime API, real (libcudart_hip_shim.cpp)
    //   libcublasLt.so — cuBLASLt, stub (opt-in METRALE_CUBLAS_GEMM path only)
...
/// to `nvcuda.dll`; met's own FFI links `-lcuda`/`-lcudart`/`-lcublasLt`, so
...
/// beside `met.exe`. Mirrors the Linux `.so` shims — same mappings, proven to
...
fn build_hip_shim_windows(manifest_dir: &std::path::Path, out_dir: &std::path::Path) {
    ...
    // 4. cudarc dlopens nvcuda.dll; met links cuda/cudart/cublasLt.lib. One DLL,
...
fn resolve_targets(workspace_root: &std::path::Path) -> Vec<Target> {
    ...
    // `-DMETRALE_NO_WARP_BLOCKSCALE_MMA` here because neither ISA has the

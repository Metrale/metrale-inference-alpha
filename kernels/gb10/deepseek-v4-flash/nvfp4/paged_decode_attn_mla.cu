...
// 2026-09-25: BF16 paged decode attention over the MLA cache at HDIM 576: paged_decode_attn, and a split-K pair.
// Owner: gb10 kernels (deepseek-v4-flash). Invariants: none beyond the types; every kernel assumes head_dim == HDIM.
#undef HDIM
#define HDIM 576

// 2026-09-25: A cache row is kv_lora_rank + qk_rope_head_dim = 512 + 64 = 576 values (MODEL.toml), NHD layout
// [num_blocks, block_size, num_kv_heads, head_dim]. paged_decode_attn runs one 8-warp CTA per (q_head, seq),
// grid (num_q_heads, num_seqs), block 256; each lane holds HDIM / 32 = 18 values. Warps take contiguous chunks
// of [0, seq_len), batch BC positions within a physical block, and merge their online-softmax states through
// shared memory. It has no sliding window: ops::paged_decode_attn_bf16 passes a trailing sliding_window
// argument that this kernel does not declare.
...
// 2026-09-25: Merges one (q_head, seq)'s num_splits partials, skipping l == 0, into BF16 O [num_seqs, num_q_heads, head_dim].

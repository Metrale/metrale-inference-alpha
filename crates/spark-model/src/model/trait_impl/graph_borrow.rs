...
//!
//! Kill switch: `METRALE_NO_GRAPH_BORROW` (PRESENCE disables, house
//! convention — `=0` is NOT off). Read once per process.

/// Borrowing enabled unless `METRALE_NO_GRAPH_BORROW` is present.
...
/// dgx2 validation run (`/tmp/metrale-dtval-serve.log`) caught declining the

...
//! 2026-09-26: What the request adapter carries into the IR (images,
//! reasoning, tool calls, the tool-result error flag, envelope fields), the
//! rendered-prompt golden, parity with the OpenAI chat wire, and the
//! IR → `MessagesResponse` direction.
//!
...
/// 2026-09-26: Lower an Anthropic request body into the IR envelope, as
/// `handlers::messages` does.
...
fn user_image_block_is_carried() {
    ...
    // 2026-09-26: The image comes first although the text block was sent
    // first.
...
fn url_image_source_becomes_url_variant() {
    ...
    let user = ir
        ...
        .find(|m| m.role == Role::User)
        .expect("user msg");
    // 2026-09-26: Typed as `Url`, so `msg_entry` fetches it when
    // `--vision-allow-remote-images` is set and refuses it with a 400
    // otherwise.
    assert_eq!(
        images_of(&user.content),
...
fn rendered_prompt_json_matches_retired_json_hop_output() {
    // 2026-09-26: Golden `build_json_messages` output for this
    // conversation; a change here changes the prompt Anthropic clients get.
    let ir = lower(serde_json::json!({
...
fn wire_surfaces_lower_to_identical_ir_core() {
    // 2026-09-26: One conversation in the Anthropic and the OpenAI chat
    // wire must lower to the same IR. The Responses surface lowers through
    // the chat wire (`lower_responses_to_chat`), so this covers it too.

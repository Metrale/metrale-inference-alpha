...
//!
//! Usage:
//!   METRALE_FAST_LOAD_BENCH_DIR=/path/to/model cargo run --release \
//!       -p metrale-model-weights --features test-utils --bin bench_fast_weights
//!
...
fn main() {
    let dirs: Vec<PathBuf> = std::env::args()
        .skip(1)
        .chain(std::env::var("METRALE_FAST_LOAD_BENCH_DIR").ok())
        .map(PathBuf::from)
        .collect();

    if dirs.is_empty() {
        eprintln!("Usage: bench_fast_weights <model_dir> [<model_dir>...]");
        eprintln!("   or: METRALE_FAST_LOAD_BENCH_DIR=/path/to/model bench_fast_weights");
        std::process::exit(2);
    }

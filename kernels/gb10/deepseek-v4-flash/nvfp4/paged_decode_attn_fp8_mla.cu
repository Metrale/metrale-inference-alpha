...
// 2026-09-25: FP8 E4M3 paged decode attention at HDIM 576: paged_decode_attn_fp8, and a split-K pair.
//
// Owner: gb10 kernels (deepseek-v4-flash).
// Invariants: none beyond the types; every kernel assumes head_dim == HDIM (defined above).
//
...
// VEC_U32_FP8 = HDIM / 128 = 4 words (16 of its VEC_BF16 = 18 values) from byte offset lane * 18.
...
// 2026-09-25: Merges one (q_head, seq)'s num_splits partials, skipping l <= 0, into BF16 O [num_seqs,
// num_q_heads, head_dim]; a sequence of length 0 writes nothing. One 32-thread block per (q_head, seq).

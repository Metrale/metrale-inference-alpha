...
//!
//! A gate runs as a CHILD `met benchmark run … --pull-request-gate`: the
...
pub fn facts_of(path: PathBuf, r: &metrale_plugin::gate::GateRecord) -> RecordFacts {
    let tallies =
        metrale_plugin::benchmarks::bfcl::aggregate::tallies_from_metrics(&r.metrics).is_some();
    RecordFacts {
...
/// arm exists so the binary still builds where `met serve` does.
#[cfg(not(unix))]
fn terminate(child: &mut std::process::Child) {
    let _ = child.kill();
}

...
mod attn_ncol_gemv;
// The `METRALE_ATTN_M16_TC` route lines (#927, H100 round 9 cell W) — `pub(crate)`
...
mod op_dump;
// `innerq_driver` calls the CUDA Driver API directly via `metrale_core::registry`,
...
// compiles on Apple Silicon without dragging in `metrale_core::registry`.
#[cfg(feature = "cuda")]
pub mod innerq_driver;
mod prefill;
// The cuBLASLt W8A8 prefill arm that replaced `METRALE_CUBLAS_GEMM=attn`'s
...
/// (`METRALE_PREFILL_HOST_TIMING=1`). Summed across layers and read+reset once
...
/// (`METRALE_PREFILL_HOST_TIMING=1`). Index: 0=qkv projections, 1=everything

...
> {
    ...
    //   * METRALE_DFLASH_DRAFTER_FP8: FP8 mirrors of the dense weights
...
pub(crate) struct LoraAdapterState {
    pub name: String,
    pub peft_config: metrale_core::config::PeftAdapterConfig,
...
pub(crate) fn load_lora_adapters(
    ...
    for (name, spec) in &args.lora_adapter {
        ...
        // Hard-error parser (metrale-core config/parsers/lora.rs) — scaling is read
        // per adapter (alpha/r, alpha/sqrt(r) under use_rslora), NEVER defaulted.
        let peft_config = metrale_core::config::parse_peft_adapter_config(&raw)
...
fn binds_vision(config: &metrale_core::config::ModelConfig) -> bool {

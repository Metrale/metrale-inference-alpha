...
describe('under-sampled windows are marked, never averaged in', () => {
  ...
  test('an under-sampled rung is EXCLUDED from k-of-n but still returned for drawing', () => {
    const metrale = [{ c: 8, energy: energyOf(rec({ metrics: cell(8, { samples: 5 }) }), 8) }];
    const rival = [{ c: 8, energy: energyOf(rec({ metrics: cell(8, { watts: 200 }) }), 8) }];
    const v = rungVerdicts(metrale, [{ label: 'vLLM', points: rival }]);
...
describe('a losing rung is returned like any other, and named', () => {
  const metraleAt = (c, watts) => ({ c, energy: energyOf(rec({ metrics: cell(c, { watts }) }), c) });
  const rivalAt = (c, watts, tokS) => ({
    c,
    energy: energyOf(rec({ metrics: cell(c, { watts, tokS }) }), c)
  });

  test('Metrale Engine losing a rung carries the factor, in the same shape as a win', () => {
    // Metrale Engine 80 W at 25 tok/s = 3.2 J/tok; vLLM 40 W at 20 tok/s = 2.0 J/tok.
    const v = rungVerdicts([metraleAt(8, 80)], [{ label: 'vLLM', points: [rivalAt(8, 40, 20)] }]);
    expect(v.rungs).toHaveLength(1);
    const r = v.rungs[0];
    expect(r.metraleCheaper).toBe(false);
  ...
  test('k = 0 flips the headline tile to name vLLM', () => {
    const v = rungVerdicts([metraleAt(1, 80), metraleAt(8, 80)], [
  ...
  test('a mixed result counts only the rungs Metrale Engine actually wins', () => {
    const v = rungVerdicts([metraleAt(1, 80), metraleAt(8, 20)], [
      { label: 'vLLM', points: [rivalAt(1, 40, 20), rivalAt(8, 40, 20)] }
    ]);
...
describe('costLadder', () => {
  ...
  test('Metrale Engine energy with no comparable vLLM energy is "metrale-only", and the vLLM series is NAMED', () => {
    ...
    expect(c.energyState).toBe('metrale-only');
  ...
  test('a refused cell of the DRAWN record is listed by sha, never coerced', () => {
    ...
    expect(c.metrale.build).toBe('facefeed11'); // the newest passing run on main is what is drawn
  ...
  test('only a PASSing record on main is the live Metrale Engine leg', () => {
    const onBranch = rec({ git_sha: 'branch1234', branch: 'pr-1', metrics: cell(8) });
    expect(costLadder(SUBJECT, [onBranch], laddersWith([])).metrale).toBeNull();
  });
});
...
describe('the empty state', () => {
  test('a subject with no records at all says so, with a count and never a zero cost', () => {
    ...
    expect(e.metrale).toContain('0 records');
    expect(e.metrale).toContain(SUBJECT.checkpoint);
  ...
  test('records without energy are counted and the missing KEY is named', () => {
    ...
    expect(e.metrale).toContain('2 concurrency-sweep records');
    expect(e.metrale).toContain('c{C}_gpu_rail_energy_j');
  });
  ...
  test('a published vLLM series is named with its instrument and whether it carries joules', () => {
    ...
    expect(e.vllm).toContain('no Metrale Engine run to fingerprint it against');
  });
...
describe('the checks can fail', () => {
  ...
  test('NEGATIVE CONTROL: Metrale Engine winning a rung produces no lose label and k = n', () => {
    const metrale = { c: 8, energy: energyOf(rec({ metrics: cell(8, { watts: 20 }) }), 8) };
  ...
  test('NEGATIVE CONTROL: a subject WITH energy is not reported as empty', () => {
    ...
    expect(e.metrale).toContain('1 of 1 records carry GPU-rail joules');
  });
});
...
describe('PUE, the second reader input', () => {
  ...
  test('THE PROPERTY THAT MAKES IT SAFE: no PUE can change who is cheaper', () => {
    ...
    const metrale = [3.2, 1.1, 0.42];
    const rival = [3.0, 1.3, 0.55];
    const base = metrale.map((a, i) => a / rival[i]);
    for (const pue of [1, 1.05, PUE_TYPICAL.low, PUE_TYPICAL.high, 2, PUE_MAX]) {
      for (const price of [0.15, 0.3, 0.02]) {
        metrale.forEach((a, i) => {
          ...
          expect(ra < rb).toBe(metrale[i] < rival[i]);
        });
      }
    }
  });

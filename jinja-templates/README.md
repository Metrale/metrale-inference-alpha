...
By default Metrale Engine renders chat from the **model's OWN** `chat_template.jinja` /
`tokenizer_config.json`. Metrale Engine's cross-cutting behaviors — auto-closing a
...
`crates/server/src/tokenizer/message_preprocess.rs`), so they work for
...
> `crates/server/src/tokenizer/tests/qwen_dense.rs`. Per-target

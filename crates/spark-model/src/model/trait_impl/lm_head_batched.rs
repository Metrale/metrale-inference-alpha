...
/// `METRALE_NO_LM_HEAD_BATCH_GEMV=1`.
///
/// Strict `== "1"` on an `METRALE_NO_*` name, not a presence check — presence
...
/// instead of exporting `METRALE_LM_HEAD_BATCHM_MAX` from a launch script —
...
pub(super) struct LmHeadM16Tc {
    ...
    /// `METRALE_LM_HEAD_M16_TC` present.
    pub enabled: bool,
    /// Requested CTA width: 32 (default) or 64.
    pub n_tile: u32,
}

/// `METRALE_LM_HEAD_M16_TC_NTILE` — 32 (default) or 64. An unrecognised value
...
fn lm_head_m16_tc_env() -> (bool, u32) {
    ...
    *ENV.get_or_init(|| {
        ...
        (
            ...
            // Y with an empty environment; `METRALE_LM_HEAD_M16_TC=0` is the A/B
...
/// wrong. Every Metrale Engine BF16 head satisfies it (Qwen3.8-27B: K=5120).
///
...
fn project_bf16_lm_head(
    ...
    // is behind `METRALE_LM_HEAD_M16_TC` and defaults OFF. With the lever unset

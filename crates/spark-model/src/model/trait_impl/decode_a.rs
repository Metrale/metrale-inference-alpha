...
/// `METRALE_REDZONE_EVERY=<n>` — scan the A55 guard bands every `n`-th decode step. 0 disables
...
/// `METRALE_REDZONE_RANGE_FILE=<path>` — see the use site. `None` disables the bisection.
...
impl TransformerModel {
    ...
    pub(super) fn decode_dispatch(
        ...
        // METRALE_SSM_H_FP16: narrow this sequence's SSM h-state to FP16 exactly
        ...
        // A55 BISECTION (`METRALE_REDZONE_RANGE_FILE=<path>` holding "LO HI"). Re-read and
        ...
        // EXPERIMENT (METRALE_EP_GRAPHS=1): allow CUDA-graph capture under EP. The
        ...
        // GDN HeadParallel TP decode graphs (METRALE_GDN_DECODE_GRAPH=1, default
        ...
        // LoRA debugging hatch (METRALE_LORA_EAGER=1): force eager decode when an
        ...
        // METRALE_DIAG_GEMMA4=1). Split into decode_a_diag.rs for the LoC budget.
        ...
        // A55 RED-ZONE SCAN (`METRALE_REDZONE=<bytes>`; no-op when unset). Runs AFTER the whole
        // decode step, with the device drained, so a violation names the step that produced
        // it. `METRALE_REDZONE_EVERY` (default 1) trades resolution for wall time — the scan

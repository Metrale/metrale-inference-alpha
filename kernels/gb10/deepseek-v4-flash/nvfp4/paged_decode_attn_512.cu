...
// 2026-09-25: BF16 paged decode attention at HDIM 512 over an NHD cache: paged_decode_attn, and a split-K pair.
...
// 2026-09-25: One 8-warp CTA per (q_head, seq): grid (num_q_heads, num_seqs), block 256 (ops::paged_decode_attn_bf16).
extern "C" __global__ void paged_decode_attn(
    ...
    // 2026-09-25: Within one physical block, positions go BC at a time and the remainder singly.
...
// 2026-09-25: Split-K: CTA (q_head, split, seq) attends the split's contiguous share of [0, seq_len), with no
// sliding window, and writes o, then m, then l to workspace [num_seqs, num_q_heads, num_splits, head_dim + 2].

...
fn nllb_metal_kernels_smoke() -> Result<()> {
    ...
    anyhow::ensure!(
        !modules.is_empty(),
        "metal kernel registry empty; run with METRALE_TARGET_HW=metal \
         METRALE_TARGET_MODEL=nllb-200-3.3b METRALE_TARGET_QUANT=bf16"
    );
...
fn upload(gpu: &dyn GpuBackend, values: &[f32]) -> Result<metrale_gpu_runtime::gpu::DevicePtr> {
...
fn upload_bf16(
    gpu: &dyn GpuBackend,
    values: &[f32],
) -> Result<metrale_gpu_runtime::gpu::DevicePtr> {
...
fn upload_u32(gpu: &dyn GpuBackend, values: &[u32]) -> Result<metrale_gpu_runtime::gpu::DevicePtr> {
    let ptr = gpu.alloc(std::mem::size_of_val(values))?;

...
COMMON="--host 0.0.0.0 --port $PORT --model-name qwen --max-seq-len 32768 --max-batch-size 1 --kv-cache-dtype bf16 --gpu-memory-utilization 0.70 --enable-prefix-caching --ssm-cache-slots 128 --ssm-checkpoint-interval 32 --mtp-quantization bf16 --tool-call-parser qwen3_xml --tool-grammar off --disable-thinking"

leg() {  # tag  <spec-flags>
  local tag="$1"; shift
  local CN="metrale-measc-$tag"
  for c in $(sudo docker ps -q --filter "name=metrale-measc-"); do sudo docker rm -f "$c" >/dev/null 2>&1; done
  sleep 4
  sudo docker run -d --name "$CN" --network host --gpus all --ipc=host \
    "${BASE_ENV[@]}" -v "$HFCACHE:/root/.cache/huggingface:ro" -v "$BIN:/usr/local/bin/met:ro" \

...
pub(super) fn load_layers(
    ...
    for (i, lt) in layer_types.iter().enumerate() {
        ...
        //
        // Diagnostic env: METRALE_FORCE_NVFP4_MOE=1 forces the NVFP4 path even
        ...
        // NVFP4 path.
        // METRALE_FORCE_NVFP4_ALL (lever-b, gfx1151 coherence): route an FP8
        ...
        // fused checkpoints through the NVFP4 expert path (as METRALE_FORCE_NVFP4_MOE
        ...
        // FP4 prefill MoE (METRALE_HOLO_MOE_GATEUP_FP4 / _DOWN_FP4) consumes the
        ...
        // engages under METRALE_HOLO_FAST_MOE_MODE=full; with the shared tables
        ...
        // METRALE_FORCE_NVFP4_MOE=1 inverts: do the prep so NVFP4 path is usable.
        ...
        // CUTLASS grouped NVFP4 gate_up (METRALE_HOLO_MOE_GROUPED_CUTLASS): swizzle
        ...
        // METRALE_FP8_DEQUANT_MOE_TO_BF16: dequant FP8 experts to BF16 at load,
        ...
        // METRALE_FP8_DEQUANT_LAYERS (PCND opt-in): restrict BF16 dequant to a
        ...
        // the Metrale Engine↔vLLM cosine floor). TP=1 only.
...
/// `METRALE_FP8_DEQUANT_LAYERS` (PCND opt-in). The spec is a comma-separated
...
/// `METRALE_HOLO_MOE_GATEUP_FP4=1` opts in to the FP4 (NVFP4 block-scaled)
...
/// `METRALE_HOLO_MOE_DOWN_FP4=1` opts in to the FP4 (NVFP4 block-scaled) down
...
/// re-reading `METRALE_HOLO_FAST_MOE_LAYERS` here. It used to read the env itself and

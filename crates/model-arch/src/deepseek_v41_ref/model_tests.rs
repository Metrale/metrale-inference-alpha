...
//! 2026-09-25: The full forward against the golden, prefill then both decode
//! steps through one model state. Each test walks one capture family across
//! every layer and regime so a failure names the first layer that diverged.
//!
...
fn final_collapse_norm_and_logits_match() {
    ...
    for (r, s) in run_all(&f) {
        ...
        // 2026-09-25: f32 logits from bf16 inputs: allow bf16-scale noise from
        // accumulation order.
        check_capture(

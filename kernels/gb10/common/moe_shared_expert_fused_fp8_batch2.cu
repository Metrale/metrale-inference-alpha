...
// 2026-09-25: Two-token FP8 MoE GEMVs with the shared expert in the same launch: gate+up,
// SiLU(gate) * up then down, and the weighted sum with the sigmoid-gated shared expert.
//
// Owner: gb10 kernels.
// Invariants:
...
// Unlike moe_weighted_sum_blend_batch2, gate_weight must be non-null: it is not checked.

...
// 2026-09-25: MLA paged decode over an NVFP4 KV cache for the deepseek-v4-flash tree: 512 latent plus
// 64 rope values per cache token, E2M1 codes with one E4M3 scale per 16 values.
//
// Owner: gb10 kernels (deepseek-v4-flash).
// Invariants: none beyond the types.

// 2026-09-25: A cache block holds block_size tokens of packed E2M1 codes (288 bytes per token and KV
// head, low nibble first), then, at data_section_bytes, their E4M3 scales (36 bytes per token and
// head). The latent and rope widths are the constants below; the kv_cache_dim argument is not read.
...
extern "C" __global__ void mla_paged_decode_nvfp4(
    ...
    const unsigned int kv_rope_dim = ROPE_DIM;
    ...
    const unsigned int kv_rope_data_offset = lane_id * (4 / 2);

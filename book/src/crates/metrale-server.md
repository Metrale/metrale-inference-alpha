...
5. **Instantiate `GpuBackend`** — `MetraleCudaBackend::new(ordinal, &ptx_modules)`.
...
- **Template-forced thinking** — some models emit `<think>` seeded by the chat template; Metrale Engine's detector had to distinguish that from the model's own `<think>`. The pass-16 fix required the opening `<think>` to be *unclosed* to count as the model's own.
...
- A new API endpoint (e.g. a Metrale-native `/v1/sessions/create`) — one handler in `api.rs`, one route in the router bindings in `main.rs`.

...
/// ONE MLP scratch for the whole layer stack; `METRALE_GLM_MLP_WS_SHARED=0` restores one per layer.
///
...
/// private copies DID buy was ≈2.1 GB of unified memory at `METRALE_GLM_PREFILL_ROWS=256` and
...
/// Kill switch for the row-batched routed path: `METRALE_NO_GLM_MOE_ROW_BATCH=1` restores the
...
/// Widest tier `w4a16_gemv_sw_moe_batchm` may be dispatched at, `METRALE_GLM_MOE_ROW_BATCH_MAX`.
///
...
/// `METRALE_MOE_BATCHM_ENTRY` list in `kernels/gb10/common/w4a16_gemv.cu` and of the
...
/// `METRALE_GLM_MOE_ROW_BATCH_MAX=2` silently disables row batching at odd widths. Only the A/B
/// lever can reach it.
///
...
/// Kill switch for the grouped path: `METRALE_GLM_MOE_HOST_DISPATCH=1` restores the

...
//! 2026-09-26: The six tools the agent loop offers (`bash`, `read`, `write`,
//! `edit`, `glob`, `grep`) and their implementations.
//!
//! Owner: bench, agentic.
//! Invariants:
//! - Every file path a tool call names goes through `resolve`.
//! - A `bash` call's time limit is at most `AgentConfig::command_timeout`.
...
/// 2026-09-26: `read` returns up to `READ_LINES` lines by default and cuts each
/// line at `READ_LINE_CHARS` characters.
const READ_LINES: usize = 2000;
const READ_LINE_CHARS: usize = 2000;

/// 2026-09-26: `grep` stops at this many matches and says so.
const GREP_LIMIT: usize = 100;

/// 2026-09-26: The tool definitions sent with every request.
...
/// 2026-09-26: Run one model-issued tool call. Each `bash` command is appended
/// to `commands`, which `score::followed_directions` reads.
pub async fn execute(
    ...
    // 2026-09-26: The schema names it `filePath`; `path` is accepted too.
    ...
    match call.name.as_str() {
        "bash" => {
            ...
            let limit = args
                ...
                // 2026-09-26: A model-supplied timeout may shorten the ceiling,
                // never raise it.
        ...
        "edit" => {
            ...
            // 2026-09-26: Sandbox-relative, like `write`, so the result holds no
            // absolute path that differs between runs or boxes.
            Ok(format!(
...
/// 2026-09-26: `read`: a directory is listed one entry per line with `/` after
/// subdirectories; a file is returned as numbered lines from `offset`.
...
/// 2026-09-26: Exact string replacement. Fails when `oldString` is empty or
/// absent, or matches more than once without `replaceAll`; the multiple-match
/// error says how to retry.
fn edit_tool(args: &Value, path: &Path, old: &str, new: &str) -> Result<usize> {
    ...
    // 2026-09-26: Not capped like [`read_capped`]: this text is written back, so
    // reading a prefix would truncate the file on disk.
...
/// 2026-09-26: Read at most `READ_LINES * READ_LINE_CHARS` bytes of a file,
/// appending a note when the cap was reached. Nothing bounds the size of a file
/// in the sandbox.
fn read_capped(path: &Path) -> std::io::Result<String> {
    ...
    let mut text = match String::from_utf8(buf) {
        Ok(text) => text,
        // 2026-09-26: Non-UTF-8 is an error (`grep` then skips the file), except
        // for a multi-byte character the cap itself cut in half.
...
/// 2026-09-26: Sandbox-relative paths of every regular file, sorted and
/// optionally filtered by a glob. `target` and `.git` directories are skipped.
///
/// Symlinks are skipped: [`entry.file_type()`][std::fs::DirEntry::file_type]
/// reports the link itself. Following them lets a few `ln -s . x` links make
/// the walk grow exponentially, and no timeout applies to `glob` or `grep`.
...
/// 2026-09-26: "No files found" in place of an empty result.
...
/// 2026-09-26: `*` (within one path segment), `**` (across segments), `?`. A
/// pattern with no `/` is matched against the basename.
...
fn matches_from(pattern: &[u8], text: &[u8]) -> bool {
    ...
    if head == b'*' {
        ...
        // 2026-09-26: `**/` also matches zero directories, so `**/*.rs` finds
        // `main.rs`.
...
/// 2026-09-26: True when the turn has no parsed tool call but its text contains
/// `<tool_call>` or `<function=`, the opening tags of the qwen3_coder call
/// format. Opening tags only, so a block the parser could not close still
/// counts.

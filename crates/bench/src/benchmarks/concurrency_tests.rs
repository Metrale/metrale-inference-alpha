...
//! 2026-09-26: Tests for the concurrency sweep: cell layout, fixtures, the
//! vacuity and cache-state rules, and the metrics map. The run-verdict tests
//! are in `concurrency_verdict_tests.rs`.
//!
...
pub(super) fn evidence(completion_tokens: usize) -> RequestEvidence {
    // 2026-09-26: accepted = 13/23 of the completion gives an accept depth of
    // about 2.3, above `arm_is_not_mtp`'s 1.5.
    evidence_with_arm(completion_tokens, Some(completion_tokens * 13 / 23))
}

/// 2026-09-26: `None` means the server reported no accept field.
...
/// 2026-09-26: `run_cell` sends a warm-up round concurrently, which is safe
/// only while no round repeats a prompt.
#[test]
...
/// 2026-09-26: The server does not accept `ignore_eos`, so no mode forces the
/// output budget, and the help must not say one does.
#[test]
...
/// 2026-09-26: The rule's own tests are in `concurrency_vacuity_tests.rs`; this
/// pins only the shapes this file's fixtures rely on.
#[test]
...
/// 2026-09-26: A cell the pool cannot hold is cold by construction, not
/// uncontrolled.
#[test]
fn the_warm_rule_applies_only_where_the_snapshot_pool_can_hold_the_cell() {
    let ladder = [1usize, 2, 4, 8, 16];
    // 2026-09-26: Each rung after the rungs below it: two live slots, two stale
    // leaves per earlier cell a prompt ran in, one re-home spare each.
    ...
    // 2026-09-26: The MoE and DFlash2 gates pin 32 slots on the 1/2/4/8/16
    // ladder. Measured 2026-09-14: the conc 8 cell ran cold twice (two and
    // three of eight tails lost).
    ...
    // 2026-09-26: A wider earlier cell ran every prompt of this one; a
    // narrower one ran only its first prompts.
    assert_eq!(slots_needed(2, &[4]), 2 + 2 + 2 + 2 + 2);
    assert_eq!(slots_needed(4, &[2]), (2 + 2) * 2 + 2 * 2 + 4);
    // 2026-09-26: Negative control: an unstated pool size does not waive the
    // rule.
...
fn a_requested_warm_path_requires_a_uniform_cache_state() {
    ...
    // 2026-09-26: `WARM_CACHE_FLOOR` decides which side a request is on
    // (80 of 100 is warm, 79 is not), and only a mixture is uncontrolled, so
    // the boundary is tested through a mixture.
...
/// 2026-09-26: A vacuous cell never supplies a throughput key, however high its
/// rate, while `min_completion_tokens` spans every request.
#[test]
fn metrics_map_reports_the_comparable_curve_and_the_evidence_floor() {
    ...
    // 2026-09-26: A later, lower comparable row does not replace the best C=4.
    b.rows
        .push(row(4, 80.0, Some(300.0), vec![evidence(128); 4], osl));
    // 2026-09-26: A vacuous C=4 cell with a high rate does not win the rung.
    b.rows

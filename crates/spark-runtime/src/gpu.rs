...
///
/// Implementations: `MetraleCudaBackend` (production), `MockGpuBackend` (tests).
pub trait GpuBackend: Send + Sync {
    ...
    /// a kernel wrote past. Returns the violation count. `Ok(0)` when `METRALE_REDZONE` is
    ...
    /// survive the `&dyn GpuBackend` vtable — every lookup in Metrale Engine goes
    ...
    /// "Metrale-own" memory when it sizes the KV cache against

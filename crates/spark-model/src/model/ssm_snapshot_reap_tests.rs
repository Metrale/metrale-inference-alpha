...
///
/// A cap eviction (`METRALE_SSM_TIER_DISK_GB`) drops a blob, but nothing tells
...
fn tier_miss_retires_the_key_instead_of_thrashing() {
    ...
    const BLK: usize = 16;
    /// Every prefix must clear `METRALE_SSM_SPILL_MIN_TOKENS` (default 1024) so
    ...
    // METRALE_SSM_TIER_DISK_GB, where every new record drops the oldest.

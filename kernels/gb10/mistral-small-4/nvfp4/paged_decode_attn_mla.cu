...
// forked-from: kernels/gb10/deepseek-v4-flash/nvfp4/paged_decode_attn_mla.cu (2026-09-24; 3 of 538 lines differ, see kernels/FORKS.md)


#undef HDIM
#define HDIM 320

// 2026-09-25: BF16 paged decode attention over the mistral-small-4 MLA cache at HDIM 320 (defined above,
// replacing the target's -DHDIM=128): paged_decode_attn, and a split-K pair.
//
// Owner: gb10 kernels (mistral-small-4).
// Invariants: none beyond the types; every kernel assumes head_dim == HDIM.
//
// NHD cache layout [num_blocks, block_size, num_kv_heads, head_dim]. paged_decode_attn runs one 8-warp CTA per
// (q_head, seq), grid (num_q_heads, num_seqs), block 256; each lane holds HDIM / 32 = 10 values. Warps take
// contiguous chunks of [0, seq_len), batch BC positions within a physical block, and merge their online-softmax
// states through shared memory. It has no sliding window: ops::paged_decode_attn_bf16 passes a trailing
// sliding_window argument that this kernel does not declare.
...
// 2026-09-25: Merges one (q_head, seq)'s num_splits partials into BF16 O [num_seqs, num_q_heads, head_dim].

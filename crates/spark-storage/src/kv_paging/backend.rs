...
//! of `METRALE_KV_PAGING`), plus [`super::connect_kv_peer_backend`], the single
...
//!
//! Zero-copy landing (`METRALE_KV_ZERO_COPY=1`) is preserved: the whole UMA
...
impl KvPagingBackend {
    ...
    /// `METRALE_EXPERT_RDMA_DEV`/`GID` = rail 0, `METRALE_KV_DUAL_RAIL=1` +
    /// `METRALE_KV_RAIL2_DEV`/`GID` = rail 1), and register one block-sized

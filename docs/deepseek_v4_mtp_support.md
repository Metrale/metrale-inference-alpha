...
- Fixed (committed): `I8` dtype; per-expert format dispatch (`load_expert_proj`): Standard NVFP4
  ...
  `metrale-ds-ep*` (env reaper kills `metrale-deepseek-ep*`); start script → new model + explicit paths.
...
Best image: `metrale-deepseek-v4:mtpload4` (loads+serves, incoherent). Branch deepseek-v4-clean.

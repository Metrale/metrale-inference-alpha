...
//
// @live because it needs `metralectl agent run` on this machine with at least one
...
//
// Run it as:  metralectl agent run --dev-origins
//
...
// ALLOWED_ORIGINS = ["https://dev.metrale.ai"] and nothing else unless that
// flag is passed (metralectl-agent/src/guard.rs). Playwright serves this suite
...
const TOKEN = process.env.METRALECTL_TOKEN ?? '';

test.describe('@live cluster launch', () => {
  test.skip(!TOKEN, 'needs METRALECTL_TOKEN and a running agent');

  test.beforeEach(async ({ page }) => {
    await page.addInitScript((t) => {
      // The key the app actually reads (`TOKEN_KEY` in protocol.js). It was
      // 'metralectl.token' here, which stores a value nothing looks for: the page

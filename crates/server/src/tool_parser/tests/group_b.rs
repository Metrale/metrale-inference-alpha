...
//! 2026-09-26: Tool-parser tests: Qwen3-Coder blocks, format dispatch and
//! system prompts, tag-style and bare calls, and tool-name clean-up.
//!
...
fn parse_qwen3_coder_two_consecutive_functions_no_field_bleed() {
    // 2026-09-26: Two bare `<function=…>` blocks give two calls, each with
    // only its own parameters: `BareFunctionAttrPass` parses one block at a
    // time, and the parameter loop of `parse_qwen3_coder_call` stops at
    // `</function>`.
...
fn parse_qwen3_coder_call_direct_two_funcs_returns_first_only_no_bleed() {
    // 2026-09-26: Given two blocks, `parse_qwen3_coder_call` returns the first
    // call with only its own parameters; `BareFunctionAttrPass` then moves
    // past that block's `</function>`.
    let input = "<function=write>\n\
            <parameter=filePath>\n/tmp/y.txt\n</parameter>\n\
            <parameter=content>\nhello\n</parameter>\n\
            </function>\n\
...
fn parse_qwen3_coder_unclosed_param_then_next_function_does_not_bleed() {
    // 2026-09-26: The last parameter has no `</parameter>`, so its value runs
    // to `</function>`; `advanced_to_func_close` then ends the loop before the
    // next block's parameters.
    let input = "<function=write>\n\
            <parameter=filePath>\n/tmp/y.txt\n</parameter>\n\
            <parameter=content>\nbody-without-close\n\
            </function>\n\
...
fn tscg_is_decided_per_model_not_per_process() {
    // 2026-09-26: TSCG comes from the `PromptLevers` passed with each call, so
    // one process renders both settings.
    let parser = HermesParser;
    let tools = vec![ToolDefinition {
        tool_type: "function".into(),
        function: FunctionDefinition {
...
fn streaming_bare_function_flush() {
    // 2026-09-26: Once `<parameters>` has opened, `bare_function_end` waits for
    // `</parameters>`, so the `</function>` that closes the name does not end
    // the call.
    let mut det = StreamingToolDetector::new();
    let out1 =
    ...
    // 2026-09-26: The stream ends before `</parameters>`; `flush` recovers
    // the call with its argument.
    let out2 = det.flush();
...
fn parse_qwen3_coder_duplicated_name_equals() {
    // 2026-09-26: `normalize_tool_name` cuts a name at its first `=`.
    let input = "<tool_call>\n\
            <function=Bash=Bash>\n\
            <parameter=command>\nwhich cargo\n</parameter>\n\
            </function>\n\
...
fn parse_qwen3_coder_json_args_grammar_mode() {
    // 2026-09-26: A JSON object between `<function=NAME>` and `</function>`
    // is taken as the arguments.
    let input = "<tool_call>\

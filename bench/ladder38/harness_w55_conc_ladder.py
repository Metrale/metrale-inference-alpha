...
# 2026-09-26: Concurrency ladder driver: C concurrent streaming requests per rep, one client for any engine.
#
    ...
    async with aiohttp.ClientSession(connector=conn) as session:
        for conc in concs:
            ...
            for r in range(a.reps):
                # 2026-09-26: SM clock and power, read once before the rep starts
                # (os.popen().read() returns before run_rep sends anything).
            ...
            print(f"[{a.label}] C={conc:>3} SERIES {['%.2f' % s for s in series]} "
                  ...
                  flush=True)
            # 2026-09-26: Written after every rung, so a crash loses at most the
            # rung in progress.

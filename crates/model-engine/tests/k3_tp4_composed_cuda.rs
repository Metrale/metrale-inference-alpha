...
//! 2026-09-25: Two Kimi K3 layers at the official config's TP4 rank-local sizes,
//! through `forward_one_layer_with_cores` with CUDA callbacks: KDA with the dense
//! MLP, then MLA with the selected packed experts and the shared MLP.
//!
//! Owner: model-engine tests.
//! Invariants: none beyond the types.
//!
//! Synthetic weights on one rank: no checkpoint loader, NCCL, full model or
//! tokenizer. The per-kernel CPU comparisons are the `k3_*_oracle` tests.
#![cfg(feature = "cuda")]
...
use metrale_model_arch::kimi_k3::dense_cuda::launch_dense_matrices;
use metrale_model_arch::kimi_k3::kda_cuda::{
    K3KdaDecodeKernels, KdaDeviceState, launch_k3_kda_decode_token_on_device,
};
use metrale_model_arch::kimi_k3::mla_cuda::{K3MlaDecodeKernels, launch_k3_mla_decode_token};
use metrale_model_arch::kimi_k3::moe_cuda::{K3MoeGemmKernels, launch_k3_latent_moe_experts};
use metrale_model_layers::weight_map::QuantizedWeight;
use metrale_model_weights::kimi_k3_host::cpu_weights::{
...
fn k3_tp4_rank_local_composed_graph_reset_and_history() -> Result<()> {
    ...
    // 2026-09-25: Host matrix storage, checked against a 3 GiB budget before
    // anything is allocated; the matrices are full size.
    ...
    // 2026-09-25: The router keeps all 896 experts, but the bias selects the
    // first 16 and only those are provisioned; the expert callback checks it.

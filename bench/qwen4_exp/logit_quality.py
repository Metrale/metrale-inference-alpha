#!/usr/bin/env python3
# 2026-09-26: Compares the last-position logits of forward_ref.py's golden with the
# first row the engine appends to METRALE_DUMP_LOGITS_PATH/logits_seq.bin for the same
# chat prompt: KL both ways, top-1 probabilities, top-20 overlap, largest disagreements.
#
...
Engine:    a raw row from METRALE_DUMP_LOGITS_PATH/logits_seq.bin captured by
...
Usage:
    1. serve with METRALE_DUMP_LOGITS_PATH=<dir>; rm <dir>/logits_seq.bin
...
ref = gold['logits'][-1].astype(np.float64)

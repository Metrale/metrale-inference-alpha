...
impl ComputeTarget for NvidiaTarget {
    ...
    fn compile(
        ...
        // tolerated. On by default; set METRALE_KERNEL_NO_STRICT=1 to disable
        // locally (only the literal "1" disables, so METRALE_KERNEL_NO_STRICT=0
        ...
        // METRALE_EXTRA_NVCC_FLAGS — global override for kernel bisection
        // tests. Whitespace-separated list of additional nvcc args
        // (typically `-D<MACRO>=1` to flip `#ifdef`-gated kernel paths).
        // Phase 2c day 2: used with `-DMETRALE_FAST_SOFTMAX_EXP=1` to
...
///
/// Pairs with the `libcuda.so` → HIP shim (metrale-kernels/hip/libcuda_hip_shim.cpp):
...
/// exports its path via `METRALE_HIP_COMPAT_INCLUDE`; the per-source mask-widen

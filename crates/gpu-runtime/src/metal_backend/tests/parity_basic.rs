...
//! 2026-09-25: Backend smoke test (alloc, copies, one launch) and parity of the
//! element-wise `bf16_add` and `sigmoid_gate` kernels.
//!
...
/// 2026-09-25: A byte pattern round-trips through `alloc`, `copy_h2d` and
/// `copy_d2h`; then `noop_smoke` with `n = 4` zeroes the first 4 floats and leaves
...
/// 2026-09-25: `bf16_add` against the f32 sum rounded to BF16, over 257 elements,
/// so the last threadgroup of 64 is partial.
#[test]
...
/// 2026-09-25: `sigmoid_gate`: `out = sigmoid(gate) * x`. The qwen3_5 full-attention
/// forward gates its attention output with it (`forward/qwen3_5/full_attention.rs`
/// in metrale-model-layers).
#[test]

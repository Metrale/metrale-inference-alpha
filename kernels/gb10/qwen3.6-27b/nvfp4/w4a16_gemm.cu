...
// Metrale Engine W4A16 GEMM — 35B model shadow.
//
// Optimizations:
...
__device__ __forceinline__ unsigned short metrale_cvt_e4m3x2_f32(float a_hi, float b_lo) {
    ...
    // port's strix-hip-real metrale_cvt_e4m3x2_f32 (numerically exact SATFINITE E4M3).
    unsigned a8 = (unsigned)scl_enc_fp8(a_hi);
    unsigned b8 = (unsigned)scl_enc_fp8(b_lo);
    return (unsigned short)((a8 << 8) | (b8 & 0xFFu));
#else
...
// (metrale_cvt_e4m3x2_f32) IS guarded, since elementwise kernels that use it
...
// Kill switch: METRALE_NO_TGEMM_PIPELINE3 (presence).
// ═══════════════════════════════════════════════════════════════════
...
// Kill switch: METRALE_NO_K64_PIPELINE3 (presence).
// ═══════════════════════════════════════════════════════════════════
#define K_STEP_T64 64
#define PAD_T64    8
...
// llama MMQ = 65.3 TFLOP/s vs faith2 44.5 (+46% real gap); on down (K17408) Metrale Engine
...
// it 63.8->42.4 TFLOP/s, landing EXACTLY on Metrale Engine's 44 plateau (long_scoreboard

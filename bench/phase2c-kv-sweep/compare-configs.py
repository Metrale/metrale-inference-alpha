...
# 2026-09-26: Per-layer cosine against the HF dumps for several KV-precision configs, side by side.
#
...
ND = pathlib.Path("/workspace/metrale-dumps/numdrift")
...
def per_layer(config_dir: pathlib.Path) -> list[float]:
    ...
    out: list[float] = []
    for i in range(40):
        ap = config_dir / f"metrale_L{i}.bin"
...
def fmt(x: float) -> str:
    if x != x:  # 2026-09-26: true only for NaN
        return "  nan "
    return f"{x:.4f}"
...
def main() -> None:
    ...
    # 2026-09-26: Deltas against the first config, when there are two or more.
    if len(headers) >= 2:
        base = data[headers[0]]
        print()

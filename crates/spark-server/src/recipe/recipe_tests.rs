...
/// **The drift guard.** Every Metrale Engine recipe must render to argv that clap parses
...
/// key Metrale Engine has no flag for is still unguarded, and is a known gap.
#[test]
...
fn a_vllm_recipe_is_readable_but_not_launchable() {
    ...
    // rendering vLLM keys as Metrale Engine flags would produce nonsense, so argv refuses.
    let all = all();
...
fn every_recipe_can_be_served_hermetically() {
    // Only `runtime: metrale` recipes can be served from here AT ALL — a
...
fn the_env_block_is_read_verbatim_and_absent_means_none() {
    ...
    // prefix (which `serve_env::declared` renames onto METRALE_*), and every
    ...
    for r in all() {
            ...
            .starts_with("deepseek-v4.1/deepseek-v4.1-flash-q2k-b200")
        {
            ...
            metrale_plugin::serve_env::declared(&r.id, &r.env)

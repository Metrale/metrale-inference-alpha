...
/// METRALE_MTP_K_LADDER nor METRALE_NO_MTP_K_LADDER; the env-reading
...
fn cap_identity_at_or_below_32_every_config() {
    ...
    // dispatch-cap value (incl. METRALE_NO_MTP_K_LADDER's 4) because the
...
fn k_minus_1_shrink_and_kill_switch_shape() {
    ...
    // uniform_verify (DFlash-γ pools / METRALE_MTP_POOL_FULL_WIDTH /
...
fn cap_bites_above_32_and_kill_switch_restores() {
    ...
    // METRALE_MTP_MAX_SEQS=48 widens the pools with the dispatch cap.
    assert_eq!(mtp_state_slots_with(64, 48, false), 48);
    // METRALE_NO_MTP_K_LADDER (cap 4) still floors at 32 — defense in depth.

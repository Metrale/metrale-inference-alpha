...
//! # Kill switch
//!
//! `METRALE_NO_GEMV_EXACT_M_TIERS=1` (presence-checked per the house convention;
...
/// Widths hidden by `METRALE_NO_GEMV_EXACT_M_TIERS=1`: the tiers added by this
...
pub struct W4a16BatchmTiers {
    ...
    wide: KernelHandle,
    /// `w4a16_gemv_batch32`, handed out for 17..=32 rows ONLY under
    /// `--w4a4-downcast` (the W4A4 path serves them; this is its fallback).
    wide32: KernelHandle,
}
...
impl W4a16BatchmTiers {
    ...
    pub fn resolve(gpu: &dyn GpuBackend) -> Self {
        ...
        for (h, w) in handles.iter_mut().zip(W4A16_BATCHM_WIDTHS) {
            ...
            // own kill switch (`METRALE_NO_BATCH8_RT=1`). All five tier
        ...
        // Model build is the one place the opt-in W4A4 projection path may
        // allocate its activation scratch (never inside a graph capture).

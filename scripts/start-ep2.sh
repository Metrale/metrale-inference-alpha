#!/usr/bin/env bash
#
# Launch Metrale Engine with Expert Parallelism (EP=2) across two GB10 nodes.
#
# Usage:
...
#   - metrale-122b:latest Docker image on both nodes (or metrale-gb10:latest)
#     Build: docker build -f docker/gb10/qwen3.5-122b-a10b/nvfp4/Dockerfile -t metrale-122b .
#   - Same image tag on BOTH nodes (mixing Metrale Engine versions across ranks
...
# per-module MTP extraction is still a TODO; the Metrale Engine pre-flight will
...
IMAGE="${IMAGE:-metrale-122b:latest}"
...
echo "=== Metrale Engine EP=2 Launch (RDMA-enabled) ==="
echo "Model:  $MODEL"
...
sudo docker rm -f metrale-ep0 2>/dev/null || true
ssh "$WORKER_IP" "sudo docker rm -f metrale-ep1 2>/dev/null || true"
...
# Optional dev binary mount (overrides container's /usr/local/bin/met)
DEV_MOUNT=""
DEV_MOUNT_WORKER=""
if [ -n "$DEV_BINARY" ]; then
  DEV_MOUNT="-v $DEV_BINARY:/usr/local/bin/met:ro"
  # For worker, copy binary first
  scp "$DEV_BINARY" "$WORKER_IP:/tmp/met-dev" 2>/dev/null || true
  DEV_MOUNT_WORKER="-v /tmp/met-dev:/usr/local/bin/met:ro"
...
echo "Monitor rank 0: sudo docker logs -f metrale-ep0"
echo "Monitor rank 1: ssh $WORKER_IP 'sudo docker logs -f metrale-ep1'"

...
//! a net wall-clock LOSS on the metric Metrale Engine currently wins 1.80x. The two
...
///
/// `METRALE_MARCONI_MIN_TOKENS=<n>` overrides; 0 restores the previous
...
/// so a run configured by `METRALE_MARCONI_MIN_TOKENS` could not state that it
...
/// 2026-09-07. `METRALE_MTP_MAX_SEQS` defaults to 32, so `multi_seq` is true on
...
/// `METRALE_MTP_CARRY_DEBUG=1` — one line per adopt/carry decision. Cheap (no

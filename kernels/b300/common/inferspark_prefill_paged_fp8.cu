...
// elements per metrale_cp16) so KV-load latency is hidden behind QK^T compute
...
// 16 FP8 elements (16 bytes) per metrale_cp16, matching the 16-byte transaction
// size. Dequant is deferred to in-register MMA read (METRALE_ATTN_FP8_SMEM path
...
// HDIM_PAD must itself be 16-aligned for the 16-byte metrale_cp16 and uint4
...
// NVFP4 paths don't define METRALE_ATTN_FP8_SMEM, so they keep PAD_KV=8.
...
// KERNEL_PREAMBLE: empty — METRALE_ATTN_FP8_SMEM path defers fp8→bf16 dequant

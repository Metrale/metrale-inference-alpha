...
impl TransformerModel {
    pub(super) fn decode_verify_graphed_kgamma_dispatch(
        ...
        // METRALE_DFLASH_DEBUG_NO_GRAPH=1 forces eager (no graph capture) so
        ...
        if need_run {
            ...
            for (layer_idx, layer) in self.layers.iter().enumerate() {
                ...
                // recorded. Under METRALE_DFLASH_EAGLE_FIX=1 OR
                // METRALE_DFLASH_UNIFIED_CTX=1, capture ALL k verify rows so
                ...
                // METRALE_DFLASH_EAGLE_FIX and this lane's
                // METRALE_DFLASH_UNIFIED_CTX -- so EITHER set to "0" turns it

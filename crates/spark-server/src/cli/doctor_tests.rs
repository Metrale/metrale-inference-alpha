...
//! `METRALE_HOME` around each. Env is process-global, so they run under one mutex.
...
/// RED. An empty `METRALE_HOME` is a real configuration people produce with
/// `export METRALE_HOME=$SOMETHING_UNSET`.
#[test]

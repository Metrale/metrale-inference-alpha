...
/// keyboard protocol under `REPORT_EVENT_TYPES`. Metrale Engine pushes no keyboard
...
pub fn tick_work(section: Section, lib: LibraryPhase) -> TickWork {
    ...
    TickWork {
        ...
        // cannot change while the process runs (neither `METRALE_HOME` nor `HOME`
        // is set, or `METRALE_HOME` is empty). The condition was

...
/// FP8 E4M3 decode and the f32 → BF16 cast live in `metrale_core::numeric`.
///
/// They used to live here AND in `metrale-quant/src/fp8.rs`, with the
...
/// on `metrale-core`, so the arithmetic moved down there together with the

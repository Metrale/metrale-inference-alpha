...
//! `OnceLock<bool>` statics reading `METRALE_*` at first touch; a static outlives
...
pub struct SchedLevers {
    ...
    /// `METRALE_TOOL_RESPONSE_STOP=0`/`false` disables.
    ...
    /// env-dependent. `METRALE_TOOL_EOS_ESCAPE=0`/`false` disables.
    pub tool_eos_escape: bool,
    /// Apply min-p during MTP verify. Ships ON; `METRALE_NO_MTP_MINP=1` opts out.
    pub mtp_minp: bool,
    /// Run the full sample pipeline during MTP verify. Ships ON;
    /// `METRALE_NO_MTP_VERIFY_SAMPLE=1` opts out.
    pub mtp_verify_sample: bool,

    // ── DFlash speculation ──
    /// The EAGLE k-gamma append fix. Ships ON since the 54.5 record config
    /// (2026-08-19); `METRALE_DFLASH_EAGLE_FIX=0` is the kill switch.
    ///
    ...
    /// exact shape `METRALE_DSPARK_ANCHOR_BIAS` had. One field now.
    pub dflash_eagle_fix: bool,
    /// `METRALE_DFLASH_STEP_TIMING=1` — split the step wall into verify (target
    ...
    /// `METRALE_VISION_TIMING` (presence) — synchronize after each prefill
    ...
    /// `active.len() <= 2` (`METRALE_DFLASH_GATE_PIN_C2=0` restores
    ...
    /// Cross-sequence batched DFlash K=γ verify (`METRALE_DFLASH_BATCH_VERIFY=0`
    ...
    /// `METRALE_DFLASH_RESUME_GUARD=N` (default 0 = off): keep the first N
    /// post-`</think>` tokens on plain serial decode.
    pub dflash_resume_guard: u32,
    /// `METRALE_MTP_SHADOW_TOPK` — the verify side of the drafter top-k probe.
    ...
    /// Forced-token fast path. Ships ON; `METRALE_DISABLE_FORCED_TOKEN=1` opts out.
...
/// `METRALE_FOO=1` enables.
fn opt_in(var: &str) -> bool {
    std::env::var(var).ok().as_deref() == Some("1")
}

/// `METRALE_FOO=0` DISABLES — a default-ON lever whose kill-switch is an
...
/// `METRALE_FOO=1` DISABLES — the flag names a negative, the field stores the
...
/// sealed this cell on every `met serve` and left `METRALE_MTP_GATE_FORCE=1`
...
pub fn mtp_gate_force() -> bool {
    MTP_GATE_FORCE_CLI
        .get()
        .copied()
        .unwrap_or_else(|| opt_in("METRALE_MTP_GATE_FORCE"))
}
...
impl SchedLevers {
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            tool_response_stop: crate::scheduler::helpers::parse_flag_default_on(
                std::env::var("METRALE_TOOL_RESPONSE_STOP").ok().as_deref(),
            ),
            tool_eos_escape: crate::scheduler::helpers::parse_flag_default_on(
                std::env::var("METRALE_TOOL_EOS_ESCAPE").ok().as_deref(),
            ),
            ...
            // reproduction key). A default `met serve --dflash` previously
            ...
            // count +31% (PR #604). `METRALE_DFLASH_UNIFIED_CTX=0` restores the
            ...
            forced_token_fastpath: crate::scheduler::helpers::parse_forced_token_fastpath(
                std::env::var("METRALE_DISABLE_FORCED_TOKEN")
                    .ok()
                    .as_deref(),
            ),
    ...
    /// Every opt-in off and every opt-out on — what a build with no `METRALE_*`

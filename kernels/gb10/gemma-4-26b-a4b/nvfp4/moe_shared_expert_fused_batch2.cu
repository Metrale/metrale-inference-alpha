...
// forked-from: kernels/gb10/common/moe_shared_expert_fused_batch2.cu (2026-09-24; 449 of 584 lines differ, see kernels/FORKS.md)

// 2026-09-25: Two-token NVFP4 MoE decode kernels: the routed and shared experts of both tokens in one launch per stage.
// blockIdx.y in [0, 2 * top_k) is routed slot y % top_k of token y / top_k (expert_indices[y]); [2 * top_k, 2 * top_k + 2)
// is the shared expert of token y - 2 * top_k. Each block reads only its own token's input, so a weight row is read once
// per token. moe_expert_gate_up_shared_batch2 computes gate (blockIdx.z 0) and up (1); moe_expert_silu_down_shared_batch2
// computes down(GELU_tanh(gate) * up), GeGLU in spite of its name; moe_weighted_sum_blend_batch2 adds the weighted routed
// outputs and sigmoid(input . gate_weight) times the shared output. Launchers: ops::moe_expert_gate_up_shared_batch2,
// ops::moe_expert_silu_down_shared_batch2, ops::moe_weighted_sum_blend_batch2 (layers/ops/moe_expert_more.rs).
// Owner: gb10 kernels (gemma-4-26b-a4b).
// Invariants: none checked. The GEMV kernels assume 128-thread blocks (BLOCK_SIZE) and K a multiple of 8, and read the
// shared expert's weight pointers without a null check; the blend kernel assumes 256-thread blocks.
...
extern "C" __global__ void moe_expert_silu_down_shared_batch2(
    ...
    __shared__ float s_lut[16];
    // 2026-09-25: K floats of dynamic shared memory, sized by the launcher.
    ...
    // 2026-09-25: GELU_tanh(gate) * up for all K, into s_act.
    ...
    // 2026-09-25: The shared expert writes sh_down_out[token * N]; routed rows write C[(token * top_k + slot) * N].

...
//! 2026-09-25: Tests of the DSA TP shard plan and `Glm5NextDsaConfig`: arithmetic only, no GPU
//! and no checkpoint.
//!
...
/// 2026-09-25: The `glm-5.3-flash` DSA geometry (`kernels/gb10/glm-5.3-flash/MODEL.toml`) with
/// `local_heads` per rank.
...
/// 2026-09-25: One DSA layer's bytes at this geometry: the sum of the per-tensor sizes below.
...
fn full_shapes_match_the_reference_checkpoint() {
    ...
    // 2026-09-25: Eleven text DSA layers (`layers_dsa` in MODEL.toml).
...
/// 2026-09-25: At TP=2 every sharded tensor partitions exactly, and everything else replicates.
#[test]
...
/// 2026-09-25: The shared latent projection has no head axis and replicates, as do the low-rank
...
/// 2026-09-25: `q_b_proj` and `kv_b_proj` shard by head at different per-head widths.
#[test]
...
/// 2026-09-25: NoPE geometry: the latent cache is `kv_lora_rank` wide (512), with no rope part.
#[test]
...
/// 2026-09-25: Selection budget and emitted row width.
#[test]
fn select_budget_and_out_width() {
    ...
    // 2026-09-25: `always_select_tail` widens the row by `index_kpool - 1`.
...
/// 2026-09-25: `index_kpool` is bounded by 8: `dsa_kpool_compress` keeps the pool logits in
/// `float lg[8]` and loops `s < KP && s < 8`, so a larger kpool would pool only the first 8 slots.
#[test]
fn config_rejects_a_kpool_beyond_the_kernel_bound() {
    ...
    // 2026-09-25: A kpool between 9 and 64 is refused.
    let mut mid = cfg(64);
    mid.index_kpool = 16;
    ...
    // 2026-09-25: 8 is the kernel's capacity and is accepted.
    let mut edge = cfg(64);
    edge.index_kpool = 8;
...
/// `glm5next_dsa_mla_decode.cu` hardcodes; `validate` refuses any other width.
#[test]

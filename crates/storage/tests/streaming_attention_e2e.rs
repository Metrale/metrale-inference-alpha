...
//! 2026-09-26: GPU tests (`#[ignore]`d by default) of streaming attention:
//! `TiledAttention` over K/V read from disk one tile at a time, through the POSIX and
//! the io_uring backend, matches the all-resident reference within 1e-2 max abs
//! difference at tile sizes `NUM_BLOCKS`, 4, 2 and 1; and one K group read back from
//! disk equals the K the test wrote.
//!
//! Owner: storage (tests).
...
use half::bf16;
use metrale_storage::backend::{ReadRequest, StorageBackend};
use metrale_storage::cuda_min::{
    CudaCtx, DeviceBuffer, copy_d_to_h_async, copy_h_to_d_async, stream_sync,
};
use metrale_storage::group::{GroupKey, KvKind};
use metrale_storage::scratch_pool::{ResidentKey, ScratchDims, ScratchPool};

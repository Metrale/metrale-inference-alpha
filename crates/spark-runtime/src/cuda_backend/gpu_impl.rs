...
//! `impl GpuBackend for MetraleCudaBackend` — production CUDA backend trait body.
//!
...
//!   on the calling thread. `MetraleCudaBackend::new` binds it once via
...
use metrale_core::registry::{RawCudaFunc, cuda_error_text};
...
/// (`METRALE_D2H_TRACE=<N>`: log a backtrace on the Nth call, and the running
...
impl GpuBackend for MetraleCudaBackend {
    ...
    fn alloc(&self, bytes: usize) -> Result<DevicePtr> {
        ...
        let mut dptr: u64 = 0;
        // A55 RED ZONE (`METRALE_REDZONE=<bytes>`, default 0 = off). Over-allocate by `pad`
        ...
        //
        // This is the detector compute-sanitizer could not be: Metrale Engine suballocates from pools,
        ...
        if pad > 0 {
            ...
            // and `METRALE_REDZONE_TRACE_IDX` adds the call site for the one that matters.
    ...
    fn kernel_registry(&self) -> Option<std::sync::Arc<metrale_core::registry::MetraleRegistry>> {
    ...
    fn copy_h2d_async(&self, src: &[u8], dst: DevicePtr, stream: u64) -> Result<()> {
        ...
        //
        // Costs nothing on the path everything takes today: no Metrale Engine call site

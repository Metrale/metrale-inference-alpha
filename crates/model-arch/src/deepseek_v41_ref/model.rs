...
//! 2026-09-25: DeepSeek-V4.1 full tiny-graph model: the config, weights, state
//! and trace types that `forward` (in `model/forward.rs`) fills. On the
//! fixture: embed, expand to `hc_mult` copies, the one-hot initial pre-mix,
//! engram before layers 1 and 4, six blocks with the delayed mixes,
//! collapse, final RMSNorm, f32 logits. The per-layer caches and the shared
//! slots carry across prefill and decode, so the golden's captures for every
//! layer and the logits are checkable.
//!
...
pub struct LayerTrace {
    ...
    pub ffn_out: Vec<f32>,
    pub h_out: Vec<f32>,
    // 2026-09-25: The `shared_*` fields snapshot the shared slots after this
    // layer, for the source-layer captures.

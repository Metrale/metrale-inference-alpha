...
//! 2026-09-26: Tests for `msg_entry`: duplicate-error masking, the
//! vacuous-system check, and `build_msg_entries`.
//!
...
mod error_dedup_tests {
    ...
    fn near_duplicate_errors_group_via_jaccard() {
        // 2026-09-26: Two long errors that differ only in their last token
        // are not equal, so they group through the Jaccard threshold.
...
mod vacuous_system_tests {
    ...
    fn substantive_prompt_is_not_vacuous() {
        ...
        // 2026-09-26: Longer than the 32-byte label limit.
        assert!(!is_vacuous_system_content(
...
mod build_tests {
    ...
    fn tool_result_image_is_collected_and_rejected_without_vision() {
        // 2026-09-26: The 400 fires only if the tool-result branch collected
        // the image.
    ...
    fn remote_url_image_rejected_with_clear_reason() {
        // 2026-09-26: With remote fetching off, the URL is refused when it is
        // collected, before the missing vision config is checked.
        let url_msg = Message {
            role: Role::User,

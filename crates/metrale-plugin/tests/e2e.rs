...
use metrale_plugin::benchmarks::{concurrency::ConcurrencySweep, ttft::TtftGate};
use metrale_plugin::{
...
async fn the_warm_gate_records_a_baseline_then_gates_against_it() {
    ...
    let mut bench = TtftGate::new(metrale_plugin::benchmarks::ttft::Mode::Warm);
    bench.load(h).await.expect("load");
    let mut values = ParamValues::defaults(&bench.parameters());
    ...
    let mut second = TtftGate::new(metrale_plugin::benchmarks::ttft::Mode::Warm);
...
async fn the_cold_gate_issues_one_request_per_sample_without_priming() {
    ...
    let mut bench = TtftGate::new(metrale_plugin::benchmarks::ttft::Mode::Cold);
    bench.load(h).await.expect("load");
    let mut values = ParamValues::defaults(&bench.parameters());
...
async fn a_headless_run_persists_a_record_the_history_pane_can_read() {
    ...
    let executor =
        metrale_plugin::BenchmarkExecutor::new(tokio::runtime::Handle::current(), store.clone());
    ...
    let outcome = tokio::task::spawn_blocking(move || {
        metrale_plugin::headless::run_blocking(
            &executor,
            metrale_plugin::headless::RunRequest {
                descriptor,
                values,
                target,
                options: metrale_plugin::headless::HeadlessOptions::cli("1.0.0-beta-preview"),
            },
            &mut metrale_plugin::headless::SilentReporter,
            &|| false,
        )
    })
    .await
    ...
    let back = metrale_plugin::history::load(&store, "concurrency-sweep");
    assert_eq!(back.len(), 1, "exactly one run in the directory");
    let r = &back[0];

    // 3. Identity and provenance.
    assert_eq!(r.schema, metrale_plugin::history::SCHEMA);
    assert_eq!(r.benchmark_name, "Concurrency Sweep");
    assert_eq!(r.source, metrale_plugin::RunSource::Cli);

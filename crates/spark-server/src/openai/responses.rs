...
//
// The Responses API is OpenAI's newer agentic-first surface. Metrale Engine
...
// use) are **not** supported — those require NSFW tool integrations Metrale Engine
// does not ship.
...
pub struct ResponsesRequest {
    ...
    /// Run the response asynchronously. Metrale Engine completes responses
    ...
    /// payloads to embed in the response. Metrale Engine accepts and ignores
    /// this; the base response already carries reasoning and tool data.
    #[serde(default)]
    pub include: Option<Vec<String>>,
    /// `truncation: "auto" | "disabled"` — OpenAI's auto-compaction
    /// hint. Metrale Engine has its own auto-compaction (`--auto-compact` CLI
    ...
    /// `parallel_tool_calls: bool` — Metrale Engine emits one tool call per
    /// turn regardless; accepted for compat.
    #[serde(default)]
    pub parallel_tool_calls: Option<bool>,
    /// Top-level `max_tool_calls` cap. Accepted for compat; Metrale Engine
    ...
    /// (Metrale Engine honors `max_output_tokens` only).
    #[serde(default)]
...
pub enum ResponsesOutputItem {
    ...
    /// Metrale Engine serves the raw `<think>` body as the item's `summary` (no
...
// SSE frames. Metrale Engine maps the existing chat-completions `StreamEvent`
...
pub enum ResponsesStreamEvent {
    ...
    // position inside the item's `summary` array (always 0 on Metrale Engine).

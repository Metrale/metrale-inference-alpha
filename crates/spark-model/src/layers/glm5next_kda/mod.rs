...
//! * 🪤 **`o_norm` is ADAPT, not REUSE.** Every Metrale Engine gated RMSNorm applies **SiLU** to the gate
...
//! * **Conv state widths differ.** HF keeps `kernel - 1` slots, Metrale Engine keeps `kernel` and shifts
//!   left before convolving, so `HF[0..k-1] == Metrale Engine[1..k]` and Metrale Engine slot 0 is a don't-care.
//! * **Decode conv fuses L2; prefill does not** and needs a separate `l2_norm_bf16` over q|k.
//!   q/k are normalised **exactly once**; **V never**.
//! * **The recurrent state is FP32 by reference semantics**, not Metrale Engine policy — HF stores it via
...
/// Dynamic shared memory available with no `cuFuncSetAttribute` opt-in in `MetraleCudaBackend`.
/// GB10 reports `sharedMemPerBlockOptin = 101376`, real but unreachable from Metrale Engine today.
...
impl Glm5NextKdaConfig {
    ...
    /// FP32 `[conv_dim, conv_kernel]` — Metrale Engine's width, one slot wider than HF's.
...
/// `METRALE_GLM_KDA_NO_SMEM=1` restores the 2R+2W recurrent kernel. Read once — this is on
...
///
/// 🪤 The conv buffer is Metrale Engine's **`conv_kernel`-wide** convention, one slot wider than HF's
/// `conv_kernel - 1`: Metrale Engine shifts left before convolving, so slot 0 is shifted out and never
/// participates. `HF[0..k-1] == Metrale Engine[1..k]` pre-shift. Any code moving state between the two

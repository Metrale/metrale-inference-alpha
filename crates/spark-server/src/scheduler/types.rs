...
pub(super) struct ActiveSeq {
    ...
    /// (METRALE_DFLASH_RESUME_GUARD) to keep the answer's opening tokens on
    /// serial decode, where the T=0 verify-vs-decode low-margin flips
    /// concentrate (measured 2026-07-07).
    pub post_think_emitted: u32,
    /// Adaptive speculation (METRALE_DFLASH_ADAPTIVE=1): rolling accept window
    ...
    /// [`metrale_kernels::ROLLBACK_RESTEER_CAP`]; once the cap is hit the

...
//! 2026-09-26: Tests of `drive` and of the executor's lifecycle order, with
//! fake benchmarks.
//!
//! Owner: bench.
...
/// 2026-09-26: A benchmark that yields `steps` running frames, then one
/// terminal frame, or an error at step `fail_at`.
struct Fake {
    steps: usize,
    seen: usize,
...
// 2026-09-26: `const`, because the traits under test borrow the descriptor
// and the metadata as `&'static`.
...
const FAKE_META: PluginMetadata = PluginMetadata::metrale("test double");
...
async fn executor_runs_the_lifecycle_and_always_cleans_up() {
    ...
    let store = ArtifactStore::with_root(
        std::env::temp_dir().join(format!("metrale-executor-lifecycle-{}", std::process::id())),
    );

...
def event(rank, seq, op="all_reduce", count=8, peer=None, dtype="Bf16", json_mode=False):
    ...
    if json_mode:
        return json.dumps(dict(target="metrale::comm", fields=fields)) + "\n"
    return "2026-09-19T00:00:00Z \x1b[32mINFO\x1b[0m metrale::comm: " + fields.pop("message") + " " + " ".join(f"{k}={json.dumps(v) if k in {'op', 'phase'} else v}" for k, v in fields.items()) + "\n"

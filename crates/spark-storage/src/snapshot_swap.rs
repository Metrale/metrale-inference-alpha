...
// `DirectSwapFile` — lives in the CUDA-/verbs-free `metrale-tier` crate and is
...
/// The generic paging core, lifted to `metrale-tier` (CUDA- and verbs-free).
/// Re-exported under the `metrale_tier` names (no historical aliases).
pub use metrale_tier::{

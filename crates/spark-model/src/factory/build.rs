...
pub fn build_model(
    ...
    //
    // The drafter's `target_layer_ids` are used DIRECTLY as Metrale Engine capture
    ...
    // head — "Metrale Engine can't read this layout" and "there is no head here" are
    ...
    //
    // We want the KV pool sized against Metrale Engine's OWN footprint (weights +
    ...
    //      without being an allocation Metrale Engine owns, inflating "Metrale-own"
    ...
    //
    //   3. MANUAL override: METRALE_KV_EXTERNAL_RESERVE_GB=<co-tenant GB> still
    ...
    if let Some(gb) = manual_reserve_gb {
        ...
        tracing::info!(
            "METRALE_KV_EXTERNAL_RESERVE_GB={gb} (manual override): discounting \
        ...
        if ledger_live > 0 && ledger_live <= used_so_far {
            tracing::info!(
                "KV budget self-relative (ledger): Metrale-own {:.1} GB live in \
    ...
    let num_kv_blocks = match hss_cache_blocks_per_seq {
        ...
        None => {
            ...
            //
            // Kill switch: `METRALE_KV_POOL_UNCLAMPED` (presence — `=0` is NOT
            // "off") restores the budget-driven pool.
            let n = if prefix_cache.is_active()
                || std::env::var("METRALE_KV_POOL_UNCLAMPED").is_ok()
            {
                budget_blocks
            } else {
    ...
    if max_concurrent < max_batch_size {
        ...
        // allocator, not a boot-time refusal). Kill switch: METRALE_KV_OVERCOMMIT=0

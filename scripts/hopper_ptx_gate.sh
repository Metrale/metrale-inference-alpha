...
#
# Does every Metrale Engine kernel compile for a given SM architecture? Answered with
...
#              tree. The source path is the RESOLVED one -- the file the
#              build compiles, wherever `[sources] use` or `inherits` found
#              it -- so it can be read against `kernels/` directly.
...
# from every architecture Metrale Engine targets. `redux.sync.max.abs.f32` is absent
...
if [ "${1:-}" = "--compile-one" ]; then
  ...
  strict=()
  if [ "${METRALE_GATE_STRICT:-0}" = 1 ]; then
...
# ── Task list ──
# THE resolver (scripts/lib/kernel_layout.py, the mirror of
# crates/closure/src/layout.rs) decides the per-quant file set: every
# layer the target reads -- its own leaf and common/, and the inherited
# tree's -- with `[sources] use` honoured, leaf over common BY FILE STEM, where
# "the leaf" is what `[model] kernel_source` resolves to. gb10/qwen3.8-27b redirects
...
# kernels/hopper and kernels/b200 define -DMETRALE_NO_WARP_BLOCKSCALE_MMA there,
...
#
# Identical (source, flags, include dirs) triples are compiled ONCE. Several
# gb10 models share one physical kernel through `[sources] use` —
# deepseek-v4-flash's w4a16_gemm.cu is three models' — and build.rs
...
tasks, mapping = {}, []
for model in models:
    # THE resolver decides the source set: every layer the target reads (its
    # own leaf and common/, and the inherited tree's), `[sources] use`
    # included, leaf over common by stem. The flag layers are every
    # KERNEL.toml it consulted, least specific first, under HARDWARE.toml's.
    try:
        lay = kl.discover(Path(root), hw, model, "nvfp4")
    except kl.LayoutError as e:
        die(str(e))
    merged = []
    for f in hw_flags + [f for cfg in lay.configs() for f in build_flags(cfg)]:
        if f not in merged:
            merged.append(f)
    # A source compiles beside the headers of its ROLE directory, then falls
    # back to the directory it is homed in — the same two places build.rs
    # (staging, then `-I<home>`) resolves a quoted include from.
    for stem, e in lay.modules().items():
        home = str(lay.layers[e.layer].dir)
        # No staging here, so the role directories stand in for it: the file's
        # home first, then every layer directory the target reads.
        layer_dirs = [str(l.dir) for l in lay.layers if l.dir.is_dir()]
        incdirs = " ".join([home] + [d for d in layer_dirs if d != home])
        src = str(e.source)
        key = hashlib.sha256(
            (src + "\0" + "\0".join(merged) + "\0" + incdirs).encode()
        ).hexdigest()[:16]
        tasks[key] = (key, src, incdirs, " ".join(merged))
        mapping.append((model, stem, key, os.path.relpath(src, root), src))

if listing:
    # (model, stem, source path) as RESOLVED — the file the build compiles.
...
L = []
L.append(f"# Metrale Engine PTX gate — `{hw}` @ `{arch}`\n")

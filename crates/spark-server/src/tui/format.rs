...
/// `free` and the HF cache report in — a Metrale Engine number a user cross-checks
...
/// number they are already looking at. Every source a Metrale Engine operator
...
/// `GiB` would make Metrale Engine the only correct thing on a screen full of `GB`, and

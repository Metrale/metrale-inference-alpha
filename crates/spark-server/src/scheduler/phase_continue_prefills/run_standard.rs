...
pub(super) fn run_standard_chunk_loop(
    ...
    // MLA correctness gate: Metrale Engine has no `prefill_attention_paged_mla_*`
    ...
    // fused mixed step stays under the TBT target. With METRALE_HOLO_ALWAYS_MIXED
    ...
    // needed) and when the abandoned METRALE_SSM_TAIL_CKPT is OFF (default).

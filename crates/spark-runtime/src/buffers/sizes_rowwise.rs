...
//! (≤500 LoC cap). Env-gated — 0 (→ NULL) unless `METRALE_FP8_ROWWISE=1`, so
...
//! `METRALE_FP8_ROWWISE` GDN prefill arms dequantise their per-row FP8 weights
...
/// on that predicate and the arms then require this slab, so `METRALE_FP8_ROWWISE=0`
/// must leave BOTH off.

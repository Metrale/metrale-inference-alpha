...
// 2026-09-25: Append one token to a contiguous cache with raw bf16 K and
// TurboQuant V: 2-, 3- or 4-bit codebook indices plus one E4M3 scale per 16
// values, one entry point per V dtype. K is copied unchanged and V is
// quantized as given: this kernel rotates neither, so a caller that wants
// WHT-rotated V passes it already rotated.
//
// Layout:
...
//
// Grid: (num_groups, 1, 1) threads, one thread per group of 16 elements.
//
// Owner: metal kernels.
...
// 2026-09-25: Codebook indices after scaling by cmax / max|v|; the E4M3 scale
// makes the reconstruction's L2 norm equal the input's.

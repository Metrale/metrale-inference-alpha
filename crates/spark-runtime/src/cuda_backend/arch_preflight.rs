...
//!
//! Metrale Engine compiles one SM architecture per build, and the driver's answer to a
...
//! [`metrale_core::arch`], and a message that names both sides. The rule itself
//! lives in metrale-core because `--check-kernels` reports it too.
//!
...
/// the new load on a fresh `metrale-swap` thread while the previous model's
...
/// constant: `metrale_core::device::sm121::NUM_SMS = 48`, named after one part,
...
///
/// Call this BEFORE constructing the backend: `MetraleCudaBackend::new` loads
...
/// `METRALE_SKIP_BUILD=1` stub registry compiles nothing and can attest to
...
impl DeviceQuery for DriverDeviceQuery {
    fn init_host(&self, ordinal: usize) -> Result<()> {
        metrale_core::cuda_host::host(ordinal).map_err(|e| anyhow::anyhow!("{e}"))?;
        Ok(())
    }

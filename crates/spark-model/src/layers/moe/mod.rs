...
pub struct MoeLayer {
    ...
    /// FP32-in/FP32-out router GEMM for METRALE_FP32_ROUTING (reads the FP32
    ...
    /// Opt-in via METRALE_MOE_GROUPED_K32=1: measured on qwen4_exp only, and
    ...
    /// when rows/expert <= 256. Bit-exact. Opt-in via METRALE_MOE_GROUPED_M256
    ...
    /// CUTLASS grouped-NVFP4 host tables (`METRALE_HOLO_MOE_GROUPED_CUTLASS`).
    ...
    /// `METRALE_UNIFIED_MOE_LAYOUT=1` opts in to the unified-layout decode
    ...
    unified_layout: bool,
    /// `METRALE_NVFP4_GATE_UP_M128=1` opts in to the M=128 fused gate+up
    /// kernel (Block D #3, Metrale Engine tile-shape rewrite). Halves block count
    ...
    /// `METRALE_HOLO_MOE_GATEUP_FP4=1` opts the prefill fused gate_up onto the
    ...
    gateup_fp4: bool,
    /// `METRALE_HOLO_MOE_DOWN_FP4=1` — same, for the prefill down projection over
    /// the shared `down_ptrs_t` table.
    down_fp4: bool,
    /// `METRALE_HYBRID_MOE_LAYOUT=1` opts in to the hybrid-layout path:
    ...
    /// M=128 variant of the K64 fused gate+up kernel (Block D #3, Metrale Engine
    ...
    /// (`METRALE_HOLO_MOE_GATEUP_FP4`). Same signature as `moe_fused_gate_up_t_k64`
    ...
    /// `None` = Metrale Engine's default softmax path. When `Some`, every top-k
    ...
    moe_w8a8_native_m64_k: KernelHandle,
    ...
    w8a16_gemm_pipelined_k: KernelHandle, // METRALE_W8A16_PIPELINED shared-expert variant
    ...
    // When the env var `METRALE_FRANKENSTEIN_DECODE_VIA_PREFILL=1` is set,

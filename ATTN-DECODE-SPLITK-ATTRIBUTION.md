...
prefill twin `attn_prefill_paged_fp8` has the identical `(24,1,1)` grid
...
**Prefill is NOT addressed here.** `attn_prefill_paged_fp8` has no
...
**Landed, C=1.** `METRALE_ATTN_DECODE_SPLITK=0` (S0) against `auto` (A15), same
binary, one variable:
...
```
paged decode attention: paged_decode_attn_splitk_fp8_hopper num_splits=11 sm_count=132 policy=auto (METRALE_ATTN_DECODE_SPLITK)
```

And `METRALE_ATTN_DECODE_SPLITK=0` boots as `attn_decode_splitk=1 (env)`: the

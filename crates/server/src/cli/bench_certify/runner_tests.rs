...
//! 2026-09-26: Tests for classifying a unit's record and for supervising a local child.
//!
...
/// 2026-09-26: Negative control: exit 0 is not evidence. No record, no pass.
#[test]
...
/// 2026-09-26: Negative control: a record for another commit is not this campaign's.
#[test]
...
/// 2026-09-26: The record's sha may be the long form of a short anchor, or vice versa.
#[test]
...
/// 2026-09-26: Negative control: an Info verdict on a plain gate, with exit 0, is not a pass.
#[test]
...
/// 2026-09-26: A shard's Info record with tallies is a completed member.
#[test]
fn an_info_shard_with_tallies_is_a_member_done() {
    ...
    // 2026-09-26: A shard record without tallies is not.
    let out = classify(
        &shard(),
        ANCHOR,
        Some(0),
...
/// 2026-09-26: Every local child, on the local-only path and the fleet's local node
/// alike (both call `reuse_args`), is told to reuse the leased server and whose
/// lease it is; `--no-serve-reuse` yields nothing.
#[test]
...
/// 2026-09-26: The real reader matches the shard, not just the id: two shards of one
/// group at one commit sit in one directory, and the newest file there is
/// whichever sibling finished last. Built from a fixture record in
/// `test_data/gate-records`, so it has a real record's shape.
#[test]
fn the_repo_reader_returns_the_record_of_this_shard_not_the_newest_sibling() {
    ...
    let source = metrale_bench::gate::records_newest_first(&ws, "bfcl-subset")
        .into_iter()
        .find(|p| p.extension().is_some_and(|e| e == "json"))
        .expect("a fixture bfcl-subset record");
    ...
    // 2026-09-26: Negative controls: a slice nobody ran, the whole draw, a record older
    // than the run.
    assert_eq!(
...
fn script(body: &str) -> (tempfile_dir::Dir, PathBuf) {
    ...
    let exe = dir.path().join("fake-met");
...
/// 2026-09-26: The child's stderr is streamed to the callback and to the log file
/// (stdout reaches the log prefixed `stdout: `), and the argv is the operator's
/// command line.
#[test]
fn the_child_is_supervised_and_its_lines_are_streamed() {
    ...
    // 2026-09-26: A shard's child is told which slice to run, and nothing else differs.
    assert_eq!(
        r.argv(&shard(), &c),
        [
            "benchmark",
...
/// 2026-09-26: Negative control: a child that runs past its deadline is killed and the
...
/// 2026-09-26: A spawn that meets `ETXTBSY` (the executable still open for writing) succeeds
/// once the writer lets go, instead of failing the unit as a harness error. Linux only: the
/// control needs exec to refuse a file that is open for writing.
#[cfg(target_os = "linux")]
#[test]
fn a_spawn_waits_out_a_busy_executable() {
    let (_dir, exe) = script("exit 0");
    let writer = std::fs::OpenOptions::new().write(true).open(&exe).unwrap();
    let plain = std::process::Command::new(&exe).spawn();
    assert_eq!(
        plain.err().and_then(|e| e.raw_os_error()),
        Some(libc::ETXTBSY),
        "control: exec must refuse the file while it is open for writing"
    );
    let release = std::thread::spawn(move || {
        std::thread::sleep(Duration::from_millis(30));
        drop(writer);
    });
    let mut child = spawn_retrying_busy(&mut std::process::Command::new(&exe))
        .expect("the spawn succeeds once the writer closes the file");

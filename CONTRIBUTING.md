# Contributing to Metrale Engine

WE thank you for your interest in Metrale Engine! This document explains how to contribute effectively.

## Philosophy

Metrale Engine follows the **AI Kernel HyperCompiling** philosophy: for every `(Hardware, Model_q)` tuple, there exists a set of kernels producing the highest performance such that it performs at the hardware's theoretical peak. Contributions should align with this — we value specialization over generalization.

### AI-First Codebase

Metrale Engine is an **AI-first codebase**. We're reversing the conventional logic:
...
# METRALE_SKIP_BUILD=1 makes metrale-kernels emit a stub instead of invoking
...
**kernel shadow structure** (`scripts/check_kernel_shadows.py` — no symlink
under `kernels/`, every target resolves through `metrale_closure::layout`, no
shadow byte-identical to what it shadows, no duplicate regular copies of one
file), file-size cap (≤500 LoC per `crates/**/*.rs`), and mdBook +
...
image? The build → verify → publish pipeline is documented in
[`docker/docker-guide.md`](docker/docker-guide.md) and `docs/releases/`.
...
vendored `cudarc`, the pure-FFI crates (`metrale-kernels`, `metrale-gpu-sys`,
`metrale-comm`, `crates/gpu-sys/src/verbs.rs`), the `layers/ops/` kernel-launch
...
Practical note: the workspace total is dominated by `crates/model-layers`, which
...
same run (`metrale-grammar` 87%, `metrale-bench` 75%, `metrale-core` 54%). Read the
...
1. Add kernel variants under `kernels/<hardware>/` optimized for the target SM
   ...
   A file that replaces a same-name file of a lower layer is declared in the
   `KERNEL.toml` beside it (`[shadow] <stem> = "<reason>"`); the resolver
   (`crates/closure/src/layout.rs`) refuses an undeclared one. To share
   a file instead of copying it, name it in that `KERNEL.toml`'s
   `[sources] use` (paths relative to `kernels/<hw>`); never symlink — the tree
   has none and CI refuses one. A hardware that is an OVERLAY of another
   (`kernels/hopper` and `kernels/b200` inherit `kernels/gb10` through
   `HARDWARE.toml [hardware] inherits`) holds only the files it tunes or adds;
   a hardware that compiles only a SUBSET of gb10 (`kernels/b300`,
   `kernels/strix`, `kernels/strix-hip`) lists the gb10 files it compiles in
   `[sources] use`. `scripts/check_kernel_shadows.py` (CI job
   `kernel-structure`) reports each overlay's own files and refuses a dead
   override or a duplicate regular copy.
2. Declare the target's SERVING levers in `kernels/<hardware>/HARDWARE.toml`
   ...
   `metrale_kernels::TARGET_DEFAULTS` / `TARGET_SM_COUNT` and every resolver in
   `metrale-model-layers` reads the declaration before the environment, so a target's
   measured configuration is a reviewable file rather than a launch script.
   `crates/kernels/build_defaults.rs` panics on an unknown key. Adding a
...
flowchart TD
    ...
    CHECKS[run the PR gate checks<br/><code>met benchmark run &lt;gate&gt; --pull-request-gate --yes</code>]:::gate
...
Metrale Engine is an **AI-first codebase**, and the PR template's Authorship field is not

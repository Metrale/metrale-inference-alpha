...
WT="${WT:-/home/claude/metrale-conglom}"; TS=$(date +%Y%m%d_%H%M%S)
...
( cd "$WT" && PATH=/usr/local/cuda/bin:$PATH METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b \
   cargo build --release -p metrale-server --bin met --features cuda ) || { echo BUILD_FAIL; exit 1; }
...
sudo docker run -d --name metrale-conglom --network host --gpus all --ipc=host \
  ...
  --mtp-quantization bf16 --tool-call-parser qwen3_xml --tool-grammar off --disable-thinking >/dev/null 2>&1

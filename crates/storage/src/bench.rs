...
//! 2026-09-26: Read micro-benchmarks for the storage backends, in MiB/s, from a file
//! opened with `O_DIRECT` (`open_test_file`, which also advises the kernel to drop the
//! file's cached pages) into device memory. `bench_cufile` reads with cuFile straight
//! into the device buffer; `bench_io_uring` and `bench_posix` read into a pinned host
//! buffer and copy each read to the device with `cuMemcpyHtoDAsync`. Sequential reads
//! cycle over the first `SEQ_ITERS` reads of the file; random reads land on
//! `RAND_IO_BYTES`-aligned offsets.
//!
//! Owner: storage.
...
pub fn bench_cufile(
    cufile: &metrale_gpu_sys::cufile::CuFile,
    fd: RawFd,
    file_bytes: u64,
    dev: &DeviceBuffer,
    io_bytes: usize,

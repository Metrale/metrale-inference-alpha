...
5. **0:05 The binary, option B: build on the box** (only with the CUDA 13.0 toolkit present; the
   ...
   and proceed with step 6 while it runs). Exact names from `crates/metrale-kernels/build.rs`,
...
8. **0:12 The serve.** The Spark recipe is `METRALE_DS41_EXPERT_CACHE_GIB=100
...
4. "The same LRU that runs on the Spark" and "`METRALE_DS41_EXPERT_CACHE_GIB` sized to the box":
...
`crates/metrale-kernels/build.rs`; `crates/metrale-core/src/arch.rs`;

...
//! 2026-09-25: Engram against the golden, one stage at a time, across prefill
//! and both decode steps.
//!
...
/// 2026-09-25: Run the hash state through all three regimes, returning
/// per-regime `[L, layers, cols]`.
...
fn tables_match_the_fixture_geometry() {
    ...
    // 2026-09-25: Each layer's row count is the sum of its primes.
    ...
    // 2026-09-25: Offsets are the exclusive prefix sums of the flattened primes.
...
fn hash_is_exact_across_prefill_and_both_decode_steps() {
    ...
    for (r, h) in f.regimes.iter().zip(&hs) {
        ...
        // 2026-09-25: The per-layer slice the engram of one layer receives.
...
fn e4m3_encoder_round_trips_the_fixture_tables() {
    ...
    // 2026-09-25: The two fp8 tables against the checksums the golden holds for them.
    let f = fx();
    let mut checked = 0;
...
fn e8m0_rounding_rule_matches_torch() {
    // 2026-09-25: The golden's scales are the generator's e8m0 cast of the
    // filler; recompute them with `f32_to_e8m0_rne`.
    let f = fx();
    let mut n = 0;
    for (li, lid) in f.t.layer_ids.iter().enumerate() {

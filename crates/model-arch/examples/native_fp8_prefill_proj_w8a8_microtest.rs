...
//! 2026-09-25: Prefill projections at Qwen3.8-27B shapes: the W8A16
//! `w8a16_gemm_pipelined` / `w8a16_gemm_n128_m128` kernels against the W8A8
//! block-scaled cuBLASLt route (`per_token_group_quant_fp8`, then
...
//!   against the W8A16 reference over rows 0..M, every value in rows
//!   0..ceil16(M) is finite, and every byte past them still holds `SENTINEL`.
//!
//! W8A8 quantizes the activation to E4M3 per token and 128-wide K group; the
//! W8A16 kernels read BF16 activations. The two differ by that quantization,
//! so the gate is a tolerance. `METRALE_W8A8_REL_RMS_GATE` overrides the
//! relative-RMS gate (`REL_RMS_GATE` otherwise).
//!
//! cuBLASLt is given `ceil16(M)` rows and writes rows `M..ceil16(M)`; at
//! M = 1193 that is 7 rows past the output. Those rows must be finite and the
//! sentinel beyond them intact.
//!
//! Each projection is also timed over `REPS` launches per route after one
//! warmup, as µs and TFLOP/s.
//!
//! Attention shapes from `kernels/hopper/qwen3.8-27b/MODEL.toml` (hidden 5120;
//! `q_proj` 12288 with the output gate; k/v 1024); the GDN `out_proj` uses
//! `VALUE_DIM` = 6144.
//!
//! Run: `cargo run --release -p metrale-model-arch --features cuda,gpu-examples
//! --example native_fp8_prefill_proj_w8a8_microtest`. The example calls the ops
//! directly, so the serve's route selection is not consulted; the serve
...
/// 2026-09-25: 1193 is not a multiple of 16, so the cuBLASLt pad rows are
/// exercised.
...
/// 2026-09-25: The W8A16 kernel a projection is compared and timed against.
#[derive(Clone, Copy, PartialEq)]
enum Ref {
    /// 2026-09-25: `w8a16_gemm_pipelined` over the row-major `[N, K]` weight.
    Pipelined,
    /// 2026-09-25: `w8a16_gemm_n128_m128` over the transposed `[K, N]` copy.
    TransposedM128,
}
...
/// 2026-09-25: `live` bytes are what the routes are compared on (rows 0..M);
/// `written` bytes are everything the W8A8 route may write (rows
/// 0..ceil16(M)), and must be finite. Beyond `written` the sentinel must
/// survive.
fn check(
    observed: &[u8],
    baseline: &[u8],
...
/// 2026-09-25: The W8A16 reference for this projection, on BF16 activations.
...
/// 2026-09-25: The route under test, as `qwen3_ssm/prefill_out_w8a8.rs` and
/// `qwen3_attention/prefill_qkv_w8a8.rs` compose it:
/// `per_token_group_quant_fp8`, then `cublas_fp8_proj_prequant`.
fn run_w8a8(
    gpu: &dyn GpuBackend,
    k: &Kernels,
    s: &Scratch,
...
/// 2026-09-25: `[N, K]` FP8 -> `[K, N]`, and `[N/128, K/128]` FP32 ->
/// `[K/128, N/128]`, the operands of the `w8a16_gemm_n128_m128` reference.
...
fn main() -> Result<()> {
    ...
    for m in ROWS {
        ...
        for p in &projections {
            ...
            if !controls_done {
                // 2026-09-25: A green run has to be able to go red: three
    ...
    println!(
        "\nALL PASS: Qwen3.8-27B prefill projections at M in {ROWS:?} — W8A8 cuBLASLt within \
         ...
         Serve spelling: METRALE_CUBLAS_GEMM=ffn,ssm,attn \
         (METRALE_SSM_OUT_W8A16_ONLY / METRALE_ATTN_QKV_W8A16_ONLY revert per site)."
    );
    Ok(())
}

# 2026-09-26: Scores one LongCat precision arm: final logits dump against the reference logits.
#
...
This script closes that: it compares Metrale Engine's FINAL logits against the HF
...
Metrale Engine:     `$METRALE_NEMO_DUMP/metrale_logits.bin`, f32, full vocab, written by
...
Usage:
    METRALE_NEMO_DUMP=<dir> bench/ngram_ref/serve_longcat_tui.sh          # arm under test
...
path = os.path.join(DUMP, 'metrale_logits.bin')
...
if not os.path.exists(path):
    print(f'no dump at {path} — is METRALE_NEMO_DUMP set on the SERVER process?')
    sys.exit(1)
...
# 2026-09-26: KL(ref || metrale) over the full vocabulary, in nats.
kl = float((p_ref * (lp_ref - lp_got)).sum())
# 2026-09-26: The probability Metrale Engine gives the reference's top-1 token.
...
print(f'metrale top-1 id     : {int(o_got[0])}  (p at ref top-1 = {p_got_at_ref_top1:.4f})')
print(f'top-1 agree        : {"YES" if top1 else "NO"}')
for k in (5, 10, 20, 100):
    print(f'top-{k:<3d} overlap    : {ov[k]}/{k}')
print(f'logit cosine       : {cos:.6f}')
print(f'KL(ref||metrale)     : {kl:.6f} nats')
...
json.dump({'label': LABEL, 'top1_agree': top1, 'overlap': ov, 'cos': cos,
           ...
           'ref_top1_id': int(o_ref[0]), 'metrale_top1_id': int(o_got[0]),

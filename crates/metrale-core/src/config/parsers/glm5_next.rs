...
pub fn parse_glm5_next(json: &str) -> Result<ModelConfig> {
    ...
    // Canonical model_type. The inner object says `glm5_next_text`; Metrale Engine keys
    ...
    // flat `linear_*` keys Metrale Engine uses for Qwen GDN.
    ...
    // routes to experts. Nothing in Metrale Engine read this before, so `mlp_only_layers` came out
    ...
    // both ways). Nothing in Metrale Engine read this before, so the value would have had to be
...
mod tests {
  ...
  "text_config": {{
    ...
    /// 🔴 A shared DSA layer must attend to the UPSTREAM layer's token set. Metrale Engine runs a
    ...
    /// that broke Metrale Engine's MLA decode dispatch.
    ///
    ...
    /// See .planning/METRALE-GLM5NEXT-SKILL-RECONCILIATION-20260826.md — the

...
//! 2026-09-25: `impl Model for NllbGpuModel`. `prefill` runs the encoder and seeds the
//! decoder with `[decoder_start, forced_bos]`, so the forced target-language token never
//! enters the sampled stream. The speculative, SSM and MTP methods either fail or do
//! nothing.
//!
//! Owner: model-engine.
...
impl NllbGpuModel {
    /// 2026-09-25: Encoder pass and decoder seed, shared by `prefill` and `prefill_chunk`.
    fn prefill_impl(&self, tokens: &[u32], seq: &mut SequenceState) -> Result<DevicePtr> {
        ...
        // 2026-09-25: Per-request source/target language (0 → deployment default).
        ...
        let src = self.lang.encoder_input_with(src_id, tokens);
        {
            ...
            // 2026-09-25: Seed the decoder: step 0 consumes `decoder_start`
            // (its logits are overwritten), step 1 consumes `forced_bos =
            // tgt_lang` and yields the logits for the first translated token.
            let row = self.prefill_logits_row(slot);
            self.forward_one(self.lang.decoder_start_id, kv, row)?;
            self.forward_one(tgt_id, kv, row)?;
        }
        self.sync()?;
        // 2026-09-25: The sequence records the decoder tokens, not the source;
        // the model's own KV holds the computed state.
...
impl ModelLifecycle for NllbGpuModel {
    ...
    fn alloc_sequence(&self) -> Result<SequenceState> {
        ...
        // 2026-09-25: Host-only sequence state; the KV lives in `self.kv`, keyed
        // by `slot`.
    ...
    fn free_sequence(&self, seq: &mut SequenceState) -> Result<()> {
        let slot = seq.slot_idx;
        if slot == usize::MAX {
            return Ok(()); // 2026-09-25: detached by `detach_slot_for_reuse`
        }
    ...
    fn cache_sequence(&self, _seq: &SequenceState) {
        // 2026-09-25: No prefix reuse for translation: nothing is cached.
    }
    fn num_free_blocks(&self) -> usize {
        // 2026-09-25: The KV lives outside the paged block cache, so there are no
...
impl ModelForward for NllbGpuModel {
    ...
    /// 2026-09-25: Batched decode: one `forward_one` per sequence, serially on
    /// the default stream, into contiguous logit rows `0..n` (batch position `i`
    /// ↔ `seqs[i]`); returns the base of `[n, vocab]`. Each sequence's KV is
    /// found by its `slot_idx`. Fails when `n` is 0, differs from
    /// `tokens.len()`, or exceeds `max_batch`.
    fn decode_batch(
        &self,
        tokens: &[u32],
        ...
        }
        {
            ...
            for (i, seq) in seqs.iter().enumerate() {
                // 2026-09-25: Per-request LoRA gate, re-armed for each sequence.
                self.set_lora_active(seq.adapter_slot);
                let kv = map
    ...
    fn generate_beam_batch(&self, reqs: &[crate::traits::BeamReq]) -> Result<Vec<Vec<u32>>> {
        // 2026-09-25: The requests' beams decode as one fused batch per step when
        // they share an adapter and `num_beams`; otherwise `beam_batched_multi`
        // runs them one at a time.
        self.beam_batched_multi(reqs)
    }
}

impl ModelLogits for NllbGpuModel {
    ...
    fn argmax_batch(&self, logits_ptr: DevicePtr, n: usize, _stream: u64) -> Result<Vec<u32>> {
        // 2026-09-25: `logits_ptr` is `[n, vocab]` bf16; argmax each contiguous row.
        (0..n)
...
impl ModelAdapters for NllbGpuModel {}

impl ModelSsmState for NllbGpuModel {
...
impl ModelVerify for NllbGpuModel {
...
impl ModelDraft for NllbGpuModel {
...
impl ModelDeviceFeed for NllbGpuModel {}

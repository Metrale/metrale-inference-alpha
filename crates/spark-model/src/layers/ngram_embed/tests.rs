...
/// 63 GB). Data dir comes from `METRALE_NGRAM_TEST_DATA`, generated by
/// `bench/ngram_ref/make_gpu_testdata.py`. GPU test: `#[ignore]` per
/// repo convention (CI is CPU-only); run with
///   METRALE_NGRAM_TEST_DATA=/tank/metrale-testdata/ngram_longcat_lite \
...
fn ngram_gpu_matches_golden() {
    ...
    let meta: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(format!("{dir}/meta.json")).unwrap())
            .unwrap();
    let gpu =
        spark_runtime::cuda_backend::MetraleCudaBackend::new(0, &metrale_kernels::ptx_modules())
            .expect("CUDA backend");

...
Phase A measured Metrale losing to vLLM at every concurrency above 1:

| C | Metrale tok/s | vLLM tok/s | Metrale TPOT p50 | vLLM TPOT p50 |
|---|---|---|---|---|
...
`crates/kernels/build.rs`, `collect_cu_files`:
...
```rust
METRALE_SSM_BATCHED_RECURRENT == "1" && self.gdn_f32_strided_k.0 != 0 && n > 1
```
...
```
DEBUG metrale_model_layers::layers: Optional kernel
...
A/B, differing ONLY by `METRALE_SSM_BATCHED_RECURRENT=1`.
...
* **The serialization root cause is NOT the GDN recurrent scan.** TPOT still
  ...
  `METRALE_SSM_MS_PROFILE=1` phase-split at C=4 to locate the ~160ms.

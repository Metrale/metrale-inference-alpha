...
Goal: run Metrale Engine on AMD GPUs **without SCALE**, by compiling the existing CUDA
...
smem loads (correctness-first; lose pipelining). For the NVFP4 **dense** model:
  - PORT: `w4a16_gemm` (✅ done, in `ported/`), `dense_gemm_tc`,
    `attn_prefill` (attention: same MMA shape + 4 cp.async sites).
  - STUB (not used by dense NVFP4): `w8a16_gemm(_t)` (FP8-weight), all `moe_*`
    (MoE), the unused `attn_prefill_paged_*` variants, `reshape_and_cache_turbo`
...
**B. Wire `HipTarget` into `build.rs`** (target/compile flow mapped 2026-06-03):
  ...
  - Before the compile loop (`build.rs:192`): copy the kernel sources to `OUT_DIR`,
    ...
    `METRALE_HIP_COMPAT_INCLUDE`. `HipTarget.compile` (already added) does the rest.
...
**C. Build + link:** `cargo build --release -p metrale-server --no-default-features
--features cuda` with the HIP env; link/LD against the shim `libcuda.so`
(`-L.../hip` first) + `libamdhip64`.

**D. Serve + measure (GPU window — bounce the SCALE demo server):** `met serve
Qwen/Qwen3.6-27B-FP8 …`; coherence check (temp=0 prompts); decode tok/s via
`~/bench-metrale.sh` (target 20). Decode needs **zero** MMA ports, so even with

...
// 2026-09-25: FP8 E4M3 -> BF16 dequantization with a block scale:
// out[n, k] = E4M3_LUT_DEQ[in[n, k]] * scale_inv[n / block_n, k / block_k], rounded to BF16.
//
// Owner: gb10 kernels.
// Invariants:
// - in and out are [N, K] row-major; scale_inv is [ceil(N / block_n), sk] and is FP32 when
//   scale_is_fp32 != 0, else BF16. Per-row and per-tensor scales are the cases
//   (block_n, block_k) = (1, K) and (N, K) with sk = 1.
// - One thread per element; host launches use grid (ceil(K / 64), ceil(N / 4), 1) and
//   block (64, 4, 1). Out-of-range threads return.
// - The LUT decodes the NaN codes 0x7F and 0xFF to 0, like metrale_core's FP8_E4M3_LUT.
...
extern "C" __global__ void dequant_fp8_blockscaled_bf16(
    ...
    const void* __restrict__ scale_inv,

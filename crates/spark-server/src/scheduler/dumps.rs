...
//! `METRALE_*` path on its first write. Both are sinks for one run's decode
...
pub struct RunDumps {
    /// `METRALE_LOGIT_DUMP=path` — per-position logits.
    pub logits: Option<Mutex<BufWriter<File>>>,
    /// `METRALE_ADADEC_DIAGNOSTIC=dir` — the adaptive-decode entropy trace.

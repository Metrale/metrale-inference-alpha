...
/// Causal depthwise conv + SiLU. Shifts Metrale-width state left, writes `x`
...
mod tests {
    ...
    fn kda_from_twin_json_is_unbounded() {
        ...
        let t = metrale_config::parse_config(twin).expect("twin json");
        let k = kda_from(&t);
        assert_eq!(
        ...
        let p = metrale_config::parse_config(official).expect("official json");

...
/// Drafter HF `config.json` (subset Metrale Engine consumes). Mirrors
...
pub struct DflashConfig {
    ...
    /// alias Metrale Engine silently drops the scaling (the RadixArk config.json had
...
/// Subset of HF `rope_scaling` block consumed by Metrale Engine. Mirrors the field
...
pub struct DflashSubConfig {
    ...
    /// anchor row's output is draft #1) versus the z-lab convention Metrale Engine's

...
# 2026-09-25: Writes the KDA recurrent-decode golden (kda_recurrent_prod_golden.json) at H=64,
# D=128 over three decode steps from a non-zero state.
#
# Owner: model-arch (GLM-5.3-Flash KDA reference).
# Invariants:
# - Writes only /w/kda_recurrent_prod_golden.json.
...
STEPS = 3
LOWER_BOUND = -5.0
SAMPLE_STRIDE = 251  # 2026-09-25: prime, so the sample does not alias the H or D period
...
rng = Lcg(0x5EED_2EC0)
# 2026-09-25: Drawn in exactly this order; the Rust side draws in the same order.
...
# 2026-09-25: The GPU decode kernel reads q/k already L2-normalised (the conv fuses the L2), so
# the golden runs with use_qk_l2norm_in_kernel=False on normalised q/k.
def l2(x):
    return x / torch.sqrt((x * x).sum(dim=-1, keepdim=True) + 1e-6)


outs, samples, checksums = [], [], []
carry = state0.unsqueeze(0).clone()
qn_steps, kn_steps = [], []

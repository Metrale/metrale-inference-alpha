...
pub struct ServeArgs {
    ...
    ///
    /// This replaces `METRALE_ALLOW_SHADOWED_KERNELS`, which covered only the
    ...
    ///
    /// A one-line JSON object (`{"metrale_kernel_check": …}`) is printed on
    ...
    // ── GDN / SSM decode path ──
    //
    // These four were `METRALE_*` environment variables. They are CONFIGURATION,
    ...
    /// agentic gate.
    ///
    /// Legacy: `METRALE_SSM_H_FP16` (presence) selects f16 — never f16-pool,
    ...
    /// arithmetic ceiling.
    ///
    /// Legacy: `METRALE_SSM_DECODE_RING=1|0` still means depth 8 and 0. It is
    ...
    ///
    /// Legacy: `METRALE_GDN_FUSED_NORM=1`, on the same terms as `--ssm-h-dtype`.
    ///
    ...
    ///
    /// Legacy: `METRALE_SSM_BATCHED_RECURRENT=1`, on the same terms as
    ...
    ///
    /// Legacy: `METRALE_PREFILL_VARLEN=1`, on the same terms as
    /// `--gdn-fused-norm`, and `Option` for the same reason.
    #[arg(long, num_args = 0..=1, default_missing_value = "true")]
    ...
    /// them.
    ///
    /// Legacy: `METRALE_PREFILL_CODISPATCH=1`, on the same terms as
    ...
    ///
    /// Legacy: `METRALE_SSM_TAIL_MIDCHUNK=0` disables when this flag is ABSENT.
    ///
    ...
    ///
    /// Legacy: `METRALE_MTP_GATE_FORCE=1` selects `force` when this flag is
    ...
    /// Replaces the former METRALE_LMHEAD_BF16 env var.
    ...
    /// Metrale Engine will use at most ~60 GB in total.
    ...
    /// Precedence (highest wins): this flag → METRALE_CONTENT_LOOP_WATCHDOG
    ...
    /// METRALE_CONTENT_LOOP_MIN_REPEATS → built-in default.
    #[arg(long)]
    ...
    /// (`tokenizer/message_preprocess.rs`) for Metrale Engine's cross-cutting chat
    ...
    /// runs full-prefix attention by default; at Metrale Engine's typical 16K
    ...
    /// `METRALE_MARCONI_MIN_TOKENS` cannot, so a run using it could not say so.
    ///
    ...
    /// 0 = disabled. Swap files stored in /tmp/metrale-swap/.
    ...
    // io_uring + a predictor-driven scratch pool. See the spark-storage crate.
    ...
    /// --swap-space-gb's /tmp/metrale-swap to avoid file collisions.
    #[arg(long)]
    ...
    /// rejected at startup). Replaces `METRALE_FP8_KV_HEADROOM`.
    ...
    /// `METRALE_FAST_LOAD_PREFETCH_SHARDS=1`.
    #[arg(long, default_value_t = false)]
    ...
    ///
    /// Also settable with `METRALE_VISION_MAX_PIXELS`.
    #[arg(long, default_value_t = 0)]
    ...
    /// like most Metrale Engine features. Enabling it makes the inference server issue
    ...
    /// animated GIF. Metrale Engine does not bundle a video decoder: GIF is decoded
    ...
    ///
    /// Defaults to off — Metrale Engine is local-by-default, so most users can
    ...
    /// of a 404. Requires `$METRALE_LORA_PEER`. Empty = today's resident-only
    ...
    /// `METRALE_LORA_ROTATE=1` (so decode runs eager and the disk swap can

...
pub(crate) fn canonicalize_model_quant(config: &metrale_core::config::ModelConfig) -> String {
    ...
    // we don't currently dispatch those on Metrale Engine; report verbatim so
...
pub(crate) fn describe_quant_source(config: &metrale_core::config::ModelConfig) -> String {
...
///
/// The current Metrale Engine build emits one bundle per (hw, model) regardless
/// of how many quant variants it dispatches at runtime: the bundle
/// label is whichever `METRALE_TARGET_QUANT` value the build script

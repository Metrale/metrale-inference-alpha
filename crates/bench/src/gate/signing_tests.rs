...
//! 2026-09-26: Tests of `signing`: a round trip, the refusals (edited record,
//! other commit, moved sidecar, unregistered signer, missing signature after
//! the cutover), the pre-cutover exemption, identity and registry reuse, the
//! private key's file mode, and a scan of the committed `.benchmarks` records.
//!
//! Owner: bench gate.
...
/// 2026-09-26: Write a record and a registry entry for a fresh identity;
/// return the temp dir, the record path and the identity.
fn fixture(recorded_at_note: &str) -> (tempdir::Dir, std::path::PathBuf, Identity) {
    let dir = tempdir::Dir::new();
    let root = dir.path();
    let home = root.join("metrale-home");
...
fn a_record_cannot_be_repointed_at_another_commit() {
    // 2026-09-26: The signed message ends with the sha, so the same bytes
    // verified against another commit fail.
    let (dir, record, id) = fixture("ok");
    sign_record(&id, &record, "abcdef1234").unwrap();
...
fn an_unregistered_signer_is_refused() {
    let (dir, record, id) = fixture("ok");
    sign_record(&id, &record, "abcdef1234").unwrap();
    // 2026-09-26: The signature and key are valid; only the registry entry is
    // missing.
    std::fs::remove_file(
        dir.path()
            .join(REGISTRY_DIR)
...
fn the_sidecar_path_appends_and_does_not_eat_a_versioned_model_name() {
    // 2026-09-26: `variant_slug` keeps `.` in a model name; the sidecar name is
    // the whole record name plus `.sig`.
...
/// 2026-09-26: Every committed record whose `recorded_at` is at or after
/// `SIGNATURE_REQUIRED_AFTER` has a `.sig` beside it. Records that cannot be
/// read or parsed, and `BASELINE.json`, are skipped. The constant's value is
/// not pinned: a later cutover exempts more records and still passes.
#[test]

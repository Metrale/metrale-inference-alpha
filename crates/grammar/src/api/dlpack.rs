...
// works on `&mut [i32]` directly, but Metrale Engine's `grammar/state.rs` still
// builds a `DLTensor` describing its bitmask buffer. We provide a
// layout-faithful, pure-Rust re-implementation so that Metrale Engine's
...
///
/// Only the variants Metrale Engine references are defined; discriminants match
...
///
/// This mirrors the field set Metrale Engine's `grammar/state.rs` populates. The

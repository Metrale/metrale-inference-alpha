...
//! 2026-09-25: The MLA branch of `prefill_attention_paged`: latent Q and KV
//! projections, RoPE on the rope parts, K and V assembled for flash attention
//! over this call's tokens, the compressed cache write, and the O projection.
//!
//! Owner: model-layers (qwen3 attention).
//! Invariants:
//! - A call with `seq_len_start != 0` fails before any launch.
...
pub(super) struct MlaPrefillArgs {
    ...
    pub nkv: u32,
    pub hd: u32,
    /// 2026-09-25: First absolute position this call processes. Above 0, earlier
    /// tokens are in the paged cache, which this path does not read.
...
impl Qwen3AttentionLayer {
    /// 2026-09-25: Run the MLA prefill chain. Returns the O projection's output,
    /// the `norm_output` buffer.
    pub(super) fn prefill_attention_paged_mla(
        ...
        // 2026-09-25: The flash call below attends over `k_contiguous` and
        // `v_contiguous`, the K and V of this call's tokens only. After a
        // prefix-cache hit only the uncached tail reaches this call, and it would
        // not attend to the cached prefix, so that case is refused. A hit that
        // leaves one token never gets here: `prefill_dispatch` (model-engine
        // `prefill_a.rs`) sends n <= 1 to `decode`, which reads the paged cache.
        anyhow::ensure!(
        ...
        // 2026-09-25: Q: latent, norm, expand to `[N, nq*hd]`, `[nope | rope]` per head.
        ...
        // 2026-09-25: KV: latent, norm, expand.
        ...
        // 2026-09-25: K_rope: `[N, rope]`, one head shared by all query heads.
        ...
        // 2026-09-25: RoPE on the Q rope parts and on K_rope, before assembly.
        ...
        // 2026-09-25: One kernel assembles K = `[nope | rope]` and extracts V.
        ...
        // 2026-09-25: `mla_cache_assemble_batched` builds the compressed K and V
        // cache rows from `kv_latent` and `k_rope`.
        ...
        // 2026-09-25: Causal flash attention on the expanded Q, K and V.
        ...
        // 2026-09-25: O projection, `[N, nq*hd]` to `[N, H]`.

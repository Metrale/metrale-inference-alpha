...
//! 2026-09-25: The one place that turns a safetensors `data_offsets` pair into a byte span.
//!
//! Owner: metrale-core.
//! Invariants:
//! - A returned `TensorSpan` satisfies `abs_offset + len <= file_len`, computed
//!   without overflow; every other input is an error.
//!
//! A checkpoint is third-party data, so every number in its header is
//! untrusted until checked. The header declares each tensor as
//! `"data_offsets": [start, end]` relative to the data section, and the naive
//! `end - start` is a `u64` subtraction that wraps on a crafted or truncated
//! file: a reversed pair yields a length near `u64::MAX`.
//!
//! Both header parsers call this: `metrale_model_weights::fast_weights`
//! (`fast_weights/header.rs`, whose spans become `pread` windows) and
//! `metrale_storage::weight_peer` (`weight_peer/shard.rs`, which builds the
//! RDMA manifest).

use anyhow::{Context, Result, bail};
use serde_json::Value;

/// 2026-09-25: A validated tensor byte span: an absolute file offset and a
/// length that is known to fit inside the file it came from.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct TensorSpan {
    /// 2026-09-25: Absolute byte offset in the shard file where the tensor starts.
    pub abs_offset: u64,
    pub len: u64,
}

/// 2026-09-25: Validate one tensor's `data_offsets` against the file that declared it.
///
...
///
/// Rejects, in this order: a value that is not an array of two `u64`s, a
/// reversed pair (`end < start`, the underflow), an absolute start or end
/// that overflows `u64`, and a span that runs past end-of-file. Callers get a
/// named error instead of a wrapped length.
pub fn tensor_span(
    tensor: &str,
    offsets: &Value,
    data_start: u64,
    ...
    // 2026-09-25: The underflow. `rel_end - rel_start` wraps to ~u64::MAX on a reversed
    ...
    // 2026-09-25: `data_start` is bounded by the 64 MiB header cap both callers
    // enforce, but `rel_start` is not, so the sum still needs checking.
...
mod tests {
    ...
    /// 2026-09-25: A well-formed pair keeps its exact span, converted to absolute.
    #[test]
    ...
    /// 2026-09-25: A zero-length tensor is accepted.
    #[test]
    ...
    /// 2026-09-25: A tensor may end exactly at EOF.
    #[test]
    ...
    /// 2026-09-25: `rel_end - rel_start` on a reversed pair would wrap to
    /// ~u64::MAX in release builds; it is rejected instead.
    #[test]
    fn rejects_reversed_offsets_instead_of_underflowing() {
        ...
        // 2026-09-25: Sanity-check that the unchecked form really does wrap, so this test
        // is guarding a live hazard and not a hypothetical one.
        assert_eq!(0u64.wrapping_sub(4096), u64::MAX - 4095);
    }

    /// 2026-09-25: A truncated shard: the header still advertises the full tensor.
    #[test]
    ...
    /// 2026-09-25: A huge `rel_start` must not wrap when added to `data_start`.
    #[test]
    ...
    /// 2026-09-25: A valid absolute start can still overflow when its nonzero length is added.
    #[test]
    ...
    /// 2026-09-25: Malformed `data_offsets` shapes are named, not indexed into blindly.
    #[test]

...
//! 2026-09-25: Runs `metrale_kernels::resolve` on the real
//! `kernels/gb10/*/MODEL.toml` declarations: which target each checkpoint
//! reference resolves to, and the collision rules build.rs enforces.
//!
//! Owner: metrale-kernels tests.
//! Invariants: none beyond the types.
//!
//! `src/resolve_tests.rs` tests the rules on synthetic fixtures. These tests
//! read MODEL.toml directly, so they run under `METRALE_SKIP_BUILD=1`, where
//! `all_ptx_sets()` is an empty stub. qwen3.6-27b and qwen3.8-27b both declare
//! `(qwen3_5, 5120)`, so resolution breaks that tie on the checkpoint
//! reference.

use metrale_kernels::ModelTypeMatch;
use metrale_kernels::resolve::resolve_pinned;
use metrale_kernels::resolve::{ResolveCandidate, TargetResolveError, resolve_target};
...
/// 2026-09-25: One gb10 target: directory name, `[[model_types]]`,
/// `[model].match_names` and `[model].kernel_source`.
struct ParsedTarget {
    name: &'static str,
...
/// 2026-09-25: `ModelTypeMatch` holds `&'static str` because build.rs bakes it,
/// so the test leaks its parsed strings.
...
fn parse_targets() -> Vec<ParsedTarget> {
    ...
    let mut names: Vec<String> = std::fs::read_dir(gb10_dir())
        ...
        .collect();
    // 2026-09-25: Sorted by model name, as build.rs orders a wildcard build's
    // models.
    names.sort();
    for name in names {
...
fn qwen38_checkpoint_resolves_to_its_own_target() {
    ...
    // 2026-09-25: The base repo id and an HF cache path resolve too.
    assert_eq!(
...
/// 2026-09-25: Qwen3.6-27B checkpoint ids resolve to qwen3.6-27b.
#[test]
...
/// 2026-09-25: A Qwen3.5-27B checkpoint reference resolves to qwen3.6-27b,
/// whose `match_names` include `"qwen3.5-27b"`.
#[test]
...
/// 2026-09-25: A reference that names neither target (`/model`) cannot break
/// the (qwen3_5, 5120) tie: resolution returns `Ambiguous` naming both
/// candidates and the `--kernel-target` remedy, and that pin then resolves.
#[test]
...
fn nemotron_puzzle_checkpoint_resolves_to_the_puzzle_target() {
    ...
    // 2026-09-25: Only nemotron-super-120b-a12b declares (nemotron_h, 4096).
    assert_eq!(
        resolve_name(
            "nemotron_h",
            4096,
...
/// 2026-09-25: The first check of build.rs's `validate_collision_match_names`,
/// repeated here because under `METRALE_SKIP_BUILD=1` build.rs does not run
/// it: every target that shares a `(model_type, hidden_size)` with another
/// declares `match_names`.
#[test]
...
/// 2026-09-25: The dominated-needle check of `validate_collision_match_names`:
/// in a colliding group, each target has a needle that contains none of a
/// sibling's needles. Otherwise every reference it matches also matches the
/// sibling, and resolution can only return `Ambiguous`.
#[test]
...
/// 2026-09-25: Only qwen3.6-27b declares (qwen3_6_moe, 5120), so the pair
/// resolves to it without a tie-break.
#[test]
fn qwen36_moe_beltandbraces_tier_resolves_uncontested() {
    ...
    // 2026-09-25: Even from a reference that names no target.
    assert_eq!(
...
/// 2026-09-25: Every gb10 `kernel_source` names another gb10 target that does
/// not itself redirect and has an `nvfp4/` tree.
#[test]
...
/// 2026-09-25: qwen3.8-27b reuses qwen3.6-27b's kernels, has no
/// `[dflash]`, and keeps `[sampling.tools]` free of penalties. The
/// qwen3.6-27b MODEL.toml records the reason: a 2026-07-04 measurement of
/// BFCL non_live falling from ~85 to ~76 with penalties on.
#[test]
fn qwen38_target_declarations_are_sound() {
    ...
    // 2026-09-25: qwen3.6-27b's `[dflash]` pairs `z-lab/Qwen3.6-27B-DFlash`;
    // qwen3.8-27b pairs none.
    assert!(

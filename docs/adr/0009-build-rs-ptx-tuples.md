...
Metrale Engine's CUDA kernels are written per **(hardware, model, quant)** tuple.
...
Metrale Engine uses approach (3) with a directory tree:
...
A single `build.rs` (`crates/kernels/build.rs`) walks the tree,
reads `HARDWARE.toml` and `MODEL.toml`, and compiles every `.cu` for
the **selected tuple** to PTX via `nvcc`. The build script:

- Picks the tuple via env vars: `METRALE_TARGET_HW`, `METRALE_TARGET_MODEL`,
  `METRALE_TARGET_QUANT`. Each accepts `*` to wildcard ("all known
...
- Marks `cargo:rerun-if-changed=` on every consumed file so incremental
  builds work.
- Provides an `METRALE_SKIP_BUILD=1` short-circuit that emits a stub
  registry — used by CI for GPU-free `cargo check` / `clippy` runs.

At runtime, `metrale-gpu-runtime::gpu` loads the embedded PTX into the CUDA
...
- Every (model, quant) added is more `nvcc` invocations at build
  ...
  `METRALE_SKIP_BUILD=1` to dodge this for non-build steps.
...
- `MODEL.toml` is now a load-bearing configuration surface that lives
  ...
  with serde-derived parsers in `metrale-core`.

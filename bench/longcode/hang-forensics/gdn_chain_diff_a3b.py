...
# 2026-09-26: GDN layer-0 stage-by-stage diff, Metrale Engine dumps versus the HF reference dumps.
#
...
Usage:  python3 gdn_chain_diff_a3b.py /workspace/dumps/metrale-gdnsub-a3b /home/claude/gdnref_a3b
"""
import sys, pathlib
import numpy as np
...
def report(name, a, b, segs, head_dim):
    ...
    if a is None or b is None:
        print(f"  MISSING  metrale={a is not None}  hf={b is not None}")
        return
    print(f"  metrale_n={len(a)}  hf_n={len(b)}")
    if len(a) != len(b):
        c, r = cos_rel(a, b)
...
print("=== GDN layer-0 chain diff (A3B; Metrale Engine vs source-grounded HF oracle) ===")
print(f"metrale={A}")
print(f"hf   ={H}")

# 2026-09-26: Qwen3.6-35B-A3B (config.json): conv segments q|k|v = 2048|2048|4096
# (16 and 32 heads of 128); the recurrence and gated norm are one 4096 value segment.
report("conv1d   (Metrale Engine conv  ~ HF conv1d post-silu)",
       rd(f"{A}/gdnsub_step0_L0_conv.bin"), rd(f"{H}/gdnref_L0_conv1d.bin"),
       [("q", 2048), ("k", 2048), ("v", 4096)], 128)
report("l2norm   (Metrale Engine l2 (post-l2norm of q,k)  ~ HF n/a — Metrale-only sanity)",
       rd(f"{A}/gdnsub_step0_L0_l2.bin"), rd(f"{H}/gdnref_L0_conv1d.bin"),
       [("q", 2048), ("k", 2048), ("v", 4096)], 128)
report("recur    (Metrale Engine gdn   ~ HF recur_in/pre-norm)",
       rd(f"{A}/gdnsub_step0_L0_gdn.bin"), rd(f"{H}/gdnref_L0_recur_in.bin"),
       [("v", 4096)], 128)
report("gnorm    (Metrale Engine gnorm ~ HF norm gated-rmsnorm)",
       rd(f"{A}/gdnsub_step0_L0_gnorm.bin"), rd(f"{H}/gdnref_L0_norm.bin"),
       [("v", 4096)], 128)

print("\n--- norms (context) ---")
for lbl, p in [
    ("metrale conv ", f"{A}/gdnsub_step0_L0_conv.bin"),
    ("metrale l2   ", f"{A}/gdnsub_step0_L0_l2.bin"),
    ("metrale gdn  ", f"{A}/gdnsub_step0_L0_gdn.bin"),
    ("metrale gnorm", f"{A}/gdnsub_step0_L0_gnorm.bin"),

...
//!
//! `cargo test` runs GPU-free with `METRALE_SKIP_BUILD=1`, where `build.rs`
...
//! MODEL.tomls, and what `build.rs` would resolve. The `METRALE_NO_WARP_
...
/// reads it and holds `metrale_core::arch::target_hint` to it, so the value has
/// to be right.
#[test]
...
/// expand `METRALE_TARGET_MODEL=*`: a subdirectory that has a MODEL.toml.
...
/// (`METRALE_TARGET_MODEL=*`) compiles exactly the directories that carry a
...
/// done. `[expected_absent]` is harvested per hardware (`met serve
...
/// reaches it: `METRALE_SKIP_BUILD=1` returns from `main()` before target

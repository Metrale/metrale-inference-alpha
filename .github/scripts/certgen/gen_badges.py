...
"""Generate Metrale Engine PR-certification masthead components (header band, QR plaque,
...
def qr_group(url=QR_URL, x=QR_X, y=QR_Y, m=QR_M, side=None):
    ...
    return (f'  <g id="qr">\n'
            ...
            f'fill="#FFFFFF" stroke="#2F3742" stroke-width="1"/>\n'
            f'    <path fill="#0E1318" d="{"".join(d)}"/>\n'
            f'  </g>\n'), side
...
DEFS = f"""  <defs id="cert-defs">
    ...
    <linearGradient id="cert-goldRim" x1="0" y1="0" x2="1" y2="1">
      ...
      <stop offset="0.72" stop-color="#9C6B12"/><stop offset="0.88" stop-color="#E4C070"/>
    ...
    <linearGradient id="cert-fadeHdr" x1="0" y1="0" x2="1" y2="0">
      <stop offset="0" stop-color="#181F28" stop-opacity="0"/>
      <stop offset="0.85" stop-color="#181F28" stop-opacity="1"/>
    </linearGradient>
...
def header_group(pr="#0000", author="author-login"):
    ...
    <rect x="64" y="92" width="888" height="84" rx="3" fill="#181F28" stroke="#2F3742" stroke-width="1.4"/>
    <rect x="67.5" y="95.5" width="881" height="77" fill="none" stroke="#232A34" stroke-width="1"/>
    <line x1="280" y1="92" x2="280" y2="176" stroke="#2F3742" stroke-width="1"/>
    <line x1="440" y1="92" x2="440" y2="176" stroke="#2F3742" stroke-width="1"/>
    <line x1="620" y1="92" x2="620" y2="176" stroke="#2F3742" stroke-width="1"/>
    <text x="84" y="117" font-size="10" fill="#8A8F99" font-weight="600" letter-spacing="2">DOCUMENT</text>
    <text x="84" y="150" font-size="15" fill="#D9D9DE" font-weight="650">PR Certification Record</text>
    <text x="300" y="117" font-size="10" fill="#8A8F99" font-weight="600" letter-spacing="2">PULL REQUEST</text>
    <text x="300" y="154" font-size="26" fill="#D9D9DE" font-weight="700" clip-path="url(#cert-clip-pr)" id="value-pr-number">{pr}</text>
    <rect x="394" y="124" width="28" height="34" fill="url(#cert-fadeHdr)"/>
    <text x="460" y="117" font-size="10" fill="#8A8F99" font-weight="600" letter-spacing="2">AUTHOR</text>
    <text x="460" y="150" font-size="13.5" fill="#D9D9DE" font-family="{MONO}" clip-path="url(#cert-clip-author)" id="value-pr-author">{author}</text>
    <rect x="566" y="127" width="28" height="30" fill="url(#cert-fadeHdr)"/>
    <text x="640" y="117" font-size="10" fill="#8A8F99" font-weight="600" letter-spacing="2">APPROVALS</text>
    <g id="cert-approvals-placeholders" stroke="#2F3742" fill="none">
      <circle cx="700" cy="134" r="30" stroke-dasharray="3 4"/>
      <circle cx="872" cy="134" r="30" stroke-dasharray="3 4"/>
      <path d="M 630 102 h 8 M 634 98 v 8 M 934 102 h 8 M 938 98 v 8 M 630 166 h 8 M 634 162 v 8 M 934 166 h 8 M 938 162 v 8" stroke="#2F3742" stroke-width="1"/>
    </g>
...
def stamp_group(user="stamper-login", sha="0000000000",
    ...
    {ring_text("\u25c6 METRALE /stamp \u25c6", 55, 8.5, "#454B55", "bottom")}
...
def seal_group(user="sealer-login", sha="0000000000",
    ...
    {ring_text("\u25c6 METRALE /seal \u25c6", 58, 9, "#4A3407", "bottom")}
...
BG_OLD = '<rect width="1200" height="928" fill="#0E1318"/>'
BG_NEW = '<rect width="1200" height="1040" fill="#0E1318"/>'
...
def standalone_header(pr, author):
    ...
    return (f'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" '
            ...
            + DEFS
            + '  <rect width="1200" height="208" fill="#0E1318"/>\n'
            + LOGO
            + '  <text x="156" y="44" font-size="23" fill="#D9D9DE" font-weight="650">PR Certification</text>\n'
            + '  <text x="156" y="66" font-size="13.5" fill="#8A8F99">How a change gets from opened to merged, and what knocks it back.</text>\n'
...
def standalone_badge(kind):
    if kind == "stamp":
        ...
        w, h, label = 300, 216, "Metrale Engine silver certification stamp"
    else:
...
def standalone_qr():
    ...
    return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {w}" width="{w}" height="{w}" '
            ...
            f'  <title>Metrale Engine PR link QR plaque</title>\n'
            f'  <rect width="{w}" height="{w}" fill="#0E1318"/>\n' + qr + '</svg>\n')

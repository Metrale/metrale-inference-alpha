...
fn every_registered_benchmark_can_be_selected() {
    let mut s = state();
    for i in 0..metrale_plugin::registry::all().len() {
        s.select(i);
...
fn the_shell_running_benchmark_asks_for_confirmation_first() {
    let mut s = state();
    let index = metrale_plugin::registry::all()
        .iter()
...
fn list_navigation_is_clamped_to_the_registry() {
    let mut s = state();
    let n = metrale_plugin::registry::all().len();
    for _ in 0..n + 5 {
...
/// found by `history::load_all` — the function `met benchmark history` calls
...
fn a_run_persisted_by_the_dashboard_is_readable_by_the_cli() {
    ...
    let store = metrale_plugin::ArtifactStore::with_root(dir.path());
    let executor = metrale_plugin::BenchmarkExecutor::new(runtime.handle().clone(), store.clone());

    let mut s = state();
    let descriptor = metrale_plugin::registry::find("concurrency-sweep").expect("registered");
    s.select(0);
    ...
    let frame = BenchmarkResult {
        ...
        summary: Vec::new(),
        table: None,
        verdict: Some(metrale_plugin::Verdict::pass("fine")),
    ...
    let found = metrale_plugin::history::load_all(&store);
...
fn a_run_is_recorded_once_even_if_the_terminal_frame_repeats() {
    ...
    s.persist(&frame);
    s.persist(&frame);
    assert_eq!(metrale_plugin::history::load_all(&store).len(), 1);
}

#[test]
fn the_target_follows_the_model_that_is_actually_serving() {
    // Captured once at dashboard start from the boot argv: empty for
    // `met serve` with no model, and stale the moment one is loaded from the
    // Library or swapped in by a request.
    let mut s = BenchState {
        target: metrale_plugin::TargetEndpoint::local(8888, String::new()),
        ..BenchState::default()
    };
...
fn a_target_the_operator_typed_is_left_alone() {
    ...
    let mut s = BenchState {
        target: metrale_plugin::TargetEndpoint::local(8888, "org/mine".to_string()),
...
fn an_empty_live_model_does_not_blank_the_target() {
    let mut s = BenchState {
        target: metrale_plugin::TargetEndpoint::local(8888, "org/a".to_string()),
        ..BenchState::default()
    };
    s.follow_live_model("");

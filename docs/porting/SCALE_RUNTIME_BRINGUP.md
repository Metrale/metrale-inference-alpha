# Metrale Engine → Strix Halo (SCALE/gfx1151) — Runtime Bring-Up Status

**Date:** 2026-05-21  **Branch:** `port/amd-strix-halo`

## Summary

On native Ubuntu (the WSL `/dev/kfd` blocker is gone), `metrale-server`
...
1. **NCCL feature-split** — `nccl` is now a Cargo feature separate from
   ...
   cuda-without-nccl fail-fast variant. (metrale-comm, metrale-server)
2. **SCALE `cuGraphInstantiate`** — SCALE's libcuda exports the 3-arg
   `cuGraphInstantiate` (no `WithFlags` alias). `metrale_scale` cfg
   (emitted by metrale-gpu-runtime/build.rs from `METRALE_TARGET_HW=strix`)
   selects the right symbol. NVIDIA path unchanged.
3. **Binary-kernel registry** — `metrale-kernels` codegen previously
...
4. **cudarc tolerant loader** — vendored cudarc 0.19.2
   ...
   now fall back to a panic-stub — symbols Metrale Engine never calls are fine;
...
Build: `bash /workspace/metrale/run-build.sh` (env: SCALE_HOME,
METRALE_TARGET_HW=strix, METRALE_TARGET_MODEL=qwen3.6-27b,
METRALE_TARGET_QUANT=fp8, CUDARC_CUDA_VERSION=12080, CUDA_HOME +
LIBRARY_PATH → SCALE gfx1151 lib).
Serve: `bash /workspace/metrale/run-serve.sh`.

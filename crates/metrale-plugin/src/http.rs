...
/// asked for". Metrale Engine answers a completion regardless of the `model` field, so
...
pub async fn list_models(target: &TargetEndpoint, timeout: Duration) -> Result<Vec<String>> {
    ...
    // Parse the FIRST value and ignore whatever follows. Metrale Engine replies with
...
/// `GET <path>` and parse the FIRST JSON value of the reply (Metrale Engine answers
...
///
/// This lives in `metrale-plugin` rather than in the server because

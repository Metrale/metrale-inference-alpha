...
impl Qwen3AttentionLayer {
    ...
    pub(super) fn prefill_inner(
        ...
        // METRALE_OP_DUMP hook: pre-input-norm hidden state (input to layer).
        if num_tokens > 0 {
        ...
        // METRALE_OP_DUMP hook: post-input-norm (input to Q/K/V GEMM).
        if num_tokens > 0 {
            super::super::op_dump::dump_bf16(
                ctx.gpu,
                normed,
        ...
        // METRALE_OP_DUMP hook: post-attn-norm output = input to MoE FFN.
        if num_tokens > 0 {
            super::super::op_dump::dump_bf16(
                ctx.gpu,
                ctx.buffers.norm_output(),
        ...
        // HOST-TIME split (METRALE_PREFILL_HOST_TIMING=1): isolate the FFN/MoE
            ...
            && self.pre_moe_norm.is_none()
        {
            ...
            // METRALE_OP_DUMP hook: the SHORTCUT MoE output (zero-experts already
        ...
        // METRALE_OP_DUMP hook: MoE output (sum of all weighted expert outputs).
    ...
    fn prefill_inner_hc(
        ...
        // Bisect taps (METRALE_QWEN4EXP_DUMP): the GDN ladder verifies clean,

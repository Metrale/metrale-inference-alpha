...
//! `METRALE_SSM_DECODE_RING` / `METRALE_DISABLE_WATCHDOGS` contract and the #915
...
///
/// * `METRALE_MTP_POOL_FULL_WIDTH` (presence, house convention — `=0` is NOT
...
/// * `METRALE_EP_PROTOCOL=v2` implies full width: v2 pins slots in place for
///   the worker mirror (no compaction — see `retire_finished_sequences`),
///   so a high slot may legitimately speculate forever.
/// * `METRALE_MTP_MAX_SEQS` participates via [`crate::speculative::mtp_max_seqs`]:
...
/// The `METRALE_MTP_POOL_FULL_WIDTH` kill switch (PRESENCE, house convention —
...
/// under `METRALE_NO_MTP_K_LADDER` (which drops the dispatch cap to 4).
...
/// `METRALE_MTP_POOL_FULL_WIDTH` restores uniform full-K pools and re-enables
/// the lift.
...
/// (with its `METRALE_MTP_K_LADDER` / `METRALE_NO_MTP_K_LADDER` overrides — a
...
/// `METRALE_SSM_MARCONI_FULL` override — so the allocating call site can log
...
/// Env contract (read HERE and nowhere else):
///
/// * `METRALE_SSM_MARCONI_FULL` (PRESENCE, house convention — `=0` is NOT
...
/// The `METRALE_SSM_MARCONI_FULL` kill switch (PRESENCE, house convention).

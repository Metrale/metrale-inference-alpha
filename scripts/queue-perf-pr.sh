...
# 2026-08-28:
#   * METRALE_HOME must be writable. If it is not, every gate fails instantly
...
echo
echo '  # METRALE_HOME must be WRITABLE. The gates read the recipe index and the'
echo '  # same-box baselines from $METRALE_HOME/.., default ~/.metrale. If that'
...
echo '  export METRALE_HOME=${METRALE_HOME:-$HOME/.metrale}'
echo
...
echo "  ./target/release/met benchmark --pull-request-gate-check --pr <N>"
echo

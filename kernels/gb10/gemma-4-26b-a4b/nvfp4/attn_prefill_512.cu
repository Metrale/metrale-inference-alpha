...
// forked-from: kernels/gb10/gemma-4-31b/nvfp4/attn_prefill_512.cu (2026-09-24; 1 of 114 lines differ, see kernels/FORKS.md)

// 2026-09-26: Scalar HDIM=512 prefill attention on contiguous BF16 Q/K/V [batch, seq_len, heads, head_dim],
// FP32 online softmax, causal when `causal` != 0, `sliding_window` ignored. 16 query rows per 128-thread block.
// ops::wide_prefill_kernel falls back to it when attn_prefill_512tc is absent or METRALE_ATTN_512_TC=0.
// Owner: gb10 kernels (gemma-4-26b-a4b). Invariants: none checked; assumes 448 < head_dim <= 512, since a lane whose 64-dim slice starts at or past head_dim returns before the full-warp shuffles, and num_kv_heads | num_q_heads.
#include <cuda_bf16.h>
...
extern "C" __global__ void attn_prefill_512(
    ...
    for (unsigned int kv_pos = 0; kv_pos < kv_len; kv_pos++) {
        ...
        // 2026-09-25: XOR 1, 2, 4 sums the partial dots of the row's 8 lanes (one aligned 8-lane group).

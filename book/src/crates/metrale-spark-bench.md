...
**Role:** the benchmark harness that produces every throughput number in this book. HTTP client that targets a running Metrale Engine Spark server and measures token rate, TTFT, and concurrency behaviour.
**Key file:** `src/lib.rs`.

## Design

This is a **client-side** harness. It does not link against `spark-runtime` or `metrale-kernels` — it just speaks OpenAI-compatible HTTP to a server on `localhost:8888` (or wherever `METRALE_BENCH_URL` points).
...
2. **Same surface as real clients.** If the HTTP stack has a latency bug, `metrale-spark-bench` sees it. If the tokenizer is slow on some prompt shape, it shows up.
...
docker run -d ... metrale/metrale-inference-gb10:latest serve <model>
...
pub fn require_server() -> String {
    ...
    match ureq::get(&format!("{url}/health")).call() {
        ...
        _ => panic!("Server not reachable at {url}. Start Metrale Engine Spark first."),
    }
}
```
...
- Not a generic LLM benchmark tool. It knows about Metrale Engine's server and its SSE streaming format.
- Not a load tester. For that you want `vegeta` or `locust` pointed at the same server.
- Not a kernel micro-benchmark. Those live in `metrale-spark-bench`'s sister tests under each primitive crate's `benches/`, Criterion-driven, no HTTP.

The job of `metrale-spark-bench` is to produce **end-to-end**, **apples-to-apples** numbers that survive the tokenizer, the scheduler, the HTTP layer, and the kernel set. When the README says "131 tok/s on Qwen3.5-35B", that number came from here.

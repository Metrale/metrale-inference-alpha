...
//! 2026-09-26: Tool-prompt tests against the Qwen3.6-35B-A3B-FP8 checkpoint template in
//! test_data/chat_templates: when that template renders the tool schema itself,
//! `parser_tool_prompt` adds no second copy, and `uses_native_qwen_tool_template` follows
//! the template `ChatTokenizer::from_model_dir` selects. No GPU is used.
//!

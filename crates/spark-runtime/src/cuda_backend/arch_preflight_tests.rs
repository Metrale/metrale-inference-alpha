...
///
/// Oracle: `crates/metrale-kernels/build.rs` under `METRALE_SKIP_BUILD=1`
...
/// a fresh `metrale-swap` thread and tears the old model down on the
...
fn a_context_addressed_query_fails_on_a_thread_that_did_not_make_the_host() {
    ...
    // Thread B: the fresh `metrale-swap` thread.
...
///
/// Oracle: `crates/metrale-kernels/build.rs` writes a stub `target_ptx.rs`
/// under `METRALE_SKIP_BUILD=1` whose `all_ptx_sets()` is empty — no arch is
...
/// ★ THE DEFECT, pinned. `metrale_core::device::sm121::NUM_SMS = 48` — a

...
// 2026-09-25: Body of the HDIM=512 paged prefill flash-attention kernel
// (attn_prefill_paged_512.cu): 32 query rows per CTA on 8 warps (256 threads),
// grid (num_q_heads, ceil(q_len / 32), 1).
//
// The prefill_paged_compute.cuh layout at HDIM=512 would need 135,936 B of
// shared memory, and GB10 has 101,376 B per SM
// (metrale_core::device::sm121::SMEM_PER_SM). This one takes 101,120 B of
// dynamic shared memory, with no row padding on Q/K/V and one K buffer:
//   Q    [32][512] BF16   32,768 B
//   K    [32][512] BF16   32,768 B
//   V    [32][512] BF16   32,768 B
//   P    [32][40]  BF16    2,560 B
//   m/l  [32][2]   FP32      256 B
// The launch passes shared_mem(101_120) (ops/prefill_attn_main_b.rs), and the
// kernel registry raises the function's dynamic shared-memory limit for any
// launch above 48 KB (gpu-runtime registry.rs).
//
// Warps 0-1 compute QK^T and the softmax, 16 rows each, while warps 2-7 load
// the V tile. P*V runs on all 8 warps: rows (w & 1) * 16, columns
// (w >> 1) * 128. The next K tile loads after P*V, on all 256 threads.
//
// The including file defines LOAD_KV_TILE_512(cache, bt, smem_ptr, kv_s, kv_l,
// kvh, t, stride), KERNEL_NAME, K_CACHE_TYPE, V_CACHE_TYPE,
...
// strix-hip/common/prefill_paged_compute_512.cuh defines synchronous stand-ins
// with the same names.
...
#define HDIM_512 512
#define PAD_P_512 8
#define N_TILES_PER_WARP_512 16  // 2026-09-25: (HDIM_512 / 8) / 4 column groups
...
extern "C" __global__ void KERNEL_NAME(
    ...
    for (unsigned int kv_block = 0; kv_block < num_kv_blocks; kv_block++) {
        ...
        // 2026-09-25: Warps 2-7 rescale their o to the row max warps 0-1 left in smem_ml.
        if(warp_id>=2){

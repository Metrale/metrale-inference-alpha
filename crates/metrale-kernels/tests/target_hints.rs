...
//! `metrale_core::arch::target_hint` against the tree it claims to describe.
//!
//! `target_hint` answers "which `METRALE_TARGET_HW=` should this operator have
//! built" for a device compute capability, and it is a hand-written `match`.
//! It lives in metrale-core because a shipped binary needs it and metrale-core is
...
//! fails here rather than silently telling operators of that GPU that Metrale Engine
    ...
    for (dir, hw) in hardware_sets() {
        ...
        assert_eq!(
            ...
            "kernels/{dir}/HARDWARE.toml declares compute_capability {cc_text:?}, \
             ...
             add the arm in crates/metrale-core/src/arch.rs"
        );
        checked.push(dir);
    }
    assert!(

...
// Public-API tests for metrale-rdma that need NO RDMA hardware and NO network:
...
/// states: verbs hosts assert `true`, METRALE_SKIP_BUILD/macOS assert `false`.
#[test]
fn verbs_enabled_matches_cfg() {
    assert_eq!(metrale_rdma::verbs_enabled(), cfg!(metrale_rdma_verbs));
}

#[cfg(metrale_rdma_verbs)]
mod with_shim {
    use metrale_rdma::{Gid, MrKeys, Verbs};

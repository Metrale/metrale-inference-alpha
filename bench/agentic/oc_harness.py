...
# 2026-09-26: Sequential agentic harness: opencode against a local server, unittest and Rust webserver tasks.
#
# Owner: bench, agentic probes.
# Invariants: none beyond the types.
"""
Agentic test harness driving OpenCode (`opencode run`) against a locally-served Metrale Engine
...
Prereqs:
  ...
  - Metrale Engine server up on :8888.
Usage:
...
WEBSERVER_PROMPT = (
    ...
    "MUST bind to the port from the METRALE_HARNESS_PORT env var (default 3001) — use "
    "`let port: u16 = std::env::var(\"METRALE_HARNESS_PORT\").unwrap_or_else(|_| \"3001\".to_string()).parse().unwrap();` "
    ...
    "`timeout 5 fuser -k ${METRALE_HARNESS_PORT:-3001}/tcp 2>/dev/null || true`."
)

# 2026-09-26: (name, kind, prompt, verify_spec).
TASKS = [
    ("calc", "unittest",
...
def run_omp(prompt, workdir, timeout, model, thinking=None):
    """Drive `opencode run`. Model is provider-qualified (metrale/<id>)."""
    m = model if "/" in model else f"metrale/{model}"
    ...
    for ln in out.splitlines():
        ...
        try:
            ...
            # 2026-09-26: A line that is not JSON is skipped; the other events are kept.
            pass
...
def verify_webserver(workdir, build_timeout=420, run_timeout=20):
    ...
    if not (wd / "Cargo.toml").exists():
        ...
        return out
    port = _free_port()
    env = {**os.environ, "METRALE_HARNESS_PORT": str(port),
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            try:
                ...
                # 2026-09-26: curl timed out or could not start; keep polling until the
                # deadline. A non-zero curl exit also keeps polling (no break above).
                pass
    finally:
        try:
            ...
            # 2026-09-26: The process group is gone or cannot be signalled; teardown
            # is best-effort.
            pass
    return out
...
def followed_directions(events, workdir):
    ...
    steps = {
        ...
        "reads_port_env": "METRALE_HARNESS_PORT" in src_text,
    }

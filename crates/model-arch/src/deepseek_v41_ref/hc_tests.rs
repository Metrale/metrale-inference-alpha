...
//! 2026-09-25: Layer 0's scaffold against the golden, in the reference's order,
//! so each failure names the first stage that diverged: attention mixes ->
//! attn_in -> (hc_post) -> FFN mixes -> ffn_in -> h_out / pre_mix_out.
//! Attention and MoE outputs are taken from the golden.
//!
...
/// 2026-09-25: Everything the scaffold produces for one regime, computed in
...
fn run(f: &Fx, w: &Layer0, r: &str) -> Run {
    ...
    // 2026-09-25: The FFN collapses with the mix this block's attention produced.
    let ffn_in = rms_norm(
...
fn ffn_mixes_match_through_hc_post() {
    // 2026-09-25: hc_post has no capture of its own; the FFN mixes are computed
    // from its output, so this checks hc_post's comb orientation as much as
    // the mixes.
    let f = fx();
    let w = layer0(&f);
    for r in &f.regimes {
...
fn block_output_and_next_pre_mix_match() {
    ...
    for r in &f.regimes {
        ...
        // 2026-09-25: The block hands `ffn_pre` to the next block as its pre_mix.
        check_capture(

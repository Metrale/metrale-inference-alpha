...
//! 2026-09-26: Tests for the report phase machine in `help_state.rs`, driven by keys through fake workers.
//!
//! Owner: server tui.
...
/// 2026-09-26: Records every spawn and keeps the sender side of each channel for the test.
#[derive(Default)]
struct FakeWorkers {
    device_calls: Arc<Mutex<Vec<String>>>,
    submit_jobs: Arc<Mutex<Vec<(String, String, String)>>>,
...
fn with_logs_off_the_preview_is_skipped_and_no_log_section_ships() {
    let mut r = rig();
    tap(&mut r.state, KeyCode::Char('a'));
    tap(&mut r.state, KeyCode::Char('s'));
    assert!(matches!(r.state.phase, ReportPhase::RequestingCode));
...
fn every_failure_keeps_the_draft_only_created_clears_it() {
    let failures: Vec<Failure> = vec![
        ...
        // 2026-09-26: A dead worker: every sender is dropped.
...
fn a_stopped_worker_settles_into_failed_not_a_stuck_spinner() {
    ...
    tap(&mut r.state, KeyCode::Char('y'));
    drop(last_tx(&r));
    r.workers.txs.lock().unwrap().clear();
    r.state.pump();
...
fn a_mid_submit_token_rotation_does_not_double_post() {
    ...
    send(&mut r, tokens());
    assert_eq!(r.workers.submit_jobs.lock().unwrap().len(), 1);
    // 2026-09-26: A second `Authorized` while Submitting is a refresh rotation from the submit worker.
    send(&mut r, tokens());
    assert!(
...
fn body_editing_owns_every_key_until_esc() {
    let mut r = rig();
    tap(&mut r.state, KeyCode::Char('j'));
    tap(&mut r.state, KeyCode::Enter);
    assert!(r.state.is_editing());
    tap(&mut r.state, KeyCode::Char('s'));
    assert!(matches!(r.state.phase, ReportPhase::Compose));

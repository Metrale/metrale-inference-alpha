...
`kernels/<hw>/` LOOKS like one tree per hardware. It is not. strix holds no kernel of
its own and `use`s 98 of gb10's; strix-hip holds 16 and `use`s 87; b300 holds 4 and
`use`s 185; hopper and b200 are overlays that INHERIT gb10 outright. So an edit to a
gb10 file silently changes what AMD compiles, and the only signal has ever been a
compile failure:

  d584c0c50  `__syncwarp()` added to gb10/common/w4a16_gemv.cu, which strix and
             strix-hip `use`; hipcc rejects it. Caught ONLY because the windows-hip
...
resolves every sharing mechanism the resolver knows (`[sources] use`, `[hardware]
inherits`, `common/` fan-out, `[model] kernel_source` redirects) and says which hardware
...
WHAT IT CANNOT DECIDE, and hands to a reviewer: whether a hunk sits inside a
`#if defined(__SCALE__)` arm the other hardware never compiles; whether a `cfg!(metrale_scale)`
...
HW_SOURCE_EXT = {"b200": ".cu", "b300": ".cu", "gb10": ".cu", "hopper": ".cu", "metal": ".metal",
                 "strix": ".cu", "strix-hip": ".cu"}
...
def build_index(root):
    ...
    Resolution is THE resolver's (`scripts/lib/kernel_layout.py`, the mirror
    of `crates/closure/src/layout.rs`): every layer a target reads —
    its own leaf and common/, and the inherited tree's — plus the transitive
    quoted includes of each module, walked lexically from the directory the
    file is STAGED in (its role directory), which is where nvcc resolves them.
    """
    kroot = root / "kernels"
    ...
    consumers, targets = {}, []
    try:
        all_targets = kl.walk(root)
    except kl.LayoutError as e:
        raise RuntimeError(f"kernels/ does not resolve: {e}") from e
    for hw, model, quant in all_targets:
        t = (hw, model, quant)
        targets.append(t)
        try:
            lay = kl.discover(root, hw, model, quant)
        except kl.LayoutError as e:
            raise RuntimeError(f"{hw}/{model}/{quant}: {e}") from e
        for p in lay.inputs():
            consumers.setdefault(os.path.realpath(p), set()).add(t)
        # Quoted includes, resolved as the compiler resolves them: against the
        # ROLE directory the file is staged beside (same-name headers of that
        # role), then the file's own home for an include that escapes it.
        for role, entries in (("leaf", lay.leaf), ("common", lay.common)):
            staged = {n: e.source for n, e in entries.items()}
            for e in entries.values():
                stack = [(e.source, lay.layers[e.layer].dir)]
                seen = set()
                while stack:
                    cur, home = stack.pop()
                    ...
                    for inc in quoted_includes(cur):
                        if "/" not in inc and inc in staged:
                            stack.append((staged[inc], home))
                            continue
                        cand = Path(home) / inc
                        if cand.exists():
                            stack.append((cand, cand.parent))
        for cfg in [kroot / hw / "HARDWARE.toml", lay.model_dir / "MODEL.toml"] + lay.configs():
            if cfg.exists():
...
def main():
    ...
    for status, rel in rows:
        ...
        # S1: a symlink is never allowed under kernels/ — sharing is declared
        # in KERNEL.toml [sources] use or HARDWARE.toml [hardware] inherits.
        if status in ("A", "T") and ap_.is_symlink():
            structural.append(("S1", rel, f"symlink -> {os.readlink(ap_)}; declare it in [sources] use"))
    ...
    if args.json:
        ...
        if attribution_only:
            ...
            print("      crates/kernels/build.rs in the SAME change (only `vendor` and")
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        (k / "strix" / "common" / "KERNEL.toml").write_text('[sources]\nuse = ["../gb10/common/shared.cu"]\n')
        (k / "gb10" / "common" / "own.cu").write_text("// gb10 only\n")
        for hw in ("b200", "b300", "hopper", "metal", "strix-hip"):
            (k / hw / "common").mkdir(parents=True)
            (k / hw / "HARDWARE.toml").write_text(f'[hardware]\nname="{hw}"\nvendor="nvidia"\narch="y"\n')

...
//! 2026-09-25: Equivalence test for the fused GDN decode + gated RMS norm kernels, at
//! Qwen3.6-35B-A3B GDN dimensions with synthetic batch-4 inputs.
//!
//! Owner: model-arch examples.
//! Invariants:
//! - Returns an error unless every check below holds.
//!
//! 1. `gated_delta_rule_decode_f32_strided_norm` against
//!    `gated_delta_rule_decode_f32_strided` followed by `gated_rms_norm_f32_input`: output
//!    cosine >= 0.99999 and state cosine >= 0.999999.
//! 2. `gated_delta_rule_decode_f32_strided_norm_smem` against `..._norm_half`: output and
//!    final state bit-identical. The _smem kernel stages the rows the _half kernel re-reads
//!    from H in shared memory instead; the arithmetic and its order are the same.
//! 3. The _half output against the leg-1 reference: cosine >= 0.99999.
...
/// 2026-09-25: Index of the first differing byte, or `None` when the two buffers are
/// bit-for-bit equal.
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: The twins must also match the leg-1 reference, or two equally wrong
    // kernels would pass the bitwise check.

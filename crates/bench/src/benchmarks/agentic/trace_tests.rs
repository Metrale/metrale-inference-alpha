...
//! 2026-09-26: Tests for the trajectory file: where it lands, what a turn
//! writes, truncation by a new run, and an unwritable path.
//!
//! Owner: bench, agentic.
...
fn sandbox(name: &str) -> PathBuf {
    let dir = std::env::temp_dir()
        .join(format!("metrale-trace-{}-{name}", std::process::id()))
        .join("run-07");
...
fn an_unwritable_trace_location_does_not_fail_the_run() {
    let missing = Path::new("/proc/metrale-does-not-exist/run-00");

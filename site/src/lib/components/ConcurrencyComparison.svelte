<script module>
  // The comparison chart is the first block of every subject tab: Metrale Engine
  ...
  //   baseline   a vLLM one-shot on the published instrument, no Metrale Engine leg at
  //              it yet (MoE): the vLLM curve and an honest empty Metrale Engine state
...
<script>
  ...
  // -- live, Metrale Engine on the gate instrument + comparable one-shots -------------
  ...
  <figure class="gate-panel cmp">
    ...
    <div class="cmp-keys" role="group" aria-label="Series drawn">
      <button type="button" class="cmp-chip" style="border-color:{color}"
        aria-pressed={shown(metrale.id)} onclick={() => toggle(metrale.id)}
        >Metrale Engine · published campaign · {measuredRange(metrale.rungs)}</button>
      {#each baselines as b}
    ...
    <p class="cmp-caption">
      ...
      not re-measured when Metrale Engine moves. Metrale Engine on this chart is the published campaign run of
      {measuredRange(metrale.rungs)} at <code>{metrale.build}</code>; the live series is in
  ...
  <figure class="gate-panel cmp">
    ...
    <svg viewBox="0 0 {W} {H}" role="img"
      ...
      {#if pts.length}
        ...
        {#each pts as p}
          ...
          >
            <title>Metrale Engine · C={p.c} · {fmtV(p.v)} tok/s{broke} · gate record {fmtDate(live.recorded_at)} · {live.git_sha} · click for record</title>
    ...
    {#if state === 'live' && pair.drawn.length}
      <p class="cmp-caption">
        ...
        when Metrale Engine moves. Metrale Engine is the newest passing run on main ({fmtDate(live.recorded_at)} ·
    ...
    {:else}
      ...
      <div class="cmp-empty">
        ...
        <ul>
          <li>one passing <code>{subject.gate}</code> run for this checkpoint merged to main → the Metrale Engine series</li>

# Metrale Engine Spark: From Zero to 99 tok/s — The Full Story

## What Is Metrale Engine Spark?

Metrale Engine Spark is a **pure Rust LLM inference server** built entirely from scratch for the **Qwen3-Next-80B-A3B-Instruct-NVFP4** model running on a single **NVIDIA DGX Spark GB10** (Blackwell SM121, 119.7 GB LPDDR5X @ 273 GB/s).
...
## Phase 3: Building Metrale Engine Spark from Scratch (Weeks 3–4)

Metrale Engine Spark was a complete rewrite of the inference pipeline in pure Rust.
...
At this point, Metrale Engine Spark at 41 tok/s already exceeded vLLM's 36.4 tok/s non-speculative baseline.
...
|-----------|-------------|-----------|---------------------|
| **Metrale Engine Spark** | **NVFP4** | **99.1 tok/s** | **No** |
...
Metrale Engine Spark at 99 tok/s **without speculative decoding** is 65% faster than vLLM's best result **with** speculative decoding (59.9 tok/s with MTP).
...
**No — speculative decoding has NOT been implemented in Metrale Engine Spark.**
...
The recommended approach for Metrale Engine Spark is **EAGLE-2 or Medusa** with trained draft heads (not n-gram matching):
...
This is the clear next step for Metrale Engine Spark, and it's the one optimization that could deliver another 2x+ improvement.
...
Phase 3: Metrale Engine Spark Infrastructure
  b1aa889  Metrale Engine Spark: pure Rust inference server for Qwen3-Next-80B
...
sudo docker exec -w /workspace/metrale metrale-build bash -c \
  "source /root/.cargo/env && cargo test -p spark-model -p spark-runtime -p metrale-kernels \

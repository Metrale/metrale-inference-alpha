...
//! 2026-09-25: Launch-geometry and guard tests for the 32-row M-tile W8A16 GEMM
//! wrappers on the mock backend: the nine kernel parameters in order, the grid,
//! the contiguous entry's pitches, the by-M selector's edges, and every guard
//! refusing before a launch is recorded. Numerics are checked on a GPU by the
//! `native_fp8_gdn_proj_m32_microtest` example, not here.
//!
...
/// 2026-09-25: The nine kernel parameters, in the order the `.cu` entry point
...
fn strided_launch_carries_both_pitches_and_tiles_m_by_32() {
    let f = Fixture::new();
    // 2026-09-25: 32 rows, N=8192, K=2048, with A rows 2048 and C rows 17408
    // elements apart.
...
/// rather than refusing; when their twin handle is nonzero, the multi-seq
/// attention arms send it every row count above 16.
#[test]
...
/// 2026-09-25: The selector: 1..=32 rows with a nonzero twin handle take the
/// 32-tile (nine args); 33 and more rows take the 128-tile
/// `w8a16_gemm_pipelined` (seven args, its own grid); a zero twin handle or a
/// K that is not a multiple of 128 takes the 128-tile at every width.
#[test]
fn by_m_flips_at_exactly_32_rows_and_on_a_missing_handle() {
    ...
    // 2026-09-25: A K that is not whole scale blocks goes to the 128 tile
    // instead of the twin's guard.
...
/// 2026-09-25: Every guard refuses before a launch is recorded.
#[test]
fn guards_refuse_without_launching() {
    ...
    // 2026-09-25: The contiguous entry inherits the K guard.

...
/// The shape a native-FP8 dense serve boots in with no `METRALE_*` set: FP8
...
fn single_scale_prefill_brings_the_q_and_o_fp8_twins_back() {
    // METRALE_FP8_SINGLE_SCALE clears `fp8_blockscaled_prefill`, which makes the
...
fn the_q_transpose_lever_keeps_only_the_q_twin() {
    // `cache_skip_qkv.rs:142` reads METRALE_ATTN_PREFILL_Q_T per prefill and is

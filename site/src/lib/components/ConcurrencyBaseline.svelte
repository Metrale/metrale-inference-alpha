<script>
  ...
  // OSL 1024, ctx 2048) and no Metrale Engine leg at that instrument yet.
  //
  ...
  const title = $derived(
    `Metrale Engine vs vLLM · published instrument · ISL ${w.isl_tokens} / OSL ${w.osl_tokens} · vLLM one-shot, no Metrale Engine run yet`
  );
</script>
...
<figure class="gate-panel cmp">
  <figcaption class="gate-panel-head">
    ...
    <span class="gate-legend">
      <span class="gl-group">
        <!-- A HOLLOW circle: the Metrale Engine mark is a filled disc, so an empty one
        ...
        <span class="gate-legend-item">
          ...
          </svg>Metrale Engine · no run at this instrument yet
        </span>
      </span>
  ...
  <svg viewBox="0 0 {W} {H}" role="img"
    aria-label="vLLM throughput versus concurrency on the published instrument for {subject.checkpoint}; no Metrale Engine series yet">
    {#each yTicks as t}
  ...
  <div class="cmp-empty">
    <p><strong>No Metrale Engine run at this instrument yet for <code>{subject.checkpoint}</code>.</strong></p>
    <p>
      What fills the comparison: one Metrale Engine leg of <code>{w.harness}</code> at these settings, filed

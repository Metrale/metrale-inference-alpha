...
// 2026-09-25: Kernels `attn_prefill_paged_turbo8` and `_64`: paged prefill flash attention over a turbo8 KV cache.
//
// Each cache block (stride `tq8_bsb` bytes) holds one FP8 E4M3 byte per value, then from byte `tq8_dsb` one BF16 scale
// (2 bytes) per 16 values, as reshape_and_cache_flash_turbo8 writes them. The loader below dequantizes to BF16 while it
// fills shared memory, and the body is prefill_paged_compute.cuh.
//
// Owner: gb10 kernels.
...
#define KERNEL_NAME attn_prefill_paged_turbo8

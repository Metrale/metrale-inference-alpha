...
/// OPT-IN switch for the tail checkpoint (`METRALE_SSM_TAIL_CKPT=1`).
///
...
/// Default-ON switch for MID-CHUNK tail SSM capture (opt-out `METRALE_SSM_TAIL_MIDCHUNK=0`).
///
...
/// clamp-based `METRALE_SSM_TAIL_CKPT` path costs.
...
/// Publishing the clap default sealed this cell on every `met serve`, which
/// made the documented `METRALE_SSM_TAIL_MIDCHUNK=0` opt-out a silent no-op — an
...
/// Publish `met serve --hermetic`. Call once, at serve time.
///
...
/// So absent leaves `METRALE_HERMETIC` live, and passing the flag wins.
...
pub fn ssm_tail_midchunk_enabled() -> bool {
    ...
    // sealing this cell before anything asked, so `METRALE_SSM_TAIL_MIDCHUNK=0`
    // did NOTHING under `met serve` while still being documented as the
    ...
    // `metrale_scale`). "flag-off byte-identical" held; it just was not evidence
...
mod tests {
    ...
    fn an_absent_flag_does_not_seal_the_midchunk_cell() {
        ...
        // with the clap default on every `met serve`, sealing the cell before
        // anything read it — so `METRALE_SSM_TAIL_MIDCHUNK=0` was documented,

...
// 2026-09-25: Append one token's K and V to a contiguous Turbo2 cache at row
// cache_pos. Per group of 16 values: 2-bit indices into a unit-Gaussian
// Lloyd-Max codebook (4 per byte, first element in the low bits) after
// scaling by TURBO2_MAX / max|x|, and an E4M3 scale that makes the
// reconstruction's L2 norm equal the input's. K and V are quantized as given:
// this kernel does not rotate them.
//
// Layout: new_k and new_v are bfloat [num_kv_heads, head_dim]; k_data and
// v_data are uchar [max_seq, num_kv_heads * head_dim / 4]; k_scales and
...
// 2026-09-25: 4-level Lloyd-Max codebook for N(0,1) and its decision boundaries.

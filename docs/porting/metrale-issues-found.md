# Metrale Engine issues found during the Strix Halo (gfx1151 / SCALE) port

Running log of bugs, waste, and design smells uncovered while bringing Metrale Engine up
on AMD Strix Halo. "Metrale-side" = our code; "SCALE-side" = Spectral's compiler/
...
**Where:** `crates/model-arch/src/weight_loader/qwen35_dense.rs`
...
## #S1 — `ld.lld -shared` produced a module SCALE's loader rejects 🔧 Metrale-side
**Where:** `crates/kernels/build_target.rs` (`ScaleTarget::compile`)
...
## #S2 — Stale `common/` kernel silently shadowed the model override 🔧 Metrale-side
**Where:** build kernel-set assembly + `run-build.sh`
`run-build.sh` set `METRALE_TARGET_QUANT=fp8`, but `model_kernel_dir =
...
**Fix:** `METRALE_TARGET_QUANT=nvfp4` (matches GB10 + doc §5).
...
## #S3 — `cuModuleGetFunction` succeeds for a missing kernel 📋 SCALE-side (+ Metrale Engine hardening)
...
This made #S2 hard to localize. Worth (a) reporting to Spectral, and (b) Metrale Engine
...
`limits.d` drop-in (`<user> - memlock unlimited`). Not a Metrale Engine bug, but Metrale Engine
...
## #A2 — `attn_prefill` LDS footprint exceeds RDNA3.5 64 KB cap 📋 Metrale-side **(gfx1151-specific)**
**Where:** `kernels/.../attn_prefill*.cu`; surfaces at runtime as
...
## #A2 — attn_prefill LDS footprint exceeds RDNA3.5 64 KB cap (open, Metrale-side, gfx1151-specific)
Where: kernels/.../attn_prefill*.cu; surfaces at runtime as
"cuFuncSetAttribute(MAX_DYNAMIC_SHARED=69688) failed: CUDA_ERROR_INVALID_VALUE"
at prefill layer 0.
The prefill attention kernel requests ~68 KB dynamic shared memory (Q/K/V/P
...
## #A3 — Generation runs but output is gibberish (open, Metrale/SCALE numerics, gfx1151)
...
1. The e4m3 m16n8k32 -> BF16 MMA rewrite (metrale_mma_e4m3 in w4a16_gemm.cu,
...
`prefill_b/forward_layers.rs` to all layers under METRALE_DUMP_LAYER_NORM):
...
1. Run the same model on GB10 with METRALE_FORCE_GLOBAL_GDN=1 (forces split4 there
...
(METRALE_W4A16_NOPIPE=1, nulls qkvz_nvfp4_t) drops qkvz_q from 167422 -> 3.743
...
- metrale_mma_e4m3 (e4m3->bf16 MMA decomposition): max|cand-cpu|=0.0
...
`sv0=(float)f0*scale2; lo=LUT*sv0`, converts lo->FP8 (metrale_cvt_e4m3x2_f32),
...
Debug env flags left in tree (gated, harmless): METRALE_DUMP_GDN, METRALE_W4A16_NOPIPE,
METRALE_DUMP_LAYER_NORM. Probes in /tmp/modprobe/ (mma_gfx, gnorm, cvtbf16, argabi,
...
- m128 w4a16_gemm_t_m128 (FP8 dequant via metrale_cvt_e4m3x2_f32 -> FP8 MMA via
  metrale_mma_e4m3 decomposition): qkvz_q=167422 WRONG (~45000x).
...
(bf16x4_to_e4m3x4 on smem_A with its stride), the dequant->FP8 (metrale_cvt_e4m3x2_f32
on small ~0.001-0.45 values), the FP8 fragment feeding of metrale_mma_e4m3, or the
transposed weight tiling/accumulation. metrale_mma_e4m3 was only verified on inputs
...
FP8-path line. Also worth trying: the METRALE_FP8_DEQUANT_*_TO_BF16 levers (route to
...
(metrale_cvt_e4m3x2_f32, w4a16_gemm.cu ~line 24, #if __SCALE__) and the A input
...
FIX (next session, two options):
  (A) Replace the broken __nv_cvt_float_to_fp8 in the #if __SCALE__ branch of
      metrale_cvt_e4m3x2_f32 (and any float->fp8 encode) with a CORRECT software
...
DEQUANT_T FP8 store (metrale_cvt_e4m3x2_f32 -> smem_B_fp8) with a BF16 store
(__float2bfloat16(LUT*bs*scale2) -> smem_B_bf16), and COMPUTE_MMA's
bf16x4_to_e4m3x4(A)+metrale_mma_e4m3 with a direct BF16 m16n8k16 MMA (A already
...
gated_delta_rule_prefill_split4 kernel (forced on gfx1151 by #A2 METRALE_FORCE_GLOBAL_GDN
...
3. METRALE_NO_FP8_PREDEQUANT=1 (init.rs predequant_for_prefill early-return): out_proj
...
Debug flags in run-serve.sh: METRALE_W4A16_VARIANT=v1, METRALE_NO_FP8_PREDEQUANT=1,
METRALE_FORCE_GLOBAL_GDN=1 (all needed); dumps gated off.
...
w4a16_gemv_fused, dense_gemv_fp8w, all paged_decode_attn_* + attn_prefill_* +
...
fix when porting those. (3) Debug dumps (METRALE_DUMP_LAYER_NORM/GDN, qkvz_qLAST) still
...
`cargo:rustc-env=METRALE_HW_DISABLE_FP8_SSM_PREFILL`, gated in qwen35_dense.rs via
`option_env!(...).is_none()`.

THE GATE WAS DEAD. `cargo:rustc-env` from metrale-kernels/build.rs is CRATE-LOCAL —
it does not reach metrale-model-layers (which has no build.rs). So `option_env!` in
metrale-model-layers was always None → fp8_ssm_prefill stayed true → the ~5.3 GB FP8 SSM
...
the SAME dead-flag trap the SCALE-port notes warned about for METRALE_HW_FORCE_BR32.

FIX (additive, SSOT/PCND): added crates/model-layers/build.rs — resolves the SAME
HARDWARE.toml (workspace/kernels/<METRALE_TARGET_HW|gb10>/HARDWARE.toml, mirroring
metrale-kernels resolution) and re-emits the `[hardware]` gating flags as
cargo:rustc-env (disable_fp8_ssm_prefill → METRALE_HW_DISABLE_FP8_SSM_PREFILL,
force_br32_prefill → METRALE_HW_FORCE_BR32). Emitted ONLY when the key is present
...
This fix ALSO un-deads METRALE_HW_FORCE_BR32 → gfx1151 prefill now routes to the

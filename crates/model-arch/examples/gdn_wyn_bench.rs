...
//! 2026-09-25: Timing of one `gated_delta_rule_wy5..wy8` launch against K launches of
//! `gated_delta_rule_decode`, each followed by a state copy, for K = 5..=8.
//!
//! Owner: model-arch examples.
//! Invariants: none beyond the types. It times only (30 warm-up, 300 timed iterations per
//! leg) and checks no values.
//!
//! GDN dimensions NK=16, NV=32, KD=VD=128: a 2 MiB FP32 state.
//!
//!   METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b \
//!   cargo run -p metrale-model-arch --release --example gdn_wyn_bench \
//!       --features cuda,gpu-examples
//!
//! The qwen3.6-27b `gated_delta_rule_decode` takes FP32 q/k/v and the wyN kernels take
//! BF16; both read buffers uploaded before timing, so only kernel and copy time is timed.
use anyhow::Result;
use half::bf16;

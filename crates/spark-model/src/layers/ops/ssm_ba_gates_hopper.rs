...
/// `METRALE_SSM_BA_GATES_HOPPER` overriding ([`super::target_defaults`]).
///
/// No `METRALE_NO_*` rung, unlike the GDN families: this lever has no accuracy
/// question behind it, so there is nothing for a kill switch to outrank that
/// `METRALE_SSM_BA_GATES_HOPPER=0` does not already say.

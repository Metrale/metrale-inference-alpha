...
/// `METRALE_LORA_NO_APPLY=1` — keep the adapter RESIDENT but skip every delta.
///
...
/// of one GEMM (`METRALE_LORA_GEMV_MAX_M`, default 8).
///
...
/// `METRALE_LORA_NO_FFN=1` — skip the dense-FFN and GDN-out_proj deltas, keep
/// the attention ones.
///
/// The companion to `METRALE_LORA_NO_APPLY`: that one answers "deltas or

"""
E2E smoke test for Metrale Engine OpenAI API compatibility PR 4 (remaining gaps).
...
Assumes a Metrale Engine server is running on localhost:8888 (set METRALE_URL to
override). The rate-limit test requires the server was started with
METRALE_RATE_LIMIT_RPM=3 so a small burst exhausts the bucket; if not set,
...
BASE = os.environ.get("METRALE_URL", "http://localhost:8888/v1")
MODEL = os.environ.get("METRALE_MODEL", "Qwen/Qwen3.5-35B-A3B-FP8")
...
rpm = os.environ.get("METRALE_RATE_LIMIT_RPM_OBSERVED", "")
if not rpm:
    record(
        "rate-limit 429",
        SKIP,
        "server must start with METRALE_RATE_LIMIT_RPM=3 (small) to trigger",
    )
else:
    try:
        statuses = []

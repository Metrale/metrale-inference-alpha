...
## A layered design for the Metrale Engine gate record pipeline
...
**Phase 0 (bug fixes, `crates/metrale-plugin/src/gate/check.rs`):**
...
- Ship `metrale bench audit <record>`: any GB10 replays k samples via prefill and checks commitments.
...
`met bench certify --with-nodes` runs one campaign across several machines.
...
spreading, from the nodes' live reports (metralectl states facts; it decides

...
//! metrale-rdma: the one-sided RDMA verbs primitive shared by every Metrale Engine RDMA
...
//!
//! `cfg(metrale_rdma_verbs)` is decided by build.rs — ON when `target_os` is not
//! macOS and `METRALE_SKIP_BUILD`/`SKIP_METRALE_BUILD` is unset (there is no
...
//! `DEP_METRALE_RDMA_SHIM_HAS_VERBS`. See build.rs for the full story.
...
/// Compiled only where the shim is (`cfg(metrale_rdma_verbs)`).
...
/// `true` iff this build of metrale-rdma compiled the verbs shim (i.e. the
/// `metrale_rdma_verbs` cfg was emitted by build.rs). A permanent, always-

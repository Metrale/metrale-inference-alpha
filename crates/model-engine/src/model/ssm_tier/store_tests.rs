...
//! 2026-09-25: `MemBlobStore` tests: round trip, misses, size refusal, overwrite
//! accounting and FIFO eviction.
//!
...
fn cap_evicts_fifo() {
    let s = MemBlobStore::new(10);
    s.put(1, &[0x11; 4]).unwrap();
    s.put(2, &[0x22; 4]).unwrap();
    s.put(3, &[0x33; 4]).unwrap();
    assert_eq!(s.len(), 2);
    assert_eq!(s.bytes_resident(), 8);
    assert_eq!(s.stats.evictions.load(Ordering::Relaxed), 1);
    let mut out = [0xa5u8; 4];

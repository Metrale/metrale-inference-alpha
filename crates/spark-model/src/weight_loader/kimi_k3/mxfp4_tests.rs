...
use super::*;
use metrale_core::{config::parse_config, scope::ModelResource};
...
fn packed_loader_marker_binds_local_weights_without_second_allocation() {
    for world in [2, 4, 8] {
        ...
        for name in metrale_core::kimi_k3::weights::required_names(&config) {
            if name.contains(".block_sparse_moe.experts.") {
                continue;
            }
            let (axis, n, k) = tensor_plan(&name, MixerKind::Kda, &config);
            let shape = if axis == metrale_core::kimi_k3::tp::TpAxis::Replicated {
                metrale_core::kimi_k3::weights_geometry::expected_replicated_shape(&name, &config)
                    .unwrap()
            } else {
                vec![n, k]
            };
        ...
        let (dense, meta) = super::super::tp::load_sharded(
            ...
            dense_name,
            MixerKind::Kda,
            metrale_core::kimi_k3::MlpKind::Dense,
            &config,
            &gpu,
        )
        .unwrap();

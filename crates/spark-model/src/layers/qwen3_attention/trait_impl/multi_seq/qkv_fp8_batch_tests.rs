...
struct Case {
    ...
    enabled: bool,
    /// `METRALE_ATTN_NCOL_GEMV` as the layer caches it — injected as a field so
    ...
    ncol_handles: bool,
    /// `METRALE_ATTN_M16_TC` as the layer caches it — a field for the same reason
    /// `ncol` is one: the production accessor is a process-global `OnceLock`.
    /// Round 6 split this from `METRALE_FFN_M16_TC`, which no longer reaches here.
    m16_tc: bool,
...
/// Kill switch: `METRALE_NO_FP8_QKV_BATCH` present ⇒ tier off. Driven through the
...
/// `attn_ncol_gemv::ncol_gemv_enabled`, where `METRALE_NO_ATTN_DECODE_BATCH`

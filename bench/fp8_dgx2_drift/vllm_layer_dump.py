...
# 2026-09-26: Offline vLLM FP8 prefill with forward hooks; writes per-layer last-token residuals and final logits.
#
# Owner: bench, FP8 drift investigation.
# Invariants: none beyond the types.
"""vLLM-FP8 per-layer residual-stream dump for Metrale Engine vs vLLM cosine diff.
...
# 2026-09-26: Set before vllm is imported (the import is inside main); the module
...
OUT_DIR = pathlib.Path("/workspace/metrale-dumps/fp8native_dgx2")
REF_TOKENS = OUT_DIR / "ref_tokens.json"
MODEL = "Qwen/Qwen3.6-35B-A3B-FP8"

# 2026-09-26: Filled by the hooks that register_hooks installs.
...
def find_text_model(model: nn.Module):
    ...
    # 2026-09-26: Fallback: the first submodule, in named_modules order, whose
    # `.layers` is a ModuleList of 30 or more.
...
def register_hooks(model: nn.Module):
    ...
    def make_hook(li):
        def hook(_module, _inp, out):
            # 2026-09-26: A tuple output is taken as (hidden_states, residual) and
            # summed; the module docstring gives the reason.
...
def main() -> None:
    ...
    # 2026-09-26: apply_model may return one result per worker; with
    # tensor_parallel_size=1 the first is used.
    ...
    # 2026-09-26: Best effort: run lm_head on the captured final-norm vector inside
    # the worker; a failure is printed and skipped.
    if "norm" in FINAL_NORM:

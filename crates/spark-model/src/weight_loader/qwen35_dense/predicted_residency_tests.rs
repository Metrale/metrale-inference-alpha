...
use super::*;
use metrale_core::config::{LayerType, ModelConfig, QuantizationConfig};
...
/// The route the round-6 serve booted in: `METRALE_DENSE_FP8=1`, tp 1, a
...
/// `METRALE_ATTN_PREFILL_Q_T=1` adds the Q twin on the W8A8-covered route: the
...
/// `METRALE_NO_GDN_FP8` takes the GDN layers off the fused-concat arm, so the

...
//! Owner: metrale-core.
//! Invariants:
//! - No CUDA and no `cfg`: the module compiles under every feature combination.
...
///
/// OCP FP8 E4M3FN: sign(1) | exponent(4) | mantissa(3), bias 7. There are no
/// infinities; `0x7F` / `0xFF` are NaN and max finite is +/-448.0 (exp 15,
/// mant 6).
///
/// NaN entries decode to signed zero: one bad byte then cannot turn a whole
/// dequantised tensor into NaN.
///
/// Built at compile time, so a decode is one indexed load with no branches.
...
pub static FP8_E4M3_LUT: [f32; 256] = {
    ...
    while i < 256 {
        ...
        let val = if exp == 0 && mantissa == 0 {
            ...
            // 2026-09-25: Subnormal: 2^(-6) * (mantissa / 8).
            (mantissa as f32) * (0.015625f32 / 8.0)
        } else {
            // 2026-09-25: Normal: 2^(exp-7) * (1 + mantissa/8), assembled directly in
...
/// 2026-09-25: Decode one FP8 E4M3 byte to f32 with a single table load.
#[inline(always)]
pub fn fp8_e4m3_to_f32(bits: u8) -> f32 {
    FP8_E4M3_LUT[bits as usize]
}

/// 2026-09-25: Convert f32 to BF16 with IEEE-754 round-to-nearest-even.
///
/// Truncation (`bits >> 16`) is biased toward zero; the tests below pin the
/// cases where the two differ.
///
/// NaN maps to the canonical quiet-NaN pattern with the sign preserved.
///
/// `METRALE_DISABLE_RNE` is a bisect escape hatch that reverts to
/// truncation. It is a presence check, not a value check: `=0` disables RNE
/// just as `=1` does.
///
/// The escape hatch is read once per process. Weight loaders call this once
/// per element (`weight_map::quant_helpers`, `weight_map::fp8_lut`), and a
/// `std::env::var` per call would allocate a `String` and take the
/// process-wide environment lock each time.
///
/// Caching is safe because the test that varies the variable re-execs the
/// test binary as a child process with it set
/// (`disable_rne_presence_uses_truncation` below), so each process resolves
/// it once at its own start. Do not convert that test to an in-process
/// `set_var`.
#[inline(always)]
...
/// 2026-09-25: Widen little-endian BF16 bytes to f32. Exact: BF16 is the
/// top 16 bits of an f32, so this is a shift, never a rounding.
#[inline(always)]
...
mod tests {
    ...
    fn fp8_lut_reference_values() {
        assert_eq!(fp8_e4m3_to_f32(0x00).to_bits(), 0x0000_0000);
        assert_eq!(fp8_e4m3_to_f32(0x80).to_bits(), 0x8000_0000);
        ...
        assert_eq!(fp8_e4m3_to_f32(0x7F).to_bits(), 0x0000_0000);
        assert_eq!(fp8_e4m3_to_f32(0xFF).to_bits(), 0x8000_0000);

        // 2026-09-25: Subnormals: 2^(-6) * mant/8.
        let eps = 1e-10;
    ...
    #[test]
    #[allow(clippy::if_same_then_else)]
    fn fp8_lut_matches_ocp_values_and_metrale_nan_policy_for_all_bytes() {
        // 2026-09-25: Re-derived from the OCP finite-value definition with float
        // math, independently of the table's bit assembly. The two OCP NaN
        // encodings map to signed zero, as `E4M3_LUT_GMOE` does on the device.
        for i in 0u16..256 {
            let bits = i as u8;
            let sign = (bits >> 7) & 1;
            let exp = (bits >> 3) & 0x0F;
    ...
    /// 2026-09-25: The assertions that separate round-to-nearest-even from
    ...
    fn f32_to_bf16_is_rne_byte_exact() {
        ...
        // 2026-09-25: Below half-ULP: round down. Truncation agrees.
        assert_eq!(convert(0x3F80_0800), 0x3F80, "1.0 + below-half-ULP -> 1.0");
        // 2026-09-25: Exactly half-ULP, LSB=0: tie -> round to even (down). Does
        // not distinguish RNE from truncation; kept for the tie coverage.
        assert_eq!(
            convert(0x3F80_8000),
            0x3F80,
            "1.0 + exact-half-ULP, LSB=0 -> 1.0 (even)"
        );
        // 2026-09-25: Above half-ULP: round up. Truncation would give 0x3F80.
        assert_eq!(
            convert(0x3F80_8001),
            0x3F81,
            "1.0 + above-half-ULP -> next bf16 (truncation would give 0x3F80)"
        );
        // 2026-09-25: Exactly half-ULP, LSB=1: tie -> round to even (up).
        // Truncation would give 0x3F81.
        assert_eq!(
            convert(0x3F81_8000),
            0x3F82,
            "1.0078125 + exact-half-ULP, LSB=1 -> 1.015625"
        );
        // 2026-09-25: Negative values round the magnitude the same way.
        ...
        // 2026-09-25: Smallest f32 subnormal (2^-149) is below half-ULP and
        // rounds down to 0.
        ...
        // 2026-09-25: Max-finite f32 rounds up to +inf in bf16.
        assert_eq!(
            convert(0x7F7F_FFFF),
            0x7F80,
            "max-finite f32 rounds to +inf bf16"
        );
        // 2026-09-25: NaN -> canonical quiet NaN, sign preserved; a signalling
        // NaN is quieted rather than passed through.
    ...
    /// 2026-09-25: Byte-exact round-to-nearest-even results for common values,
    /// including cases where truncation gives a different result.
    #[test]
    ...
    fn disable_rne_presence_uses_truncation() {
        let this = format!(
            "{}::disable_rne_presence_uses_truncation",
            module_path!()
                .split_once("::")
                .map_or(module_path!(), |(_, p)| p)
        );
        const CHILD_MARKER: &str = "METRALE_NUMERIC_RNE_CHILD";
        ...
        for value in ["0", "1"] {
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env(CHILD_MARKER, "1")
                .env("METRALE_DISABLE_RNE", value)
                .output()
                .unwrap();
            assert!(
                output.status.success(),
                "METRALE_DISABLE_RNE={value} child failed:\n{}",

...
//! `metrale_core::device::sm121::NUM_SMS` — the GB10 constant, 48 — and picked
//!
//! ```text
...
/// The target's declaration, overridden by `METRALE_ATTN_DECODE_SPLITK`.
///
...
/// can only be settled by an A/B on the box. `METRALE_ATTN_DECODE_GQA_PACK=1`
/// is how that A/B is run — the same mechanism `METRALE_ATTN_DECODE_SPLITK=auto`
...
/// [`DECODE_GQA_PACK_DECLARED`], overridden by `METRALE_ATTN_DECODE_GQA_PACK`.
///

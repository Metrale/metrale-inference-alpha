...
//!
//! Every `METRALE_*` variable below is a process constant: nothing mutates the
...
//!
//! Both were `OnceLock<bool>` statics reading `METRALE_*` at first touch. Two
...
pub struct ModelLevers {
    ...
    /// WY17 GDN recurrence variant. Ships ON; `METRALE_GDN_WY17=0` opts out.
    pub gdn_wy17: bool,
    /// WY-N GDN recurrence variant. Ships ON; `METRALE_GDN_WYN=0` opts out.
    pub gdn_wyn: bool,

    // ── FFN / MoE ──
    /// Lossless single-warp decode GEMV (`w4a16_gemv_sw`, `w4a16_gemv_dual_sw`).
    /// Ships ON; `METRALE_NO_GEMV_SW=1` restores the 64-thread kernels.
    pub gemv_sw: bool,
    /// Route decode FFN through the tile GEMM rather than the scalar GEMV.
    pub decode_ffn_via_gemm: bool,
    /// Small-M FFN GEMM tile shape. Ships ON; `METRALE_FFN_SMALLM=0` opts out.
    ...
    /// `METRALE_FP32_ROUTING=1` — emit the MoE-input norm in FP32 so the gate
    ...
    /// `METRALE_FP32_GATE=1` — the batched-gate sibling of
    /// [`Self::fp32_routing`].
    pub fp32_gate: bool,
    /// `METRALE_FRANKENSTEIN_DECODE_VIA_PREFILL=1` — route the five DFlash
    ...
    /// `METRALE_K2_DIAG=1` — K=2 routed-decode diagnostics.
    pub k2_diag: bool,
    ...
    /// `METRALE_NO_DECODE_SPLIT_SILU` (presence) restores the fused kernel.
    ...
    /// `METRALE_BF16_TC_PREFILL` (presence) — BF16 tensor-core prefill GEMM.
    ...
    /// `METRALE_FP8_M64_PREFILL` (presence) — m16n8k32 e4m3 M64 prefill GEMM,
    /// ~1.47x vs v2 BF16. Lossy (cosine 0.9997), so opt-in only.
    pub fp8_m64_prefill: bool,
    /// `METRALE_INT8_PREFILL` (presence) — requant→`int8_gemm_faith2` prefill
    /// (cosine 0.999978 vs the host full-precision dequant GEMM).
    pub int8_prefill: bool,
    /// `METRALE_INT8_FAITH5` (presence) — int32 per-sub-block accumulation,
    ...
    /// `METRALE_NO_FFN_NVFP4_MMQ` (presence) is the kill switch.
    pub ffn_nvfp4_mmq: bool,
    /// The same MMQ arm for the down projection — t_m128 runs the narrow-N
    /// down at only ~34 TFLOP/s. Ships ON; `METRALE_NO_FFN_NVFP4_MMQ_DOWN`
    ...
    /// `METRALE_FFN_MMQ` (presence) — Q4_K MMQ prefill arm.
    pub ffn_mmq: bool,
    /// `METRALE_FFN_MMQ_DOWN_Q4K` (presence) — keep the down projection ON
    ...
    /// `METRALE_FP4_PREFILL` (presence) — native W4A4 FP4 tensor cores
    ...
    /// Ships ON; `METRALE_DISABLE_PREFILL_V2` (presence) forces v1 so the two
    ...
    /// `METRALE_HOLO_MOE_GROUPED_CUTLASS=1` — single-launch CUTLASS grouped
    ...
    /// `METRALE_HOLO_MOE_GROUPED_DOWN=1` — take the down projection through
    ...
    /// `METRALE_MOE_PREFILL_EXACT_TILES=1|0` overrides the tile bound;
    ...
    /// `METRALE_MOE_PREFILL_MAX_LOAD_FACTOR=<n>` — cap the per-expert tile
    ...
    /// `METRALE_MOE_PREFILL_ZERO=1` — memset the grouped scratch before
    ...
    /// `METRALE_MOE_PREFILL_FP8_DOWN=1` — FP8 grouped GEMM for the routed
    /// down projection.
    ...
    /// `METRALE_NO_SSM_W4A4` (presence) is the kill switch.
    pub ssm_w4a4: bool,
    /// The chunked SSD scan. Ships ON; `METRALE_NO_SSD` (presence) falls back
    ...
    /// `METRALE_NO_SSM_PERSISTENT` (presence) disables, for a same-binary A/B
    /// against the sequential scan.
    pub ssm_persistent: bool,
    /// Zero the grouped-MoE intermediate arena buffers before dispatch.
    /// Ships ON; `METRALE_MOE_NO_ZERO_INTERMEDIATES` (presence) skips.
    ///
    ...
    /// `METRALE_MOE_MAX_M_TILES_ESTIMATE` (presence) — restore the old
    ...
    /// `METRALE_MOE_W4A4` (presence) — W4A4 grouped up-projection at N >= 512.
    pub moe_w4a4: bool,
    /// W4A4 for the shared-expert UP projection at N >= 512. Ships ON;
    /// `METRALE_NO_SHARED_W4A4` (presence) is the kill switch.
    pub shared_w4a4: bool,
    /// `METRALE_SHARED_W4A4_DOWN` (presence) — the DOWN half of the same, and
    ...
    /// `METRALE_DFLASH_DEBUG_DUMP_FULL=1` — the model-side half of the DFlash
    ...
    /// `METRALE_DSPARK_ANCHOR_BIAS` came to have two implementations.
    pub dflash_debug_dump_full: bool,
    /// `METRALE_MTP_DEBUG_NORMS=1` — per-stage norm dumps inside the MTP
    ...
    /// `METRALE_SSM_SAVE_DUMP` (presence) — the CBD scratch/SSM-state
    ...
    /// `METRALE_MLA_PERSEQ_FALLBACK=1|true` — route MLA batches through the
    /// per-sequence path instead of the batched one.
    pub mla_perseq_fallback: bool,
    /// `METRALE_HC_PERSEQ_DECODE=1` — per-sequence hyper-connection decode.
    ...
    /// `METRALE_DECODE_BATCH_LOG=1` — log the batch's slot/position vectors
    /// each step.
    pub decode_batch_log: bool,
    /// `METRALE_MS_PROFILE=1` — per-phase multi-seq profiling, which forces
    /// eager execution so the per-phase syncs are legal under capture.
    ///
    /// NOT [`Self::ssm_ms_profile`], which is `METRALE_SSM_MS_PROFILE`. Two
    /// different variables one underscore apart, both live.
    pub ms_profile: bool,
    /// `METRALE_CONC_HSD=1|true` — per-sequence hidden-state dump, to localize
    /// where `pos >= 1` diverges from `pos 0` in concurrent batched decode.
    pub conc_hsd: bool,

    // ── Decode graph capture ──
    /// `METRALE_EP_GRAPHS=1|true` — allow CUDA-graph capture under expert
    ...
    pub ep_graphs: bool,
    /// `METRALE_GDN_DECODE_GRAPH=1|true` — capture the whole single-token GDN
    ...
    /// kernel is resolved and n <= 16. `METRALE_SSM_GEMV_BATCH4=0` opts out.
    ...
    /// `METRALE_MTP_SHADOW_TOPK=k` (0 = off, clamped to 8): the drafter D2Hs
    /// its logits and logs the top-k candidates. Observational only.
    pub shadow_topk: usize,
    /// `METRALE_KV_POISON=1` — fill a fresh KV block with NaN instead of zero,
    ...
    pub kv_poison: bool,
    /// MTP drafter context policy (`METRALE_NO_DRAFTER_CONTEXT` /
    /// `METRALE_DRAFTER_PREFILL_ONLY`), resolved and logged once per model.

...
//! 2026-09-26: Tests for `BenchState` selection, parameter validation, the pre-flight and the run-pane bookkeeping.
//!
//! Owner: server tui.
...
/// 2026-09-26: The first benchmark's form, pointed at an endpoint nothing is listening on, with no executor.
fn state() -> BenchState {
...
fn selecting_past_the_end_of_the_registry_lands_on_the_last_benchmark() {
    let mut s = state();
    s.select(usize::MAX);
    assert_eq!(s.selected, metrale_bench::registry::all().len() - 1);
...
fn an_empty_value_is_refused_for_every_parameter_of_every_benchmark() {
    // 2026-09-26: An empty field is an error, never a fallback to the default.
    let mut s = state();
    for i in 0..metrale_bench::registry::all().len() {
        s.select(i);
...
fn a_number_outside_its_domain_names_the_bound_it_broke() {
    ...
    let row = s
        .specs
        .iter()
        .position(|spec| matches!(spec.kind, metrale_bench::ParamKind::Int { .. }))
...
fn a_row_that_does_not_exist_is_answered_rather_than_panicking() {
    // 2026-09-26: The row accessors take a raw index and answer for any value.
    let s = state();
...
fn a_start_with_the_probe_switched_off_still_refuses_for_the_real_reason() {
    // 2026-09-26: `begin_start` with `Skip` is `start`, so the refusal is `start`'s.
    let mut s = state();
    s.coherence = metrale_bench::CoherencePolicy::Skip;
...
fn a_doomed_run_never_reaches_the_endpoint_check() {
    // 2026-09-26: An invalid form is refused before any endpoint check opens.
    let mut s = state();
    s.edit[0] = "nonsense".into();
    s.commit_row(0);
    let err = s.begin_start().unwrap_err();
...
fn the_run_log_is_a_window_that_drops_its_oldest_lines() {
    // 2026-09-26: The run log keeps the newest `LOG_CAPACITY` lines.
    let mut s = state();
...
fn the_detail_pane_has_something_to_say_before_anything_is_selected() {
    // 2026-09-26: Before `attach` has run, `meta` is unset and the fallback metadata is shown.
...
fn a_selected_benchmark_reports_its_own_provenance() {
    ...
    for i in 0..metrale_bench::registry::all().len() {
        ...
        assert!(
            meta.official,
            "everything in the registry ships with Metrale Engine"
        );

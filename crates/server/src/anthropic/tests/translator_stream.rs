...
//! 2026-09-26: Event-sequence tests for the Anthropic streaming translator.
//!
...
fn text_stream_framing() {
    ...
    // 2026-09-26: The input count arrives only with the Finish delta, so
    // `message_delta` reports it.
...
fn multi_tool_calls_close_and_reopen_blocks() {
    // 2026-09-26: The second start closes the first tool block and opens a
    // new one at the next index.
    let evs = drive(vec![
        StreamDelta::ToolCallStart {
            index: 0,
            id: "c1".into(),
...
fn finalize_without_finish_reports_truncation_not_end_turn() {
    // 2026-09-26: No Finish delta arrived. `finalize` closes the block and
    // the message with `max_tokens`, the reason `convert_stop_reason` also
    // gives a deadline cut, because the turn is incomplete.
    let mut t = AnthropicTranslator::new("m".to_string());
    let mut out = Vec::new();
    t.on_delta(
        &StreamDelta::Content {
...
/// 2026-09-26: A copy of the payload `api::chat_stream::handle_error` puts on
/// `StreamDelta::Error`.
...
fn stream_error_terminates_the_turn_as_an_error_not_a_success() {
    // 2026-09-26: A failure after content was sent. The HTTP 200 is already
    // committed, so the terminal event is the client's only failure signal:
    // it must be `error`, with no success framing after it.
    let evs = drive(vec![
        StreamDelta::Content {
            text: "partial".into(),
            token_ids: Vec::new(),

...
use anyhow::Result;
use metrale_core::config::LayerType;
...
/// `METRALE_NO_MTP_VERIFY_GRAPHS` (house convention — `=0` is NOT off).
...
/// Per-layer stream-sync diagnostic (`METRALE_K4_DIAG=1`, VALUE check — this
...
/// Verify argmax D2H arm: `METRALE_VERIFY_D2H_DEFAULT_STREAM=1` restores the
...
/// Verify argmax D2H arm: `METRALE_NO_PINNED_VERIFY_D2H=1` forces the pageable
...
/// `METRALE_NO_VERIFY_WY_CACHE` (house convention — `=0` is NOT off).
...
pub(super) enum VerifyGraphOutcome {
    ...
    Capture,
    /// No graph at all: `METRALE_NO_MTP_VERIFY_GRAPHS`, `METRALE_K4_DIAG`, or a
    /// batch with a slotless sequence.
    Eager,
}

/// Periodic INFO summary of the batched-verify graph outcomes, under the
/// existing `METRALE_MTP_ACCEPT_DEBUG` gate (checked FIRST, so a default serve
...
impl TransformerModel {
    ...
    /// `METRALE_NO_CANONICAL_VERIFY_KEY` (scheduler side) restores the
    ...
    /// sentinel is in the CUDA-graph key — and `METRALE_NO_VERIFY_WY_CACHE`

...
fn loads_single_tensor_cpu_fallback() {
    ...
    unsafe { std::env::set_var("METRALE_GGUF_FORCE_CPU", "1") };

    let vals = [1.0f32, -2.0, 3.5, 0.0, 7.0, -0.25];
    let bytes = build_single_f32_gguf("token_embd.weight", &vals);

    let dir = std::env::temp_dir().join(format!("metrale_gguf_test_{}", std::process::id()));
    ...
    unsafe { std::env::remove_var("METRALE_GGUF_FORCE_CPU") };
}

#[test]
fn find_gguf_picks_first() {
    let dir = std::env::temp_dir().join(format!("metrale_gguf_find_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn find_gguf_skips_mmproj_and_find_mmproj_pairs() {
    let dir = std::env::temp_dir().join(format!("metrale_gguf_mmproj_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    ...
    let dir2 = std::env::temp_dir().join(format!("metrale_gguf_textonly_{}", std::process::id()));

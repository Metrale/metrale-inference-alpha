...
// Metrale Engine DENSE BF16 tensor-core decode GEMM — 16-row M tile.
//
//   C[M,N] = A[M,K] (BF16) * B[N,K]^T (BF16),  1 <= M <= 16
//
// WHY (#927/#928, 1xH100, nsys round 7, 2026-09-11, Qwen/Qwen3.8-27B-FP8 with
// `--lm-head-dtype bf16` and `METRALE_LM_HEAD_BATCHM_MAX=16`, decode batch 16).
...
// `METRALE_LM_HEAD_M16_TC` and OFF by default until an H100 receipt says it wins.
...
// 64 (`dense_gemm_m16_bf16_n64`, opt-in via `METRALE_LM_HEAD_M16_TC_NTILE=64`),
...
// (`METRALE_LM_HEAD_M16_TC_NTILE=64`). Both must be a multiple of 32 so each of
...
/// `ceil(N/64)`.
///
/// Opt-in (`METRALE_LM_HEAD_M16_TC_NTILE=64`) and NOT the default: it is the A/B

...
// 2026-09-25: Kernels `paged_decode_attn`, `paged_decode_attn_splitk` and `paged_decode_attn_reduce`: decode attention
// over a BF16 paged K/V cache in NHD layout, [num_blocks, block_size, num_kv_heads, head_dim], read through a
// per-sequence block table.
//
// `paged_decode_attn`: one CTA of 8 warps per (q_head, seq). Each lane holds VEC_BF16 = HDIM / 32 elements of the
// head (HDIM is 256 unless the build defines it). Each warp takes a contiguous chunk of the attended positions;
// within each physical block it processes BC positions at a time and the rest singly, then the warps merge their
// online-softmax states through shared memory. Grid (num_q_heads, num_seqs, 1).
// The split-K pair is described above `paged_decode_attn_splitk`.
//
// Owner: gb10 kernels.
// Invariants: none beyond the types. All three assume head_dim == HDIM, and the first two blockDim.x == 256
// (NUM_WARPS * WARP_SIZE); none checks it.
...
#define NUM_WARPS 8
#define BC 4            // 2026-09-25: KV positions batched per loop iteration
...
// 2026-09-25: Address of (pos, kv_head) in an NHD paged cache. Nothing in this file calls it.
...
extern "C" __global__ void paged_decode_attn(
    ...
    // 2026-09-25: Positions are processed BC at a time within one physical block, and the tail of each block run
    // singly.

    unsigned int pos = my_start;

    while (pos < my_end) {
        // 2026-09-25: batch_count runs from pos to the end of its block or of this warp's chunk, whichever is first.
        unsigned int logical_block = pos / block_size;
        unsigned int block_offset = pos % block_size;
...
// (q_head, split, seq), grid (num_q_heads, num_splits, num_seqs). It writes each range's unnormalised output
// and its (m, l) to `workspace`; `paged_decode_attn_reduce` merges the splits. It takes no sliding window.
...
extern "C" __global__ void paged_decode_attn_splitk(
    ...
    const __nv_bfloat16* __restrict__ K_cache,
    const __nv_bfloat16* __restrict__ V_cache,
    float* __restrict__ workspace,               // 2026-09-25: [num_seqs, num_q_heads, num_splits, head_dim + 2]: o, then m, then l
...
// 2026-09-25: Merges one (q_head, seq)'s num_splits partials, skipping splits with l == 0, and writes BF16 O.

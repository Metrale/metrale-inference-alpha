...
// ~20 realistic Metrale-flavored chunks (Rust engine + CUDA kernels + docs).
...
fn encode_e2m1(x: f32) -> u8 {
  ...
  ['src/engine/mod.rs', 'rust', 1, `//! Metrale Engine core: request lifecycle from tokenize -> prefill -> decode
...
pub fn sample_batch(logits: &Tensor, params: &[SamplingParams]) -> Vec<TokenId> {
  ...
  ['docs/architecture.md', 'markdown', 1, `# Metrale Engine architecture

Metrale Engine is a single-node inference engine for DGX Spark (GB10). The serving

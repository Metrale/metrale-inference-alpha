...
# 2026-09-25: Pure-torch stand-ins for the reference's tilelang kernels,
# imported as `kernel` by gen_ds41_tiny_golden.py.
#
...
_FP8_MAX = 448.0
_FP4_MAX = 6.0
# 2026-09-25: The e2m1 values in code order (the index is the 3-bit code);
# `_to_e2m1_rne` rounds ties to the even code.
...
def _to_e2m1_rne(y: torch.Tensor) -> torch.Tensor:
    ...
    # 2026-09-25: Index of the largest grid value <= a.
    ...
    pick_hi = dhi < dlo
    tie = dhi == dlo
    # 2026-09-25: On a tie choose the even code.
...
def sparse_attn(q, kv, attn_sink, topk_idxs, softmax_scale):
    ...
    g = kvf[torch.arange(b, device=kv.device)[:, None, None], safe]

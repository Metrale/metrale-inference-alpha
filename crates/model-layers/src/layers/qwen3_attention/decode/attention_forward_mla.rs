...
//! 2026-09-25: Absorbed-MLA single-token decode, and the argument struct it shares with the V4
//! path. The chain: Q latent, norm, expand; absorbed Q; Q rope scatter; KV latent and norm; K rope
//! and RoPE; cache assemble and write; paged decode attention; V extraction; O projection.
//!
...
pub(in crate::layers::qwen3_attention) struct DecodeMlaArgs {
    ...
    pub bs: usize,
    pub stream: u64,
    /// 2026-09-25: Host-side position for the V4 compressed-pool decode append. `attention_forward`
    /// passes `Some(seq_len - 1)`; the multi-sequence path (`trait_impl::multi_seq::mla`) passes
    /// `None`, which skips the append. The absorbed-MLA chain ignores it.
    pub pos: Option<u32>,
}

impl Qwen3AttentionLayer {
    /// 2026-09-25: Run the absorbed MLA decode chain, returning the O-projection output
    ...
    pub(super) fn attention_forward_mla(
        ...
        // 2026-09-25: Absorbed Q, by one batched GEMV when loaded, else one GEMV per head.
        ...
        // 2026-09-25: K rope projection, RoPE on the Q and K rope parts, then the Q rope writeback.

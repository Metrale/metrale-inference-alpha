...
//!
//! Kill switch `METRALE_NO_VERIFY_GDN_BATCH` (PRESENCE check per the house
...
/// `METRALE_MTP_ACCEPT_DEBUG` gate (checked FIRST — a default serve pays one
...
impl Qwen3SsmLayer {
    ...
    pub(super) fn decode_batched_conv_gdn_multi(
        ...
        let wy_k = match kk {
            ...
            // None (out of range / module absent / METRALE_GDN_WYN=0 / f16
        ...
        // METRALE_SSM_H_FP16: a zero handle below turns into `Ok(false)` and the

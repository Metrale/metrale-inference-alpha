...
// 2026-09-25: GPU Q4_K weight quantizer: BF16 [nrows, n_per_row] -> block_q4_K rows (q4k_vendor/ggml-common.h),
// the layout q4k_mmq.cu reads. The scale search is ported from llama.cpp's Q4_K quantizer without an importance
// matrix: weights av_x + |x|, make_qkx3 per 32 values, then make_qp over the 8 scales and the 8 mins.
// Owner: gb10 kernels (qwen3.6-27b).
// Invariants: one thread per 256-weight superblock; columns past the last full 256 are not quantized.
#include <cuda_bf16.h>
#include <cstdint>

#define QK_K 256
struct metrale_bq4k { uint16_t d; uint16_t dmin; uint8_t scales[12]; uint8_t qs[128]; };
...
// 2026-09-25: Weighted fit x ~ scale * L - *the_min with L in [0, nmax] and min <= 0; returns scale.
...
// 2026-09-25: Weighted fit x ~ scale * L with L <= nmax, for non-negative x; returns scale.
...
extern "C" __global__ void q4k_quantize(const __nv_bfloat16* __restrict__ x, metrale_bq4k* __restrict__ y,
    ...
    // 2026-09-25: Re-derive L from the packed 6-bit scales and mins.
    for (int j=0;j<8;++j){

...
//! `~/.metrale/artifacts/vision` — the fixture images, laid down on `load()`.
//!
//! Follows the BFCL provisioning pattern (`benchmarks/bfcl/provision.rs`):
//! assets are compiled into the binary from `crates/metrale-plugin/assets/`,
...
///
/// ★ METRALE APPLIES THE TAG, so the pair must DISAGREE: rotating 90 CW carries
...
/// `~/.metrale/artifacts/vision`, matching BFCL's `~/.metrale/artifacts/bfcl`.

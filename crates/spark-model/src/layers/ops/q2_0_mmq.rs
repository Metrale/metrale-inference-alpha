...
// entries `metrale_q2_0_mmq128_nc/_wc`). Keeps the 2-bit weight PACKED and does
...
// `super::quantize_act_q8_1` (kernel `metrale_q8_1_quantize_ds4_bf16`, DS4 layout)
...
/// Sub-flag gating the native Q2_0 MMQ prefill path (`METRALE_GGUF_NATIVE_Q2_MMQ=1`).
/// Default off: keep the transient-dequant stopgap so the two can be A/B'd on GPU.
/// (`METRALE_GGUF_NATIVE_Q2` still gates keep-packing overall — this only chooses
...
pub fn q2_0_mmq_gemm(
    gpu: &dyn GpuBackend,
    kernel_nc: KernelHandle, // metrale_q2_0_mmq128_nc
    kernel_wc: KernelHandle, // metrale_q2_0_mmq128_wc

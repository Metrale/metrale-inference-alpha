...
//! 2026-09-25: End-to-end greedy inference of Qwen3.5-4B (MLX 8-bit) on the Metal backend:
//! tokenize, embed, run every decoder layer `config.json` lists through
//! `metrale_model_layers::forward::qwen3_5`, final RMSNorm, LM head through the tied
//! `embed_tokens` weight, argmax, detokenize.
//!
//! Owner: model-arch examples (Metal Qwen3.5 driver).
//! Invariants: none beyond the types.
//!
//! The per-layer launches live in `forward::qwen3_5`; this driver loads the weights, resolves
//! the kernels, allocates scratch, KV caches and GDN states, and runs the token loop. The model
//! directory is `METRALE_MLX_MODEL_DIR`, the prompt `PROMPT`, and the decode length
//! `METRALE_DECODE_TOKENS`.
//!
//! Run with:
//!
...
//!     cargo run --release -p metrale-model-arch --example metal_qwen35_inference \
...
fn main() -> Result<()> {
    ...
    println!("=== Metrale Engine Metal · Qwen3.5-4B-MLX-8bit inference ===");
    ...
    if modules.is_empty() {
        bail!(
            "metal kernel registry empty — re-build with \
             METRALE_TARGET_HW=metal METRALE_TARGET_MODEL=qwen3-5-4b-vlm-mlx-int8 \
             METRALE_TARGET_QUANT=mlx_int8"
        );
    }
    ...
    let n_decode_budget: u32 = std::env::var("METRALE_DECODE_TOKENS")
        .ok()
    ...
    // 2026-09-25: `METRALE_KV_DTYPE` picks the full-attention KV cache format, one of the
    // `MetalKvDtype` names; unset means bf16.
    let kv_dtype: qwen3_5::MetalKvDtype = std::env::var("METRALE_KV_DTYPE")
    ...
    let lin_states: Vec<LinearAttentionState> = (0..lin_attn_count)
        ...
        .collect::<Result<_>>()?;
    // 2026-09-25: Layer index to KV-cache or GDN-state slot.
    ...
    // 2026-09-25: RoPE inverse frequencies: `rotary_dim / 2` entries of
    // 1 / theta^(2i / rotary_dim).
    ...
    // 2026-09-25: Rewritten before every token with its absolute position.
    let positions_ptr = backend.alloc(4)?;

    // 2026-09-25: Dequantise `embed_tokens` once to a BF16 [vocab, hidden] table (1.27 GB)
    // for `embed_lookup`. The LM head reads the int8 weight through `embed_tokens.gemv`.
    println!();
    ...
    let n_decode: usize = std::env::var("METRALE_DECODE_TOKENS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(20);
    println!();
    ...
    for step in 0..n_decode {
        ...
        // 2026-09-25: A greedy run stops at token 248044; a teacher-forced run ignores it, so
        // it covers its whole list.
    ...
    println!(
        "All 32 layers fired (8 full_attention + 24 linear_attention via GDN). \
         Per-layer orchestration runs through `metrale_model_layers::forward::qwen3_5` — \

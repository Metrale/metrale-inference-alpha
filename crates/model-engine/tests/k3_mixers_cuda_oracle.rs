...
//! 2026-09-25: Kimi K3 KDA and MLA decode at TP8 head counts on CUDA, against the
//! CPU implementations in `metrale_model_weights::kimi_k3_host`.
//!
//! Owner: model-engine tests.
//! Invariants: none beyond the types.
//!
//! Inputs are synthetic projected vectors, not weights. A cached prefix is
//! handed to decode; no fused prefill runs. Both sides return FP32 and must
//! agree within 2e-6 absolute + 2e-5 relative (`close`), far below BF16
//! rounding.
//! Build with METRALE_TARGET_MODEL=kimi-k3 METRALE_TARGET_QUANT=mxfp4 and matching
//! METRALE_TARGET_HW. On an idle GPU, keep those variables and run:
...
use metrale_model_arch::kimi_k3::mla_cuda::{
...
fn mla_oracle(gpu: &dyn GpuBackend, stream: u64, cfg: &MlaConfig, theta: f32) -> Result<()> {
    ...
    // 2026-09-25: A 31-token NoPE prefix built on the host, then decode at
    // positions 31 to 36.
    for pos in 0..31 {
...
fn k3_tp8_mixers_match_core_cpu() -> Result<()> {
    ...
    let mut config = metrale_config::parse_config(include_str!(

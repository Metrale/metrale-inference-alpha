...
//!
//! ```text
//! HOME=/path/to/writable cargo test -p metrale-plugin --test kat_equality_e2e -- --ignored
//! ```
...
use metrale_plugin::{ArtifactStore, ParamValues, TargetEndpoint, VerdictKind};
...
async fn run_against(port: u16) -> metrale_plugin::RunRecord {
    run_with_orders(port, "2").await
}

async fn run_with_orders(port: u16, orders: &str) -> metrale_plugin::RunRecord {
    ...
    let descriptor = metrale_plugin::registry::find("kat-equality-gate").expect("registered");
    ...
    tokio::task::spawn_blocking(move || {
        metrale_plugin::headless::run_blocking(
            ...
            metrale_plugin::headless::RunRequest {
                ...
                values,
                target,
                options: metrale_plugin::headless::HeadlessOptions::cli("test"),
            },

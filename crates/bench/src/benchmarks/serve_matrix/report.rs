...
//! 2026-09-26: Rendering the matrix: one table row per roster entry, one tile row
//! of counts, the verdict, and the per-round log line.
//!
//! Owner: bench, serve matrix.
//! Invariants:
//! - The table has a row for every round of the plan, skipped and excluded
//!   ones included.
...
/// 2026-09-26: The full matrix, including the rounds that were skipped and why.
...
fn result_row(label: &str, r: &RoundResult) -> Vec<Cell> {
    ...
    vec![
        Cell::new(label),
        // 2026-09-26: Whether the endpoint served this round's checkpoint.
...
pub fn summary(tally: &Tally) -> Vec<Stat> {
    vec![
        ...
        // 2026-09-26: 0/0 is "nothing was measured", which the verdict renders
        // as Info, so the tile is dimmed rather than red.
...
/// 2026-09-26: PASS/FAIL, or `Info` when no round was planned.
...
/// 2026-09-26: The per-round log line: enough to attribute a failure without the table.
    ...
    match &r.outcome {
        ...
        Outcome::Probed(s) => {
            ...
            if !bars.is_empty() {
                ...
                // 2026-09-26: The detail of every failed signal, so the line
                // stands alone.

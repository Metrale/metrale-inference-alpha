...
//! 2026-09-26: Tests for the quick speed probe: defaults, prompts, the request
//! body, the averages and the summary.
//!
...
fn a_fixture_isl_loads_the_committed_text_and_any_other_synthesizes() {
    ...
    for (isl, digest) in expected {
        ...
        // 2026-09-26: Fixtures are natural text, not the filler corpus.
        assert!(
    ...
    // 2026-09-26: The same ISL always yields the same bytes: the warm-path
    // premise.
...
fn averages_are_computed_from_the_server_timings() {
    ...
    // 2026-09-26: TPOT is derived from the server decode rate, per run then
    // averaged: (1000/50 + 1000/70) / 2.
    let want_tpot = (1000.0 / 50.0 + 1000.0 / 70.0) / 2.0;
    assert!((avg.server_tpot_ms.unwrap() - want_tpot).abs() < 1e-9);
    // 2026-09-26: Client E2E rate includes prefill: 128 tok / 4 s = 32 tok/s,
    // lower than the 60 tok/s decode rate.
    assert_eq!(avg.client_e2e_tok_s, Some(32.0));
    assert_eq!(avg.output_tokens, Some(128.0));
    assert_eq!(avg.e2e_ms, Some(4000.0));
}

/// 2026-09-26: No server timing gives no decode rate and no TPOT, never a
/// client-clock substitute.
#[test]
fn without_server_timings_no_decode_rate_or_tpot_is_fabricated() {
    ...
    // 2026-09-26: The client-side numbers are still reported.
    assert_eq!(avg.client_e2e_tok_s, Some(32.0));

    // 2026-09-26: The metrics map omits the absent keys rather than writing
    // 0.0.
    let mut b = configured();
    ...
    // 2026-09-26: A mixed set averages only the runs that reported.
    let mixed = [
...
/// 2026-09-26: EOS before the OSL cap is a data point, not an error: the
/// arithmetic uses the tokens actually produced, and the summary names the cap
...
fn the_headline_stat_is_the_server_decode_rate_and_says_so() {
    ...
    // 2026-09-26: The client rate's label says it is the client rate.
...
/// 2026-09-26: Registered, but not a required PR gate. `coverage_map_tests`
/// requires its `NOT_REQUIRED` entry; this test pins the descriptor's
/// gate-free shape.
#[test]

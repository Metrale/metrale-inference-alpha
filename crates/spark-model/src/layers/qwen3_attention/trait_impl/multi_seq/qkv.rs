...
//! - n>=2 + native FP8 → strided batched GEMV, M32 tile past 16 (`qkv_fp8_batch`)
...
/// Kill switch: `METRALE_NO_FUSED_QKV=1` restores the three separate GEMMs.
...
impl Qwen3AttentionLayer {
    ...
    fn ms_qkv_norms(&self, c: &MultiSeqCtx<'_>) -> Result<()> {
        ...
        // Bit-identical: one block per row either way. Kill: METRALE_NO_QK_NORM_STRIDED=1.
    ...
    pub(super) fn wide_verify_gemm(
        ...
        if let Some(wt) = w_t {
            ...
            // METRALE_FFN_SMALLM=0 kill-switch.
    ...
    fn ms_qkv_batchn(&self, c: &MultiSeqCtx<'_>) -> Result<()> {
        ...
        // Kill switch: METRALE_NO_FUSED_QKV=1.

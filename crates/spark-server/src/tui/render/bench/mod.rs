...
pub(super) fn origin_badge(meta: &metrale_plugin::PluginMetadata) -> Span<'static> {
    if meta.official {
        Span::styled(
            " OFFICIAL ",
...
pub(super) fn metadata_lines(meta: &metrale_plugin::PluginMetadata) -> Vec<Line<'static>> {
...
pub(super) fn verdict_line(verdict: &metrale_plugin::Verdict) -> Line<'static> {
    use metrale_plugin::VerdictKind as K;
...
pub(super) fn draw_stats(f: &mut Frame, stats: &[metrale_plugin::Stat], area: Rect) {
    if stats.is_empty() {
        return;
    }
...
pub(super) fn draw_table(
    f: &mut Frame,
    table: &metrale_plugin::ResultTable,
    scroll: usize,
    area: Rect,
) -> usize {
    let block = panel(

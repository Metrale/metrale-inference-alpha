...
//! The remote runner over a scripted metralectl: what each way a node can
...
use super::*;
use anyhow::Result;
use metrale_plugin::hardware::equivalence::HardwareFingerprint;
use metrale_plugin::hardware::policy::Sensitivity;
...
fn scratch(tag: &str) -> Scratch {
    ...
    let sha = metrale_plugin::gate::read_record(&newest).unwrap().git_sha;
...
fn the_deadline_pays_for_a_build_only_on_a_cold_node() {
    ...
    // metralectl's 64, the gate name loses its front, never the shard tail —

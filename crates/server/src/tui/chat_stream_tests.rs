...
//! 2026-09-26: Tests of `stream_chat` over TCP against the loopback fake, so
//! the client reads the reply in the pieces the fake writes.
//!
//! Owner: server tui.
...
/// 2026-09-26: Run one request to completion and collect everything it emitted.
...
/// 2026-09-26: One short tag per delta, for assertions.
...
async fn a_non_200_carries_the_servers_own_explanation_not_just_the_status() {
    let f = serve(|s| {
        refuse(
            ...
            "application/json",
            r#"{"error":{"message":"no model is loaded; run `met serve`"}}"#,
        )
    });
...
async fn a_frame_split_mid_character_across_reads_arrives_whole() {
    // 2026-09-26: `stream_chat` decodes only complete lines, so a read that
    // ends inside a multi-byte character is not lossy-decoded.
    let json = r#"{"choices":[{"delta":{"content":"héllo — ✓"}}]}"#;
    let whole = frame(json);
    // 2026-09-26: One byte past the first 0xE2 lead byte, the start of `—`, so
...
async fn a_very_long_single_frame_is_delivered_as_one_token() {
    // 2026-09-26: 64 KiB in one delta takes several of the client's 8 KiB
    // reads.
...
async fn a_stream_whose_reader_hung_up_stops_instead_of_filling_a_dead_channel() {
    // 2026-09-26: With the receiver dropped, the first failed send ends
    // `stream_chat`.
    let f = serve(|s| {
        use std::io::Write as _;
...
fn an_empty_conversation_still_produces_a_well_formed_body() {
    ...
    assert_eq!(body["model"], serde_json::json!("metrale-tui"));
}

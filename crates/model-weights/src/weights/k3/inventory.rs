...
use crate::kimi_k3_host::tp::{TpBytePlan, plan_tensor_bytes};
use crate::kimi_k3_host::{K3Graph, MixerKind};
...
pub(super) fn scan(dir: &Path, config: &ModelConfig) -> Result<Vec<ShardPlan>> {
    ...
    for file_name in names {
        ...
        for name in tensor_names {
            ...
            if crate::kimi_k3_host::tp::tensor_plan(name, mixer, config).0
                == crate::kimi_k3_host::tp::TpAxis::Replicated
            {
                crate::kimi_k3_host::weights_geometry::validate_replicated_shape(
                    name,
                    view.shape(),
                    config,
                )?;
    ...
    for required in crate::kimi_k3_host::weights::required_names(config) {

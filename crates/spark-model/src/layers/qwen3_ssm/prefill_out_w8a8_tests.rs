...
fn not_selected_without_the_ssm_scope() {
    // The whole point of the scoped lever: `METRALE_CUBLAS_GEMM=ffn` must leave
...
fn single_scale_kill_switch_refuses() {
    // METRALE_FP8_SINGLE_SCALE clears `dispatch.fp8_blockscaled_prefill`.
    assert!(!selected(
        true,
        false,
        false,
...
fn missing_handles_fall_back() {
    ...
    // it so `METRALE_CUBLAS_SCALE_LAYOUT=rowmajor` in the environment does not

...
/// `met benchmark <list|run|history>` — or `--pull-request-gate-check` on
...
pub struct RunArgs {
    ...
    /// (`metrale_plugin::hardware::ids::KNOWN_HARDWARE_IDS`). A registered class
    ...
    /// answer "did this pass" — no `~/.metrale` state required.
    #[arg(long)]
...
impl RunArgs {
    ...
    pub fn reject_orphan_image_args(&self) -> Result<(), String> {
        ...
        if let Some(raw) = &self.output_image_args {
            metrale_plugin::gate::card::parse_args(raw)

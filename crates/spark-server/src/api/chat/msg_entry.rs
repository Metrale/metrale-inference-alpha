...
pub(super) fn build_msg_entries(
    ...
    for m in input.iter() {
        ...
        // Wave 3 (2026-05-26): `METRALE_STRIP_REASONING_HISTORY=1` drops
        // historical reasoning_content entirely. Matches MLC commit
        // d75d64e (Apr 2026) `strip_reasoning_in_history` for qwen3,
        // whose PR description matches Metrale Engine's Wave-1 failure mode
    ...
    // METRALE_NO_ERROR_DEDUP=1 restores verbatim history. MUST run
    ...
    // 3D-chess prompt). We can't fix the client, so Metrale Engine adapts: treat
    ...
    // by METRALE_BASH_WANDER_WATCHDOG (PCND, default-off).
    if tools_active
...
/// P1-6 (2026-07-09): kill-switch — `METRALE_NO_ERROR_DEDUP=1` restores

...
/// `METRALE_BF16_TC_PROJ` already falls into two tests above. Getting one
...
///
/// `METRALE_DFLASH_DEBUG_DUMP_FULL` arms both halves of the DFlash reference
...
/// copy — the same shape as `METRALE_DSPARK_ANCHOR_BIAS`, which had two
...
fn the_two_halves_of_the_dflash_dump_name_the_same_flag() {
    ...
    for (rel, field) in [
        ...
        // break it was `METRALE_DFLASH_DEBUG_DUMP_FULL_TYPO`, which contains
...
fn the_batched_decode_levers_keep_their_neighbours_spellings() {
    ...
    // ★ `METRALE_MS_PROFILE` and `METRALE_SSM_MS_PROFILE` are two live variables

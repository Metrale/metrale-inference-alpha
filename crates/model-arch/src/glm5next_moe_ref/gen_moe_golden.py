...
# 2026-09-25: Writes /w/moe_golden.json: the full routed MoE site (router, top-k, NVFP4 experts,
# shared expert, combine) for the layers in argv[1], under both router dtype ladders.
#
...
real = torch.load("/w/real_router.pt")["X"].float()
...
for L in LAYERS:
    ...
    # 2026-09-25: Every fixture is rounded to BF16 here, once. The routing below and the
    # emitted `__inputs` both use these rounded values, and the engine-side runner reads its
    # input from `__inputs`, so both sides start from the same tensor.
    def bf(x):
        return x.to(torch.bfloat16).float()
    fx = {"t1": bf(Lcg(0x0F0E_5EED).t(1, HID) * 0.5),
          "t7": bf(Lcg(0x0F0E_5EED).t(7, HID) * 0.5),
          "real32": bf(real[:32].clone())}
    # 2026-09-25: `nearcut` is the real row with the smallest gap between its K-th and
    # (K+1)-th biased router score.
    with torch.no_grad():
    ...
    # 2026-09-25: Inputs are emitted once per layer and unstrided: `real32` and `nearcut`
    # come from the real prefix, so the engine side cannot regenerate them from the LCG.
    inp = {}
    ...
    for mode in ("hf_fp32", "vllm_bf16"):
        for rn, x in fx.items():
            ...
            final = routed + shared            # 2026-09-25: shared is not routed-scaled

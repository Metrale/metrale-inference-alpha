...
This is the chapter you read when you're about to write a kernel. It covers the conventions every Metrale Engine kernel follows, the tools that matter on GB10 SM121, and the workflow that takes an idea from a profile to a merged PR.
...
## Conventions every Metrale Engine kernel follows
...
1. **Start with `nsys profile`** against the live server running a benchmark. The first question is always *which kernel is the bottleneck* — do not tune in the abstract.
   ```bash
   nsys profile --trace=cuda,cudnn,cublas,osrt -o metrale.qdrep \
     /path/to/met serve <model>
   # in another terminal: drive bench load
   nsys stats --report cuda_gpu_kern_sum metrale.qdrep | head -20
   ```
...
- Warp specialisation. SM121's scheduler is good enough that explicit producer/consumer warp roles rarely pay back on kernels of the shapes Metrale Engine runs.
...
5. **Wire the launch** via the `KernelLaunch` builder in `spark-model/src/layers/ops.rs`:
   ...
   KernelLaunch::new(gpu, self.kernel_handle)
       ...
       .launch(stream)
   ```
6. **Benchmark.** Add a shape in `metrale-spark-bench` or a micro-benchmark in the relevant primitive crate. A kernel without a benchmark is not allowed to claim "faster".
...
- Authoritative designs: [`docs/adr/`](https://github.com/Metrale/metrale-inference-alpha/tree/main/docs/adr) in the repo

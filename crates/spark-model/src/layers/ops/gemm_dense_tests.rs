...
//! run nothing on a GPU — `cargo test` runs with `METRALE_SKIP_BUILD=1`, so the
//! PTX-level sibling (`metrale-kernels/tests/kernel_arity.rs`) is VACUOUS in CI
...
/// the CPU-side stand-in for `metrale-kernels/tests/kernel_arity.rs`, which reads
/// the real PTX and is vacuous under `METRALE_SKIP_BUILD=1`.
#[test]

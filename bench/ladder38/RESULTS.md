...
- Metrale Engine: binary `d92fc2488` (PR #533 tip), env `METRALE_PREFILL_CODISPATCH=1
  METRALE_FP8_ROWWISE=1`, flags: `--max-seq-len 2048 --max-batch-size 128
...
and should run MTP here. Re-measured with every workload axis matched to Metrale Engine — same
...
**MTP K=4 on both** (Metrale Engine `--num-drafts 3`, vLLM `num_speculative_tokens: 3`):
...
Measured root cause of the open rungs: Metrale Engine's marginal cost per added concurrent sequence
is **4.28 ms/token/seq** vs vLLM's **1.94** (TPOT fits Metrale Engine `58.9 + 4.28n`, collinear
...
per step; Metrale Engine additionally paid 96 eager copy launches per sequence per step for SSM
...
served with `METRALE_PREFILL_CODISPATCH=1 METRALE_FP8_ROWWISE=1` and `--prefill-varlen-batch`.

| C | Metrale Engine | vLLM | ratio | rung |
|---:|---:|---:|---:|---|
...
- C=32: deficit is the prefill ramp (Metrale Engine ~620-745 tok/s prefill vs vLLM ~2.9k);
  Metrale Engine DECODES 10.5% faster per token at this rung (TPOT p50 128.7 vs 143.5 ms).
...
Qwen MTP band (0.7-0.85). Metrale Engine's drafter is not the problem.** What collapses at n>=16 is
...
Metrale Engine's marginal cost per added concurrent sequence is **4.28 ms/token/seq vs vLLM's 1.94**.
...
Stack `b508679e4`, Metrale Engine served at **fp8 KV** (matching the reference at last) with
...
`METRALE_MTP_K_LADDER="4:3,8:3,16:2,32:2"` (deeper drafts at the widths where the default
...
3. **`presence_penalty=1.5` in the `non_thinking` preset disables four fast-sampling
   ...
   ★ This was also a LIKE-FOR-LIKE VIOLATION: Metrale Engine injects that penalty when a request
...
48 GDN layers. Both now emit periodic RATES under `METRALE_MTP_ACCEPT_DEBUG`, so the next
...
kill switch (`METRALE_NO_CANONICAL_VERIFY_KEY=1`) at C=2 and C=4, reading the graph-capture
...
| Metrale Engine | 23.66 | 30.83 | 1.30x | 42.3 ms | 64.9 ms | **+22.6 ms/token** |
...
This is why Metrale Engine WINS C=1 (1.20x) and loses C=2 (0.79x). n=1 and n=2 run different
...
`METRALE_MTP_K_LADDER=1:3,2:1,4:3,8:2,16:1` (same-session A/B, 2 reps each):
...
|---|---:|---:|---:|---:|---:|
| **Metrale Engine** | 41.69 | 39.99 | 41.37 | **41.02** | 4.15% |
| vLLM+MTP | 37.62 | 36.52 | 37.18 | **37.11** | 2.94% |

**Ratio 1.105x**, against the recorded 1.004x. The distributions do not overlap: Metrale Engine's
...
Against the recorded vLLM 38.79 instead of today's 37.11, Metrale Engine still wins by 1.058x, so
...
- **This file's header block (line ~17) lists `--kv-cache-dtype bf16`**, which is the ROUND 1
  Metrale Engine config. The certified comparison is **fp8 KV on both** (round 4 moved Metrale Engine to fp8
...
Provenance: box dgx2 (spark-43fa), Metrale Engine `529fcb04fa` served with the round-11 flags at
`--kv-cache-dtype fp8`, env `METRALE_PREFILL_CODISPATCH=1 METRALE_FP8_ROWWISE=1
METRALE_MTP_DCUT_RATIO=1.0 METRALE_MTP_K_LADDER=1:3,2:1,4:2,8:2,16:1`; vLLM
...
`c2_metrale_dgx2_20260818.json` and `c2_vllm_mtp_dgx2_20260818.json`.
...
| engine | rep 1 | rep 2 | rep 3 | mean | spread |
|---|---:|---:|---:|---:|---:|
| Metrale Engine | 123.33 | 124.94 | 121.38 | **123.22** | 2.89% |
| vLLM+MTP | 121.92 | 120.60 | 122.40 | **121.64** | 1.49% |

**Ratio 1.013x**, against the certified 1.012x — reproduced to within 0.1%.

★ **Both engines measured ~2.2% BELOW their certified absolutes** (Metrale Engine 123.22 vs 125.95,
...
distributions here OVERLAP (Metrale Engine min 121.38 < vLLM max 122.40), so a single draw can
...
Raw series: `c8_metrale_dgx2_20260818.json`, `c8_vllm_mtp_dgx2_20260818.json`. Same
...
| C | Metrale Engine | vLLM+MTP | same-day | certified |
|---:|---:|---:|---:|---:|
...
Metrale Engine fell 5.1% at C=32 while vLLM fell only 2.1%, and C=1/2/4 got WIDER on the same sweep
...
| `crates/model-layers/**` | 2 files | **0 non-comment lines** (rustdoc link fixes) |
| `crates/gpu-runtime/**` | 0 files | — |
| `crates/server/{cli,tui}` | 3 files | benchmark CLI/TUI wiring only |
| `crates/metrale-plugin/**` | 34 files | benchmark harness (#569, #581) |
...
the only `metrale-model-layers` changes are two doc-comment link fixes with zero non-comment lines.
...
Metrale Engine never pays this because its speculation self-disables above 32 concurrent sequences,
...
Metrale Engine recovered at **every** rung that had looked soft — C=16 195.19 -> 201.32 and C=32
...
The mechanism is now clearer. The pre-wedge Metrale Engine leg ran on a box that had been serving
benchmarks for hours; the clean-boot leg ran on a box minutes old. Metrale Engine runs FIRST in this
...
during **vLLM C=32** — a rung whose Metrale Engine counterpart had completed minutes earlier in the
...
Metrale Engine does not exhibit this at any rung, which is worth stating plainly: it completed
...
The certified table pins **batch cap 128 on BOTH engines at every rung** (Metrale Engine
...
- The certified vLLM numbers were all taken at 128. A rung measured at a lower cap is a
  different configuration, and comparing it to the 128-cap Metrale Engine column is precisely the
...
Metrale Engine's `--max-batch-size` to the same value**, so the pair is at least internally
...
metrale#581 made `s_per_turn` the agentic speed bound and recorded `decode_tps`
...
certified 1.032x), its K ladder was swept the way C=8's was. Metrale-only, one box (dgx1),
...
### ⚠ C=32 DOES NOT CURRENTLY WIN (2026-08-20) — Metrale Engine is ~4% below its certified value
...
| config | Metrale Engine | certified Metrale Engine | delta |
|---|---:|---:|---:|
...
|---|---:|---:|---:|---:|
| Metrale Engine | 275.98 | 277.78 | 278.17 | **277.31** |
| vLLM+MTP | 283.08 | 287.54 | 283.00 | **284.54** |

**Ratio 0.975x — vLLM wins.**

The two views agree in direction: Metrale Engine has moved down ~4% at this rung. (vLLM's cap-32
...
touch `metrale-model-layers` or `kernels/` — four are benchmark harness, one is a Cargo.lock bump —
...
The sweep is ~30 minutes, Metrale-only, and carries no wedge risk. It needs dgx3 idle; it has
been serving another session throughout this investigation.

★ **This does NOT invalidate the same-day A/B results.** Each of those compared Metrale Engine against
...
them cannot be checked against a fresh Metrale Engine number without a fresh vLLM number beside it.

Ruled out so far: code (three commits measured, all ~279, and no `metrale-model-layers` or `kernels/`
...
**What this costs us.** The certified C=32 ratio of 1.027x rests on a Metrale Engine number that
...
Raw series: `c32_metrale_cap128_dgx2_20260820.json`, `c32_metrale_cap32_dgx2_20260820.json`,
...
(batch cap 128, util 0.85, fp8 KV, MTP K=4) at C=32, same box, same day as the Metrale Engine number:

| engine (cap 128, C=32, dgx2, same day) | rep 1 | rep 2 | rep 3 | mean | spread |
|---|---:|---:|---:|---:|---:|
| **Metrale Engine** | 279.32 | 278.45 | 279.02 | **278.93** | 0.31% |
| vLLM+MTP | 275.10 | 278.02 | 278.24 | **277.12** | 1.13% |

**Ratio 1.007x — Metrale Engine wins, and the distributions do not overlap** (Metrale Engine's worst rep 278.45
beats vLLM's best 278.24).

**vLLM fell too.** Its certified 283.48 is now 277.12, down 2.2% — so the fleet-wide shift
documented above is not Metrale-specific. It costs Metrale Engine ~4% and vLLM ~2.2% at this rung, which
...
|---|---:|---:|---|
| Metrale Engine | 277.31 | 278.93 | roughly flat (+0.6%) |
| vLLM+MTP | 284.54 | 277.12 | **+2.7% for vLLM at cap 32** |

vLLM gains materially from the smaller cap; Metrale Engine does not. So the matched-cap-32 pair —
...
|---|---:|---:|---:|
| Metrale Engine | 291.01 | 278.93 | -4.2% |
| vLLM+MTP | 283.48 | 277.12 | -2.2% |
| | | **net against Metrale Engine** | **~1.8%** |
...
(`1575873582`), Metrale-only, isolated worktree:
...
above, where vLLM's certified 283.48 measures 277.12 today against Metrale Engine's 278.93 on the same
box and day. The absolutes across the fleet have shifted; the same-day ratios have not.

Raw series: `sweep_metrale_stacktip_dgx3_20260820.json`.
...
powercycles — Metrale Engine's own absolutes were measured there today (dgx1, idle, certified commit
`1575873582`, Metrale-only, zero wedge risk) and compared against the CERTIFIED vLLM column.

That comparison is deliberately unfair to Metrale Engine: vLLM has demonstrably FALLEN with the fleet
(its certified C=32 283.48 measures 277.12 today, -2.2%), so crediting it with its old number
understates Metrale Engine's true margin.

| C | Metrale Engine today | certified vLLM | ratio vs certified vLLM | certified ratio |
|---:|---:|---:|---:|---:|
...
differential-based bound argued and this now demonstrates with measured Metrale Engine numbers at the
rungs themselves.
...
Raw series: `metrale_c64_c128_dgx1_20260820.json`.
...
within 0.06%, C=64 to 0.12%. **Metrale Engine beats vLLM+MTP at every concurrency from 1 to 128.**
...
A/B with the kill switches is running (`METRALE_NO_DRAFTER_SMALL_M_TIER=1`, then
`METRALE_NO_GEMV_EXACT_M_TIERS=1`). If the drafter tier is confirmed, the fix is the same
...
| `METRALE_NO_GEMV_EXACT_M_TIERS=1` | **1020 s** | 10/10 + 10/10 |
| `METRALE_NO_DRAFTER_SMALL_M_TIER=1` | running | |
...
Two operational notes recorded so they are not rediscovered: killing a `met serve` can
...
| `METRALE_NO_GEMV_EXACT_M_TIERS=1` | 1020 s | 10/10 + 10/10 |
| `METRALE_NO_DRAFTER_SMALL_M_TIER=1` | **1078 s** | 10/10 + 10/10 |

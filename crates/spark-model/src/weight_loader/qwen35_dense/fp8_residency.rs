...
//! native-FP8 profile (`METRALE_DENSE_FP8=1`, `--lm-head-dtype bf16`):
//!
...
//!   sized**, and `METRALE_MEM_PROFILE` recorded GPU-free falling **437 MB per
...
//!
//! `METRALE_FFN_W8A16_ONLY` is deliberately NOT an input: it steers the dense FFN
...
//! wants. `METRALE_DENSE_FP8_KEEP_NVFP4` is the escape hatch that restores the
...
///
/// PRESENCE (any value, including empty), matching `METRALE_FFN_W8A16_ONLY` —
/// this is an escape hatch an operator reaches for while a serve is
/// misbehaving, and `METRALE_DENSE_FP8_KEEP_NVFP4=0` meaning "on" is a trap.
...
pub struct DenseFp8Inputs {
    ...
    /// (`METRALE_DENSE_FP8=1`, tp_size 1, `Fp8Dequanted`, gate_proj native FP8).
    pub ffn_fp8: bool,
    /// The native FP8 attention overlay will be installed on this layer.
    pub attn_fp8: bool,
    /// `METRALE_DENSE_FP8_KEEP_NVFP4` — restore the pre-#915 behaviour.
    pub keep_nvfp4: bool,
    ...
    /// `METRALE_ATTN_W4A4` is set. `prefill/paged_oproj.rs:38-42` builds its W4A4
    ...
    pub attn_w4a4: bool,
    /// `METRALE_ATTN_PREFILL_Q_T=1`. `prefill/cache_skip_qkv.rs:142` reads it per
...
impl DenseFp8Plan {
    ...
    ///   fused twins survive only behind the `METRALE_CUTLASS_NVFP4_*` levers,
    ///   which default off.
    ...
    ///   `METRALE_ATTN_PREFILL_Q_T=1` (`cache_skip_qkv.rs:142`) and O is routed
...
impl DerivedResidency {
    ...
    /// without an `METRALE_MEM_PROFILE` rerun: `weights` is the checkpoint,

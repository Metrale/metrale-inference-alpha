...
//! Metrale Engine plugins — and the benchmark suite the `met serve` TUI drives.
//!
//! Two layers:
//!
...
//!   the run-glow), the `~/.metrale` artifact store, the endpoint it is pointed
...
//!   artifacts   ~/.metrale layout, asset writing, provisioning stamps

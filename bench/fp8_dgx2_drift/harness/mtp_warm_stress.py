...
# 2026-09-26: MTP with warm-restore stress: long tool-using chats at temperature 0.3, replies scanned for path stutter.
#
...
# 2026-09-26: Each prompt names exact /tmp/stress-zq7 paths, so a changed path in a
# reply shows up in `scan`.
PROMPTS = [
...
STUTTER = re.compile(
    r"(/tmp/[\w\-./]*?)(\w{2,6})\2(?=[\w\-./])"  # 2026-09-26: a repeated 2-6 char run inside a /tmp path
)
...
def scan(s):
    ...
    # 2026-09-26: Any /tmp path outside the two expected roots is flagged.

...
//! in `api/chat/prepare.rs` — Metrale Engine then silently DROPS the caller's `tools`
...
/// Replay parity: what Metrale Engine writes back into assistant history must be what

...
4. **`hc_norm` dropped the offset-from-1.** *(Found and fixed in phase A —
   ...
   Metrale Engine already dispatches this globally through
...
2. **One GPU, one Metrale Engine instance.** `--gpu-memory-utilization` reserves its
...
- [x] **PLE refused at LOAD** unless `METRALE_QWEN4EXP_NO_PLE=1`, with the
...
`METRALE_QWEN4EXP_NO_PLE=1` that logs a loud warning and is refused by default.
...
- [ ] **Answer the open question: does Metrale Engine's DeepSeek CSA selection match
      Qwen's?** Metrale Engine has `index_n_heads` / `index_head_dim` / `index_topk`,

...
//! 2026-09-25: The body of `glm5next_ffn_microtest`. Gate 4 runs the BF16 dense
//! FFN of layer 0 and the BF16 shared expert of layer 3; gate 3 runs routed
//! experts 0 to 3 of layer 3 through the NVFP4 W4A16 path. Every stage is graded
//! against a floor taken from the golden.
//!
...
pub(crate) fn run() -> Result<()> {
...

    {
        ...
        for &(rn, t, sc) in REGIMES.iter() {
            ...
            // 2026-09-25: Clamp control on `clamp64`: recompute the activation on the host
            // from the GPU's own gate and up, once with no clamp and once with a symmetric
            // gate clamp. The regime must drive gate past both -limit and +limit and |up|
            // past limit, and the no-clamp result must differ from the GPU's by at least
            // 100 bf16 ulps of the activation magnitude. The symmetric result is printed only.
            if rn == "clamp64" {
                let gate_v = down_bf16(&gpu, d_g, t * inter)?;
                let up_v = down_bf16(&gpu, d_u, t * inter)?;
                let act_v = down_bf16(&gpu, d_a, t * inter)?;
                let mut sym_max = 0.0f32;
                let mut none_max = 0.0f32;
    ...
    for e in 0..4usize {
        ...
        for proj in ["gate_proj", "up_proj", "down_proj"] {
            ...
            // 2026-09-25: Floor C: `dequant_nvfp4_to_bf16` writes bf16, so the golden's f32
            // `deq_*` values are rounded to bf16 first and every sampled element must be equal.
            ...
            if mismatches != 0 {
                bail!(
                    "expert {e} {proj}: Metrale Engine's CUDA dequant disagrees with the ModelOpt \
        ...
        for &(rn, t, sc) in REGIMES.iter() {
            ...
            // 2026-09-25: Floor D: the gate projection alone, against the golden's f32 gate.
            ...
            // 2026-09-25: Floor E: the whole expert FFN, against the golden's f32 output.
    ...
    for r in &rows {
        // 2026-09-25: A zero floor marks a bit-exact row (floor C): it passes only when the
        // residual is zero, and its ratio prints as 0.
        let exact = r.b == 0.0;

...
- **Build time** — `metrale-kernels/build.rs` decides *which PTX to embed*.
- **Startup** — `metrale-server::main` decides *which embedded PTX to upload to the GPU* based on the model being served.
...
`metrale-kernels/build.rs` runs during `cargo build`. Its job:

1. Read the three wildcards:
   - `METRALE_TARGET_HW` (default `gb10`; accepts `*`)
   - `METRALE_TARGET_MODEL` (default `*` — all)
   - `METRALE_TARGET_QUANT` (default `*` — all)
...
6. Emit an auto-generated Rust file, `$OUT_DIR/target_ptx.rs`, that `include!()`'s back into `metrale-kernels/src/lib.rs`. The generated file contains one `pub static PTX_<TARGET>: &[PtxModule]` per target plus an `all_ptx_sets()` function that returns the whole set.
...
1. **Parse the model config.** `metrale_config::ModelConfig::from_hf(&model_path)` reads `config.json` and its nested text/vision configs.
...
3. **Resolve the KernelTarget.** Given `model_type` and the selected quantization (from config or `--kv-cache-dtype` when overriding), `metrale-kernels::select_target(hw, model, quant)` looks up the matching `KernelTarget`. Fail fast with a clear error if there's no match.
4. **Instantiate the GpuBackend.** `MetraleCudaBackend::new(gpu_ordinal, &ptx_set.modules)` uploads every embedded PTX module for the chosen target to the GPU, via `cuModuleLoadData`. Kernel handles are cached per `(module_name, function_name)` pair.
5. **Instantiate the ModelWeightLoader.** `metrale_model_engine::factory::loader_for_config(&config)` matches on the canonical `model_type` and returns `Box<dyn ModelWeightLoader>`.
...
metrale_server::scheduler              (SLAI or FIFO)
 ...
 │
 ▼
metrale_model_engine::engine::InferenceEngine::decode_step
 │
 ▼  for layer in layers:
...
Both virtual calls are unavoidable consequences of the specialization thesis: we want `metrale-server` to not know what `GpuBackend` it's talking to, and we want `InferenceEngine` to not know what layer shape it's running. That's how new hardware and new models plug in.
...
| Question | File |
|---|---|
| "How is `KernelTarget` resolved at startup?" | `crates/kernels/src/lib.rs`, look for `select_target()` + `include!(target_ptx.rs)` |
| "How does a kernel get compiled at build time?" | `crates/kernels/build.rs`, `crates/core/src/compute.rs` |
| "How does a layer launch a kernel?" | `crates/model-layers/src/layers/ops.rs`, look for `KernelLaunch::new(gpu, kernel).grid(...).arg_ptr(...).launch(stream)` |
| "How does the engine loop over layers?" | `crates/model-engine/src/engine.rs` |

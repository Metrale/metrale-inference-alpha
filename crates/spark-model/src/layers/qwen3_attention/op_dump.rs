...
//!
//! Companion to the existing `METRALE_GDN_DUMP` infrastructure in
...
//!
//! Activation:
//!     `METRALE_OP_DUMP=<dir>`                     (required to enable; no-op when unset)
//!     METRALE_OP_DUMP_LAYERS=0,7,11,15,19,23,...  (csv of absolute layer
//!                                                indices; default = all)
//!     METRALE_OP_DUMP_OPS=q_proj,k_proj,...       (csv of op names; default = all)
//!
//! Filename convention:
//!     `<METRALE_OP_DUMP>/metrale_op_L{abs_layer}_{op}.bin`
...
/// Returns Some(dir) if METRALE_OP_DUMP is set, the layer index is allowed,
...
/// `<METRALE_OP_DUMP>/metrale_op_L{layer_idx}_{op}.bin`.

...
//! Metrale Engine cannot faithfully apply must error with a named reason — never be
...
/// Parsed subset of a PEFT `adapter_config.json` that Metrale Engine consumes.
///
...
pub struct PeftAdapterConfig {
    ...
    /// Accepted `modules_to_save` leaves — the subset Metrale Engine can apply as a
...
struct RawPeftAdapterConfig {
    ...
    r: usize,
    lora_alpha: f64,
    /// Array of strings, or the string "all-linear" (rejected — Metrale Engine
...
fn parse_target_modules(v: &serde_json::Value) -> Result<(Vec<String>, Option<String>)> {
    match v {
        ...
        // layer", which Metrale Engine cannot enumerate against fused/quantized layouts
...
/// the name-level gate.
/// `METRALE_LORA_ALLOW_PARTIAL=1` — load an adapter naming target modules Metrale Engine
...
fn validate_target_module(entry: &str) -> Result<()> {
    ...
    match leaf {
        ...
        // (`classify_key`) + `METRALE_LORA_EXPERTS` are the authority on whether the

...
## The three variants in Metrale Engine
...
## Why SSMs are fast on Metrale Engine

Three things:

1. **Fused QKVZ preprocess.** The `ssm_preprocess.cu` kernel deinterleaves the 4-way projection output (Q, K, V, Z) and computes the GDN gate (softplus + sigmoid) in one pass. PyTorch does this in three separate kernels plus a reshape; Metrale Engine does it in one.
...
| Op | Metrale Engine (ms) | PyTorch (ms) | Speedup |
|---|---:|---:|---:|
...
The 9.95× at conv1d seq=512 is the largest compute speedup in the whole repo. PyTorch's `causal_conv1d_fn` on this shape walks the full sequence per batch element; Metrale Engine's kernel fuses the whole thing into one launch with shared-memory tiling.
...
Metrale Engine's **Marconi** (inside-joke name for the SSM snapshot cache) stores the SSM state at the end of each cached prefix alongside the KV. A warm prefix-cache hit restores both the attention KV *and* the SSM state. Output is byte-identical to the cold run.

Costs: an extra ~GB of snapshot storage per top-level cache entry. Worth it for repeat agent workloads. Controlled by `--ssm-cache-slots` (default 16) and `--ssm-checkpoint-interval` (default 256). See the `marconi.md` note and `crates/telemetry/src/prefix_cache.rs`.
...
where `dt` is the delta-time projection. The gate is used both to attenuate the state update and to form the output. Metrale Engine fuses this into `ssm_preprocess.cu` — the gate appears as a byproduct of the QKVZ deinterleave.

Numerically, softplus is the chronic problem: `softplus(large x) = x`, but `softplus(very large x)` can overflow in FP32 if you compute naively. Metrale Engine uses the standard `max(0, x) + log1p(exp(-|x|))` stable form.
...
- **Upstream Mamba bug on chunked prefill** — the vLLM fix approach (different from Mamba-2 reference) is what Metrale Engine uses; the reference implementation has a subtle boundary issue at chunk seams.
...
`crates/model-layers/src/layers/qwen3_ssm/mod.rs` and `nemotron_mamba2.rs` contain the layer-level state machines. They:
...
Each step calls into `metrale-gpu-runtime::GpuBackend` via the layer's cached `KernelHandle`s — the per-step Rust code is short because the real work is in the 3–4 kernel calls it issues.

...
//! 2026-09-25: `gated_delta_rule_recompute_wu` chained into
//! `gated_delta_rule_chunk_delta_h_ksplit`, against an f64 CPU reference (W/U forward
//! substitution and the state recurrence), at Qwen3.6-35B-A3B GDN dimensions.
//!
//! Owner: model-arch examples.
//! Invariants:
//! - Exit 1 unless, for every t, the chunk-entry states S_c, uc and the final state all
//!   reach cosine >= 0.999 against the reference.
//!
//! Run:
//!   cargo run -p metrale-model-arch --release --example gdn_chunk_delta_h_gateb
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: `_ksplit` is the entry `qwen3_ssm/init.rs` resolves; the SSM prefill
    // does not resolve the bare `gated_delta_rule_chunk_delta_h`.
    ...
    for &t in &[64usize, 128, 200] {
        ...
        let mut sc_ref = vec![0.0f32; nt * NV * KD * VD];
        let mut uc_ref = vec![0.0f32; nt * NV * C * VD];
        let mut sf_ref = vec![0.0f32; NV * KD * VD];
        for vh in 0..NV {
            let kh = vh / HR;
            ...
            for c in 0..nt {
                ...
                // 2026-09-25: uc = U - W S, with S the chunk-entry state.
        ...
        // 2026-09-25: Equal to `smem_dh` in ops/ssm_gdn_a3.rs.
        let smem2 = (2 * (C * (2 * KD + VD) * 2) + 2 * C * 4 + 2 * (C + 1) * 4) as u32;
        KernelLaunch::new(g, k_dh)
            .grid([NV as u32, 1, 1])
            // 2026-09-25: 256 threads, as ops/ssm_gdn_a3.rs launches the ksplit spine.
            .block([256, 1, 1])
            .shared_mem(smem2)
            .arg_ptr(hp)
            .arg_ptr(wp)

...
//! 2026-09-26: Tests for the Benchmarks section: the shared table, tiles, verdict and badges, the run, History and params panes, and `basis_of`.
//!
//! Owner: server tui.
...
fn a_scroll_past_the_last_row_still_shows_the_last_page() {
    // 2026-09-26: `draw_table` clamps a scroll past the end to the last page.
    let t = table(60);
    let rows = draw_into(80, 20, |f, area| {
...
fn the_origin_badge_reserves_brand_green_for_first_party_benchmarks() {
    let official = metrale_bench::PluginMetadata {
        official: true,
        ..*crate::tui::render::tests::app().bench.plugin_metadata()
    };
    let community = metrale_bench::PluginMetadata {
        official: false,
        ..official
    };
...
mod run {
    ...
    fn a_run_that_cannot_know_its_total_says_so_rather_than_drawing_a_full_bar() {
        ...
        // 2026-09-26: Nothing is running in the test, so the caption is "idle".
...
/// 2026-09-26: The History pane, drawing stored `RunRecord` frames.
mod history {
    use super::*;
...
/// 2026-09-26: The Suite list scrolls the selection into view at 80x24.
#[test]
...
/// 2026-09-26: The params form keeps the selected row and the probe status
/// on screen.
#[test]
...
// 2026-09-26: `basis_of`: equal for like-for-like runs, different when the
// model, any parameter, or the serve overrides differ.
...
fn any_parameter_counts_not_a_hand_picked_subset() {
    // 2026-09-26: Every parameter is hashed, including ones no code names.
...
/// 2026-09-26: Same model and parameters, different serve overrides: a
/// different basis.
#[test]
...
/// 2026-09-26: The override key is named in the basis, not only hashed.
#[test]
...
/// 2026-09-26: Empty overrides read `unpinned`: a run this process did not
/// serve records none, whatever the server had.
#[test]
...
/// 2026-09-26: The same override key with different values gives different
/// bases, through the hash.
#[test]
...
/// 2026-09-26: `basis_of` with no serve overrides.

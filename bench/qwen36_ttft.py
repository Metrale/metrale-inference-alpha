#!/usr/bin/env python3
# 2026-09-26: Times one-token completions at three prompt lengths (TTFT) and greedy
# completions at two output lengths (completion tokens over request wall time), writes
# them as JSON, and exits 3 if a decode rate is more than DECODE_GUARD_PCT below --baseline.
#
# Owner: bench, TTFT probes.
...
then re-measures after each Metrale Engine change. Fails the run if decode TPS
...
def build_prompt(target_tokens: int) -> str:
    # 2026-09-26: 4 characters per token is an estimate; measure_ttft records the served
    # prompt_tokens.

#!/usr/bin/env python3
"""Phase 2b cosine compare: post-RNE Metrale Engine image vs both references.

Reads:
  - /workspace/metrale-dumps/numdrift/rne/metrale_L{0..39}.bin    (NEW post-RNE)
  - /workspace/metrale-dumps/numdrift/metrale_L{0..39}.bin        (OLD truncating)
  - /workspace/metrale-dumps/numdrift/hf_L{0..39}.bin           (unquant BF16 ref)
  - /workspace/metrale-dumps/fp8dequant/hf_L{0..39}.bin         (FP8->BF16 ref)

Reports four series:
  B_old: Metrale Engine[truncating] vs HF[unquant]      -- Phase α baseline
  B_new: Metrale Engine[RNE]        vs HF[unquant]      -- Phase 2b result
  C_old: Metrale Engine[truncating] vs HF[FP8->BF16]    -- Phase 2a Metrale-fidelity baseline
  C_new: Metrale Engine[RNE]        vs HF[FP8->BF16]    -- Phase 2b Metrale-fidelity result
...
NUMDRIFT = pathlib.Path("/workspace/metrale-dumps/numdrift")
DEQUANT = pathlib.Path("/workspace/metrale-dumps/fp8dequant")
RNE = pathlib.Path("/workspace/metrale-dumps/numdrift/rne")
N_LAYERS = 40
...
def main() -> None:
    ...
    for i in range(N_LAYERS):
        metrale_old_p = NUMDRIFT / f"metrale_L{i}.bin"
        metrale_new_p = RNE / f"metrale_L{i}.bin"
        hf_unquant_p = NUMDRIFT / f"hf_L{i}.bin"
        hf_fp8dq_p = DEQUANT / f"hf_L{i}.bin"
        missing: list[str] = []
        for p, label in [
            (metrale_old_p, "metrale_old"),
            (metrale_new_p, "metrale_new"),
        ...
        if missing:
            ...
            continue
        metrale_old = load(metrale_old_p)
        metrale_new = load(metrale_new_p)
        hf_unquant = load(hf_unquant_p)
        hf_fp8dq = load(hf_fp8dq_p)
        rA = cmp_pair(hf_fp8dq, hf_unquant)
        rB_old = cmp_pair(metrale_old, hf_unquant)
        rB_new = cmp_pair(metrale_new, hf_unquant)
        rC_old = cmp_pair(metrale_old, hf_fp8dq)
        rC_new = cmp_pair(metrale_new, hf_fp8dq)
    ...
    sB_old = summarize("B_old: Metrale Engine[trunc] vs HF[unquant]       ", cosines_B_old)
    sB_new = summarize("B_new: Metrale Engine[RNE]   vs HF[unquant]       ", cosines_B_new)
    sC_old = summarize("C_old: Metrale Engine[trunc] vs HF[FP8->BF16]      ", cosines_C_old)
    sC_new = summarize("C_new: Metrale Engine[RNE]   vs HF[FP8->BF16]      ", cosines_C_new)
    ...
    if sC_new["mean"] >= 0.997:
        print("RESULT: TARGET MET. C >= 0.997, Metrale Engine compute path at ceiling.")

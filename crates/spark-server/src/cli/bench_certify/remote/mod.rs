...
//! unless `--remote-only`, plus every address that metralectl reaches and
...
use metrale_plugin::gate::signing;
use metrale_plugin::hardware::{Hardware, HardwareState};
...
use metrale_plugin::hardware::limits::ThermalEnvelope;
use node::Node;
...
///
/// # Errors
/// When metralectl cannot be run, or no node at all is admitted.
pub fn assemble(

...
running Metrale Engine, on storage retained between server/container restarts:

```bash
mkdir -p /work/cache/metrale-cuda
test -w /work/cache/metrale-cuda
```

Replace `/work/cache/metrale-cuda` with the actual absolute path on the rental.
...
```json
"CUDA_CACHE_PATH": "/work/cache/metrale-cuda"
```
...
used on a DGX Spark after replacing its identities. For an eight-GPU B300 run use
...
```json
{
  ...
  "master_port": 29500,
  "port_base": 18888,
  "env": {"METRALE_COMM_DIAGNOSTICS": "1", "NCCL_DEBUG": "INFO"},
  "max_seq_len": 1024,
...
`false`. The prefill budget and KV dtype are forwarded explicitly, so the
binary's defaults cannot alter the run; prefix caching is a presence flag, so
`true` passes `--enable-prefix-caching` and `false` passes nothing (off is its
default and `met serve` accepts no value for it).
...
from `nvidia-smi` or proof of device-code coverage. Metrale Engine's own architecture
...
`METRALE_*`, `NCCL_*`, `RUST_LOG`, `LD_LIBRARY_PATH`, and the validated
...
master addresses. For two separate DGX Sparks, use a reviewed multi-host launcher

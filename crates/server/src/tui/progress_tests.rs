...
//! 2026-09-26: Tests for `ProgressModel`: reset, the load window and rate, the bars, and out-of-range events.
//!
//! Owner: server tui.
...
/// 2026-09-26: After `reset` every phase is `Pending` again and can re-enter
/// `Running`; `enter_phase` alone never moves a `Done` phase back.
#[test]
...
/// 2026-09-26: `reset` clears the closed load window, so the next load is
/// measured afresh.
#[test]
fn reset_reopens_the_frozen_load_window() {
    let mut m = ProgressModel::default();
    // 2026-09-26: The window opens on the first shard, so one must start
...
/// 2026-09-26: GB/s is measured from the first shard, not from `started_at`,
/// and stops changing once the last shard is done.
#[test]
fn load_rate_is_windowed_and_freezes_at_the_last_shard() {
    ...
    ms(120); // 2026-09-26: before the first shard; not in the rate.
    for shard in 1..=2u64 {
        m.apply(ProgressEvent::ShardStart {
            shard,
            total: 2,
...
/// 2026-09-26: The shard total is zero until the first shard event, and the
/// overall fraction must still be finite.
#[test]
...
/// 2026-09-26: With no shard events the overall bar reads 100% once ready.
#[test]
...
/// 2026-09-26: A shard number past its total is clamped to a full bar.
#[test]
...
fn a_ready_before_any_shard_leaves_the_load_window_unopened() {
    // 2026-09-26: `Ready` closes the window only if a shard opened it.
    let mut m = ProgressModel::default();
    m.apply(ProgressEvent::Ready { port: 8888 });
...
#[test]
fn the_memory_history_is_bounded() {
    let mut m = ProgressModel::default();
...
fn a_phase_index_past_the_checklist_completes_it_instead_of_panicking() {
    // 2026-09-26: An index past `PHASE_NAMES` finishes every phase and does
    // not panic.
    let mut m = ProgressModel::default();
    m.apply(ProgressEvent::Phase {
        phase: 99,
...
fn a_shard_done_with_no_total_does_not_close_the_load_window() {
    // 2026-09-26: A zero total never closes the window (`total > 0` check).
    let mut m = ProgressModel::default();
    m.apply(ProgressEvent::ShardStart {
        shard: 1,
        total: 0,
...
fn the_shard_bar_fills_on_done_and_only_resets_on_a_new_shard() {
    ...
    assert_eq!(m.shard_target(), 1.0);
    // 2026-09-26: A repeated `ShardStart` for the same shard keeps the bar
    // full.
    m.apply(ProgressEvent::ShardStart {
        shard: 1,
        total: 4,

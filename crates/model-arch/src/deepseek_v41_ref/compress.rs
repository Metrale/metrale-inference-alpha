...
//! 2026-09-25: DeepSeek-V4.1 compressed attention (`compress_ratio > 0`), CPU
//! reference against the golden: YaRN RoPE with `compress_rope_theta`, the
//! two fp4 quantisers, `linear_f32`, and `attention_any`, the attention
//! forward for any ratio. The compressor and the indexer are in the
//! `compressor` and `indexer` submodules; the ratio-0 pieces are shared with
//! `attn.rs`.
//!
//! Precision: the ratio > 1 compressor is f32 until its pooled result is
//! rounded to bf16 for the RMSNorm; the indexer's q and k go through the e2m1
//! round trip (power-of-two scales, block 32) and its scores are bf16 values;
//! the compressed KV latent goes through e2m1 with e4m3 scales (block 16).
//! The tests compare index selections exactly and tensors at bf16 tolerance.
//!
...
/// 2026-09-25: Values per power-of-two scale in the indexer's fp4 round trip.
pub const FP4_BLOCK: usize = 32;
/// 2026-09-25: Values per e4m3 scale in the compressed KV latent's fp4 round trip.
pub const LATENT_BLOCK: usize = 16;
const FP4_MAX: f32 = 6.0;
const E2M1: [f32; 8] = [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0];

/// 2026-09-25: Round onto the e2m1 grid with ties to the even code, keeping the
/// sign (as `_to_e2m1_rne` in `ds41_ref_shims.py`).
...
/// 2026-09-25: The shims' `fp4_act_quant` with e8m0 scales, in place: scale =
/// power-of-two ceiling of `amax / 6` (amax floored at `6 * 2^-126`), values
/// to e2m1, dequantised and rounded to bf16.
...
/// 2026-09-25: The shims' `fp4_act_quant` with e4m3 scales, in place: amax
/// floored at `6 * 2^-9`, scale = `amax / 6` rounded through e4m3, values to
/// e2m1, dequantised and rounded to bf16.
...
/// 2026-09-25: The RoPE table with YaRN frequency correction, which the
/// compressed layers build from `compress_rope_theta`: `[seqlen][dim/2]` of (cos, sin).
...
/// 2026-09-25: f32 `x · w^T` (the ratio > 1 compressor's `wkv` / `wgate` are
/// f32 weights). `w` is `[out_dim, in_dim]`.
...
/// 2026-09-25: The attention forward for any `compress_ratio`, updating the
/// layer state and the shared slots.
...
pub fn attention_any(
    ...
    if c.ratio > 0 {
        ...
        // 2026-09-25: The indexer needs the latent before RoPE, so it runs
        ...
        // 2026-09-25: Read the compressed rows after this layer's write.

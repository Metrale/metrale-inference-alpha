...
//!
//! Set `METRALE_GDN_DUMP=<dir>` to enable, and optionally
//! `METRALE_GDN_DUMP_LAYERS=0,15,29` (comma-list of SSM-layer indices) to
...
/// `<METRALE_GDN_DUMP>/gdnsub_step0_L{layer_idx}_{stage}.bin`. Skips
/// when `METRALE_GDN_DUMP` is unset or `layer_idx` not in
/// `METRALE_GDN_DUMP_LAYERS`. Latches per-(layer, stage) so each pair
...
pub(super) fn maybe_dump_gdn_buf(
    ...
    // METRALE_GDN_DUMP_N_SSM=<count> if needed.

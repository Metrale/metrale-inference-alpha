...
# 2026-09-26: Mixed-step logits aliasing probe: a counting request decodes while long prefills land.
#
# Owner: bench.
...
def counter_request():
    ...
    body = {
        ...
        # 2026-09-26: Room for the whole count, so the counter is still decoding
        # when the filler prefills arrive.
        "max_tokens": 1200,
        "temperature": 0.0,
...
def check_sequence(text, upto):
    ...
    for tok in nums:
        if int(tok) != expect:
            ...
            # 2026-09-26: Resync, so one bad token is one break, not a cascade.
            expect = int(tok) + 1
        else:
            expect += 1
        if expect > upto:
...
for rnd in range(1, ROUNDS + 1):
    ...
    t.start()
    # 2026-09-26: Start the fillers 3 s after the counter, so the counter is
    # already decoding when they prefill.
    time.sleep(3.0)

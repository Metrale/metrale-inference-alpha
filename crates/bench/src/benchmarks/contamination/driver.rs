...
//! 2026-09-26: The contamination state machine. Each `next()` runs one step
//! (endpoint probe, a leg, or scoring), so the pane updates as each leg lands
//! and a cancellation is seen before the next step.
//!
//! Legs, in order (the design is in `score.rs`):
//! 1. prime: every probe once, solo, unmeasured, so later legs can run
//!    cache-warm. A failed prime is logged as a warning.
//! 2. two references: every probe solo, twice. A probe whose two runs
//!    disagree is `AloneUnstable`.
//! 3. rungs: per configured concurrency, one batch with the probes in its
//!    first slots and copies of the probes as ballast (see `rung_slots`).
//! 4. post: every probe solo again. A divergence here is `Persistent`.
//!
//! A transport failure becomes [`RequestOutcome::Error`], which the scorer
//! classifies as `Unmeasured`; any unmeasured cell fails the verdict.
//!
...
pub const DESCRIPTOR: BenchmarkDescriptor = BenchmarkDescriptor {
    ...
    // 2026-09-26: Any served model is a valid subject: this tests the engine,
    // not a checkpoint.
    intended_for: None,
    threshold_params: &[],
    // 2026-09-26: Token identity does not depend on clock speed, so a
    // throttled box gives the same result.
...
/// 2026-09-26: The request every leg sends: streamed, at temperature 0,
/// because the scorer compares transcripts for equality.
...
/// 2026-09-26: Which probe each slot of a rung batch runs. The first
/// `n_probes` slots are the measured ones, in probe order, because
/// [`super::score::Legs`] indexes outcomes by probe. The rest cycle through
/// the probes as unmeasured ballast. A batch never has fewer than `n_probes`
/// slots.
pub(super) fn rung_slots(n_probes: usize, conc: usize) -> Vec<usize> {
    (0..conc.max(n_probes)).map(|s| s % n_probes).collect()
}

/// 2026-09-26: An error becomes [`RequestOutcome::Error`] rather than an
/// abort, so one failed request does not discard the finished legs.
...
impl CrossContamination {
    ...
    /// 2026-09-26: prime, two references, one per rung, post, score.
    ...
    /// 2026-09-26: Every probe once, one request at a time.
    ...
    /// 2026-09-26: The whole batch in flight at once; only the first
    /// `PROBES.len()` outcomes are kept (see [`rung_slots`]).
    ...
    /// 2026-09-26: Scores the recorded legs. Reads only collected state, so
    /// the tests call it without a server.
...
impl Benchmark for CrossContamination {
    ...
    fn configure(&mut self, values: &ParamValues) -> Result<()> {
        ...
        // 2026-09-26: A floor at or above the budget would make every reply
        // Unmeasured, so it is rejected here, before any request.
        if floor >= max_tokens {
            bail!(

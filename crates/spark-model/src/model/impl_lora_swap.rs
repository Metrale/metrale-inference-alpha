...
impl TransformerModel {
    /// RDMA-swap the adapter named `adapter_name` (staged on `$METRALE_LORA_PEER`
    ...
    /// armed (`METRALE_LORA_ROTATE`/`$METRALE_LORA_PEER`) so decode is eager.
    #[cfg(feature = "cuda")]
    ...
    /// (`METRALE_LORA_ROTATE=1`) — the inner swap enforces it.
    ...
    /// rotation armed (`METRALE_LORA_ROTATE=1`/`$METRALE_LORA_PEER`) so decode is

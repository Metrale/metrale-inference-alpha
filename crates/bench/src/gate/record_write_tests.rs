...
//! 2026-09-26: Tests for `write_record`: a failing record is never
//! overwritten, and the re-run name it uses instead.
//!
...
/// 2026-09-26: 2026-08-05T00:56:22Z, the fixture's `recorded_at`.
...
fn the_rerun_name_carries_the_time_and_keeps_every_suffix_in_place() {
    ...
    // 2026-09-26: A lexical sort, which `bench_card` uses, puts the re-run
    // after the record it follows: `-` sorts before `T`.
    assert!("2026-08-05-b72dad1893.json" < "2026-08-05T005622Z-b72dad1893.json");
}

/// 2026-09-26: A FAIL then a same-day PASS at the same commit: both files
/// exist, and `records_newest_first` puts the PASS first.
#[test]
...
/// 2026-09-26: A PASS is replaced in place by a same-day re-run, including by
/// a FAIL.
#[test]
...
fn a_second_failure_is_kept_beside_the_first_and_a_rewrite_of_one_is_refused() {
    ...
    // 2026-09-26: A third write in the same second, with both names holding a
    // FAIL, is refused.
...
/// 2026-09-26: A shard's re-run name keeps its `-s0of2` suffix, and
/// `GateRecord::shard()` reads the same `(0, 2)` from both records.
#[test]

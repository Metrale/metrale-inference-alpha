...
All notable changes to Metrale Engine are documented here. The format is based on
...
[Metrale Engine Spark Journey](docs/METRALE_SPARK_JOURNEY.md).

## [Unreleased]

### Added
- `met benchmark <list|run|history>` — the dashboard's benchmark suite as a
...
- **Serving defaults are now per-hardware-target and live in the repository.**
  ...
  configuration with no `METRALE_*` prefix at all, and prints one
  ...
  `metrale-kernels/tests/target_defaults.rs`, so GB10 behaviour is unchanged. The
  first lever to differ is `ssm_batched_recurrent`, which `kernels/hopper`
  declares ON.
- **`METRALE_SSM_BATCHED_RECURRENT=0` now means OFF.** It was read as `== "1"`,
  ...
  without editing a launch script. `VAR=1` is unchanged, and the `METRALE_NO_*`
  kill switches stay presence-gated. `METRALE_GDN_PREFILL_TC` joins it as
...
- `--gpu-memory-utilization` now enforces a hard ceiling on total GPU
  ...
  caps Metrale Engine at ~60 GB total.  (#180)

## [0.1.0] — 2026-05-06

Initial public release. Metrale Engine is a pure-Rust LLM inference engine
...
[`docs/METRALE_SPARK_JOURNEY.md`](docs/METRALE_SPARK_JOURNEY.md) and the

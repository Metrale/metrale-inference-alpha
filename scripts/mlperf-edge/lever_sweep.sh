...
#
#  (METRALE_DECODE_GRAPHS_MULTISEQ was investigated and REJECTED before costing a
...
#
#  noregres METRALE_NO_GDN_REGRESIDENT=1   (NOTE: this leg REMOVES the lever)
...
#           it is to take it away. A positive `METRALE_GDN_REGRESIDENT` does not
...
#
#  bf16proj METRALE_BF16_TC_PROJ=1
...
#           sibling METRALE_BF16_TC_PREFILL=1, so today's config is asymmetric:
...
# (METRALE_DECODE_GRAPHS_MULTISEQ is the exception — it is a strict `== "1"` —
# but it is treated the same way here so the pattern cannot be got wrong.)
# `noregres` is the one leg that INVERTS this: METRALE_NO_GDN_REGRESIDENT is a
...
#
# Usage: lever_sweep.sh <metrale_bin> <outdir> [legs...]   default: all four
set -u
BIN="${1:?path to the built met binary}"
OUT="${2:?output dir}"
shift 2
...
extra_for() {
  case "$1" in
    control)  echo "" ;;
    graphs)   echo "-e METRALE_DECODE_GRAPHS_MULTISEQ=1" ;;
    noregres) echo "-e METRALE_NO_GDN_REGRESIDENT=1" ;;
    bf16proj) echo "-e METRALE_BF16_TC_PROJ=1" ;;
    *) echo "UNKNOWN_LEG" ;;
  esac
}
...
for leg in "${LEGS[@]}"; do
  ...
  sudo docker rm -f metrale-lever >/dev/null 2>&1; sleep 3
  # shellcheck disable=SC2086
  sudo docker run -d --name metrale-lever --network host --gpus all --ipc=host \
    ...
    -e METRALE_SSM_TAIL_LEASE_TTL=128 -e METRALE_BF16_TC_PREFILL=1 $EXTRA \
    -v "$HOME/.cache/huggingface:/root/.cache/huggingface:ro" \
    -v "$BIN:/usr/local/bin/met:ro" \
    metrale-gb10:followups serve "$MODEL" \
  ...
  for _ in $(seq 1 180); do
    ...
    sudo docker ps --format '{{.Names}}' | grep -q metrale-lever || { echo "SERVE_DIED leg=$leg"; break; }
    sleep 5
  done
  [ $ok -eq 1 ] || { sudo docker logs metrale-lever 2>&1 | tail -30 > "$OUT/$leg.died.txt"; continue; }

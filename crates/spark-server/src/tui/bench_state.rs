...
//! pane over `~/.metrale/runs`. Nothing here awaits: the executor owns
...
pub struct BenchState {
    ...
    meta: Option<&'static metrale_plugin::PluginMetadata>,
    ...
    /// empty for `met serve` with no model — so runs recorded a blank model
    ...
    executor: Option<metrale_plugin::BenchmarkExecutor>,
    ...
    /// just its id.
    running_descriptor: Option<&'static metrale_plugin::BenchmarkDescriptor>,
    /// Whether to require a coherent endpoint before measuring. Defaults to
    /// requiring it; `p` in the form toggles.
    pub coherence: metrale_plugin::CoherencePolicy,
    ...
    pub history: Vec<metrale_plugin::RunRecord>,
...
impl BenchState {
    ...
    pub fn attach(&mut self, executor: metrale_plugin::BenchmarkExecutor, target: TargetEndpoint) {
    ...
    pub fn plugin_metadata(&self) -> &'static metrale_plugin::PluginMetadata {
    ...
    pub fn begin_start(&mut self) -> Result<(), String> {
        ...
        if self.coherence == metrale_plugin::CoherencePolicy::Skip {
            return self.start();
        }
        let executor = self
            .executor
    ...
    ///
    /// Goes through `metrale_plugin::history`, the same writer the CLI uses, so a
    ...
    fn persist(&mut self, frame: &BenchmarkResult) {
        ...
        let mut record = metrale_plugin::RunRecord::new(
            descriptor,
            &self.values,
            &self.target,
            ...
            Default::default(),
            metrale_plugin::RunSource::Tui,
            crate::cli::METRALE_VERSION,
            frame.clone(),
        );
        if let Err(e) = metrale_plugin::history::save(executor.artifacts(), &mut record) {
...
const FALLBACK_METADATA: metrale_plugin::PluginMetadata =
    metrale_plugin::PluginMetadata::metrale("no benchmark selected");

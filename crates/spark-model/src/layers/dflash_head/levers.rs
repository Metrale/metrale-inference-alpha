...
///
/// Plain `Copy` data. Every field is a pure function of one `METRALE_*`
...
pub struct DFlashLevers {
    ...
    ///
    /// ★ Presence, not truth — `METRALE_DFLASH_BLOCK_DUMP=0` suppresses graph
    ...
    /// `METRALE_DFLASH_BLOCK_DUMP=<invalid utf-8>` used to leave capture ON.
    ...
    // ── One-shot dumps ──
    /// `METRALE_DFLASH_DEBUG_DUMP=1` — first 10 BF16 floats of each key
    /// intermediate, for element-wise comparison against a Python reference.
    pub debug_dump: bool,
    /// `METRALE_DFLASH_DEBUG_DUMP_FULL=1` — full tensors, not the first 10.
    pub debug_dump_full: bool,
    /// `METRALE_DFLASH_LOG_DRAFTS=1` — log the γ drafts each propose returns.
    pub log_drafts: bool,
    /// `METRALE_DFLASH_BLOCK_DUMP=1` — per-layer `.bin` dumps of the block
    /// inputs and every layer's output.
    pub block_dump: bool,
    /// `METRALE_DFLASH_BLOCK_DUMP_AT_POS=<n>` — arm the block dump only at
    ...
    /// config (#649); `METRALE_DFLASH_OPTION_B=0` is the kill switch.
    ///
    ...
    pub option_b: bool,
    /// `METRALE_DFLASH_OPTION_B_NO_CTX=1` — force `ctx_count = 0` in the layer
    ...
    /// the components; `METRALE_DFLASH2=0` disables.
    pub dflash2: bool,
    /// `METRALE_DFLASH_BATCH_PROPOSE=<width>` caps the cross-sequence batch.
    ...
    /// `METRALE_DFLASH_DRAFT_CAP=<n>` — submit at most n drafts per propose.
    /// `None` means the head's own γ.
    pub draft_cap: Option<usize>,

    // ── Propose-path diagnostics ──
    /// `METRALE_DFLASH_VERIFY_TRACE=1` — log all γ drafts BEFORE the cap, so
    /// an echo at position 0 can be told from an echo on every noise row.
    pub verify_trace: bool,
    /// `METRALE_DFLASH_PRECOMPUTE_DUMP=1` — one-shot dump of the fused ctx K/V
    /// GEMM inputs and outputs.
    pub precompute_dump: bool,
    /// `METRALE_DFLASH_CTX_PARITY_DUMP=1` — one-shot dump of the accumulated
    /// ctx hidden rows for a PyTorch parity diff.
    pub ctx_parity_dump: bool,
    /// `METRALE_DFLASH_DEBUG_NO_DECODE_APPEND=1` — skip the post-decode ctx
    /// append entirely.
    pub no_decode_append: bool,
    /// `METRALE_DFLASH_DEBUG_FULL_PRECOMPUTE=1` — recompute the whole ctx
    ...
    /// `METRALE_DFLASH_CTXLEN_PROBE=1` — assert `ctx_positions` is strictly
    ...
    /// head; `METRALE_DSPARK_MARKOV=0` degrades to the batched argmax path
    /// bit-for-bit.
    pub dspark_markov: bool,
    /// `METRALE_DSPARK_CONF_TAU=<t>` — sigmoid-space acceptance threshold for
    /// the confidence head. `0.0` (unset) disables the head entirely,
    /// matching the reference's `threshold <= 0.0 -> full block`.
    pub conf_tau: f32,
    /// `METRALE_DSPARK_SHIFT=1|0` forces the SpecForge shifted-row convention
    ...
    /// `METRALE_DSPARK_ANCHOR_BIAS=0` exempts it. Confidence truncation reads
    /// this too — rows without the chain never write their confidence slot.
    pub dspark_anchor_bias: bool,
    /// `METRALE_DSPARK_CONF_TRACE=1` — log the confidence logits and sigmoids.
...
impl DFlashLevers {
    ...
    /// What a head resolves to with no `METRALE_*` set: every diagnostic off,

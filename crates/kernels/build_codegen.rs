...
//! 2026-09-25: `target_ptx.rs` code generation and CUDA / SCALE toolkit discovery for build.rs.
//!
//! Owner: metrale-kernels build.
//! Invariants: none beyond the types.
//!
//! Included via `#[path = "build_codegen.rs"] mod build_codegen;` so types
//! from build.rs (`Target`, `SamplingCat`) are reachable via `super::`.
...
/// 2026-09-25: Generate the target_ptx.rs source with per-target kernel
/// constants and lookup functions.
///
/// Kernel blobs are emitted uniformly as `&[u8]` via `include_bytes!`: PTX is
/// text but valid as bytes, and the other backends produce binary objects.
///
/// `cuda_api` is true for backends that load kernels via the CUDA module
/// API (NVIDIA, SCALE, HIP); for those the real `all_ptx_sets()` registry is
/// generated. Metal passes false and gets an empty `all_ptx_sets()` stub
...
pub(super) fn generate_target_ptx_rs(
    ...
    // 2026-09-25: `const_ty` drops the `'static` (consts default to `'static`;
    // clippy::redundant_static_lifetimes); `modules_ty` keeps it (no elision
    // is possible in a function return type).
        ...
        for (stem, module_name) in modules {
            ...
            g.push_str(&format!(
                "pub const {const_name}: {const_ty} = \
                 {include_macro}(concat!(env!(\"METRALE_PTX_DIR\"), \"/t{idx}__{stem}.{output_ext}\"));\n"
            ));
        }
        g.push('\n');
    ...
    // 2026-09-25: Multi-target builds: `ptx_modules()` aliases the first target.
    if !single_target {
        g.push_str(
    ...
    // 2026-09-25: `metallib_modules()` is a plain alias of `ptx_modules()`
    // under the Metal-side name, so metal-feature consumers compile.
    g.push_str(
    ...
    // 2026-09-25: Non-CUDA backends (Metal) get an empty `all_ptx_sets()` so the
    // shared surface still type-checks.
    if !cuda_api {
        g.push_str(
    ...
    for (idx, target) in targets.iter().enumerate() {
        // 2026-09-25: HARDWARE.toml declares nvcc's feature arch (sm_121f,
        // sm_90a). The registry records both readings of it
        // (`build_arch::target_arch_fields`): `KernelTarget.arch` is the base SM
        // (sm_121, sm_90); `ptx_arch` is the declaration verbatim, which the GPU
...
/// 2026-09-25: Locate the SCALE install root for the `amd` / `rocm` / `scale`
/// vendors: a directory holding `bin/scaleenv` and a `targets/` dir.
///
/// Resolution order: `$SCALE_HOME` / `$SCALE_ROOT` (which must be a root, or
/// the build panics), then a short list of conventional locations, then every
/// `scale*` directory directly under `$HOME` or `/opt` and one level below it.
/// Panics if none is a SCALE root.

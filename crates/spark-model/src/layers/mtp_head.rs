...
pub struct MtpHead {
    ...
    /// (`METRALE_NO_LMHEAD_TGEMM=1`), or under the propose-local kill switch
    /// `METRALE_NO_MTP_LMHEAD_TGEMM` (PRESENCE — `=0` is NOT off). Zero extra
    ...
    /// override `METRALE_PROPOSE_META_STRIDE=<bytes>`). The fixed 2048 capped
    ...
    /// Drafter-prefill scratch; `None` unless METRALE_MTP_DRAFTER_PREFILL=1.
...
/// * With `refeed_accepted` (METRALE_MTP_REFEED_ACCEPTED), the ACCEPTED rows

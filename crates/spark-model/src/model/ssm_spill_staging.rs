...
//! 66,846,720 B) with `METRALE_SSM_TIER_TIMING=1`:
//!
//! ```text
...
///
/// Allocated on FIRST use: the tier is default-off (`METRALE_SSM_TIER` unset), so
...
impl StagingGuard<'_> {
    ...
    /// `"pinned"` / `"heap"` for the `METRALE_SSM_TIER_TIMING` line.

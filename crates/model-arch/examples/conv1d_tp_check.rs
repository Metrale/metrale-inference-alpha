...
//! 2026-09-25: Bit-exactness and speed check of the token-parallel
//! `causal_conv1d_update_prefill_tp` against the serial
...
fn main() -> Result<()> {
    ...
    let run = |kern: &str, grid: [u32; 3], blk: [u32; 3]| -> Result<(Vec<u16>, f64)> {
        let ps = up_f32(g, &s0)?; // 2026-09-25: each kernel starts from its own copy of `s0`.

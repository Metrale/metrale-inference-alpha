...
//! 2026-09-25: Dry run of a Kimi K3 safetensors `index.json` weight map, loading no tensors:
//! lists the distinct shard files, and fails when a class of `TEXT_CLASSES` is missing or a key
...
/// 2026-09-25: Result of walking a K3 `weight_map`. In a returned report `missing_text_classes`
/// and `unknown_keys` are always empty: either one being non-empty is an error instead.
#[derive(Debug, Clone)]
pub struct KimiK3DryRun {
    pub shards: Vec<String>,
    pub present_text_classes: Vec<String>,
    pub missing_text_classes: Vec<String>,
    /// 2026-09-25: Vision and mm_projector classes seen; filled only when `language_model_only`
    /// is set.
    pub ignored_vision_classes: Vec<String>,
    pub unknown_keys: Vec<String>,
}

/// 2026-09-25: Parse a safetensors `index.json` and dry-run its `weight_map`.
...
/// 2026-09-25: Fail unless the map names exactly `n` shard files. The TSV in `docs/k3` records
/// 96 shards for the full checkpoint.
#[cfg(test)]
...
mod tests {
    ...
    /// 2026-09-25: Synthetic 96-shard map: one key per text class, plus keys of the first class
    /// until every shard file appears.

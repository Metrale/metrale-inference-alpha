...
//! TIERED-CACHE-CONSOLIDATION §4 fix, step 3: the `METRALE_SSM_TIER_UNIFIED`
...
/// Opt-in truthy parse for `METRALE_SSM_TIER_UNIFIED` (style-matching
/// `METRALE_HSS_COALESCE_WRITE_RUNS` in spark-storage/high_speed_swap.rs).
...
/// ([`metrale_tier::Residency`] — two-level LRU, never rejects) instead of the
...
/// 3. **Swap files leak.** Flag-ON swap files (`metrale-ssm-{tag}.{pid}.swap`,
///    `metrale-decode-ring.{pid}.swap`) are per-PID and never unlinked, and the disk
...
// ─────────────────────────────────────────────────────────────────────────
// §4 unification (TIERED-CACHE-CONSOLIDATION step 3) — METRALE_SSM_TIER_UNIFIED
//
...
// lifted from the peer (`metrale_tier::Residency`: LRU over a hot arena, spill
...
/// the [`metrale_tier::SlotArena`] hot-tier seam: slot `i` lives at offset
...
impl metrale_tier::SlotArena for TransportSlotArena {
...
/// The flag-ON [`SnapshotBlobStore`]: a `Mutex`-shared [`metrale_tier::Residency`]
...
pub(crate) struct UnifiedSnapshotStore {
    inner: Mutex<
        metrale_tier::Residency<Box<dyn metrale_tier::SlotArena>, Box<dyn metrale_tier::SwapStore>>,
    >,
    blob_bytes: usize,
    ...
    /// because `metrale-tier`'s dependency budget is `anyhow` (+libc) — no
    /// `tracing` — by design.
...
impl UnifiedSnapshotStore {
    ...
    pub(super) fn new(
        arena: Box<dyn metrale_tier::SlotArena>,
        swap: Box<dyn metrale_tier::SwapStore>,
        blob_bytes: usize,
    ) -> Result<Self> {
    ...
    pub(super) fn new_capped(
        ...
        let residency = metrale_tier::Residency::new_capped(arena, swap, max_disk_slots)?;
        Ok(Self {
...
/// The unified stores' swap tier. `METRALE_SSM_TIER_SWAP_DIR` selects the lifted
...
pub(super) fn build_unified_swap(
    ...
    (
        Box::new(metrale_tier::MemSwapStore::new(blob_bytes)),
        SwapBacking::HostRam,
    )
}

/// Hot-arena slot count for the unified stores (`METRALE_SSM_TIER_SLOTS`,
...
/// `METRALE_SSM_DECODE_TIER` arm in selectors.rs.
///

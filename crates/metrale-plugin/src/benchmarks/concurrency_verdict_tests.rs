...
fn a_clean_sweep_that_clears_every_floor_passes() {
    ...
    // at 512/320 and describe nothing here, so each is the published Metrale Engine
    ...
    // The fixture below is the PUBLISHED Metrale Engine ladder, which every re-cut
...
fn a_sweep_below_one_floor_fails_naming_the_cell() {
    ...
    // Metrale Engine value except C=8, which is derived a hair under its own committed

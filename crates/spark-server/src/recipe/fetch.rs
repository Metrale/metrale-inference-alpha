...
//! `metrale-plugin/src/executor.rs`, both of which legitimately spawn tokio
...
pub(super) const AGENT: &str = concat!("metrale-spark/", env!("CARGO_PKG_VERSION"));
...
/// The recipe cache under `root`, which is the Metrale Engine home:
/// `<root>/metrale-recipes`.
///
...
pub fn cached(root: &Path) -> Index {
    ...
    let text = match std::fs::read_to_string(&path) {
        ...
        // Measured case: `$HOME/.metrale` created by uid 1000 while the server
        // runs as uid 996. The index was complete and unreadable; `cached`
        // answered "no recipes", and `bench_selfstart` therefore told the
        // operator to run `met sync-recipes` to populate a file that was

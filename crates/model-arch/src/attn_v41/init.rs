...
//! 2026-09-25: [`AttnV41`] construction and teardown: kernel lookup, the RoPE
//! tables and the per-call workspaces (`new`), and their release (`free`).
//!
...
use crate::deepseek_v41_ref::attn::freqs_cis;
use crate::deepseek_v41_ref::compress::yarn_freqs_cis;
...
impl AttnV41 {
    pub fn new(gpu: &dyn GpuBackend, cfg: AttnV41Cfg) -> Result<Self> {
        ...
        // 2026-09-25: `n_heads * head_dim` is in the maximum because
        // `wo_a_grouped` quantises the whole rotated attention row at once.
        let kmax = cfg
            .dim
            .max(cfg.q_rank)
            .max(cfg.gw())
    ...
    /// 2026-09-25: The layer output buffer, bf16 `[max_tokens, dim]`: a pointer
    /// the captured decode steps bake.

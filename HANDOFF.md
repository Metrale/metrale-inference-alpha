...
### Gate records (`met benchmark --pull-request-gate-check`)
...
| `bfcl-subset` | **BLOCKED** — needs `qwen3.6/qwen3.6-27b-nvfp4-unsloth` PRed to `metrale-recipes`; fails loudly (exit 1) meanwhile |
...
- ~~**PR `qwen3.6-27b-nvfp4-unsloth` to `metrale-recipes`** — unblocks `bfcl-subset`.~~ **Done**: `bfcl-subset` has a passing record at `cc1ebf27` (88.04 / 88.74, n=995). The upstream recipe PR (branch `feat/qwen3.6-27b-nvfp4-unsloth`) is still an outward action needing owner sign-off, but it no longer blocks a gate.
...
- **`.webmanifest` missing from metrale2's global `/etc/nginx/mime.types`** — patched for dez.rs only; every other PWA on that host has the same latent bug.
...
Image: **`metrale/metrale-inference-gb10:7241a95`** (id `c52999044e25`, binary 75,939,392 B = all-target `METRALE_TARGET_MODEL="*"` build).
...
Gate C's control was built fresh from `c19481aa` as `metrale-inference-gb10:mainctl388` (do **not** reuse `mainctl-tui` — it predates main's tip by two days).
...
- **New flags** in `cli/serve_args.rs`:
  ...
  - `METRALE_ALLOW_SHADOWED_KERNELS` **deleted** — one switch, not two.
...
|---|---|---|
| dgx1 | 10.10.10.1 (local) | full C=1..128 Metrale-vs-vLLM sweep on the final image |
| dgx2 | 10.10.10.2 | **owner's interactive TUI session — do not touch** |
| dgx3 | 10.10.10.3 | compile verification |

**Rules that are not negotiable:**
- **ONE bench per box.** Co-tenancy does not add noise, it **shifts the mean** — 16.3 GB of co-tenants cost Metrale Engine **32% at C=16** while costing vLLM ~0%.
...
- Always `METRALE_TARGET_MODEL="*"` (67 MB / 22 targets; a single-model build is ~52 MB and cannot serve other models).
...
### Configuration
8. ★ **`auto` is a DEFERRAL, not a value.** The 27B checkpoint declares `kv_cache_quant_algo: FP8`, so vLLM's `auto` resolved to fp8 while Metrale Engine ran bf16 — confounding both a speed and an accuracy result.
...
12. ★ **Use recipes.** An unpinned config does not fail loudly — it produces a plausible number. `metrale-recipes/.../qwen3.6-27b-nvfp4.yaml` already pins `disable_thinking: true`; the owner's serve deviated from it, which is exactly why no gate ever saw the 18 s.
...
20. ★ **`pgrep -f` self-matches** — your own `bash -c` contains the pattern you're grepping for. And `./target/release/met serve` does **not** match a `met serve` pattern.
...
| BFCL "regression" to ~83 | **Not a regression** — Metrale Engine +0.37 ahead on a common basis; 87.24 was inflated by a 4096-context sample exclusion |
...
| #92 | Metrale Engine **over-calls tools** where vLLM abstains — the whole BFCL gap is hallucination/irrelevance, not construction | accuracy lever |
...
`/usr/local/bin/met:ro` (image binary predates `--check-kernels`), weights at `/model`, and
...
`step3p7/load_layers.rs:414`. Fixed in `metrale-core/src/config/methods.rs`: sliding-attention
...
- `--check-kernels` exit code = unresolved count (clamped 255, clamp announced); JSON blob line `{"metrale_kernel_check": …}` on stdout.
- Tight-memory targets (122B, Super-120B, Mistral) need check-only flags: `--max-seq-len 512 --max-batch-size 1 --max-num-seqs 1 --ssm-cache-slots 0 --gpu-memory-utilization 0.99 --oom-guard-mb 512`. 122B-class also needs `METRALE_KV_OVERCOMMIT` default behavior. **Never serve with these flags.**
- dgx3 quirk: run met with `LD_LIBRARY_PATH=/home/claude/ttft-llama` (libnccl.so.2) or the binary won't load.

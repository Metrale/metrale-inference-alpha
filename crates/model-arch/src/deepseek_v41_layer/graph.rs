...
//! 2026-09-25: The captured single-token step (`METRALE_DS41_GRAPH=1`): the
//! step split into graph segments around the host work in the middle of every
//! layer, the segment runner (capture and replay), and the oracle that checks
//! a replayed step against the eager path.
//!
...
use crate::attn_v41::{AttnV41, AttnV41LayerState};
use crate::moe_v41::MoeV41;
use metrale_model_layers::layer::ForwardContext;
use metrale_model_layers::layers::ops;

impl DeepSeekV41Layer {
    /// 2026-09-25: Segment A's opening: the attention site's mixes, the
    /// delayed-pre collapse and the attention norm into `normed`. Device work only.
    ...
    /// 2026-09-25: Segment A's close: `hc_post` of the attention, the ffn
    /// site's mixes, the collapse, the ffn norm into `normed`, the router
    /// launch (`MoeV41::route_launch`). Device work only.
    ...
    /// 2026-09-25: Segment B: the expert compute for the `ne` staged experts
    /// (`MoeV41::compute_m1`), `hc_post`, the delayed-mix copy, and on the
    /// last layer the final collapse. Device work only.
    ...
    /// 2026-09-25: Replay `slot`'s graph, or capture `body` into it and launch
    /// it. Once captures are disabled, `body` runs eagerly. A capture that
    /// cannot begin or end runs `body` eagerly and disables further captures.
    /// An error inside the body aborts the capture; a capture-state error
    /// (status 900 / 901, `STREAM_CAPTURE`) then runs `body` eagerly and
    /// disables captures, any other error propagates.
    pub(super) fn run_segment(
        ...
        if let Err(e) = body() {
            ...
            // 2026-09-25: Under capture the body was recorded, not run, so
            // running it eagerly now is the step.
            tracing::warn!(
    ...
    /// 2026-09-25: The captured single-token step: segment A (replayed or
    /// captured), the host span (`MoeV41::stage_m1`), segment B. The per-token
    /// inputs are read from device buffers the host refills before each
    /// replay, and `sparse_attn` runs at a fixed, -1-padded `topk`;
    /// `step_oracle` compares the result with the eager step bit for bit.
    pub(super) fn step_graph(
        ...
        // 2026-09-25: The host half of the attention: position and padded
        // selection onto the device (only what changed since the last upload).
        ...
        if capturable {
            ...
            // 2026-09-25: A kv/index source: the compressor group and the index
            // top-k are host work in the middle of the attention, so it runs
            // eagerly.
            ...
            // 2026-09-25: It wrote pos / head_pos / idx_dev for itself.
    ...
    /// 2026-09-25: `METRALE_DS41_GRAPH_ORACLE=1`: the captured step, then the
    /// highway and delayed mix put back and the eager step, and the two
    /// results (the streams, the delayed mix, the collapsed hidden) compared
    /// bit for bit and logged. Diagnostics: every layer runs twice.
    pub(super) fn step_oracle(
        ...
        // 2026-09-25: The collapsed hidden is the layer's output only on the
        // last layer; elsewhere it is the ffn input scratch, which both paths write.
    ...
    /// 2026-09-25: The per-step diagnostic line (`METRALE_DS41_DIAG=1`), from
    /// the last layer.
    pub(super) fn step_line(&self, m: usize, start_pos: usize, how: &str) {
        ...
        // 2026-09-25: One guard at a time: two `lru.lock()` temporaries in a
        // single statement deadlock on the std Mutex (the first guard lives to

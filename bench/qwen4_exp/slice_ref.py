# 2026-09-26: Recomputes the engine's METRALE_QWEN4EXP_DUMP taps for the embedding and
# layer 0's GDN sublayer from the checkpoint and the reference GDN module, and compares
# each tap by cosine (OK above 0.999). Exits 1 if L00_in or L00_post_gdn differs.
#
...
So reproduce the same taps Metrale Engine writes (`METRALE_QWEN4EXP_DUMP`) and diff.

WHAT MAKES THIS AFFORDABLE. The obvious blocker is the 512-expert MoE on every
layer. Two things get around it:

  * Metrale Engine taps the highway at the SUB-LAYER boundary — after a block's
...
def main() -> int:
    ...
    ap.add_argument('--dump-dir', required=True,
                    help='directory METRALE_QWEN4EXP_DUMP wrote')
    ...
    # 2026-09-26: hc_expand copies the embedding into each of the hc streams.
    ...
    # 2026-09-26: Layer 0's GDN sublayer (hc_pre, GDN, hc_post) needs only layer 0's
    # attn_hyper_connection and linear_attn tensors; no experts.
    lp = f'{pfx}.layers.0'
    eps = tc['rms_norm_eps']
    ...
    # 2026-09-26: mixed is [T, H], inj is [T, hc].
    mixed = (w * normed.unflatten(-1, (hc, h))).mean(dim=-2)
    inj = 2 * torch.sigmoid(normed @ inject.T / hc)
    ...
    if got is not None:
        print(f'    metrale inj = {np.round(got[:hc], 6).tolist()}')
        print(f'    ref   inj = {np.round(inj[0].numpy(), 6).tolist()}  (token 0)')
        compare('L00 hc_pre inj', got, inj.numpy())

    # 2026-09-26: The GDN block is the reference module, loaded with layer 0's nine
    # linear_attn tensors.
    ...
    with torch.no_grad():
        ...
        # 2026-09-26: Wrap gdn.norm.forward to record the gated norm's output, which the
        # reference passes to out_proj.
        captured = {}
    ...
    block_out = block_out[0]
    # 2026-09-26: The projection check needs only in_proj_qkv and in_proj_z.
    ...
    if got is not None:
        ...
        # 2026-09-26: The engine taps the deinterleaved projection: Q, K, V, then Z.
        want_qkvz = torch.cat([mixed @ qkv_w.T, mixed @ z_w.T], dim=-1)
        compare('L00 qkvz preconv', got, want_qkvz.detach().numpy())

    # 2026-09-26: Causal depthwise conv (kernel width from conv1d.weight; 4 in the cached
    # config), then SiLU, over the Q, K and V channels only; the reference applies the
    # activation inside `causal_conv1d_fn` and does not convolve z.
    ...
    # 2026-09-26: The engine's gates tap holds, per token, nv gate values then nv beta
    # values, FP32.
    ...
    if got is not None:
        ...
        want_gates = torch.cat([g_ref, beta_ref], dim=-1)
    ...
    # 2026-09-26: hc_post: residual[t, s*H+d] += block_out[t,d] * inj[t,s]

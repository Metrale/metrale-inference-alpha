...
#
# Usage: run_replay_distance.sh <metrale_bin> <outdir> [conversation_id] [max_turns]
set -u
...
sudo docker rm -f metrale-replaydist >/dev/null 2>&1; sleep 3
sudo docker run -d --name metrale-replaydist --network host --gpus all --ipc=host \
...
for _ in $(seq 1 180); do
  ...
  sudo docker ps --format '{{.Names}}' | grep -q metrale-replaydist || { echo SERVE_DIED; exit 1; }
  sleep 5
done
...
sudo docker logs metrale-replaydist 2>&1 | grep -a "Marconi" > "$OUT/marconi.log" || true
...
PY
sudo docker rm -f metrale-replaydist >/dev/null 2>&1

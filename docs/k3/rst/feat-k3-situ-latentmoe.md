...
TEST NOTES
`cargo test -p metrale-core --lib -- kimi_k3::situ kimi_k3::latent_moe`
`cargo test -p metrale-model-layers --lib -- deny_kernel_resolve_bails_not_silent_cpu` (Linux)

BUGS
#N/A this slice.

TEST NOTES (review 1080)
Host unpack SSOT is `metrale_core::mxfp4_e8m0` (moved from `metrale_model_layers::weight_map::fp8_lut`). Same LUT, nibble order, exp=0/255 → 0.0. Gate tip: this commit on `feat/k3-situ-latentmoe`. GPU GEMM remains DSV4 extra_cu, not a second stack.

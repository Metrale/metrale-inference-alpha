...
EXTRA_DEFS = """    <clipPath id="cert-clip-a1"><rect x="64" y="272" width="780" height="36"/></clipPath>
    ...
    <linearGradient id="cert-fadeBg" x1="0" y1="0" x2="1" y2="0">
      <stop offset="0" stop-color="#0E1318" stop-opacity="0"/>
      <stop offset="0.85" stop-color="#0E1318" stop-opacity="1"/>
    </linearGradient>
...
class _Logo:
    """The kit's mark in the chevron logo's old box: `s` scales a 636-unit
    height, as the chevron template's `scale` did."""
    def format(self, x, y, s):
        return G.mark_svg(x, y, s)
...
def cell(x, y, w, label, value, vid, clip=None, vfill="#D9D9DE", fs=13.5):
...
def certificate(authors, **kw):
    ...
    return (
        ...
        + cells
        + '  <text x="964" y="596" font-size="9.5" fill="#8A8F99" font-weight="600" letter-spacing="1.5" text-anchor="end">SCAN TO READ</text>\n'

# 2026-09-26: Runs the reference PLE layer on a fixed token list with the checkpoint's
# weights and n-gram rows, and writes the ids, rows, intermediates and output to an npz,
# ple_id_fixtures.json, and with --bin-dir raw .bin files plus meta.json.
#
...
# 2026-09-26: resolve_snapshot tries QWEN4EXP_PATH, then DEFAULT_SNAP if it exists,
# then the HF cache.
...
# 2026-09-26: Tokens 0-1 repeat at 3-4, so two positions share an n-gram, and 248044
# (the cached config's eos_token_id) sits mid-list, so `_shift_right_ignore_eos` meets a
# segment boundary.
...
class IdRecorder(torch.nn.Module):
    ...
    @property
    def weight(self) -> torch.Tensor:  # 2026-09-26: the reference reads `.weight.device`.
...
def write_bins(out_dir: str, dump: dict) -> None:
    ...
    # 2026-09-26: ids and embeddings are read by ple_tests.rs `ple_gather_reads_the_right_rows`.
...
def main() -> int:
    ...
    ap.add_argument('--out', default=os.path.join(os.path.dirname(__file__),
                                                  'ple_golden.npz'))
    ap.add_argument('--bin-dir', default=os.environ.get('METRALE_PLE_TEST_DATA'))
    args = ap.parse_args()
    ...
    # 2026-09-26: ple_layer_ids is 1-indexed (the reference calls
    # `ple_layer_ids.index(layer_idx + 1)`), so [2] is layer_idx 1. The cached
    # checkpoint has `.ple.` tensors only under `layers.1`.
    ...
    # 2026-09-26: The constructor allocates the whole n-gram nn.Embedding (the cached
    # checkpoint has 128 shards of 2,500,012 rows x 160), so it is stubbed while the
    # module is built; the ids come from the reference's forward and the rows from the
    # shards.
    ...
    # 2026-09-26: The checkpoint carries layer_multipliers, ngram_heads_vocab_sizes
    # and ngram_heads_offsets. They replace the module's derived buffers, which
    # are only compared with them and printed.
    ...
    with torch.no_grad():
        ng(input_ids, None)
    ids = rec.ids[0].numpy().astype(np.uint64)   # 2026-09-26: [T, ngram_heads]
    t, n_heads = ids.shape
    ...
    # 2026-09-26: Keep the batch dim, so `ng.forward` returns [1, T, heads * head_dim]
    # and the `[0]` indexing below selects the batch, not token 0.
    rec.rows = torch.from_numpy(rows.reshape(1, t, n_heads, head_dim))

    # 2026-09-26: Same stub: `Qwen4ExpTextPLELayer` builds its own NGramEmbedding.
    _nn.Embedding = _StubEmbedding
    try:
    ...
    # 2026-09-26: `ple.ple_embedding` is the whole NGramEmbedding, not the inner table.
    # The one built above returns the checkpoint rows for these ids.
    ...
    # 2026-09-26: Intermediates. The projections and norms are the layer's own
    # submodules; the gate lines repeat `Qwen4ExpTextPLELayer.forward`.
    with torch.no_grad():
        # 2026-09-26: key_out is [1, T, hc*H], value_out [1, T, H].
        key_out = ple.key_proj(embeddings)
        value_out = ple.value_proj(embeddings)
    ...
    # 2026-09-26: Read by crates/model-layers/src/layers/ple/tests.rs, which needs no GPU
    # and no checkpoint.

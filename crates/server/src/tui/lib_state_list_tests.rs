...
//! 2026-09-26: Tests for the Library list: its order, the filter, and where the cursors land when a refresh changes the list.
//!
//! Owner: server tui.
...
fn what_can_be_run_right_now_sorts_to_the_top() {
    // 2026-09-26: Rank order: runnable, then recipe without weights, then
    // local-only.
    let s = state(
        vec![
...
fn a_half_finished_download_ranks_with_the_downloadable_not_the_runnable() {
    // 2026-09-26: A checkpoint with `has_weights: false` is not runnable, so
    // it ranks below one that is.
    let s = state(
        vec![
...
fn the_filter_matches_the_model_the_recipe_id_and_the_architecture() {
    // 2026-09-26: `Entry::matches` searches the model id, every recipe id and
    // the architecture.
    let mut s = state(
...
fn a_filter_that_hides_the_selected_row_leaves_no_dangling_selection() {
    ...
    s.selected = 2;
    s.filter = "org/a".into();
    // 2026-09-26: `selected` indexes the filtered list, so until a re-clamp it
    // can point past the end; `current` returns `None` then.
...
fn a_refresh_that_lengthens_the_list_keeps_the_cursor_on_the_same_model() {
    // 2026-09-26: The new rows sort ahead of the selected one; the cursor
    // follows its model.
...
fn a_row_becoming_runnable_moves_the_cursor_with_it() {
    // 2026-09-26: A row that gains weights moves from rank 1 to rank 0; the
    // cursor follows it.
    let mut s = state(
        vec![
...
fn the_filter_is_applied_before_the_selection_is_anchored() {
    // 2026-09-26: `rebuild` anchors on the visible list, so a refresh while a
    // filter is set keeps the cursor on a visible row.
...
fn a_refresh_that_shortens_the_form_cannot_leave_the_row_cursor_dangling() {
    // 2026-09-26: `row` indexes the config form, which a recipe with fewer
    // keys shortens.
    let mut s = state(
...
fn an_index_refresh_that_changes_nothing_leaves_the_cursor_exactly_where_it_was() {
    // 2026-09-26: A refresh that returns the same index keeps the selection,
    // the form row and the view.
    let recipes = vec![

...
/// `metrale-recipes` repo (`ci.yml`), and `invalidating_paths` diffs *this* one.
...
/// the whole `crates/metrale-plugin/src/gate` prefix from **all ten** gates, so
...
fn crates_paths_split_into_fully_covered_and_not_covered_at_all() {
    ...
    let machinery = required_for(
        &["crates/metrale-plugin/src/gate/telemetry.rs".to_string()],
        &[cat("performance/scheduling")],
        &roots,
    );

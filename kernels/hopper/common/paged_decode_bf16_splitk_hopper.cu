...
// 2026-09-25: Hopper paged-decode split-K for a BF16 KV cache, and its reduce.
//
// Owner: hopper kernels.
// Invariants: those of paged_decode_splitk_hopper.cuh, which holds the split
// and warp partitions, the partial write and the reduce.
//
// The structure of `paged_decode_fp8_splitk_hopper.cu`, with the BF16 unpack
// in place of the FP8 dequant and no k/v scales. The page stride is
// computed from block_size, num_kv_heads and head_dim, as gb10's
// `paged_decode_attn` computes it, instead of being passed as `cache_stride`.
//
...
extern "C" __global__ void paged_decode_attn_splitk_bf16_hopper(
    ...
    const __nv_bfloat16* __restrict__ K_cache,  // 2026-09-25: [blocks, block_size, kv_heads, hd] BF16
    ...
    // 2026-09-25: 1.0f: a BF16 cache has no v_scale. An empty split emits l = 0, which
    // the reduce skips.
...
// 2026-09-25: Reduce the BF16 twin's partials. Grid: (num_q_heads, num_seqs, 1) Block: (32,1,1)
//
// The body is the FP8 twin's (`pd_reduce_body`); the kernel is separate because
// a module is a file and the two twins are separate modules.

...
// Hopper native-input M128 trial, bounded by adaptive_fp8's existing plan.
// Same per-row reduction as native-input M16 in the standalone oracle;
// NOT bit-identical to original BF16 PM4. Retains K64 promotion and scales.
// The BF16 M128 module remains available for all other shapes and fallback.
#include <cuda_bf16.h>
#include <cuda_fp16.h>
#define NL128_M_TILE 128
#define NL128_N_TILE 64
#define NL128_K_STEP 32
#define NL128_K_SUB 16
#define NL128_K_SUBS (NL128_K_STEP / NL128_K_SUB)
#define NL128_PAD 2
#define NL128_A_STRIDE (NL128_K_STEP + 8)
#define NL128_THREADS 256
#define NL128_NT_PER_WARP (NL128_N_TILE / 8)
#define NL128_STAGES 2
#define NL128_K_PROMOTE 64
#define NL128_FP8_BLOCK 128

__device__ __forceinline__ __nv_bfloat16 nl128_e4m3_to_bf16(unsigned char b) {
...
__device__ __forceinline__ void nl128_cp_async_cg_16(void* smem_ptr, const void* gmem_ptr) {
    unsigned int s = (unsigned int)__cvta_generic_to_shared(smem_ptr);
    asm volatile("cp.async.cg.shared.global [%0], [%1], 16;\n" ::"r"(s), "l"(gmem_ptr));
}
__device__ __forceinline__ void nl128_cp_async_commit() {
    asm volatile("cp.async.commit_group;\n" ::);
}
template <int N>
__device__ __forceinline__ void nl128_cp_async_wait_group() {
    asm volatile("cp.async.wait_group %0;\n" ::"n"(N));
}
__device__ __forceinline__ void nl128_cp_async_wait_le(unsigned int n) {
    switch (n) {
        case 0:  nl128_cp_async_wait_group<0>(); break;
        case 1:  nl128_cp_async_wait_group<1>(); break;
        default: nl128_cp_async_wait_group<2>(); break;
    }
}
...
__device__ __forceinline__ unsigned nl128_sanitize(unsigned packed) {
...
__device__ __forceinline__ void nl128_mma_kstep(
 const unsigned char* A, const unsigned char* B,
 float inner[NL128_NT_PER_WARP][4], unsigned warp_m_offset,
 unsigned group_id, unsigned tid) {
 const unsigned r0=warp_m_offset+group_id,r1=r0+8;
 unsigned a0=nl128_sanitize(*(const unsigned*)&A[r0*32+4*tid]);
 unsigned a1=nl128_sanitize(*(const unsigned*)&A[r1*32+4*tid]);
 unsigned a2=nl128_sanitize(*(const unsigned*)&A[r0*32+16+4*tid]);
 unsigned a3=nl128_sanitize(*(const unsigned*)&A[r1*32+16+4*tid]);
 #pragma unroll
 for(unsigned nt=0;nt<NL128_NT_PER_WARP;nt++) {
  unsigned row=nt*8+group_id;
  unsigned b0=nl128_sanitize(*(const unsigned*)&B[row*32+4*tid]);
  unsigned b1=nl128_sanitize(*(const unsigned*)&B[row*32+16+4*tid]);
...
extern "C" __global__ void __launch_bounds__(NL128_THREADS, 2) pm4_native128(
    ...
    __shared__ __align__(16) unsigned char smem_Araw[NL128_STAGES][NL128_M_TILE][NL128_K_STEP];
    __shared__ __align__(16) unsigned char smem_Braw[NL128_STAGES][NL128_N_TILE][NL128_K_STEP];
    ...
    for (int wid = blockIdx.x; wid < total; wid += (int)gridDim.x) {
        ...
        const unsigned int cta_m_local = mt * NL128_M_TILE;
        const unsigned int cta_n = nt * NL128_N_TILE;

        float inner_acc[NL128_NT_PER_WARP][4];
        float outer_acc[NL128_NT_PER_WARP][4];
        #pragma unroll
        for (int i = 0; i < NL128_NT_PER_WARP; i++) {
        ...
        const unsigned int k_blocks = (K + NL128_FP8_BLOCK - 1) / NL128_FP8_BLOCK;
        const unsigned int n_block = cta_n / NL128_FP8_BLOCK;
        const unsigned int n_steps = (K + NL128_K_STEP - 1) / NL128_K_STEP;
        const unsigned int steps_per_promote = NL128_K_PROMOTE / NL128_K_STEP;   // 2
        ...
        auto prefetch = [&](unsigned int step, unsigned int stage) {
            unsigned int k_base = step * NL128_K_STEP;

            // A raw: 128 rows x K_STEP FP8 bytes, 16-B chunks, gathered per row.
            const unsigned int a_chunks = (NL128_M_TILE * NL128_K_STEP) / 16;   // 256
            #pragma unroll
            for (unsigned int c = threadIdx.x; c < a_chunks; c += NL128_THREADS) {
                unsigned int row  = c / (NL128_K_STEP / 16);
                unsigned int kcol = (c % (NL128_K_STEP / 16)) * 16;
                ...
                if (m_global < (unsigned int)M_expert && gk + 16 <= K) {
                    ...
                    nl128_cp_async_cg_16(dst, &A_fp8[(unsigned long long)token_id * K + gk]);
                } else {
                    #pragma unroll
            ...
            const unsigned int b_chunks = (NL128_N_TILE * NL128_K_STEP) / 16;   // 128
            #pragma unroll
            for (unsigned int c = threadIdx.x; c < b_chunks; c += NL128_THREADS) {
                unsigned int nrow = (c * 16) / NL128_K_STEP;
                unsigned int kcol = (c * 16) % NL128_K_STEP;
                unsigned int gn = cta_n + nrow;
                unsigned int gk = k_base + kcol;
                unsigned char* dst = &smem_Braw[stage][nrow][kcol];
                if (gn < N && gk + 16 <= K) {
                    nl128_cp_async_cg_16(dst, &B_exp[(unsigned long long)gn * K + gk]);
                } else {
                    #pragma unroll
                    for (unsigned int e = 0; e < 16; e++) {
                        unsigned int gke = gk + e;
            ...
            nl128_cp_async_commit();
        };

        #pragma unroll
        for (unsigned int p = 0; p < NL128_STAGES - 1; p++) {
            if (p < n_steps) prefetch(p, p % NL128_STAGES);
        }
        unsigned int k_step_in_prom = 0;

        for (unsigned int step = 0; step < n_steps; step++) {
            unsigned int cur = step % NL128_STAGES;

            unsigned int ahead = step + (NL128_STAGES - 1);
            if (ahead < n_steps) prefetch(ahead, ahead % NL128_STAGES);
            unsigned int committed = min(n_steps, NL128_STAGES + step);
            unsigned int target = committed - (step + 1);
            nl128_cp_async_wait_le(target);
            __syncthreads();   // raw A/B for `cur` resident for all threads

            nl128_mma_kstep(&smem_Araw[cur][0][0], &smem_Braw[cur][0][0],
            ...
            if (k_step_in_prom == steps_per_promote || step + 1 == n_steps) {
                const unsigned int k_block = (step * NL128_K_STEP) / NL128_FP8_BLOCK;
        ...
        #pragma unroll
        for (int n_tile = 0; n_tile < NL128_NT_PER_WARP; n_tile++) {

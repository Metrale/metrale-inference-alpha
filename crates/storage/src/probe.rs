...
//! 2026-09-26: Storage capability probe (`run_probe`): loads cuFile and opens its driver
//! when it can, checks for the nvidia-fs module, runs the `bench` read benchmarks
//! (cuFile's only when its driver opened), and recommends a `Backend` from the results
//! (`decide`). A benchmark that fails is logged and left out of the result.
//!
//! Owner: storage.
...
pub fn run_probe(cfg: &ProbeConfig) -> Result<ProbeResult> {
    ...
    let nvidia_fs = metrale_gpu_sys::cufile::nvidia_fs_loaded();
    let cufile_load = metrale_gpu_sys::cufile::CuFile::load();
    ...
    if let Some(cufile) = cufile.as_ref() {
        ...
        if r.err == metrale_gpu_sys::cufile::CU_FILE_SUCCESS {
            version = Some(v);
        }
        ...
        if r.err == metrale_gpu_sys::cufile::CU_FILE_SUCCESS {
            ...
            driver_open_err = Some(format!(
                "{} ({})",
                r.err,
                metrale_gpu_sys::cufile::err_to_str(r.err)
            ));
        }
    }

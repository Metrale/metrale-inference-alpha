...
//! CPU-only timing of grammar exported from Metrale Engine's native tool compiler.
use metrale_grammar::compiler::{
    CompiledGrammar, CompiledGrammarImpl, GrammarCompiler, RuleLevelCache,
};
use metrale_grammar::matcher::GrammarMatcher;
use metrale_grammar::tokenizer::TokenizerInfo;
...
fn main() {
    ...
    let meta = metrale_grammar::detect_metadata_from_hf(&raw).unwrap();

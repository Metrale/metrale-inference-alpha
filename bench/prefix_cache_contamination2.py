#!/usr/bin/env python3
# 2026-09-26: Checks that greedy completions from HEAD match a second server (WORKER,
# from METRALE_WORKER_URL) before and after HEAD serves an unrelated prompt (ALPHA)
# three times. Exits 1 on any mismatch.
#
# Owner: bench, prefix-cache probes.
...
WORKER = os.environ.get("METRALE_WORKER_URL", "http://127.0.0.1:8888")
...
VICTIMS = {
    # 2026-09-26: Differs from ALPHA at the first word, so it shares no prefix.
    "B_math_tutor": (
    ...
    # 2026-09-26: Repeats ALPHA's persona text word for word and differs only in the
    # question, so its prompt shares ALPHA's leading tokens.
    "D_shared_persona": (
        "You are a pirate named Captain Blackbeard. Answer every question "
        "in exaggerated pirate speech with 'arrr' and 'matey'.\n\n"
        "Q: Name three fruits.\nA:"

...
// of `depth` registered bounce buffers (env `METRALE_KV_PIPELINE_DEPTH`, default
// 16). With `METRALE_KV_DUAL_RAIL=1` the client opens 2 rails (env
// `METRALE_EXPERT_RDMA_DEV`/`GID` = rail 0, `METRALE_KV_RAIL2_DEV`/`GID` = rail 1)
...
pub struct RdmaKvBackend {
    ...
    /// Zero-copy restore (METRALE_KV_ZERO_COPY=1): RDMA READ lands directly into
...
impl RdmaKvBackend {
    ...
    /// [`metrale_rdma::railset::RailSet`], and allocate each rail's ring.
    pub fn connect(addr: &str, layout: GroupLayout) -> Result<Self> {
        ...
        // the KV rail-2 env. Dual-rail only when METRALE_KV_DUAL_RAIL=1. Fresh

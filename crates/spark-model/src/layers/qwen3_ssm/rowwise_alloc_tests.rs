...
//! Allocation contract for the two `METRALE_FP8_ROWWISE` GDN prefill arms —
...
//! `MetraleCudaBackend`s (`qsa_tests.rs`, `ngram_embed/tests.rs`), so a process
...
/// `METRALE_FP8_ROWWISE` ARMED, without touching the process environment:
...
/// configuration a `METRALE_FP8_ROWWISE=0` arena would present to row-wise

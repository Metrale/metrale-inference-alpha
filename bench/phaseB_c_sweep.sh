...
#   metraleB_nographs — the Phase B binary (SSOT ladder w/ 12+16, can_mix fusion
#                     fix, vectorized non-MTP sampler picks, bounded sends,
#                     detached terminal frames), graphs still OFF.
#                     vs metrale_synth (Phase A binary): the pure code delta.
#   metraleB_graphs   — same binary + METRALE_DECODE_GRAPHS_MULTISEQ=1 (strict
#                     =="1"): the n>=2 CUDA-graph lever on top.
#                     vs metraleB_nographs: the pure graphs delta.
#
...
# on serve failure. Serve geometry matches the Phase A metrale leg EXACTLY
...
teardown() { sudo docker rm -f metrale-bsweep >/dev/null 2>&1; sleep 3; }
...
run_leg() { # $1 leg name, $2 extra -e args
  ...
  sudo docker run -d --name metrale-bsweep --network host --gpus all --ipc=host \
  ...
  for _ in $(seq 1 200); do
    ...
    sudo docker ps --format '{{.Names}}' | grep -q metrale-bsweep || break
    sleep 5
  done
  if [ $ok -eq 1 ]; then
    ...
    sudo docker logs metrale-bsweep 2>&1 | grep -ac "graph" > "$CS/$leg.graphlines" || true
  else
    sudo docker logs metrale-bsweep 2>&1 | tail -60 > "$CS/$leg.deathlog" || true
...
run_leg metraleB_nographs ""
run_leg metraleB_graphs "-e METRALE_DECODE_GRAPHS_MULTISEQ=1"
note "PHASEB_DONE"
echo "PHASEB_DONE"

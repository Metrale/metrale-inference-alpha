...
//! sites, each with its own convention (`METRALE_SSM_H_FP16` presence-gated —
...
impl GdnFlags {
    ...
    /// used to be `METRALE_SSM_BATCHED_RECURRENT=1` in an H100 launch script
    /// outside this repository, which is the structure the 2026-09-11
    /// maintainer review asked for. `METRALE_SSM_BATCHED_RECURRENT` still
    ...
    /// to `1`.
    ///
    /// `METRALE_SSM_H_FP16` stays PRESENCE-gated here on purpose: that is how
...
/// `--ssm-h-dtype f16` (legacy `METRALE_SSM_H_FP16`).
...
/// `--gdn-fused-norm` (legacy `METRALE_GDN_FUSED_NORM=1`).
pub fn gdn_fused_norm_enabled() -> bool {
    flags().fused_norm
}

/// `--ssm-batched-recurrent` (legacy `METRALE_SSM_BATCHED_RECURRENT=1`).
...
/// 128-row M-tile. `None` (kill switch `METRALE_NO_SSM_M128`, PRESENCE check —
...
/// `METRALE_GDN_WOA=1`.** With it off, the parent wy4 kernel runs (writes every
...
mod tests {
    ...
    /// `METRALE_*` variable for it on purpose (house rule: no new env knobs),
    ...
    fn env_fallback_never_enables_exact_verify() {
        ...
        // legacy `METRALE_SSM_H_FP16=1` script keeps the FP32-sized pool.

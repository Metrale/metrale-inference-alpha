...
/* metrale-gpu-runtime red-zone allocator (cuda_backend.rs) reaches these two
...
# The Holo-3.1/Ornith enablement added cublaslt.rs (METRALE_CUBLAS_GEMM FFI)
# and a cudaMemcpy2DAsync path; build.rs emits -lcublasLt/-lcudart even
# under METRALE_SKIP_BUILD. Every symbol returns 1 so a real invocation

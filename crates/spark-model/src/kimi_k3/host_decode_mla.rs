...
use metrale_core::kimi_k3::{K3CpuModel, MixerKind};
...
fn mla_decode_keep_state(
    tokens: usize,
) -> anyhow::Result<(usize, usize, metrale_core::kimi_k3::LayerCache)> {
    ...
    let mut seed_state = K3CpuFallbackState::new(metrale_core::kimi_k3::LayerCache::Kda(
        metrale_core::kimi_k3::KdaState::new(&model.kda),
    ));
    seed.decode_host(
        hidden,
        DevicePtr::NULL,
    ...
    let mut state = K3CpuFallbackState::new(metrale_core::kimi_k3::LayerCache::Mla(
        metrale_core::kimi_k3::MlaKv::default(),
    ));
    ...
    let cache = metrale_core::kimi_k3::LayerCache::from_bytes(&bytes)?;
    state.release(&gpu)?;
...
fn serving_resident_mla_h2d_does_not_grow_and_snapshot_keeps_history() {
    ...
    let metrale_core::kimi_k3::LayerCache::Mla(kv) = cache else {

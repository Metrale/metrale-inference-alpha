...
//! 2026-09-25: Layer 0's attention against the golden, the three regimes run
//! in order through one window cache: `sa_q`, `sa_kv` (the rows attention
//! read: the chunk in prefill, the ring in decode), `sa_topk_idxs` exactly,
//! `sa_o`, and `attn_out` (a full-resolution capture).
//!
...
/// 2026-09-25: Run every regime in order through one cache; returns (regime,
/// start_pos, run).
...
fn query_and_kv_rows_match_within_bf16() {
    ...
    for (r, _, run) in run_all(&f, &w) {
        ...
        // 2026-09-25: Prefill reads the chunk's own rows, decode the whole ring,
        // so this checks the cache too.

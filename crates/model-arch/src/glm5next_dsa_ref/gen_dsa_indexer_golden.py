...
# 2026-09-25: Writes the kpool indexer golden (dsa_indexer_golden.json) from the transformers
# glm5_next indexer on real layer weights.
#
...
def run_indexer(m, hidden, q_resid, mask, q_len=None):
    ...
    # 2026-09-25: Fraction of per-head (query, head, pool) entries the ReLU clamps. Measured here,
    # not on `index_scores`: that is a weighted sum over the heads, zero only when every head
    # clamped.
    ...
    # 2026-09-25: Each row sorted. The indices become a mask (`topk_to_mask`), so their order
    # carries no meaning, and a positional comparison of sorted rows compares sets.
    st["topk_sorted"] = torch.sort(topk.to(torch.int64), dim=-1).values.clone()
    tk64 = topk.to(torch.int64)
    valid_tk = tk64.ge(0)
    st_meta = {
        # 2026-09-25: Per-row set digests: the sum and count of the valid entries, which ignore
        # order.
    ...
    # 2026-09-25: Ties at the select_k cutoff, the only ties that change the selected set. The
    # ReLU turns every negative score into exactly 0.0, so the cutoff can fall inside a block of
    # equal zeros.
    sc = index_scores[0]
...
# 2026-09-25: (name, seq_len, leading_pad, q_len_for_decode, negate_q). `relu_probe` negates the
# query so dot products go negative and the ReLU clamps them.

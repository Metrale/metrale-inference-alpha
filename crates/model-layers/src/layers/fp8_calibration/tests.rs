...
//! 2026-09-25: Tests of the glue around the calibration state machine in
//! [`super::Fp8KvCalibration`], on `MockGpuBackend`, plus source pins on the
//! attention initializer and the graph-suppression call sites.
//!
...
/// 2026-09-25: Kernel-arg count of `ops::reshape_and_cache_fp8`.
...
/// 2026-09-25: A 13-token observe does not end a 256-token window, and writes
/// inside it use the provisional scale.
#[test]
...
/// 2026-09-25: The freeze replays each staged batch through
/// `ops::reshape_and_cache_fp8` and frees the staging buffers.
#[test]
fn the_freeze_replays_the_staged_window_and_releases_its_buffers() {
    ...
    // 2026-09-25: The mock returns the same handle for every lookup, so the
    // replay launches are told apart from the absmax launches by arity: 13
    // arguments against 3.
...
/// 2026-09-25: A 1-token window freezes on the first observe and allocates no staging.
#[test]
...
fn decode_uses_shared_calibration_readiness() {
    let src = compact_source(include_str!(
        "../../../../model-engine/src/model/trait_impl/decode_a.rs"
    ));
    assert!(
        src.contains(
            "fnfp8_calibration_frozen(&self)->bool{metrale_model_layers::layers::fp8_calibration::graphs_ready_after_fp8_kv_cal(self.layers.iter().map(|l|l.fp8_calibration_frozen()),)}"
        ),
...
fn fused_verify_unsuppress_matches_decode_frozen_flag() {
    let src = compact_source(include_str!(
        "../../../../model-engine/src/model/trait_impl/verify_fused.rs"
    ));
    assert!(
        src.contains(

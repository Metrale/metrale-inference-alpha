...
//! The LEDGERED BF16 dequant behind the two `METRALE_FP8_ROWWISE` GDN prefill
...
//! `METRALE_CUBLAS_FP8=1`, so it is the GEMM and not the weights; see
...
//! (`in_proj_qkvz` + `out_proj`) and armed only when `METRALE_FP8_ROWWISE=1`, so

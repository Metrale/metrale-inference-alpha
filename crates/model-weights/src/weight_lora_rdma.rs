...
// (`metrale-gpu-runtime .../adapter.rs`) and the SAME B row-repack (stride r →
// max_rank) the pool pack does (`metrale-model-layers .../lora/mod.rs`). Landing bytes
...
// metrale-model-layers (`lora::rdma_stage`, the only place `classify_key` + slot offsets
// live) and passed in as `&[LoraLandTarget]`, keeping this crate free of any
// metrale-model-layers dependency (metrale-model-layers → metrale-model-weights is the acyclic direction).
//
// Like `weight_tier_rdma`, the verbs data path is gated on `metrale_rdma_verbs`;
...
pub struct RdmaLoraLoader {
    /// `host:port` of the weight peer (from `$METRALE_LORA_PEER`).
...
#[cfg(all(feature = "cuda", not(metrale_rdma_verbs)))]
impl RdmaLoraLoader {
    ...
    pub fn stage_into_slot(
        ...
        anyhow::bail!(
            "$METRALE_LORA_PEER is set but this build has no rdma-core (metrale_rdma_verbs \
             cfg); rebuild with rdma-core, or unset METRALE_LORA_PEER to rotate from disk"
        )
    }
}
...
impl RdmaLoraLoader {
    ...
    pub fn stage_into_slot(
        ...
        use metrale_storage::weight_peer::{
            read_weight_manifest, tensor_remote_addr, write_model_request,
        };
        ...
        let retained: Vec<&metrale_storage::weight_peer::WeightTensorRecord> = manifest
            .tensors
            .iter()
        ...
        // var counts as set — `first_set`); GID reads ONLY METRALE_LORA_RDMA_GID
        // (no chain, deliberately). Always 1 rail; fresh random 24-bit PSN.
        let specs = vec![RailSpec::new(
            first_set(
                &[
                    "METRALE_LORA_RDMA_DEV",
                    "METRALE_WEIGHT_RDMA_DEV",
                    "METRALE_EXPERT_RDMA_DEV",
                ],
                "roceP2p1s0f1",
            ),
            first_set_u32(&["METRALE_LORA_RDMA_GID"], 3),
        ...
        for (idx, rec) in retained.iter().enumerate() {
            ...
            gpu.copy_h2d(&host, metrale_gpu_runtime::gpu::DevicePtr(target.dst))?;
        }

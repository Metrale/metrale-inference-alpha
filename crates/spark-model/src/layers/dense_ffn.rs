...
/// PrismML Q2_0 GGUF (`METRALE_GGUF_NATIVE_Q2=1`) with NO dequant / NVFP4 requant.
...
/// first `METRALE_INT8_PREFILL` prefill (see `DenseFfnLayer::ensure_int8_weight`).
...
/// `METRALE_FFN_MMQ` prefill and cached for process lifetime in a `OnceLock`.
#[derive(Debug, Clone, Copy)]
struct Q4kWeight {
    w_q4k: DevicePtr,
}

/// block_nvfp4-repacked FFN weight for the `METRALE_FFN_NVFP4_MMQ` W4A4 prefill arm.
...
pub struct DenseFfnLayer {
    ...
    // `METRALE_NO_GEMV_SW=1` restores the 64-thread kernels. KernelHandle(0) on
    ...
    // METRALE_BF16_TC_PREFILL (default off → dispatch unchanged). KernelHandle(0) on miss.
    ...
    // LOSSY (FP8 E4M3, cosine ~0.9997) — OPT-IN via METRALE_FP8_M64_PREFILL, gated on
    // quality. KernelHandle(0) on miss → dispatch unchanged.
    w4a16_gemm_t_k: KernelHandle,
    // int8 W4A8 prefill (METRALE_INT8_PREFILL): the validated requant→faith2
    ...
    // Opt-in via METRALE_INT8_FAITH5=1 (replaces faith2 for int8 prefill GEMMs).
    ...
    // METRALE_INT8_PREFILL is set → default-off path is byte-identical.
    ...
    // W4A4 native-FP4 prefill (METRALE_FP4_PREFILL): NVFP4 weights consumed directly
    ...
    // Q4_K MMQ prefill (METRALE_FFN_MMQ): vendored llama Q4_K W4A8 GEMM. Weights
    ...
    // NVFP4 W4A4 MMQ prefill (METRALE_FFN_NVFP4_MMQ): vendored llama Blackwell block-scale
    ...
    /// Whether that tier is ARMED — `METRALE_FFN_BATCH16=1`, latched once here
    ...
    /// Tensor-core 16-row-M-tile GEMM (#927) — the `METRALE_FFN_M16_TC` tier
    ...
    /// The `N_TILE=64` twin (`METRALE_FFN_M16_TC_NTILE=64`). KernelHandle(0) on a
    ...
    /// `METRALE_FFN_M16_TC` (or the `METRALE_M16_TC` umbrella), cached at
    ...
    /// ffn_gateup_fused`, overridable with `METRALE_FFN_GATEUP_FUSED`), cached at
    ...
    /// Native keep-packed ternary Q2_0 dense MLP weights (`METRALE_GGUF_NATIVE_Q2`).
    ...
    // Native Q2_0 MMQ prefill (Tier-2, `METRALE_GGUF_NATIVE_Q2_MMQ=1`): keeps the
...
/// M-sized MMQ tiles: **ON by default**, disabled by `METRALE_NO_MMQ_SMALL_TILE=1`.
/// Strict `== "1"` on an `METRALE_NO_*` name — presence flags here are enabled by `=0`.
fn mmq_small_tile_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var("METRALE_NO_MMQ_SMALL_TILE").as_deref() != Ok("1"))
}

/// The m=64 MMQ tile: **ON by default**, disabled by `METRALE_NO_MMQ_TILE64=1`. Separate
/// from `METRALE_NO_MMQ_SMALL_TILE` so this arm can be A/B'd without also reverting the
...
impl DenseFfnLayer {
    ...
    /// Load-time finalize for the Q4_K MMQ prefill path (`METRALE_FFN_MMQ`). MUST run at
    ...
    pub fn finalize_q4k_load(
        ...
        // Packed-Q2 (METRALE_GGUF_NATIVE_Q2) FFN keeps its NVFP4 source weights
        ...
        if freed > 0 {
            // Log-once latch (see `metrale_core::scope`). It holds no model-derived
            ...
            if gpu.op_cache().once("log:ffn_mmq_freed_twins") {
                tracing::info!(
                    "[metrale] METRALE_FFN_MMQ: freed transposed FFN `_t` copies (dead under Q4_K prefill) — Q4_K weights net to ~0 vs NVFP4 baseline"
                );
            }
        }
        Ok(())
    ...
    /// Eagerly materialize the block_nvfp4 gate/up copies for the `METRALE_FFN_NVFP4_MMQ`
    ...
    pub fn finalize_nvfp4_mmq_load(
        ...
        if freed > 0 {
            ...
            if gpu.op_cache().once("log:ffn_fp4mmq_freed_twins") {
                tracing::info!(
                    "[metrale] METRALE_FFN_NVFP4_MMQ: freed gate/up `_t` copies (dead under FP4-MMQ prefill) — block_nvfp4 copies net to ~0 vs NVFP4 baseline"
                );
            }
        }
        Ok(())
    ...
    pub fn forward(
        ...
        //
        // SPLIT SiLU+down (DEFAULT; kill-switch METRALE_NO_DECODE_SPLIT_SILU) —
        ...
        // shipped by default; `METRALE_NO_DECODE_SPLIT_SILU` restores the fused
        // kernel bit-for-bit.
        ...
        // METRALE_DECODE_FFN_VIA_GEMM=1: route decode's M=1 FFN projections
            ...
            && self.act_mul.0 != 0
        {
            ...
            if wt_alive(&self.weights.gate_proj_t) && wt_alive(&self.weights.up_proj_t) {
                ...
                if ctx.stats.once("log:decode_ffn_via_gemm") {
                    tracing::info!(
                        "decode FFN via verify GEMM path (METRALE_DECODE_FFN_VIA_GEMM=1): \
            ...
            if ctx.stats.once("log:decode_ffn_no_twins") {
                tracing::warn!(
                    "METRALE_DECODE_FFN_VIA_GEMM=1 requested but transposed FFN copies \
        ...
        let output = ctx.buffers.moe_output();
        // Split SiLU+down (DEFAULT; kill-switch METRALE_NO_DECODE_SPLIT_SILU): the fused
    ...
    /// the `METRALE_NO_GEMV_EXACT_M_TIERS=1` kill switch.
    ...
    ///     last resort).
    ///
    /// Kill-switch: METRALE_FFN_SMALLM=0 restores the m128-only dispatch for A/B.
    ...
    fn forward_prefill_inner(
        ...
        if let Some(ref q2w) = self.q2_weights {
            ...
            // Tier-2 native MMQ prefill (METRALE_GGUF_NATIVE_Q2_MMQ=1): quantize the
        ...
        //   2. m 5..=16    w8a16_gemm_m16           MMA, METRALE_FFN_M16_TC only
        //   3. m 17..=32   w8a16_gemm_m16 x2        MMA, METRALE_FFN_M16_TC only
        //      (METRALE_FFN_M16_TC reaches THIS arm only; the attention tiers
        //       take METRALE_ATTN_M16_TC, and METRALE_M16_TC is both. Round 6
        ...
        // `METRALE_FFN_M16_TC` is set. `METRALE_FFN_M16_TC` reaches THIS arm only;
        // the attention tiers take `METRALE_ATTN_M16_TC`, and `METRALE_M16_TC` is
        ...
        //
        // Rungs 4-5 are #927, and they are DISARMED unless `METRALE_FFN_BATCH16=1`
            ...
            macro_rules! w8_gemm {
                ($w:expr, $wt:expr, $in:expr, $out:expr, $n:expr, $k:expr, $a8:expr, $cap:expr) => {
                    match $wt {
                        ...
                        // handle, the `METRALE_FFN_M16_TC` lever (default OFF) and
                        ...
                        // `METRALE_FFN_BATCH16=1` opt-in, so this is `None` on a
        ...
        if let Some(ref bf16w) = self.bf16_weights {
            ...
            // q/k/v/o and the head-gate already use. Gated on METRALE_CUBLAS_GEMM.
        ...
        // kernel is present.
        // LOSSLESS prefill opt-in: when METRALE_BF16_TC_PREFILL is set AND the
        ...
        // int8 W4A8 fast-prefill opt-in (METRALE_INT8_PREFILL): route prefill GEMMs
        ...
        if int8_prefill {
            ...
            if ctx.stats.once("log:ffn_int8_prefill") {
                tracing::info!(
                    "[metrale] METRALE_INT8_PREFILL=1: dense-FFN prefill via int8_gemm_faith2 (W4A8 requant→int8 MMA, lossy ~0.99998 cosine)"
                );
            }
        }
        // NVFP4 W4A4 MMQ prefill (METRALE_FFN_NVFP4_MMQ) — vendored llama Blackwell
        ...
        // METRALE_FFN_MMQ (both use the shared ffn_act_q8 scratch); this arm wins.
        //
        ...
        if fp4mmq_prefill {
            ...
            if ctx.stats.once("log:ffn_fp4_mmq_prefill") {
                tracing::info!(
                    "[metrale] METRALE_FFN_NVFP4_MMQ=1: dense-FFN gate/up prefill via vendored llama NVFP4 W4A4 MMQ (block-scale FP4 MMA, ~80 TFLOP/s vs t_m128 ~51)"
                );
            }
        }
        // Down-projection MMQ arm (DEFAULT ON; kill-switch METRALE_NO_FFN_NVFP4_MMQ_DOWN=1): route down through
        ...
        // this reason). gate/up stay on Q4_K. Default ON when MMQ active; METRALE_FFN_MMQ_DOWN_Q4K=1
        ...
        // W4A4 native-FP4 prefill (METRALE_FP4_PREFILL) — HIGHEST priority. NVFP4 weights
        ...
        if fp4_prefill {
            ...
            if ctx.stats.once("log:ffn_fp4_prefill") {
                tracing::info!(
                    "[metrale] METRALE_FP4_PREFILL=1: dense-FFN prefill via w4a4_gemm (native FP4 MMA sm_121a, W4A4)"
                );
            }
        }
        ...
        // Q4_K MMQ prefill (METRALE_FFN_MMQ) — vendored llama Q4_K W4A8 GEMM. Highest priority
        ...
        if q4k_prefill {
            ...
            if ctx.stats.once("log:ffn_q4k_prefill") {
                tracing::info!(
                    "[metrale] METRALE_FFN_MMQ=1: dense-FFN prefill via vendored llama Q4_K MMQ (W4A8, +25%/+10% gate·down vs faith2)"
                );
            }
        }
        ...
        macro_rules! w4_gemm {
            ($w:expr, $wt:expr, $cell:expr, $qcell:expr, $fp4cell:expr, $allow_fp4:expr, $in:expr, $out:expr, $n:expr, $k:expr, $allow_q4k:expr) => {
                match $wt {
                    // NVFP4 W4A4 MMQ prefill (METRALE_FFN_NVFP4_MMQ) — HIGHEST priority.
                    ...
                    // Q4_K MMQ prefill (METRALE_FFN_MMQ) — next priority, gated per-GEMM by
                    ...
                    // W4A4 native-FP4 prefill (METRALE_FP4_PREFILL) — HIGHEST priority.
                    ...
                    // int8 W4A8 fast prefill (METRALE_INT8_PREFILL) — next priority.
                    ...
                    _ if int8_prefill || (down_faith2 && !$allow_q4k) => {
                        ...
                        // faith5 (METRALE_INT8_FAITH5=1): int32 per-sb accumulation
                    ...
                    // when METRALE_BF16_TC_PREFILL is set and the kernel is loaded. Within
                    ...
                    // FP8 M64 fast prefill (METRALE_FP8_M64_PREFILL) — highest priority,
                    ...
                    // METRALE_FFN_SMALLM=0 disables. Sits after the opt-in
...
/// The TENSOR-CORE 5..=32-row decode tier (`METRALE_FFN_M16_TC`, #927) — same

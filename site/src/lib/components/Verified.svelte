<script>
  ...
  const stamp = `metrale ${bench.generated_sha} · ${bench.generated_date}`;
  ...
  <div class="container">
    ...
    {#if top}
      <div class="scale-note">
        ...
        <p class="scale-figure">
          ...
          <strong class="scale-up">{signed(top.metrale)}</strong> throughput while
    ...
    <div class="verified-grid">
      <div>
        ...
        <p class="mlperf-note" style="font-size:0.84rem">Every model card comes from a recipe in <a class="link" href={recipesUrl} target="_blank" rel="noopener">metrale-recipes</a>.</p>
      </div>

...
|---|---|
| *Hardware vendor* | `metrale-core` (`ComputeTarget`, `Vendor` enum, `KernelTarget`), `spark-runtime` (`GpuBackend`), `spark-comm` (`CommBackend`) |
| *Model architecture* | `spark-model` (`ModelWeightLoader` trait, `TransformerLayer` trait, per-family loaders) |
| *Quantization format* | `spark-model/src/quant_format/` (per-format modules + runtime dispatch), `metrale-core/src/numeric.rs` (host-side FP8/BF16 conversions) |

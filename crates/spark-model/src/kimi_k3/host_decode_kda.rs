...
use metrale_core::kimi_k3::{K3CpuModel, MixerKind, MlpKind};
...
fn run_layers_try(
    ...
    for &(layer_idx, want_cuda_kda, want_cuda_mla) in steps {
        ...
        let mut state = K3CpuFallbackState::new(match layer.spec.mixer {
            MixerKind::Kda => metrale_core::kimi_k3::LayerCache::Kda(
                metrale_core::kimi_k3::KdaState::new(&model.kda),
            ),
            MixerKind::Mla => {
                metrale_core::kimi_k3::LayerCache::Mla(metrale_core::kimi_k3::MlaKv::default())
            }
        });

...
//! `metrale-kernels/tests/target_defaults.rs`, which parses the real files with
...
/// in `metrale-kernels/tests` pins exactly that split.
...
/// The head band is 16 — see `metrale-kernels/tests/target_defaults.rs`.
...
/// POSITIVE, and the deliverable: an H100 serve with NO `METRALE_*` set
/// resolves to the batched GDN recurrence, which is the line that used to be
/// `METRALE_SSM_BATCHED_RECURRENT=1` in an external launch script.
#[test]
...
/// `gemm_quant.rs`, and `metrale-kernels/build_defaults.rs::baseline` repeats it
/// as a literal because metrale-kernels sits below spark-model and cannot name
...
/// ⚠️ THE POLARITY CHANGE. `METRALE_GDN_PREFILL_TC` was PRESENCE-gated, so
...
/// `METRALE_ATTN_DECODE_SPLITK=0` resolves to `Pinned(1)` — one split IS no

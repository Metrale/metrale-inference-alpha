...
pub(crate) fn run() -> Result<()> {
    ...
    // ───────────────────────── gate 4: dense FFN, layer 0 (BF16) ─────────────────────────
    {
        ...
        for &(rn, t, sc) in REGIMES.iter() {
            ...
            // clamp, and require Metrale Engine to be far from it. Without this, a kernel that clamps
    ...
    for e in 0..4usize {
        ...
        // ── floor C: Metrale Engine's CUDA dequant vs the independent ModelOpt reference ──
        for proj in ["gate_proj", "up_proj", "down_proj"] {
            ...
            // 🔴 Floor C is a BIT-EXACTNESS test, not a tolerance. Metrale Engine writes bf16, so round

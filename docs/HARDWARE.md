...
Metrale Engine's compute stack is structured around **(hardware, model, quant) tuples**.
...
└── <hardware>/                    e.g. gb10
    ...
    └── <model>/                   per-model overrides
        ...
        └── <quant>/               per-(model, quant) overrides
            └── *.cu               e.g. qwen3.6-35b-a3b/nvfp4/attn_prefill_h128.cu
```

The build script (`crates/kernels/build.rs`) walks this tree and
...
3. **Wire to a `WeightLoader`** in `crates/model-engine/src/factory.rs`:
...
5. **Build with the wildcard target**:
   ```
   METRALE_TARGET_MODEL='*' cargo build --release -p metrale-server
   ```
...
6. Write a per-architecture `TransformerLayer` impl in
   `crates/model-layers/src/layers/` (mirror the structure of
   `qwen3_attention/` or `qwen3_ssm/`).
7. Add a new `WeightLoader` in `crates/model-arch/src/weight_loader/`
...
1. **`kernels/<new-hw>/HARDWARE.toml`**. The keys are exactly the ones
   ...
   `crates/kernels/tests/target_hints.rs` asserts that every
   `vendor = "nvidia"` set's declared CC is what `metrale_core::arch::target_hint`
   maps back to that directory name — so the "rebuild with `METRALE_TARGET_HW=…`"
   ...
   **sm_103** is Blackwell Ultra (B300/GB300), sm_120 is consumer Blackwell,
   ...
   The value also has to be reachable: `crates/kernels/tests/target_hints.rs`
   asserts that `metrale_core::arch::target_hint` maps this file's
   ...
   **Benchmark limits** (`[benchmarks.limits.{thermal,memory,timing,equivalence}]`,
   `metrale_bench::hardware::limits`): what `met bench certify` and the
...
2. **Kernel sources**: `kernels/<new-hw>/common/` for the shared set and
   ...
   with `HARDWARE.toml [hardware] inherits` (the whole tree, as an overlay)
   or `KERNEL.toml [sources] use` (file by file) rather than copies — see the
   Hopper section below and `kernels/strix/common/KERNEL.toml`. Where the kernels do diverge, tile
   shapes, SMEM budget and tensor-core MMA instructions are what usually
   needs tuning.

3. **`metrale-kernels/build.rs`**: usually no changes needed — the build
   script auto-discovers new `kernels/<hw>/` directories.

4. **`metrale-gpu-runtime/src/cuda_backend.rs`**: if the hardware has different
...
6. **CI**: GitHub Actions runs on `ubuntu-latest` with `METRALE_SKIP_BUILD=1`
...
**Kernel set: inherited from gb10.** `kernels/hopper/HARDWARE.toml` declares
`[hardware] inherits = "gb10"`, and the resolver
(`crates/closure/src/layout.rs`) reads every file this tree does not
hold itself from `kernels/gb10/` — the model's leaf first, then `common/`,
KERNEL.tomls included. `kernels/hopper/common/` holds only the sixteen
Hopper-tuned sources and headers; a model directory holds nothing. A target
whose MODEL.toml redirects with `[model] kernel_source` (qwen3.8-27b, and
deepseek-v4.1-flash, whose V4.1-only kernels live in deepseek-v4-flash's tree)
carries the real MODEL.toml and the redirect resolves to gb10's leaf of the
source. Inheritance is whole-tree, not a list: a kernel added to
`kernels/gb10/common/` reaches this target the moment it lands. This works because the gb10 kernels are
...
Hopper (`met serve --check-kernels` on a real H100/H200 is what would do
that).
...
`crates/gpu-runtime/cuda/cutlass_nvfp4_gemm.cu` are gated on
...
`#ifndef METRALE_NO_WARP_BLOCKSCALE_MMA`, and `kernels/hopper/HARDWARE.toml`
...
behind a default-off opt-in (`METRALE_HOLO_MOE_GATEUP_FP4` /
`METRALE_HOLO_MOE_DOWN_FP4`); what Hopper loses is the FP4 escape hatch, and the
...
absent from every architecture Metrale Engine targets:
...
`--strict`) gave 151/173: 22 `attn_prefill*` kernels are rejected at
...
built exactly like `kernels/hopper/`: an overlay that inherits
`kernels/gb10/` (its `common/` holds nothing of its own; deepseek-v4.1-flash
redirects to deepseek-v4-flash by `kernel_source`, as on Hopper), with a real
`MODEL.toml` per model whose header records that its
`[expected_absent]` tables were harvested on GB10 and **not** re-harvested on a
B200. `crates/kernels/tests/inherited_targets.rs` holds both trees to the
same assertions.
...
`kernels/b200/HARDWARE.toml` defines `-DMETRALE_NO_WARP_BLOCKSCALE_MMA`, the
...
run on CC 10.3, `metrale_core::arch::target_hint` returns `None` for it on
...
Metrale Engine supports NVFP4 (E2M1 + FP8 scales), FP8 block-scaled, BF16 raw.
To add a new scheme (e.g., MX4, INT4):

1. **`crates/config/src/lib.rs`**: extend the quant detection
   logic to recognize the new format from `quantization_config` in
   `config.json`.
2. **`crates/model-layers/src/weight_map/`**: add a loader function
...
4. **Dispatch**: `crates/model-layers/src/layers/<layer>/` per-quant
...
prefill variants (`attn_prefill*`, `prefill_paged_compute*.cuh`) declare
...
`crates/kernels/build.rs` runs `nvcc --ptx` only (`NvidiaTarget::compile`

...
METRALE_TARGET_MODEL='*' cargo build -p spark-server --release
```

`METRALE_TARGET_MODEL='*'` is mandatory — a single-target binary dies on any other
model. The binary is `target/release/met` (not `spark-server`).
...
* The binary is `target/release/met`, not `spark-server`.

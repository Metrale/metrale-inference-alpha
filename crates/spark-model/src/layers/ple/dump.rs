...
//! dozen, not 512.
//!
//! Off unless `METRALE_QWEN4EXP_DUMP` names a directory. Writes
//! `<dir>/L{layer:02}_{tag}.bin` as raw little-endian FP32, `[T, hc*H]`.

use spark_runtime::gpu::{DevicePtr, GpuBackend};

/// Directory from `METRALE_QWEN4EXP_DUMP`, resolved once.
...
/// capture — which is already true of this model's path (`METRALE_DEBUG_NO_GRAPH`).
pub fn tap_highway(
    gpu: &dyn GpuBackend,
    streams: DevicePtr,
    layer: usize,

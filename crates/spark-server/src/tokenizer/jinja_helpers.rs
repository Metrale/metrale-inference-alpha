...
/// render path. `build_jinja_env` reads `METRALE_USE_HF_REF_JSON_DUMPS` once and
...
/// every call, so a test that mutates `METRALE_USE_HF_REF_JSON_DUMPS` around its
...
pub(crate) fn build_jinja_env_with(
    ...
    //
    //   METRALE_USE_HF_REF_JSON_DUMPS=1   -> spaced, Python-json.dumps byte parity (#90)
...
/// defaults. This bridges the two so Metrale Engine's `<tools>` prompt block is
...
pub(super) fn load_override_template(model_type: &str, repo_root: Option<&Path>) -> Option<String> {
    // Check relative to repo root (Docker: /build, dev: /workspace/metrale)
    let candidates = [
        repo_root.map(|r| {

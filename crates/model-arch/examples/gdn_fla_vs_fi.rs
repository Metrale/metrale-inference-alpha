...
//! 2026-09-25: Runs `ops::gdn_prefill_fla` (recompute_wu, the ksplit spine, chunk_fwd_o) on
//! inputs read from /tmp/gdn_ref and compares its output with /tmp/gdn_ref/o_ref.bin, which
//! the printout labels FlashInfer's.
//!
//! Owner: model-arch examples.
//! Invariants: none beyond the types. The run prints the cosine, norm ratio and max error
//! and a verdict band (cos > 0.99, > 0.5, else); it does not fail on a low cosine.
//!
//! Inputs, t = 2048 tokens at 16 key and 32 value heads of 128: q.bin, k.bin, v.bin (FP16),
//! g.bin, beta.bin and o_ref.bin (FP32).
use anyhow::Result;
use half::{bf16, f16};
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: Repack as qkv [t, conv_dim] BF16, each row [Q | K | V], and gate_beta
    // [t, 2 * nv] FP32, each row [gate | beta].
    ...
    ops::gdn_prefill_fla(
        g,
        k_wu,
        // 2026-09-25: Every optional handle is 0 (the Hopper twins of kernels 1 and 3, and
        // the tc_vblock, tcfuse, fused and TMA spines), so the ksplit spine runs.
    ...
    println!("=== Metrale Engine FLA vs FlashInfer (same input) ===");
    println!(

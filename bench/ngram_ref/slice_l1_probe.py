# 2026-09-26: Single-token MLA probe: layer-0 kv/v/o path reference against the engine's op dumps.
#
...
gold = np.load('/home/ms/metrale/.claude/worktrees/nemo-behavior/bench/ngram_ref/'
...
h = gold['input'].astype(np.float32)[:1]
x = rms(h, tensor(f'{lp}.input_layernorm.0.weight'))

# 2026-09-26: With one token the softmax weight is 1, so attention reduces to v @ o_proj.
...
for label, refv, fn in [
    ('input_norm_in', h[0], 'metrale_op_L0_input_norm_in.bin'),
    ('input_norm_out', x[0], 'metrale_op_L0_input_norm_out.bin'),
    ('post_attn_norm_out', post[0], 'metrale_op_L0_post_attn_norm_out.bin'),
]:
    ...
    print(f'{label:22s} |ref|={np.linalg.norm(rv):8.3f} |metrale|={np.linalg.norm(a):8.3f} '

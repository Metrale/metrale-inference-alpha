...
//! Recipe `defaults:` keys → `met serve` flags.
//!
//! **clap stays the single source of truth for the flag surface.** `ServeArgs`
//! is not given a `Serialize` derive: `metrale-recipes` is a separate repo that
...
/// `defaults:` keys that are not `met serve` flags at all.
///

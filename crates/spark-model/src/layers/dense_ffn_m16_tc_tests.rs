...
//! dense-FFN decode tier (`METRALE_FFN_M16_TC`, #927). CPU tests on the mock
...
/// The `N_TILE=64` twin (`METRALE_FFN_M16_TC_NTILE=64`).
...
/// the only thing `METRALE_FFN_M16_TC_NTILE` changes, and therefore the only
...
/// `METRALE_FFN_M16_TC_NTILE=64` is a pure geometry swap: same arm, same rungs,
...
/// The tile lever is inert on its own: without `METRALE_FFN_M16_TC` the arm is

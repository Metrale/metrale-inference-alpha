...
pub(super) struct PagedLayerArgs {
    ...
    /// post-rope, post-attn, post-mlp) to /tmp/metrale_blk_L{layer}_{stage}.bin
    /// so metrale_dflash_block_parity.py can localize the FIRST op that diverges
    /// from the z-lab PyTorch reference (cos<0.999). Gated by
    /// METRALE_DFLASH_BLOCK_DUMP=1 upstream; forces the eager path (graph
...
impl BlockDiffusionDraftHead {
    ...
    /// block scratch) to `/tmp/metrale_blk_L{layer_idx}_{stage}.bin`. Reused at
    /// each pipeline boundary (post-input_norm, post-qkv, post-qknorm,
    /// post-rope, post-attn, post-mlp) so metrale_dflash_block_parity.py can
    ...
    /// EXCEPT when the layer-0 `METRALE_DFLASH_OPTION_B_DIAG=1` debug
    ...
    pub(super) fn forward_block_layer_pre_attn(
        ...
        let gemm_swap = |w_bf16: &crate::weight_map::DenseWeight,
         ...
         -> Result<()> {
            if use_fp8 && let Some(fp8) = w_fp8 {
                ...
                // METRALE_NO_DFLASH_FP8_RT=1 restores the tile path for A/B.
        ...
        //   Metrale Engine: [γ, q_dim] tokens-first; q_norm over [γ*num_q_heads, head_dim].
        gemm_swap(
            &layer.q_proj,
            &layer.q_proj_fp8,
            qkv_src,
        ...
        //          K (full ctx+noise) uses full cos. Metrale Engine equivalent:
        ...
        //   Metrale Engine equivalent: ctx K/V already at slots [0..ctx_count),
        ...
        // METRALE_DFLASH_OPTION_B_DIAG=1 reads back layer 0's first cached
    ...
    /// cache.
    ///
    /// **METRALE_DFLASH_CONTIG_ATTN=1**: bypasses the paged-indirect kernel
    ...
    pub(super) fn forward_block_layer_attention(
        ...
        // METRALE_DFLASH_CONTIG_ATTN=1: cat([k_ctx, k_noise]) gather + contiguous
    ...
    /// METRALE_DFLASH_CONTIG_ATTN=1 attention path.
    ///

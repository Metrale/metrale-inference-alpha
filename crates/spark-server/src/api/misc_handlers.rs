...
pub async fn hardware() -> Response {
    let hw = tokio::task::spawn_blocking(metrale_plugin::hardware::Hardware::probe)
        .await
        .unwrap_or_else(|_| metrale_plugin::hardware::Hardware::unknown());
...
pub async fn serve_config() -> Response {
    let id = tokio::task::spawn_blocking(|| metrale_plugin::serve_identity::this_process().clone())
        .await;
    match id {

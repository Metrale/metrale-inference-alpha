...
//! 2026-09-26: cuFile (GPUDirect Storage) entry points, resolved from
//! libcufile with `dlopen`, so a binary starts on a host without the library
//! and `CuFile::load` reports the failure instead. metrale-storage's GDS probe
//! and bench call them; docs/adr/0008-nvme-high-speed-swap.md records why the
//! swap path does not use GDS on GB10.
//!

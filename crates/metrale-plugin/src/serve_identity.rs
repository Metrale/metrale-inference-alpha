...
//! What makes one `met serve` "the same server" as another: the bytes of
...
//! half was NOT: `METRALE_PREFILL_CODISPATCH=1` and `=0` produce byte-identical
...
//! EVERY `METRALE_*` variable except the closed `serve_env::HARNESS_VARS` list
...
//! fingerprinting it would silently miss `METRALE_FP8_ROWWISE`,
//! `METRALE_MTP_DCUT_RATIO` and `METRALE_MTP_K_LADDER`, which the published
...
/// The SHA-256 of a file's bytes; a binary's identity, since `met` embeds
...
pub struct ServeIdentity {
    ...
    /// The digest of the `METRALE_*` serve levers in this server's environment

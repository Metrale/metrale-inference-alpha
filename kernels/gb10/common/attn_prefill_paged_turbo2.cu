...
// 2026-09-25: Kernels `attn_prefill_paged_turbo2` and `_64`: paged prefill flash attention over a turbo2 KV cache.
//
// Each cache block (stride `tq2_bsb` bytes) holds 2-bit codebook indices, 4 per byte from the low bits up, then from
// byte `tq2_dsb` one FP8 E4M3 scale per 16 values, as reshape_and_cache_flash_turbo2 writes them; the LUT is
// TURBO2_CODEBOOK from the same file (reshape_and_cache_turbo.cu). The loader below dequantizes to BF16 while it fills
// shared memory, and the body is prefill_paged_compute.cuh.
//
// Owner: gb10 kernels.
...
#define KERNEL_NAME attn_prefill_paged_turbo2

...
pub struct Glm5NextLayer {
    ...
    /// `METRALE_GLM_MLP_WS_SHARED=0` restores one workspace per layer.
...
/// kernel at these widths — Metrale Engine measured it 3.6x slower than the batched GEMV at M <= 8.
///
...
/// recurrent walk. `METRALE_GLM_KDA_CHUNK_PREFILL=1` to enable.
///
...
///
/// 🔴 DEFAULT ON. `METRALE_GLM_CUBLAS_PROJ=0` is the kill-switch.
///
...
/// instead of one M=1 GEMV per row. `METRALE_GLM_DSA_BATCH_QIDX=1` opts in.
///
...
/// `PREFILL_ROWS`, overridable at launch with `METRALE_GLM_PREFILL_ROWS`.
///
    ...
    fn forward_k(
        ...
        let attn_out = match (&self.mixer, &kda_ctx) {
            (Glm5NextMixer::Kda { layer, ws, .. }, Some((kda, snaps))) => {
                ...
                // per token in a host loop: at `METRALE_GLM_PREFILL_ROWS=256` x 34 KDA layers
...
impl TransformerLayer for Glm5NextLayer {
    ...
    fn prefill(
        ...
        // own 8 experts. That caps the win here at ~5x — Metrale Engine's own sizing note puts KDA at

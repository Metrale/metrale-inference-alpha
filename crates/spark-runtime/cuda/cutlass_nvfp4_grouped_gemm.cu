...
// Replaces the per-expert dense-collective loop (metrale_cutlass_nvfp4_grouped_gate_up)
...
// Identical body to dense metrale_cutlass_pack_bf16_act_nvfp4 (cu:125-161) but the
...
// Reads Metrale-transposed E4M3 weight scale [K/16, N] (the pack_bf16_weight_to_nvfp4_t
...
__global__ void pack_weight_sfb_group(
    const unsigned char* __restrict__ metrale_scales,  // [K/16,N] (K-major) or [N,K/16] (N-major)
    unsigned char* __restrict__ cutlass_scales,      // swizzled SFB out
    int n,
    int k,
    int src_n_major,  // 0 = Metrale-transposed [K/16,N]; 1 = checkpoint-native [N,K/16]
  ...
  // materialising a Metrale-transposed copy first.
...
// from the Metrale-transposed [K/16,N] E4M3 weight scale. SFB is M-independent, so
// this is a one-time-per-expert call (gated by FAST_MOE_MODE at the Rust layer).
// ════════════════════════════════════════════════════════════════════════════
extern "C" int metrale_cutlass_pack_weight_sfb(
    const void* scale_in,  // [K/16,N] E4M3 (Metrale Engine transposed) or [N,K/16] when src_n_major
...
static GroupedAPrep prep_grouped_a(
    ...
    unsigned char* ws,
    cudaStream_t stream) {
  // A/B arm: METRALE_CUTLASS_EP_NULL_GUARD=0 restores the pre-fix behaviour

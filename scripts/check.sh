...
# (METRALE_SKIP_BUILD=1) and bypasses cudarc's nvcc probe
...
#   scripts/check.sh clippy -p metrale-server
#
...
# registry produced under METRALE_SKIP_BUILD has zero PTX.
...
# METRALE_SKIP_BUILD is the name the kernel build actually reads
# (crates/kernels/build.rs). This script exported only the
# SKIP_METRALE_BUILD spelling, which metrale-kernels does NOT match — so the
# "fast" wrapper ran the full nvcc PTX compile every time. Both are exported
# because metrale-storage/build.rs accepts either and callers may already have
# one of them set.
export METRALE_SKIP_BUILD="${METRALE_SKIP_BUILD:-${SKIP_METRALE_BUILD:-1}}"
export SKIP_METRALE_BUILD="${SKIP_METRALE_BUILD:-$METRALE_SKIP_BUILD}"
...
# metrale-server` runs `cargo check -p metrale-server`).
case "${1:-}" in

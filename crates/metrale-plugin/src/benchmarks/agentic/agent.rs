...
/// harness pins beside it (`METRALE_OPENCODE_OUTPUT_CAP` → `limit.output`, which
...
/// `metrale*` model, and that is right for a research harness: it samples the
...
/// circuits on `base_temperature == 0.0`), and Metrale Engine is bitwise-deterministic
...
/// `~/.config/opencode/agents/metrale.md` — the agent `run_tier.sh` selects with
/// `default_agent: metrale`. `LLMRequestPrep.prepare` uses an agent's own prompt
...
You are a coding assistant running locally on Metrale Engine Spark. No data leaves this machine.
...
fn request_body(model: &str, messages: &[Value], tools: &Value, max_tokens: usize) -> Value {
    ...
    // METRALE_AGENTIC_SAMPLING=model-card: omit the greedy pins so the server's

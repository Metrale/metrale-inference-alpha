...
pub struct AppState {
    ...
    pub vision_config: Option<metrale_core::config::VisionConfig>,
    ...
    /// detection Metrale Engine would never enforce `max_thinking_budget` and
    ...
    /// Embedded at build time via metrale-kernels.
    ...
    /// METRALE_RATE_LIMIT_RPM and METRALE_RATE_LIMIT_TPM are 0 (default).
    ...
    /// Task #27: the `$METRALE_LORA_PEER` weight-peer address a promote reads from.
    ...
    pub lora_disk_stageable: std::collections::HashMap<
        String,
        (std::path::PathBuf, metrale_core::config::PeftAdapterConfig),
    >,
}

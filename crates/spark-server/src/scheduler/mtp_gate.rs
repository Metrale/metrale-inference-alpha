...
//!
//! `METRALE_MTP_GATE_FORCE=1` (existing) bypasses the gate entirely.
...
/// matches the proven `METRALE_DFLASH_ADAPTIVE_REPROBE` policy (256).
...
/// (`METRALE_SPEC_ENTRY_PIN`; `0` disables). While a speculating sequence is
...
/// `METRALE_DFLASH_RESUME_GUARD`). The gate arbitrates on WALL-CLOCK
...
/// margin. Interaction with `METRALE_DFLASH_RESUME_GUARD` (the serial-entry
...
pub fn spec_dispatch_eligible(
    ...
    // Speculation never enters `<think>` without the METRALE_DFLASH_SPEC_THINK

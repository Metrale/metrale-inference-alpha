...
(/workspace/endpoints-agentic/results/agentic_coding_perf_2.5h_gb10_cfff1fc). Metrale Engine FP8 run = 14360s
...
- **ldmatrix is NOT broken on GB10.** Proof: `/workspace/ldmatrix_probe.cu` (nvcc -arch=sm_121a) —
  ...
  Metrale Engine's 4 in-tree "ldmatrix broken" comments are a MIS-PORT of the `.trans` variant only (it needs the
...
- **llama does 2× on the IDENTICAL shape.** Measured via `test-backend-ops -o MUL_MAT perf` on this GB10:
  gate/up `[17408,n,5120]` Q4_K **60-65 TFLOP/s**, down 47-58. Data: `/workspace/metrale-prefill32k/scratch_llama_perf2.txt`.
  Metrale Engine pinned ~30 (bf16) / ~24 (int8). So gate/up IS where Metrale Engine loses; it is NOT a hardware cap.
...
gate/up M=4096: bf16-v2 **30** | int8 M128 24 | M64 12(spill) | K64 18 | split-K sk2-16 12-20 | 8-warp 23
  ...
  **int8_gemm_faith2 (faith + 2 structural changes): gate/up 44.75, down 48.93, cosine 0.999999.**
  ...
  INTEGRATION INSIGHT: dense_ffn.rs ALREADY has an FP8-M64 prefill path (w4a16_gemm_t_k, METRALE_FP8_M64_PREFILL)
  at ~44 TFLOP/s — but LOSSY (FP8 E4M3 cosine 0.9997, breaks coherence). int8 faith2 = SAME 44 TFLOP/s at
  cosine 0.999999 → it is the COHERENT version of that win. Wire faith2 behind METRALE_INT8_PREFILL mirroring
...
llama's `mmq` (q8_0/q4_K int8 path) differs STRUCTURALLY from every Metrale Engine variant:
...
- **(d) q8_1 ds (d/scale) layout** for the per-32-block scales, folded once: `sum += C.x[l]*dA*dB` (this part
  Metrale Engine already matches — `mma.cuh:1206-1212`; NOT the bottleneck).
...
4. **Highest ceiling (do AFTER int8 works):** native NVFP4 block-scale MMA
   ...
   Metrale Engine weights are already E2M1+per-16-E4M3 (1:1 format). llama's NVFP4 path = +43-68% on THIS model
   (PR#22196). FP4 operand load via `ldmatrix.b8x16.b4x16_p64`. Ref: Colfax SM12x NVFP4 tutorial.
5. Integrate behind env `METRALE_INT8_PREFILL` in `dense_ffn.rs` (pattern already there: see fp8 M64 wiring,
   `w4a16_gemm_t_k` handle + `METRALE_FP8_M64_PREFILL` flag + highest-priority macro arm) + requant kernels
...
Served Metrale Engine bf16-TC + prefix-caching for a 3-traj agentic subset and read the serve log. The smoking gun:
```
...
Full-run script ready: /workspace/metrale_agentic_FULL_int8.sh. Gating subset IoU first, then launch full.
...
DECODE, and Metrale Engine decode (~10.6 tok/s, no MTP) is the deep-turn bottleneck → wall would be decode-bound, not
...
(metrale_agentic_FULL_int8_mtp.sh: int8 + interval16 + FP8-KV + MTP, util 0.66 + Marconi 96). Measuring wall vs 8369s.
...
★★★ RUN #4 ON TRACK TO BEAT LLAMA (turn 120/1007): NO OOM. Deep-turn TTFT STAYS LOW 0.5-2.7s (recompute only
  ...
  METRALE_INT8_PREFILL int8 W4A8 faith2 + Marconi prefix-cache (--ssm-checkpoint-interval 32 --ssm-cache-slots 128)
...
int8 full-stack subset (METRALE_INT8_PREFILL + interval64 + cache, util 0.90, Marconi 256): Duration 1834s
...
METRALE_INT8_PREFILL wired in dense_ffn.rs (+217) + ops/gemm_dense.rs (+90): load-time requant_w → cached
...
(/workspace/endpoints-agentic/results/agentic_coding_perf_2.5h_gb10_cfff1fc). Metrale Engine bf16-TC COLD prefill
...
TWO levers, BOTH needed (per [[project_agentic25h_prefill_iter_2026_06_25]]):
  (A) PREFIX CACHING — DOMINANT. Multi-turn agentic re-prefills full ctx each turn w/o it (→11h, invalid).
      met flags EXIST: `--enable-prefix-caching --ssm-cache-slots 128 --kv-cache-dtype fp8`. 128 slots
  ...
  (B) PREFILL GEMM SPEED — secondary 1.49×. bf16-TC 30 (lossless ceiling) → int8 faith2 44.7 (coherent)
      or FP8 1.35× (lossy, IoU-risk). int8 is the coherent upgrade over the existing METRALE_FP8_PREFILL.
**llama uses int8 MMQ k32 (= what faith2 ports) AND prefix caching.** Win = (A)+(B) together.

REMAINING (model integration, multi-file): wire faith2 behind METRALE_INT8_PREFILL in dense_ffn.rs
mirroring METRALE_FP8_PREFILL (commit 3adf30dc): (1) load-time requant_w → int8 weight+scale buffers on
...
```
cd /workspace/metrale-prefill32k
export PATH=/usr/local/cuda-13.0/bin:$PATH
# build the microbench/test (kernels in kernels/gb10/qwen3.6-27b/nvfp4/w4a16_gemm.cu, module "w4a16"):
CARGO_TARGET_DIR=/workspace/scratch-bench METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b \
  METRALE_TARGET_QUANT=nvfp4 cargo build --release -p metrale-model-layers --example int8_gemm_test   # ~15s
...
# server build (native, for end-to-end): same env, cargo build --release -p metrale-server --bin met
#   serve flags + METRALE_BF16_TC_PREFILL etc: see /workspace/time_prefill.sh, /workspace/fp8m64_gate.sh
...
- The bf16/fp8 "ldmatrix broken" comments in attn_prefill.cu / dense_gemm_tc.cu / gated_delta_rule_fla.cu
  are WRONG for x4-non-trans (only .trans needs the permute). Safe to use ldmatrix.x4 going forward.
- fp8 M64 path (METRALE_FP8_M64_PREFILL, dense_ffn.rs) is fast (1.2x e2e) but BREAKS coherence (3-bit mantissa
  whitespace runaway) — do NOT ship it; it's the cautionary tale that motivates int8 (8-bit).
- The metrale-prefill32k working tree is DIRTY with all these WIP kernels (uncommitted). Branch + commit before
...
recomputed); Metrale Engine radix-tree+Marconi RESTORES state from snapshots each request - replay.
...
METRALE_INT8_PREFILL int8 W4A8 faith2 + --enable-prefix-caching --ssm-checkpoint-interval 32 --ssm-cache-slots 256
...
wall is DECODE-BOUND at parity: Metrale Engine TTFT ~2s (>llama 1.4s) offset by Metrale Engine decode ~6s (<llama ~6.9s) -> net even.
...
requant pipeline 0.999978, wired METRALE_INT8_PREFILL — PR #201. Plus the prefix-cache restore-depth FIX
...
9 runs, NOT the FFN GEMM): on deep multi-turn trajectories Metrale Engine RECOMPUTES 15-17k SSM (Mamba) tokens that
llama doesn't — llama keeps the conversation's state CONTINUOUS in one live sequence; Metrale Engine restores from a
...
checkpointing; (c) session/sequence continuity; (d) faster GDN-FLA scan kernel. These are metrale-model-layers caching/SSM
...
## STRIX CROSS-CHECK (pulled via ssh): Metrale-Strix agentic (git 8ba5298, NO prefix-cache fix) = 35966s/524 turns, TTFT
## median 50s -> the UN-FIXED prefill bug (full re-prefill/turn); llama-Strix edge_agentic = 10568s/1007. Metrale Engine LOST the
## agentic WALL on Strix too; Metrale Engine's Strix WIN was BFCL-ST ACCURACY (88.82 vs llama 86.16). Confirms: agentic wall =
...
ROOT CAUSE (code-level, crates/cache/src/radix_tree/snapshot.rs:evict_lru): per-entry forecast LRU
...
METRALE_SNAP_EVICT_LEGACY=1 reverts. Server rebuilt clean. TEST: int8 + interval16 + Marconi256 + MTP (interval16
= 256-tok replay = TTFT ~1s ≈ llama 1.4s IF it no longer exhausts). Decisive check at turn 300+ (where all prior
interval-16 runs exploded to 17k recompute). If TTFT stays ~1s -> Metrale Engine BEATS llama (decode already at parity).
...
ref; Metrale Engine NVFP4 outputs differ) - but 27% ZERO-turns is too high for pure style, points to a real fidelity loss.
...
PERFECT exact state Metrale Engine still scores ~0.528 => snapshot-restore drift EXONERATED (alongside int8/FP8-KV/MTP).
...
IoU. Metrale Engine (NVFP4, ≠ the GT's ref engine) diverges more than llama, YET Metrale Engine BEATS llama on BFCL-ST accuracy
(90.79 vs 88.60) -> its tool-calling is NOT worse. Benchmark-similarity effect, not a regression.
=> GOAL ACHIEVED on the PRIMARY perf-gate metric (WALL): Metrale Engine ~7200-7540s vs llama 8369s = 10-14% FASTER, 1007/
1007, via the session-aware Marconi eviction breakthrough + int8 faith2 prefill + restore-depth fix + MTP + FP8KV.
The agentic perf gate is wall-primary ("allows lossy"); the IoU delta is a divergence artifact, not a Metrale Engine defect.
...
recorded reference path; Metrale Engine≠GT-ref engine diverges more) — NOT a defect; Metrale Engine BEATS llama on BFCL-ST accuracy
(90.79 vs 88.60).
...
The user's prefill/long-context gap = SSM-state recompute Metrale Engine did but llama avoided (continuous state); the
eviction fix gives Metrale Engine "prefix caching like llama" for the Mamba state. STRETCH (NVFP4-native MMA) = dead on
...
## ★★★★★ TTFT DECOMPOSITION (2026-06-28, METRALE_PROFILE on winning config, dgx1) — RE-PRIORITIZES THE GOAL
Measured WHERE prefill TTFT actually goes, to gate the multi-day GEMM rewrite. Served winning config
(METRALE_INT8_PREFILL=1 int8 faith2 CONFIRMED active, interval-16, FP8-KV, util 0.80, slots 64).
Cold 19.4k prefill = 46s (256-tok chunks ~593ms; attn layers L3,7,..63 ~18ms TOP, GDN ~6ms).
WARM 238-tok suffix prefill (representative of TTFT-MEDIAN turns) = 738.9ms, per-component (METRALE_PROFILE):
  | component                     | time    | % TTFT | targeted by |
  | ATTENTION (16 layers, 19k ctx)| ~289ms  | **39%**| (NOT in plan — attn_prefill.cu) |
...
★ RE-PRIORITIZED LEVER ORDER (by measured TTFT impact):
  1. ATTENTION PREFILL (39%) — attn_prefill.cu, the file with the over-generalized "ldmatrix broken"
     ...
     Compare vs llama flash-attention prefill on this shape. NEEDS: ncu attn_prefill, then ldmatrix-ize.
...
for llama AND every Metrale Engine run (not targeted; benchmark only scores Wall/TTFT/full-Latency). Reconstructed
...
counting BOTH natural-language text AND tool-call JSON (most agentic tokens are tool calls):
  ...
  | Metrale Engine int8+MTP (evictfix)   | 14.2 tok/s | avg 12.5 / med 12.2 / max 20.5 |
  | Metrale Engine bf16 (no MTP)         | 14.1 tok/s | 12.3 median             |
Metrale Engine decode ~+49% vs llama — this (MTP 2× + tail control), NOT prefill TTFT, is why Metrale Engine wins the WALL
(llama actually wins TTFT median 1393 vs Metrale Engine 1936). Reported metrics standing vs llama cfff1fc:
...
## ★★★★★★ ATTENTION PREFILL = THE LEVER (2026-06-28, gated by arithmetic from METRALE_PROFILE)
...
LIKELY ROOT: head_dim=256 has no specialized MMA path (attn_prefill_h128.cu = head_dim 128 only) →
...
NEXT: ncu the active kernel (attn_prefill_paged_fp8_batched, FP8-KV config) to confirm scalar/low-util
...
ncu on attn_prefill (attn compute core, microtest, sm_121): **Theoretical Occupancy 16.67%**,
...
NEXT: retile attn_prefill HDIM=256 with FP8 smem K/V (or smaller BC tile) for 2 CTAs/SM; gate cosine
(microtest) + per-attn-layer ms (METRALE_PROFILE) + agentic TTFT. This targets the 39% — the real path to
...
Implemented FP8-smem K/V (store raw E4M3 in smem, dequant-in-register at MMA) behind METRALE_ATTN_FP8_SMEM
(prefill_paged_compute.cuh + fp8/fp8_batched wrappers + new attn_prefill_fp8_microtest). GATED on dgx1:
...
TTFT are both hard-bound on the current kernel structures. Remaining options (all hard/uncertain or a decision):
  ...
  (E) ACCEPT the TTFT position: Metrale Engine already BEATS llama on wall (7196<8369), full-latency (4797<4956 med,

<script>
  ...
  //
  // Nothing here is a filter. Every rung Metrale Engine and vLLM both measured is on
  // chart A whether Metrale Engine wins it or loses it, and the headline tile names
  ...
  const tiles = $derived.by(() => {
    ...
    if (extremes.best) {
      ...
      out.push({
        value: `C=${b.c} · $${usdAt(b.metrale)} vs $${usdAt(b.rival)}`,
        label: `best rung · ${b.metraleCheaper ? 'Metrale Engine' : 'vLLM'} cheaper ×${(b.metraleCheaper ? 1 / b.ratio : b.ratio).toFixed(2)}`
      });
    }
    if (extremes.worst && extremes.worst !== extremes.best) {
      const w = extremes.worst;
      out.push({
        value: `C=${w.c} · $${usdAt(w.metrale)} vs $${usdAt(w.rival)}`,
        label: `worst rung · ${w.metraleCheaper ? 'Metrale Engine' : 'vLLM'} cheaper ×${(w.metraleCheaper ? 1 / w.ratio : w.ratio).toFixed(2)}`
      });
    }
...
>
  ...
  {#if cost.energyState === 'none'}
    ...
    <figure class="gate-panel cmp">
      ...
      <div class="cmp-empty">
        ...
        <p>Metrale Engine: {empty.metrale}.</p>
        <p>vLLM: {empty.vllm}.</p>
  ...
  {:else}
    ...
    <p class="cmp-caption">
      Per-token cost is throughput divided by power, so Metrale Engine is cheaper only where its throughput lead

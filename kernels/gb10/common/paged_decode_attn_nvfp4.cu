...
// 2026-09-25: Paged decode attention over an NVFP4 KV cache: 4-bit E2M1 indices and one FP8
// E4M3 scale per 16 elements. Entry points: paged_decode_attn_nvfp4 (one pass), and
// paged_decode_attn_splitk_nvfp4 with paged_decode_attn_reduce_nvfp4 (split-K).
//
// Owner: gb10 kernels.
// Invariants:
// - paged_decode_attn_nvfp4: grid (num_q_heads, num_seqs, 1); split-K: grid (num_q_heads,
//   num_splits, num_seqs); both 256 threads (NUM_WARPS * WARP_SIZE). Reduce: grid
//   (num_q_heads, num_seqs, 1) and 32 threads, one VEC_BF16 slice per lane.
// - Assumes the runtime head_dim equals HDIM, which sizes each lane's register slice.
// - A sequence with seq_len == 0 returns before any write in every kernel; its O rows
//   keep their bytes.
//
// A cache block holds block_size * num_kv_heads token-major head rows of head_dim / 2
// bytes, two indices per byte with the low nibble first, then from data_section_bytes one
// scale byte per 16 elements. There is no k/v scale argument and no sliding window.
//
// Split-K: split y of grid dimension y covers [y * ceil(seq_len / num_splits), the next
// split's start) and writes its unnormalised output, then m, then l, to workspace
// [num_seqs][num_q_heads][num_splits][head_dim + 2]. The reduce kernel merges splits
// 1..num_splits-1 into split 0, skipping any with l <= 0, and writes O as BF16.
...
// (HDIM=128) of E2M1 indices, low nibble first, each times the group's FP8 scale.

...
//!   * `…-config.json` — the checkpoint's own config, parsed by Metrale Engine's real `glm5_next` parser.
...
use metrale_core::config::{LayerType, parse_config};
...
/// sigmoid-weighted sum and Metrale Engine's CUDA kernel implements that one; GLM carries zero

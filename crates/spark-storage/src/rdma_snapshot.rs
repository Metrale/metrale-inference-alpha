...
mod imp {
    ...
    struct ArenaInner {
        ...
        /// dual-rail path (METRALE_SSM_STAGING); None = single-WR bounce fallback.
    ...
    impl RdmaSnapshotArena {
        ...
        /// bounces. Rail devices/GIDs reuse the KV env (`METRALE_EXPERT_RDMA_DEV`/`GID`
        /// = rail 0, `METRALE_KV_RAIL2_DEV`/`GID` = rail 1, dual only when
        /// `METRALE_KV_DUAL_RAIL=1`). `arena_bytes` = `arena_slots × blob_bytes`.
        ...
        fn connect_inner(
            ...
            // ONE contiguous staging buffer (METRALE_SSM_STAGING) registered on EVERY
        ...
        /// contiguous staging buffer (METRALE_SSM_STAGING). `write_src` set = WRITE

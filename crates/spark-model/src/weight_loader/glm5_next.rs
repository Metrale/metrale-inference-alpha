...
impl TensorRole {
    ...
    /// something. (Hazard carried from `notavault-metrale`: RMSNorm weights have
    /// silently corrupted Metrale Engine numbers before.)
    ...
    /// Text-model roles Metrale Engine must eventually implement. Vision is excluded.

...
// 2026-09-25: Gated delta rule (GDN) kernels for the qwen3.6-27b target: token-sequential prefill with split,
// split4 and multi-stream variants; single-token, 2-token and 3-token decode; decode fused with the gated RMS norm
// and with the conv1d update; strided multi-sequence decode; FP16 h-state twins.
// forked-from: kernels/gb10/common/gated_delta_rule.cu (2026-09-24; 2213 of 2514 lines differ, see kernels/FORKS.md)
//
// Per v-head, H is [k_dim, v_dim] (FP32, or __half in the FP16 twins). Per token, with g = clamp(gate, 1e-6, 1 - 1e-6):
//   v_new = (v - g * H^T k) * beta,   H = g * H + k v_new^T,   out = H^T q / sqrt(k_dim).
// One thread owns one v_dim column of H.
//
// This file shadows gb10/common/gated_delta_rule.cu by stem, so a kernel defined only there does not exist for
// this target.
//
// Owner: gb10 kernels (qwen3.6-27b, and the targets that list this file in `[sources] use`).
// Invariants: none beyond the types. The kernels assume k_dim <= 128 (the prefill kernels k_dim == v_dim == 128)
// and check neither. The single-stream prefill kernels read q/k/v and gate/beta at row t for every b.


#include <cuda_bf16.h>

#define K_DIM 128
// 2026-09-25: `gated_delta_rule_prefill`: Grid (num_v_heads, batch, 1), Block (128, 1, 1); H column in registers.
...
gated_delta_rule_prefill(
    ...
    // 2026-09-25: Two buffers each for k and q, 4 * K_DIM floats; the launcher passes 4 * k_dim * 4 bytes.
    ...
    for (unsigned int t = 0; t < seq_len; t++) {
        ...
        // 2026-09-25: Load the next token's k and q into the other buffer.
        if (t + 1 < seq_len) {
        ...
        // 2026-09-25: hk_dot = H^T k, as four independent partial sums.
        ...
        // 2026-09-25: Update H and compute q_dot = H^T q from the new values, as four partial sums.
        ...
        __syncthreads();  // 2026-09-25: next k/q stored; this token's buffers free to overwrite
    }

store_h:
    // 2026-09-25: When seq_len <= 1, scale this head's H down to Frobenius norm 100 if it is larger.
    if (seq_len <= 1) {
        float local_sq = 0.0f;
        #pragma unroll
        for (int j = 0; j < K_DIM; j++) {
...
// 2026-09-25: `gated_delta_rule_prefill_split`: `gated_delta_rule_prefill` with v_dim split across 2 blocks of 64
// threads per v-head. Block `split` owns columns split * 64 + tid_local, and each thread loads 2 of the 128 k and q
// values. It has no seq_len <= 1 norm clamp. Grid (num_v_heads * 2, batch, 1), Block (64, 1, 1).
...
// 2026-09-25: `gated_delta_rule_prefill_split4`: the same with 4 blocks of 32 threads per v-head; each thread loads
// 4 of the 128 k and q values. No seq_len <= 1 norm clamp. Grid (num_v_heads * 4, batch, 1), Block (32, 1, 1).
...
// 2026-09-25: `gated_delta_rule_prefill_split4_batched`: split4 over `batch_size` streams of the same seq_len. Stream
// b's state is at h_state_ptrs[b], and its q/k/v, gate/beta and output rows start b * seq_len rows into the stacked
// buffers. Per stream the computation is split4's.
...
// 2026-09-25: `gated_delta_rule_decode`, `_decode_f32`, `_chunk2` and `_chunk3`: one block per (v-head, sequence),
// H read from and written to global memory. chunk2 / chunk3 run 2 / 3 consecutive token rows (b * 2 + i, b * 3 + i)
// in one launch, writing the states after the earlier tokens to h_state_intermediate / h_state_inter0, inter1.
...
// 2026-09-25: The four kernels from `gated_delta_rule_decode_f32_norm` to `gated_delta_rule_decode_f32_strided_norm`
// have the same code as their namesakes in gb10/common/gated_delta_rule.cu; the shadow would otherwise drop them
// for this target.
...
// 2026-09-25: Per-head Frobenius clamp used by the decode kernels below: after the update, a head whose ||H||_F
// exceeds SSM_STATE_MAX_NORM is scaled down to that norm.
...
extern "C" __global__ void gated_delta_rule_decode_f32_norm(
    ...
    for (unsigned int j = 0; j < k_dim; j += 4) {
        ...
        q_dot += h0 * smem_q[j] + h1 * smem_q[j+1] + h2 * smem_q[j+2] + h3 * smem_q[j+3];
#ifdef SSM_STATE_NORM_ENABLED
        // 2026-09-25: The squared Frobenius norm for the clamp below, accumulated from the values just stored.
...
// 2026-09-25: `gated_delta_rule_decode_f32_conv_norm`: for one decode token, the conv1d update + SiLU (with L2 norm
// of q and k), the GDN recurrence and the gated RMS norm in one launch. Grid (num_k_heads, batch, 1), Block
// (head_repeat * v_dim, 1, 1). A block owns k-head kh and its head_repeat v-heads, so it is the only writer of their
// q, k and v conv_state rows. Thread tid serves v-head kh * head_repeat + tid / v_dim, column tid % v_dim.
// Channels of new_input and conv_state [batch, conv_dim, d_conv] are [Q (num_k_heads * k_dim) | K | V].
// conv_weight is [conv_dim, d_conv]; conv_bias may be null; z_gate and output are [batch, num_v_heads, v_dim].
// warp_sums[8] and the tid >> 7 groups assume 256 threads with k_dim == v_dim == 128 (head_repeat == 2); the caller
// admits the kernel only for that shape (qwen3_ssm/trait_decode_multi_seq/ssm_batched_recurrent.rs).
...
extern "C" __global__ void gated_delta_rule_decode_f32_conv_norm(
    ...
    // 2026-09-25: Step 1: threads [0, k_dim) update q channel kh * k_dim + tid, threads [k_dim, 2 * k_dim) the
    // matching k channel: conv update, then SiLU.
    float qk_silu = 0.0f;
    ...
    // 2026-09-25: L2-normalise q (warps 0-3) and k (warps 4-7) separately.
    {
    ...
    // 2026-09-25: Step 1b: conv update and SiLU for this thread's v channel, with no L2 norm.
    float v_i = 0.0f;
    {
    ...
    __syncthreads();   // 2026-09-25: smem_q and smem_k are complete

    // 2026-09-25: Step 2: the recurrence for v-head vh, column vlocal.
    ...
    // 2026-09-25: Frobenius clamp per v-head, reduced over that v-head's 128 threads.
    #ifdef SSM_STATE_NORM_ENABLED
    {
        float local_sq = 0.0f;
        for (unsigned int j = 0; j < k_dim; j++) {
    ...
    // 2026-09-25: Step 3: gated RMS norm per v-head, x * rms * norm_weight * silu(z), to output.
...
// 2026-09-25: `gated_delta_rule_decode_f32_strided`: `gated_delta_rule_decode_f32` plus the Frobenius clamp, for
// `batch_size` sequences whose q/k, v, gate/beta and output rows are qk_stride, v_stride, gb_stride and out_stride
// elements apart, so the rows can sit inside a wider per-sequence buffer.
...
// 2026-09-25: `gated_delta_rule_decode_f32_strided_norm`: `gated_delta_rule_decode_f32_strided` followed by the
// gated RMS norm (x * rms * norm_weight * silu(z)), writing BF16 output rows out_stride elements apart.
...
// 2026-09-25: Rows of each H column kept in registers by the _half and _smem kernels.
...
extern "C" __global__ void gated_delta_rule_decode_f32_strided_norm_half(
    ...
    float v_i = v_ptr[tid];
    float hk_dot = 0.0f;
    // 2026-09-25: Keep rows 0 .. GDN_HALF_KD - 1 of this thread's H column in registers, so the update pass re-reads
    // only the remaining rows. Every hreg index is a compile-time constant (fully unrolled loops); a runtime index
    // would put hreg in local memory.
    ...
    float norm_acc = 0.0f;
#endif
    // 2026-09-25: Update the retained rows from registers.
    #pragma unroll
    for (unsigned int j = 0; j < GDN_HALF_KD; j += 4) {
        float h0 = hreg[j + 0];
        float h1 = hreg[j + 1];
    ...
    // 2026-09-25: Update the remaining rows, re-read from H. j ascends across both loops, so q_dot and norm_acc add
    // in the same order as in `gated_delta_rule_decode_f32_strided_norm`.
...
// 2026-09-25: `gated_delta_rule_decode_f32_strided_norm_smem`: `gated_delta_rule_decode_f32_strided_norm_half` with
// rows GDN_HALF_KD .. 127 staged in shared memory (smem_h) on the first pass instead of re-read from H on the update
// pass. Each thread reads back only the values it staged, and the arithmetic and its order are unchanged, so the
// output and H match the _half kernel bit for bit; examples/gdn_strided_norm_microtest.rs compares the two.
// smem_h is sized for k_dim == 128 (GDN_HALF_KD + GDN_SMEM_KD); the caller admits the kernel only when
// k_dim == v_dim == 128 (ssm_batched_recurrent.rs).
...
extern "C" __global__ void gated_delta_rule_decode_f32_strided_norm_smem(
    ...
    __shared__ float smem_k[128];
    __shared__ float smem_q[128];
    // 2026-09-25: smem_h[r][tid] holds row GDN_HALF_KD + r of thread tid's column: GDN_SMEM_KD * 128 floats (32 KB).
    // Consecutive threads touch consecutive words, so the accesses have no bank conflicts.
...
// 2026-09-25: FP16 h-state twins, selected by --ssm-h-dtype f16 (METRALE_SSM_H_FP16). H is stored as __half and
// widened on load. All arithmetic is FP32, and q_dot and norm_acc use the values before narrowing; stores go
// through gdn_f16_store (saturate, then round to nearest even).
//
// FP16 rather than BF16: g * h rounds back to h when 1 - g is below half an ulp of h, and FP16's 10 mantissa bits
// put that threshold 3 bits lower than BF16's 7.
...
// 2026-09-25: Saturating FP16 store. The per-head clamp runs after the update, so it cannot bring back an element
// that overflowed to inf on store (inf * scale is inf), and an inf in H makes that head's next hk_dot inf. Clamping
// to +-65504 keeps H finite; such an element means the head's pre-narrowing norm exceeds SSM_STATE_MAX_NORM, so the
// clamp scales the head on the same step. For |v| <= 65504 the result equals __float2half(v).
...
// 2026-09-25: Rows of each H column kept in registers by `gated_delta_rule_decode_f16_strided_norm_half`.
...
extern "C" __global__ void gated_delta_rule_decode_f16_strided_norm_half(
    ...
    unsigned int z_stride,
    unsigned int out_stride,
    // 2026-09-25: Per-sequence pitch of the h-state pool, in __half elements. Under --ssm-h-dtype f16 the pool stays
    // FP32-sized and each slot holds its FP16 state in its first half, so the pitch is twice the dense FP16 size
    // (crates/model-layers/src/layers/qwen3_ssm/ssm_h_fp16.rs).
    ...
    float v_i = v_ptr[tid];
    float hk_dot = 0.0f;
    // 2026-09-25: hreg holds the values widened to FP32.
...
// 2026-09-25: FP16 h-state twin of `gated_delta_rule_decode_f32_norm`. It takes no slot pitch: sequence b's state
// starts b * num_v_heads * k_dim * v_dim __half elements in.

...
//! 2026-09-26: `gds-probe`: reports whether cuFile/GDS works on this host and measures
//! the storage read backends (`run_probe`). Run it with
//! `cargo run --release -p metrale-storage --example gds-probe -- --dir <dir>`; the report
//! goes to stderr, and the result as one JSON line to stdout.
//!
//! Owner: storage.
...
use metrale_storage::{ProbeConfig, run_probe};
...
fn main() -> Result<()> {
    tracing_subscriber::fmt()
        ...
        .init();
    let cfg = parse_args()?;
    eprintln!("== Metrale Engine storage probe ==");

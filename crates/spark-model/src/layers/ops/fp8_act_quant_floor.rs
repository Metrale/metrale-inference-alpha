...
/// `METRALE_FP8_ACT_QUANT_HOPPER` overriding ([`super::target_defaults`]).
///
/// No `METRALE_NO_*` rung: the two kernels emit the same bytes, so there is no
/// accuracy question for a kill switch to outrank what
/// `METRALE_FP8_ACT_QUANT_HOPPER=0` already says.
...
/// operator's `METRALE_FP8_ACT_QUANT_HOPPER=0` means "prefer the parent", and on

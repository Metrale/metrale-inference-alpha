...
| container | `metrale-gb10:followups` (any image with the CUDA runtime; the binary is bind-mounted in) |
...
METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b \
  cargo build --release -p metrale-server --bin met --features cuda
```

**`METRALE_TARGET_MODEL=qwen3.6-27b` is not optional.** Without it the build defaults to
...
```
metrale-kernels: compiled 158 kernels for target 0 (gb10, qwen3.6-27b, nvfp4)
```
...
```bash
METRALE_BIN=$PWD/target/release/met \
...
```
METRALE_NO_FFN_NVFP4_MMQ=1     METRALE_SSM_TAIL_MIDCHUNK=0    METRALE_MTP_CATCHUP=0
...
**Trap — presence flags.** Several `METRALE_*` flags are read with `is_some()` / `is_none()`, so
setting one to `0` still turns it **on**. `METRALE_NO_FFN_NVFP4_MMQ` and `METRALE_BF16_TC_PREFILL`
are presence-checked; to disable them you must unset them, not set them to zero. Others
(`METRALE_SSM_TAIL_MIDCHUNK`, `METRALE_MTP_CATCHUP`) do compare against `"0"`/`"1"`. Check the
callsite before assuming.

Kill-switches that are deliberately left at their defaults: `METRALE_GDN_WYN=0` disables the
...
between legs. It is not a Metrale fault and not a signal that anything regressed. If you see a

...
//!
//! NOTE: This is the Metrale-vendored copy of cudarc, trimmed to the only two
//! backends Metrale uses (`driver` + `nvrtc`). The upstream crate also wraps
...
// Metrale-vendored trim: only the `driver` and `nvrtc` backends are kept. The
...
// FFI bindings) have been removed — Metrale never references them.
#[cfg(feature = "driver")]
pub mod driver;
#[cfg(feature = "nvrtc")]
pub mod nvrtc;

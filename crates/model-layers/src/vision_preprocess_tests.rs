...
//! 2026-09-25: Tests for `vision_preprocess.rs`: target sizing, pad counts,
//! geometry refusals, decoder limits and EXIF orientation.
//!
...
fn still_image_pad_count_uses_the_spatially_merged_grid() {
    // 2026-09-25: A 2×2 spatial merge folds each patch block into one token.
    assert_eq!(
...
fn still_image_pad_count_is_identity_without_merging() {
    // 2026-09-25: spatial_merge_size 1: one token per patch.
    assert_eq!(
...
fn still_image_pad_count_treats_zero_merge_as_one() {
    // 2026-09-25: A zero merge size is treated as 1.
    assert_eq!(
...
/// 2026-09-25: A valid `vision_config`: patch 16, temporal patch 2, merge 2.
fn ok_cfg() -> VisionConfig {
    VisionConfig {
        ...
        // 2026-09-25: The tests pass any bound explicitly; the config-carried
        // one is not under test.
        max_pixels: None,
        ..VisionConfig::default()
    }
}

/// 2026-09-25: A 1×1 PNG.
const TINY_PNG_B64: &str = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==";

/// 2026-09-25: `parse_vision_config` reports a missing key as 0. A zero
/// `patch_size` is an error, not a divide-by-zero panic in a request.
#[test]
...
/// 2026-09-25: A zero `spatial_merge_size` would make `grid_unit` 0 and the
/// target 0×0; it is an error.
#[test]
...
/// 2026-09-25: A zero `temporal_patch_size` would give an empty pixel buffer; it
/// is an error.
#[test]
...
/// 2026-09-25: A valid config yields `num_patches × patch_dim` floats; a red
/// pixel normalises to 1.0 in the red channel and -1.0 in the others.
#[test]
...
/// 2026-09-25: Bad base64 and undecodable image bytes fail with different errors.
#[test]
...
/// 2026-09-25: A 33-byte PNG whose IHDR declares 65535×65535 (~12.9 GB of RGB).
/// The dimension limit rejects it from the header, before any pixel buffer is
/// reserved.
#[test]
fn decode_bomb_dimensions_are_rejected_before_allocation() {
    ...
    let crc = {
        // 2026-09-25: CRC-32 over the chunk type + data, as PNG requires.
    ...
    // 2026-09-25: The fixture must exceed the limit under test.
    const { assert!(65535 > DECODE_MAX_SIDE) };
    // 2026-09-25: Assert the limit error specifically: the fixture is also a
    // truncated PNG, so a decoder without limits would fail too, only later.
...
// 2026-09-25: grid_unit 32 = patch_size 16 × spatial_merge_size 2.
const GU: u32 = 32;

/// 2026-09-25: Merged vision tokens for a (h, w) target at patch 16, 2×2 merge.
...
fn no_declared_bound_keeps_the_1280_fallback() {
    // 2026-09-25: With no declared bound the long side is capped at
    // `FALLBACK_MAX_DIM` (1280).
...
fn declared_area_bound_raises_above_the_fallback() {
    // 2026-09-25: A declared area bound (16777216 = 4096²) replaces the
    // fallback clamp, so the long side can exceed 1280.
...
fn declared_area_bound_still_downscales_when_exceeded() {
    // 2026-09-25: 8192² is 4× over the bound and comes back to the bound, not
    // to 1280.
...
fn declared_area_bound_survives_grid_rounding() {
    // 2026-09-25: Grid rounding alone would give 96×2752 = 264,192 pixels, over
    // the 262,144-pixel bound; the shrink step brings it back inside.
...
fn absolute_ceiling_bounds_a_pathological_aspect_ratio() {
    // 2026-09-25: max_pixels is an area, so a 1×N strip satisfies any area
    // bound at an unbounded long side; ABS_MAX_DIM bounds it.
...
fn an_aligned_small_image_is_not_scaled_up_to_a_large_declared_bound() {
    // 2026-09-25: A small image under a large bound is not scaled up.
...
/// 2026-09-25: Copies of the vision benchmark's EXIF fixtures
/// (`crates/bench/assets/vision/`, byte-identical at this commit): red top
/// half, blue bottom half; one tagged `Orientation = 6`, one untagged.
...
/// 2026-09-25: Mean red value over a region whose corners are given as fractions
/// of the image size.
...
/// 2026-09-25: The untagged control: nothing to apply, so red stays on top.
#[test]
...
/// 2026-09-25: The tagged image comes out rotated. `Orientation = 6` means
/// "rotate 90° clockwise to display", which moves the stored top edge to the
/// right side, so the red half ends up on the right.
#[test]
...
/// 2026-09-25: Rotation happens before the grid snap, so a landscape-shaped
/// stored image tagged as portrait is measured at its displayed shape. The
/// asymmetric fixture makes an ignored or late-applied orientation swap
/// `(4, 2)` back to `(2, 4)`.
#[test]
fn exif_rotation_precedes_grid_sizing() {
    ...
    // 2026-09-25: Insert a minimal big-endian EXIF APP1 segment carrying
    // Orientation=6 immediately after the JPEG SOI marker.

...
//! 2026-09-25: Steady-state guard for CUDA-graph borrowing (`graph_borrow.rs`).
//!
//! A borrow replays a wider captured graph for a batch whose exact key missed,
//! padding the tail rows. It never captures the narrow key, so a batch that
//! settles below an earlier peak width keeps replaying the wide graph. Measured
//! 2026-09-23 on dgx1 (Qwen3.8-27B NVFP4): after one 32-wide burst, a steady
//! C=16 verified through the captured 32-seq graph (64 rows, the tile/MMQ
//! kernels instead of the 32-row GEMV). Acceptance was unchanged (tok_step
//! 1.70), but each step was about 35% slower: 184 tok/s against 223-243 tok/s
//! for the same C=16 with no burst before it.
//!
//! After [`STEADY_BORROW_LIMIT`] consecutive borrows for one exact key the
//! borrow is declined, so that step captures the exact key. Kill switch:
//! `METRALE_NO_BORROW_STREAK_LIMIT` (any non-empty value, `0` included,
//! disables the guard; gate records disclose it in `PERF_CONTROLS`, default
//! `unset`).
//!
//! Owner: model-engine (CUDA-graph cache).
//! Invariants: none beyond the types.

use std::sync::Mutex;

/// 2026-09-25: Consecutive borrows of one exact key before that key gets its own
/// capture. A borrowed step runs every replay at the borrowed width, which
/// `borrow_width_cap` allows up to `2 * n.next_power_of_two()` rows; a capture
/// costs one eager step.
pub(super) const STEADY_BORROW_LIMIT: u32 = 8;

/// 2026-09-25: The `METRALE_NO_BORROW_STREAK_LIMIT` rule: the guard is on unless
...
/// 2026-09-25: Consecutive-borrow counter for one graph family (decode or verify).
...
impl BorrowStreak {
    ...
    /// 2026-09-25: Record one borrow the caller is about to make for `exact_key`.
    /// Returns false once this key has borrowed [`STEADY_BORROW_LIMIT`] times in
    /// a row: the caller then declines the borrow and captures the exact key.
    ...
    fn allow_with(&self, exact_key: &[u32], enabled: bool) -> bool {
        // 2026-09-25: FNV-1a-style hash over the length and values; 0 is reserved
        // for "no key yet".
...
mod tests {
    ...
    fn a_changing_drain_key_keeps_borrowing() {
        ...
        for i in 0..(4 * STEADY_BORROW_LIMIT) {
            // 2026-09-25: A drain: the composition changes before the limit.

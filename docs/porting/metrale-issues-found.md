...
**Where:** `crates/metrale-kernels/build_target.rs` (`ScaleTarget::compile`)
...
## #A2 — `inferspark_prefill` LDS footprint exceeds RDNA3.5 64 KB cap 📋 Metrale-side **(gfx1151-specific)**
...
## #A2 — inferspark_prefill LDS footprint exceeds RDNA3.5 64 KB cap (open, Metrale-side, gfx1151-specific)
Where: kernels/.../inferspark_prefill*.cu; surfaces at runtime as
"cuFuncSetAttribute(MAX_DYNAMIC_SHARED=69688) failed: CUDA_ERROR_INVALID_VALUE"
at prefill layer 0.
The prefill attention kernel requests ~68 KB dynamic shared memory (Q/K/V/P

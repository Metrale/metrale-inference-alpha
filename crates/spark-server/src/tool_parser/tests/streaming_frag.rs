...
//! decode. Metrale Engine's StreamingToolDetector is structurally immune
...
fn qwen3_coder_streaming_same_name_tool_calls_no_collision() {
    ...
    // collide two consecutive `Read` calls into one. Metrale Engine keys by
...
fn qwen3_coder_streaming_close_with_final_value_in_same_chunk() {
    ...
    // first and dropped the value. Metrale Engine's buffer-until-close design

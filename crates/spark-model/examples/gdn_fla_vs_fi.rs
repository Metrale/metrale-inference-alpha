...
//! Cross-impl A/B: Metrale Engine FLA gdn_prefill_fla vs FlashInfer chunk_gated_delta_rule.
//! Loads the SAME q/k/v/g/beta the FI reference used (/tmp/gdn_ref/*.bin) + FI's output
//! (o_ref.bin), runs Metrale Engine's full FLA scan on identical input, diffs the two outputs.
...
fn main() -> Result<()> {
    ...
    // Pack into Metrale Engine layout: qkv [t, conv_dim] bf16 ([Q|K|V]); gate_beta [t, 2*nv] f32.
    ...
    // Read Metrale Engine output (bf16) -> f32, compare to FI o_ref.
    let n = t * value_dim;

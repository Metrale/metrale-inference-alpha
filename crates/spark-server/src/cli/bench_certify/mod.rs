...
//! `met bench certify` — the certification campaign, in the binary.
//!
...
//! gate is a child `met benchmark run … --pull-request-gate`, the drift
...
use metrale_plugin::hardware::equivalence::EquivalencePolicy;
use metrale_plugin::{ArtifactStore, gate, history};
...
pub async fn certify_cmd(args: CertifyArgs) -> Result<i32> {
    ...
    let hardware = match &args.hardware {
        Some(h) => h.clone(),
        None => {
            let k = metrale_plugin::hardware::Hardware::probe().gate_key();
            if k == "unknown" {
    ...
    let Some(limits) = metrale_plugin::hardware::limits::limits(&root, &hardware)? else {
        bail!(
    ...
    // it would otherwise read as "another met process" below.
...
fn measured_secs(store: &ArtifactStore, id: &str) -> Option<(u64, u64)> {
    history::load(store, id)
        .into_iter()
        .find(|r| r.frame.status == metrale_plugin::result::RunStatus::Completed)
        .map(|r| {
            (

...
//! 2026-09-26: Tests of the headless driver. They run the registry's
//! `concurrency-sweep` against `127.0.0.1:1`, where its first step's endpoint
//! probe fails, so the whole loop (start, drain, terminal frame, record,
//! save) runs without a server.
//!
//! Owner: bench.
...
/// 2026-09-26: Counts what the driver reported to the caller.
#[derive(Default)]
struct Counting {
    started: usize,
    events: usize,
...
/// 2026-09-26: A small `concurrency-sweep` request aimed at port 1, which the
/// tests assume nothing listens on.
...
fn a_run_produces_a_record_and_writes_it() {
    ...
    assert_eq!(outcome.record.metrale_version, "1.0.0-beta-preview");
...
fn an_invalid_parameter_fails_before_anything_runs() {
    ...
    values.set("osl", crate::params::ParamValue::Int(-5));

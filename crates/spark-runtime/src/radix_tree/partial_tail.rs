...
//! reader of `METRALE_PREFIX_SUBBLOCK`.
//!
...
//! `METRALE_PREFIX_SUBBLOCK=1` restores the pre-#1193 behaviour completely
...
/// `METRALE_PREFIX_SUBBLOCK=1` re-enables sub-block tail matching (unsound; see
...
/// can turn an unsound path on must be unambiguous — `METRALE_PREFIX_SUBBLOCK=

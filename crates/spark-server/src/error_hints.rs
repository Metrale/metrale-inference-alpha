...
//!
//! Benchmarks live in `metrale-plugin`, and `spark-server` depends on
//! `metrale-plugin`, not the reverse — so the plugin crate cannot import this
...
//! every client already reads, so `metrale-plugin` gets it for free by surfacing
...
// Reading an error body back out is `metrale_plugin::http`'s job, not this
...
// `metrale_plugin::http::{message_from_body, error_message_from_response}`.

#!/usr/bin/env python3
"""
Gemma-4 HuggingFace reference dump for Metrale Engine divergence diagnosis.

Purpose
-------
Metrale Engine's Gemma-4-31B (NVFP4) produces degenerate output starting around
...
Metrale Engine to isolate the first divergent layer.
...
Arguments:
    ...
    --output   JSON path. Default: /workspace/metrale/tests/gemma4_hf_reference.json
    --device   cpu | cuda. Default: cpu (safe when GPUs are busy; warns slow).
    --dtype    Default: bfloat16.

How to diff against Metrale Engine
-------------------------
Metrale Engine has a diagnostic env var `METRALE_DIAG_GEMMA4_HIDDEN=1` that logs per-layer
hidden-state norms during decode. Run Metrale Engine with that env var on the same
prompt, then compare layer-by-layer norms / first-8 values / abs-max indices
against the JSON this script writes.

Relevant Metrale Engine files:
    /workspace/metrale/crates/model-layers/src/layers/qwen3_attention/trait_impl.rs
        - decode forward + `gemma4_diag_enabled()` instrumentation
    /workspace/metrale/crates/model-arch/src/weight_loader/gemma4.rs
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--output", default="/workspace/metrale/tests/gemma4_hf_reference.json")
...
def main() -> int:
    ...
    # Apply chat template to match how Metrale Engine serves the prompt.

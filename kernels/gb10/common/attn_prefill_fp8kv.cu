...
// 2026-09-25: Kernel `attn_prefill_fp8kv_64`: contiguous prefill flash attention with BF16 Q and unscaled FP8 E4M3 K/V.
//
// Q and O are [batch, seq, num_q_heads * head_dim]; K and V are [batch, seq, num_kv_heads * head_dim]. K and V tiles
// are dequantized to BF16, with no scale, as they are written to shared memory. A block takes 64 query rows of one head
// on 256 threads: warps 0-3 compute QK^T and the softmax for 16 rows each, and all 8 warps compute P*V (warps w and
// w + 4 share 16 rows, each taking half of the head_dim columns). K is double-buffered. Under __SCALE__ a block takes
// 32 rows and K is single-buffered. Grid (num_q_heads, ceil(seq_len/64), batch). No host code launches this kernel.
//
// Owner: gb10 kernels.
...
// 2026-09-25: Software E4M3 decode for the SCALE build, where fp8_to_bf16 uses it instead of
// __nv_cvt_fp8_to_halfraw. Finite codes decode exactly; the NaN codes (exponent 15, mantissa 7) decode to 0.
...
// 2026-09-25: The __SCALE__ build takes 32 query rows per block and a single K buffer, to reduce shared memory for the
// AMD target.
...
// 2026-09-25: Dequantizes 8 E4M3 values at `src` into smem_row[col .. col + 8) with one 16-byte store.
...
extern "C" __global__ void attn_prefill_fp8kv_64(
    ...
    for (unsigned int kv_block = 0; kv_block < num_kv_blocks; kv_block++) {
        ...
        // 2026-09-25: No barrier between the V load and QK^T: QK^T reads only smem_Q and smem_K, and the barrier after
        // the softmax orders every V write before P*V reads smem_V64.
        ...
        // 2026-09-25: Warps 4-7 ran no softmax; they rescale their P*V accumulators to the new row max in smem_ml64.
        if (warp_id >= 4) {
        ...
        // 2026-09-25: Next K tile, into the other buffer on NVIDIA builds; P*V below does not read smem_K64.
        ...
        if (kv_block + 1 < num_kv_blocks) {
            ...
            for (unsigned int idx = tid; idx < TILE_CHUNKS_KV; idx += 256) {
                ...
                if (k_row < seq_len) {
                    LOAD_FP8_CHUNK(smem_K64[METRALE_KB(1 - buf)][row], col,

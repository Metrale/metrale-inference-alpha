#!/bin/bash
# Phase A driver: the C=[1,2,4,8,16] synthetic scoreboard, Metrale Engine vs vLLM, one box.
#
...
#  - Metrale Engine runs its golden env/flags with --max-batch-size 16 and fifo
...
#    a throughput sweep).
#  - vLLM is tried FIRST on the SAME checkpoint Metrale Engine serves (centml W4A4); if
...
METRALE_BIN=$WT/conc_sweep/spark_phaseA_baseline

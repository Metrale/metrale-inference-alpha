...
// 2026-09-25: Causal attention for num_tokens query tokens against contiguous
// K/V, one threadgroup per (token, head) on a flat 1-D grid of
// num_tokens * num_heads threadgroups:
//
//   scores[m, s] = (Q[m, h] · K[s, kv_h]) * scale, -∞ for s > m
//   out[m, h, d] = sum_s softmax(scores[m])[s] * V[s, kv_h, d]
//
// with kv_h = h / (num_heads / num_kv_heads). Query m sits at position m: Q
// and K/V both start at position 0.
//
// Layout:
...
// Owner: metal kernels. Invariants: assumes seq_len <= MAX_SEQ_PREFILL; the

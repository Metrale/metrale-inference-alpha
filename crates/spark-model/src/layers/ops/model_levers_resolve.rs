...
pub(super) fn from_values(
    ...
    ModelLevers {
        ...
        // ON, which is the shipped default. `METRALE_NO_DECODE_SPLIT_SILU` stays
...
impl ModelLevers {
    ...
    /// of `METRALE_*` environment variables, which cannot change after start —
    ...
    /// path re-reads every `METRALE_*` variable.
    ...
    /// What a build resolves to with no `METRALE_*` set — every opt-in off, the
    ...
    pub fn defaults() -> Self {
        Self {
            ...
            ffn_small_m: true,
            gemv_sw: true,
            // Opt-out: ships ON, `METRALE_SSM_GEMV_BATCH4=0` disables. Every

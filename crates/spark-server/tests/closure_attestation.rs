...
//!
//! `metrale-kernels/build.rs` computes a target's sources with `collect_cu_files`
//! and bakes the hash into the binary. The gate recomputes them with
//! `metrale_plugin::gate::taxon::sources` and compares. Those are two separate
...
use metrale_plugin::gate::closure::{Attestation, TargetClosure};
use metrale_plugin::gate::taxon;
...
///
/// Skipped when the binary carries no attestation — an `METRALE_SKIP_BUILD=1`

...
The build script (`crates/metrale-kernels/build.rs`) walks this tree and
...
2. **Write `MODEL.toml`**:
   ...
   # crates/metrale-kernels/src/resolve.rs.
...
5. **Build with the wildcard target**:
   ```
   METRALE_TARGET_MODEL='*' cargo build --release -p spark-server
   ```
...
1. **`kernels/<new-hw>/HARDWARE.toml`**. The keys are exactly the ones
   `crates/metrale-kernels/build.rs` reads, plus documentation:
   ```toml
   [hardware]
   ...
   `crates/metrale-kernels/tests/target_hints.rs` asserts that every
   ...
   **sm_103** is Blackwell Ultra (B300/GB300), sm_120 is consumer Blackwell,
   ...
   The value also has to be reachable: `crates/metrale-kernels/tests/target_hints.rs`
   ...
   **Benchmark limits** (`[benchmarks.limits.{thermal,memory,timing,equivalence}]`,
   `metrale_plugin::hardware::limits`): what `met bench certify` and the
...
B200. `crates/metrale-kernels/tests/inherited_targets.rs` holds both trees to the
same assertions.
...
1. **`crates/metrale-core/src/config.rs`**: extend the quant detection
...
`crates/metrale-kernels/build.rs` runs `nvcc --ptx` only (`NvidiaTarget::compile`

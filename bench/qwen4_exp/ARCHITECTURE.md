...
The model **loads** today; it does not serve because these
...
### Why Metrale Engine's existing mHC kernels are the wrong ones
...
- it runs on ONE layer (`ple_layer_ids = [2]`), and its 320M-row table is
  served off NVMe by the LongCat n-gram row cache
...
q(4×128) + k(1×128). Metrale Engine has DeepSeek-V4's CSA machinery
...
## 4. Correction: the n-gram HASH does not transfer from LongCat

`ARCHITECTURE.md` §2 said the n-gram machinery from
LongCat was reusable for PLE. That is **half right, and the wrong
...
reading, and it is why LongCat's `ngram_parity.py`-style bit-exactness harness
...
§2 above read `ple_layer_ids: [2]` as "model layer 2".
...
Metrale Engine already dispatches this globally via `ships_vanilla_norm_weights`
(`crates/model-layers/src/lib.rs`), which lists only `deepseek_v4` and

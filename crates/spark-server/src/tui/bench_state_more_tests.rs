...
fn selecting_past_the_end_of_the_registry_lands_on_the_last_benchmark() {
    ...
    let mut s = state();
    s.select(usize::MAX);
    assert_eq!(s.selected, metrale_plugin::registry::all().len() - 1);
...
fn a_number_outside_its_domain_names_the_bound_it_broke() {
    ...
    let row = s
        .specs
        .iter()
        .position(|spec| matches!(spec.kind, metrale_plugin::ParamKind::Int { .. }))
...
fn a_start_with_the_probe_switched_off_still_refuses_for_the_real_reason() {
    ...
    let mut s = state();
    s.coherence = metrale_plugin::CoherencePolicy::Skip;

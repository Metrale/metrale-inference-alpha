...
impl TransformerLayer for K3BoundLayer {
    ...
    fn decode(
        ...
        self.decode_host(
            ...
            ctx,
            stream,
            metrale_core::kimi_k3::cuda_kda_enabled(),
            metrale_core::kimi_k3::cuda_mla_enabled(),
        )
    }
    ...
    fn alloc_state(&self, _gpu: &dyn GpuBackend) -> Result<Box<dyn LayerState>> {
        let cache = match self.spec.mixer {
            MixerKind::Kda => {
                LayerCache::Kda(metrale_core::kimi_k3::KdaState::new(&self.shared.kda))
            }
            MixerKind::Mla => LayerCache::Mla(metrale_core::kimi_k3::MlaKv::default()),
        };

...
/// `METRALE_NO_LMHEAD_TGEMM=1`. Evaluated ONCE at construction — the switch
/// decides whether the transposed twin is built at all, so setting it later has
/// no effect. Presence-style check (`METRALE_*=0` is NOT "off").
///
...
impl TransformerModel {
    pub fn new(
        ...
        // Lossless BF16-MMA sibling for lm_head, OPT-IN via METRALE_LMHEAD_LOSSLESS=1.
        ...
        // `METRALE_LM_HEAD_M16_TC` decides whether it is ever launched.
        ...
        // Fail fast if an SSM tier was requested (`METRALE_SSM_TIER`) on a model
        ...
        // Default ON (kill: METRALE_NO_LMHEAD_TGEMM=1). KV impact is nil: the pool
        ...
        // via `METRALE_GEMMA4_FP32_LMHEAD=1`. Keep `use_fp32_logits=false`
        ...
        // a GPU-side FP32 argmax kernel. `METRALE_GEMMA4_FP32_LMHEAD=1`
        ...
        // FP32 logits (METRALE_GEMMA4_FP32_LMHEAD) required an FP32 residual
        ...
        Ok(Self {
            ...
            suppress_graphs: std::sync::atomic::AtomicBool::new(
                has_fp8_calibration
                    ...
                    // so METRALE_DEBUG_SYNC_KERNELS can synchronize per launch and

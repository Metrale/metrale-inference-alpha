...
# 2026-09-26: Per-metric Mann-Whitney U comparison of two tiers, Bonferroni-adjusted.
#
...
def mannwhitney_u_self(a: list[float], b: list[float]) -> tuple[float, float]:
    ...
    # 2026-09-26: Tied values share the average of their ranks.
    ...
    while i < len(combined):
        ...
        while j + 1 < len(combined) and combined[j + 1][0] == combined[i][0]:
            j += 1
        avg = (i + j + 2) / 2.0  # 2026-09-26: mean of the 1-indexed ranks i+1..j+1
    ...
    U_b = n_a * n_b - U_a
    U = min(U_a, U_b)
    # 2026-09-26: Normal approximation, without a tie correction.
    mu = n_a * n_b / 2.0
    sigma = (n_a * n_b * (n_a + n_b + 1) / 12.0) ** 0.5
    if sigma == 0:
        return (U, 1.0)
    z = (U - mu) / sigma
    # 2026-09-26: Two-sided p.
...
def main() -> int:
    ...
    rows.sort(key=lambda r: r[5])  # 2026-09-26: by Bonferroni-adjusted p

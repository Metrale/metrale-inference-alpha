...
//! `met doctor` — is this box ready to run a benchmark?
//!
...
//!
//! * `~/.metrale` owned by uid 1000 while the process was uid 996, so the recipe
//!   index was unwritable. Nothing in the tree checked ownership.
//! * `sync-recipes` never run on a fresh box.
//! * A signing identity minted into a scratch `METRALE_HOME`, so a campaign's
...
use anyhow::Result;
use metrale_plugin::artifacts::{HomeFault, MetraleHome};
...
pub fn check_recipes() -> Finding {
    ...
    // rename the index is under `metrale-recipes`, and doctor reporting "no
...
/// `met doctor`. Exit code 1 when anything is wrong, so a script can rely on it.

...
//! 2026-09-25: The Qwen3.8-27B attention o_proj shape (N = hidden 5120,
//! K = q_heads x head_dim = 6144, `kernels/hopper/qwen3.8-27b/MODEL.toml`):
//! per-row scalar `w8a16_gemv` against `w8a16_gemv_batch4` in chunks of 4
//! rows (M <= 5) and `w8a16_gemv_batch16` (M > 5).
//!
//! Owner: model-arch examples (GPU microtests).
//! Invariants:
//! - The run exits 0 only if, at every M in {2, 4, 5, 8, 16}, the batched
//!   output equals the scalar output byte for byte, both are finite, neither
//!   wrote into the guard bands, and each known-bad mutation was refused.
...
fn main() -> Result<()> {
    ...
    let scales: Vec<u8> = (0..N / 128 * (K / 128))
        ...
        .collect();
    // 2026-09-25: Rows past ORIGINAL_M are drawn after the scales, so the
    // first ORIGINAL_M rows and the scales do not depend on MAX_M.
    ...
    for m in [2_usize, 4, 5, 8, 16] {
        ...
        if m > 5 {
            // 2026-09-25: `w8a16_gemv_batch16`, launched directly.

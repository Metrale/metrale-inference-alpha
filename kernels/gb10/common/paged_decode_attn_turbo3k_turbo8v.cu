...
// 2026-09-25: Paged decode attention over a Turbo3 K pool (3-bit indices, 8-entry codebook, one
// FP8 E4M3 scale per 16 elements) and a Turbo8 V pool (FP8 E4M3 values, one BF16 scale per 16
// elements). Each pool has its own block stride and data-section size.
//
// Owner: gb10 kernels.
// Invariants:
...
//
// Rows are token-major: head_dim * 3 / 8 bytes (K) or head_dim bytes (V) per head, then from the
// pool's data-section size one scale per 16 elements (1 byte for K, 2 for V). The kernel applies
// no WHT: Q must be in the basis K and V were written in, which the caller arranges
...
// 2026-09-25: Turbo3 K, bit layout as paged_decode_attn_turbo3.cu; at HDIM=128 even lanes take 0-3.
...
// 2026-09-25: Turbo8 V: VEC_BF16 E4M3 bytes, low byte first, each times the group's BF16 scale.

...
//! 2026-09-26: What has to be true before a campaign runs its first unit.
//!
//! Owner: server CLI (`met benchmark certify`).
//! `gather` reads the facts once; `evaluate` is a pure function over them, so
//! each refusal is tested by flipping exactly one fact.
...
pub struct PreflightFacts {
    ...
    pub metrale_home: String,
    pub metrale_home_writable: bool,
    pub other_engine_pids: Vec<u32>,
    /// 2026-09-26: `MemAvailable / MemTotal`; `None` when `/proc/meminfo` is unreadable.
    pub free_fraction: Option<f64>,
    /// 2026-09-26: The class's floor for it (`HARDWARE.toml` `[benchmarks.limits.memory]`),
    /// the same number the child's self-start applies
    /// (`bench_selfstart::check_box_is_free_enough`).
...
/// 2026-09-26: Every reason not to start, or none.
    ...
    if !f.committed_signers.iter().any(|s| s == &f.signer) {
        out.push(Finding(format!(
            "signer {} ({}) is not committed in .github/record-signers/ — every record \
             ...
             METRALE_HOME at an identity that is committed)",
            f.signer, f.metrale_home
        )));
    }
    if !f.metrale_home_writable {
        out.push(Finding(format!(
            "METRALE_HOME {} is not writable: the run history and the signing identity \
             live there",
            f.metrale_home
        )));
    }
    if !f.other_engine_pids.is_empty() && !f.remote_only {
        out.push(Finding(format!(
            "another met process is running (pid {}): a gate needs the GPU to itself, \
...
/// creates one under `METRALE_HOME` if there is none.
pub fn gather(
    root: &Path,
    anchor: &str,
    ...
    let store = metrale_bench::ArtifactStore::discover().context("locating METRALE_HOME")?;
    let metrale_home = store.root().display().to_string();
    let identity = gate::signing::load_or_create(store.root())
        .with_context(|| format!("loading the signing identity under {metrale_home}"))?;
    let committed_signers = gate::signing::committed_signers(root)?;
    let metrale_home_writable = {
    ...
    Ok(PreflightFacts {
        ...
        committed_signers,
        metrale_home,
        metrale_home_writable,
        other_engine_pids: other_engine_pids(),
...
/// name (`pgrep -x met`). Empty when `pgrep` cannot run.
pub fn other_engine_pids() -> Vec<u32> {
...
/// 2026-09-26: `MemAvailable / MemTotal` from `/proc/meminfo`, the same ratio the
/// self-start applies; `None` when unreadable or `MemTotal` is not positive.
...
mod tests {
    ...
    fn clean() -> PreflightFacts {
        PreflightFacts {
            ...
            metrale_home: "/x".into(),
            metrale_home_writable: true,
            other_engine_pids: vec![],
            free_fraction: Some(0.95),
            min_free_fraction: 0.85,
            guard_ref: Some("metrale/main".into()),
            no_guard: false,
    ...
    /// 2026-09-26: `--remote-only` waives the two findings about this box's `met`
    /// processes and memory and nothing else.
    #[test]
    fn remote_only_waives_the_local_box_findings_only() {
        let mut f = clean();
        f.other_engine_pids = vec![4242];
        f.free_fraction = Some(0.10);
        assert_eq!(evaluate(&f).len(), 2);
        f.remote_only = true;
        assert!(evaluate(&f).is_empty(), "{:?}", evaluate(&f));
        // 2026-09-26: Negative control: an uncommitted signer is still refused.
    ...
    /// 2026-09-26: Each fact flips exactly one finding, and the finding names the remedy.
    #[test]
    fn each_fact_flips_one_finding() {
        ...
        let cases: Vec<(&str, Flip, &str)> = vec![
            ...
            (
                "home",
                Box::new(|f| f.metrale_home_writable = false),
                "not writable",
            ),
            (
                "met",
                Box::new(|f| f.other_engine_pids = vec![4242]),
                "pid 4242",
            ),
    ...
    fn the_memory_threshold_is_the_declared_floor() {
        ...
        // 2026-09-26: Another class, another floor: the same reading passes at 0.5.

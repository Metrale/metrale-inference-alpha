...
/// only by `METRALE_NO_SSM_FFN_PREFILL=1`.
///
...
/// C=4/C=8 have headroom too — `METRALE_SSM_FFN_PREFILL_MIN_N` exists to find it
...
impl Qwen3SsmLayer {
    ...
    pub(super) fn decode_multi_seq_inner<'a, 'b: 'a>(
        ...
        match n {
            ...
            //
            // MEASURED 2026-07-27 (METRALE_SSM_MS_PROFILE, 9168 samples/n): the
            ...
            _ => {
                        ...
                        == Some("1")
                {
                    ...
                    // Opt out fully with METRALE_MOE_LEGACY_PERTOKEN_DECODE=1.
                    if self
                        .ffn

# 2026-09-26: Runs the reference QSA indexer of model layer 3 on a seeded T=2200 input
# with checkpoint weights, and writes the mask and intermediates to an npz and to raw
# .bin files plus meta.json in <out>_bins for qsa_tests.rs.
#
...
LAYER = 3  # 2026-09-26: the first full_attention entry of the cached config's layer_types.
T = 2200
SEED = 0
...
def main() -> None:
    ...
    # 2026-09-26: For a bool causal mask the indexer returns a bool [1, 1, T, T] mask.
    selected = mask.squeeze(0).squeeze(0).to(torch.bool)

    # 2026-09-26: Intermediates. The projections, norms and rope are the reference's;
    # the block pooling and scoring lines repeat `Qwen4ExpTextQSAIndexer.forward`.
    with torch.no_grad():
    ...
    # 2026-09-26: One .bin per tensor plus meta.json; qsa_tests.rs reads this directory
    # from METRALE_QSA_TEST_DATA.
    ...
    # 2026-09-26: FP32 cos and sin for all T positions. Their width is head_dim x
    # partial_rotary_factor (256 x 0.25 = 64 in the cached config).
    wbin('rope_cos', cos.squeeze(0), bf16=False)
    wbin('rope_sin', sin.squeeze(0), bf16=False)
    # 2026-09-26: The last query's selected token indices (row T-1), ascending, as i32.
    ...
    # 2026-09-26: The mask rows from token_budget + compress_ratio - 1 on, as row-major
    # u8 [rows, T].

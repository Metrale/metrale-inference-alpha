...
//! 2026-09-25: Per-stage report and gate of `kda_layer_microtest`: one `Row` of
//! floors and residuals per stage, and the pass rule on the final output and the
//! recurrent state.
//!
...
pub(crate) struct Row {
    ...
    /// 2026-09-25: Largest |value| in the f32 golden sample.
    pub(crate) mag: f64,
    /// 2026-09-25: Floor A: the pure CPU reference against the f32 golden (NaN without one).
    pub(crate) floor_a: f64,
    /// 2026-09-25: Floor B: the bf16 golden against the f32 golden.
    pub(crate) floor_b: f64,
    /// 2026-09-25: Floor D: the GPU against the bf16-rounding CPU reference.
...
/// 2026-09-25: Layers run against the real-checkpoint golden; `main` refuses any that is not
/// in the audit's `kda_layers` list.
...
pub(crate) fn stage_report(
    ...
    let Some(v) = golden else {
        // 2026-09-25: Without a golden only floor D exists; the run passes when its worst
        // residual is at most 5e-2.
        let d = [
    ...
    // 2026-09-25: E is the final output's GPU residual against the bf16 golden, printed beside
    // floor B.
    ...
    let _ = dm;
    // 2026-09-25: The final output and the recurrent state each pass when their residual against
    // the bf16 golden is at most max(8 × floor B, 1% of the stage's magnitude).

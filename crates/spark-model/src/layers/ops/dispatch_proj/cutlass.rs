...
/// `METRALE_CUTLASS_GEMM=1` and OFF by default; a build without `CUTLASS_HOME`
...
/// a faster Metrale Engine kernel, not a promotion. See the module docs on
...
/// `weight_t` must be Metrale Engine's transposed NVFP4 layout `[K/2,N]` plus
/// `[K/16,N]` scales, as produced by `QuantizedWeight::transpose_for_gemm`.
#[allow(clippy::too_many_arguments)]
/// Transpose a native NVFP4 checkpoint weight from Metrale Engine `[K/2,N]` into the
...
/// Metrale-transposed NVFP4 data/scales and reused for future calls.
#[allow(clippy::too_many_arguments)]

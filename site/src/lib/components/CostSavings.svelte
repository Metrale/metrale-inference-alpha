<script>
  // Energy savings over time: what serving the SAME token demand on Metrale Engine
  ...
  const result = $derived(
    savingsOverTime({
      metraleJPerTok: jOf(cost?.metrale),
...
<section class="cost-savings" aria-labelledby="cost-savings-h">
  ...
  {:else}
    ...
    <p class="cost-savings-figure" class:is-loss={losing}>
      {#if losing}
        Metrale Engine costs <strong>${fmtUsd(Math.abs(result.totalUsd))}</strong> more
      {:else}

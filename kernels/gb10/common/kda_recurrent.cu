...
//
// ⚠️ Metrale Engine's OWN Qwen GDN uses the OPPOSITE convention on BOTH axes:
...
// INPUT CONTRACT
// ─────────────────────────────────────────────────────────────────────────────
// `q` and `k` arrive **already L2-normalised** — Metrale Engine's convention, where
...
// and Metrale Engine's existing `h_state_bytes` comment `FP32 [nv, kd, vd]`. vLLM stores the
...
// Production entry point: bf16 q/k/v (Metrale Engine's conv writes bf16). Gate, beta, state and
// output stay fp32 — the recurrent state is fp32 by REFERENCE SEMANTICS, not by Metrale Engine
...
// `METRALE_GLM_KDA_NO_SMEM=1` restores the 2R+2W kernel.

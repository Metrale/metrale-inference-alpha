...
//! 2026-09-26: `met benchmark aggregate <group>`: the group's number from its shard records.
//!
//! Owner: server CLI (`met benchmark`).
//! GPU-free. It applies the gate's own partition rule
//! (`gate::group::select_partition`), so an incomplete partition (a missing
//! shard, or shards at different commits) is reported as incomplete.
//! Invariants: exit code 0 only when a complete partition was found.

use anyhow::{Result, bail};
use metrale_bench::benchmarks::bfcl::aggregate;
...
pub fn aggregate_cmd(args: AggregateArgs) -> Result<i32> {
    ...
    // 2026-09-26: Every readable shard record of this group at the commit (sha prefix
    // match either way) that carries tallies, newest first.
...
/// 2026-09-26: One shard record with its tallies, as read from the group's directory.
struct Row {
    shard: (usize, usize),
    git_sha: String,
    recorded_at: u64,

...
//! has never compiled.
//!
//! Controls: `METRALE_GRAMMAR_CACHE=0` disables persistence entirely;
//! `METRALE_GRAMMAR_CACHE_DIR` relocates the file (for a read-only model
...
/// `METRALE_GRAMMAR_CACHE=0` (or `false`/`off`/`no`) disables the on-disk
...
///
/// `METRALE_GRAMMAR_CACHE_DIR` wins when set — a model directory pulled

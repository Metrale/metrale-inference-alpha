...
// This is peer POLICY with no home in `metrale-tier`/`metrale-rdma`: the
...
// (deliberately NOT lifted to `metrale-tier`, whose charter excludes unsafe
...
use metrale_tier::{DirectSwapFile, Residency};
...
pub(super) fn get_or_init_shared_paging(
    ...
    // swap file (verify gap: orphaned metrale-snap-shared.swap on upgrade).
    if !reg.cap_init {

...
//! 2026-09-26: The Poolside v1 tool-call format, including its bare-name
//! zero-argument call.
//!
//! Owner: server (tool parsing).
//! Invariants:
//! - `parse_poolside_v1_call` returns `None` for a repeated key, and for a
//!   name or key that is not a plain name (`is_tool_name_component`).

use super::*;

/// 2026-09-26: Poolside v1 format:
...
impl ToolCallParser for PoolsideV1Parser {
    ...
    fn promotes_bare_call_names(&self) -> bool {
        // 2026-09-26: Poolside v1's zero-argument call is a bare name inside
        // the envelope: `<tool_call>get_status</tool_call>`. The trait
        // method's doc says why the default stays false.
        true
    }
...
mod tests {
    ...
    fn blocking_parser_accepts_complete_zero_argument_call() {
        // 2026-09-26: The bare-name form is a Poolside encoding, so a caller
        // opts in through the promoting entry point, chosen by
    ...
    fn default_parse_drops_bare_identifier_instead_of_fabricating_a_call() {
        ...
        // `<tool_call>` is malformed output; promoting it would make up a

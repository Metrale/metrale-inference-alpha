...
//! Real CUDA GPU backend using MetraleRegistry.
//!
//! SBIO IORouter: all CUDA operations flow through `GpuBackend`.
//! Uses `MetraleRegistry` for kernel loading/launching and raw CUDA
...
use metrale_core::registry::MetraleRegistry;
...
unsafe extern "C" {
    ...
    // `WithFlags` alias. `metrale_scale` (set by build.rs from METRALE_TARGET_HW)
...
/// Production GPU backend wrapping MetraleRegistry + raw CUDA driver API.
///
/// **Owns this model's kernel modules.** The registry used to be a process
/// singleton reached through `MetraleRegistry::get()`; it is now loaded per model
...
pub struct MetraleCudaBackend {
    ...
    /// `METRALE_DEBUG_SYNC_KERNELS=1` — sync after every launch. Read once here
    ...
    /// `METRALE_REDZONE=<bytes>` — every live allocation's trailing guard band.
    ///
    ...
    /// `METRALE_REDZONE_FILL` at birth. [`MetraleCudaBackend::scan_redzones`] reads them back and
...
/// One allocation's trailing guard band. See [`MetraleCudaBackend::scan_redzones`].
#[derive(Clone, Copy)]
...
/// `METRALE_REDZONE=<bytes>` — guard-band size, 0 (default) disables. Rounded up to 16.
...
/// `METRALE_REDZONE_MIN_IDX=<n>` — pad only allocations whose creation index is `>= n`.
///
...
/// so `METRALE_REDZONE_MIN_IDX=57346` guards exactly the plausible set for ~1 MB.
///
...
/// `METRALE_REDZONE_TRACE_IDX=<n>` — dump a Rust backtrace at the allocation with this creation
...
/// `METRALE_REDZONE_FILL=<decimal byte>` — the poison value, default `0xEE`.
///
...
impl MetraleCudaBackend {
    ...
    /// `metrale_kernels::ptx_for_model()` or `ptx_modules()` to obtain the
    ...
    pub fn sweep_unreleased(&self) -> usize {
        ...
        for (raw, _) in outstanding {
            ...
            if status != 0 && !metrale_core::registry::is_teardown_noop(status) {
...
/// described in `metrale_core::scope` — nothing is allocating against a backend
...
///
/// Applies the same rule as `MetraleCudaBackend`'s `free_memory`: host

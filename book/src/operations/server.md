...
Metrale Engine serves via `metrale-server` — an OpenAI and Anthropic compatible HTTP API over axum. This chapter is the operator's reference for CLI flags, protocols supported, and the knobs that matter in production. The authoritative flag list is always `met serve --help`; the headings below match the groupings in the CLI so cross-referencing is easy.

## CLI structure

```
met serve <MODEL> [--flags...]
met serve --model-from-path <PATH> [--flags...]
met --version
met --help
```

Every runtime configuration flag has a long-form name. Most are documented inline with `#[arg]` doc-strings in `crates/server/src/cli/serve_args.rs`.
...
| Flag | Default | Notes |
|---|---|---|
| `--gpu-memory-utilization` | `0.90` | Fraction of GPU memory Metrale Engine will claim |
...
| Flag | Default | Notes |
|---|---|---|
| `--kv-cache-dtype` | `fp8` | `bf16`, `fp8`, `nvfp4`, `turbo2`, `turbo3`, `turbo4`, `turbo8`, plus nine asymmetric K/V pairings — `KvCacheDtype`'s `FromStr` (`crates/cache/src/kv_cache.rs`) is the authority on the accepted set |
...
See [FP8](../deep-dives/fp8.md) and [NVFP4](../deep-dives/nvfp4.md) for the trade-offs. Metrale Engine's recommendation per model family:
...
Every boolean flag is presence-only and takes no value: `--enable-prefix-caching`
turns prefix caching on, and `--enable-prefix-caching true` is refused. A flag
is named for its non-default state, so a feature that is on by default is
turned off with `--no-<feature>` (`--no-ssm-tail-midchunk`,
`--no-high-speed-swap-graph`). A lever whose default is decided elsewhere — by
the compiled target, MODEL.toml or the environment — takes `auto`, `on` or
`off` instead: `--ssm-batched-recurrent`, `--content-loop-watchdog` and
`--tool-grammar`. In a recipe, a boolean key is written `true` or left out;
`key: false` restates the default and the recipe is refused.

## Environment levers

Some settings are `METRALE_*` environment variables rather than flags. Every
one the engine reads is declared once in `metrale_config::levers` with its
type, default, the flag that shadows it (if any) and the file that reads it;
`met dump-serve-options` prints the list under `levers`. `met` refuses to
start when the environment carries a `METRALE_*` name that is not declared: it
is a typo or a lever that no longer exists, and running as if it applied would
mislabel whatever was measured. A recipe's `env:` block may name only a
`runtime` lever.
...
| `--scheduler` | `fifo` | `fifo` or `slai` (SLO-aware) |
| `--tbt-deadline-ms` | `100` | SLAI decode deadline |
| `--auto-compact` | off | Active context compression threshold (e.g. 0.75 = 75% of max-seq-len) |

Agent workloads (Claude Code, OpenCode): always enable `--enable-prefix-caching` and `--scheduler slai`. The prefix cache dominates wall-clock for system prompts + tool schemas; SLAI keeps streaming smooth under concurrent load.
...
| `--swap-space-gb` | `3` | Disk-backed KV swap at `/tmp/metrale-swap/` |
...
|---|---|---|
| `GET /v1/models` | OpenAI | Returns one `ModelInfo` (Metrale Engine serves one model per process) |
...
- Token-bucket rate limiter per key (`crates/server/src/rate_limiter.rs`). Off by default; enable by setting `METRALE_RATE_LIMIT_RPM` (requests/min) and/or `METRALE_RATE_LIMIT_TPM` (tokens/min) > 0 (bursts via `METRALE_RATE_LIMIT_BURST_RPM` / `METRALE_RATE_LIMIT_BURST_TPM`, default = the cap). A MAX_KEYS DoS guard bounds the key table.
- Body-size limit env-configurable via `METRALE_MAX_BODY_BYTES` (default **32 MiB** — `main_modules/serve_router.rs`).
...
- **The dashboard.** `met serve` with no MODEL starts the listener and opens
...
For deployments where the served model is part of the contract, leave
`--auto-swap` off (the default): no client can then change what the endpoint
is running. There is no opposing `--no-auto-swap`; a boolean flag names only
its non-default state (see [Boolean flags](#boolean-flags)).
...
Prometheus-style metrics are exposed on `/metrics` (registered in `crates/server/src/metrics.rs`, plus the prefix-cache, token-entropy and kernel-lookup series that `api/misc_handlers.rs` appends). Among them:

- `metrale_requests_total`, `metrale_requests_active`
- `metrale_time_to_first_token_seconds{model}`
- `metrale_prompt_tokens_total`, `metrale_generation_tokens_total`, `metrale_decoded_tokens_total`
- `metrale_spec_decode_verify_total{k, outcome}`
- `metrale_tool_calls_total`
- `metrale_prefix_cache_hits_total`, `metrale_prefix_cache_misses_total`, `metrale_prefix_cache_hit_rate`

Structured logs go to stderr; they're the primary operational signal. Metrale Engine logs a brief line per completed request (model, prompt tokens, generated tokens, TTFT, elapsed, tools used). Per-token DECODE spam is deliberately not logged — it's useless.

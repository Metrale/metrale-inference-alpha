...
// Metrale Engine W8A16 tensor-core DECODE GEMM — 16-row M tile, FP8 E4M3 block-scaled.
//
...
// `METRALE_FFN_M16_TC` and OFF by default until an H100 receipt says it wins.
...
// 64 (`w8a16_gemm_m16_n64`, opt-in via `METRALE_FFN_M16_TC_NTILE=64`).
//
...
// and the A/B that would settle it is `METRALE_FFN_M16_TC_NTILE=64` against the
...
// 64 is the opt-in wide arm (`METRALE_FFN_M16_TC_NTILE=64`). Both must divide
...
///
/// Opt-in (`METRALE_FFN_M16_TC_NTILE=64`) and NOT the default: it exists to A/B

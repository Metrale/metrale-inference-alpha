...
use metrale_model_weights::kimi_k3_host::tp::{TpAxis, plan_tensor_bytes, tensor_plan};
use metrale_model_weights::kimi_k3_host::weights::required_names;
use metrale_model_weights::kimi_k3_host::weights_geometry::expected_replicated_shape;
    ...
    for name in required_names(&c) {
        ...
        for (name, shape, bytes) in tensors {
            ...
            if p.axis == TpAxis::Replicated {
                metrale_model_weights::kimi_k3_host::weights_geometry::validate_replicated_shape(
                    &name, &shape, &c,
                )?;
            }
            counted_tensors += 1;
            binding_extra += metrale_model_weights::kimi_k3_host::binding_memory::extra_gpu_bytes(
                &name,
                bytes == 4,
                p.local_bytes() / bytes,
            )?;

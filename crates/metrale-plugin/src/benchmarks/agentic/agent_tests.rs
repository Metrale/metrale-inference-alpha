...
fn tool_call_ids_are_positional_and_never_the_servers() {
    // Metrale Engine mints ids from a per-process counter, so the same turn of the same
...
fn the_system_prompt_is_the_harness_agent_prompt_plus_the_environment() {
    ...
    assert!(p.starts_with("You are a coding assistant running locally on Metrale Engine Spark."));

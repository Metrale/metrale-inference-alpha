...
# 2026-09-26: Single-request ISL/OSL sweep: TTFT, TPOT and decode tok/s per regime.
#
# Owner: bench.
# Invariants: none beyond the types.
"""
Metrale Engine ISL/OSL sweep at batch=1.
...
def send_request(url: str, model: str, prompt: str, max_tokens: int) -> dict:
    body = json.dumps({
        ...
        "temperature": 0.0,
        "stream": True,
        # 2026-09-26: Ask for usage and per-chunk token_ids, so the output-token
        # count comes from the server: one SSE chunk is not one token. A mismatch
        # between the two server counts is reported on stderr (see below).
    ...
    for raw_line in resp:
        ...
        # 2026-09-26: Timing and chunk_count follow non-empty content deltas only.
    ...
    # 2026-09-26: Output-token count, in order: usage.completion_tokens, then
    # the sum of token_ids, then the content-chunk count (approximate).
    ...
    # 2026-09-26: When both server counts are present and differ, warn on stderr;
    # ids_match_usage records the comparison either way.
    ids_match_usage = (
...
def main():
    parser = argparse.ArgumentParser(
        description="Metrale Engine ISL/OSL sweep (batch=1)")

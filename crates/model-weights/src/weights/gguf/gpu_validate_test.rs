...
//!   cargo test -p metrale-model-weights --features cuda -- --ignored gguf_gpu_validate
//!
...
#[test]
#[ignore = "requires a real CUDA GB10 device + compiled PTX (no METRALE_SKIP_BUILD)"]
fn gguf_gpu_validate_matches_cpu_oracle() {
    let gpu = MetraleCudaBackend::new(0, &metrale_kernels::ptx_modules())
        .expect("construct MetraleCudaBackend (needs a CUDA device + real PTX)");

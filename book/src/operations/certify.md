# Certification: `met bench certify`

A certification campaign is every required gate the current commit still
owes, run to completion, with records committed for each. `met bench
...
```
met bench certify                     # everything still open, on this box
met bench certify --dry-run           # the plan and the preflight, nothing run
met bench certify --gates bfcl-subset # one group (the shards it still owes)
met bench certify --shards 8          # cut each group's draw eight ways
met bench certify --pr 1027 --yes     # a real campaign, agentic gate confirmed
```

`bench` is the short spelling of `benchmark`; both work everywhere.

```
met bench certify --with-nodes 10.10.10.2,dgx3.local   # this box + two nodes, in parallel
met bench certify --with-nodes 10.10.10.2 --remote-only # from a laptop: nodes only
```

## `--with-nodes`

The same campaign, on several machines at once. Every address is asked
through `metralectl bench nodes` (`ip[:port]`, `[v6]:port`, `host.local`,
`dns.name`; port omitted → metralectl's peer port), and a node is **admitted**
...
`met benchmark run … --serve-reuse`: instead of loading the checkpoint in
...
(`<METRALE_HOME>/serve-lease.json` names the one it may). The campaign stops
...
server per unit; `met benchmark serve-release` stops a leased server by
...
`METRALECTL_CONFIG_DIR=/path met bench certify …`.
...
1. **Plan.** `gate::check_gates` at the anchor (HEAD) says which required gates
   ...
   `~/.metrale/runs` when there is one. The local order is the long shard sets
...
2. **Preflight.** Refuses to spend a GPU minute unless: HEAD is the anchor, no
   `PERF_PATHS` file is uncommitted, the signing identity in `METRALE_HOME` is
   committed in `.github/record-signers/`, `METRALE_HOME` is writable, no other
...
3. **Lock.** `.oracle_should_begin_cert` at the repo root (schema v1). A live lock (its driver
   ...
   heartbeat.
4. **Run.** Each unit is a child `met benchmark run <id> --pull-request-gate

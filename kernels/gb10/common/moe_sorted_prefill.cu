...
// 2026-09-25: NVFP4 MoE GEMVs over expert-sorted rows, routed experts only: gate+up, then
// SiLU(gate) * up and down. blockIdx.y is the sorted position, so consecutive blocks
// along y use the same expert's weights.
//
// Owner: gb10 kernels.
// Invariants:
// - gate_out, up_out and C are indexed by sorted position; gate+up reads input row
//   sorted_token_ids[y], and both kernels use expert sorted_expert_ids[y].
// - A null weight pointer writes zeros. K must be a multiple of 8, and at most 1024 in down.
// - Launch: grid (ceil(N / 8), total_expanded, 2 for gate+up or 1 for down), block 128.
...
// 2026-09-25: SiLU(gate) * up into s_act, then the down GEMV, in sorted order.

...
`METRALE_MTP_MAX_SEQS` is a GUARD, not a knob:
...
| what | where |
|---|---|
| scheduler gate | `crates/server/src/scheduler/mod.rs:551` (`active.len() <= mtp_max_seqs()`) |
| cap constant | `scheduler/mod.rs:189-197` (`mtp_max_seqs`, now defaults 1) |
| spec step driver | `crates/server/src/scheduler/spec_step.rs:94, 266` (`decode_verify`, `decode_verify_graphed`) |
| verify dispatch (per-K) | `crates/model-engine/src/model/trait_impl/verify_a.rs:31`, `verify_b.rs:39`, `verify_c.rs:47`, `verify_c2.rs:36` (K=4), `verify_d.rs:40` (K=gamma) |
| **the missing piece** | a BATCHED `decode_verify` taking `n` sequences x (K+1) rows. The model trait has only single-sequence forms. |
| MTP single-seq assumptions | `crates/model-layers/src/mtp_carry.rs:37`; one MTP slot at `model/types.rs:187` |
| prefill-continuation gate | `scheduler/phase_continue_prefills.rs:101` (also `active.len() == 1`) |
| multi-seq decode (the path to generalize) | `crates/model-engine/src/model/trait_impl/decode_a2.rs` — already handles n sequences x 1 token with `padded_n` rows, `meta.positions`, `meta.slot` |
...
does not merely close the gap — it is how Metrale passes it.
...
★ Check `METRALE_GDN_FUSED_CONV17` (`:92`) — the fused path is gated by it; do not assume it is on.
...
`METRALE_MTP_MAX_SEQS=4` at C=4 must go from 25.8 (today's serialized number) to >48.5 (today's
...
1. **Batched cross-sequence PROPOSE** (the measured gap: 12 per-seq drafter forwards x ~5 ms =
   ...
   Kill switch `METRALE_NO_MTP_BATCH_PROPOSE`. Measured alone: 46.3-46.8 vs 42.9-43.6 control.
2. **out_proj M>8 dispatch fix** (m_dispatch class): the R=16 verify rows fell into the
   ...
   `METRALE_NO_VERIFY_OUTPROJ_TGEMM`.
...
1. **Slot-vector-keyed CUDA graphs for the batched K=4 verify** (`verify_e.rs` +
   ...
   `METRALE_NO_MTP_VERIFY_GRAPHS` (PRESENCE). METRALE_K4_DIAG still forces eager.
2. **Cross-sequence batched GDN conv+WY** (`trait_decode_batched_conv_gdn_multi.rs`):
   ...
   `METRALE_NO_VERIFY_GDN_BATCH` (PRESENCE). Contract delta: the batched conv
...
Validation still owed (validator): serve + `METRALE_MTP_MAX_SEQS` sweep vs the

...
//   - C[M, N] BF16
//
// FP8-input MMA PTX; Hopper machine lowering is compiler-dependent.
...
//
// Hopper short shared-expert tile: 16 x 32 x 32.
...
extern "C" __global__ void shared_fp8_m16n32(
    ...
    // Each 32-column CTA remains inside one 128-column weight scale block.
    const unsigned int n_block_lo = cta_n / K_BLOCK;
    // All 32 output columns share n_block_lo.
    ...
    #define FFB_LOADS(buf, kb) do { \
        ...
        { \
            for (unsigned int my_n = threadIdx.x; my_n < N_TILE_LG; my_n += blockDim.x) { \
    ...
    // Each warp spans 32 N columns within ONE 128-column scale block

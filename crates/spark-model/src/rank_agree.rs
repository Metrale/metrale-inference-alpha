...
//!
//! 🔴 Some tuning levers are not perf knobs. `METRALE_GLM_PREFILL_ROWS` is read independently on
...
//! That is the harness being careful, not the engine being safe; `METRALE_EP_PROTOCOL` carried the
...
//! `METRALE_GLM_MOE_ROW_BATCH_MAX` is included anyway because a rank skew there is still a

...
#
# Runs all GB10 models with metrale-gb10:latest, writes BENCHMARK_RESULTS.md
#
# Usage:
...
run_ep2_model() {
  ...
  if wait_server "$url" "$label"; then
    ...
    local docker_log
    docker_log=$(sudo docker logs metrale-ep0 2>&1 | tail -20)
    coherence_out="SKIPPED — server did not start"
    bench_out="SKIPPED — server did not start\n\nDocker log tail:\n${docker_log}"
  fi
...
cat > "$OUTPUT" << HEADER
# Metrale Engine GB10 — Full Model Benchmark Results

**Date:** $(date '+%Y-%m-%d')
**Hardware:** 2× NVIDIA GB10 Grace Blackwell (119.7 GB GPU memory each)
**Image:** \`metrale-gb10:latest\` (METRALE_TARGET_MODEL=*)
...
# ── 6. Qwen3.5-122B EP=2 ─────────────────────────────────────────────────────
# metrale-122b:latest (16:52, head) pushed to worker; use it on both nodes.

...
//! 2026-09-25: Oracle for `moe_topk_softmax`: BF16 `gate_logits[num_experts]` to the top_k
//! indices (descending, lower index on ties) and their softmax-over-all weights, renormalised
//! over the top k when `normalize = 1`. Grid (1, 1, 1), block 256. The CPU reference uses
//! `f32::exp`.
//!
//! Owner: model-arch examples (MoE kernels).
//! Invariants:
//! - The run exits 1 unless the indices match exactly and every weight is within 0.01.
...
fn main() -> Result<()> {
    ...
    let mut r = Rng(seed);
    // 2026-09-25: Logits: uniform draws in [-7, -1.5), rounded to BF16.

...
fn fixture(dir: &Path, bad_scale: bool) -> Vec<u8> {
    use crate::kimi_k3_host::MixerKind;
    use crate::kimi_k3_host::tp::{TpAxis, tensor_plan};
    let c = config(0, 2);
    let packed: Vec<u8> = (0..64 * 32).map(|i| (i % 251) as u8).collect();
    let mut owned: Vec<(String, Dtype, Vec<usize>, Vec<u8>)> = Vec::new();
    for name in crate::kimi_k3_host::weights::required_names(&c) {
        if name.contains(".block_sparse_moe.experts.") {
            ...
            let shape = if axis == TpAxis::Replicated {
                crate::kimi_k3_host::weights_geometry::expected_replicated_shape(&name, &c).unwrap()
            } else {
                vec![n, k]
            };

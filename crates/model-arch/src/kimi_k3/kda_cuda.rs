...
//! 2026-09-25: Host launch of the Kimi K3 KDA decode kernels (PTX module [`MODULE`]) for one token.
//!
//! Two launches per token: [`CONV_ENTRY`] updates the short-conv state and
//! applies SiLU, then [`RECURRENT_ENTRY`] L2-normalises q and k and runs the
//! gated delta-rule step. The CPU reference is
//! [`kda_decode_token`](metrale_model_weights::kimi_k3_host::kda_decode_token).
//! Decode uses [`launch_k3_kda_decode_token_on_device`] unless `K3_CUDA_KDA=0`.
//!
...
//! - [`KdaDeviceState::alloc_and_upload`], when it fails, calls `free` on what
//!   it allocated, ignoring a failed `free`.

use anyhow::{Context, Result, bail, ensure};
use metrale_gpu_runtime::gpu::{DevicePtr, GpuBackend, KernelHandle};
use metrale_gpu_runtime::kernel_args::{KernelLaunch, div_ceil};
use metrale_model_weights::kimi_k3_host::{KDA_L2_EPS, KdaConfig, KdaState};

/// 2026-09-25: Built from `kernels/gb10/kimi-k3/bf16/kda_decode.cu`.
...
/// 2026-09-25: One sequence's KDA conv and recurrent state on the device.
/// [`launch_k3_kda_decode_token_on_device`] updates it in place and copies
/// only the output back; `download` copies it to a host [`KdaState`].
///
/// [`launch_k3_kda_decode_token`] instead uploads and downloads the state
/// every token; at [`KdaConfig::production`] the recurrent state alone is
/// 96 x 128 x 128 FP32, 6.3 MB per layer.
...
/// 2026-09-25: One decode token against a host [`KdaState`]: uploads the
/// state, runs both kernels, and downloads the updated state.
...
/// 2026-09-25: One decode token against a [`KdaDeviceState`]; only the output
...
mod tests {
    ...
    use metrale_model_weights::kimi_k3_host::kda_decode_token;

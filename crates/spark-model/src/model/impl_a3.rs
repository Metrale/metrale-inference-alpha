...
/// `METRALE_NO_LMHEAD_BATCHED_WIDE` restores the M64-tile `w4a16_gemm` fallback.
/// Presence, not value — `=0` is NOT "off" (see `metrale_env_presence_check_trap`).
...
impl TransformerModel {
    ...
    fn lm_head_batched_wide(
        ...
        if let Some((ref nvfp4_t, ldb)) = self.lm_head_nvfp4_t {
            // LOSSLESS path (METRALE_LMHEAD_LOSSLESS): BF16 MMA, no activation
        ...
        // No twin (METRALE_NO_LMHEAD_TGEMM): the M<=16 weight-streaming GEMV still
    ...
    /// Kill switch: `METRALE_NO_LMHEAD_VOCAB_TP=1`. Only reachable from the plain BF16 dense

...
# integration. Spins up two met servers (with HSS / without HSS), sends
# the same long-context prompt to both, and compares output.
#
# Run after stopping any existing met server holding the GPU. Usage:
#
#     bash scripts/verify-high-speed-swap.sh \
#         /path/to/qwen3-bf16-model \
#         /mnt/nvme0/metrale-hsw-test
#
...
PROMPT_FILE=${PROMPT_FILE:-/tmp/metrale-hsw-prompt.txt}
...
HSW_LOG=/tmp/metrale-hsw-with.log
"$MET_BIN" serve "$MODEL" \
    --port "$PORT_HSW" \
    --max-seq-len 32768 \
    --high-speed-swap \
...
REF_LOG=/tmp/metrale-hsw-without.log
"$MET_BIN" serve "$MODEL" \
    --port "$PORT_REF" \
    --max-seq-len 32768 \
    > "$REF_LOG" 2>&1 &
...
if [[ "$RESPONSE_HSW" == "$RESPONSE_REF" ]]; then
  ...
  echo "      for every attention layer in the met log), or"

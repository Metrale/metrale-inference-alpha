...
jobs: { j: { runs-on: metrale-cmd, steps: [{ uses: actions/checkout@v4, with: { ref: main } }] } }
Y
want_rc 1 "control: pull_request trigger on the command runner" \
  sh -c "cd '$TMP/wf' && python3 assert-cmd-runner-safe.py"
cat > "$TMP/wf/.github/workflows/a.yml" <<'Y'
on: { pull_request_target: { types: [opened] } }
jobs: { j: { runs-on: [self-hosted, metrale-cmd], steps: [{ uses: actions/checkout@v4, with: { ref: "${{ github.event.pull_request.head.sha }}" } }] } }
Y
want_rc 1 "control: checks out the PR head on the command runner" \
  sh -c "cd '$TMP/wf' && python3 assert-cmd-runner-safe.py"
cat > "$TMP/wf/.github/workflows/a.yml" <<'Y'
on: { issue_comment: { types: [created] } }
jobs: { j: { runs-on: metrale-cmd, steps: [{ uses: actions/checkout@v4 }] } }
Y
...
cat > "$TMP/wf/.github/workflows/a.yml" <<'Y'
on: { pull_request: { types: [opened] } }
jobs: { j: { runs-on: metrale-pr-cheap, steps: [{ uses: actions/checkout@v4 }] } }
Y
...
jobs:
  j:
    runs-on: "${{ github.event.pull_request.head.repo.full_name == github.repository && 'metrale-pr-cheap' || 'ubuntu-latest' }}"
...
jobs:
  j:
    runs-on: "${{ vars.USE_METRALE_UBUNTU_RUNNERS == '1' && (github.event_name != 'pull_request' || github.event.pull_request.head.repo.full_name == github.repository) && (vars.PR_CHEAP_RUNNER || 'ubuntu-latest') || 'ubuntu-latest' }}"
    steps: [{ uses: actions/checkout@v4 }]
Y
...
if [ -s "$TMP/oc.sh" ]; then
  ...
  # tests with red/green controls under `cargo test -p metrale-bench agreement`,
...
STUB
  ...
  certed()  { grep -qE 'POST.*issues/1/comments.*metrale-certificate' "$TMP/bcalls"; }
...
PY
  ...
  # `metrale-certification-state` in the file, so this check stayed green while
  # the lookup's `contains("<!-- metrale-certification-state:")` matched nothing,
...
m = re.search(r'contains\("(<!-- metrale-certification-state[^"]*)"\)', t)
...
STUB
  ...
  ( PATH="$TMP/bin:$PATH" BCALLS="$TMP/bcalls" REPO=o/r PR=1 DEFAULT_BRANCH=main \
    STATE=pr-certification-merged HEADLINE=h COMMENT_ID= HEAD_SHA=abc1234567 \
    bash "$TMP/bot.sh" >/dev/null 2>&1 )
  if grep -qE 'POST.*issues/1/comments.*metrale-certificate' "$TMP/bcalls"; then
...
# workflow-level METRALE_SKIP_BUILD=1 (there so the ubuntu jobs type-check
# without nvcc). metrale-kernels' build.rs honours it first and emits a stub
...
rc_metal_env='
steps = [s for s in d["jobs"]["test-macos-metal"]["steps"] if s.get("name","").startswith("cargo test -p metrale-gpu-runtime")]
...
sg_sabotage ci.yml "$rc_metal_env"'env["METRALE_SKIP_BUILD"] = "1"'
...
# workflow-level METRALE_SKIP_BUILD=1 reach the job again. A guard that only
# looked at the step's own env would pass here.
sg_sabotage ci.yml "$rc_metal_env"'env.pop("METRALE_SKIP_BUILD")'
want_rc_msg 1 "is about the stub, not the kernels" \
  "control: dropping the override so the workflow-level stub env is inherited is caught" \
  python3 "$TMP/sg/scripts/assert-gates-are-wired.py"

# Without METRALE_TARGET_HW, build.rs takes its macOS auto-skip and embeds
# nothing even with METRALE_SKIP_BUILD=0 -- the same empty set by another route.
sg_sabotage ci.yml "$rc_metal_env"'env.pop("METRALE_TARGET_HW")'
want_rc_msg 1 "build.rs takes the macOS auto-skip" \
  "control: dropping METRALE_TARGET_HW from the metal suite is caught" \
  python3 "$TMP/sg/scripts/assert-gates-are-wired.py"
...
mkdir -p "$TMP/bo/moved/crates/server/src/recipe"
...
mkdir -p "$TMP/bo/dirty/crates/server/src/tui" "$TMP/bo/dirty/crates/server/src/recipe"
printf 'fn f() { handle.block_on(fut); }\n' > "$TMP/bo/dirty/crates/server/src/tui/draw.rs"
...
# `CLI booleans are presence flags`: scans only the files that declare clap
# arguments, so the two ways it could go green unenforced are finding no such
# files and finding them but missing the form. The step must also still be
# wired into the cheap-checks collector.
python3 - <<'PY'
import pathlib
ci = pathlib.Path(".github/workflows/ci.yml").read_text()
assert "bash .github/scripts/check-cli-booleans.sh" in ci, \
    "ci.yml no longer runs check-cli-booleans.sh"
PY
mkdir -p "$TMP/cb"
cp .github/scripts/check-cli-booleans.sh "$TMP/cb/step.sh"
want_rc 0 "the CLI-boolean gate passes on the real tree" bash "$TMP/cb/step.sh"
mkdir -p "$TMP/cb/empty/crates/server/src/cli"
touch "$TMP/cb/empty/crates/server/src/cli.rs"
want_rc_msg 1 "this check cannot vouch for a surface it did not find" \
  "control: the CLI-boolean gate refuses a tree with no clap declarations" \
  env -C "$TMP/cb/empty" bash "$TMP/cb/step.sh"
for form in 'pub x: Option<bool>,' '#[arg(long, default_missing_value = "true")]'; do
  rm -rf "$TMP/cb/dirty"; mkdir -p "$TMP/cb/dirty/crates/server/src"
  cp -r crates/server/src/cli crates/server/src/cli.rs "$TMP/cb/dirty/crates/server/src/"
  printf '#[derive(clap::Args)]\nstruct S {\n    #[arg(long)]\n    %s\n}\n' "$form" \
    > "$TMP/cb/dirty/crates/server/src/cli/zz_injected.rs"
  want_rc_msg 1 "A command-line boolean takes no value" \
    "control: the CLI-boolean gate catches \`$form\`" \
    env -C "$TMP/cb/dirty" bash "$TMP/cb/step.sh"
done
...
# to the common file it overrides is a dead override — DECLARED in [shadow],
# because an undeclared one is refused by the resolver first (RULER), which
# would make this control pass on the wrong error. Every tree in HW_TREES
# must exist for the same reason.
L="$TMP/ks/live/kernels"
mkdir -p "$L/gb10/common" "$L/gb10/m1/q" \
         "$L/hopper" "$L/b200" "$L/b300" "$L/metal" "$L/strix" "$L/strix-hip"
printf '[hardware]\nname = "gb10"\nvendor = "nvidia"\n' > "$L/gb10/HARDWARE.toml"
printf '[model]\nname = "m1"\n' > "$L/gb10/m1/MODEL.toml"
printf '[shadow]\nk = "control"\n' > "$L/gb10/m1/q/KERNEL.toml"
printf '__global__ void k() {}\n' > "$L/gb10/common/k.cu"
cp "$L/gb10/common/k.cu" "$L/gb10/m1/q/k.cu"
want_rc_msg 1 "RULE1" "control: the kernel-shadow check still catches a dead override" \
  python3 scripts/check_kernel_shadows.py "$L"
# ...and a symlink, the sharing mechanism this tree retired.
rm "$L/gb10/m1/q/k.cu" "$L/gb10/m1/q/KERNEL.toml"
ln -s ../../common/k.cu "$L/gb10/m1/q/k.cu"
want_rc_msg 1 "RULE0" "control: the kernel-shadow check refuses a symlink" \
  python3 scripts/check_kernel_shadows.py "$L"
...
tt_tree() {  # tt_tree <clean|dirty|renamed>
  rm -rf "$TMP/tt/w"; mkdir -p "$TMP/tt/w/crates/server/src/tui"
  case "$1" in
    renamed) mkdir -p "$TMP/tt/w/crates/old-server/src/recipes" ;;
    *)       mkdir -p "$TMP/tt/w/crates/server/src/recipe" ;;
  esac
  printf 'fn tick() { let _ = rx.try_recv(); }\n' > "$TMP/tt/w/crates/server/src/tui/chat.rs"
  [ "$1" = dirty ] && printf 'fn tick() { rt.block_on(f); }\n' > "$TMP/tt/w/crates/server/src/tui/bad.rs"
  return 0
}
...
if hits=$(grep -rnE '\.(block_on|block_in_place)\(' \
            ...
            crates/server/src/recipe/ 2>/dev/null); then
...
case "\$url" in
  ...
  */install.ps1)  printf '# metrale installer\n' ;;
  *)              printf '<title>Metrale Engine, pure Rust inference</title>' ;;
esac
exit 0
STUB
...
dl_tree; printf '[a](/api/metrale_model_layers/)\n[b](/api/)\n' > "$TMP/dl/book/src/more.md"
...
# and mentioned nowhere in the docs. /expedite skips certification outright.
...
mkdir -p "$TMP/cd/book/src/project"
printf 'we document /help and /stamp and /seal here.\n' > "$TMP/cd/book/src/project/landing.md"
want_rc 0 "control: a landing page covering every verb passes" \
  python3 "$TMP/cd/.github/scripts/assert-commands-documented.py"

cd_case '/help|/stamp|/seal|/expedite'
want_rc_msg 1 "accepts \`/expedite\`" "control: a command missing from the landing page is caught" \
  python3 "$TMP/cd/.github/scripts/assert-commands-documented.py"
...
# fails.
QB_COV=crates/bench/src/gate/coverage.rs
...
echo "== comment-ttl configuration =="
# 2026-09-25: Each refusal is pinned by its message: all of them exit 1.
CT=.github/scripts/comment-ttl-config.py
want_rc 0 "the comment-ttl configuration as it stands is consistent" python3 "$CT" check
mkdir -p "$TMP/ct"
{ cat .comment-ttl.toml; echo 'fail_on_flag = false'; } > "$TMP/ct/overridden.toml"
want_rc_msg 1 "which the Action always overrides" \
  "control: a key the Action overrides is refused in .comment-ttl.toml" \
  python3 "$CT" check --config "$TMP/ct/overridden.toml"
grep -v 'q4k_vendor' .comment-ttl.toml > "$TMP/ct/leaky.toml"
want_rc_msg 1 "e.g. kernels/gb10/qwen3.6-27b/nvfp4/q4k_vendor/" \
  "control: a third-party path the licence config ignores must be exempt" \
  python3 "$CT" check --config "$TMP/ct/leaky.toml"
printf 'crates/no-such-directory/\n' > "$TMP/ct/missing.txt"
want_rc_msg 1 "holds no tracked file" "control: an enforced directory that does not exist is refused" \
  python3 "$CT" check --enforced "$TMP/ct/missing.txt"
printf 'vendor/\n' > "$TMP/ct/exempt.txt"
want_rc_msg 1 "lies under an exempt path" "control: an enforced directory under an exempt path is refused" \
  python3 "$CT" check --enforced "$TMP/ct/exempt.txt"
printf 'kernels/gb10/qwen3.6-27b/\n' > "$TMP/ct/dots.txt"
want_out '^(kernels/gb10/qwen3\.6-27b/)' "the enforced scan_dirs regex escapes a dot in a directory name" \
  python3 "$CT" enforced-regex --enforced "$TMP/ct/dots.txt"

echo "== nothing glued to the SPDX line =="
# 2026-09-26: A throwaway repository with this .comment-ttl.toml. The separated shapes
# pass, metadata lines under the SPDX line included, and so does an exempt file with
# glued text; each glued comment shape is then refused by its path and line, and code
# directly under the SPDX line passes.
SP=.github/scripts/assert-spdx-separated.py
SPR="$TMP/spdx"
git init -q "$SPR"
cp .comment-ttl.toml "$SPR/"
mkdir -p "$SPR/vendor"
printf '// SPDX-License-Identifier: MIT\n\n//! 2026-09-26: text\n' > "$SPR/sep.rs"
printf '// SPDX-License-Identifier: MIT\n// forked-from: a.cu\n\n// 2026-09-26: text\n' > "$SPR/fork.cu"
printf '// SPDX-License-Identifier: MIT\n// provenance-id: 00\n\n//! 2026-09-26: text\n' > "$SPR/prov.rs"
printf '# SPDX-License-Identifier: MIT\n# SPDX-FileCopyrightText: A\n\nimport os\n' > "$SPR/copy.py"
printf '// SPDX-License-Identifier: MIT' > "$SPR/one.rs"
printf '// SPDX-License-Identifier: MIT\n//\n// text\n' > "$SPR/vendor/glued.rs"
git -C "$SPR" add -A
want_rc_msg 0 "ok:" "blank line after SPDX and its metadata passes, and an exempt path is skipped" \
  python3 "$SP" --root "$SPR"
sed 's|\^vendor/|^not-vendor/|' "$SPR/.comment-ttl.toml" > "$TMP/spdx-noexempt.toml"
cp "$SPR/.comment-ttl.toml" "$TMP/spdx-kept.toml"
cp "$TMP/spdx-noexempt.toml" "$SPR/.comment-ttl.toml"
want_rc_msg 1 "vendor/glued.rs:2" "control: the exempt path comes from .comment-ttl.toml" \
  python3 "$SP" --root "$SPR"
cp "$TMP/spdx-kept.toml" "$SPR/.comment-ttl.toml"
spdx_refused() {  # 2026-09-26: <file> <content> <expected path:line> <label>
  printf '%b' "$2" > "$SPR/$1"
  git -C "$SPR" add "$1"
  want_rc_msg 1 "$3" "$4" python3 "$SP" --root "$SPR"
  git -C "$SPR" rm -q --cached "$1"
}
spdx_refused glued.rs '// SPDX-License-Identifier: MIT\n//\n// text\n' "glued.rs:2" \
  "control: a separator and text glued to the SPDX line are refused"
spdx_refused fork2.cu '// SPDX-License-Identifier: MIT\n// forked-from: a.cu\n// text\n' "fork2.cu:3" \
  "control: text directly under a forked-from line is refused"
spdx_refused prov2.rs '// SPDX-License-Identifier: MIT\n// provenance-id: 00\n//! text\n' "prov2.rs:3" \
  "control: a module doc directly under a provenance-id line is refused"
spdx_refused glued.py '# SPDX-License-Identifier: MIT\n# text\n' "glued.py:2" \
  "control: a # comment glued to the SPDX line is refused"
printf '// SPDX-License-Identifier: MIT\n#![allow(dead_code)]\n' > "$SPR/code.rs"
printf '# SPDX-License-Identifier: MIT\n[package]\n' > "$SPR/Cargo.toml"
git -C "$SPR" add code.rs Cargo.toml
want_rc_msg 0 "ok:" "code directly under the SPDX line is not glued comment text" \
  python3 "$SP" --root "$SPR"

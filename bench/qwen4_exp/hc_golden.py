# 2026-09-26: Runs the reference Qwen4ExpTextGatedResidual on checkpoint weights for
# three hyper-connection sites and writes the inputs and outputs to an npz, and with
# --bin-dir raw .bin files plus meta.json for hyper_connection_lowrank_tests.rs.
#
...
# 2026-09-26: Layer 0's attn and mlp sites and the model-level mixer. The layer sites
# have block_inject_weight, so they run with use_combine=True and also return
# injection_weights.
SITES = {
    'attn': 'model.language_model.layers.0.attn_hyper_connection',
    'mlp': 'model.language_model.layers.0.mlp_hyper_connection',
    # 2026-09-26: The model-level mixer has no block_inject_weight, so it returns
    # mixed_input only. The cached checkpoint has no top-level `norm.weight`; this
    # mixer's hc_norm is the final norm.
...
def write_bins(out_dir: str, dump: dict, weights: dict) -> None:
    ...
    put('post_expected', dump['post_expected'].astype(np.float32).tobytes(),
        'f32')
    # 2026-09-26: BF16 for the parameters hyper_connection.cu declares __nv_bfloat16.
...
def main() -> int:
    ...
    ap.add_argument('--bin-dir', default=os.environ.get('METRALE_HC_TEST_DATA'),
    ...
    # 2026-09-26: A seeded highway with a different scale per stream. With equal scales
    # a single global RMS would equal the grouped one and hide a wrong norm.
    ...
    dump: dict[str, np.ndarray] = {
        ...
        # 2026-09-26: hyper_connection_lowrank_tests.rs refuses a fixture with any other value.
        'norm_convention': np.bytes_(b'normed * (1.0 + weight)'),
    }
    # 2026-09-26: The weights go to a separate *_weights.npz, which .gitignore excludes;
    # they are copied from the checkpoint and can be regenerated.
    ...
    for site, prefix in SITES.items():
        ...
        # 2026-09-26: Recompute the grouped norm by hand, print its difference from the
        # module, and print what a global RMS and a dropped +1 offset would give.
        if site == 'attn':
            x = hyper[0].float()
    ...
    # 2026-09-26: hc_post expectation:
    #   out[t, s*H + d] = residual[t, s*H + d] + block_out[t, d] * inj[t, s]
    # hc_post reads block_out as BF16, so it is rounded to BF16 before the expectation
    # is computed.

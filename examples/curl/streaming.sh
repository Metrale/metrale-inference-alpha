#!/usr/bin/env bash
# Stream a chat completion from Metrale Engine via SSE.
# Metrale Engine exposes the OpenAI-compatible /v1/chat/completions endpoint,
# so any OpenAI streaming client works.
#
# Usage:
#   bash examples/curl/streaming.sh
#   METRALE_URL=http://192.168.1.10:8888 METRALE_MODEL=Sehyo/Qwen3.5-35B-A3B-NVFP4 \
#     bash examples/curl/streaming.sh

set -euo pipefail

METRALE_URL="${METRALE_URL:-http://localhost:8888}"
METRALE_MODEL="${METRALE_MODEL:-Sehyo/Qwen3.5-35B-A3B-NVFP4}"
PROMPT="${1:-Write a haiku about the ocean.}"

echo "Streaming from $METRALE_URL with $METRALE_MODEL ..."
echo "Prompt: $PROMPT"
echo
...
curl -sN -X POST "$METRALE_URL/v1/chat/completions" \

...
Branch `perf/decode-fold-2026-07-24` == `feat/tree-spec-decode` == **`1dcf2755`** on `metrale`
...
| 4 | **`wy5`–`wy8` batched GDN verify** | `kernels/gb10/common/gated_delta_rule_wyn.cu` (K-templated) + dispatch | Replaces the serial per-token GDN fallback at K=5..8. Kill-switch `METRALE_GDN_WYN=0`. |
| 5 | shadow top-k instrumentation | `mtp_head/forward.rs`, `verify_k3/k4_step.rs`, `scripts/` | `METRALE_MTP_SHADOW_TOPK=k`, observational only (default off). Produced the 19k-sample acceptance measurement that drove the design. |
...
```bash
# 1. code
git fetch metrale && git checkout 1dcf2755        # perf/decode-fold-2026-07-24

# 2. build (the METRALE_TARGET_MODEL is load-bearing — without it you get a wrong binary)
PATH=/usr/local/cuda/bin:$PATH METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b \
...
sudo docker run -d --name metrale-golden-e2e --network host --gpus all --ipc=host \
  ...
  metrale-gb10:followups serve centml/Qwen3.6-27B-NVFP4-W4A4-mlpinf \
...
#   METRALE_BIN=$PWD/target/release/met HARNESS_DIR=/workspace/endpoints-fresh \

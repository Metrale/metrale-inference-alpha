...
//! Serving recipes from `Metrale/metrale-recipes`.
//!
//! A recipe is a validated `met serve` configuration for one checkpoint: the
...
pub struct Recipe {
    ...
    pub version: String,
    pub model: String,
    /// `metrale` or `vllm`. Absent on v1 recipes, which predate the key.
    ...
    /// The `env:` block verbatim: the `METRALE_*` serve levers this recipe is
    /// measured under. The same key metrale-recipes' launcher exports into the
...
impl Recipe {
    ...
    /// Whether this recipe drives Metrale Engine. A `vllm` recipe is listed but cannot
    /// be launched from here.
    pub fn is_metrale(&self) -> bool {
        self.runtime.as_deref() == Some("metrale")
    }

    /// The full `met serve` argv, with `overrides` replacing recipe values.
    ///

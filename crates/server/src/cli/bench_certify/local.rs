...
//! 2026-09-26: The local driver: the campaign one unit at a time on this box.
//!
//! Owner: server CLI (`met benchmark certify`).
//! With a guard ref, the drift guard runs before every unit and on a
//! `GUARD_EVERY` timer beside it.
...
/// 2026-09-26: The serial loop: one unit at a time on this box, until
/// `Campaign::next_to_start` has none or a guard stop breaks the loop.
...
pub(super) fn drive_local(
    ...
    while let Some(i) = campaign.next_to_start() {
        ...
        // 2026-09-26: Guard before every start.
        let mut guard_rc = 0;
        ...
        // 2026-09-26: Guard on a timer while the unit runs: a `Stop` judgement stores
        // its result and sets the cancel flag, and the result is read back below
        // so the abort says why.

...
// ── scheduler performance-control provenance (metrale#812) ───────────────────
//
// The defect these pin: `--pull-request-gate` starts the server inside this
// process, so the scheduler reads METRALE_PREFILL_CODISPATCH* from the inherited
...
/// `metrale-plugin` does not depend on `spark-server`. This is the test that
...
fn perf_env_defaults_match_the_scheduler() {
    ...
    assert!(
        resolution("METRALE_PREFILL_CODISPATCH_WINDOW_MS").contains("unwrap_or(100)"),
    ...
    assert!(
        resolution("METRALE_PREFILL_CODISPATCH_SETTLE_MS").contains("unwrap_or(10)"),

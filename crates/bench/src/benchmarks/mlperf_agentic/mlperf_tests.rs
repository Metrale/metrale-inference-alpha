...
//! 2026-09-26: Tests for the MLPerf agentic driver: provisioning, the request
//! body, the parameters, aggregation, the verdict and the gate record.
//!
...
fn handle(root: &str) -> (PluginHandle, std::path::PathBuf) {
    ...
    std::mem::forget(rx);
    let dir = std::env::temp_dir().join(format!("metrale-mlperf-{root}-{}", std::process::id()));
...
/// 2026-09-26: Provisioning without the dataset fails, and the error names
/// the missing file, the upstream status and the no-proxy rule.
#[test]
...
fn a_present_dataset_is_hashed_and_a_wrong_pin_refuses_to_run() {
    ...
    // 2026-09-26: The right pin passes (case-insensitively), the wrong one
    // refuses.
...
/// 2026-09-26: The request carries the fixed sampling set plus the seed and
/// nothing else, pinned by exact key set.
#[test]
fn the_request_body_is_the_official_immutable_set_plus_seed() {
    ...
    // 2026-09-26: No tools: no tools key, not tools: null.
...
/// 2026-09-26: No sampling parameter is exposed, checked against the full
/// parameter list, so any added parameter fails this test.
#[test]
...
fn aggregation_follows_the_upstream_denominator_semantics() {
    ...
    // 2026-09-26: Two scored coding turns (one a missing-output 0), one scored
    // workflow turn, one excluded turn.
    let turns = vec![
    ...
    // 2026-09-26: OSL is over turns that produced output (3 of 4).
    assert!((s.osl_per_turn_mean - (200.0 / 3.0)).abs() < 1e-9);

    // 2026-09-26: Nothing scorable gives None, which `next` turns into a
    // refusal, not a 0.0 score.
...
/// 2026-09-26: The verdict is info, even for perfect scores.
#[test]
...
/// 2026-09-26: The fingerprint on the terminal frame reaches the gate record,
/// and a record without the field still parses.
#[test]
fn the_dataset_fingerprint_survives_into_a_gate_record() {
    ...
    // 2026-09-26: And it round-trips the record file format.
    ...
    // 2026-09-26: Records without the field still parse.
...
/// 2026-09-26: `write_responses` uses a fixed filename, which is safe only
/// while one leg writes into this benchmark's artifact directory. If this
/// benchmark became a group, its legs would overwrite each other's
/// `responses.jsonl` while the runs still passed; this test fails first.
#[test]

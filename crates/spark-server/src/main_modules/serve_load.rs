...
//!
//! * **Plain-log byte-identity.** `met serve <M> --no-tui` output is a grep
...
impl Carried {
    ...
    ///
    /// # Errors
    /// When any of the `METRALE_RATE_LIMIT_*`, `METRALE_STORE_*` or
    /// `METRALE_CONVERSATION_*` variables holds a value that does not parse.
...
pub(crate) fn load_model(
    ...
    // CLI `--lm-head-dtype` override (replaces METRALE_LMHEAD_BF16). Validate eagerly (PCND).
    ...
    // QUANT. With METRALE_TARGET_QUANT=* the build emits one bundle per
    ...
    // Kbenkhaled/Qwen3.5-27B-NVFP4) carry a `vision_config`, but their Metrale Engine
    ...
    //
    // Two module names because Metrale Engine has two towers: the Qwen3-VL-shaped
    ...
    // decode_logits_seq.rs / compile_tools.rs; `lean` was a Metrale-only
    ...
    // (set METRALE_EP_PROTOCOL=v2) so the worker routes commands by slot_idx
    ...
    // construction and accept craters. METRALE_DFLASH_MASKED_VERIFY=1 routes
    ...
    for (name, peer_id, dir) in &args.lora_stageable {
        ...
        let peft = metrale_core::config::parse_peft_adapter_config(&raw)
    ...
    for (name, spec) in &args.lora_stageable_disk {
        ...
        let peft = metrale_core::config::parse_peft_adapter_config(&raw).with_context(|| {
            format!(
    ...
    // (decode runs eager). METRALE_LORA_ROTATE=1 arms it; a peer being set also

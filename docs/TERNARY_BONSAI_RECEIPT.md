...
| Component | Value |
|---|---|
| Container | `metrale-inference-gb10:b12x-ready` (cargo/rustc, CUDA 13.2, `libcute_dsl_runtime.so`, `CUTE_DSL_ARCH=sm_121a`, FlashInfer at `/opt/flashinfer`) |
...
| GDN-FI AOT lib | `3rdparty_patches/gdn_aot/libmetralegdn.so` (prebuilt; syms `metrale_gdn_load`, `metrale_gdn_prefill_packed_managed`; transpose handoff fixed) |
...
```
METRALE_TARGET_MODEL='*' cargo build -p spark-server --release --bin met \
...
| `METRALE_GDN_FLASHINFER` | `1` | GDN-FI **prefill** (48 GDN layers); prefill-only, decode stays FLA (transpose handoff fixed → coherent) | **RECOMMENDED** (container-only) |
| `METRALE_GDN_LIB` | `.../gdn_aot/libmetralegdn.so` | dlopen path for the GDN-FI AOT lib | with GDN-FI |
...
docker run --rm --gpus all --network host \
  ...
  -e METRALE_GDN_LIB=/work/3rdparty_patches/gdn_aot/libmetralegdn.so \
  -e METRALE_GGUF_NATIVE_Q2=1 -e METRALE_GGUF_NATIVE_Q2_MMQ=1 \
  -e METRALE_GDN_FLASHINFER=1 \
...
- `METRALE_GDN_FLASHINFER: FlashInfer GDN kernel loaded (opt-in)` — GDN-FI engaged (fires on first prefill).

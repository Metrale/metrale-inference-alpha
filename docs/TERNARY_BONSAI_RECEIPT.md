# Ternary-Bonsai-27B on Metrale Engine — Serving Receipt
...
**native-Q2 (Tier-1/2/3).** Versus the BF16-dequant baseline on the same
model/prompts/hardware:

| Axis | BF16 dequant | native-Q2 | Win |
|---|---|---|---|
| Resident weights | 50.96 GB | **14.10 GB** | **3.6× leaner** |
| Prefill (1449-tok, C=1) | 554 tok/s | **781 tok/s** | **1.41×** |
| Decode (C=1) | 14.3 tok/s | **21.1 tok/s** | **1.48×** |
| Concurrent decode (C≥2) | — | works | Tier-3 fixed the hard crash |

Coherent throughout. native-Q2 cuts **weight bandwidth** (decode + memory).

The receipt originally also ran an opt-in FlashInfer GDN prefill loaded from a
prebuilt library (943 tok/s at 1449 tokens, column 3 of §6). That path has been
removed from the engine; prefill now always runs the default FLA GDN recurrence.
...
| Component | Value |
|---|---|
| Container | `metrale-inference-gb10:b12x-ready` (cargo/rustc, CUDA 13.2, FlashInfer at `/opt/flashinfer`) |
| Binary | Host-built `met` **runs as-is in the container** (glibc 2.39 both, cudart.so.13 soname-compat). No in-container rebuild needed for the production stack. |
...
```
METRALE_TARGET_MODEL='*' cargo build -p metrale-server --release --bin met \
...
|---|---|---|---|
| `METRALE_GGUF_NATIVE_Q2` | `1` | Keep id-42 weights **2-bit packed** (Tier-1 decode GEMV + the 22 GB memory win) | **REQUIRED** |
| `METRALE_GGUF_NATIVE_Q2_MMQ` | `1` | **Tier-2** native packed tensor-core MMQ prefill (no BF16 dequant tax, no co-dispatch race) | **REQUIRED** |
| _(Tier-3 batched decode)_ | — | Always-on code (no flag) — C≥2 concurrent decode via `q2_0_gemv_vec_batchm` | auto |
| `METRALE_PREFILL_CODISPATCH` | `1` | Fuse concurrent prefills within a window | **RECOMMENDED** |
| `METRALE_PREFILL_CODISPATCH_WINDOW_MS` | `80` | Co-dispatch fusion window | with codispatch |
| `METRALE_KV_OVERCOMMIT` | `1` | On-demand paged KV instead of a hard "pool fits N seqs" error / 400s at long ctx or many seqs | **RECOMMENDED** |
| `LD_LIBRARY_PATH` | `…/cuda-13.2/compat:/usr/local/lib:/usr/local/cuda/lib64` | compat driver (13.2) + CUDA libs | container only |

### Do NOT set (attn-FlashInfer — not usable for Bonsai)
`METRALE_FLASHINFER_PREFILL`, `METRALE_PREFILL_VARLEN`, `METRALE_Q12_BATCHED_FIRST_CHUNK`.
...
```
met serve --model-from-path <dir> --bind 127.0.0.1 --port <p> \
...
docker run --rm --gpus all --network host \
  -v /home/ms/metrale/.claude/worktrees/ternary-bonsai:/work \
  -v /tank:/tank -v <modeldir-parent>:/models \
  -e LD_LIBRARY_PATH=/usr/local/cuda-13.2/compat:/usr/local/lib:/usr/local/cuda/lib64 \
  -e METRALE_GGUF_NATIVE_Q2=1 -e METRALE_GGUF_NATIVE_Q2_MMQ=1 \
  -e METRALE_PREFILL_CODISPATCH=1 -e METRALE_PREFILL_CODISPATCH_WINDOW_MS=80 \
  -e METRALE_KV_OVERCOMMIT=1 \
  --entrypoint bash metrale-inference-gb10:b12x-ready -c \
  '/work/target/release/met serve --model-from-path /models/bonsai-vision \
...
- `METRALE_GGUF_NATIVE_Q2=1: keeping id-42 FFN projections packed (group 128)` — native-Q2 on.
- Coherence probe returns `Paris`.

## 6. Measured performance (single GB10)

**BF16-dequant vs native-Q2, identical fixed prompts:**
| prompt | metric | BF16 | native-Q2 (FLA) | native-Q2 + GDN-FI (removed) |
|---|---|---|---|---|
...
The GDN-FI column is historical: that opt-in prefill path has been removed, and
the FLA column is what the engine runs today. Decode is
...
- **Tier-2** MMQ prefill `q2_0_mmq.cu` (`METRALE_GGUF_NATIVE_Q2_MMQ=1`) — keep-packed int8 tensor-core, kills the ~2 s dequant tax + the co-dispatch race.

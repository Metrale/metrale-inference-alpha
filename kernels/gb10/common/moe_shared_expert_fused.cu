...
// 2026-09-25: Single-token NVFP4 MoE GEMVs in which blockIdx.y == top_k runs the shared
// expert beside the routed experts: gate+up, then SiLU(gate) * up and down.
//
// Owner: gb10 kernels.
// Invariants:
// - blockIdx.y < top_k is routed slot y (expert expert_indices[y]); y == top_k is the
//   shared expert, read through direct pointers.
// - A null routed weight pointer, or a null sh_gate_packed / sh_down_packed, writes zeros.
// - K must be a multiple of 16: inputs past the last multiple of 16 are not read.
...
// adjacent outputs, 8 per block of 128 threads. Routed results go to row slot of
// gate_out/up_out ([top_k, N]), the shared expert's to sh_gate_out/sh_up_out.
// Launch: grid (ceil(N / 8), top_k + 1, 2), block 128.
...
// 2026-09-25: SiLU(gate) * up, then down. The block writes the K activations into dynamic
// shared memory (s_act) once; each warp then computes two adjacent outputs. Routed
// results go to row slot of C, the shared expert's to sh_down_out.
// Launch: grid (ceil(N / 8), top_k + 1, 1), block 128, K * 4 bytes of dynamic shared memory.
...
extern "C" __global__ void moe_expert_silu_down_shared(
    ...
    // 2026-09-25: Routed experts clamp gate to at most SWIGLU_LIMIT and up to
    // [-SWIGLU_LIMIT, SWIGLU_LIMIT] before SiLU; the shared expert is not clamped. The limit
    // is a constant, not a per-model argument, so it applies to every model that loads this
    // file. The other kernels in this family (the _t, _bf16, _fp8, _batch2 and _batch3
    // files, moe_prefill.cu and moe_sorted_prefill.cu) do not clamp, so a routed expert's
    // activation here can differ from the same expert's in those paths.

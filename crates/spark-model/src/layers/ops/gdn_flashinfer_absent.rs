...
//! `dlopen("libmetralegdn.so")` — POSIX dynamic loading, and a `.so` that is only
...
//! `METRALE_GDN_FLASHINFER=1`, and its own docs note that "the binary builds and
...
/// Always false: there is no `libmetralegdn.so` to dlopen on this platform, so
...
pub fn flashinfer_gdn_prefill(
    ...
    bail!("FlashInfer GDN prefill requires dlopen(libmetralegdn.so); unavailable on this platform")
}

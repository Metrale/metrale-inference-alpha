...
const comparisonSvg = (page) => {
  const from = page.indexOf('class="gate-panel-title">Metrale Engine vs vLLM');
...
describe('the MoE tab today: a vLLM one-shot on the published instrument, no Metrale Engine leg', () => {
  const page = renderTab('qwen36-35b-a3b');
  ...
  test('the vLLM series is drawn as squares at its measured rungs only, every value from the ladder', () => {
    expect(page).toContain(
      'class="gate-panel-title">Metrale Engine vs vLLM · published instrument · ISL 128 / OSL 1024 · vLLM one-shot, no Metrale Engine run yet</span>'
    );
    ...
    // No Metrale Engine marks, no Metrale Engine line: the hollow key says why.
    expect(svg).not.toContain('gc-mark');
    expect(svg).not.toContain(`stroke="${colorFor(MOE.checkpoint)}"`);
    expect(page).toContain('Metrale Engine · no run at this instrument yet');
  ...
  test('the copy: once, engine+build, the forced kernel and its crash, the absent rungs, then what fills it', () => {
    inOrder(
      ...
      'not re-measured when Metrale Engine moves',
      ...
      'physical powercycle',
      'No Metrale Engine run at this instrument yet for <code>Qwen/Qwen3.6-35B-A3B-FP8</code>',
  ...
  test('POSITIVE CONTROL: a one-shot whose fingerprint equals the gate record IS drawn against it', () => {
    ...
    inOrder(
      ...
      'Metrale Engine is the newest passing run on main'
    );
...
describe('the DFlash tab today: Metrale Engine only, absent rungs answered in place', () => {
  ...
  test('the Metrale Engine curve is drawn dashed (same engine, another configuration) at its measured rungs only', () => {
    const svg = comparisonSvg(page);
  ...
  test('the legend and caption say vLLM was not run on THIS instrument, and what would fill it', () => {
    expect(page).toContain(`Metrale Engine · live · latest gate ${fmtDate(live.recorded_at)} · ${live.git_sha}`);
    expect(page).toMatch(/<rect [^>]*fill="none"[^>]*><\/rect>\s*<\/svg>vLLM · not measured on this instrument/);
    expect(page).toContain('class="gate-panel-title">Metrale Engine vs vLLM · gate instrument · ISL 512 / OSL 200 · natural fixture · batch cap 16 · fp8 KV<');
    inOrder(
      page,
...
describe('the dense tab today: the live gate record paired with the published vLLM bar', () => {
  ...
  // vLLM+MTP one-shot instead of the frozen August Metrale Engine series, and the two
  ...
  test('both charts are titled with ONE instrument', () => {
    expect(page).toContain(`class="gate-panel-title">Metrale Engine vs vLLM · gate instrument · ${instrument}</span>`);
  ...
  test('the gate record\'s numbers are drawn on the comparison chart; the frozen August Metrale Engine series is not', () => {
    const svg = comparisonSvg(page);
    const gateC128 = (+live.metrics.c128_aggregate_tok_s).toFixed(1);
    expect(svg).toContain(`<title>Metrale Engine · C=128 · ${gateC128} tok/s · gate record ${fmtDate(live.recorded_at)} · ${live.git_sha} · click for record</title>`);
  ...
  test('legend chips: Metrale Engine is the live record, vLLM+MTP a dated one-shot, the no-speculation leg refused by name', () => {
    expect(page).toContain(`Metrale Engine · live · latest gate ${fmtDate(live.recorded_at)} · ${live.git_sha}`);
  ...
  test('the caption says "once" in bold, names engine, build, box, the instrument and the record, and why nospec is not drawn', () => {
    inOrder(
      ...
      'not re-measured when Metrale Engine moves',
      `Metrale Engine is the newest passing run on main (${fmtDate(live.recorded_at)} · <code>${live.git_sha}</code>)`,
...
describe('the dense tab FALLBACK: the published pair when no live record pairs', () => {
  ...
  test('each chart is titled with its own instrument', () => {
    expect(page).toContain(
      `class="gate-panel-title">Metrale Engine vs vLLM · published campaign · ISL ${publishedLadder.workload.isl_tokens} / OSL ${publishedLadder.workload.osl_tokens}</span>`
    );
  ...
  test('legend chips: Metrale Engine is the campaign, every vLLM leg is a dated one-shot, dates from the series', () => {
    const metrale = series.find((s) => s.role === 'subject');
    expect(page).toContain(`>Metrale Engine · published campaign · ${range(metrale)}</button>`);
  ...
  test('the caption says "once" in bold, names engine, build, box and the Metrale Engine build', () => {
...
describe('ConcurrencyLadder takes the ladder it draws', () => {
  test('defaults to the published JSON, and a different ladder changes the copy', () => {
    ...
    expect(html(Ladder, { ladder: lost })).toContain('Metrale Engine vs vLLM, C=1 to 128 — 7 of 8 rungs');
  });
});
...
describe('the comparison state, decided once', () => {
  ...
  test('instrument, range and batch-cap readers print only what is recorded', () => {
    ...
    expect(batchCapOf('met serve --max-batch-size 16')).toBe('16');
    expect(batchCapOf('met serve')).toBeNull();
...
describe('the published pair: series pills and the ladder they drive', () => {
  ...
  test('the published card is the same chrome as the gate sweep: one figure.gate-panel each, head in a figcaption', () => {
    expect(page).toContain(
      '<figure class="gate-panel cmp"><figcaption class="gate-panel-head"><span class="gate-panel-title">Metrale Engine vs vLLM · published campaign'
    );
    expect(page).toContain(
  ...
  test('hiding a vLLM series removes its line, its marks, its legend key and its table column; the rest stays', () => {
    ...
    expect(after).toContain('<title>Metrale Engine · C=128');
    expect(cols(mainTable(before))).toEqual(['C', 'Metrale Engine', 'vLLM + MTP', 'vLLM, no speculation', 'Ratio']);
    expect(cols(mainTable(after))).toEqual(['C', 'Metrale Engine', 'vLLM, no speculation', 'Ratio']);
  ...
  test('hiding the tall series rescales the y axis to what is still drawn', () => {
    ...
    // Metrale Engine gone: its column and the ratio (Metrale Engine over vLLM) go together.
...
describe('ITL naming', () => {
  ...
  // for a subject with no Metrale Engine leg renders ConcurrencyBaseline; the ITL column

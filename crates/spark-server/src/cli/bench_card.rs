...
pub(crate) fn write_card(
    root: &std::path::Path,
    record: &metrale_plugin::gate::GateRecord,
    target: &str,
    ...
    let svg = metrale_plugin::gate::card::render(&template, record, card_args);
...
/// `met benchmark card <record>` — a card from an already-measured result.
pub(crate) fn card_cmd(args: crate::cli::bench_args::CardArgs) -> Result<()> {
    let record_path = resolve_record(&args.record)?;
    let record = metrale_plugin::gate::read_record(&record_path)

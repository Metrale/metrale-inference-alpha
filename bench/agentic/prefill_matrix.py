...
# 2026-09-26: Prefill matrix: aggregate prefill tok/s over a concurrency x input-length grid.
#
...
Prefill time is taken from Metrale Engine's own `usage.time_to_first_token_ms`, so decode
...
def one(model, prompt, timeout):
    req = urllib.request.Request(
        ...
        data=json.dumps({
            ...
            # 2026-09-26: Decode length does not enter the result: `cell` uses only
            # TTFT and prompt_tokens.
            "max_tokens": 250,
        }).encode(),

...
//   metrale-recipes), the competitive numbers from ladder.generated.json, and the
//   MLPerf status from mlperf.json. Nothing here can drift from the page,
//   because there is no second copy to drift.
//
// Prose that is genuinely editorial (what Metrale Engine is, what it is not) is read out
...
const { tagline, hero, githubUrl, recipesUrl, discordUrl, guideUrl, hardware } = data;
...
push(
  ...
  '',
  `Result: Metrale Engine wins ${s.won} of ${s.rungs} rungs, margin ${fmt(s.min_ratio)}x to ${fmt(s.max_ratio)}x`,
  'against the matched vLLM + MTP configuration at each concurrency.',
  '',
  '| concurrency | Metrale Engine tok/s | matched vLLM tok/s | ratio |',
  '| --- | --- | --- | --- |'
);
for (const r of ladder.rows) {
  const best = r.baselines.find((b) => b.id === r.best_baseline_id);
  push(`| ${r.c} | ${r.metrale.toFixed(2)} | ${best.tok_s.toFixed(2)} (${best.label}) | ${fmt(r.ratio_vs_best)}x |`);
}
push(
  '',
...
push(
  ...
  '',
  'Every model below maps to one recipe in metrale-recipes; the site cannot list a',
  'model that has no recipe. Run any of them with `metralectl run <id>`.',
  ''
);

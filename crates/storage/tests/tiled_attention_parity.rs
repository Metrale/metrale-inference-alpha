...
//! 2026-09-25: GPU tests of `TiledAttention`: one tile over all blocks matches
//! the CPU reference (`attention_ref`) within 1e-2, and tiles of 1, 2, 4 and 8
//! blocks match the single tile within 1e-2 (max) and 1e-3 (mean). The running
//! state carried between tiles is FP32.
//!
//! Owner: storage (tests).
...
use metrale_storage::attention_ref::{AttnState, finalize_ref, step_tile_ref};
...
fn run_gpu(tile_size: usize, q: &[bf16], k: &[bf16], v: &[bf16], block_table: &[i32]) -> Vec<bf16> {
    ...
    for t in 0..n_tiles {
        ...
        let n = end - start;
        // 2026-09-25: The kernel reads only the first `n` entries, the count
        // uploaded beside the tile.

...
- Box: dgx1. Model: 27B (Metrale: `centml/Qwen3.6-27B-NVFP4-W4A4-mlpinf`; vLLM: same checkpoint if it
  loads, else `nvidia/Qwen3.6-27B-NVFP4` — the leg json records which).
- Metrale serve: golden flags + `--max-batch-size 16`, fifo scheduling (SLAI starves prefill at load),
  env incl. `METRALE_MTP_GATE_FORCE=1`. Binary: pushed tip of PR #369.
- vLLM serve: `sparkrun-eugr-vllm:latest` (vLLM 0.23.1rc1.dev207), `--max-num-seqs 128`,
  `--max-model-len 32768`, util 0.85.
- Synthetic scoreboard: `bench/bench-metrale-concurrency.py`, C=[1,2,4,8,16], default 4 ISL/OSL
  regimes (≤4096); agentic-harness `target_concurrency` sweep follows as a second driver.

## Log (appended by drivers)
5048fa13d69c2870420a8b9050f54221  conc_sweep/met_phaseA_baseline
- 2026-07-25T18:13:50Z LEG metrale_synth SERVE_DIED
- CONFIG CHANGE after first metrale_synth SERVE_DIED: --max-batch-size 16 + slots 128 + nd=3 needs
  ~52G of SSM reservations (seq-state 14.2G + rollback ring 18.9G + Marconi 18.9G) + 17.5G weights
  before ANY KV — preflight refusal territory at util 0.70. New metrale C-config: --max-batch-size 20
...
- 2026-07-25T19:17:44Z LEG metrale_synth SERVE_DIED (deathlog: conc_sweep/metrale_synth.deathlog)
- 2026-07-25T19:17:47Z PHASEA_DONE
- SECOND metrale_synth death, deathlog decisive: "39.7 GB consumed + 50.5 GB inference reserve =
...
51fc31d43a6e59aec8e9eaced56a02b2  conc_sweep/met_phaseB
5048fa13d69c2870420a8b9050f54221  conc_sweep/met_phaseA_baseline
- 2026-07-25T22:10:42Z LEG metrale_synth DONE -> results/metrale_synth.json
- 2026-07-25T22:10:46Z PHASE A compare written -> results/compare.txt
- 2026-07-25T22:10:46Z PHASEA_DONE
- 2026-07-26T00:40:49Z LEG metraleB_nographs DONE -> results/metraleB_nographs.json
- 2026-07-26T02:57:30Z LEG metraleB_graphs DONE -> results/metraleB_graphs.json
- 2026-07-26T02:57:34Z PHASEB_DONE
- 2026-07-26T18:21:44Z LEG metraleC_perseq DONE -> results/metraleC_perseq.json
- 2026-07-26T21:07:11Z LEG metraleC_batched DONE -> results/metraleC_batched.json
- 2026-07-26T21:07:15Z PHASEC_DONE
- 2026-07-27T02:57:15Z LEG metraleD_kmarm DONE -> results/metraleD_kmarm.json
- 2026-07-27T05:32:37Z LEG metraleD_kmarm_graphs DONE -> results/metraleD_kmarm_graphs.json
- 2026-07-27T05:32:41Z PHASED_DONE

## 2026-07-27 — the n=16 step decomposed (this is where the gap lives)

Instruments: `METRALE_MS_PROFILE` (branch split), `METRALE_SSM_MS_PROFILE` (mixer vs FFN inside the SSM
layers), `METRALE_SSM_DETAIL_PROFILE` (mixer stages). Config: phase-D binary, bs=16, fifo, slots 32.
...
`METRALE_SSM_BATCHED_RECURRENT` + `METRALE_GDN_FUSED_NORM` = **+2.6% at C=16** (53.7 → 55.1 tok/s),
coherence preserved. `METRALE_GDN_FUSED_CONV` adds nothing on top. This confirms the older
...
| + FFN NVFP4 MMQ (drop `METRALE_NO_FFN_NVFP4_MMQ`) | 61.2 | +11.3%, C=1 neutral, output identical |
...
`METRALE_NO_FFN_NVFP4_MMQ` is a PRESENCE flag: `=0` does NOT enable MMQ, the variable must be absent.
...
1. **Metrale non-spec at C=1 is 14.1 tok/s; vLLM at C=1 is 14.2.** Identical. At batch 1 both engines
...
2. **Scaling, normalised to each engine's own C=1 non-spec throughput:**
   ...
   Metrale: 1.0x -> 1.46x -> 2.60x -> ~3.9x -> ~4.3x
   vLLM scales nearly linearly to C=8; Metrale saturates around 4.3x. The gap is a BATCHING-EFFICIENCY
...
Zero-edit probe (`METRALE_SSM_MS_PROFILE=1`, one serve, drove C=4/8/16, 9168 samples per n):
...
`multi_seq/ffn.rs:135`), behind an `METRALE_NO_SSM_FFN_PREFILL` kill switch. n<=8 must keep
...
"WIDE-VERIFY BATCHED DENSE FFN" branch. Default ON, kill switch `METRALE_NO_SSM_FFN_PREFILL=1`
...
(`METRALE_MTP_MAX_SEQS`, default 2).
...
2. Mixer tensor-core projections (`METRALE_SSM_TC_PROJ`, ssm_batched.rs) — qkvz/out_proj still run
...
Flipping `METRALE_MTP_MAX_SEQS` to 2 exposed that the spec-eligibility predicate reads
...
Now summed over all active sequences. (Inert under `METRALE_MTP_GATE_FORCE=1`, which the benchmarks
...
| C | 1 | 2 | 4 | 8 | 16 |
|---|---|---|---|---|---|
| Metrale | 25.5 | 25.3 | 37.9 | **57.9** | **79.5** |
...
Metrale instead dispatches BY M into a scalar-FMA GEMV ladder whose runtime is proportional to M.
That single design difference is the marginal-cost gap.

`METRALE_SSM_TC_PROJ` routes the mixer's qkvz/out_proj onto `w4a16_gemm_t` (M64/N128 FP8-MMA tile
...
identical, but a BFCL gate is owed before merge. Same debt applies to `METRALE_MTP_MAX_SEQS=2`.
...
1. **LM head kernel** — `decode_a2.rs:429` calls `w4a16_gemm` unconditionally; its M64 tile wastes
   75% of the MMA at M=16, giving a FLAT ~20 ms/step against a 2.65 ms roofline. Metrale ALREADY owns
...
Metrale already owned the fix: the MTP verify path (`impl_a3.rs`) routes M<=8 to the batched GEMV
...
Every win this session is the SAME bug in a different place: **Metrale dispatches by M into a
...
this choice — Marlin issues mma at M=1. Anywhere Metrale still selects a kernel BY M is a suspect.
...
### Re-measured decomposition at n=16 (eager, METRALE_MS_PROFILE, 190 samples)
...
| C | 1 | 2 | 4 | 8 | 16 |
|---|---|---|---|---|---|
| Metrale | 25.5 | 24.4 | 46.1 | 65.5 | **93.2** |
| vLLM | 14.2 | 27.8 | 53.3 | 98.8 | 168.9 |
...
Implemented: `metrale_nvfp4_mmq{16,32}_{nc,wc}` instantiations of the SAME template (mmq_x is a free
...
`METRALE_NO_MMQ_SMALL_TILE=1`. Verified the new entries really compiled (present in t0__nvfp4_mmq.ptx).
...
`METRALE_DECODE_GRAPHS_MULTISEQ` is now DEFAULT-ON (`METRALE_NO_DECODE_GRAPHS_MULTISEQ=1` disables). Its
...
**Metrale C=1 non-spec is 14.1 tok/s; vLLM C=1 is 14.2. PER-SEQUENCE PARITY on identical silicon.
Yet vLLM scales 11.9x to C=16 where Metrale scales 6.8x.** The whole C=8/C=16 deficit is
...
### Full step budget at C=16, eager, 190 samples/point (METRALE_MS_PROFILE + METRALE_SSM_DETAIL)
...
- Metrale 95.9 tok/s = 167 ms/step = **49% of roofline**.
...
`METRALE_MS_PROFILE` forces eager, but `METRALE_SSM_MS_PROFILE` / `METRALE_SSM_DETAIL_PROFILE` only
...
`METRALE_NO_DECODE_GRAPHS_MULTISEQ=1` with the SSM profilers. (Same class as the METRALE_MTP_TIMING
...
Report: /tmp/metrale_prof3.nsys-rep. Invocation that WORKS: plain `nsys profile` + SIGTERM to
the met PID. `--delay/--duration` produced no report; `--cpuctxsw` is invalid on
`nsys launch`; the driver script's port must match the serve's.

| kernel                                   | %GPU | inst/step | avg     | ms/step | vs floor |
|------------------------------------------|------|-----------|---------|---------|----------|
| metrale_nvfp4_mmq16_nc (FFN)               | 35.5 | 154       | 283 us  | 43.6    | 1.29x    |
...
| metrale_nvfp4_mmq32_nc                     | 3.6  | 16        | 276 us  | 4.4     | --       |
...
The 38.5 ms that METRALE_MS_PROFILE attributes to "attention layers" is those layers' FFN
...
4. Persistent/stream-K: ~10-12 ms but high risk; split-K first. Note `mul_mat_q_stream_k_fixup`
   exists (`q4k_vendor/mmq.cuh:3789`) but Metrale bypasses it with `fixup=false`, justified as
...
- `2db1b349` **fused q|k|v into ONE N=14336 GEMM writing qkv_buf DIRECTLY.** 3 GEMMs
  ...
  (+1.6%), sigma 0.09, distributions disjoint.** Kill switch METRALE_NO_FUSED_QKV=1.
...
Kill switch `METRALE_NO_THINKENDED_GPU_ARGMAX=1`.
...
4. CUDA-graph interaction — NO: faults identically with `METRALE_NO_DECODE_GRAPHS_MULTISEQ=1`.
...
The FFN (`metrale_nvfp4_mmq16_nc`) is the largest single block: **192 inst/step, 54.3 ms,
...
at 32, vs 100 KiB/SM on sm_121 — so TWO CTAs fit, yet every Metrale entry carried
...
dispatch fix, and it is the code path Metrale owns least.
...
`METRALE_W4A16_K64_MIN_K=<n>` so any A/B can pin a prior threshold exactly.
...
| `metrale_nvfp4_mmq16_nc` | 40 | ffn_down N=5120 K=17408 | 285.5 us | **175.6** |
| `metrale_nvfp4_mmq16_nc` | 136 | ffn gate/up N=17408 K=5120 | 289.5 us | **173.2** |
...
- Debt is UNDISCHARGEABLE while [[feedback_no_accuracy_gate_until_vllm_parity]] stands, and it
  STACKS on the existing `METRALE_SSM_TC_PROJ` W4A8 debt (ssm_batched.rs:28-32 already records
...
Measured 4 reps/leg, byte-identical, control pinned via a new `METRALE_FUSED_QKV_MIN_N=9`:
...
the cross-turn carry (`METRALE_MTP_CARRY_DRAFTER`) does not also apply, the proposer starts empty.
...
`crates/model-layers/src/mtp_carry.rs` documents this exact mechanism from a prior
...
**What is NEW tonight is the SCOPE.** The shipped fix (`METRALE_MTP_CARRY_DRAFTER`, on by
...
`METRALE_MARCONI_MIN_TOKENS=<n>` overrides; 0 restores always-restore.
...
All seven kill switches engaged simultaneously (`METRALE_NO_W4A16_K64=1`,
`METRALE_NO_ATTN_BATCH_CACHE_WRITE=1`, `METRALE_NO_FUSED_QKV=1`,
`METRALE_NO_THINKENDED_GPU_ARGMAX=1`, `METRALE_NO_ARGMAX_BATCH=1`, `METRALE_NO_GDN_HALF_REG=1`,
`METRALE_MARCONI_MIN_TOKENS=0`):
```
...
is positive with 7 of 8 reps favouring. Kill switches: `METRALE_NO_ROPE_STRIDED=1`,
`METRALE_NO_QK_NORM_STRIDED=1`.
...
A standalone repro (`crates/model-arch/examples/w4a16_lmhead_716_repro.rs`) reproduces the serve's
...
11. the `METRALE_NO_DECODE_GRAPHS_MULTISEQ=1` elimination — VALID (the check is value-based,
...
`crates/model-engine/src/model/trait_impl/meta_argmax.rs` `argmax_batch_dispatch` IGNORES its `_stream`
...
(`cargo run -p metrale-model-layers --release --example w4a16_m17_bench --features cuda,gpu-examples`;
...
the ONE concurrency where Metrale already beats vLLM 1.79x (25.4 vs 14.2). Crossover is M ~= 8.
...
### Status
Twin implementation is BUILT and reverted-in-place (kill switch `METRALE_NO_LMHEAD_TGEMM=1`
...
- Dispatch at `padded_n >= 5` only. Default ON, kill switch `METRALE_NO_LMHEAD_TGEMM=1`.
...
`METRALE_NO_LMHEAD_TGEMM=1`, same binary:
...
Metrale >= vLLM at C=1..16. It bounds the RISK, it does not discharge the DEBT.
...
relayout in `metrale_nvfp4_repack` (Metrale owns the layout) so the x-tile fill vectorizes 16-B,
...
the identical phase-serial loop and no cp.async either, so this would be Metrale-original work.
...
- ★ MEASURED ON GB10 ITSELF — llama.cpp PR #22196 (Blackwell native NVFP4, benchmarked on a DGX
  DGX Spark by NVIDIA's own engineer): prefill 506->611 t/s (+21%), decode 12.01->**11.91** t/s
...
is `METRALE_NO_FFN_NVFP4_MMQ`, and it measures **MMQ off 55.0/54.8/54.8 (mean 54.9) vs MMQ on
...
`metrale_nvfp4_repack` emitted an array of 36-byte `block_nvfp4`, which INTERLEAVES 4 scale bytes
...
qualify (5120->80, 17408->272) and MMQ already requires K % 512 == 0. Safe because Metrale exposes
...
hatch is the existing `METRALE_NO_FFN_NVFP4_MMQ`.
...
2. Resolve the new handle at `qwen3_ssm/init.rs:122` and `qwen3_attention/init.rs:419`, falling
   back to `w4a16_gemm_t_k64` under **`METRALE_NO_K64_PIPELINE3`** (PRESENCE check — `=0` is NOT off).
   Call sites untouched.
3. Gates: build with `METRALE_TARGET_MODEL=qwen3.6-27b` and **md5 the binary vs previous** or the A/B
...
(`bf3a0b07...` on both legs). Default ON, kill `METRALE_NO_K64_PIPELINE3` (PRESENCE, not value).
...
(`bf3a0b07...` both legs). Default ON, kill `METRALE_NO_TGEMM_PIPELINE3` (PRESENCE). Three qwen3
...
### ★ MEASURED: `METRALE_MTP_MAX_SEQS` is a GUARD, NOT A KNOB — raising it HALVES throughput
| cap | C=2 | C=4 |
|---|---|---|
| **2 (default)** | 25.3 | **48.5** |
| 4 | 25.7 | **25.8** |
...
1. **Batched cross-seq propose** — one M=n drafter forward per draft position (drafts chain
   ...
   A/B alone: 46.3-46.8 vs 42.9-43.6 kill-switch (`METRALE_NO_MTP_BATCH_PROPOSE`), disjoint.
2. **out_proj M>8 dispatch** (m_dispatch class strikes again, in reverse): R=16 verify rows fell
   ...
   fingered the SSM arm. New arm via `deep_k_gemm`, kill `METRALE_NO_VERIFY_OUTPROJ_TGEMM`.
...
gate. Batched K=4 MTP at C<=4, bit-for-bit MTP-off fallback at C>4. `METRALE_MTP_MAX_SEQS` default
...
- ★ C=2 slow-mode finding: after ~4+ drives on one serve, C=2 bimodally drops to ~32-33 tok/s
  ...
  REPRODUCED EXACTLY with `METRALE_NO_MTP_K_LADDER=1` (control serve: 35.7/35.0/35.4/35.4 then
...
`METRALE_TARGET_MODEL=qwen3.6-27b cargo build -p metrale-server --release --features cuda`;
...
- `3313a733` perf(mtp): default ladder `4:3,8:2` -> `4:3,8:3`. Kill switch
  `METRALE_MTP_K_LADDER=4:3,8:2`. Unit tests updated (3 pass), rustfmt clean.
...
`METRALE_TARGET_MODEL=qwen3.6-27b cargo build -p metrale-server --release --features cuda`; ONE FRESH
...
### Accept telemetry by width (`METRALE_MTP_ACCEPT_DEBUG`)
...
rows below are variance measurements, not regression risk. Kill switch `METRALE_MTP_MAX_SEQS=8`.
...
free. Kill switch `METRALE_NO_MTP_DCUT` (presence).
...
- `mtp_max_seqs` default 8 → 16, anchored in the function body (`speculative/ladder.rs`).
  Kill switch `METRALE_MTP_MAX_SEQS=8`.
- Default ladder `4:3,8:3` → `4:3,8:3,16:1`, anchored in `mtp_ladder_steps`'s body (NOT a bare
  literal regex — the burned-twice trap). Kill switch `METRALE_MTP_K_LADDER=4:3,8:3`.
- D-Cut `dcut_enabled` OFF-by-presence → ON-by-default with `METRALE_NO_MTP_DCUT`; `dcut_ratio`
  default 0.5 → 0.75.
...
`METRALE_TARGET_MODEL=qwen3.6-27b cargo build -p metrale-server --release --features cuda`)
ONE FRESH SERVE PER C; no `METRALE_MTP_DCUT` / `METRALE_MTP_MAX_SEQS` / `METRALE_MTP_K_LADDER` anywhere
in the environment (only `METRALE_MTP_ACCEPT_DEBUG=1` for the telemetry columns) — the binary's own
...
`METRALE_SSM_H_FP16` was decoded independently in TWO places (the kernel accessor and the
...
| was | is | default |
|---|---|---|
| `METRALE_SSM_H_FP16` | `--ssm-h-dtype {f32,f16}` | `f32` |
| `METRALE_GDN_FUSED_NORM=1` | `--gdn-fused-norm` | off |
| `METRALE_SSM_BATCHED_RECURRENT=1` | `--ssm-batched-recurrent` | off |
| `METRALE_SSM_TAIL_MIDCHUNK=0` | `--ssm-tail-midchunk <bool>` | on |
| `METRALE_MTP_GATE_FORCE=1` | `--mtp-gate {auto,force}` | `auto` |

★ **The clap defaults sealed all five under `met serve`. That is fixed now — but every
...
`metrale_gpu_runtime::set_ssm_tail_midchunk` takes an `Option<bool>` and returns without writing
...
script that set `-e METRALE_SSM_TAIL_MIDCHUNK=0` **without** also passing
...
**The other six of the ten did nothing.** `METRALE_MTP_CATCHUP=0`, `METRALE_MTP_DRAFT_CONF=0.0`,
`METRALE_SSM_TAIL_PROTECT=1`, `METRALE_SSM_TAIL_LEASE_TTL=128`, `METRALE_BF16_TC_PREFILL=1`
(shadowed by the MMQ arm) and `METRALE_MTP_ACCEPT_DEBUG=1` (a log line) were each already the
compiled default, inert, or unread. Every ladder from wave 17 on carried all ten.

### 3. `METRALE_GDN_FUSED_NORM` / `METRALE_SSM_BATCHED_RECURRENT` were NOT promoted to defaults
...
sha256 6412b12d). Image `metrale/metrale-inference-gb10:7241a95` = gate image = this branch modulo bench-only
harness deltas, so this IS the #388 binary's sweep. Recipe-derived serve
(`serve_metrale.sh` ← `recipes/qwen3.6/qwen3.6-27b-w55-sweep-dev.yaml`): util 0.85, bs 128, bf16 KV,
...
| C | Metrale | vLLM | ratio | prev(2026-08-02) |
|---|---|---|---|---|
...
**8/8 rungs reconfirmed** — Metrale wins every rung on tok/s. Metrale absolute tok/s
within ±0.6% at C≤2, −1.5..−2.7% at C=4..32, −5.3% at C=64, −9.0% at C=128 vs the
2026-08-02 ladder. Spread: vLLM 0.05–0.63%; Metrale 1.0–4.1% (C=1 rep spread 9%).
Two confounds on the original ladder are now explained, not denied:
1. The C=128 "timeout" finish-reason in the Metrale leg traces to `--request-timeout 300s`
   (Metrale default; vLLM runs no comparable deadline). Control leg reran C=128 with
   REQ_TIMEOUT=0: 439.43 tok/s vs spec-on 429.52 — the deadline was costing ~2%, not the engine.
2. Clock probe healthy on every rung (2236–2457 MHz Metrale, 2463–2483 vLLM) — no 513 MHz clamp.

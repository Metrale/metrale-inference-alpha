...
/// Kill switch for this tier: PRESENCE of `METRALE_NO_FP8_QKV_BATCH` (any value)
...
impl Qwen3AttentionLayer {
    ...
    /// below 5 rows, then the `METRALE_FFN_M16_TC` MMA arm, the bit-exact
    ...
    fn ms_qkv_batchm_fp8_gemv(
        ...
        // REASSOCIATES the K reduction, so it is behind `METRALE_ATTN_M16_TC`
        // (or the `METRALE_M16_TC` umbrella) and off by default; `h % 128 == 0`
        ...
        let (launch, kernel): (StridedBatchGemv, KernelHandle) = if n <= 4 {
            ...
            // sets `METRALE_FFN_M16_TC` is asking for the MMA route explicitly,

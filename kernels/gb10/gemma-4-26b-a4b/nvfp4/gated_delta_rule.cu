...
// forked-from: kernels/gb10/common/gated_delta_rule.cu (2026-09-24; 2069 of 1769 lines differ, see kernels/FORKS.md)

// 2026-09-25: Gated delta rule (GDN) kernels: token-sequential prefill (one block per v-head, or split into 2 or 4
// blocks, plus a per-stream batched split4), single-token decode with BF16 or FP32 output, 2- and 3-token decode (chunk2,
// chunk3), and FP32 decode fused with the gated RMS norm, with the conv1d update, or over strided rows.
...
// and the chunk kernels also v_dim == 128, one thread per column) and check neither. The single-stream prefill kernels read q/k/v and gate/beta at
// row t for every b. gated_delta_rule_decode_f32_norm offsets z_gate and output by v-head only, so it is correct for
// batch_size 1, which every ops::gdn_decode_f32_norm call passes.
...
gated_delta_rule_prefill(
    ...
    // 2026-09-25: Dynamic shared memory, 4 * K_DIM floats: k and q of two tokens (the launcher passes 4 * k_dim * 4 bytes).
    ...
    for (unsigned int t = 0; t < seq_len; t++) {
        ...
        // 2026-09-25: Load token t + 1's k and q into the other buffer; the barrier that ends the step publishes them.
        if (t + 1 < seq_len) {
...
// 2026-09-25: gated_delta_rule_prefill_split with four 32-thread blocks per v-head (blockIdx.x = vh * 4 + split).
...
// 2026-09-25: gated_delta_rule_prefill_split4 over batch_size streams of seq_len tokens each: stream b's state is
// h_state_ptrs[b], and its q/k/v, gate/beta and output rows start at row b * seq_len; the per-token statements are the same.
...
// 2026-09-25: Single-token decode: FP32 q/k/v (row b for sequence b), BF16 output; H is read into registers once and
// written back once.
...
// 2026-09-25: Two-token decode: the tokens of sequence b are rows 2b and 2b + 1; H stays in registers across both.
// h_state_intermediate is neither read nor written.
...
// 2026-09-25: Three-token decode: rows 3b, 3b + 1 and 3b + 2; h_state_inter0 and h_state_inter1 are neither read nor written.
...
// 2026-09-25: Helpers of the kernels below, with the same bodies as in common/gated_delta_rule.cu.
...
// 2026-09-25: FP32 decode fused with the conv1d update + SiLU (q and k then L2-normalised) and the gated RMS norm.
// Grid [num_k_heads, batch]: block kh handles k-head kh and its head_repeat v-heads, so it alone updates their conv_state
// channels. Thread tid: which_v = tid / v_dim, vlocal = tid % v_dim. The reductions assume 256 threads, q in warps 0-3
// and k in warps 4-7: head_repeat * v_dim == 256 and k_dim == v_dim == 128. new_input and conv_state channels are
// [Q (num_k_heads * k_dim) | K | V (num_v_heads * v_dim)].
...
// 2026-09-25: FP32 single-token decode where sequence b's q/k, v, gate/beta and output rows are qk_stride, v_stride,
// gb_stride and out_stride elements apart, so q, k and v can be rows of one [Q | K | V] buffer.
...
// 2026-09-25: gated_delta_rule_decode_f32_strided fused with the gated RMS norm (z rows z_stride apart); writes BF16.

...
fn run_one(
    ...
    metrale_tier::pio::read_at_least_at(
        file,
    ...
    if let Some((sblock, swithin, sdst)) = job.scale {
        ...
        // that runs past EOF.
        metrale_tier::pio::read_at_least_at(sf, bounce.blocks(1), sblock, swithin + 4)
...
/// `METRALE_PLE_SERIAL_FAULT=1`: keep the pre-parallel QD=1 arm, so the
/// speedup can be measured against the thing it replaced rather than
/// asserted (same A/B convention as `METRALE_HC_DECODE_SPLIT`).

...
echo "=========================================================="
echo " Running Metrale Engine Kernel Microbenchmarks (Serially)"
echo "=========================================================="
...
cargo bench -p metrale-spark-bench --bench norm

echo ""
echo "=> Benchmarking: Activations (SiLUxMul)"
cargo bench -p metrale-spark-bench --bench activation

echo ""
echo "=> Benchmarking: GEMM (Dense TC, W4A16)"
cargo bench -p metrale-spark-bench --bench gemm

echo ""
echo "=> Benchmarking: RoPE (Rotary Position Embeddings)"
cargo bench -p metrale-spark-bench --bench rope

echo ""
echo "=> Benchmarking: MoE (Grouped W4A16 GEMM)"
cargo bench -p metrale-spark-bench --bench moe

echo ""
echo "=> Benchmarking: Attention (Paged Decode FP8)"
cargo bench -p metrale-spark-bench --bench attention

echo ""
echo "=> Benchmarking: SSM (Causal Conv1d, Gated Delta Rule)"
cargo bench -p metrale-spark-bench --bench ssm

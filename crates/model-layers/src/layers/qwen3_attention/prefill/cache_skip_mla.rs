...
//! 2026-09-25: The MLA branch of `prefill_attention_with_cache_skip`: Q and KV
//! through their latent projections, the latent + rope row written to the
//! paged cache, then attention on K/V expanded from the latent by `wkv_b`
//! (unabsorbed) with `prefill_attention_64`.
//!
...
impl Qwen3AttentionLayer {
    /// 2026-09-25: Runs the MLA prefill chain and returns the O-projection
    /// output (`qkv_output`). It writes the paged cache for all `n` rows: the
    /// arguments carry no `kv_write_start`.
    pub(super) fn prefill_attention_cache_skip_mla(
        ...
        // 2026-09-25: KV latent (normed) and the rope part of K.
        ...
        // 2026-09-25: Extract Q's rope part and apply YaRN RoPE to it and K's.
        ...
        // 2026-09-25: Assemble the latent + rope cache row and write it to the
        // paged cache.
        ...
        // 2026-09-25: Unabsorbed attention: expand K/V from the latent with
        // `wkv_b`, then run `prefill_attention_64`.
        ...
        // 2026-09-25: METRALE_OP_DUMP hooks: the assembled V and the attention
        // output. With one token, softmax over one key is 1.0, so the output is
        // V0 and the layer result is V0 @ o_proj.
        if n > 0 {
        ...
        // 2026-09-25: O projection into `qkv_output`.

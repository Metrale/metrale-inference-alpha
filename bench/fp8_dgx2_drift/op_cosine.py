...
def compare_ssm_stages(metrale_dir: pathlib.Path, hf_dir: pathlib.Path, results: list):
    ...
    Stage mapping (Metrale Engine → HF op):
      ...
      moe_out          → moe_out
    """
    # Stage map. Note: ssm.pre_norm is known-corrupt (Metrale Engine dumps f32 residual
    # as if it were BF16 — pre-existing dump bug). Marked unreliable in output.
    # Conv1d shape mismatch (Metrale Engine dumps 8192 BF16 last-token interleaved;
    # HF outputs [1, 8192, T+3] and our naive extract picks the last channel,
    # not last time-step). Marked unreliable in output.
    # gnorm: Metrale Engine dumps full value_dim=4096; HF norm.weight is [head_dim=128]
    ...
    for abs_i in SSM_LAYERS:
        ...
        for metrale_stage, hf_op in stage_map:
            ...
            if hf_op is None:
                # Metrale-only stage; record file presence so the master
            ...
            # Metrale Engine SSM dumps are BF16 raw bytes (2 bytes each). Need to widen.
            if metrale_path.exists():
                raw = np.fromfile(str(metrale_path), dtype=np.uint16)
                metrale_arr = np.frombuffer(
                    np.left_shift(raw.astype(np.uint32), 16).tobytes(),
...
def compare_attention_ops(metrale_dir: pathlib.Path, hf_dir: pathlib.Path, results: list):
    ...
    # HF-only ops captured (no direct Metrale Engine counterpart yet) — emit as
    ...
    for rel_i, abs_i in enumerate(FULL_ATTN_LAYERS):
        for op in full_attn_ops:
            ...
            if metrale_arr is None or hf_arr is None:
                ...
                results.append(row)
                continue
            # For q_proj_full Metrale Engine dumps Q+Gate interleaved (2× q_dim).
            # HF q_proj output also is 2× q_dim (qwen3.6 attn_output_gate=True),
            # but the layout differs (Metrale Engine interleaves; HF concatenates as

...
/// to record which Metrale Engine produced an artifact should use this rather than
/// re-deriving it.
...
#[command(
    ...
    about = "Metrale Engine Spark — pure Rust LLM inference server"
)]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
...
pub enum Command {
    ...
    /// (0 cached)` — whatever the real cause was: an `~/.metrale` owned by another
    /// uid, a `sync-recipes` that was never run, or a signing identity minted
    /// into a scratch METRALE_HOME whose key nobody committed.
    ///

...
impl Qwen3SsmLayer {
    ...
    /// `ssm_deinterleaved` buffer. `force_bf16` (= `METRALE_GDN_BF16_WEIGHTS`)
    ...
    pub(super) fn prefill_qkvz_proj(
        ...
        // Env override: METRALE_GDN_BF16_WEIGHTS=1 forces the BF16 dense
        ...
        // (`METRALE_FP8_ROWWISE=1`), dequantised ONCE to BF16 — into the
        ...
        // block-scaled path with `METRALE_CUBLAS_FP8=1`, so it is the GEMM and
        ...
        //
        // `force_bf16` still wins, so the `METRALE_GDN_BF16_WEIGHTS` A/B lever
        // keeps working.
        ...
        // NO cuBLASLt-BF16 ARM HERE, deliberately. `METRALE_CUBLAS_GEMM=1` used

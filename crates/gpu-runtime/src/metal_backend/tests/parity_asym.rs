...
//! 2026-09-25: Parity of the BF16-K / TurboQuant-V kernels:
//! `kv_cache_append_bf16k_turbo{4,3,2}v` against a CPU quantizer, and
//! `attention_decode_bf16k_turbo4v` against a CPU attention reference.
//!
...
/// 2026-09-25: One append through a `kv_cache_append_bf16k_turbo*v` kernel; returns
/// host copies of row `cache_pos` of (k_cache as BF16, v_data, v_scales).
...
/// 2026-09-25: For all three kernels K is copied bit-exact and every V scale byte
/// matches the CPU quantizer; the V index packing is compared for the 4-bit kernel.
#[test]
...
/// 2026-09-25: `attention_decode_bf16k_turbo4v` against a CPU reference: scores from
/// the raw BF16 K, V dequantised on the CPU, and the kernel's sparse-V skip (a
/// position with `exp(score - max) <= 1e-3` leaves the V sum).
#[test]

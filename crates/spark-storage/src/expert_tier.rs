...
///   * `rdma`          — connect to `$METRALE_EXPERT_PEER` over TWO-SIDED TCP.
///   * `rdma-verbs`    — connect to `$METRALE_EXPERT_PEER` over ONE-SIDED RDMA READ
///     (verbs); device/GID from `$METRALE_EXPERT_RDMA_DEV`/`$METRALE_EXPERT_RDMA_GID`.
///

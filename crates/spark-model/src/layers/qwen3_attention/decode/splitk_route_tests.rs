...
///
/// `sm_count` is read from `metrale_kernels::TARGET_SM_COUNT` — the compiled
...
/// `METRALE_ATTN_DECODE_SPLITK=0` is the pre-#928 control, and the line must say
...
/// (`metrale-kernels/tests/kernel_arity.rs`); that pin needs a non-stub build

...
//! the `METRALE_CUBLAS_GEMM=attn` route for chunk 0 of a prefill, which
...
//! `Qwen/Qwen3.8-27B-FP8`, recipe `METRALE_CUBLAS_GEMM=ffn,ssm,attn`
...
//! `METRALE_ATTN_QKV_W8A16_ONLY` (presence) restores W8A16.
...
/// `METRALE_ATTN_QKV_W8A16_ONLY` kill switch: PRESENCE (any value, including
/// empty) keeps the cache-skip Q/K/V prefill on the W8A16 kernels. Presence
/// rather than `=1`, matching `METRALE_FFN_W8A16_ONLY` — an escape hatch whose
...
/// * `cublas_attn` — `METRALE_CUBLAS_GEMM` must name the `attn` family.
/// * `fp8_blockscaled_prefill` — the `METRALE_FP8_SINGLE_SCALE` kill switch.

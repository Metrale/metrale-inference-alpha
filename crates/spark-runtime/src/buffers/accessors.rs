...
impl BufferArena {
    ...
    /// stream), no free. NULL unless `METRALE_GGUF_NATIVE_Q2`.
    ...
    /// (`METRALE_GGUF_NATIVE_Q2_MMQ`). NULL unless the flag is set.
    ...
    /// Env-gated (`METRALE_SSM_SAVE_DUMP`) per-buffer checksum probe.
    ///

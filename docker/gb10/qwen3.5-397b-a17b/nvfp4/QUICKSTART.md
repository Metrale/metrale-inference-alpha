...
```bash
docker build -f docker/gb10/qwen3.5-397b-a17b/nvfp4/Dockerfile -t metrale-397b .
```

The builder stage compiles only the `qwen3.5-397b-a17b` kernel target
(`METRALE_TARGET_MODEL=qwen3.5-397b-a17b`, `METRALE_TARGET_QUANT=nvfp4`); the runtime
...
```bash
/home/cluster/launch-metrale-ep4.sh          # orchestrates ranks 0..3 across the 4 nodes
```
...
- vLLM and Metrale Engine are mutually exclusive on the cluster (both reserve unified memory).

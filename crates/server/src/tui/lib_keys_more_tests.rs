...
//! 2026-09-26: Edge-case tests for `lib_keys.rs`: arrow aliases, the filter box, unbound keys and pane-specific letters.
//!
//! Owner: server tui.
...
fn leaving_the_form_clears_the_error_it_was_showing() {
    ...
    s.on_key(key(KeyCode::Enter));
    s.on_key(key(KeyCode::Enter));
    // 2026-09-26: A free-text row; enumerated rows open a picker that offers only valid values.
    s.row = s
        .config_rows()
        .iter()
...
fn cancelling_an_edit_keeps_an_error_the_previous_commit_earned() {
    ...
    // 2026-09-26: A free-text row; the first row (`enable_prefix_caching`) is a boolean with a picker.
    s.row = s
        .config_rows()
        .iter()
        .position(|r| r.key == "port")
...
#[test]
fn enter_on_a_non_metrale_card_explains_instead_of_opening_the_form() {
    let mut r = recipe();

...
# 2026-09-26: Edit-flow probe: seed a crate with three todo!() stubs, have each client edit them, then cargo test.
#
...
mechanics / Metrale Engine tool path (Edit old_string/new_string arg handling, multi-turn
read→edit→verify, tool-arg corruption) rather than model coding competence.

Runs BOTH clients against the live Metrale Engine at :8888, then `cargo test`. Writes
...
def kill_opencode():
    # 2026-09-26: Kills every process named exactly opencode; run before and after
    # each opencode run. -x matches the process name, not the command line, so this
...
def still_has_todo(target):
    # 2026-09-26: Skip // lines: the crate doc in LIB_RS mentions todo!() too.
    try:

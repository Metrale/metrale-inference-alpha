...
//!
//! Metrale Engine compiles ONE SM architecture per build — `kernels/<hw>/HARDWARE.toml`
...
/// Metrale Engine ships nothing for that GPU, and is the honest answer for every CC not
...
/// kernels tree — metrale-core is a leaf crate with no build script and no TOML
...
/// `metrale-kernels/tests/target_hints.rs` closes it: it reads every

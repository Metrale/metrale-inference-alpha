...
/// `METRALE_NO_DECODE_GRAPHS_MULTISEQ=1`.
///
/// Strict `== "1"` on an `METRALE_NO_*` name rather than a presence check —
...
impl TransformerModel {
    pub(super) fn decode_batch_dispatch(
        ...
        // `METRALE_MLA_PERSEQ_FALLBACK` escape hatch, as a guarded safety net
        ...
        //   * METRALE_HC_PERSEQ_DECODE=1 (A/B escape hatch), or
    ...
    pub(crate) fn decode_batch_compute_main(
        ...
        // CUDA graphs for multi-sequence decode (METRALE_DECODE_GRAPHS_MULTISEQ=1).
        //
        ...
        // METRALE_NO_DECODE_GRAPHS_MULTISEQ=1. Measurements + the rewrite this
        // retired: `decode_graph_key.rs`.
        let ms_profile = self.levers.ms_profile;
        // METRALE_MS_PROFILE forces eager (graphs off) so per-phase syncs are legal.
        // METRALE_LORA_EAGER: same LoRA graph-vs-eager debugging hatch as decode_a.

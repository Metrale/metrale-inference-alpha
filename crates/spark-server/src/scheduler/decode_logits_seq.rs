...
// `METRALE_FORCE_TEMP_ZERO` is now `SchedLevers::force_temp_zero`, carried on
...
pub fn process_seq_logits(
    ...
    // be compared against the post-pipeline METRALE_LOGIT_DUMP view.
    ...
    // Run the unified per-position pipeline: METRALE_FORCE_TEMP_ZERO bypass →

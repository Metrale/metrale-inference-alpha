...
fn adding_a_changed_file_never_removes_a_required_gate() {
    let base = ["crates/metrale-plugin/src/benchmarks/bfcl/report.rs"];
    let additions = [
        "kernels/gb10/common/paged_decode_attn_fp8.cu",
        "crates/spark-model/src/layers/ops/fp8_moe.rs",
        "Cargo.lock",
        "3rdparty_patches/gdn_aot/libmetralegdn.so",
...
/// `METRALE_GDN_LIB`, and a committed recipe fixture points that at
/// `3rdparty_patches/gdn_aot/libmetralegdn.so` on a config claiming +17-20% on
...
fn replacing_the_aot_gdn_library_invalidates_every_gate() {
    let hit = coverage::invalidated_by(["3rdparty_patches/gdn_aot/libmetralegdn.so"]);
    assert_eq!(
        hit, REQUIRED_GATES,
...
/// `crates/metrale-plugin/src/gate`, which contains `coverage.rs`. That overlap
...
fn a_driver_change_invalidates_only_its_own_gate() {
    let hit = coverage::invalidated_by(["crates/metrale-plugin/src/benchmarks/bfcl/report.rs"]);
...
fn the_flat_concurrency_driver_invalidates_only_its_own_gate() {
    ...
    for path in [
        "crates/metrale-plugin/src/http.rs",
        "crates/metrale-plugin/src/benchmarks/stats.rs",
        "crates/metrale-plugin/src/benchmarks/transcript.rs",
        "crates/metrale-plugin/src/benchmarks/concurrency.rs.bak",
...
fn a_change_to_the_equality_driver_reopens_that_gate_and_no_other() {
    let hit =
        coverage::invalidated_by(["crates/metrale-plugin/src/benchmarks/kat_equality/compare.rs"]);
    assert_eq!(
        hit,
        ["kat-equality-gate"],

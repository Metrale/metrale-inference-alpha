...
//! 2026-09-26: Tests of the coherence probe's decisions without a server; the
//! socket-level tests are in `tests/coherence.rs`.
//!
//! Owner: bench.
...
/// 2026-09-26: `judge` on the text alone, as `ask` calls it.
...
fn truncate_counts_characters_not_bytes() {
    // 2026-09-26: `é` is two bytes, so slicing by bytes would panic here.
    let s = "é".repeat(200);
...
fn a_wrong_model_name_is_reported_ahead_of_the_answers() {
    ...
    assert_eq!(
        ...
        "http://127.0.0.1:8888 is serving nvidia/Qwen3.6-27B-NVFP4 — not \"does/not-exist\", \
         which this benchmark is set to request. Metrale Engine answers whatever model name it is sent, \
         so the run WILL produce numbers; they will just be for a different model than the one named."
    );
    assert!(!report.is_clean());
}

/// 2026-09-26: The agentic gate accepts its three BENCH.toml subjects: the
/// 35B MoE and both dense 27Bs, which are non-default variants
/// (`status = "unmeasured"` in kernels/gb10/qwen3.6-27b/BENCH.toml,
/// `status = "measured"` in kernels/gb10/qwen3.8-27b/BENCH.toml). Any other
/// family is reported.
#[test]
...
fn a_transport_error_carrying_a_hint_is_not_cut_off_mid_clause() {
    // 2026-09-26: A message under the 280-character bound comes back whole,
    // including the instruction at its end.
...
fn a_thinking_model_that_reasons_to_the_answer_passes() {
    // 2026-09-26: The fact is only in the reasoning and the text is empty.

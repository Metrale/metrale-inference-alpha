...
When the user runs `met serve <model-id>`, `spark-server/src/main.rs` does the following, roughly in order:

1. **Parse the model config.** `metrale_core::config::ModelConfig::from_hf(&model_path)` reads `config.json` and its nested text/vision configs.
...
6. **Load weights.** The loader translates HF weight names (`model.layers.0.self_attn.q_proj.weight`) into Metrale Engine layer types (`Qwen3AttentionLayer`), going through `WeightStore` (the `O_DIRECT` fast path) and the quantization helpers in `spark_model::weight_map`.
...
| Question | File |
|---|---|
| "How is `KernelTarget` resolved at startup?" | `crates/metrale-kernels/src/lib.rs`, look for `select_target()` + `include!(target_ptx.rs)` |
| "How does a kernel get compiled at build time?" | `crates/metrale-kernels/build.rs`, `crates/metrale-core/src/compute.rs` |
...
The [spark-runtime chapter](../crates/spark-runtime.md) expands on `GpuBackend`; [spark-model](../crates/spark-model.md) on the layer/factory split; [metrale-kernels](../crates/metrale-kernels.md) on the build-time codegen. The [SBIO chapter](./sbio.md) explains why every arrow in the diagram above goes through a trait.

...
fn not_selected_when_the_kill_switch_is_set() {
    // METRALE_FFN_W8A16_ONLY — injected, not read from the environment: the
...
fn not_selected_when_the_blockscaled_prefill_lever_is_off() {
    // METRALE_FP8_SINGLE_SCALE clears `dispatch.fp8_blockscaled_prefill`; it
...
/// pinned as data by `metrale-kernels/tests/target_defaults.rs`.
#[test]
...
fn cublas_arm_requires_a_multiple_of_four_weight_scale_column_stride() {
    ...
    // layouts"), and Metrale Engine hands over the checkpoint's `[N/128, K/128]` grid

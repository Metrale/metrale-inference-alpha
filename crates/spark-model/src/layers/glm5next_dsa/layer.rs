...
/// ON by default; `METRALE_DSA_SELECT_ROWS=0` is the kill-switch back to per-row selection.
///
...
/// which is false under `METRALE_GLM_VERIFY_GRAPHS=0`, under high-speed swap, and under
/// `METRALE_LORA_EAGER`. **Both eager and graphed verification keep the original per-row
/// path.**
///
...
///   `METRALE_DSA_SELECT_ROWS=0`, which is what keeps that arm's heap layout byte for byte.
...
///   `use_graphs`, false under `METRALE_GLM_VERIFY_GRAPHS=0`, high-speed swap, or
///   `METRALE_LORA_EAGER`).
...
pub struct Glm5NextDsaWorkspace {
    ...
    /// by the batched prefill selector. NULL when `METRALE_DSA_SELECT_ROWS=0` — see
...
impl Glm5NextDsaWorkspace {
    ...
    pub fn new(gpu: &dyn GpuBackend, cfg: &Glm5NextDsaConfig, max_rows: usize) -> Result<Self> {
        ...
        Ok(Self {
            ...
            //
            // 🔴 Allocated ONLY when `METRALE_GLM_DSA_PERSIST_BT=1`. Not a micro-optimisation:
...
pub struct Glm5NextDsaLayer {
    ...
    /// token. ON by default; `METRALE_GLM_DSA_ALLOC_PER_STEP=1` restores the old path.
    pub persist_bt: bool,
}
...
impl Glm5NextDsaLayer {
    ...
    /// [`Self::attend_rows`]. On by default; disabled by `METRALE_DSA_SELECT_ROWS=0`.
    ///
    ...
    fn select_rows_batched(
        ...
        // replaces: "no FP32-out batchm twin exists". `q_idx_rows` is FP32, and Metrale Engine's
    ...
    pub fn decode_k(
        ...
        // it from `use_graphs`, which is false under `METRALE_GLM_VERIFY_GRAPHS=0`, under
        // high-speed swap, and under `METRALE_LORA_EAGER`. See `select_rows_batched`.
        is_prefill: bool,
    ) -> Result<()> {
        use crate::layers::glm5next_layer::profile;
        ...
        // `METRALE_GLM_VERIFY_GRAPHS=0`, high-speed swap is engaged, or `METRALE_LORA_EAGER` is
        ...
        //
        // The buffers are NULL under `METRALE_DSA_SELECT_ROWS=0`, so this is false on that arm
        ...
        for row in 0..k {
            ...
            let (d_bt, d_sl) = match (bt_dev_meta, sl_dev_meta) {
                ...
                _ => {
                    ...
                    // for the alloc/copy/free cluster). Kill switch `METRALE_GLM_DSA_ALLOC_PER_STEP=1`.
                    //

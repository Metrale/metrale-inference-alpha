...
/// `METRALE_DS41_PREFILL_GEMV=1`, read once: prefill groups of more than eight
...
impl MoeV41 {
    ...
    pub fn forward<S: ExpertSource + ?Sized>(
        ...
        // METRALE_DS41_M1_LOOP=1 (diagnostic): run the single token through the
        ...
        // METRALE_DS41_DEVICE_ROUTE=1: the whole selection, the plan and the
        ...
        // METRALE_DS41_PREFILL_GEMV=1: every group through the decode GEMV arm in
        ...
        if self.timing_sync {
            // METRALE_DS41_DIAG=1: make compute_ms the GPU time, not the launch time

...
//! 2026-09-25: GPU tests comparing the predictor's `project_q`,
//! `project_kv_block` and `score_blocks` with the Rust reference
//! (`predictor_ref`) on random inputs. The projections pass within 5e-2
//! absolute; the scores within 5e-2 relative to the largest reference score.
//!
//! Owner: storage (tests).
...
use metrale_storage::predictor::{Predictor, PredictorDims, read_k_lr_slot};
use metrale_storage::predictor_ref::{predictor_score_ref, project_kv_block_ref, project_q_ref};
use metrale_storage::projection::{PredictorShape, build_projection};
...
fn kv_lowrank_project_parity() {
    ...
    .unwrap();
    pred.project_kv_block(&ctx, /* 2026-09-25: slot read below */ 1, 3, k_dev.ptr)
        .unwrap();

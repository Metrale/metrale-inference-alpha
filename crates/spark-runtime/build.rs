...
fn main() {
    ...
    // Register the `metrale_scale` cfg so `#[cfg(metrale_scale)]` does not trip
    // the `unexpected_cfgs` lint. `metrale_scale` selects SCALE/AMD (gfx1151)
    ...
    // `cuGraphInstantiateWithFlags`). Driven by the same `METRALE_TARGET_HW`
    // signal the metrale-kernels build uses; covers both the SCALE (`strix`)
    ...
    if matches!(
        ...
        // Even under METRALE_SKIP_BUILD (CI no-GPU test build, no nvcc), the
    ...
    // MetraleCudaBackend is compiled in). The metal feature build on
    // Apple Silicon must not request -lcuda.
    if std::env::var_os("CARGO_FEATURE_CUDA").is_none() {
        return;
    }

    // Link libcuda for MetraleCudaBackend's raw CUDA driver API calls.
    // The actual CUDA driver is a stub at compile time; at runtime
    // it resolves to the NVIDIA driver installed on the system.
    println!("cargo:rustc-link-lib=dylib=cuda");
    // cuBLASLt for the high-efficiency GEMM path (METRALE_CUBLAS_GEMM=1). The
    ...
    // symbol). Previously only emitted in the METRALE_SKIP_BUILD stub path and
    ...
    // kernel object, and the binary serves entirely on Metrale Engine's own kernels.
    // Setting them only makes the opponent available behind its runtime
    // opt-in (`METRALE_CUTLASS_GEMM=1` / `METRALE_FLASHINFER_PREFILL=1`), which
...
/// `metrale-kernels/build.rs` compiles Metrale Engine's own kernels from. Hard-coding
...
/// GPUs' code. `METRALE_CUDA_ARCH` still wins when set explicitly.
///

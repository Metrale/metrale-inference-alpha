...
// Opt-in fast path: `METRALE_FAST_SOFTMAX_EXP` — the original FA4-style
...
// METRALE_ATTN_LDMATRIX: use ldmatrix.x4 for the A-operand (Q for QK^T, P for PV)
...
// -DMETRALE_DISABLE_ATTN_LDMATRIX (the prior default-off) if a regression appears.
...
extern "C" __global__ void KERNEL_NAME(
    ...
    __shared__ __nv_fp8_storage_t smem_K[METRALE_KBUFN][BC][HDIM_PAD];  // double-buffered
    __shared__ __nv_fp8_storage_t smem_V[BC][HDIM_PAD];
#else
    __shared__ __nv_bfloat16 smem_K[METRALE_KBUFN][BC][HDIM_PAD];  // double-buffered (single under SCALE)
    ...
    // Bisect: `METRALE_DISABLE_FP16_PV` reverts the Phase 2c FP16 P×V path
    ...
    for (unsigned int kv_block = kv_block_lo; kv_block < num_kv_blocks; kv_block++) {
        ...
        // Bisect: METRALE_DISABLE_FP16_PV reverts to the pre-Phase-2b BF16
...
// is clamped to match by cfg!(metrale_scale) in ops/prefill_attn_main_a.rs

...
# Multi-DGX-Spark deployments MUST use pure EP (tp-size 1, ep-size 2).
#
# Usage:
...
#   - metrale-deepseek-v4:latest Docker image on both nodes
#     Build: docker build -f docker/gb10/deepseek-v4-flash/nvfp4/Dockerfile -t metrale-deepseek-v4 .
...
IMAGE="${IMAGE:-metrale-deepseek-v4:latest}"
...
echo "=== Metrale Engine DeepSeek-V4-Flash EP=2 bring-up (RDMA-enabled) ==="
...
sudo docker rm -f metrale-ds-ep0 2>/dev/null || true
ssh "$WORKER_IP" "sudo docker rm -f metrale-ds-ep1 2>/dev/null || true"
...
sudo docker run -d \
  ...
  $NCCL_ENV \
  -e RUST_LOG=info \
  -e METRALE_DIAG_V4_ALL_LAYERS=${METRALE_DIAG:-0} \
  -e METRALE_EP_GRAPHS=${METRALE_EP_GRAPHS:-0} \
  $VOL_HEAD \
  "$IMAGE" serve \
...
echo "Monitor rank 0: sudo docker logs -f metrale-ds-ep0"
echo "Monitor rank 1: ssh $WORKER_IP 'sudo docker logs -f metrale-ds-ep1'"

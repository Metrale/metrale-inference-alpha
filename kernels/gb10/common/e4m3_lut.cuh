...
// 2026-09-25: Shared FP8 E4M3 -> FP32 lookup table E4M3_LUT, indexed by the raw byte.
//
// Owner: gb10 kernels.
// Invariants:
// - OCP E4M3FN: 1 sign bit, 4 exponent bits (bias 7), 3 mantissa bits, no infinities.
//   The two NaN codes 0x7F and 0xFF decode to +0 and -0.
// - The W8A16 kernels include this header (gb10 w8a16_gemm*.cu and the hopper W8A16
//   kernels), so the table is written once in source.
// - Each including translation unit gets its own __constant__ E4M3_LUT; each kernel file
//   is compiled to its own module, so the copies never meet at link time.

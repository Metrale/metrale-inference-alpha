...
/// `std::env::var("METRALE_GDN_PREFILL_TC").is_ok()` at this layer would break,
/// and which nothing else in the suite would notice:
///
///  * `METRALE_GDN_PREFILL_TC=0` is an explicit OFF under the 2026-09-11
...
/// Which value the TOML holds is `metrale-kernels`' own `target_defaults.rs`.
#[test]
...
/// `[defaults] gdn_prefill_tc` now true on Hopper, `METRALE_GDN_PREFILL_TC=0` is

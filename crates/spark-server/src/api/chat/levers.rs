...
pub struct ChatLevers {
    ...
    /// `METRALE_BASH_WANDER_WATCHDOG=1` — append the BW1 steering hint when the
    /// agent has made many tool calls with no productive file output.
    /// Default-off (PCND).
    pub bash_wander: bool,
    /// `METRALE_CHAT_PHASE_TIMING=1` — emit per-phase `CHAT_PHASE` timing lines
    ...
    /// `METRALE_INTHINK_TOOL_LEAK_OPENERS=N` — how many tool-call openers
...
/// SSOT parse for `METRALE_INTHINK_TOOL_LEAK_OPENERS`. A set-but-unparseable

...
//! 2026-09-26: Envelope-aware sanitizing with the `MinimaxXmlParser` markers. Inside each
//! of its three `envelope_open` spellings the inner `<invoke …>` and `<parameter name=…>`
//! tags pass through as content; outside an envelope they are orphan fragments and are
//! dropped.
//!
//! Owner: server API.

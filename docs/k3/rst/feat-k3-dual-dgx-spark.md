# RST session — feat/k3-dual-dgx-spark (extracted)
...
AREAS
feat/k3-dual-dgx-spark
C7 (lab dual-DGX-Spark aviation MATCH + kill rank 1 timeout: umbrella `c7-k3-tp2.md`)

ORACLE
- `supports_tp() == true` — `--tp-size 2` must not fail-fast.
- Twin TP=2: q_proj column `[8*32, 1024]`, o_proj row `[1024, 8*32]`.
- Umbrella lab: two DGX Spark hosts (rank 0 + rank 1) aviation 16 tokens MATCH TP=1; kill rank 1 → TimeoutError, not bee-fly.
...
TEST NOTES
`cargo test -p metrale-model-layers --lib -- kimi_k3::tp` (Linux). Metrale-core plan tests run wherever metrale-model-layers compiles.

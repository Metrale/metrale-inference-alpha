...
//! 2026-09-25: `moe_v41_route_select` against `route_from_logits`: picks,
//! weight bits, plan weights, plan slots, pointer table and miss flag, over 400
//! tokens of random logits and a slot table with every fifth expert absent.
//! GPU only (`#[ignore]`).
//!
...
fn device_route_select_matches_the_host_chain_bitwise() {
    ...
    // 2026-09-25: Slot table row for layer 3: every fifth expert absent.
    let layer = 3u32;
    ...
    for t in 0..400 {
        let mut logits: Vec<f32> = (0..NR)
            ...
            .collect();
        if t % 10 == 0 {
            // 2026-09-25: Every tenth token makes `logit + bias` equal for
            // experts 5 and 6. The ranking key is
            // `sqrt(softplus(logit)) + bias`, so this does not tie the key.

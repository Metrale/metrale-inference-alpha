...
//!
//! Slice 2 down-graded mHC to REUSE because Metrale Engine's `hc_mult = 4` and `hc_sinkhorn_iters = 20`
//! equal GLM's. That is a **config** match. This runs Metrale Engine's real `hc_pre` / `hc_post` CUDA
...
//! `base`/`scale` (180 tensors); `fn` is the other 90. Metrale Engine's kernel takes an `f32*`, so the
...
//!
//!   MHC_PACKET_DIR=/home/msi1/metrale-scratch/mhc-family \

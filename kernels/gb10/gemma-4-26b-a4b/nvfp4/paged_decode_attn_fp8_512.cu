...
// 2026-09-25: HDIM=512 paged decode attention over an E4M3 KV cache, BF16 Q and O. Owner: gb10 kernels (gemma-4-26b-a4b, and gemma-4-31b through `[sources] use`). Invariants: none checked; head_dim must equal HDIM and num_kv_heads must divide num_q_heads.
#undef HDIM
#define HDIM 512
...
// 2026-09-25: One 256-thread block per (q_head, seq) (ops::paged_decode_attn_fp8), looked up as
// paged_decode_attn_fp8_512::paged_decode_attn_fp8 and launched for an FP8 KV cache when head_dim > 256 and the
// split-K route is not taken. K and V are multiplied by k_scale and v_scale. No sliding window.
...
extern "C" __global__ void paged_decode_attn_fp8(
    ...
    // 2026-09-25: Each lane holds VEC_BF16 (16) consecutive dims: 16 BF16 of Q and O, 16 E4M3 bytes of K and V.
    ...
    // 2026-09-25: Physical blocks are cache_stride elements apart; within a block, positions are
    // num_kv_heads * head_dim apart.
    ...
    while (pos < my_end) {
        ...
        // 2026-09-25: Positions that share a physical block are read BC at a time; the tail of a block one at a time.
...
// 2026-09-25: Split-K form over [0, seq_len): block (q_head, split, seq) writes unnormalised O, m and l to
// workspace[seq][head][split][head_dim + 2]; paged_decode_attn_reduce_fp8, written for one warp per (q_head, seq),

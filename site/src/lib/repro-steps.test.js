...
const sweep = () => ({
  ...
  metrale_version: '1.0.0-beta-preview',
  ...
  perf_env: { METRALE_PREFILL_CODISPATCH: '0' },
...
describe('shell safety', () => {
  ...
  it('renders a bare boolean flag on its own line', () => {
    const argv = ['met', 'benchmark', 'run', 'agentic-webserver', '--param', 'iterations=10', '--yes', '--pull-request-gate'];
...
describe('a partition aggregate reproduces as N shard commands', () => {
  ...
  it('lists every member command by shard and the real aggregate subcommand', () => {
    ...
    const runs = cmds.filter((c) => c.lines[0].startsWith('met benchmark run'));
    ...
    expect(cmds[cmds.length - 1].lines).toEqual(['met benchmark aggregate bfcl-subset --sha a87b41905f']);
  ...
  it('never presents the inherited single command as the run', () => {
    ...
    const runs = measureOf(plan).commands.filter((c) => c.lines[0].startsWith('met benchmark run'));
    ...
    expect(singleRuns.filter((c) => c.lines[0].startsWith('met benchmark run'))).toHaveLength(1);
    expect(singleRuns[0].label).toBe('the recorded run');
    expect(singleRuns.some((c) => c.lines[0].startsWith('met benchmark aggregate'))).toBe(false);
  });
...
describe('a lone shard record', () => {
  it('derives the sibling commands, marks them derived, and adds the aggregate', () => {
    ...
    expect(cmds[cmds.length - 1].lines[0]).toBe('met benchmark aggregate bfcl-subset --sha a87b41905f');
...
describe('a missing field is named, never filled in', () => {
  it('a record without a command shows no run command anywhere', () => {
    ...
    expect(cmdLines.some((l) => l.startsWith('met benchmark run'))).toBe(false);
    ...
    expect(full.script).toMatch(/met benchmark run concurrency-sweep/);
  });
...
describe('derived flags are appended and reported', () => {
  const ttft = () => ({
    ...
    params: { repeats: '12' },
    serve_overrides: {},
    command: ['met', 'benchmark', 'run', 'ttft-cold-gate', '--param', 'repeats=12', '--pull-request-gate']
  });
...
describe('caveats', () => {
  ...
  it('an unsigned record and an invalidated box state are said, a valid one is not', () => {
    ...
    expect(ok.steps.find((s) => s.id === 'record').facts).toContainEqual(['signer', 'https://github.com/Metrale/metrale-inference-alpha/blob/main/.github/record-signers/02156264cbf75bd7.pub']);
  });
...
describe('the plan shape', () => {
  it('has the eight pipeline steps in execution order and real env exports', () => {
    ...
    expect(linesOf(plan)).toContain('export METRALE_PREFILL_CODISPATCH=0');
    ...
    expect(plan.steps.find((s) => s.id === 'serve').facts[0][1]).toBe('https://github.com/Metrale/metrale-recipes/blob/main/recipes/qwen3.8/qwen3.8-27b-nvfp4-unsloth.yaml');
  });

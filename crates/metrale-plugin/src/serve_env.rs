...
//! The `METRALE_*` environment a gate's server is measured under.
//!
//! A serve LEVER is any `METRALE_*` variable the server reads: `METRALE_FP8_ROWWISE`
...
/// `METRALE_*` names that are NOT levers: they place, log or build, and the
...
///
/// Closed on purpose. A name that is not here is a lever, so a new `METRALE_*`
...
pub const HARNESS_VARS: &[&str] = &[
    ...
    // live, not what is measured. The node agent exports it as `METRALE_HOME`.
    "METRALE_HOME",
    // Where the TUI tees its log (`tui::init`).
    "METRALE_TUI_LOG_FILE",
    // Build-script inputs (`metrale-kernels/build.rs`, `spark-model/build.rs`,
...
pub fn declared(owner: &str, raw: &BTreeMap<String, String>) -> Result<BTreeMap<String, String>> {
    ...
    for (name, value) in raw {
        ...
        if HARNESS_VARS.contains(&name.as_str()) {
            bail!(

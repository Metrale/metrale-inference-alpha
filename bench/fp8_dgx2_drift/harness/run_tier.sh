#!/usr/bin/env bash
# Run N opencode probes sequentially against the currently-running Metrale Engine
# container (and optionally a remote vLLM via SSH tunnel) and score each one.
# Intended for statistical comparison of Metrale Engine drift-mitigation tiers
...
REMOTE_ONLY=0
BAIL=0
# --claude-code: drive Claude Code (the `claude` CLI) against Metrale Engine instead of
...
# uses (METRALE_WARM_TARGET_DIR, SSOT with warm_cargo_cache.sh + score_run.py).
...
# the slow runs (260-360s) were dominated by a Metrale-side FP8 deep-context
...
# Set METRALE_OPENCODE_OUTPUT_CAP to a lower value only for such experiments.
...
run_one() {
  ...
  : > "${METRALE_LOG}"  # empty by default; populated below if local
  ...
  # METRALE_HARNESS_PORT is exposed both to opencode (so the model can read it
  ...
  if [[ "${CLAUDE_CODE}" == "1" ]]; then
    # Drive Claude Code against Metrale Engine. Runs as user `claude` with its real
    ...
    # routes to Metrale Engine; cwd is the target dir (claude has no --dir flag). Default
  ...
  # run's target dir, so we never touch the Metrale Engine container or anything else.
  ...
  # Metrale Engine log window for THIS run only (local only).
  if [[ "${label}" == "local" ]]; then
  ...
  # captured assistant text (degeneration fingerprint) and (b) Metrale-side

...
# 2026-09-26: Edit-flow probe, Claude Code only: a buried stub in a file of about 1000 lines, and one large single-response write.
#
# Owner: bench, FP8 drift harness.
# Invariants: none beyond the types.
"""Edit-flow probe v3 — push the two paths most likely to surface a real Metrale Engine
...
Trivial coding on purpose -> failures point at Metrale Engine mechanics, not competence.
"""
...
def envelope_cuts():
    rc, out, err = run(["bash", "-lc",
                        "sudo docker logs --since 12m metrale-camp 2>&1 | "
                        "grep -c 'Stuck in tool-call ENVELOPE'"])
    return out.strip()


# 2026-09-26: bigedit: 200 functions in one lib.rs; scale_123 is the only todo!() stub.
def seed_bigedit(t):
    N = 200
    STUB = 123
...
def check_bigedit(t, stub):
    ...
    # 2026-09-26: The functions on either side of the stub must be intact.
...
# 2026-09-26: bigwrite: one RPN calculator lib.rs requested in a single response.
def seed_bigwrite(t):

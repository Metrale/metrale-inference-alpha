# Metrale Engine Kernel Registry
...
| Shape (M×N×K) | Metrale Engine TC | cuBLAS | Notes |
|---------------|----------|--------|-------|
...
| Operation | Metrale Engine W4A16 | cuBLAS per-expert (BF16) | Speedup |
|-----------|------------|--------------------------|---------|
| Gate-up: 800×1024×2048 | **5.58ms** | 7.14ms | **1.28×** |
| Down: 800×2048×512 | **2.83ms** | — | — |
| Full pipeline | **8.39ms** | — | — |

**Metrale Engine beats cuBLAS** for MoE workload because:
...
5. **Global MetraleRegistry (OnceLock)**: All PTX modules, CUDA context, and stream cached in a singleton. First call loads everything (~168ms), subsequent calls are instant.
...
| 2026-02-23 | MetraleRegistry: OnceLock singleton | **2.2-7.8× speedup** across all kernel calls |

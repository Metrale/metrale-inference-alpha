// Metrale <-> FlashInfer GDN bridge: wraps the static-inline C-ABI header into real
...
// S[v][k]; Metrale's decode kernel reads S[k][v]. Stream-ordered after the FI kernel.
extern "C" void metrale_transpose_heads(float* S, int nheads, int N, void* stream);

static gdn_holo_0_Kernel_Module_t g_module;
static int g_loaded = 0;

extern "C" void metrale_gdn_load() {
...
extern "C" int metrale_gdn_prefill(
...
// Metrale-NATIVE entry: takes Metrale's packed QKV ([Q(key_dim)|K(key_dim)|V(value_dim)] bf16,
...
// conv_dim strides (no copy). This is what Metrale's prefill_gdn_full_inner will call directly.
static void* s_alpha=nullptr; static void* s_beta=nullptr; static size_t s_cap=0;
extern "C" int metrale_gdn_prefill_packed(
    ...
    int conv_dim, int gb_stride, int num_seqs, void* stream)
{
  ...
  // FI wrote h_state (g_st) as S[v][k]; Metrale decode reads S[k][v]. Transpose last two
  // dims per head (num_seqs*nv heads of vd x vd). Same stream => runs after the FI kernel.
  metrale_transpose_heads((float*)h_state, num_seqs * nv, vd, st);
  return ret;
}
...
// (Metrale layout S[k][v], = carried state across chunked-prefill steps; zero on chunk 0) is
...
extern "C" int metrale_gdn_prefill_packed_managed(
    ...
    int conv_dim, int gb_stride, int num_seqs, void* stream)
{
  ...
  // init = incoming h_state transposed Metrale S[k][v] -> FI S[v][k] (chunk 0: h_state=0 -> 0).
  cudaMemcpyAsync(m_init, h_state, in, cudaMemcpyDeviceToDevice, st);
  metrale_transpose_heads((float*)m_init, num_seqs*nv, vd, st);
  ...
  return metrale_gdn_prefill_packed(qkv,gate_beta,output,h_state,m_init,m_tm,m_cu,

...
sudo docker rm -f metrale-slots-ab >/dev/null 2>&1; sleep 3
sudo docker run -d --name metrale-slots-ab --network host --gpus all --ipc=host \
...
for i in $(seq 1 180); do curl -sf -m4 http://localhost:8888/v1/models 2>/dev/null | grep -q Qwen && break
  sudo docker ps --format '{{.Names}}' | grep -q metrale-slots-ab || { echo "SERVE_DIED slots=$SLOTS"; exit 1; }; sleep 5; done
...
sudo docker logs metrale-slots-ab 2>&1 \
...
sudo docker rm -f metrale-slots-ab >/dev/null 2>&1

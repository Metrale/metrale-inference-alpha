...
//! GGUF tensor-name → Metrale Engine HF tensor-name translation.
//!
//! GGUF names decoder weights as `blk.N.<sub>` plus a handful of top-level
//! tensors (`token_embd`, `output_norm`, `output`). Metrale Engine per-arch loaders ask
...
//! single `[n_expert, …]` tensor that Metrale Engine expects as `num_experts` separate
...
pub enum GgufName {
    ...
    /// Tensor carries no learnable weight for Metrale Engine (e.g. precomputed rope
    /// frequencies); the loader should skip it.
    Drop,
}

/// The HF weight prefix Metrale Engine defaults to when `weight_prefix` is empty
...
/// Translate a GGUF tensor name to its Metrale Engine HF equivalent for architecture
...
/// runtime (`crate`-external `metrale_model_engine::model::nllb`) indexes weights by
...
/// HF prefix Metrale Engine's Qwen3.6 ViT tower loads its tensors under. See
...
/// `model.visual.*` HF name Metrale Engine's Qwen3.6 vision encoder expects.
///
...
///
/// GDN → Metrale Engine HF mapping (`model.layers.N.linear_attn.*`, consumed by
...
fn translate_default(gguf_name: &str) -> Option<GgufName> {
    ...
    match gguf_name {
        ...
        // Precomputed rope frequency table — Metrale Engine builds rope itself.
...
/// keep-packed Q2_0 decode path (`METRALE_GGUF_NATIVE_Q2=1`) can serve without

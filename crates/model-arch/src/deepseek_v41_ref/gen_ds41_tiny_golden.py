...
# 2026-09-25: Generate ds41_tiny_golden.json by running the reference model on
# the tiny fixture with capture hooks. Needs the reference checkout at REF.
#
...
and emits every intermediate a Metrale Engine implementation must reproduce. Nothing here re-derives an
...
Run from the workspace root:   python3 crates/model-arch/src/deepseek_v41_ref/gen_ds41_tiny_golden.py
"""
import functools
import json
import math
...
# 2026-09-25: Toy sizes. head_dim, index_head_dim and engram_head_dim are
# multiples of 32, the quantisation block.
FIX = dict(
    ...
    # 2026-09-25: The candidate source (layer 4) and its consumer (index source
    # 5) share compress ratio 1: a source and consumer of different ratios would
    # give the candidate mask and the scores different widths.
    ...
    # 2026-09-25: Each layer's row count is the sum of its bucket primes
    # (asserted in main).
...
PREFILL = 12  # 2026-09-25: > window_size (8) and 12 % 8 != 0, so the ring's cutoff branch runs.
B = 1
...
# 2026-09-25: Captures a sub-module test needs as its input are stored at full
# resolution (stride 1), so the sub-module can be checked without the layers
# that would otherwise produce that input.
FULL_CAPTURES = {
    "L1.engram_in", "L4.engram_in",
    # 2026-09-25: Layer 0 (no engram, ratio 0): every input the block scaffold,
    # the MoE and the ratio-0 attention need, so each can be tested on its own.
...
def emit_tensor(t: torch.Tensor, is_int: bool, name: str = "") -> dict:
    ...
    if is_int:
        ...
        # 2026-09-25: json.dump writes floats as repr(), the shortest round-trip
        # form, so exact bf16 values survive.
...
def install_hooks():
    def block_forward(orig, self, x, start_pos, pre_mix, image_mask, *attn_args):
        ...
        # 2026-09-25: The shared slots change only after a source layer writes
        # them; caches are recorded over their used prefix so an off-by-one
        # cannot hide behind zero-filled tail rows.
    ...
    def engram_forward(orig, self, x, hash_ids, token_mask=None):
        # 2026-09-25: Label by the engram's own layer_id: the engram runs before
        # its block's forward, so CUR["layer"] still names the previous block.
        lid = self.layer_id
...
def main():
    ...
    # 2026-09-25: The tiny vocab has no tokenizer: an identity compressed map of
    # the same size.
    V = FIX["vocab_size"]
    ...
    # 2026-09-25: The final norm's input is the collapsed stream (`h_final`).
    model.norm.register_forward_hook(lambda m, i, o: rec("h_final", i[0]))
    # 2026-09-25: Mid-chain engram captures, so a mismatch localises to hash /
    # table / projection / gate.
    ...
    out = {
        "fixture": {
            **{k: (list(v) if isinstance(v, tuple) else v) for k, v in FIX.items()},
            ...
            # 2026-09-25: The hash tables are shipped so the Rust engram consumes
            # given tables, as the loader takes them from the GGUF metadata.
            "engram": {
                ...
                "multipliers": model.engram_hash.multipliers.tolist(),
                "primes": model.engram_hash.primes.tolist(),
                "offsets": model.engram_hash.offsets.tolist(),
                # 2026-09-25: Per-block e8m0 scales of each fp8 table, as floats
                # (powers of two). The Rust reference takes dequantised rows;
                # e8m0_rounding_rule_matches_torch pins the rounding rule.
                "scale_block": 32,

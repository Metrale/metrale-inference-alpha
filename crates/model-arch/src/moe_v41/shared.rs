...
//! 2026-09-25: The shared expert's projections (`shared_expert_body`). An
//! input whose q8_1 rows are already in the given scratch is not quantised
//! again, and when w2 is K-quant at `m <= 8` the SwiGLU writes `sh` and its
//! q8_1 rows in one launch (`kquant_swiglu_q8_1_rows_bf16`).
//!
...
impl MoeV41 {
    /// 2026-09-25: The shared expert up to its output `sd` (`[m, dim]` bf16),
    /// with the q8_1 scratch buffers given (the side stream passes `sh_q8` as
    /// its `h_q8`); `acc` is not touched. `x_pre`: `a_q8` already holds the
    /// q8_1 rows of `x`; only the K-quant arms at `m <= 8` read it.
    ...
    pub(super) fn shared_expert_body(
        ...
        let c = &self.cfg;
        // 2026-09-25: One projection: bf16 (GEMV at m == 1, tiled GEMM above)
        // or the GGUF's K-quant blocks on the routed experts' kernels (GEMV at
        // m <= 8, MMQ above). `ready`: the q8_1 rows of `a` are already in
        // `a_q8`, so the GEMV arms skip the quantisation.
        let kq = |a: DevicePtr,
                  a_q8: DevicePtr,
                  ready: bool,
                  wt: ResidentMat,
        ...
        // 2026-09-25: The K-quant w1 GEMV arm left the rows of `x` in `a_q8`.

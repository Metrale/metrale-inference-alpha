...
# docs/campaigns/gb10-decode-fold-2026-07/. It serves Metrale Engine with the frozen
...
#
# Required:
#   METRALE_BIN     path to a `met` built with METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b
...
#   IMAGE         serve container. Default metrale-gb10:followups.
...
METRALE_BIN="${METRALE_BIN:?path to the built met binary}"
...
ND="${ND:-3}"
TAG="${TAG:-golden}"
IMAGE="${IMAGE:-metrale-gb10:followups}"
PORT="${PORT:-8888}"
...
CONTAINER="metrale-${TAG}-e2e"

[ -x "$METRALE_BIN" ] || { echo "METRALE_BIN is not executable: $METRALE_BIN" >&2; exit 1; }
...
sleep 3
# Frozen c2final env. NOTE on METRALE_* presence-flags: for those, a value of 0 is
...
sudo docker run -d --name "$CONTAINER" --network host --gpus all --ipc=host \
  ...
  --tool-call-parser qwen3_xml --tool-grammar off --disable-thinking >/dev/null
...
echo "serve up (nd=${ND}, K=$((ND + 1)), slots=${SLOTS}, bin=${METRALE_BIN})"

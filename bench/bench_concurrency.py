...
# 2026-09-26: Concurrency and latency sweep: client TTFT/TPOT/E2E plus server usage stats.
#
# Owner: bench.
# Invariants: none beyond the types.
"""
bench_concurrency.py — Metrale Engine concurrency + latency benchmark.
    ...
    try:
        ...
        with urlopen(req, timeout=600) as resp:
            for raw_line in resp:
                ...
                # 2026-09-26: A chunk with usage carries the server-side stats; the
                # rest of that chunk is not read.
                ...
                if usage:
                    ...
                    # 2026-09-26: Prefer the server's completion count; the local
                ...
                # 2026-09-26: Any delta with a 'content' key counts as one token, even
                # an empty string.
                if "content" in delta:
...
def concurrent_bench(
    ...
    """
    # 2026-09-26: Warmup requests run one at a time and are not measured.
...
def _p(values: list[float], p: int, fmt: str = ".0f") -> str:
    ...
    return format(v, fmt) if v == v else "n/a"  # 2026-09-26: false only for NaN
...
def main() -> None:
    ...
    print("Metrale Engine — Concurrency Benchmark")

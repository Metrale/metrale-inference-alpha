...
//! Per-batch-width MTP acceptance telemetry (`METRALE_MTP_ACCEPT_DEBUG`).
//!
//! # Why this exists
//!
...
//! periodic `tracing::info!`. Still gated: presence of `METRALE_MTP_ACCEPT_DEBUG`.
...
///
/// Kill switch `METRALE_MTP_ACCEPT_FOLD_AT_16` (presence — house convention,
/// `=0` is NOT off) restores the pre-fix fold for an A/B.
const MAX_N: usize = 33;

/// PRESENCE check for `METRALE_MTP_ACCEPT_FOLD_AT_16`: restores the pre-fix
...
fn fold_at_16() -> bool {
    ...
    *ON.get_or_init(|| std::env::var_os("METRALE_MTP_ACCEPT_FOLD_AT_16").is_some())
}
...
/// ★ Accumulation is UNCONDITIONAL as of wave 28 — `METRALE_MTP_ACCEPT_DEBUG`

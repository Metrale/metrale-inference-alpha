...
const SERIES = [
  { id: 'metrale', label: 'Metrale Engine' },
...
describe('toggleSeries', () => {
  test('a press hides, a second press shows, order of the rest untouched', () => {
    ...
    expect(visibleOf(SERIES, off.hidden).map((s) => s.id)).toEqual(['metrale', 'vllm-nospec']);
  ...
  test('a refusal clears on the next accepted press', () => {
    const refused = toggleSeries(SERIES, ['metrale', 'vllm-mtp'], 'vllm-nospec');
    const shown = toggleSeries(SERIES, refused.hidden, 'metrale');

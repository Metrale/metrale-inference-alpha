#!/usr/bin/env python3
# 2026-09-26: Writes a BF16 copy of a block-scaled FP8 checkpoint: each FP8 weight times
# its block scale, cast to BF16, with the scales dropped and config.json from --ref-config.
#
# Owner: bench, FP8 dequant audit.
# Invariants: none beyond the types.
"""Dequant Qwen3.6-35B-A3B-FP8 to plain BF16 via canonical PyTorch ops.

Phase 2a of the Metrale Engine FP8 dequant audit. Produces a snapshot that HF
...
three-way cosine comparison:
    ...
    B: Metrale Engine[FP8]     vs  HF[unquantized BF16]    -> existing Phase alpha
    C: Metrale Engine[FP8]     vs  HF[FP8->BF16]           -> Metrale Engine compute fidelity
...
The dequant math is the same canonical formula that Metrale Engine's
quant_helpers.rs uses, with PyTorch's RNE in place of Metrale Engine's truncation.
"""
...
# 2026-09-26: The local Qwen3.6-35B-A3B-FP8 config.json has
...
def dequant_one_tensor(
    ...
    which uses RNE per IEEE-754. This is the canonical reference Metrale Engine's
    ...
    # 2026-09-26: Expand the [nb, kb] block scales to [N, K], cropping the last partial block.
...
def main() -> None:
    ...
    # 2026-09-26: config.json comes from --ref-config, not from the FP8 snapshot.
        ...
        with safe_open(str(shard), framework="pt") as f:
            ...
            for k in keys:
                ...
                if scale_name and scale_name in scale_keys and t.dtype == torch.float8_e4m3fn:
                    ...
                    # 2026-09-26: Copied as is. In the local Qwen3.6-35B-A3B-FP8 snapshot
                    # every tensor without a weight_scale_inv partner is BF16.
                    out_tensors[k] = t
                    total_passthru += 1

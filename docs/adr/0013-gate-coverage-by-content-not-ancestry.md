...
**Metrale Engine squash-merges.** A record is always written on a PR branch, against a
...
`metrale-plugin`'s coverage logic will read red on its own PR for the same

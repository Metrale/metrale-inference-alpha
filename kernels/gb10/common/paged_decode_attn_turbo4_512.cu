...
//
// The layout is NVFP4's (paged_decode_attn_nvfp4.cu) with another codebook. run_paged_decode.rs
// launches this HDIM=512 build when head_dim > 256.
//
...
#undef HDIM
// 2026-09-25: This file is always the HDIM=512 build. A target's -DHDIM (a KERNEL.toml
// extra_nvcc_flags entry) would otherwise reach it; the #undef drops it first.
...
// 2026-09-25: Decodes one lane's VEC_BF16 elements from one uint64 (HDIM=512), uint32 (256)
// or uint16 (128) of 4-bit indices, low nibble first, each times the group's FP8 scale.

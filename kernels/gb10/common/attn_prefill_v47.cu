...
// 2026-09-25: Kernel `attn_prefill_v47`: contiguous BF16 prefill flash attention for head_dim 256, 64 query rows per
// block on 128 threads (__launch_bounds__(128, 2)).
//
// Q is loaded once and moved into registers; then one smem array holds K in rows 0-31 and V in rows 32-63. Each KV
// step issues the V copy by cp.async before QK^T; after the first barrier (every K read done) it issues the next K,
// runs the softmax, waits for V only (wait_group 1) and, after the second barrier, runs P*V. Query blocks are visited
// in reverse (q_block = gridDim.y - 1 - blockIdx.y). No host code launches this kernel.
//
// Owner: gb10 kernels.
// Invariants: head_dim must equal HDIM (256): every tile row loads HDIM elements at head_dim strides.
...
#define BR 64
#define BC 32
// 2026-09-25: This kernel is head_dim 256 only. The #undef keeps out the -DHDIM=128 that some model KERNEL.toml files
// pass for the generic attention kernels, without the redefinition warning that the strict build (--Werror
// all-warnings) would turn into an error.

#undef HDIM
#define HDIM 256
#define PAD 8
#define HDIM_PAD (HDIM + PAD)
#define Q_CHUNKS (BR * (HDIM / 8))
#define KV_CHUNKS (BC * (HDIM / 8))

...
Concurrency is not optional here. A prefill lever that FAILS OPEN silently
falls back, and one such fallback (the since-removed FlashInfer GDN prefill)
cost 40-50% of prefill AT CONCURRENCY while C=1 could not see it. A C=1-only
number would report "the flag did nothing" and "the flag worked" identically.

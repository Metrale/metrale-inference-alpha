...
//! 2026-09-26: The Stats section: the tile row (requests, throughput, TTFT,
//! GPU), the TTFT histogram, the throughput chart, the sequences and memory
//! gauges, the speculation and cache panel, and the thermal panel.
//!
//! Owner: server tui.
...
pub fn draw(f: &mut Frame, app: &App, area: Rect) {
    ...
    // 2026-09-26: Thermal is a fixed six-row panel under speculation and cache,
    // so the bottom row keeps two columns.
...
fn tile(f: &mut Frame, area: Rect, title: &str, value: Line, sparkline: Option<&[u64]>) {
...
fn draw_tiles(f: &mut Frame, app: &App, area: Rect) {
    ...
    // 2026-09-26: `—`, not 0.0, when the GPU memory read failed.
    let gpu = if s.gpu_known {
        Line::from(vec![
            Span::styled(
                format!(" metrale {:.1} GB", s.metrale_used_gb),
                theme::text().add_modifier(Modifier::BOLD),
            ),
...
fn draw_ttft_hist(f: &mut Frame, app: &App, area: Rect) {
    ...
    // 2026-09-26: The buckets are cumulative, so each bar is the difference
    // from the previous one. The +Inf bucket is skipped, and buckets from 2.5 s
    // up are drawn in the warning colour.
...
fn draw_sequences(f: &mut Frame, app: &App, area: Rect) {
    ...
    let (active, prefill, swapped, queue) = s
        ...
        .unwrap_or_default();
    // 2026-09-26: The rows are placed by hand, not by a `Layout`. `row` returns
    // `None` at or below the pane's bottom, so a short pane skips rows instead
    // of drawing outside itself.
    ...
    if let Some(r) = row(y) {
        if s.gpu_known {
            ...
            // 2026-09-26: A 0 % bar would claim the GPU is empty; show a dash.
            f.render_widget(

...
# 2026-09-25: Write the mHC golden JSON (`OUT`): the reference `Glm5NextTextHyperConnection` and
# `Glm5NextTextHyperHead` run on the per-layer packets extract_mhc_packets.py writes, in FP32 and
# BF16, at the token counts in `REGIMES`.
#
# Owner: model-arch (GLM-5.3 offline reference scripts).
# Invariants: none beyond the code.
...
Why this exists: Slice 2 down-graded mHC to REUSE because Metrale Engine's `hc_mult`/`hc_sinkhorn_iters`
...
def run(m, streams, block_out, dtype):
    ...
    st["post"] = post[0].float().clone()
    st["comb"] = comb[0].float().clone()
    st["collapsed"] = collapsed[0].float().clone()
    ...
    st["site_out"] = out[0].float().reshape(-1).clone()
    return st, out

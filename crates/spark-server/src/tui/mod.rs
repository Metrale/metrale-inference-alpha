...
//! Metrale Engine TUI — the ratatui dashboard for `met serve`.
//!
...
//!   events          input/tick event loop on the dedicated "metrale-tui" thread
...
//!     `bench_preflight` + `metrale-plugin`'s executor (benchmarks are async
...
//! `metrale-tui` (this render loop) · `metrale-recipes` · `metrale-recipe-date` ·
//! `metrale-libscan` · `metrale-download` · `metrale-freshness` · `metrale-swap` ·
//! `metrale-report` (GitHub device flow + issue submit).
//!
...
//! `metrale-download` is a STREAMING producer (many progress messages, not one
//! result), and `metrale-swap` sends only on FAILURE — silence means "still
...
pub fn start(
    ...
    match std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            metrale_plugin::benchmarks::serve_matrix::host::install(std::sync::Arc::new(
            ...
            match metrale_plugin::ArtifactStore::discover() {
                Ok(store) => app.bench.attach(
                    metrale_plugin::BenchmarkExecutor::new(runtime, store),
                    metrale_plugin::TargetEndpoint::local(port, model),
                ),
...
///
/// Gates, in order: explicit `--no-tui`, `METRALE_NO_TUI=1`, non-interactive

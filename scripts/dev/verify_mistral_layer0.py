#!/usr/bin/env python3
"""
Verify Mistral Small 4 Layer 0 forward pass against Metrale Engine.

Computes: embedding → attention_norm → MLA Q/KV → RoPE → attention → O proj
          → residual → ffn_norm → MoE gate (top-4 routing only).

Reports hidden state norms at each step for comparison with Metrale Engine output.
"""
...
def main():
    ...
    # ── 4d. RoPE ──
    # Metrale Engine uses partial_rotary_factor=1.0 → rotary_dim=128 for GQA fallback
    ...
    # --- Full RoPE (rotary_dim=128, what Metrale Engine does) ---
    ...
    # But Metrale Engine applies full RoPE due to GQA fallback design

    print(f"=== Step: RoPE (Metrale Engine: rotary_dim=128, full head_dim) ===")
    ...
    # ── Summary ──
    print("=" * 70)
    print("SUMMARY OF NORMS (for Metrale Engine comparison)")
    print("=" * 70)
    ...
    print("-" * 50)
    print("  Metrale Engine sets shared_expert = ExpertWeight::null() in load_moe_mistral()")
    print("  Metrale Engine sets shared_expert_intermediate_size = 0")
    ...
    print("-" * 50)
    print("  Metrale Engine uses rotary_dim = head_dim = 128 (partial_rotary_factor=1.0)")
    ...
    print("  Since Metrale Engine applies RoPE to BOTH Q and K consistently,")

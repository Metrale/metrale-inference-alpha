...
**SBIO** — *Separation of Business logic and I/O* — is the user-level naming of a specific pattern the Metrale Engine codebase applies aggressively: business logic never performs I/O directly. It calls a trait. Real I/O is implemented behind that trait. Tests swap in mock implementations.

The payoff is concrete. The Metrale Engine test suite runs ~80% of the code without a GPU. You can verify the scheduler's fairness properties, the OpenAI/Anthropic protocol parsers, the sampler's numeric behavior, the tokenizer's template expansion, and every weight loader's shape checks on a vanilla Linux laptop. The only tests that need a GB10 are the ones that exercise a real CUDA kernel end-to-end.

## The I/O surfaces in Metrale Engine
...
`GpuBackend` is the most load-bearing of the four. Simplified shape (full trait in `crates/gpu-runtime/src/gpu.rs`):
...
A layer's forward pass calls `gpu.launch(...)`. It does not know, and cannot know, whether `gpu` is `MetraleCudaBackend` or `MockGpuBackend`. That opacity is the whole point.
...
`SingleGpuBackend` in `crates/comm/src/lib.rs` is the distributed-comms analogue:
...
- Kernel correctness vs a PyTorch/reference implementation (covered by the `*_microtest` and parity examples in `crates/model-arch/examples/` and the integration tests in `tests/`).
...
SBIO is a discipline, not a mechanism. But because the trait surface is small, the discipline is easy to enforce in review: if you see `cudaXxx` or `socket` or `fs::open` inside `metrale-model-layers` or `metrale-server`, that is a bug.
...
1. The file you edited — does it import `cudarc`, `nccl_sys`, `std::net`, or `std::fs`? If it's inside `metrale-model-layers`, `metrale-server`'s non-handler code, or any `metrale-*` primitive crate, the answer should be *no*. Route through the matching trait.
...
Read [Kernel Dispatch](./dispatch.md) next to see how SBIO composes with the runtime dispatch path — or [metrale-comm](../crates/metrale-comm.md) for the collective-ops version of the same pattern.

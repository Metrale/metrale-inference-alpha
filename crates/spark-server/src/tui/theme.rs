...
//! The Metrale Engine TUI design system.
//!
...
/// where `metrale-plugin`'s style-free results acquire color.
pub fn cell_style(style: metrale_plugin::CellStyle) -> Style {
    use metrale_plugin::CellStyle as S;
    match style {
        S::Neutral => text(),
        S::Dim => dim(),
        S::Accent => brand_cyan(),

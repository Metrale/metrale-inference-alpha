...
fn live_fetch_against_github() {
    ...
    // Every Metrale Engine recipe upstream must still produce a valid serve config —
...
///
/// The measured failure: `$HOME/.metrale` created by uid 1000 while the server
...
/// the operator to run `met sync-recipes` — which fetches from GitHub and

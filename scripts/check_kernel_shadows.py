...
The build (`crates/metrale-kernels/build.rs::collect_cu_files`) resolves each
...
`crates/metrale-kernels/build_shadow.rs`, and it is enforced by
`crates/metrale-kernels/tests/kernel_shadow_detector.rs` in the same CI run as
...
# crates/metrale-kernels/tests/support/inherited.rs `INHERITED`; adding a target

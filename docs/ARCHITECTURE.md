...
Metrale Engine is a single Cargo workspace with 15 crates organized into four layers:

```
                         ┌─────────────────────────┐
   HTTP / scheduling     │      metrale-server       │
                         │  (OpenAI + Anthropic    │
                         │  API, scheduler, tool   │
                         │  parsing, grammar)      │
                         └──────────┬──────────────┘
                                    │
                         ┌──────────┴──────────────┐
   model + layers        │       metrale-model-layers       │
                         │  (TransformerModel,     │
                         │  per-arch layers, MoE,  │
                         │  SSM, attention, MTP,   │
                         │  weight loading)        │
   ...
   ┌────────┴────────┐ ┌────────────┴───────┐  ┌──────────┴────────┐
   │  metrale-gpu-runtime  │ │    metrale-comm      │  │  metrale-storage    │
   ...
   └────────┬────────┘
            │
   ┌────────┴────────┐
   │  metrale-kernels  │   PTX bytes embedded at compile time
   ...
   │  metrale-core     │    — ModelConfig, registry, hardware capability
   │                 │    enums, error types.
   └─────────────────┘

   Test/bench crates:
   metrale-bench  (Criterion microbenchmarks per kernel category)
   metrale-gpu-sys    (FFI for NVIDIA cuFile / GDS — currently dormant on GB10)
```
...
3. For each `(hardware, model, quant)` tuple selected via `METRALE_TARGET_*` env vars (or `*` wildcard), compiles every `*.cu` under `kernels/<hw>/<model>/<quant>/` and `kernels/<hw>/<quant>/` (shared) to PTX via `nvcc`, deduplicating model-specific overrides over the shared pool.
...
Skip mode (`METRALE_SKIP_BUILD=1`): emits a stub registry. Used by CI for GPU-free `cargo check` / `clippy` runs.

### `metrale-gpu-runtime`
GPU-side runtime primitives:
- `cuda_backend.rs` — `GpuBackend` trait + `MetraleCudaBackend` (cudarc-based) impl. Wraps device pointers, kernel handles, streams, allocator.
...
Hardware-agnostic: the `GpuBackend` trait could be implemented by another vendor backend. Today only `MetraleCudaBackend` exists.
...
What happens:
1. Cargo invokes `metrale-kernels/build.rs`.
...
4. At runtime, `metrale-gpu-runtime::gpu` loads the PTX into the CUDA driver and exposes kernel handles.

For a multi-target binary (sweep mode): `METRALE_TARGET_MODEL='*' METRALE_TARGET_QUANT='*'` compiles every kernel target. The binary picks the right target at startup based on the loaded model's `model_type` and `hidden_size`.

...
# width = K=3) FIXED. Only toggled var: METRALE_GDN_FUSED_VERIFY.
...
BASE_ENV=(
  ...
  -e METRALE_MTP_CARRY_DEBUG=1
)

# leg <tag> <extra -e args...>
leg() {
  local tag="$1"; shift
  local CN="metrale-l6-$tag"
  for c in $(sudo docker ps -q --filter "name=metrale-l6-"); do sudo docker rm -f "$c" >/dev/null 2>&1; done
  sleep 4
...
leg A_unset || exit 1
leg B_fused -e METRALE_GDN_FUSED_VERIFY=1 || exit 1

...
//! 2026-09-26: Loader tests. No dataset file is in the repository, so every
//! test writes its own rows.
//!
...
/// 2026-09-26: A well-formed coding conversation with a system prompt, an
/// assistant row with tool_calls and no content, a tool_results row, and a
/// second tool-calling assistant row.
...
fn prebuilt_messages_are_teacher_forced_snapshots() {
    ...
    // 2026-09-26: Turn 1: system + the user row, with dataset-only fields
    // removed.
    ...
    // 2026-09-26: Turn 3: system + user + recorded assistant (content: null
    // filled in) + the tool_results expanded to a tool message.
    ...
    // 2026-09-26: Ground truth: the immediately following assistant row, raw.
    ...
    // 2026-09-26: Tools come from the first user row.
...
fn the_role_grammar_is_enforced() {
    ...
    // 2026-09-26: A tool row without a tool-calling assistant before it.
    let p = write(
        "grammar2",
        &[
    ...
    // 2026-09-26: A tool row without results.
    let mut rows = coding_rows("proj-a");
    ...
    // 2026-09-26: A client turn must be answered before another client turn
    // begins.
    let p = write(
        "grammar4",
        &[
    ...
    // 2026-09-26: A role outside the grammar is refused rather than dropped
    // from the formatted history.
    let p = write(
        "grammar5",
        &[
            json!({"conversation_id": "c", "turn": 1, "role": "user", "content": "one"}),

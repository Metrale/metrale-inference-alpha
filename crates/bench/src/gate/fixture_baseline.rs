...
//! 2026-09-26: Test fixture: write a gate baseline into a scratch repo as the
//! `kernels/<hw>/<model-dir>/BENCH.toml` entries that `read_baseline` assembles.
//!
//! Owner: bench gate.
...
/// 2026-09-26: Append one `[[benchmarks]]` entry per (hardware, checkpoint) of
/// `baseline` to that model directory's `BENCH.toml`. It also creates the
/// `HARDWARE.toml` that `bench::load_all` needs to see the hardware directory, a
/// `MODEL.toml` and an `nvfp4/` quant directory.
pub(super) fn write_baseline(root: &Path, benchmark_id: &str, baseline: &GateBaseline) {
    for (hw, entry) in &baseline.hardware {
        for (checkpoint, model) in &entry.models {
            // 2026-09-26: One model directory per checkpoint, named with `/` replaced
            // by `_`; the gate keys entries on the `checkpoint` field, not the name.

...
// forked-from: kernels/gb10/common/rms_norm.cu (2026-09-24; 1111 of 1433 lines differ, see kernels/FORKS.md)

// 2026-09-25: RMS norm kernels: out = x * rsqrt(mean(x^2) + eps) * w in FP32 math, one block per row. Variants fuse a
// residual add or a residual copy, take an FP32 input or keep an FP32 residual, multiply by SiLU(gate) (gated_rms_norm*),
// or L2-normalise in place (l2_norm_bf16). KERNEL.toml maps this file to module `norm`. Nothing in crates/ looks up
// rms_norm_residual_f32_abs, residual_add_rms_norm_f32_abs, rms_norm_f32, f32_residual_add, residual_add_rms_norm_f32,
// rms_norm_f32_in_abs or rms_norm_residual_f32.
// Owner: gb10 kernels (gemma-4-26b-a4b).
// Invariants: every kernel with a weight scales by the stored w; common/rms_norm.cu, which this file shadows, scales its rms_norm by (1 + w).
#include <cuda_bf16.h>
...
// 2026-09-25: out = x * rsqrt(mean(x^2) + eps) * w over hidden_size BF16 values; blockIdx.x is the row.
...
// 2026-09-25: hidden += src (stored as BF16), then rms_norm of the updated row into `output` and a copy into `residual`.
...
// 2026-09-25: rms_norm_residual and residual_add_rms_norm with an FP32 input or hidden and an FP32 residual copy; BF16 output.
...
// 2026-09-25: out = rms_norm(x) * w * SiLU(gate) per row; gate rows are gate_stride elements apart; group_size is ignored.
// x is cached in registers between the passes, at most 16 values per thread, so hidden_size <= 16 * blockDim.x; hidden_size
// must be a multiple of 4 (a tail is not written).
...
// 2026-09-25: gated_rms_norm over an FP32 input row, without the register cache; gate, weight and output are BF16.
...
// 2026-09-25: gated_rms_norm per (head, token): blockIdx.x is the head, blockIdx.y the token. Input and output rows are
// input_token_stride elements apart, gate rows gate_token_stride; head_dim has the limits of gated_rms_norm's hidden_size.
...
// 2026-09-25: In-place L2 norm of one head_dim slice: x *= rsqrt(sum(x^2) + eps). blockIdx.x is the head, blockIdx.y the
// token; token rows are `stride` elements apart.
...
// 2026-09-25: residual_add_rms_norm with an FP32 hidden (the add is in FP32) and an FP32 residual copy; BF16 output.
...
// 2026-09-25: residual_add_rms_norm that also writes the normed row in FP32 to output_f32; hidden, output and residual are
// computed as in residual_add_rms_norm. Looked up with try_kernel (qwen3_attention and qwen3_ssm init).
...
// 2026-09-25: rms_norm over an FP32 input row, BF16 output; the same statements as rms_norm_f32.
...
// 2026-09-25: rms_norm_residual over an FP32 input row: BF16 output, FP32 residual copy; the same math as rms_norm_residual_f32_abs.
...
// 2026-09-25: rms_norm over gridDim.y groups of gridDim.x rows; row (x, y) starts at y * row_stride + x * hidden_size.
// Per row the statements are those of rms_norm. Looked up with try_kernel (qwen3_attention init).

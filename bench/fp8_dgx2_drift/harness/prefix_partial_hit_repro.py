...
# 2026-09-26: Warm-versus-cold prefix-cache repro: the same final turn, after a priming request and with a fresh nonce.
#
...
It needs ONLY ONE server (the cache-ON `metrale-gb10:pfxfix` build). It does
...
def make_filler(n_words):
    # 2026-09-26: Deterministic: n_words words cycled from a fixed list.
    words = (
...
def main():
    ...
    for t in range(args.trials):
        ...
        # 2026-09-26: Warm arm: send the first two messages (system, user1) to fill
        # the cache, then the full four-message transcript, which shares that prefix.
        prime = msgs[:2]
        ...
        # 2026-09-26: Cold arm: a new nonce near the start of the system prompt, so
        # almost none of the prefix can come from the cache.

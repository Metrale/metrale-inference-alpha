...
//! 2026-09-25: The per-layer indexer check of `dsa_indexer_microtest`: runs the
//! compress, compact, scores, top-k and expand kernels over six regimes, in a
//! bf16 and an f32 arm, and compares them with the golden and with the
//! `glm5next_dsa_ref` CPU reference.
//!
...
pub(crate) fn indexer_layer(
    ...
    for (rname, s, pad, q_rows, neg_q) in regimes {
        ...
        // 2026-09-25: `kept_pools` keeps only the pools whose every slot is a valid token,
        // and `n_pools` and `select_k` follow the kept count. It reads only `valid` and `s`,
        // so both arms see the same pools.
        ...
        // 2026-09-25: The whole pipeline runs twice. The bf16 arm rounds the inputs and every
        // host front-end output (the projections and the key LayerNorm) to bf16 and is compared
        // with the bf16 golden; the f32 arm keeps f32 and is compared with the f32 golden.
        for arm in [Arm::Bf16, Arm::F32] {
            ...
            // 2026-09-25: The floor is the golden's own bf16-against-f32 difference; the
            // structural gate allows four times it.
            ...
            let gscb: Vec<f32> = entry(gbf, "index_scores")?
                ...
                .map(|x| *x as f32)
                .collect();
            // 2026-09-25: Entries that are non-finite or equal to f32::MIN (masked) are left
            ...
            let set_diff = sample(&sorted_gpu, e_tk.stride)
                ...
                .count();
            // 2026-09-25: When the golden stores every entry (stride 1), also compare the full
            // row sets; `row_digest_diff` below takes the larger of the two counts.
            ...
            // 2026-09-25: Per-row digest against the golden: the sum and count of the
            // non-negative entries, which do not depend on their order.
            ...
            // 2026-09-25: GPU against the CPU reference by the same row digest; reported, not
            // gated.
        ...
        // 2026-09-25: Without budget pressure (`n_pools <= select_k`) both arms must match the
        // golden's rows exactly; under pressure the row differences are reported, not gated.

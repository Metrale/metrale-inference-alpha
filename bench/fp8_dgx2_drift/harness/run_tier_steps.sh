...
# Metrale Engine drift-mitigation tiers (N≥10 per tier required to overcome the
# FP8 per-run variance).
#
# Usage:
#   ./run_tier.sh <tier-name> <N> [--container metrale-qwen-final]
#
# Outputs:
...
# Verify metrale container is up + responsive before burning time on probes.
...
# Capture the metrale startup-log marker so per-run windows can offset
...
for i in $(seq 1 "${N}"); do
  ...
  # Metrale Engine log window for THIS run only. Docker logs --since accepts

...
use metrale_plugin::TargetEndpoint;
...
pub(crate) async fn write_gate_record(
    record: &metrale_plugin::RunRecord,
    url: &str,
    model: &str,
    ...
    // The `METRALE_*` levers the gate applied to its server
    ...
    if record.frame.status != metrale_plugin::RunStatus::Completed {
        bail!(
    ...
    let hardware = metrale_plugin::http::fetch_hardware(&target, gate::HARDWARE_TIMEOUT).await;
    ...
    // of which should be minting keys in a real ~/.metrale.
    let store = metrale_plugin::artifacts::ArtifactStore::discover()?;

...
//! 2. First entry of `X-Forwarded-For` — when Metrale Engine sits behind a reverse
//!    proxy or load balancer. Trusted because Metrale Engine is typically deployed
...
//!   METRALE_RATE_LIMIT_RPM       — requests per minute cap
//!   METRALE_RATE_LIMIT_TPM       — tokens per minute cap
//!   METRALE_RATE_LIMIT_BURST_RPM — max request burst (default = RPM)
//!   METRALE_RATE_LIMIT_BURST_TPM — max token burst   (default = TPM)
//!
...
impl RateLimitConfig {
    /// # Errors
    /// When any `METRALE_RATE_LIMIT_*` variable is set to something that is not
    /// a whole number. That used to fall through to the default, and the
    /// default here is 0 — *the limit is off*. An operator who typed
    /// `METRALE_RATE_LIMIT_RPM=1oo` got an unlimited server that started

...
# 2026-09-26: followed_directions: which of the prompt's process steps the agent itself ran.
...
The harness PROMPT (run_tier.sh) instructs, verbatim:
  "create a pure rust Axum project ... ping/pong endpoint. The server MUST bind
   to the port from the METRALE_HARNESS_PORT env var ... Add tests, run them and
...
Each instruction becomes a checked sub-step. Evidence sources:
  ...
  - the filesystem the agent left behind (tests/ dir, #[test] in source,
    METRALE_HARNESS_PORT reference)
...
_REQUIRED_STEPS = (  # 2026-09-26: followed_directions is true when all of these are.
    "wrote_project",
    "wrote_tests",
    "ran_tests",
    "ran_server",
...
def _reads_port_env(target: pathlib.Path) -> bool:
    for p in _source_files(target):
        try:
            if "METRALE_HARNESS_PORT" in p.read_text(errors="replace"):
                return True
        except Exception:
            continue
    return False
...
def compute_followed_directions(
    ...
    return {
        ...
        # 2026-09-26: Informational; not part of followed_directions.

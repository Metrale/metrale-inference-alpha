...
|---|---|---|
| dense-FFN prefill via `w4a4_gemm` | `METRALE_FP4_PREFILL` | implemented, **never A/B'd** |
| QKV prefill via `w4a4_gemm_mfast` | `METRALE_ATTN_W4A4` | already tried and **rejected** — hallucinated-multiple-choice signature on long prompts, and only ~2 ms |
...
c2final env, nd=3, greedy temp 0 / seed 42. Only `METRALE_FP4_PREFILL` differs. Each prompt is
...
The flag was confirmed live rather than assumed — the `[metrale] METRALE_FP4_PREFILL=1: dense-FFN

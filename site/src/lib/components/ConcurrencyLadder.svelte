<script>
  ...
  // the published claim is Metrale Engine against the matched vLLM baseline, and
  // letting a second Metrale Engine configuration into that comparison would change
...
<svelte:element this={embedded ? 'div' : 'section'} id="concurrency"
  ...
  <div class={embedded ? 'cl-embed-inner' : 'container'}>
    ...
    <svelte:element this={embedded ? 'h3' : 'h2'} class={embedded ? 'cl-h' : 'stitle'}>
      ...
      {:else}
        Metrale Engine vs vLLM, C=1 to 128 — {ladder.summary.won} of {ladder.summary.rungs} rungs
      {/if}
    </svelte:element>
    <p class="cl-sub">
      {ladder.workload.checkpoint} on one GB10. {ladder.aggregate}. The matched baseline
      runs vLLM's own MTP speculative decoding at K=4, same as Metrale Engine, on the same box,
    ...
    <div class="cl-tablewrap">
      <table class="cl-table">
        <caption class="cl-caption">
          Throughput in tok/s. Ratio is Metrale Engine over the matched vLLM + MTP configuration
          at that rung. The unmatched no-speculation vLLM leg is shown, not scored.
        </caption>
        <thead>
          <tr>
            <th scope="col">C</th>
            {#if subjectShown}<th scope="col">Metrale Engine</th>{/if}
        ...
        <tbody>
          {#each ladder.rows as row}
            <tr>
              ...
              {#if subjectShown}<td class="mono cl-win">{fmtV(row.metrale)}</td>{/if}
        ...
        {#each shown as s}
          <article class="cl-series">
            ...
            {#if s.parity === 'unmatched'}
              <p class="cl-warn">
                Not matched to Metrale Engine: {s.parity_deltas.join('; ')}. Shown for

...
//! (`METRALE_DS41_STEP_GRAPH=1`, needs `METRALE_DS41_DEVICE_ROUTE=1`).
//!
...
/// `METRALE_DS41_STEP_GRAPH=1`, read once.
    ...
    pub(super) fn step_seg(
        ...
        if let Some(sg) = seg.graphs[self.idx].as_ref() {
            ...
            let Some(k) = first_miss else {
                seg.skip_until = end;
                if trace_on() {
                    // `METRALE_DS41_TRACE=1`: the highway after the segment, to
...
/// `METRALE_DS41_STEP_GRAPH_LOG=1`: one line per token with the running counts.

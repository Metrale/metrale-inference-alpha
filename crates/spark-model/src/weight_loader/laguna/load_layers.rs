...
fn load_moe_ffn(
    ...
    // Native NVFP4 CUTLASS grouped MoE (METRALE_HOLO_MOE_GROUPED_CUTLASS=1).
...
/// (`METRALE_HOLO_MOE_GROUPED_CUTLASS=1`). Costs ~7.1 GB of device memory for
...
/// `METRALE_LAGUNA_ROPE_TABLE=0` (falls back to the on-the-fly rope kernel).

...
//! 2026-09-25: Numeric and bandwidth microbench for the keep-packed ternary
//! Q2_0 decode GEMVs: the baseline `q2_0_gemv[_batchm]` against
//! `q2_0_gemv_vec[_batchm]`, the kernels the model layers launch.
//!
//! Owner: model-arch examples (GPU microtests).
//! Invariants:
//! - The run exits 0 only if both kernels' max relative error against the CPU
//!   oracle is below 1e-2 on both shapes at M = 1 and M = 4.
//!
//! It builds synthetic packed Q2_0 weights `[N, K]` (random codes 0..=3,
//! random fp16 per-group scales) and random BF16 activations `[M, K]`, runs
//! each kernel, and compares against a CPU oracle that dequantizes each block
//! (`value = (code - 1) * d`) and takes an f32 dot product with the BF16
//! activation. Shapes are gate/up N=17408 K=5120 and down N=5120 K=17408,
//! group 128. It reports per kernel, shape and M: relative error, µs per call,
//! weight GB/s and speedup over the baseline.
//!
//! Run:
//!   METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL='*' \
...
/// 2026-09-25: Pack a weight `[n, k]` (codes row-major, scales row-major) into
/// contiguous `block_q2_0` bytes: `[fp16 d][GROUP/4 bytes, 4 codes/byte,
/// low bits first]`.
...
/// 2026-09-25: CPU oracle: `out[m,n] = sum_k a[m,k]*(code-1)*d`.
...
/// 2026-09-25: One packed weight on the device, plus its host codes and
/// scales for the oracle.
struct Weight {
    d: DevicePtr,
    codes: Vec<u8>,
    scales: Vec<f32>,
...
/// 2026-09-25: Run one kernel: error against the oracle, then timing over
/// `ITERS` calls.
#[allow(clippy::too_many_arguments)]
fn run_variant(
    g: &dyn GpuBackend,
    ...
    // 2026-09-25: For m > 1 the caller passes a batchm kernel, which takes M
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: Baseline `q2_0_gemv` (2 warps per output row) against
    // `q2_0_gemv_vec` (vector loads, activations staged in shared memory, one
    // warp per row).
    ...
    for (name, n, k) in shapes {
        ...
        for m in [1usize, 4] {
            // 2026-09-25: Baseline: the M=1 kernel for m == 1, batchm otherwise.
            let (bk, bg) = if m == 1 {
                (base1, 4u32)
            } else {
                (base_m, 4u32)

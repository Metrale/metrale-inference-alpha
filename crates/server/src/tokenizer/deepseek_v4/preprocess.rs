...
//! 2026-09-26: Message preprocessing for the DeepSeek-V4 encoder: request-level tools go on
//! the first system (else developer) message, tool messages become `tool_result` blocks
//! sorted by call order, and assistant turns before the last user message lose their
//! reasoning.
//!
...
pub(super) fn drop_thinking_messages(messages: &[Value]) -> Vec<Value> {
    ...
    for (index, message) in messages.iter().enumerate() {
        if index >= last_user
            || matches!(
                role(message),
                // 2026-09-26: `developer` reaches this encoder unrenamed:
                // `build_msg_entries` keeps the role when `preserve_developer_role` is set,
                // which the chat path does for DeepSeek-V4.
                Some(
                    "user"
                        | "system"
                        | "developer"

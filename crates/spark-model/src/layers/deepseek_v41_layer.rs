...
//! instead (`METRALE_DS41_GRAPH=1`, see [`GraphMode`]): the host work in the
...
pub struct V41Runtime {
    ...
    /// per-step totals across layers, printed by the last layer when METRALE_DS41_DIAG=1
    ...
    /// The whole-step segment graphs (`METRALE_DS41_STEP_GRAPH=1`, `step_seg.rs`).
    ...
    /// `METRALE_DS41_PREDICT_TRACE`: the last three layers' MoE inputs
...
/// How the single-token step runs. `METRALE_DS41_GRAPH_ORACLE=1` runs every
/// layer BOTH ways and compares the highway bit for bit; `METRALE_DS41_GRAPH=1`

...
//! metrale-core keeps no GGUF dependency) and produces a validated
...
/// Map a GGUF `general.architecture` string to a Metrale Engine `model_type` (must be
/// a supported loader string) and whether attention Q is gated.
///
/// Plain-decoder GGUFs (llama/qwen2) have no dedicated Metrale Engine arch loader; the
...
pub fn config_from_gguf(inputs: &GgufConfigInputs) -> Result<ModelConfig> {
    ...
    // Metrale Engine's 1e-6 default — we set it explicitly rather than inherit).
    ...
    if model_type == "deepseek_v41" {
        ...
        // SwiGLU clamp. Shipped per-layer; Metrale Engine carries one scalar, so require

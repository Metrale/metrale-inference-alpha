...
//! 2026-09-25: Byte-parity gate for the batched W8A16 GEMVs `w8a16_gemv_batch4`
//! and `w8a16_gemv_batch16` (both `w8a16_gemv_batchm_impl`) against M
//! single-row `w8a16_gemv` launches.
//!
//! It compares raw BF16 bytes, not a cosine, over three seeds and two shapes, at
//! M = 2, 3, 4 (batch4) and 8, 12, 16 (batch16): 36 legs.
//!
    ...
    for seed in [1u64, 99, 12345] {
        for (label, n, k) in SHAPES {
            ...
            let a_bytes: Vec<u8> = (0..max_m * k)
                ...
                .collect();
            // 2026-09-25: Weights are `[n, k]` raw FP8 E4M3 bytes; block scales are
            // FP32 `[ceil(n/128), ceil(k/128)]`.

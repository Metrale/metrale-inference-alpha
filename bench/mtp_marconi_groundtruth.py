...
# 2026-09-26: Greedy multi-turn capture (with a tool turn) of per-turn tokens, to diff two server configs.
#
# Owner: bench.
...
# 2026-09-26: Each case is a list of user turns; a turn containing "Create a file" is
# sent with TOOLS.
CASES = [
    [
...
def chat(messages, tools=None, max_tokens=384):
    ...
    # 2026-09-26: Tool calls are kept as name(arguments) strings for the diff.
...
def main():
    ...
    for ci, turns in enumerate(CASES):
        ...
        for ti, u in enumerate(turns):
            ...
            turn_outs.append(o)
            # 2026-09-26: Only the assistant's content goes back into the history, not
            # its tool calls.
            asst = {"role": "assistant", "content": o["content"]}
            msgs.append(asst)
            if o["tool_calls"]:
                # 2026-09-26: A fixed tool result follows any turn that called a tool.

...
Repo rules for every new file: SPDX line 1, ≤500 LoC/.rs, tests in own `*_tests.rs`, clippy deny-warnings, no `cargo fmt --all`, add unit tests alongside each edit, run `cargo test` on `metrale-core`+`spark-model` before commit, **push origin only**, do not build/deploy while a sibling subagent runs.

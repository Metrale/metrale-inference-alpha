...
// 2026-09-25: Hopper small-expert geometry of kernels/gb10/common/moe_w8a8_grouped_gemm.cu,
...
__device__ __forceinline__ __nv_bfloat16 p16_e4m3_to_bf16(unsigned char b) {
    // 2026-09-25: E4M3 -> f32 by bit arithmetic: place the 7 magnitude bits in an f16
...
// 2026-09-25: MMA over one resident K_STEP (2 x m16n8k16 sub-MMAs in ascending K order —
...
__device__ __forceinline__ void p16_mma_kstep(
    const __nv_bfloat16* smem_A,   // 2026-09-25: [P16_M_TILE][P16_A_STRIDE]
    const __nv_bfloat16* smem_B,   // 2026-09-25: [P16_N_TILE][P16_K_STEP + P16_PAD]
...
extern "C" __global__ void __launch_bounds__(P16_THREADS, 2) pm4_m16(
    ...
    const float* __restrict__ a_scale,                      // 2026-09-25: [total_tokens, K/128] FP32
    const unsigned long long* __restrict__ B_weight_ptrs,   // 2026-09-25: [num_experts] -> [N, K] FP8
    const unsigned long long* __restrict__ B_scale_ptrs,    // 2026-09-25: [num_experts] -> [N/128, K/128] FP32
    ...
    const int* __restrict__ total_tiles                     // 2026-09-25: [1]
) {
    ...
    for (int wid = blockIdx.x; wid < total; wid += (int)gridDim.x) {
        __syncthreads();   // 2026-09-25: fence smem reuse before re-priming the pipeline
        ...
        const unsigned int steps_per_promote = P16_K_PROMOTE / P16_K_STEP;   // 2026-09-25: 2

        // 2026-09-25: Per-warp fragment rows: token ids resolved ONCE per tile for the
        ...
        auto prefetch = [&](unsigned int step, unsigned int stage) {
            ...
            // 2026-09-25: A raw: 128 rows x K_STEP FP8 bytes, 16-B chunks, gathered per row.
            const unsigned int a_chunks = (P16_M_TILE * P16_K_STEP) / 16;   // 2026-09-25: 256
            #pragma unroll
                ...
                if (m_global < (unsigned int)M_expert && gk + 16 <= K) {
                    ...
                    for (unsigned int e = 0; e < 16; e++) {
                        ...
                        if (m_global < (unsigned int)M_expert && gke < K) {
                            ...
                            dst[e] = 0;   // 2026-09-25: dequants to +0.0
                        }
                    }
                }
            }

            // 2026-09-25: B raw: N_TILE rows x K_STEP FP8 bytes, 16-B chunks.
            const unsigned int b_chunks = (P16_N_TILE * P16_K_STEP) / 16;   // 2026-09-25: 128
            #pragma unroll
        ...
        // 2026-09-25: Arithmetic-dequant just-arrived raw A and B for `stage` into the
        ...
        for (unsigned int step = 0; step < n_steps; step++) {
            ...
            __syncthreads();   // 2026-09-25: raw A/B for `cur` resident for all threads

            dequant(cur);
            __syncthreads();   // 2026-09-25: smem_A/B[cur] fully written before MMA reads

            p16_mma_kstep(&smem_A[cur][0][0], &smem_B[cur][0][0],
                         inner_acc, warp_m_offset, group_id, tid);
            __syncthreads();   // 2026-09-25: done reading smem_*[cur]; safe for reuse

            // 2026-09-25: K_PROMOTE boundary: fold per-row a_scale x b_scale, reset inner.
            k_step_in_prom++;

...
// The handshake wire codecs moved verbatim to the CUDA-free `metrale-rdma`
// crate (extracted to metrale-rdma); re-exported here at their old paths so
...
mod server_impl {
    ...
    /// connection. Blocking; intended to run as its own process (`metrale-expert-peer`).
...
mod tests {
    ...
    // `crates/metrale-rdma/tests/wire_roundtrip.rs` (extracted to metrale-rdma);

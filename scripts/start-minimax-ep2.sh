...
echo "=== Metrale Engine MiniMax EP=2 bring-up (RDMA-enabled) ==="
echo "Model:   $MODEL"
echo "Image:   $IMAGE"
...
sudo docker run -d \
  ...
  $NCCL_ENV \
  -e RUST_LOG=info \
  ${METRALE_PROFILE_FIRST:+-e METRALE_PROFILE_FIRST=$METRALE_PROFILE_FIRST} \
  ${METRALE_UNIFIED_MOE_LAYOUT:+-e METRALE_UNIFIED_MOE_LAYOUT=$METRALE_UNIFIED_MOE_LAYOUT} \
  ${METRALE_HYBRID_MOE_LAYOUT:+-e METRALE_HYBRID_MOE_LAYOUT=$METRALE_HYBRID_MOE_LAYOUT} \
  ${METRALE_NVFP4_GATE_UP_M128:+-e METRALE_NVFP4_GATE_UP_M128=$METRALE_NVFP4_GATE_UP_M128} \
...
echo "Monitor rank 0: sudo docker logs -f metrale-minimax-ep0"
echo "Monitor rank 1: ssh $WORKER_IP 'sudo docker logs -f metrale-minimax-ep1'"

...
`main` is **25 commits ahead** and two of them touch perf paths:
    ...
    6cd39ba02  feat(cli): met sync-recipes            11 perf files
...
## The run
    ...
    export METRALE_HOME=${METRALE_HOME:-$HOME/.metrale}   # MUST be writable, or every
    ...
    for g in decode-floor vision-fidelity video-fidelity \
      ...
      timeout 21600 ./target/release/met benchmark run "$g" --pull-request-gate --yes
...
`$METRALE_HOME/runs/ttft-cold-gate/baseline-<model>.json`. Listing them twice
...
wrong — but check for that file before assuming you need it:
    ...
    ls $METRALE_HOME/runs/ttft-{cold,warm}-gate/baseline-*.json
...
## Judge by the record, never by rc
    ...
    ./target/release/met benchmark --pull-request-gate-check --pr 746
...
it. Run the two imports `score.py` actually performs:
    ...
    $METRALE_HOME/artifacts/bfcl/venv/bin/python -c "

...
/// Why the `METRALE_GDN_PREFILL_TC` spine is NOT running — `None` means it is.
///
...
/// FLA multi-kernel chunked GDN prefill (`METRALE_GDN_FLA=1`).
///
...
pub fn gdn_prefill_fla(
    ...
    // non-zero AND METRALE_GDN_TC_VBLOCK=1, replaces the scalar ksplit spine (drop-in
    ...
    // METRALE_GDN_PREFILL_TC (presence, default OFF). Drop-in ABI == the fused
    ...
    // `METRALE_GDN_PREFILL_TC` overriding — a VALUE, not a presence check, so `=0`
    // turns the family off. `METRALE_NO_GDN_PREFILL_TC_REMNANTS=1` is the A/B that
    ...
    //
    // `METRALE_GDN_VTILE=1` raises the same core to SPLIT=4 / 512 threads for 2.15x.
    ...
    // with `METRALE_GDN_PREFILL_TC` overriding, the same rung as every other
    ...
    let tma_reject: Option<&str> = if !tma_requested {
        ...
        // Both levers are set: TMA yields, because METRALE_GDN_PREFILL_TC is the

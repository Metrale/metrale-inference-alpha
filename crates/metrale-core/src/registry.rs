...
//!
//! Usage:
//!   let reg = MetraleRegistry::get_or_init(ordinal, &[("gemm", PTX_SRC), ...])?;
...
// for the lifetime of the owning CUcontext (the Metrale Engine registry binds the
...
///
/// Model-scoped: the blob set comes from `metrale_kernels::ptx_for_model`, so it
...
///
/// Obtain one with [`MetraleRegistry::load`] and propagate it (`Arc<MetraleRegistry>`);
...
impl Drop for MetraleRegistry {
    ...
    fn drop(&mut self) {
        ...
        if !failures.is_empty() {
            // No `tracing` in metrale-core's dependency budget, and a `Drop` has
...
impl MetraleRegistry {
    ...
    pub fn raw_function_cached(
        ...
        let status =
            ...
            // call still type-checks on aarch64 (Metrale Engine's actual GB10 target).
    ...
    /// Get the raw CUstream handle for Metrale Engine's own stream.
    ...
    /// - `stream_ptr` must be a valid CUstream handle (or 0 to use Metrale Engine's own stream).
    ...
    pub unsafe fn launch_on_stream(
        ...
        // Never fall back to Metrale Engine's private stream — that breaks ordering

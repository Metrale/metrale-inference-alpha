...
//! hard misconfigurations like `METRALE_SSM_DECODE_TIER=nvme` with no dir).
...
/// model with no recurrent state is a startup error. (`METRALE_SSM_SWAP_NS` /
/// `METRALE_SSM_DECODE_NS` are namespace *overrides*, not tier selectors, and

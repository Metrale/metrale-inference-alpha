#!/usr/bin/env python3
"""
Metrale Engine Overnight Marathon Test Suite
Runs all models in parallel across 2 DGX Spark nodes for 7+ hours.
Self-repairs issues and produces a final summary table.

Usage: python3 overnight_marathon.py 2>&1 | tee /workspace/metrale/overnight_results.log
"""
...
def main():
    ...
    log(f"═══ METRALE OVERNIGHT MARATHON ═══")
    ...
    while time.time() < end_time:
        ...
        for pair in pairs:
            ...
            for _, _, node in pair:
                cname = f"metrale-{'ep0' if node == 'dgx1' else 'ep1'}"
        ...
        if time.time() < end_time:
            for model, extra in EP2_MODELS:
                ...
                docker_stop("metrale-ep0")
                docker_stop("metrale-ep1", DGX2_IP)

                # Head
                ep_head = f"--master-addr {DGX1_IP} --world-size 2 --rank 0"
                ok0 = docker_start("metrale-ep0", model, 8888, extra, None, HF_CACHE_DGX1, 16384, ep_head)

                # Wait for NCCL listener
                for _ in range(84):
                    logs, _ = run("sudo docker logs metrale-ep0 2>&1 | tail -5", timeout=10)
                ...
                ok1 = docker_start("metrale-ep1", model, 8889, extra, DGX2_IP, HF_CACHE_DGX2, 16384, ep_worker)

                if ok0 and ok1 and wait_ready("metrale-ep0", timeout_s=480):
    ...
    # Save JSON
    with open("/workspace/metrale/overnight_results.json", "w") as f:
        json.dump({"elapsed": str(elapsed), "iterations": iteration, "results": all_results}, f, indent=2)
    log(f"\nJSON saved to /workspace/metrale/overnight_results.json")

...
# 2026-09-26: Preemption probe: N concurrent long generations against a starved KV pool.
#
...
MODEL = os.environ.get("METRALE_MODEL", "laguna-s-2.1")
...
def one(i):
    body = {"model": MODEL,
            # 2026-09-26: Each prompt opens with its stream index, so the BASE text
            # after it cannot be reused from another stream's prefix cache.

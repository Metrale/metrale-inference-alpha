# Metrale Engine FP8 Drift — Statistical Harness

A small harness for measuring opencode drift modes on a running Metrale Engine
...
A/B claim about a Metrale Engine intervention must rest on a statistical
...
```bash
# 1. Ensure metrale container is live & serving on localhost:8888.
sudo docker ps --filter name=metrale-qwen-final
...
| **Metrale Engine** | `metrale_ws1_mask_fires` | diagnostic INFO log `ws1/am1 mask active` |
| **Metrale Engine** | `metrale_b1_drift_fires` | B1 margin-ratio drift gauge summary lines |
| **Metrale Engine** | `metrale_tier5c_retries` | Tier 5c retry success count |
| **Metrale Engine** | `metrale_a2_fuzzy_fires` | A2 fuzzy_repair rescue count |
| **Metrale Engine** | `metrale_tool_call_lines` | total Metrale-side tool_call log lines |
...
- One probe ≈ 3-6 min wall time. Metrale Engine serves single agentic requests
  ...
  dominated by metrale decoding, not opencode logic.

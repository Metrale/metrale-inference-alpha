...
// 2026-09-25: FP8-weight GEMV over two activation rows in one pass over the weight:
// C[t, n] = row_scale[n] * sum_k A[t, k] * fp8(B[n, k]) for t in {0, 1}.
//
// Owner: gb10 kernels.
// Invariants:
// - A is [2, K] BF16 contiguous, B is [N, K] FP8 E4M3 bytes row-major, row_scale is [N]
//   FP32, C is [2, N] BF16 (row 1 starts at C + N).
// - Launch: grid (ceil(N / 4), 1, 1), block (256, 1, 1); 64 threads (2 warps) per output.
// - Assumes K % 16 == 0: there is no scalar tail.
// - Not bit-identical to two dense_gemv_fp8w calls: mac4 adds four products together
//   before adding them to the accumulator, where dense_gemv_fp8w adds each product in turn.
//
// Caller: the FP8 lm_head for a two-token step (model-engine impl_a3_lm_head.rs).
...
extern "C" __global__ void dense_gemv_fp8w_batch2(
    ...
    if (lane == 0) {
        ...
        C[(unsigned long long)N + n] = __float2bfloat16(r1);
    }
}

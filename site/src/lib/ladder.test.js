...
test('scales the top two rungs against the lower rung leader', () => {
  const h = headroom(
    rungs(
      { c: 64, metrale: 100, best_baseline_id: 'x', baselines: [{ id: 'x', label: 'X', tok_s: 100 }] },
      { c: 128, metrale: 150, best_baseline_id: 'x', baselines: [{ id: 'x', label: 'X', tok_s: 90 }] }
    )
  );
  expect(h.from).toBe(64);
  expect(h.to).toBe(128);
  expect(h.metrale).toBe(50);
...
test('refuses to report when the leader was not run at the top rung', () => {
  expect(
    headroom(
      rungs(
        { c: 64, metrale: 100, best_baseline_id: 'x', baselines: [{ id: 'x', label: 'X', tok_s: 100 }] },
        { c: 128, metrale: 150, best_baseline_id: 'y', baselines: [{ id: 'y', label: 'Y', tok_s: 90 }] }
      )
    )
  ).toBeNull();
});
...
test('needs two rungs', () => {
  ...
  expect(headroom([{ c: 1, metrale: 1, best_baseline_id: 'x', baselines: [] }])).toBeNull();
...
test('the shipped ladder still supports the claim the copy makes', () => {
  // If a regenerated ladder ever stops showing Metrale Engine climbing while the
  ...
  expect(h.metrale).toBeGreaterThan(0);
  expect(h.metrale).toBeGreaterThan(h.baseline);
});
...
test('C=128 is 1.333× against matched MTP, not 1.225× against unmatched nospec', () => {
  ...
  expect(row.metrale).toBe(478.11);

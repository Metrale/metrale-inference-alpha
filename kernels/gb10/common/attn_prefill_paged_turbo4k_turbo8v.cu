...
// 2026-09-25: Kernels `attn_prefill_paged_turbo4k_turbo8v` and `_64`: paged prefill flash attention with a turbo4 K
// cache and a turbo8 V cache.
//
// The body is prefill_paged_compute_asym.cuh. Each cache has its own block stride and scale-section offset
// (`tq4_k_bsb`/`tq4_k_dsb`, `tq8_v_bsb`/`tq8_v_dsb`). The K loader is attn_prefill_paged_turbo4.cu's with the
// turbo4 codebook in `k_lut`, and the V loader is attn_prefill_paged_turbo8.cu's (FP8 E4M3 values, one BF16 scale per
// 16); both dequantize to BF16 while they fill shared memory.
//
// Owner: gb10 kernels.
...
#define KERNEL_NAME attn_prefill_paged_turbo4k_turbo8v

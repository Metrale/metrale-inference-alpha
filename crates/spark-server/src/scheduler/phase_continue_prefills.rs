...
/// Driver API directly via `metrale_core::registry`), so this collapses to
...
pub(super) fn continue_in_progress_prefills(
    ...
    // injects. When METRALE_HOLO_ALWAYS_MIXED is OFF the scheduler is
    ...
    if always_mixed {
        ...
        if fusable_mixed {
            ...
            // METRALE_MIXED_SLICE_TOKENS: experimental override of the
            ...
            let cap = *MIXED_SLICE.get_or_init(|| {
                std::env::var("METRALE_MIXED_SLICE_TOKENS")
                    .ok()
    ...
    // BISECT: METRALE_BISECT_Q12_DISABLE=1 forces the per-stream FIFO path
    ...
    let q12_dispatch_disabled = std::env::var("METRALE_BISECT_Q12_DISABLE")
    ...
    let can_batch_prefill_only = !q12_dispatch_disabled
        ...
        && active.is_empty()
        && !model.is_ep();
    // When METRALE_HOLO_ALWAYS_MIXED is on, COLLAPSE the multi-prefill+decode
        ...
        if use_twophase {
            ...
            match model.prefill_twophase(
                ...
                Ok(logits) => {
                    ...
                    // Kill-switch: METRALE_NO_MTP_MINP=1.

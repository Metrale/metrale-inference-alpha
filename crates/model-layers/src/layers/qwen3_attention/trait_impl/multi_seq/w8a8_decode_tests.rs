...
//! 2026-09-25: Tests for the attention decode W8A8 arm: which `METRALE_CUBLAS_GEMM` families arm it, and that its strided Q/K/V write stays inside the QKV buffer.
//!
...
//! three plans this layer builds from `per_seq_qkv`, `q_proj_bytes` and the K/V
//! offsets.
...
/// 2026-09-25: Qwen3.8-27B attention: hidden 5120, 24 q-heads, 4 kv-heads,
/// head_dim 256, output gate on. So `q_proj` is the interleaved `[Q|gate]` at
/// 12288, k/v are 1024, and one sequence's `[Q|K|V]` row is 14336 BF16 elements.
const H: u32 = 5120;
const Q_N: u32 = 12288;
const KV_N: u32 = 1024;
const PER_SEQ_QKV: u32 = Q_N + 2 * KV_N;
/// 2026-09-25: A 16-row `qkv_output`, the smallest the arena allocates (it pads
/// its row count to a multiple of 16).
...
/// 2026-09-25: The three plans `qkv_decode_w8a8_plans` builds, reproduced from
/// the same offsets: Q at 0, K at `q_proj_bytes`, V after K.
...
/// 2026-09-25: `attn` (alone or in a list) and `all`/`1`/`true` arm it; no other
...
/// 2026-09-25: Q, K and V are each bounded from their own offset. The expected
/// extents are written as numbers, not as a call to the formula: at 16 padded
/// rows each write reaches 15 row pitches plus its own output width.
#[test]
...
/// 2026-09-25: The phantom-row guard. `ceil16` is 16 for every row count in the
/// band, so at n=5 eleven phantom rows are written, into rows 5..16 of
/// `qkv_output`. A buffer sized only for the live rows must make the arm
/// decline.
#[test]
fn phantom_rows_never_leave_the_qkv_buffer() {
    for rows in [5usize, 8, 12, 15] {
        assert_eq!(qkv_plans(rows, QKV_CAPACITY)[0].1.m_pad(), 16);
        // 2026-09-25: With 16 rows of room the arm is accepted, and the extent
        // is the same at every row count because the pad is.
        assert!(
            all_selected("attn", rows, false, QKV_CAPACITY),
            "rows={rows}"
        );
...
/// 2026-09-25: One BF16 element short of V's extent (V has the least room of
/// the three) takes all of Q/K/V off the arm.
#[test]

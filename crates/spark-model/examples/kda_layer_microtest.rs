...
//!
//! 🪤 Metrale Engine's L2 writes **bf16** (fused on decode, `l2_norm_bf16` on prefill); HF normalises in
//! **fp32 inside** the KDA kernel. Metrale Engine therefore carries one extra bf16 rounding on q|k that
//! HF does not, and every downstream stage inherits it. That is a contract difference, not an
//! error — it is why floor D is measured against a CPU reference that reproduces Metrale Engine's exact
...
// ────────────────────────────────────────────────── CPU reference, Metrale Engine's ladder

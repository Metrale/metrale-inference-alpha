...
Metrale Engine was built for a specific operator: NVIDIA DGX Spark GB10 (sm_121),
...
Metrale Engine is **pure Rust + cudarc** end-to-end. There is no PyTorch in the hot
path. Each supported model family has its own `TransformerLayer` impl in
`crates/model-layers/src/layers/<arch>/`, its own `WeightLoader` for
safetensors → typed-struct mapping, and its own per-quant CUDA kernels under
`kernels/<hw>/<model>/<quant>/*.cu`.

The shared "vocabulary" lives in `metrale-core` (`ModelConfig`, `LayerType`,
`KernelTarget`). The build script `metrale-kernels/build.rs` compiles `*.cu`

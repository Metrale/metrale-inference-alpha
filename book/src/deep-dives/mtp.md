...
Acceptance rate is lower (~60%) than MTP (~85%), but it works on any model. Metrale Engine ships this for coverage; operators typically use MTP when the checkpoint supports it.
...
N-gram speculative was experimented with heavily on TRT-LLM (see the `project_ngram_*` notes); Metrale Engine's Rust implementation lives in `spark-server/src/ngram.rs` and is much simpler.
...
- `docs/METRALE_SPARK_JOURNEY.md` — release journey and bug-sweep history.

...
|---|---|---|
| `METRALE_HOLO_FP4_PROJ_DECODE=1` | NVFP4 projection weights on the decode path (less HBM traffic). In the proven Holo NVFP4 config; easy to drop on non-Holo qwen3.6. | `weight_loader/qwen35/load_layers.rs:215` |
| `METRALE_GDN_DECODE_GRAPH=1` | GDN (linear-attn) decode captured as a graph | `model/trait_impl/decode_a.rs:202` |
| `METRALE_GDN_FUSED_CONV=1` | fused causal-conv in the GDN decode step | `qwen3_ssm/.../ssm_batched_recurrent.rs:270` |
| `METRALE_GDN_FUSED_NORM=1` | fused norm in the GDN decode step | qwen3_ssm decode |
| `METRALE_DECODE_OPT=1` | dense-FFN decode fast path | `layers/dense_ffn.rs:262` |

`METRALE_HOLO_FP8_SSM_DECODE` is **redundant** here — the SSM arms already load FP8-native for decode (`Layer N: SSM native FP8 — w8a16 decode + prefill`).

⚠️ `METRALE_FP8_SINGLE_SCALE` must stay **unset on block-scaled FP8** checkpoints (disables block-scaled prefill → ~14× drift); it is valid only on NVFP4 (single-scale). This matrix is NVFP4, so it's set.

## Config (this matrix)

- Model: `unsloth/Qwen3.6-35B-A3B-NVFP4`, GB10, `metrale-inference-gb10:b12x-ready` (CUDA 13.2, CUTLASS).
- Prefill flags: `METRALE_FLASHINFER_PREFILL METRALE_CUBLAS_GEMM METRALE_CUTLASS_WORKSPACE_MB=512 METRALE_PREFILL_VARLEN METRALE_PREFILL_CODISPATCH(+WINDOW_MS=100) METRALE_MOE_PREFILL_EXACT_TILES METRALE_SSM_BATCHED_RECURRENT METRALE_HOLO_MOE_GROUPED_CUTLASS METRALE_HOLO_MOE_GROUPED_DOWN METRALE_HOLO_FAST_MOE_MODE=full METRALE_HOLO_FAST_MOE_LAYERS=0-39 METRALE_HOLO_NATIVE_FP8_ATTN METRALE_HOLO_NATIVE_FP8_SSM METRALE_HOLO_LOW_MEMORY_MOE METRALE_Q12_BATCHED(+_FIRST_CHUNK) METRALE_GDN_TC_VBLOCK=0 METRALE_FP8_SINGLE_SCALE` (`METRALE_KV_OVERCOMMIT` dropped — see below)
- Decode flags: the 5 above + `METRALE_MOE_BATCHED_DECODE`
...
★ **Three names in this matrix are not live knobs; reproduce with care.**
  - `METRALE_DECODE_GRAPHS_MULTISEQ` has **no read site** — multi-seq decode graphs are on by
    default and the only gate is the negative `METRALE_NO_DECODE_GRAPHS_MULTISEQ`
    (`model/trait_impl/decode_a2.rs`). Setting the positive form does nothing; it has been
    removed from the list.
  - `METRALE_KV_OVERCOMMIT` is **default-ON** (`factory/build.rs` reads it as
    "on unless `0`/`false`"), so setting it is a no-op rather than an opt-in.
  - The measured prefill set also enabled the opt-in FlashInfer GDN prefill (a
    prebuilt library). That path and its flag were removed on 2026-09-24, so the
    flag has been removed from the list; the prefill numbers here include it.

★ **These are bare names, but the read sites require the literal string `1`.** Verified
  strict-`"1"` reads: `METRALE_FLASHINFER_PREFILL`,
  `METRALE_CUBLAS_GEMM`, `METRALE_PREFILL_VARLEN`, `METRALE_PREFILL_CODISPATCH`,
  `METRALE_MOE_PREFILL_EXACT_TILES`, `METRALE_HOLO_MOE_GROUPED_CUTLASS`,
  `METRALE_HOLO_MOE_GROUPED_DOWN`, `METRALE_HOLO_NATIVE_FP8_ATTN`, `METRALE_HOLO_NATIVE_FP8_SSM`,
  `METRALE_HOLO_LOW_MEMORY_MOE`, `METRALE_Q12_BATCHED`, `METRALE_MOE_BATCHED_DECODE`,
  `METRALE_FP8_SINGLE_SCALE`, `METRALE_SSM_BATCHED_RECURRENT`. `export METRALE_FLASHINFER_PREFILL`
  with no value silently does nothing — write `=1` on every one of them.

★ `METRALE_SSM_BATCHED_RECURRENT` additionally has a CLI flag now
  (`--ssm-batched-recurrent`). Its clap default USED to seal the value before the env read
  ran, which made the env form inert under `met serve`; the flag is an `Option` now and

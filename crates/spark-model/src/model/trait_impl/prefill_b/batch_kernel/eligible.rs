...
/// `--prefill-codispatch` (legacy `METRALE_PREFILL_CODISPATCH=1`) or by the older
/// `METRALE_Q12_BATCHED_FIRST_CHUNK=1` spelling of the same path.
///
...
impl TransformerModel {
    ...
    /// `METRALE_CODISPATCH_BTCHECK=1`; no-op otherwise.
...
/// VARLEN batched prefill enabled? (`METRALE_PREFILL_VARLEN=1`). Co-admits
...
/// `METRALE_Q12_EFFECTIVE_ARENA=1` — charge the Q12 batched-prefill arena budget

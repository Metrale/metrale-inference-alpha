...
//! 2026-09-26: Key handling for Help ▸ Report Issue: one handler each for Compose, Preview, Done and Failed, and one shared by the two auth phases.
//!
//! Owner: server tui.
//! Invariants:
//! - `HelpState::on_key` does nothing on the Guide sub-tab or in `ReportPhase::Submitting`.
...
impl HelpState {
    pub(super) fn on_key(&mut self, key: KeyEvent, ctx: &ReportCtx) {
        ...
        match self.phase {
            ...
            // 2026-09-26: Not cancellable, since the POST may already have created the
            // issue. Each request of the submit ends within the agent's 30 s timeout
            // (`report_http.rs`).
    ...
    fn compose_key(&mut self, key: KeyEvent, ctx: &ReportCtx) {
        if self.title_editing {
            match key.code {
                // 2026-09-26: Esc keeps the draft title and only leaves editing.
    ...
    fn preview_key(&mut self, key: KeyEvent, ctx: &ReportCtx) {
        match key.code {
            ...
            // 2026-09-26: Toggling the log attachment here re-composes the preview.
            KeyCode::Char('a') => {
                self.attach_logs = !self.attach_logs;
    ...
    fn auth_key(&mut self, key: KeyEvent) {
        match key.code {
            KeyCode::Esc => {
                // 2026-09-26: The device-flow thread checks `cancel` every 200 ms while it
                // waits between polls. The draft title and body are kept.
    ...
    fn failed_key(&mut self, key: KeyEvent) {
        match key.code {
            // 2026-09-26: Retry sends the stored `pending` body without re-composing it,
            // requesting a device code first when no tokens are held.
...
impl super::app::App {
    /// 2026-09-26: Route a key into the Help section with the context the composer needs.

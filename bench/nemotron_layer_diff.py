...
# 2026-09-26: Per-layer last-token residual diff, Metrale Engine versus HF, for Nemotron-3-Nano.
#
# Owner: bench.
# Invariants: none beyond the types.
"""Per-layer divergence comparator: Metrale Engine (METRALE_NEMO_DUMP) vs HF oracle.
...
Usage: nemotron_layer_diff.py <metrale_dir> <hf_dir>
"""
import json
import pathlib
import sys
...
# 2026-09-26: hybrid_override_pattern of Nemotron-3-Nano-30B-A3B (52 layers, from
# its config.json).
...
def main():
    ...
    print(f"{'layer':>6} {'kind':>6} {'cosine':>10} {'relL2':>10} "
          f"{'maxdiff':>10} {'|metrale|':>10} {'|hf|':>10}")
    print("-" * 70)
    first_div = None
    rows = []
    # 2026-09-26: nemotron_hf_ref_cpu.py writes no hf_L{last}.bin; the final_norm row
    # below covers the end of the stack instead.
    for i in range(nlayers):
        a = rd(A / f"metrale_L{i}.bin")
        b = rd(H / f"hf_L{i}.bin")
        if a is None or b is None:
            ...
            print(f"{i:>6} {KIND.get(PATTERN[i],'?'):>6}   MISSING "
                  f"metrale={a is not None} hf={b is not None}{note}")
            continue
    ...
    for name in ("final_norm", "logits"):
        a = rd(A / f"metrale_{name}.bin")
        b = rd(H / f"hf_{name}.bin")
        if a is not None and b is not None:
            cos, rel, maxd, na, nb = metrics(a, b)
            print(f"{name:>13} cos={cos:.6f} relL2={rel:.6f} "
                  f"maxdiff={maxd:.4f} |metrale|={na:.2f} |hf|={nb:.2f}")

    af = A / "metrale_logits.bin"
    hf = H / "hf_logits.bin"
    if af.exists() and hf.exists():
        ...
        at = np.argsort(-al)[:10]
        ht = np.argsort(-hl)[:10]
        print(f"\nmetrale top-10 token ids: {at.tolist()}")
        print(f"hf    top-10 token ids: {ht.tolist()}")
        print(f"argmax match: metrale={int(at[0])} hf={int(ht[0])} "

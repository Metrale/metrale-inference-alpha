...
//! 2026-09-25: Byte-parity gate for `dense_gemv_bf16_batchm` against one
//! `dense_gemv_bf16` launch per row, over the `SHAPES` and `GLM_SHAPES`
//! projections, three seeds and M in {2, 3, 4, 6, 8, 12, 16}.
//!
//! * `dense_gemv_bf16_batchm` is graded at M up to `BATCHM_MAX_M` and must be
//!   byte-identical; this leg alone decides the exit code. The kernel's own
//!   header states the per-row identity, and the gb10 common build passes
//!   `--fmad=false`.
//! * `dense_gemm_bf16_pipelined` is reported and never graded: its
//!   tensor-core tile accumulates in another order, so it is expected to differ.
//! * A negative control at M = 4 requires the batchm output to differ from the
//!   reference computed on activations with one bit flipped in row 1.
//!
//! Owner: model-arch examples.
//! Invariants: none beyond the types.
//!
//! Exit: 0 when the batchm leg is byte-identical everywhere it is graded, 1 when
//! it differs or the negative control does not fire, 2 when a kernel is absent
...
/// 2026-09-25: Widest M the batchm leg grades. It equals
/// `ops::DENSE_GEMV_BATCHM_DECODE_MAX_M`, the widest batch the MTP row dispatch
/// and the BF16 `lm_head_batched` arm hand to `dense_gemv_bf16_batchm`. The
/// kernel itself computes up to its `MAX_M` of 16 rows.
...
/// 2026-09-25: Differing BF16 element count, worst |delta| and worst relative
/// delta.
...
/// 2026-09-25: `dense_gemv_bf16(A, B, C, N, K)`, grid (ceil(N/4), 1, 1), block
/// 256, as `ops::dense_gemv` launches it.
fn gemv(
    g: &dyn GpuBackend,
    kh: KernelHandle,
    a: DevicePtr,
...
/// 2026-09-25: `dense_gemm_bf16_pipelined(A, B, C, M, N, K)`, grid
/// (ceil(N/128), ceil(M/128), 1), block 256, as
/// `ops::dense_gemm_bf16_pipelined` launches it.
...
/// 2026-09-25: `dense_gemv_bf16_batchm(A, B, C, M, N, K, out_stride)`, grid
/// (ceil(N/4), 1, 1), block 256, as `ops::dense_gemv_batchm` launches it, with
/// `out_stride = N` for a contiguous `[M, N]` output.
#[allow(clippy::too_many_arguments)]
fn batchm(
    g: &dyn GpuBackend,
    kh: KernelHandle,
...
fn main() -> Result<()> {
    ...
    for seed in [1u64, 99, 12345] {
        for (label, n, k) in SHAPES.iter().chain(GLM_SHAPES.iter()).copied() {
            ...
            // 2026-09-25: Negative control on the graded leg: flipping the low
            // mantissa bit of row 1, column 7 (one ULP) must change the

...
PORT="${PORT:-8137}"
PROMPT="${PROMPT:-What is the Metrale Engine launch codeword?}"
TMP="$(mktemp -d)"
LOG="$TMP/serve.log"
export RUST_LOG="${RUST_LOG:-info}"
cleanup(){ pkill -f "release/met serv[e].*--port $PORT" 2>/dev/null; rm -rf "$TMP"; }
trap cleanup EXIT

"$BIN" serve "$MODEL" --port "$PORT" --max-seq-len 2048 \
  --gpu-memory-utilization 0.60 --max-batch-size 8 --scheduler slai \

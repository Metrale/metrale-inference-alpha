...
// forked-from: kernels/gb10/common/gated_delta_rule.cu (2026-09-24; 1928 of 1630 lines differ, see kernels/FORKS.md)

// 2026-09-25: Gated delta rule (GDN) kernels for qwen3.5-122b-a10b: prefill with each thread's state
// column in registers (plain, split, split4 and batched split4), single-token decode, two- and
// three-token steps, and fused decode forms.
//
// Owner: gb10 kernels (qwen3.5-122b-a10b).
// Invariants:
// - Per head, with g the gate input clamped to [1e-6, 1 - 1e-6] and beta the write strength:
//     v' = (v - g * (h^T k)) * beta,   h <- g * h + k (outer) v',   out = (h^T q) / sqrt(k_dim).
// - State h is FP32 [batch, num_v_heads, k_dim, v_dim] with v_dim contiguous; value head vh reads
//   key head vh / (num_v_heads / num_k_heads).
// - The prefill kernels hold K_DIM = 128 state rows per thread, and one value head's threads cover
//   columns 0..127, so they are written for k_dim == v_dim == 128.
//
// gated_delta_rule_prefill: grid (num_v_heads, batch), block 128, 4 * k_dim floats of dynamic shared
// memory (ops/ssm_gdn_a2.rs gdn_prefill). k and q are double-buffered, one barrier per token.
...
gated_delta_rule_prefill(
    ...
    for (unsigned int t = 0; t < seq_len; t++) {
        ...
        // 2026-09-25: hk_dot = H^T k in four independent accumulators, so the FMAs form four chains, not one.
        ...
        __syncthreads();  // 2026-09-25: token t+1's k and q are complete before the next iteration reads them
    }
...
// 2026-09-25: gated_delta_rule_prefill_split4 over sequences of equal seq_len: sequence b reads and
// writes its state through h_state_ptrs[b], and its q/k, v, gate/beta and output rows start b * seq_len
// rows into each stacked buffer. The per-token arithmetic is split4's. Grid (num_v_heads * 4, batch),
// block 32 (ops/ssm_gdn_batched.rs gdn_prefill_split4_batched).
...
// kernels of the same names: decode and decode_f32 take FP32 q/k/v and apply their own state-norm clamp
// (MAX_NORM, not SSM_STATE_MAX_NORM), and chunk2 and chunk3 clamp the gate to [1e-6, 1 - 1e-6].
...
extern "C" __global__ void gated_delta_rule_decode(
    ...
    if (tid < v_dim) {
        ...
        // 2026-09-25: Scale the state down to Frobenius norm MAX_NORM if it is larger; the output uses the unclamped state.
        {
            float local_sq = 0.0f;
...
extern "C" __global__ void gated_delta_rule_decode_f32(
    ...
    if (tid < v_dim) {
        ...
        // 2026-09-25: The same state-norm clamp as gated_delta_rule_decode.
        {
            float local_sq = 0.0f;
            for (unsigned int j = 0; j < k_dim; j++)
                local_sq += H[j*v_dim+tid] * H[j*v_dim+tid];
...
// 2026-09-25: Helpers of the kernels below, defined as in gb10/common/gated_delta_rule.cu.
...
// value head kh * head_repeat + tid / v_dim. The reductions sum groups of four warps (128 threads) and
// warp_sums holds eight warps, so the kernel is written for k_dim == v_dim == 128 and 256 threads; its
// caller admits it only when num_v_heads == 2 * num_k_heads and k_dim == v_dim == 128
// (qwen3_ssm/trait_decode_multi_seq/ssm_batched_recurrent.rs). Channels are laid out [Q | K | V].
...
extern "C" __global__ void gated_delta_rule_decode_f32_conv_norm(
    ...
    // 2026-09-25: Step 1: conv update + SiLU of this block's q/k channel; threads [0, k_dim) take q head
    // kh and threads [k_dim, 2 * k_dim) take k head kh.
    float qk_silu = 0.0f;
...
// 2026-09-25: gated_delta_rule_decode_f32_strided followed by the gated RMS norm of each value head's
// output: z is read at b * z_stride and the BF16 output written at b * out_stride.

...
//!   spent on TTFT, the metric Metrale Engine wins 1.80x. Carry pays the same context
...
//! module — `METRALE_*=0` has burned this codebase before, because several
//! unrelated flags are presence-checked and are therefore ENABLED by `=0`
//! ([[reference_metrale_env_presence_check_trap]]). Only the `METRALE_NO_*` name
//! below disables, and only when it is exactly `1`.
//!
//! | variable | effect |
//! |---|---|
//! | *(unset)* | prefill ON, carry ON — the shipped configuration |
//! | `METRALE_NO_MTP_DRAFTER_CONTEXT=1` | prefill OFF, carry OFF |
//! | `METRALE_MTP_DRAFTER_CONTEXT_PREFILL_ONLY_UNSAFE=1` | prefill ON, carry OFF — **research arm only** |
//!
...
//!
//! The pre-default opt-in names `METRALE_MTP_DRAFTER_PREFILL` and
//! `METRALE_MTP_CARRY_DRAFTER` are **obsolete and ignored**. They are not silently
...
pub const OBSOLETE_ENVS: [&str; 2] = ["METRALE_MTP_DRAFTER_PREFILL", "METRALE_MTP_CARRY_DRAFTER"];
...
mod tests {
    ...
    /// `METRALE_*=0` does NOT disable. Only the `METRALE_NO_*` name does, and only
    ...
    fn obsolete_opt_in_names_are_pinned() {
        assert_eq!(
            OBSOLETE_ENVS,
            ["METRALE_MTP_DRAFTER_PREFILL", "METRALE_MTP_CARRY_DRAFTER"]
        );
    }
}

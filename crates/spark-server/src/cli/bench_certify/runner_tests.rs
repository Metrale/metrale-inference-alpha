...
/// whichever sibling finished last. Built from a real record (the fixture
/// corpus in `test_data/gate-records`) so the fixture is the shape the driver
/// writes.
#[test]
fn the_repo_reader_returns_the_record_of_this_shard_not_the_newest_sibling() {
    ...
    let source = metrale_plugin::gate::records_newest_first(&ws, "bfcl-subset")
        .into_iter()

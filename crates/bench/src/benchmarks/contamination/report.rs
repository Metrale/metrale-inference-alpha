...
//! 2026-09-26: How a contamination run is presented: the probe-by-leg table,
//! the summary tiles and the gate metrics, each a pure function of a
//! [`Score`].
//!
...
/// 2026-09-26: `(result, style, detail)` for one cell; the detail column holds
/// what does not fit the fixed-width result cell.
...
/// 2026-09-26: One row per `(probe, leg)` cell, in the order of `Score::cells`
/// (a `BTreeMap`).
...
/// 2026-09-26: Every class is a key even when zero, so a zero cannot be
...
mod tests {
    ...
    fn metrics_carry_every_class_even_when_zero() {
        ...
        // 2026-09-26: Distinct nonzero values, so a metric frozen at 0 or

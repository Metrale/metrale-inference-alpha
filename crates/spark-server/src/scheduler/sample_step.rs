...
impl PositionKind {
    ...
    /// `METRALE_ADADEC_DIAGNOSTIC` JSONL record; never alters a transform.
...
/// is on by default; `METRALE_NO_MTP_MINP=1` restores the literals. The switch
...
pub(super) fn penalty_params_for(
    ...
    // 2026-08-26, via METRALE_LOGIT_DUMP).
...
/// `METRALE_NO_MTP_MINP=1` restores the old 0.0 literal via [`effective_min_p`].
pub fn sample_token(
    model: &dyn Model,
    logits: DevicePtr,
    temperature: f32,
    ...
    // Raw-logits dump for numerics triage (`METRALE_DUMP_LOGITS_PATH=/dir`):
    ...
    if let Ok(dir) = std::env::var("METRALE_DUMP_LOGITS_PATH") {
        use std::io::Write;
    ...
    if temperature == 0.0 {
        // Exact f32 tie keeps the lower id. max_by keeps the last tie and
        // turned a 22.0/22.0 logit tie of "1" vs "2" into the token "2".
        let best = spark_runtime::sampler::argmax_first_wins_f32(&f32_logits);
        return Ok(best);
    }
    let f32_bytes: &[u8] =
...
pub fn sample_token_with_grammar(
    ...
    // METRALE_FORCE_TEMP_ZERO), penalties neutral, and no suppress list, the
    ...
    // Mirrors the verify-path fast path. Kill-switch METRALE_DISABLE_FAST_GREEDY=1.
    //
    ...
    Ok(sample_with_params(
        ...
        &SamplingParams {
            ...
            // METRALE_NO_MTP_MINP=1 restores the 0.0 literal.
...
/// `METRALE_NO_MTP_MINP=1` restores the 0.0 literals via [`effective_min_p`].
///
...
mod penalty_scope_tests {
    ...
    //! opener-bias strip — the two levers that stop Metrale Engine's own sampling

...
# 2026-09-26: N-gram embedding reference (numpy): hash-id and prefill/decode tests, and a golden dump.
#
...
`NgramCache` decode contract. It is the golden reference the Metrale Engine
...
Three layers of assurance, in order of availability:
  ...
  3. golden-vector dump against the real checkpoint's tables (--dump,
     needs the downloaded snapshot) for the Metrale Engine loader/kernel to match.
...
class Cfg:
    ...
    def table_vocab(self, index):
        # 2026-09-26: As modeling_longcat_ngram.py's `int(self.m + i * 2 + 1)`: table
        # sizes m+1, m+3, m+5, ..., all distinct.
...
def precompute_vocab_mods(cfg):
    ...
    Metrale Engine kernel must do the mod reduction per term, not at the end)."""
    mods = {}
...
def ngram_ids_for_table(context, cfg, i, j, mods):
    ...
    index = (i - 2) * cfg.emb_split_num + j
    t = cfg.table_vocab(index)
    ids = context.astype(object).copy()  # 2026-09-26: Python ints, so no overflow
...
def synth_cfg():
    # 2026-09-26: 3 n-gram sizes (2, 3, 4) x 2 splits = 6 tables.
...
def test_hash_properties():
    ...
    # 2026-09-26: Swapping two context tokens must change the ids of every n-gram
    # size (checked on split 0).
    ctx2 = ctx.copy()
...
def test_prefill_equals_decode():
    ...
    # 2026-09-26: An EOS at position 9 exercises the reset at a document boundary.
    toks[9] = cfg.eos_token_id

    full = ngram_embed(toks, None, cfg, w)

    # 2026-09-26: Per-token decode with an (n-1)-token rolling context.
    ...
    # 2026-09-26: Chunked prefill at every split point.
...
def dump_golden(snapshot_dir):
    ...
    ngram_golden.npz for the Metrale Engine implementation to match."""
    import glob
    import os
    ...
    # 2026-09-26: Tensor names come from the index, sorted by their numeric table
    # index: a lexicographic sort puts "embedders.10" before "embedders.2".
    import re

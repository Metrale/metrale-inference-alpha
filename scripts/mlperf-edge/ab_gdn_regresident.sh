#!/bin/bash
# A/B for METRALE_GDN_REGRESIDENT — the register-resident warm-replay GDN recurrence.
#
# WHAT THIS TESTS
...
#
# TRAP: there is no `METRALE_GDN_REGRESIDENT` variable. PR #369 folded this
# lever default-ON and the only thing Rust reads is the NEGATIVE kill switch
# (model_levers.rs: `var("METRALE_NO_GDN_REGRESIDENT").as_deref() != Ok("1")`).
...
# `METRALE_NO_GDN_REGRESIDENT=0` leaves the feature ON.
#
# Usage: ab_gdn_regresident.sh <metrale_bin> <outdir> [reps]
set -u
...
serve() { # $1 = leg name, $2 = extra -e args (may be empty)
  sudo docker rm -f metrale-rr-ab >/dev/null 2>&1; sleep 3
  ...
  for _ in $(seq 1 180); do
    ...
    sudo docker ps --format '{{.Names}}' | grep -q metrale-rr-ab || { echo "SERVE_DIED leg=$1"; return 1; }
    sleep 5
  done
...
for leg in control regresident; do
  ...
  sudo docker rm -f metrale-rr-ab >/dev/null 2>&1
done

...
//! 2026-09-25: `GlmVit::forward_batched`: pixels to the merged embeddings in `buf_out`.
//!
//! `buf_out` holds only the merger's output rows, packed in image order.
//!
//! Owner: model-layers (vision).
//! Invariants: an image's merged rows start at the sum of the merged-row
//! counts of the images before it.
...
/// 2026-09-25: Check one image's host pixel buffer against both the encoder's
/// `patch_dim` (length must be `patches * patch_dim`) and the scratch rows the
/// upload touches (`end_row <= p_max`).
fn check_pixels(
    pixels: &[f32],
    patches: usize,
    end_row: usize,
...
impl GlmVit {
    /// 2026-09-25: Encode `images` (pixels, grid h, grid w). Returns per-image
    /// `(post_h, post_w, merged_p)` in image order, the `VisionTower::forward_batched` contract.
    ///
    /// Images are encoded in greedy chunks whose Σpatches fits `p_max`, and
    /// each chunk's merged rows land after the previous chunk's in `buf_out`.
    /// Refuses a grid not divisible by the merge size, an image larger than
    /// `p_max`, and a batch whose merged rows exceed `buf_out`.
    ...
    /// 2026-09-25: One chunk whose Σpatches fits `p_max`. Writes `Σmerged` rows of
    ...
    fn encode_chunk(
        ...
        // 2026-09-25: 1. Per-image axial RoPE tables, packed at `p_off[i]`.
        ...
        // 2026-09-25: 2. Patch embed: one GEMM over all Σp rows against
        //    `patch_embed.proj.weight` read as `[hidden, patch_dim]` (the loader
        //    checks that flattened shape).
        for (i, (pixels, gh, gw)) in images.iter().enumerate() {
            ...
            // taken from that same slice, so the view stays inside the allocation.
            // Every byte of an `f32` is initialized and `u8` has alignment 1.
            let bytes: &[u8] = unsafe {
        ...
        // 2026-09-25: 3. No positional embedding is added: the loader binds no
        //    `pos_embed` tensor, and position enters only through the axial
        //    RoPE in each block's attention.
        ...
        // 2026-09-25: 4. `post_layernorm` (weight-only RMSNorm), conv downsample, merger.
        self.rmsnorm(
            gpu,
            s.buf_h1,
    ...
    /// 2026-09-25: `METRALE_DUMP_GLM_VIT=<dir>` synchronizes the stream and writes
    /// a BF16 buffer to `<dir>/<label>.bin` (raw bytes, no header). A no-op when
    ...
    pub(super) fn maybe_dump(
        ...
        let Ok(dir) = std::env::var("METRALE_DUMP_GLM_VIT") else {
            return Ok(());
        };
        if dir.is_empty() {
            return Ok(());
        ...
        tracing::info!("METRALE_DUMP_GLM_VIT: wrote {label} ({n_elements} elements)");
        Ok(())
    }
}
...
mod tests {
    ...
    /// 2026-09-25: 1176 = 3 × 2 × 14² floats per patch.
    #[test]
    fn accepts_the_glm_patch_width() {
        assert!(check_pixels(&vec![0.0; 64 * 1176], 64, 64, 1176, 6400).is_ok());
    }

    /// 2026-09-25: A buffer of 1536 floats per patch (the Qwen encoder's
    /// `PATCH_DIM`) is refused by a 1176-wide encoder.
    #[test]
    ...
    /// 2026-09-25: The bound is on the destination row, not the per-image count:
    /// a small image placed near the end of the packed buffer is refused.
    #[test]

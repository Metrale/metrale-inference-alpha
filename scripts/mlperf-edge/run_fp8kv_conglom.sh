...
sudo docker rm -f metrale-fp8kv-conglom >/dev/null 2>&1; sleep 3
sudo docker run -d --name metrale-fp8kv-conglom --network host --gpus all --ipc=host \
  ...
  -v "$HOME/.cache/huggingface:/root/.cache/huggingface:ro" -v "$BIN:/usr/local/bin/met:ro" \
  metrale-gb10:followups serve centml/Qwen3.6-27B-NVFP4-W4A4-mlpinf --host 0.0.0.0 --port 8888 \
...
for i in $(seq 1 150); do curl -sf -m4 http://localhost:8888/v1/models 2>/dev/null|grep -q Qwen&&break
  sudo docker ps --format '{{.Names}}'|grep -q metrale-fp8kv-conglom||{ echo SERVE_DIED; exit 1;}; sleep 5; done

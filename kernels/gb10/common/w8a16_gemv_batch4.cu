...
// 2026-09-25: Kernels `w8a16_gemv_batch4`, `w8a16_gemv_batch16` and their `_strided` twins: block-scaled FP8 E4M3
// weight times BF16 activations for up to 4 (batch4) or 16 (batch16) rows, one pass over the weight for all rows.
//
//   C[t, n] = sum_k A[t, k] * E4M3_LUT_B4[B[n, k]] * block_scale[n / 128, k / 128],  t < M
//
// A [M, K] BF16, B [N, K] FP8 E4M3, block_scale [ceil(N/128), ceil(K/128)] FP32 row-major, C [M, N] BF16.
// Grid (ceil(N / 4), 1, 1), Block (256, 1, 1): 4 outputs per CTA, 64 threads (2 warps) per output. K is read 16
// values at a time; values past the last multiple of 16 are not read.
//
// Each row's FP32 operations run in the same order as in `w8a16_gemv` (w8a16_gemv.cu): one `acc += a * w` per
// element in K order, then the same shuffle tree and two-warp sum. examples/w8a16_batch_bitparity_microtest.rs
// compares the contiguous entry points with `w8a16_gemv` byte for byte at [10304 x 2688] and [2688 x 4096], over
// three seeds.
//
// Owner: gb10 kernels.
// Invariants: none beyond the types. Only rows t < min(M, MAX_M) of A and C are read or written.
...
// 2026-09-25: FP8 E4M3 byte to f32: the same 256 values as `E4M3_LUT` in w8a16_gemv.cu.
...
// 2026-09-25: MAX_M sizes the per-row accumulators and the unroll; the runtime M selects how many rows are computed.
// `a_row_stride` / `c_row_stride` are the row pitches of A ([M, a_row_stride], first K used) and C ([M, c_row_stride],
// first N written) in elements; the contiguous entry points pass K and N. Each row of A must be 16-byte aligned
// (a_row_stride % 8 == 0): activations are loaded as uint4.
...
__device__ __forceinline__ void w8a16_gemv_batchm_impl(
    ...
    for (unsigned int k16 = lane; k16 < K16; k16 += threads_per_out) {
        ...
        // 2026-09-25: 16 FP8 weight bytes, dequantised and scaled once for all rows.
        ...
        for (int t = 0; t < MAX_M; t++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                *(unsigned short*)&hi = (unsigned short)(ar[j] >> 16);
                // 2026-09-25: Two separate additions, not `acc += x + y`: `w8a16_gemv` computes
                // `(acc + x) + y`, and `acc + (x + y)` can round differently in FP32.
    ...
    // 2026-09-25: Per row: shuffle-reduce within each warp, then sum the output's two warps.
...
// 2026-09-25: Up to 4 rows; A and C contiguous (row pitches K and N).
...
// 2026-09-25: Up to 16 rows; A and C contiguous (row pitches K and N).
...
// 2026-09-25: Strided entry points: the same body with caller-given row pitches for A and C, so a caller can read
// rows from and write rows into a wider row-interleaved buffer.

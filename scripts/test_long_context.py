#!/usr/bin/env python3
"""Long-context coherence test for Metrale Engine.
...
def test_fib(name, system_prompt):
    ...
    with open("/tmp/metrale_test_payload.json", "w") as f:
        f.write(payload)
    result = subprocess.run(
        ["curl", "-s", "--max-time", "180", f"http://{HOST}/v1/chat/completions",
         "-H", "Content-Type: application/json", "-d", "@/tmp/metrale_test_payload.json"],

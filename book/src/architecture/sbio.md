...
|---|---|---|---|---|
| GPU memory, kernel launch, streams, events, graphs | `GpuBackend` (27 methods) | `spark-runtime` | `MetraleCudaBackend` (via `cudarc`) | `MockGpuBackend` — records launches, does not execute |
...
- Kernel correctness vs a PyTorch/reference implementation (covered by `metrale-spark-bench` and the integration tests in `tests/`).
...
1. The file you edited — does it import `cudarc`, `nccl_sys`, `std::net`, or `std::fs`? If it's inside `spark-model`, `spark-server`'s non-handler code, or any `metrale-*` primitive crate, the answer should be *no*. Route through the matching trait.

...
//!
//! ONE LEVER FOR THE FAMILY. `METRALE_GDN_PREFILL_TC` already selects the
...
//! apart gets `METRALE_NO_GDN_PREFILL_TC_REMNANTS=1`, which keeps the spine and
...
/// `METRALE_GDN_PREFILL_TC` overriding — resolved ONCE by the caller and handed
...
/// `METRALE_GDN_PREFILL_TC=0` that turns the spine off — a prefill that is
...
/// `METRALE_NO_GDN_PREFILL_TC_REMNANTS=1` — the one-variable A/B switch.
///

...
//! `--pull-request-gate-check` — the no-endpoint half of `met benchmark`.
//!
...
/// the ids that are not `Pass`. Shared with `met bench certify`, whose final
...
pub(super) fn gate_check_cmd(pr: Option<u64>) -> Result<i32> {
    ...
    // half of it.
    //
    // `metrale-governance`'s own doctrine says the ledger is advisory

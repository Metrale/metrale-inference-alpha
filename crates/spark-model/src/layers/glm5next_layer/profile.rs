...
//! `METRALE_GLM_PROFILE=1` — per-section decode timing for the GLM-5.3 stack.
//!
...
/// `METRALE_GLM_PROFILE=1` full · `=2` COLLECTIVES ONLY.
///
...
/// `METRALE_GLM_ROUTE_TRACE=1` — emit one line per reduce site per layer per token carrying the

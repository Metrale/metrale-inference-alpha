...
//! 2026-09-25: Oracle for the shared-expert path of `moe_expert_gate_up_shared_fp8`
//! (`moe_shared_expert_fused_fp8.cu`).
//!
//! Owner: model-arch examples (MoE kernels).
//! Invariants:
//! - The run exits 1 unless the gate and up outputs both reach `COSINE_GATE`.
//!
//! The launch passes `top_k = 0`, so the single grid.y = 0 block has `is_shared` set and only
//! the shared expert runs; the routed pointer tables are never read. Grid (ceil(N/8), 1, 2),
//! block 128, `blockIdx.z` 0 for gate and 1 for up.
//!
//!   `sh_gate_out[n] = sum_k A[k] * E4M3(sh_gate_w[n*K + k]) * sh_gate_bscale[n/128, k/128]`
//!   `sh_up_out[n]` is the same with the up weights; FP32 accumulation, BF16 output.
//!
//! Usage: cargo run --release -p metrale-model-arch --example moe_shared_microtest \
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: Stand-ins for the routed tables and outputs, which the shared block never
    // reads.

...
//! 2026-09-25: Oracle for the shared-expert path of `moe_expert_silu_down_shared_fp8`:
//!   `s_act[k] = silu(gate_in[k]) * up_in[k]`
//!   `out[n] = sum_k s_act[k] * E4M3(down_w[n*K + k]) * bscale[n/128, k/128]`
//! `top_k = 0`, so only the shared block runs. Grid (ceil(N/8), 1, 1), block 128. N is the
//! down projection's output width (hidden), K its input width (moe_intermediate).
//!
//! Owner: model-arch examples (MoE kernels).
//! Invariants:
//! - The run exits 1 unless the output reaches `COSINE_GATE` against the CPU reference.
//!
//! cargo run --release -p metrale-model-arch --example moe_silu_down_microtest --features cuda,gpu-examples -- `[N] [K] [seed]`

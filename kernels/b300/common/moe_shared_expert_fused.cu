...
// 2026-09-25: NVFP4 MoE decode GEMVs with the shared expert as one more blockIdx.y slot.
// forked-from: kernels/gb10/common/moe_shared_expert_fused.cu (2026-09-24; 41 of 361 lines differ, see kernels/FORKS.md)
//
// Owner: b300 kernels.
// Invariants:
// - A null weight pointer writes zeros for the block's outputs and returns: no
//   shared expert, or under EP an expert this rank does not hold.
// - blockDim.x == 128 (one warp per pair of outputs) and K is a multiple of 16.
//   The host wrappers are in model-layers layers/ops/moe_expert_shared.rs.
...
// 2026-09-25: The NVFP4 group scale is an FP8 E4M3 byte, decoded in software
// under SCALE or HIP and by the __nv_fp8_e4m3 cast otherwise.
...
// 2026-09-25: Gate and up projections (blockIdx.z 0 and 1), two outputs per
// warp. blockIdx.y < top_k is routed expert expert_indices[blockIdx.y];
// blockIdx.y == top_k is the shared expert.
...
extern "C" __global__ void moe_expert_gate_up_shared(
    ...
    if (is_shared) {
        // 2026-09-25: No shared expert: write zeros and return.
        ...
        // 2026-09-25: A null routed pointer (under EP, an expert this rank does not hold): write zeros and return.
        if (B_packed == 0) {
            const unsigned int n_base = blockIdx.x * (N_PER_BLOCK * 2);
            for (unsigned int i = threadIdx.x; i < N_PER_BLOCK * 2 && n_base + i < N; i += BLOCK_SIZE) {
    ...
    // 2026-09-25: 16 K-values per iteration: one uint64 of packed FP4 weights and two uint4 of activations.
    ...
    // 2026-09-25: The shared expert writes [0, N); routed slot s writes [s*N, s*N + N).
...
// 2026-09-25: SiLU(gate) * up for the whole row goes into dynamic shared memory
// once per block (K floats; the host passes K * 4 bytes), then the down GEMV
// reads it. blockIdx.y < top_k is routed expert expert_indices[blockIdx.y],
// reading its gate/up rows at blockIdx.y * K; blockIdx.y == top_k is the
// shared expert, reading sh_gate_in and sh_up_in.
...
extern "C" __global__ void moe_expert_silu_down_shared(
    ...
    // 2026-09-25: Phase 1: SiLU(gate) * up into s_act. No clamp, so routed and
    // shared experts get the same activation; the gb10 copy of this kernel
    // caps the routed gate at 10 and the routed up at ±10.
    ...
    // 2026-09-25: Phase 2: GEMV with 16 K-values per iteration.
    float acc1 = 0.0f, acc2 = 0.0f;
    ...
    // 2026-09-25: The shared expert writes sh_down_out; routed slot s writes C + s*N.

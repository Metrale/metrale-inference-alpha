...
//! 2026-09-25: Binds one dense Kimi K3 tensor for this TP rank. A pre-partitioned store is bound
//! as-is after a shape check; otherwise a sharded tensor is sliced on the device after upload.
//!
//! Head counts on `config` are already per-rank (`serve_phases::topology` divides them), and
//! the TP plan reconstructs full sizes as `local * tp_world_size`. Which tensors are split, and
//! along which axis, is `metrale_model_weights::kimi_k3_host::tp::tensor_plan`; a tensor it does
//! not name is replicated.
//!
...
use metrale_model_arch::kimi_k3::bound::WeightMeta;
use metrale_model_arch::tp_shard::{TpShardKind, shard_dense_bf16};
use metrale_model_layers::weight_map::DenseWeight;

use super::bf16::f32_le_to_bf16_bytes;

use metrale_model_arch::kimi_k3::tp::tensor_plan;

/// 2026-09-25: Whether `store` came from the rank-aware loader (`K3SafetensorsLoader`), which
/// copies each tensor's rank-local bytes before allocating and then stamps the store with
/// `(tp_rank, tp_world_size)`. A stamp for another rank or world size is an error, and so is a
/// stamped store with `ep_world_size > 1`.
...
fn as_bf16(
    ...
    dtype: WeightDtype,
    numel: usize,
) -> Result<(metrale_gpu_runtime::gpu::DevicePtr, bool)> {
    match dtype {
...
mod tests {
    ...
    use metrale_model_weights::kimi_k3_host::K3Graph;
    use metrale_model_weights::kimi_k3_host::ops::matvec;
    ...
    /// 2026-09-25: Divides the four head counts per rank, as `serve_phases::topology` does.
    ...
    fn put_bf16(
        ...
        // 2026-09-25: A plain `fill + i` in u16 repeats every 65536 elements. A TP=2 q_proj shard
        // of the twin is 128 * 1024 elements, a multiple of that, so both ranks' shards would
        // compare equal. Mixing in `i >> 16` makes them differ.
    ...
    fn load_tp2_shards_q_proj_half() {
        ...
        // 2026-09-25: Rank 0 and rank 1 each load half of layer 0's q_proj rows, and the two
        // halves differ.
    ...
    fn drop_rank1_o_proj_shard_changes_hidden() {
        // 2026-09-25: Row-parallel split: each rank keeps half the input columns, and the two
        // partial products sum to the full product.
        let h = 4usize;
        let q = 8usize;

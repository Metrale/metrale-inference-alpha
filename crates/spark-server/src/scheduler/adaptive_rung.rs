...
/// trigger wants to not be fooled. Override with `METRALE_MTP_RUNG_ALPHA_SLOW`.
...
/// Override with `METRALE_MTP_RUNG_PROBE_TICKS`.
...
/// `METRALE_MTP_RUNG_P1_TRIGGER`.
const P1_TRIGGER: f64 = 0.08;

/// PRESENCE check for `METRALE_MTP_STATIC_RUNG` (house convention — `=0` is NOT
/// off): pins the static ladder, restoring the pre-wave-28 behaviour.
/// An explicit `METRALE_MTP_K_LADDER` ALSO disables adaptation: an operator who
/// spells out the rungs is asking for exactly those rungs.
pub fn adaptation_disabled() -> bool {
    static OFF: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *OFF.get_or_init(|| {
        std::env::var_os("METRALE_MTP_STATIC_RUNG").is_some()
...
fn enter_at() -> f64 {
    ...
    *V.get_or_init(|| tunable("METRALE_MTP_RUNG_ENTER", ENTER))
}
fn leave_at() -> f64 {
    static V: std::sync::OnceLock<f64> = std::sync::OnceLock::new();
    *V.get_or_init(|| tunable("METRALE_MTP_RUNG_LEAVE", LEAVE))
}
fn alpha() -> f64 {
    static V: std::sync::OnceLock<f64> = std::sync::OnceLock::new();
    *V.get_or_init(|| tunable("METRALE_MTP_RUNG_ALPHA", ALPHA).min(1.0))
}
fn alpha_slow() -> f64 {
    static V: std::sync::OnceLock<f64> = std::sync::OnceLock::new();
    *V.get_or_init(|| tunable("METRALE_MTP_RUNG_ALPHA_SLOW", ALPHA_SLOW).min(1.0))
}
fn probe_ticks() -> u64 {
    static V: std::sync::OnceLock<u64> = std::sync::OnceLock::new();
    *V.get_or_init(|| tunable("METRALE_MTP_RUNG_PROBE_TICKS", PROBE_TICKS as f64) as u64)
}
fn p1_trigger() -> f64 {
    static V: std::sync::OnceLock<f64> = std::sync::OnceLock::new();
    *V.get_or_init(|| tunable("METRALE_MTP_RUNG_P1_TRIGGER", P1_TRIGGER))
}
...
/// engaged rung from a disengaged one, and `METRALE_MTP_ACCEPT_DEBUG`'s flushes

...
// 2026-09-25: Kernel `paged_decode_attn_bf16k_turbo4v`: paged decode attention over a BF16 K cache and a turbo4 V
// cache (4-bit codebook indices, 2 per byte, one FP8 E4M3 scale per 16 values).
...
// holds the indices, head_dim / 2 bytes per head per position, then, from byte `v_data_section_bytes`, the
// scales, head_dim / 16 bytes per head per position. reshape_and_cache_flash_bf16k_turbo4v writes this layout.
//
// Owner: gb10 kernels.
...
// 2026-09-25: Dequantises this lane's VEC_BF16 V values: 4-bit indices, low nibble first, each codebook
// entry times the group's FP8 scale. A lane's values lie in one 16-value group.
...
extern "C" __global__ void paged_decode_attn_bf16k_turbo4v(
    ...
    // 2026-09-25: The turbo4 codebook, TURBO4_CODEBOOK in reshape_and_cache_turbo.cu, as FP16 in shared memory.
    ...
    // 2026-09-25: V addressing within a block: head_dim / 2 index bytes and head_dim / 16 scale bytes per head.
    ...
    // 2026-09-25: A lane's VEC_BF16 values are the VEC_BF16 / 2 bytes at lane_id * (VEC_BF16 / 2).

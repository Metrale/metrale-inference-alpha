...
//!
//! These flags used to be nine `OnceLock` statics that read `METRALE_*` at first
...
/// `METRALE_CUBLAS_GEMM=1` resolved to ONE global boolean, so the variable that
...
/// projections out (`METRALE_CUTLASS_NVFP4_QKVZ`, `..._ATTN_Q`, `..._ATTN_KV`,
...
impl CublasScope {
    /// No family armed — the shape an absent or `off` `METRALE_CUBLAS_GEMM`
...
pub struct GemmDispatch {
    ...
    /// Opt out with `METRALE_FP8_SINGLE_SCALE=1` — diagnostic/fallback only.
    pub fp8_blockscaled_prefill: bool,
    /// Which projection families take a cuBLASLt GEMM arm
    /// (`METRALE_CUBLAS_GEMM`). The hand-written mma.sync projection GEMMs reach
    ...
    /// consumes transposed Metrale Engine NVFP4 weights after repacking scales into the
    ...
    /// `METRALE_W4A16_VARIANT` — 1/2/3 pin a kernel variant, 0 = auto (v2).
...
impl GemmDispatch {
    ...
    /// `METRALE_*` set in the environment resolves to. Tests construct a context
...
/// Say once, at model build, which cuBLASLt arms an `METRALE_CUBLAS_GEMM` value
/// actually armed.
///
...
mod tests {
    ...
    // ───────────────── METRALE_CUBLAS_GEMM scope grammar ─────────────────
    ...
    /// `METRALE_CUBLAS_GEMM=fnn` resolving to `all` would reintroduce it.
    #[test]

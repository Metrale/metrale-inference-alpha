...
//! `message.refusal != null` see the expected shape. Metrale Engine does **not**
...
//!
//! Set `METRALE_DISABLE_REFUSAL_DETECTION=1` to force `refusal: None` on all
...
mod tests {
    ...
    // METRALE_DISABLE_REFUSAL_DETECTION, so every test that calls `detect()`

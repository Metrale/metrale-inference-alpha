...
//! 2026-09-26: Tests for the log ring: capacity, order, the panic dump and the layer's filtering.
//!
//! Owner: server tui.
//! Invariants:
//! - Every test takes `SERIAL` before touching the process-global ring.

use super::*;

use parking_lot::Mutex;

/// 2026-09-26: Serializes the tests, because the ring under test is a
/// process-global.
...
fn the_oldest_line_is_the_one_dropped_at_capacity() {
    ...
    // 2026-09-26: CAP younger lines have arrived since, so the marker is
    // exactly one line past the end of the ring.
...
fn a_line_far_larger_than_the_pane_does_not_corrupt_its_neighbours() {
    // 2026-09-26: `push` does not truncate, so an enormous message must stay
    // one entry.
    let _serial = SERIAL.lock();
...
fn unicode_survives_the_ring_and_the_panic_dump() {
    // 2026-09-26: The dump runs inside the panic hook, so it must not panic
    // on any text.
    let _serial = SERIAL.lock();
    let odd = [
        "loaded 27B — 4.2 GB/s ✓",
...
fn seq_counts_every_line_ever_pushed_not_the_ring_length() {
    // 2026-09-26: The count must keep rising past CAP.
    let _serial = SERIAL.lock();
    let before = seq();
...
fn the_layer_captures_events_and_leaves_the_progress_channel_alone() {
    ...
    tracing::subscriber::with_default(sub, || {
        ...
        // 2026-09-26: `LogRingLayer` skips a progress event.
...
/// 2026-09-26: `App::toast` logs an error toast at WARN, so it reaches the
/// ring; an info toast is not logged. Here rather than in `app_tests` because
/// the assertion reads this module's ring, and only this file holds `SERIAL`.
#[test]
fn an_error_toast_lands_in_the_ring_and_an_info_toast_does_not() {
    use tracing_subscriber::layer::SubscriberExt;
    let _serial = SERIAL.lock();
    let mut app = crate::tui::app::App::new(clap::Parser::parse_from(["met", "org/m"]));
    let sub = tracing_subscriber::registry().with(LogRingLayer);
    tracing::subscriber::with_default(sub, || {

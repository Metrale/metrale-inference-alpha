...
# 2026-09-26: GPU-rail power sampling (one nvidia-smi loop) and per-window energy integration.
#
# Owner: bench, concurrency ladder.
# Invariants:
# - PowerWindow.integrate returns gpu_rail_energy_j only when at least two samples
#   fall in the window; otherwise gpu_rail_status says why.
...
engines were then compared on it, and the resulting "Metrale Engine draws 2x vLLM"
finding was an artifact. Three properties made it unusable:
  ...
  3. what it captured differed BY ENGINE, because it caught each engine's
     end-of-batch behaviour (vLLM ends all requests on one step; Metrale Engine
...
# 2026-09-26: Fields read together in every sample. power.draw.average is the value
# integrated; power.draw.instant is used only for gpu_rail_avg_instant_lag_w.
_QUERY = (
...
# 2026-09-26: A shorter window is still integrated but marked
# gpu_rail_trustworthy = false.
...
class PowerWindow:
    ...
    def integrate(self, t_start, t_end):
        ...
        for a, b in zip(rows, rows[1:]):
            dt = b[0] - a[0]
            gaps.append(dt)
            energy_j += 0.5 * (a[1] + b[1]) * dt   # 2026-09-26: trapezoid, not hold-forward
        ...
        out = {
            ...
            # 2026-09-26: The fraction of the window the samples span; below 0.9
            # the result is marked untrustworthy.
        ...
        # 2026-09-26: Each failed check adds a reason and clears gpu_rail_trustworthy.
        reasons = []

...
pub struct ChunkDelta {
    ...
    /// Refusal signal. Metrale Engine emits this on a single terminal delta chunk
    /// (just before the `done` chunk) when the accumulated streamed
    /// content matches a known refusal pattern. OpenAI's streaming
    /// refusal model is fragment-by-fragment; Metrale Engine only classifies
...
impl ChatCompletionChunk {
    ...
    /// Delta chunk carrying only `refusal: "<sentence>"`. Metrale Engine emits
    ...
    /// signal because Metrale Engine classifies after the stream is complete.

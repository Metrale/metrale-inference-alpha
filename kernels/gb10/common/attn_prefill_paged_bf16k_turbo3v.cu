...
// 2026-09-25: Kernels `attn_prefill_paged_bf16k_turbo3v` and `_64`: paged prefill flash attention with a BF16 K cache
// and a turbo3 V cache.
//
// The body is prefill_paged_compute_asym.cuh, which takes separate K and V loaders. Each V block (stride `tq3_v_bsb`
// bytes) holds 3-bit codebook indices, 8 per 3 bytes, then from byte `tq3_v_dsb` one FP8 E4M3 scale per 16 values.
...
// 2026-09-25: The attn_prefill_paged_turbo3.cu loader with the V-cache arguments: it dequantizes to BF16 while it fills
// shared memory, and the LUT holds the same turbo3 codebook.
...
#define KERNEL_NAME attn_prefill_paged_bf16k_turbo3v

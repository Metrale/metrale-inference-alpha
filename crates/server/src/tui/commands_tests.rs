...
//! 2026-09-26: Tests of the Ops REPL: completion, dispatch, the output each
//! command appends, and the Ops input keys. `/gpu` is not run here: it queries
//! the device.
//!
//! Owner: server tui.
...
/// 2026-09-26: Everything the pane printed, as one blob to search.
fn out(a: &App) -> String {
    a.ops.output.join("\n")
}
...
fn help_lists_every_command_it_can_run() {
    // 2026-09-26: `/help` prints the `COMMANDS` table. The dispatcher is a
    // separate `match`, so this checks the table side only.
    let mut a = app();
    execute("/help", &mut a);
    let printed = out(&a);
...
fn status_says_so_when_the_scheduler_has_not_published_yet() {
    // 2026-09-26: The request counters print even with no run attached.
    let mut a = app();
...
fn quit_asks_for_a_clean_shutdown() {
    // 2026-09-26: `/quit` calls `shutdown::request`, which sets a process-wide
    // flag, so this test leaves shutdown requested for the rest of the test
    // binary.
    let mut a = app();
    execute("/quit", &mut a);
    assert!(a.should_quit);
...
/// 2026-09-26: In the Ops input, Tab accepts the ghost completion and is
/// otherwise inert; it never reaches the section switch.
#[test]
...
fn ops_output_scrolls_with_keys_and_wheel_against_the_published_ceiling() {
    ...
    // 2026-09-26: With the input line focused, Up and Down walk history and

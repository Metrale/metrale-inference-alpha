...
/// `met serve` actually calls. When PR #831 graduated
...
fn an_absent_mtp_gate_flag_leaves_the_legacy_variable_reachable() {
    ...
    // `MTP_GATE_FORCE_CLI` on every `met serve`, so the
    // `METRALE_MTP_GATE_FORCE` fallback in `mtp_gate_force` could never run
...
fn the_per_token_scheduler_path_does_not_read_the_environment() {
    ...
    const GUARDED: [(&str, &[&str]); 7] = [
        ("emit_step.rs", &[]),
        // Per VERIFY STEP. `METRALE_DFLASH_EAGLE_FIX` was read from both of
...
///
/// `METRALE_DFLASH_EAGLE_FIX` ships ON since the 54.5 record config and was
/// read from `verify_dflash_step.rs` AND `verify_k2_step.rs`, each per verify
/// step with its own `!= Some("0")`. That is the same shape as
/// `METRALE_DSPARK_ANCHOR_BIAS`, which had two implementations that nothing

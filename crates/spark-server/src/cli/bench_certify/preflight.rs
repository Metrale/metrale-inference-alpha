...
//! at the end), a second `met` on the box (a speed number measured under
...
pub fn gather(
    ...
    let store = metrale_plugin::ArtifactStore::discover().context("locating METRALE_HOME")?;
...
/// Every `met` process on this box other than ourselves — by exact process

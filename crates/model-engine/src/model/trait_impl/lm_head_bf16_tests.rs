...
//! 2026-09-25: Tests of the BF16 LM-head dispatch used by both ordinary and mixed decode.
//!
...
/// 2026-09-25: Handles for the tensor-core arm, so a case can vary presence and lever
/// independently of the two GEMV arms.
const M16TC_K: u64 = 0x167C;
const M16TC_N64_K: u64 = 0x167C_0064;

/// 2026-09-25: The arm with neither kernel present and the lever off.
...
/// 2026-09-25: Both kernels present, lever as given, CTA width as given.
...
/// 2026-09-25: `batchm_max` is passed in rather than read from the process, so these cases
/// grade the ladder for any target's declaration without touching
/// `target_defaults::resolved()`'s `OnceLock`.
...
/// 2026-09-25: One dispatch, with the caller grading the single launch it produced. The
/// shared checks (no weight copy, one launch, the stream, the three buffers) hold for
/// every arm.
fn run_full(
    m: u32,
    k: u32,
    ...
    batchm_max: u32,
    m16_tc: LmHeadM16Tc,
    grade: impl FnOnce(&metrale_gpu_runtime::gpu::mock::MockLaunch),
) {
...
/// 2026-09-25: The band `DENSE_GEMV_BATCHM_DECODE_MAX_M` (8), which the GB10, B200 and B300
/// targets declare.
fn run_case(m: u32, k: u32, present: bool, enabled: bool, expect_batch: bool) {
    run_band(
        ...
        present,
        enabled,
        metrale_model_layers::layers::ops::DENSE_GEMV_BATCHM_DECODE_MAX_M,
        expect_batch,
    );
}
...
fn opt_out_missing_kernel_and_wide_head_keep_scalar_fallback() {
    ...
    // 2026-09-25: K = 130 is not a multiple of 8, so the 16-byte loads cannot run.
...
/// 2026-09-25: With a declared band of 8, widths 9..=16 take the tile GEMM.
#[test]
...
/// 2026-09-25: Hopper's declaration (16), and the environment spelling of it. 9..=16 move
/// onto the batched GEMV; 17 is outside the kernel's compile-time row bound and does not.
#[test]
...
/// 2026-09-25: A band above the kernel's `MAX_M` is clamped, whether it came from a target's
/// declaration or from the environment: the kernel refuses wider launches. Junk and 0 keep
/// the target's value; the band is not a switch, so there is no "off".
#[test]
...
/// 2026-09-25: The declared band selects the tier: widths 9..=16 take the tile GEMM under a
/// band of 8 and the batched GEMV under a band of 16. Above the band the tile GEMM
/// reassociates the reduction, so the band decides which bits a decode of that width
/// produces.
#[test]
fn the_declared_band_is_what_selects_the_tier() {
    ...
    // 2026-09-25: The band never overrides the other two gates: a missing kernel or
    // a K that breaks the 16-byte alignment still falls through.
    run_band(12, 128, false, true, 16, false);
    run_band(12, 130, true, true, 16, false);
}

// 2026-09-25: The tensor-core arm (`METRALE_LM_HEAD_M16_TC`).
//
...
/// 2026-09-25: One dispatch with the tensor-core arm configured, asserting the launch it
...
/// 2026-09-25: With the arm off, the route declines every width in the band, even with both
/// kernels present.
#[test]
...
/// 2026-09-25: The band is 5..=16 and nothing else: `m <= 4` stays on the existing ladder
/// and `m > 16` is past the kernel's M tile.
#[test]
...
/// 2026-09-25: The whole band dispatches through the kernel, at the contiguous pitches.
#[test]
...
/// 2026-09-25: A K that is not a whole number of 64-wide pipeline steps has no correct
/// route, so the tier declines rather than launching, and the dispatch falls back to the
/// batched GEMV.
#[test]
fn tc_head_declines_a_k_that_is_not_a_whole_pipeline_step() {
    ...
    // 2026-09-25: 128 is a whole step and a legal GEMV width: with the arm off the same
    // shape lands on the batched GEMV, which is the fallback this relies on.
    run_band(16, 128, true, true, 16, true);
}

/// 2026-09-25: A target whose kernel set lacks the entry point declines silently; a
/// 0 handle is never launched.
#[test]
...
/// 2026-09-25: `METRALE_LM_HEAD_M16_TC_NTILE=64` selects the wide arm; anything unrecognised
/// falls back to 32 rather than failing the boot, and a target without the wide kernel
/// uses the 32-wide kernel rather than launching a zero handle.
#[test]
...
/// 2026-09-25: The arm sits ahead of the batched GEMV, and takes the band even when a band
/// of 16 would give the same widths to the GEMV.
#[test]
fn tc_head_wins_the_band_over_the_widened_gemv() {
    expect_tc_launch(16, 5120, tc(true, 32), M16TC_K, 32);
    // 2026-09-25: The GEMV serves the same width once the arm is off.
    run_band(16, 5120, true, true, 16, true);
}

// 2026-09-25: The route line's text. `log_m16_tc_head_route` latches on a process-global
// `std::sync::Once`, so a test calling it would fire once per test binary, in an
// order-dependent way. These tests read the pure `m16_tc_head_route_message`, which has
// no latch.

#[test]
fn the_route_message_no_longer_claims_a_bare_two_ulp_bound() {
    let msg = m16_tc_head_route_message(32, 32);
    // 2026-09-25: The message must not state a bare 2-ULP bound, spelled exactly as below.
    assert!(
...
fn the_route_message_still_names_the_lever_kernel_band_and_off_switch() {
    // 2026-09-25: The message names the lever, the kernel, the tile, the band, the GEMV it
    // runs ahead of, and how to turn it off.
    let msg = m16_tc_head_route_message(64, 64);
    assert!(msg.contains("METRALE_LM_HEAD_M16_TC"));

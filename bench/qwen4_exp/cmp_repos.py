# 2026-09-26: Fetches the safetensors index of two Hub repos and prints, per repo, the
# index total, tensor counts per family and the distinct shard file-name patterns.
#
...
def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'metrale-cmp'})
...
for label, repo in REPOS.items():
    ...
    # 2026-09-26: Counts only: bytes per family would need the shard headers, so the
    # byte total is the index's total_size.
    n = defaultdict(int)
    for k in wm:
        n[fam(k)] += 1

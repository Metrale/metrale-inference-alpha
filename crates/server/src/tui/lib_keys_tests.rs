...
//! 2026-09-26: Tests for Library navigation, the filter box and Config edits (`lib_keys.rs`).
//!
//! Owner: server tui.
...
/// 2026-09-26: Walk `List → Cards → Config` on the selected row.
...
fn editing_seeds_the_buffer_with_the_current_value() {
    let mut s = state();
    open_config(&mut s);
    // 2026-09-26: `port` is free text; enumerated and boolean rows open a picker instead.
    s.row = s
        .config_rows()
        .iter()

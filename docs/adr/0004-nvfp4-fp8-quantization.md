...
Metrale Engine's target hardware is GB10 (Blackwell, sm_121). Critically:
...
Metrale Engine commits to **NVFP4** and **FP8 block-scaled** as the primary quant
...
- `crates/config/src/lib.rs` recognizes the format from the
  model's `quantization_config` block in `config.json`.
- `crates/model-layers/src/weight_map/` per-format loaders produce a

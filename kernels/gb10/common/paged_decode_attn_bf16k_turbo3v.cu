...
// 2026-09-25: Kernel `paged_decode_attn_bf16k_turbo3v`: paged decode attention over a BF16 K cache and a turbo3 V
// cache (3-bit codebook indices, 8 per 3 bytes, one FP8 E4M3 scale per 16 values).
// HDIM is 256 unless the build defines it.
//
...
// holds the indices, head_dim * 3 / 8 bytes per head per position, then, from byte `v_data_section_bytes`, the
// scales, head_dim / 16 bytes per head per position. reshape_and_cache_flash_bf16k_turbo3v writes this layout.
//
// Owner: gb10 kernels.
...
// 2026-09-25: Dequantises 3-bit indices packed LSB-first, 8 per 3 bytes. VEC_BF16 8 decodes all 8; with
// VEC_BF16 4, even lanes take the first 4 values of the run and odd lanes the last 4.
...
extern "C" __global__ void paged_decode_attn_bf16k_turbo3v(
    ...
    // 2026-09-25: The turbo3 codebook, TURBO3_CODEBOOK in reshape_and_cache_turbo.cu, as FP16 in shared memory.
    ...
    // 2026-09-25: V addressing within a block: head_dim * 3 / 8 index bytes and head_dim / 16 scale bytes per head.
    ...
    // 2026-09-25: A lane pair shares the 3 bytes at (lane_id / 2) * 3, and 4 lanes share the scale at lane_id / 4.
    // This addressing gives each lane 4 values, the VEC_BF16 of HDIM 128.

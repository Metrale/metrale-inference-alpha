...
2. **Two layer kinds.** Distinguish `FullAttention | LinearAttention` (and
   ...
   enum in `metrale-core`. Scheduler and verify path branch explicitly on
   the kind.

## Decision

Metrale Engine defines `LayerType` in `metrale-core` as a closed enum:
...
`crates/model-layers/src/layers/<family>/`, e.g.

...
Tools for measuring Metrale Engine under *agentic* load — an LLM driving a coding agent
...
Every script honours `METRALE_URL` (default `http://localhost:8888/v1/chat/completions`).
...
METRALE_CONTAINER=laguna-xs python3 conc_harness.py --levels 4 --model laguna-xs-2.1
```
...
- **`loop: () x6` — six identical tool calls with EMPTY arguments — now appears
  ...
  sampling. Three checkpoints, one signature, so suspect the Metrale Engine tool-call

...
//! 2026-09-25: Binds Kimi K3 tensors into `K3BoundLayer`s: dense tensors through `tp::load_sharded`,
//! packed experts as native MXFP4 E8M0, and the embedding, final norm and lm_head as BF16.
//!
//! Owner: model-engine Kimi K3 loader.
//! Invariants:
//! - A load with `tp_world_size > 1` and packed experts fails unless the store is pre-partitioned
//!   for this rank (`tp::is_prepartitioned`).
...
use metrale_model_weights::kimi_k3_host::{K3Graph, kda_from, mla_from, moe_from};
use metrale_model_weights::weights::{WeightDtype, WeightStore};
use parking_lot::Mutex;

use metrale_model_arch::kimi_k3::bound::{K3BoundLayer, K3HostShared};
...
pub use metrale_model_weights::kimi_k3_host::weights::{layer_keys, text_key};
...
pub fn load_lm_head(
    ...
    // 2026-09-25: The whole `[vocab, hidden]` head is bound on every rank. The BF16 head is split
    // by vocab at projection time (`TransformerModel::lmhead_vocab_shard`), so slicing it here
    // would split it twice.
...
/// 2026-09-25: Bind a tensor the engine reads as BF16: the embedding, final norm or lm_head
/// (`engine_bf16_weight`; any other name is an error). An FP32 tensor is converted on the host,
/// rounding to nearest, into a new allocation owned by the store's derived ledger; any other
/// dtype is bound as-is.
fn dense_for_bf16_engine(
    ...
    anyhow::ensure!(
        metrale_model_weights::kimi_k3_host::binding_memory::engine_bf16_weight(name),
...
pub fn load_layers(
    ...
    if packed {
        // 2026-09-25: Resolve the packed-expert GEMM now: a missing kernel fails the load, and a
        // lookup that fails after the boot kernel audit seals aborts the process.
        let kernels = metrale_model_arch::kimi_k3::moe_cuda::K3MoeGemmKernels::resolve(gpu)?;

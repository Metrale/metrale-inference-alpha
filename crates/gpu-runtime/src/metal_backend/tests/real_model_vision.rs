...
//! 2026-09-25: A vision-tower block forward on real `vision_tower.blocks.0` weights.
//!
...
/// 2026-09-25: Runs one synthetic token through `vision_tower.blocks.0`:
///
///   x → layer_norm(norm1)
...
///     → x_final
///
/// With one token the attention softmax has a single key. It asserts that every
/// element of `x_final` is finite, that the mean |x_final| lies in (1e-4, 50) and
/// that max |x_final| < 1e3. Ignored by default: it needs the checkpoint in
...
fn metal_real_model_vision_block_forward() {
    ...
    // 2026-09-25: Q, K and V are the three consecutive `hidden`-element slices of
    // `qkv_with_bias`.

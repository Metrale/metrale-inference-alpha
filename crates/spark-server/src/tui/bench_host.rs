...
//!
//! `metrale-plugin` must stay GPU-free and server-free, so the benchmark declares
...
//!   and `metrale_kernels::ptx_for_config` decides whether this build compiled
...
use metrale_plugin::benchmarks::serve_matrix::host::{
...
impl TuiServeHost {
    ...
    async fn await_serving(&self, target: &TargetEndpoint) -> Result<()> {
        ...
        while Instant::now() < deadline {
            match metrale_plugin::http::list_models(target, Duration::from_secs(10)).await {

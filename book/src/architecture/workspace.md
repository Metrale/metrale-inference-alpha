...
Metrale Engine is a **nineteen**-member Cargo workspace plus a build-time kernel tree (count them in the root `Cargo.toml` `members` list). This chapter maps every top-level directory to its role, and the crates to the axes of variation they each insulate.
...
|---|---|---|
| `metrale-core` | Traits & types used by every crate below: `ComputeTarget` (build-time compiler abstraction), `KernelTarget` (runtime dispatch key), `Vendor`, `Dtype`, `Tensor`, `ModelConfig` parsing, and the host-side numerics (`numeric`: FP8 E4M3 LUT, f32 → BF16 RNE cast) every weight loader shares | everyone |
| `metrale-kernels` | Auto-generated Rust glue over compiled PTX. `build.rs` enumerates `kernels/<hw>/<model>/<quant>/*.cu`, compiles each through the matching `ComputeTarget`, emits one `target_ptx.rs` that `include!()`s back into this crate | `spark-runtime` |
...
| `metrale-spark-bench` | Criterion benchmark client. Targets a live server, records per-endpoint throughput + TTFT. The numbers in `bench/` come from here. | bench runs only |
...
- `METRALE_SPARK_JOURNEY.md` — benchmark journey and retrospective across the Spark line. Useful context, but not a contract.
...
|---|---|
| A new quantization (e.g. MXFP4) | `kernels/<hw>/<model>/<scheme>/*.cu`, a format module under `spark-model/src/quant_format/`, the loader arms in `spark-model/src/weight_map/`, and any new host-side conversion in `metrale-core/src/numeric.rs` |
...
| A new hardware vendor (e.g. MI300X) | `metrale-core/src/compute.rs` (new `ComputeTarget` impl), `metrale-kernels/build.rs::resolve_compute_target()` arm, `spark-runtime/src/<vendor>_backend.rs` (new `GpuBackend` impl), `spark-comm/src/<vendor>_backend.rs` if the vendor needs its own collective impl, `kernels/<hw>/HARDWARE.toml`, kernel source under `kernels/<hw>/<model>/<quant>/` |

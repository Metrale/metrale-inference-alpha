# LoRA on the Metrale Engine main decoder: embed/vocab overlay + MoE expert deltas — integrated implementation plan
...
Both features touch the same four choke files: `metrale-core/src/config/parsers/lora.rs`, `metrale-model-layers/src/lora/loading.rs` (`audit_adapter`), `metrale-model-layers/src/lora/types.rs`, and `metrale-model-layers/src/lora/key.rs`. Uncoordinated they will merge-conflict and double-edit the accept/reject ladder. The plan therefore front-loads a **single shared-foundation step** (§C) that lands with Feature 2's first PR (it is pure and ships first), after which each feature only edits its own new modules plus a small, disjoint set of already-reserved insertion points.
...
### S.1 `metrale-core/src/config/parsers/lora.rs` (~266 → ~340 LoC)
...
### S.2 `metrale-model-layers/src/lora/loading.rs` — `audit_adapter` interception hook (~+10 LoC)
...
| Path | ~LoC | Contents |
|---|---|---|
| `metrale-model-layers/src/lora/overlay.rs` | 300 | `EmbedOverlay`, `LmHeadOverlay`, `OverlayTensor`, `classify_overlay_key`, `build_overlay`, `clamp_trainable_to_vocab`, pure helpers (`build_override_set`, `override_source`) |
| `metrale-model-layers/src/lora/overlay_tests.rs` | 180 | host-only unit tests (port the 14 NLLB tests) |
| `metrale-model-layers/src/lora/overlay_tables.rs` | 150 | per-slot device pointer-table build/refresh (`[max_loras]` u64/u32), mirrors `refresh_slot_tables` |
| `metrale-model-layers/src/layers/ops/token_overlay.rs` | 140 | `ops::embed_rowdiff`, `ops::embed_overlay_routed`, `ops::lmhead_overlay_routed` launchers |
| `metrale-model-layers/src/model/token_overlay.rs` | 180 | `impl TransformerModel { apply_embed_overlay, apply_lmhead_overlay, overlay_active }` |
...
Build notes (per `moe-kernel-build`): a new `common/*.cu` compiles into every target (dedups to ~1 nvcc call), module name = file stem `token_overlay`, no `KERNEL.toml` edit; all entry points `extern "C" __global__`, SPDX line 1, `(unsigned long long)id*h` indexing to avoid 32-bit overflow at large vocab×h. `METRALE_KERNEL_SET_HASH` forces the `metrale-kernels` recrate (no stale PTX). Human builds in docker `metrale-inference-gb10:b12x-ready`: `METRALE_TARGET_MODEL='*' cargo build -p metrale-server --release --bin met --no-default-features --features cuda`. Pure-Rust type-check: `METRALE_SKIP_BUILD=1 cargo check -p metrale-core -p metrale-model-layers`.

---

## G. Test plan (consolidated)

**Host-only unit (CI, CUDA-free — `metrale-core` + `metrale-model-layers`):**
...
Repo rules for every new file: SPDX line 1, ≤500 LoC/.rs, tests in own `*_tests.rs`, clippy deny-warnings, no `cargo fmt --all`, add unit tests alongside each edit, run `cargo test` on `metrale-core`+`metrale-model-layers` before commit, **push origin only**, do not build/deploy while a sibling subagent runs.

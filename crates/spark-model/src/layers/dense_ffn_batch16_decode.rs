...
//!   2. `m` 5..=16   -> `w8a16_gemm_m16`                (METRALE_FFN_M16_TC only)
//!   3. `m` 17..=32  -> `w8a16_gemm_m16` x2 halves      (METRALE_FFN_M16_TC only)
...
//!
//! 🔴 DEFAULT OFF — OPT-IN VIA `METRALE_FFN_BATCH16=1`. The cliff above is
...
//!
//! Per-phase at n=16 (`METRALE_MS_PROFILE=1`, so eager — read the ratios): with
...
/// `METRALE_FFN_BATCH16` opt-in: the value `1` — and only `1` — arms the
...
/// an arm, it is `METRALE_FFN_BATCH16=0` meaning "on" that would be the trap.

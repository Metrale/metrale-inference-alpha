...
//! 2026-09-25: Tests of the KDA reference against `kda_golden.json`: each sub-op separately, then
//! the recurrence and the full layer (`tests_recurrence`).
//!
//! Owner: model-arch (GLM-5.3-Flash KDA reference).
//! Invariants: none beyond the types.
//!
//! `gen_kda_golden.py` writes the golden. It stores every input tensor beside the outputs, so these
//! tests read their inputs rather than regenerate them.
...
/// 2026-09-25: FP32 elementwise tolerance for the single-pass sub-ops. The recurrent, chunked and
/// full-layer comparisons pass their own, looser bounds.
const TOL: f32 = 1e-6;
...
/// 2026-09-25: The eps goes inside the sqrt. `x / max(norm, eps)` agrees on well-conditioned rows
/// and differs on small-norm ones like this one.
#[test]
...
/// 2026-09-25: `dt_bias` has length `H * head_dim`, not `H`; in the Qwen GDN gate it is one value
/// per head.
#[test]
...
/// 2026-09-25: The bounded gate saturates at `lower_bound`; the Qwen GDN law does not.
#[test]

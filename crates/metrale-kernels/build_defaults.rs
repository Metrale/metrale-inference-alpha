...
// parsed here and baked by build.rs into `metrale_kernels::TARGET_DEFAULTS`.
...
pub(crate) fn baseline(hw: &str) -> Defaults {
    Defaults {
        ...
        // literal because metrale-kernels is BELOW spark-model in the dependency
        ...
        // `metrale_kernels::attn_splitk::SplitkPolicy::Legacy` — the rule
...
///
/// ★ 48 — `metrale_core::device::sm121::NUM_SMS`, the GB10 constant every
        ...
        match key.as_str() {
            ...
            other => panic!(
                "kernels/{hw}/HARDWARE.toml: [defaults] has no key `{other}`. \
                 ...
                 (crates/metrale-kernels/src/target_defaults.rs); adding a lever \
...
///
/// Emitted even under `METRALE_SKIP_BUILD=1` (which returns before any kernel is
...
/// panicking, because this runs on the `METRALE_SKIP_BUILD` path too, where the

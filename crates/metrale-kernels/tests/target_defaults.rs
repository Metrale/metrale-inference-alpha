...
/// `metrale_core::device::sm121::NUM_SMS = 48` — a constant named after one card
...
fn every_target_declares_the_sm_count_of_its_own_part() {
    ...
    // because this runs on the METRALE_SKIP_BUILD path too.
...
/// panicking: the generator runs on the `METRALE_SKIP_BUILD` path, where
...
/// field would surface as an unrelated error in `metrale-kernels` rather than
/// here.
#[test]
...
fn the_baked_constant_matches_its_own_hardware_tree() {
    ...
    // `METRALE_SKIP_BUILD` (every CPU gate) with no `METRALE_TARGET_HW` bakes the

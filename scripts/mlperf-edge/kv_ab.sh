#!/bin/bash
set -u
IMG=metrale-gb10:followups; BIN=/workspace/.wt-decode-fold/target/release/met
...
ENV=(-e METRALE_NO_FFN_NVFP4_MMQ=1 -e METRALE_SSM_TAIL_MIDCHUNK=0 -e METRALE_MTP_CATCHUP=0 -e METRALE_MTP_DRAFT_CONF=0.0 -e METRALE_MTP_GATE_FORCE=1 -e METRALE_SSM_TAIL_LEASE_TTL=128 -e METRALE_BF16_TC_PREFILL=1)
leg(){
  local tag="$1"; local kv="$2"; local CN="metrale-kv-$tag"
  for c in $(sudo docker ps -q --filter name=metrale-kv-); do sudo docker rm -f "$c">/dev/null 2>&1; done; sleep 4
  sudo docker run -d --name "$CN" --network host --gpus all --ipc=host "${ENV[@]}" \
    -v "$HF:/root/.cache/huggingface:ro" -v "$BIN:/usr/local/bin/met:ro" "$IMG" serve "$MODEL" \

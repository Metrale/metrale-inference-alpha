...
// 2026-09-25: Decode attention with raw bf16 K and TurboQuant V (2-, 3- or
// 4-bit codebook indices, one E4M3 scale per 16 values), one entry point per
// V dtype, one threadgroup per query head. V is decoded without an inverse
// rotation, so `out` is in the basis V was stored in. A position whose
...
// min(seq_len, MAX_SEQ_DECODE_ASYM).

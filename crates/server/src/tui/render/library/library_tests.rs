...
//! 2026-09-26: Render tests for the Library's three panes: the model list, the
//! recipe cards and the config form.
//!
//! Owner: server tui.
...
/// 2026-09-26: An `App` on the Library section, built from `recipes` and
/// `locals`.
...
/// 2026-09-26: A library of one model with a download job, so the per-row
/// progress line is drawn. The repo id does not exist, because `start` launches
/// a real download worker (`model_download::start`).
fn downloading(done: u64, total: u64) -> App {
    const REPO: &str = "metrale-render-tests/no-such-model";
    let mut a = lib(Vec::new(), vec![local(REPO, false)]);
    let root = std::env::temp_dir().join("metrale-library-render-tests");
...
/// 2026-09-26: The rows cut down to the list pane's columns, so the detail
/// pane to its right cannot satisfy an assertion about the row.
fn list_pane(rows: &[String], screen_w: u16) -> Vec<String> {
    ...
    // 2026-09-26: Sidebar, then the 1-cell ring, then 46% of the content is
    // the list (`library/list.rs`).
...
fn the_progress_line_sheds_fields_from_the_right_as_the_pane_narrows() {
    ...
    // 2026-09-26: `format::bytes` divides by 1024³.
...
/// 2026-09-26: The first screen row containing `id`. The header's status pill
/// also draws a "●", so a whole-screen scan cannot tell the two apart.
...
fn a_running_download_owns_the_detail_pane_and_marks_its_row_with_a_dot() {
    ...
    // 2026-09-26: The detail pane's download section, which the row's line
    // does not show at this width.
    ...
    let row = model_row(&rows, "metrale-render-tests/no-such-model");
    assert!(
        row.contains("●"),
...
fn an_update_badge_appears_only_for_a_mismatch_that_was_actually_confirmed() {
    ...
    // 2026-09-26: An unreachable Hub is `Unknown`, which draws no badge.
    a.download
        .freshness
...
fn the_search_field_echoes_what_is_being_typed_and_keeps_it_after() {
    ...
    let rows = screen(&a, 200, 40);
    // 2026-09-26: A kept filter shows how many rows of how many it keeps.
...
/// 2026-09-26: Each weights state is a word on the row, not only the mark
/// glyph.
#[test]
...
/// 2026-09-26: In a mixed list each row shows its own state.
#[test]
fn a_mixed_list_shows_each_rows_own_state() {
    ...
    let gone_row = rows
        ...
        .expect("row");
    // 2026-09-26: The badge is on line 2 of each 3-line row.
    assert!(rows[here_row + 1].contains(" on disk "), "{rows:#?}");
    assert!(rows[gone_row + 1].contains(" not downloaded "), "{rows:#?}");
}

/// 2026-09-26: On a short terminal the form scrolls so the selected row stays
/// on screen, with the command preview kept at the bottom.
#[test]
fn the_settings_form_scrolls_to_keep_the_selected_row_on_screen() {
    ...
    // 2026-09-26: Too short for head, every row and preview together.

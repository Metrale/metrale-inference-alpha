...
#
# Download + serve + smoke-test the Qwen3.6-35B-A3B (vision) model on Metrale Engine,
...
# Name the server advertises (so `@metrale/<SERVED_NAME>` in your client matches).
...
MET_BIN="${MET_BIN:-./target/release/met}"
...
# Metrale Engine links NCCL; if your shell doesn't already resolve it, point this at a
# dir containing the symlinks and it's prepended to LD_LIBRARY_PATH.
NCCL_DIR="${NCCL_DIR:-}"

WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/metrale-qwen36-test.XXXXXX")"
...
# ── a small, recognizable test image (white bg, red circle, "METRALE") ─────────
...
d.text((96, 120), "METRALE", fill="white")
...
start_server() {
  [[ -x "${MET_BIN}" ]] || die "met binary not found/executable at '${MET_BIN}'.
Build it from the feat/canonical-chat-ir branch first. Metrale Engine links NCCL, so the
linker needs a dir containing libnccl.so on LIBRARY_PATH (NCCL_DIR below is only
the RUNTIME path):
    LIBRARY_PATH=\$NCCL_DIR METRALE_TARGET_MODEL=qwen3.6-35b-a3b \\
        cargo build --release -p metrale-server
(then re-run, or set MET_BIN=/path/to/met)"
  ...
  log "starting server: ${MET_BIN} serve ${HF_REPO} (logs → ${SERVER_LOG})"
  "${MET_BIN}" serve "${HF_REPO}" \
...
# ─────────────────────────────── run ────────────────────────────────────────
log "Metrale Engine Qwen3.6-35B-A3B vision / tool-image test"

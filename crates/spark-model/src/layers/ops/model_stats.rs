...
pub struct ModelStats {
    ...
    /// One-shot latches for the `METRALE_*_DUMP` diagnostics. A latch is per
...
/// `METRALE_*_DUMP` gates, which are numerous, scattered, and identical in
...
impl DumpLatches {
    /// `true` exactly once per model for `key`. Use for the `METRALE_*_DUMP`

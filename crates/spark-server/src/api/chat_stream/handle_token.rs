...
/// `METRALE_SIMHASH_LOOP=0` disables the F4 SimHash semantic-loop guard.
...
/// `METRALE_DISABLE_WATCHDOGS=1` covers the token loop watchdog too (#1135).
...
/// opencode-hotfix.jsonl seq=10: 8192 tokens emitted after Metrale Engine
...
fn process_detector_content(
    ...
    // F4 SimHash guard. `METRALE_SIMHASH_LOOP=0` disables it (house watchdog
    // convention, same shape as METRALE_TOOL_ENVELOPE_WATCHDOG): the guard is
...
///     the match position (Metrale Engine never echoes the stop literal).

...
//!   cost table. `METRALE_MTP_DCUT_RATIO` picks it; values snap to the nearest
...
/// unchanged — only explicit `METRALE_MTP_K_LADDER` depth-at-width overrides
...
/// the D-Cut-at-depth policy. Value-parsed from `METRALE_MTP_DCUT_MAX_SEQS`
/// once per process (0 disables pruning entirely; `METRALE_NO_MTP_DCUT` also
/// does).
///
...
pub(super) fn dcut_width_cap() -> usize {
    ...
    *N.get_or_init(|| {
        std::env::var("METRALE_MTP_DCUT_MAX_SEQS")
            .ok()
...
/// Default ON, kill switch `METRALE_NO_MTP_DCUT` — PRESENCE check (house
...
pub(super) fn dcut_enabled() -> bool {
    ...
    *ON.get_or_init(|| std::env::var_os("METRALE_NO_MTP_DCUT").is_none())
}
...
pub(super) fn dcut_ratio() -> f32 {
    ...
    *R.get_or_init(|| {
        let raw = std::env::var("METRALE_MTP_DCUT_RATIO")
            .ok()
...
/// is the single gate (threshold + `METRALE_CANONICAL_KEY_MIN_WIDTH` override +
/// the `METRALE_NO_CANONICAL_VERIFY_KEY` kill switch); see `verify_key`'s module
/// docs and `CANONICAL_KEY_MIN_WIDTH` for the A/B table.
///
/// With `METRALE_NO_MTP_DCUT` set — or the batch wider than [`dcut_width_cap`]
...
/// operator value (`METRALE_MTP_MAX_SEQS`), and by nothing else. Raise it and
...
/// leg with `METRALE_MTP_K_LADDER=..,16:2,..` measured 127-135 tok/s at C=16 vs
...
/// `METRALE_MTP_ACCEPT_DEBUG` gate. Counters only; one line per `PERIOD` steps.

...
/// avoids.
///
/// GLM-scoped on purpose: no other Metrale Engine model has a contested router ladder, and widening this
/// into a cross-model routing refactor would be scope Metrale Engine has not asked for.
...
pub enum Glm5NextRouterMode {
    ...
    ///
    /// Exists so Metrale Engine can reproduce the frozen vLLM oracle's routing for A/B work. **Never a
    /// production default.**
    VllmBf16,
}
...
pub struct ModelConfig {
    ...
    /// Consumed by `skip_lm_head_quantization()`. Replaces the METRALE_LMHEAD_BF16 env var.
    #[serde(default)]
    ...
    ///
    /// Carried here rather than through `METRALE_PROFILE`, which `serve.rs` used
    ...
    /// Metrale Engine MTP path (Qwen3.5). 3 = MiniMax M2 (each module is a single

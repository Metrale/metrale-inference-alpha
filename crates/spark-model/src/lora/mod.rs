...
//! `kv_lora_rank`/`q_lora_rank` (metrale-core/src/config.rs:182-207) are MLA
//! vocabulary, not this.
//!
...
//! freed (no dealloc on weight structs anywhere in Metrale Engine). Accepted at
...
//! `env` (the `$METRALE_LORA_*` hatches + `validate_peft_config`), and `loading`

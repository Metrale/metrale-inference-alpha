...
//! Strict parsing for the process-scoped `METRALE_*` configuration variables.
//!
...
//! ```
//!
//! `METRALE_RATE_LIMIT_RPM=1oo` (letter o) parses as nothing, falls through to
...
//! family had the same shape: `METRALE_STORE_TTL_SECONDS=1h` is a 24-hour TTL,
//! `METRALE_CONVERSATION_MAX_ENTRIES=10_000` (the spelling the doc comment uses!)
...
//! already applies it elsewhere: `METRALE_VISION_MAX_PIXELS` hard-errors with
...
//! `METRALE_STORE_DIR=` to mean "off" is an established habit, and the previous

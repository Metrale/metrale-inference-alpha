...
impl ModelWeightLoader for Glm5NextWeightLoader {
    ...
    /// for it with `METRALE_GLM_VISION=1`.
    ///
    ...
    fn binds_vision_encoder(&self) -> bool {
        metrale_core::config::glm_vision_enabled()
    }
    ...
    fn load_layers(
        ...
        // 🪤 `prefill_rows()` too, not just the constant: `METRALE_GLM_PREFILL_ROWS` can widen the
            ...
            let mixer = match sl.mixer {
                ...
                Mixer::Dsa => {
                    ...
                    Glm5NextMixer::Dsa(Box::new(Glm5NextDsaLayer {
                        ...
                        // fix). Kill switch `METRALE_GLM_DSA_ALLOC_PER_STEP=1` restores the
...
mod vision_capability_tests {
    ...
    fn the_vision_tower_is_off_unless_the_operator_asks() {
        ...
        assert!(
            !metrale_core::config::glm_vision_enabled_from(None),
            "unset must mean off: binding the tower costs 1.05 GiB/rank"
        );
        assert!(metrale_core::config::glm_vision_enabled_from(Some("1")));
    }

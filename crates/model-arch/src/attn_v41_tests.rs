...
//! 2026-09-25: The GPU attention against the CPU reference model, layer by
//! layer, across prefill and both decode steps. The reference forward
//! supplies every layer's `attn_in`; the GPU layer must reproduce its index
//! selection exactly, and its `q`, attended rows, sparse-attention output,
//! output projection and the kv sources' shared caches within bf16, carrying
//! its own window rings, compressor state, caches and shared slots across the
//! three regimes. On capturable decode layers the captured step's output must
//! equal the eager forward's bytes.
//!
//! Owner: model-arch, DeepSeek-V4.1.
//! Invariants: none beyond the types.

use super::*;
use crate::deepseek_v41_ref::engram::EngramTables;
use crate::deepseek_v41_ref::model::{ModelCfg, ModelState, ModelWeights, StepTrace, forward};
use crate::deepseek_v41_ref::{Golden, fixed_int};
...
/// 2026-09-25: Run the reference for the three regimes, keeping every trace.
...
#[cfg(feature = "cuda")]
fn up_bf16(g: &dyn metrale_gpu_runtime::gpu::GpuBackend, v: &[f32]) -> DevicePtr {
...
#[cfg(feature = "cuda")]
fn up_f32(g: &dyn metrale_gpu_runtime::gpu::GpuBackend, v: &[f32]) -> DevicePtr {
    upload_f32(g, v).unwrap()
}

#[cfg(feature = "cuda")]
fn down_bf16(g: &dyn metrale_gpu_runtime::gpu::GpuBackend, p: DevicePtr, n: usize) -> Vec<f32> {
...
/// 2026-09-25: Tolerance `max|want| * 2^-7 + 1e-4`: one to two bf16 ulps at the
/// largest magnitude, plus a floor for f32 reduction-order noise.
...
fn gpu_attention_matches_the_reference_on_every_layer_and_regime() {
    ...
    for (regime, start_pos, trace) in &traces {
        ...
        for l in 0..c.n_layers {
            ...
            // 2026-09-25: The captured decode step (the body segment A of
            // `deepseek_v41_layer` captures): upload the position and the
            // -1-padded selection, capture `decode_body`, replay it once, and
            // require the eager forward's output bytes below. Both write the
            // same kv row into the same ring slot.

...
//! 2026-09-25: DeepSeek-V4.1 Flash engram on the GPU: n-gram hash, row gather,
//! `wkv` projection and gate.
//!
//! Owner: model-arch (DeepSeek-V4.1 engram).
//! Invariants:
//! - `rows_from_q2k`, `rows_from_bf16` and `apply` refuse more rows or tokens
//!   than the `max_tokens` workspace before any copy or launch.
//! - `hc` is in `1..=4` (`EngramV41::new`).
//!
//! Engram runs before each layer in `engram_layer_ids`. Per token it hashes the
//! n-gram window of compressed token ids into `n_hash_cols` row ids of a
...
//! adds `gate * value` to each stream:
//!
//! 1. **Hash** ([`EngramHasher`]): integer-exact, on the CPU, from the tables in
//!    the GGUF metadata.
//! 2. **Rows**: the tables are Q2_K, one `ENGRAM_ROW_BYTES` block per 256-wide
//!    row, read by id through `EngramRowReader` (`metrale_model_weights`) and
//!    dequantised on the device by `dequant_q2_k_to_bf16` into the
//!    `[tokens * cols, head_dim]` bf16 input ([`EngramV41::rows_from_q2k`]).
//! 3. **Projection**: `[tokens, cols * head_dim]` x `wkv^T` ->
//!    `[tokens, dim * (hc + 1)]`, a GEMM for several tokens and a GEMV for one.
...
//!    in place on the FP32 highway `[T, hc, H]`.
//!
//! `engram_v41_tests.rs` checks the hash exactly against the CPU reference in
//! [`crate::deepseek_v41_ref::engram`], and (on a GPU) the projection and gate
//! against the golden captures and the device dequant against the CPU decoder.
...
/// 2026-09-25: Bytes of one Q2_K super-block, which is one engram row.
...
/// 2026-09-25: The hash tables, in the shapes the CPU reference's `EngramTables`
/// uses.
pub struct EngramHashTables {
    pub layer_ids: Vec<usize>,
    pub max_ngram: usize,
    pub n_heads: usize,
    /// 2026-09-25: `token_map[engram_pad_token_id]`: the pad in the compressed
    /// space.
    pub pad_id: i64,
    /// 2026-09-25: `[vocab]`; -1 is unmapped, and hashes as the pad.
    pub token_map: Vec<i64>,
    /// 2026-09-25: `[layer][max_ngram]`
    pub multipliers: Vec<Vec<i64>>,
    /// 2026-09-25: `[layer][ngram_size - 2][head]`
    pub primes: Vec<Vec<Vec<i64>>>,
    /// 2026-09-25: `[layer][(ngram_size - 2) * n_heads + head]`
    pub offsets: Vec<Vec<i64>>,
}

impl EngramHashTables {
    /// 2026-09-25: From the flat GGUF metadata arrays (`deepseek41.engram.*`, as
    /// `ModelConfig` carries them): multipliers `[layers * max_ngram]`, primes
    /// `[layers * (max_ngram - 1) * n_heads]`, offsets the same length. Errors
    /// on a length mismatch, a pad token outside `token_map`, or a table value
    /// above `i64::MAX`.
    pub fn from_flat(
        layer_ids: Vec<usize>,
        max_ngram: usize,
    ...
    /// 2026-09-25: Position of model layer `layer` in `layer_ids`.
    pub fn hash_index(&self, layer: usize) -> Option<usize> {
        self.layer_ids.iter().position(|&l| l == layer)
    }
}

/// 2026-09-25: The n-gram hash for one sequence. It keeps the compressed id of
/// every position, so decode steps see the prefill's look-back.
...
impl EngramHasher {
    ...
    /// 2026-09-25: Hash `input_ids` placed at `start_pos..`. Returns
    /// `[seqlen, n_layers, n_hash_cols]` row ids; errors past `max_seq` or on a
    /// token outside `token_map`.
    ...
    /// 2026-09-25: The row ids one engram layer needs, in `[token][col]` order,
    /// from a `hash()` result of `tokens` positions.
...
/// 2026-09-25: One engram layer's resident weights on the device.
pub struct EngramLayerWeights {
    pub layer: usize,
    /// 2026-09-25: `[dim * (hc + 1), cols * head_dim]` bf16.
    pub wkv: DevicePtr,
    /// 2026-09-25: `[hc, dim]` f32, the elementwise product `q_weight * k_weight`.
    pub qk: DevicePtr,
    /// 2026-09-25: This layer's row workspace (`add_layer` allocates it): the raw
    /// Q2_K rows and their bf16 expansion, so every engram layer's rows for a
    /// step can be resident at once.
    pub raw: DevicePtr,
    pub rows: DevicePtr,
    /// 2026-09-25: `wkv` as the GGUF ships it, `[out, in / 256]` Q2_K blocks of
    /// `ENGRAM_ROW_BYTES` (84 bytes per 256 weights, against 512 in bf16). The
    /// single-token projection reads it instead of `wkv`; the two GEMVs agree
    /// bit for bit (checked on a GPU by
    /// `gpu_engram_wkv_q2k_gemv_is_bitwise_the_bf16_gemv_over_the_expansion`).
    /// Null: bf16 only.
    pub wkv_q2k: DevicePtr,
}

/// 2026-09-25: The GPU side: kernels, per-layer weights, and workspaces for up
/// to `max_tokens` positions per call.
pub struct EngramV41 {
    ...
    gemm_k: KernelHandle,
    /// 2026-09-25: The single-token projection from the bf16 `wkv`.
    gemv_k: KernelHandle,
    /// 2026-09-25: The same projection off the raw Q2_K blocks (`wkv_q2k`).
    gemv_q2k_k: KernelHandle,
    gate_k: KernelHandle,
    dequant_k: KernelHandle,
    layers: Vec<EngramLayerWeights>,
    /// 2026-09-25: `[max_tokens * cols * head_dim]` bf16.
    rows: DevicePtr,
    /// 2026-09-25: `[max_tokens * cols * ENGRAM_ROW_BYTES]` raw Q2_K blocks.
    raw: DevicePtr,
    /// 2026-09-25: `[max_tokens * dim * (hc + 1)]` bf16.
    kv: DevicePtr,
}
...
impl EngramV41 {
    ...
    /// 2026-09-25: Register a layer's weights; its row workspace is allocated
    /// here, replacing `w.raw` and `w.rows`.
    ...
    /// 2026-09-25: `q_weight * k_weight` (both `[hc, dim]`, given as f32 values
    /// of the stored bf16) uploaded as the gate's `qk`.
    ...
    /// 2026-09-25: Fill the row input from raw Q2_K blocks
    /// (`n_rows * ENGRAM_ROW_BYTES` bytes, one block per row in `[token][col]`
    /// order): upload, then dequantise on the device. Requires `head_dim == 256`.
    ...
    /// 2026-09-25: Fill the row input from bf16 rows (as bits); the GPU oracle
    /// test uses it.
    ...
    /// 2026-09-25: The shared workspace's dequantised rows (`[n_rows, head_dim]`
    /// bf16), the target of `layer` = `None`.
    pub fn rows_ptr(&self) -> DevicePtr {
        self.rows
    }

    /// 2026-09-25: `(raw, rows)` of layer `layer`'s own workspace, or the shared
    /// one for `None`.
    ...
    /// 2026-09-25: The rows `apply` reads for `w`: its own workspace, or the
    /// shared one when `w.rows` is null.
    fn rows_of(&self, w: &EngramLayerWeights) -> DevicePtr {
        if w.rows.is_null() { self.rows } else { w.rows }
    }

    /// 2026-09-25: The projected `[tokens, dim * (hc + 1)]` bf16 buffer after
    /// `apply`.

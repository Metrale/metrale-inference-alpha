...
CARGO_TARGET_DIR=/workspace/scratch-bench METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b \
  METRALE_TARGET_QUANT=nvfp4 cargo build --release -p spark-model --example int8_gemm_test   # ~15s
...
# server build (native, for end-to-end): same env, cargo build --release -p spark-server --bin met

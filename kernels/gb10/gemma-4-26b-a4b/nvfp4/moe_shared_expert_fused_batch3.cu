...
// forked-from: kernels/gb10/common/moe_shared_expert_fused_batch3.cu (2026-09-24; 37 of 420 lines differ, see kernels/FORKS.md)

// 2026-09-25: Three-token NVFP4 MoE decode kernels: the routed and shared experts of three tokens in one launch per stage. blockIdx.y in [0, 3 * top_k) is routed
// slot y % top_k of token y / top_k (expert_indices[y]); [3 * top_k, 3 * top_k + 3) is the shared expert of token
// y - 3 * top_k. gate_up computes gate (blockIdx.z 0) and up (1); silu_down computes down(GELU_tanh(gate) * up), GeGLU in
// spite of its name; the blend adds the weighted routed outputs and sigmoid(input . gate_weight) times the shared output.
// Launchers: ops::moe_expert_*_batch3, ops::moe_weighted_sum_blend_batch3 (layers/ops/moe_expert_more.rs).
// Owner: gb10 kernels (gemma-4-26b-a4b).
// Invariants: none checked. The GEMV kernels assume 128-thread blocks and K a multiple of 8, and read the shared expert's
// weight pointers without a null check; the blend kernel assumes 256-thread blocks.

...
//!
//! The paging peer (metrale-cache-peer) owns ONE residency map shared across
...
// SSOT: the durable-key hash primitives live in metrale-tier (pure, dep-free, a
...
pub(crate) use metrale_tier::hash::{FNV_OFFSET, fnv1a_64, mix64};
...
impl ModelFingerprint {
    ...
    /// it. `METRALE_MODEL_ID` (optional) is an extra salt for the one case
    ...
    pub(crate) fn derive(cfg: &ModelConfig, blob_bytes: usize) -> Result<Self> {
        ...
        //
        // NOT just a peer: METRALE_SSM_SWAP=1 is a LOCAL swap FILE, which two
    ...
    /// KV-paging fingerprint (`METRALE_KV_PAGING`): the SAME canonical
...
/// Marconi swap namespace: `METRALE_SSM_SWAP_NS` override (strict — junk or 0
...
///
/// `METRALE_SSM_DECODE_CLIENT_ID` pins it (strict parse; 0 is PERMITTED — the
...
fn decode_client_salt() -> Result<u64> {
    ...
    // identity: it exists so two Metrale Engine processes sharing a tier cannot
    ...
    let salt = match std::env::var("METRALE_SSM_DECODE_CLIENT_ID").ok() {
        ...
        None => metrale_tier::entropy::random_u64()?,
    };
...
/// Decode namespace: `METRALE_SSM_DECODE_NS` override (strict, wins UNSALTED —
...
pub(crate) fn resolve_decode_ns(fp: ModelFingerprint) -> Result<NonZeroU64> {
    if let Ok(raw) = std::env::var("METRALE_SSM_DECODE_NS") {
        // Mirror the METRALE_MODEL_ID warn in `ModelFingerprint::derive`: the

...
//! ```
//!
//! and `dense_fp8_enabled()` is `METRALE_DENSE_FP8 == "1"`. `git grep
//! METRALE_DENSE_FP8` returns ONE hit — its own reader. No CI job, no
...
//! METRALE_DENSE_FP8=1, W8A16 (capped)     2555.8 ms   1.78x slower
//! METRALE_DENSE_FP8=1, W8A8  (no cap)     3343.3 ms   2.33x slower
//! ```
//!
...
//!     block-FP8 scheme), when `METRALE_CUBLAS_GEMM=1`. The Hopper fast path.
...
//! arithmetic ran is visible in any TTFT report. `METRALE_FFN_W8A16_ONLY=1`
...
/// `METRALE_FFN_W8A16_ONLY` kill switch: PRESENCE (any value, including empty)
...
/// is misbehaving, and `METRALE_FFN_W8A16_ONLY=0` meaning "on" is a trap.
///
...
/// * `fp8_blockscaled_prefill` — the `METRALE_FP8_SINGLE_SCALE` kill switch that
...
impl DenseFfnLayer {
    ...
    /// folded in FP32 — cuBLASLt when `METRALE_CUBLAS_GEMM` is set and the output

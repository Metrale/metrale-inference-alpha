...
BASE_ENV=(
  ...
  -e METRALE_MTP_CARRY_DEBUG=1   # surfaces draft/accept counters in the log
)

leg() {
  local nd="$1" tag="d${1}"
  local CN="metrale-sweep-$tag"
  for c in $(sudo docker ps -q --filter "name=metrale-sweep-"); do sudo docker rm -f "$c" >/dev/null 2>&1; done
  sleep 4

...
// 2026-09-25: Hopper paged-decode split-K for an FP8 E4M3 KV cache, and its reduce.
//
// Owner: hopper kernels.
// Invariants: those of paged_decode_splitk_hopper.cuh.
//
// Derived from gb10's `paged_decode_attn_splitk_fp8`
// (kernels/gb10/common/paged_decode_attn_fp8.cu). It adds the PD_BC batched
// inner loop and `pd_split_bounds`' minimum split size; the dequant, the
// hoisted `k_scale` / `v_scale`, the shared-memory tree merge and the
// `[o, m, l]` F32 workspace layout are the gb10 kernel's.
//
...
// 2026-09-25: 4 FP8 E4M3 packed in a u32 -> 4 F32, unscaled.
//
// The arithmetic of `unpack4_fp8_raw` in paged_decode_attn_fp8.cu, including
// the paired `fp8x2` converts and the byte order (low byte = element 0).
// The dequant is linear, so `k_scale` goes into the score multiply and
// `v_scale` into the per-warp shared-memory write.
...
extern "C" __global__ void paged_decode_attn_splitk_fp8_hopper(
    const __nv_bfloat16* __restrict__ Q,             // 2026-09-25: [num_seqs, q_stride] BF16
    const __nv_fp8_storage_t* __restrict__ K_cache,  // 2026-09-25: [blocks, block_size, kv_heads, hd] FP8
    const __nv_fp8_storage_t* __restrict__ V_cache,
    float* __restrict__ workspace,                   // 2026-09-25: [seqs, heads, splits, hd+2] F32
    ...
    const float k_scale,
    const float v_scale,
    const unsigned int q_stride,            // 2026-09-25: query.stride(0) in elements
    const unsigned long long cache_stride,  // 2026-09-25: k_cache.stride(0) in elements
    const unsigned int sliding_window       // 2026-09-25: 0 = full attention
) {
    ...
    if (has_work) {
        // 2026-09-25: Q is BF16 and may be a non-contiguous QKV split view.
        ...
        // 2026-09-25: k_scale folds into the score multiply, v_scale into the one
        // shared-memory write in pd_emit_partial: the dequant is linear.
        ...
        while (pos < my_end) {
            ...
            // 2026-09-25: Remainder: the block tail, one position at a time.
    ...
    // 2026-09-25: Every CTA emits, including an empty split: it writes l = 0, which tells
    // the reduce to skip it. Returning early instead would leave stale
...
// 2026-09-25: Reduce the FP8 twin's partials. Grid: (num_q_heads, num_seqs, 1) Block: (32,1,1)

#!/usr/bin/env python3
"""
Single-GPU Model Test Suite for Metrale Engine on DGX Spark
...
# Models whose Metrale Engine tool-call parser is a known gap. We score their
...
def run_tool_call_tests(base_url, model):
    ...
    For models whose Metrale Engine tool-call parser is a known gap (Mistral Small 4
    uses [TOOL_CALLS][ARGS] format instead of Hermes JSON; Nemotron
    models have no Metrale Engine parser at all), we emit N/A results that are
    ...
    for target in targets:
        ...
        if is_api_error or completion_tokens == 0:
            ...
            if needle in text.upper():
                ...
                # test is to validate that Metrale Engine handles large prefills
                ...
                # correct Metrale Engine behavior — the miss is a model-level
                ...
                #
                # Real Metrale Engine bugs (crashes, repetition loops, OOM) are
...
def _vision_sample_data_uri():
    ...
    sample with METRALE_VISION_TEST_IMAGE=<path>. Default:
    tests/fixtures/mona_lisa.jpeg (alongside this script)."""
    path = os.environ.get("METRALE_VISION_TEST_IMAGE") or os.path.join(
...
def run_vision_test(base_url, model):
    ...
    # Override the sample via METRALE_VISION_TEST_IMAGE; if you do, adjust the
    # keyword set or set METRALE_VISION_TEST_KEYWORDS (comma-separated).
    kw_env = os.environ.get("METRALE_VISION_TEST_KEYWORDS")
    keywords = (
...
def main():
    parser = argparse.ArgumentParser(description="Metrale Engine Single-GPU Model Test Suite")
    ...
    print(f"{'='*60}")
    print(f"METRALE SINGLE-GPU TEST SUITE")

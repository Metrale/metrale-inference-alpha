...
"""Diff Metrale Engine's ViT output against an HF transformers reference.

Pipeline:
  1. Run Metrale Engine serving a vision model with `METRALE_DUMP_VIT=/tmp/metrale_vit`.
  ...
  3. For each layer, compute cosine similarity + max-abs-diff against
     Metrale Engine's dump. Print a table; any layer below cosine 0.90 is the
     first divergence and worth investigating.

The goal is NOT bit-exact match (Metrale Engine does FP8 dequant + BF16 GEMM, HF
does all-BF16 or FP16); it's to localize the FIRST block where Metrale Engine
diverges noticeably from HF, so we can focus on fixing that block's
kernel rather than guessing.

Usage:
  # on Metrale Engine host, start server with dump:
  sudo docker run -d --name metrale-vit-debug ... \
      -e METRALE_DUMP_VIT=/tmp/metrale_vit ...
...
def load_metrale_dumps(dump_dir: Path):
    """Load Metrale Engine's per-layer BF16 dumps; return {label: f32 tensor}."""
    import numpy as np
    out = {}
...
def encode_image_for_metrale(max_dim: int = 320) -> tuple[str, int, int]:
...
def trigger_metrale(base_url: str, model_id: str) -> None:
    """Send one Mona Lisa request to an already-running Metrale Engine server that
    was launched with METRALE_DUMP_VIT set. The dump happens as a side
    effect of the ViT forward pass."""
    data_url, w, h = encode_image_for_metrale()
    body = {
        "model": model_id,
    ...
    print(f"metrale response {r.status_code}: "
...
def compute_hf_reference(hf_id: str, out_dir: Path) -> None:
    ...
    per-block outputs so we can diff against Metrale Engine."""
    import numpy as np
    import torch
    from PIL import Image
...
def diff_dumps(metrale_dir: Path, hf_dir: Path) -> None:
    """Compare Metrale Engine's BF16 dumps against HF's layer-by-layer. Emit a
    cosine-similarity / max-abs-diff table."""
    import numpy as np
    metrale = load_metrale_dumps(metrale_dir)
    hf = load_metrale_dumps(hf_dir)
    labels = sorted(set(metrale) & set(hf),
                    key=lambda s: (s != "patch_embed",
                                   s != "final",
                                   int(s[5:]) if s.startswith("block") else 999))

    print(f"\n{'layer':15s} {'metrale#':>9s} {'hf#':>9s} "

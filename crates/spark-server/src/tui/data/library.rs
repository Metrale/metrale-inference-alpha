...
//! with what `met serve <model>` would actually find.
...
pub fn scan(cache_dir: Option<&Path>) -> Vec<LibraryEntry> {
    ...
    for e in rd.flatten() {
        ...
        let mut entry = LibraryEntry {
            ...
            // would measure the link, not the model. Metrale's own downloader
        ...
        if let Ok(json) = std::fs::read_to_string(snap.join("config.json"))
            && let Ok(cfg) = metrale_core::config::parse_config(&json)
        {

...
//!
//! Metrale Engine's degeneration watchdogs (content-phase loop, fuzzy-repetition,
//! inter-tool prose budget) historically *hard-stopped* a sequence —
//! `finished = true` — which kills the response, often mid-tool-call.
//!
//! Per arXiv:2603.27905 (METRALE-RTC) and ROM boundary-truncation
...
//!
//! Metrale Engine uses a **paged** attention KV cache. A decode step writes K/V
...
// ROM model: it needs a per-model trained head that Metrale Engine does not have,

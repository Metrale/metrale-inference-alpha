#!/usr/bin/env python3
"""Multi-turn agentic coherence test for Metrale Engine.
...
def chat(messages, max_tokens=500):
    ...
    with open("/tmp/metrale_mt_payload.json", "w") as f:
        f.write(payload)
    result = subprocess.run(
        ["curl", "-s", "--max-time", "180", f"http://{HOST}/v1/chat/completions",
         "-H", "Content-Type: application/json", "-d", "@/tmp/metrale_mt_payload.json"],
...
def run_session(name, system_prompt):
    ...
    passed = 0
    failed = 0
    tmpdir = tempfile.mkdtemp(prefix="metrale_test_")

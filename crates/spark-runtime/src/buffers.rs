...
pub struct BufferArena {
    ...
    /// MoE gate logits: [M, num_experts] in FP32 (METRALE_FP32_GATE path).
    gate_logits_f32: DevicePtr,
    /// MoE-input norm output: [M, hidden_size] in FP32 (METRALE_FP32_ROUTING).
    ...
    /// model is a 128-dim-linear-head GDN model (METRALE_GDN_FLA path).
    ...
    /// (`METRALE_GGUF_NATIVE_Q2_MMQ`). Shared by every kept-packed projection;
    /// each seam quantizes its activation here then runs the packed MMQ GEMM.
    q2_act_q8: DevicePtr,
    /// Row-wise FP8 GDN prefill BF16-weight slab (`METRALE_FP8_ROWWISE`). One
...
impl BufferArena {
    ...
    pub fn from_sizes(
        ...
        // (size 0 → NULL → METRALE_GDN_FLA dispatch stays disabled).
        ...
        // Q2_0 MMQ prefill q8_1 activation scratch. 0 → NULL unless METRALE_GGUF_NATIVE_Q2_MMQ.
        let q2_act_q8 = if sizes.q2_act_q8 > 0 {
            gpu.alloc(sizes.q2_act_q8)?
        } else {
            DevicePtr::NULL
        };
        // Row-wise GDN prefill BF16 weights. 0 → NULL unless METRALE_FP8_ROWWISE.

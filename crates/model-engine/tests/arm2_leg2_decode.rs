...
//! 2026-09-25: GPU test of the E8M0 decode kernel
//! `moe_expert_gate_up_shared_t_e8m0` (module `moe_shared_expert_fused_t`).
//!
//! Owner: model-engine tests.
//! Invariants: none beyond the types.
//!
//! - CHECK 1: routed E8M0 gate/up outputs against `host_gemv`, within one
//!   BF16 ULP, with scale exponents in [-14, 14].
//! - CHECK 2: the NVFP4 shared-expert outputs of `moe_expert_gate_up_shared_t`
//!   and of the `_e8m0` kernel are bit-identical.
//! - CHECK 3: in one launch with routed E8M0 and shared NVFP4 weights, each
//!   output has the same bits as in a launch with only that weight.
//!
...
//!     cargo test -p metrale-model-engine --test arm2_leg2_decode -- --ignored --nocapture
...
// 2026-09-25: The shared expert is NVFP4 in both kernels (`<GROUP_SIZE, false>`).
// The routed pointer table is [0], so the routed slot writes 0 without reading
// a weight. Only sh_*_out is compared.
fn check2_rider_a3_shared_nvfp4_bit_identical(gpu: &dyn GpuBackend, st: u64) -> Result<()> {
    ...
    let a: Vec<u16> = (0..k)
        .map(|_| f32_to_bf16_bits(rng.unit() * 2.0 - 1.0))
        .collect();
    // 2026-09-25: NVFP4 shared weight: random nibbles and E4M3 scale bytes,
    // with the NaN encodings 0x7F/0xFF replaced.
    let g16 = k / 16;
...
fn check3_rider_a4_mixed_no_cross_contamination(gpu: &dyn GpuBackend, st: u64) -> Result<()> {
    ...
    // 2026-09-25: The e8m0 kernel with the routed E8M0 weights and the given
    // shared weight; returns the routed and shared gate outputs.
    ...
    // 2026-09-25: Routed only: a null shared weight.
    let (routed_only_gate, _) = run_routed(null, null)?;
    // 2026-09-25: Shared only: a zero routed pointer table.

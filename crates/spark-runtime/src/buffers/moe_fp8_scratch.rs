...
//! One arena-owned slab reused sequentially by every grouped FP8 MoE layer.
...
impl Layout {
    pub(super) fn new(c: &ModelConfig, rows: usize) -> Self {
        ...
        let scales = (rows * shared_k.div_ceil(128))
            ...
            * 4;
        // Same PM4 geometry as the grouped kernel: M=128, N=64. Per-expert
...
impl BufferArena {
    /// Fixed offsets for a given shape, without allocating or mutating ownership.

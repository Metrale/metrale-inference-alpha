...
// 2026-09-25: Paged decode attention over an FP8 E4M3 K pool and a Turbo3 V pool (3-bit indices
// into an 8-entry codebook, one FP8 E4M3 scale per 16 elements).
//
// Owner: gb10 kernels.
// Invariants:
...
//
// K block: block_size * num_kv_heads * head_dim bytes (a stride computed here), each times the
// one k_scale. V block (stride v_block_stride_bytes): token-major head rows of head_dim * 3 / 8
// bytes, then from v_data_section_bytes one scale byte per 16 elements. The kernel applies no
// WHT; the caller inverts the V-side WHT on O (attention_forward.rs). With 0 < sliding_window <
// seq_len the range is [seq_len - sliding_window, seq_len), else [0, seq_len).
...
// 2026-09-25: One lane's VEC_BF16 V elements, bit layout as paged_decode_attn_turbo3.cu. At
// HDIM=128 two lanes read the same 3 bytes: the even lane takes values 0-3, the odd 4-7.
...
extern "C" __global__ void paged_decode_attn_fp8k_turbo3v(
    ...
    // 2026-09-25: Lane offsets as in paged_decode_attn_turbo3.cu: at HDIM=256 each lane has its
    // own 3 bytes and two lanes share a scale; at HDIM=128 a lane pair shares 3 bytes.
#if VEC_BF16 == 8

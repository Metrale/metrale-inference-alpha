...
//
// `METRALE_TARGET_HW` selects which `kernels/<hw>/` tree gets compiled, and it
...
/// Hardware target compiled when `METRALE_TARGET_HW` is unset.
...
fn examples(kernels_root: &Path) -> String {
    let lines: Vec<String> = subdirs(kernels_root)
        ...
        .map(|hw| {
            ...
            format!("   METRALE_TARGET_HW={hw} METRALE_TARGET_MODEL={model} METRALE_TARGET_QUANT={quant} cargo build --release -p spark-server")
        })
        .collect();
    if lines.is_empty() {
...
/// Panic text for an `METRALE_TARGET_HW` naming no `kernels/<hw>/` directory.
...
/// Panic text for an `METRALE_TARGET_MODEL` with no directory under this target.
...
///
/// The `/metrale-release` skill states the rule this serves: "no stage runs on an

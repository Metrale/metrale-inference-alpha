...
//! measured.
//!
//! A run stored only in `~/.metrale` answers for one box and one person. A gate
...
///   (`crates/metrale-plugin/assets/bfcl/*.py`), which define the score itself.
...
/// The short commit id for this working tree. `METRALE_GATE_SHA` overrides —

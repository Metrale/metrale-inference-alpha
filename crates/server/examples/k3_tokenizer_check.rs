...
//! 2026-09-26: Checks a derived `tokenizer.json`, and the serving directory's
//! `ChatTokenizer`, against the cases in `tokenizer-oracle.json`. It loads no
//! model weights.
//!
...
fn main() -> Result<()> {
    ...
    let config =
        metrale_config::parse_config(&std::fs::read_to_string(serving.join("config.json"))?)?;
    ensure!(
        config.eos_ids() == vec![163586],
        "official model EOS changed"
    );
    let chat = metrale_server::tokenizer::ChatTokenizer::from_model_dir(
        serving,
        config.eos_token_id,
        true,
        &config.model_type,
    ...
    println!(
        "PASS: {} official K3 tiktoken cases match Metrale Engine Rust and ChatTokenizer; model EOS preserved",
        cases.len()
    );
    Ok(())
}

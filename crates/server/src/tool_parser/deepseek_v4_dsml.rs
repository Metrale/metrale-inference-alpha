...
//! 2026-09-26: DeepSeek-V4 DSML tool calls: prompt rendering of earlier
//! calls, blocking parse and the streaming detector's DSML branch.
//!
//! Owner: server (tool parsing).
//! Invariants:
//! - An invoke whose parameters do not all parse is dropped whole
//!   (`parse_dsml_parameters` returns `None`); a repeated key counts as a
//!   parse failure.
...
impl ToolCallParser for DeepseekV4DsmlParser {
    ...
    fn system_prompt(
        ...
        // 2026-09-26: The DeepSeek-V4 prompt renderer writes the DSML tool
        // instructions and schemas itself (`tokenizer/deepseek_v4/render.rs`
        // `render_tools`); a prompt here would render them twice.
        String::new()
    }

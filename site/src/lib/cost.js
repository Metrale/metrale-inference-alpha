...
export const PRICE_STORAGE_KEY = 'metrale.cost.usd_per_kwh';
...
export const PUE_LOW_STORAGE_KEY = 'metrale.cost.pue_low';
export const PUE_HIGH_STORAGE_KEY = 'metrale.cost.pue_high';
...
export const TOKENS_PER_DAY_STORAGE_KEY = 'metrale.cost.tokens_per_day';

/**
 * Cumulative energy and money saved by serving a FIXED token demand on Metrale Engine
 ...
 *
 * ★ A NEGATIVE RESULT IS RETURNED, NOT CLAMPED. If Metrale Engine spends more per token
 ...
 */
export function savingsOverTime({ metraleJPerTok, baseJPerTok, tokensPerDay, usdPerKwh, pue, days }) {
  ...
  // one -- Metrale Engine energy that has not been measured on this instrument yet.
  if (!Number.isFinite(metraleJPerTok)) return { state: 'unavailable', why: 'Metrale Engine energy is not measured on this instrument' };
  ...
  const perDayKwh = ((baseJPerTok - metraleJPerTok) * tokensPerDay * pue) / 3.6e6;
 ...
 *
 * The Metrale/vLLM pairing decision is `comparisonStateOf`'s, the SAME decision
...
export function costLadder(subject, records, ladders) {
  ...
  if (state === 'published') {
    ...
    metrale = { label: s.label, source: 'published campaign', build: s.build, points };
  } else if (live) {
    metrale = {
      label: 'Metrale Engine',
  ...
  const verdicts = metrale ? rungVerdicts(metrale.points, baselines) : { rungs: [], k: 0, n: 0 };
  const metraleHasEnergy = Boolean(metrale) && anyMeasured(metrale.points);
  return {
    state,
    energyState: !metraleHasEnergy ? 'none' : baselines.length === 0 ? 'metrale-only' : 'paired',
    metrale,
    baselines,
    refused,
    noEnergy,
 ...
 */
export function idleAvailability(metrale, baselines) {
  const sides = [metrale, ...baselines].filter(Boolean);
...
/**
 * Per rung: is Metrale Engine cheaper, and by what factor. Every rung with a
 ...
 */
export function rungVerdicts(metralePoints, baselines) {
  const rungs = [];
  for (const p of metralePoints) {
  ...
  return { rungs, k: counted.filter((r) => r.metraleCheaper).length, n: counted.length };
}
 ...
 * a Metrale Engine trend would read as a vLLM that moved when it did not.
 */
...
export function emptyStateOf(subject, records, ladders) {
    ...
    series.length === 0
      ...
      : series
          .map((b) => {
            ...
            const where =
              cmp === null
                ? 'no Metrale Engine run to fingerprint it against'
                : cmp.ok
                  ? 'the gate instrument'
  ...
  return {
    ...
    vllm,
    fills:
      `one ${subject.gate} run on main with the energy sampler → the Metrale Engine cost curve; ` +
...
export const DISCLOSURES = Object.freeze([
  ...
  },
  {
    ...
    body:
      'A vLLM line is drawn only when its run matched the Metrale Engine run on every workload axis (ISL, OSL, ' +

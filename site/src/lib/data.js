...
//
// SCOPE: Metrale Engine spans a range, from edge class accelerators through workstations
...
export const blogUrl = 'https://blog.dev.metrale.ai';
...
export const recipesUrl = 'https://github.com/Metrale/metrale-recipes';
export const guideUrl =
  'https://github.com/Metrale/metrale-inference-alpha/blob/main/docs/GB10_DEPLOYMENT_GUIDE.md';
export const verifiedAnchor =
  'https://github.com/Metrale/metrale-inference-alpha/blob/main/docs/GB10_DEPLOYMENT_GUIDE.md#8-what-verified-means-so-you-can-trust-an-image';
export const gateSrcUrl =
  'https://github.com/Metrale/metrale-inference-alpha/blob/main/tests/gate_results.py';
export const discussionsUrl = 'https://github.com/Metrale/metrale-inference-alpha/discussions';
export const issuesUrl = `${githubUrl}/issues`;
export const goodFirstIssuesUrl =
  'https://github.com/Metrale/metrale-inference-alpha/labels/good%20first%20issue';
// Single source of truth for contact addresses (footer + reach-out section).
export const contactEmails = ['thomas@metrale.ai'];
...
export const strixPrUrl = 'https://github.com/Metrale/metrale-inference-alpha/pull/187';
...
// closing hint, `scripts/install.sh` in Metrale/metrale-recipes
...
export const quickInstall = 'cargo install metralectl';
...
export const installerUrl = 'https://dev.metrale.ai/install.sh';
...
export const powershellInstallerUrl = 'https://dev.metrale.ai/install.ps1';
...
export const startAgentCommand = 'metralectl agent install';
...
/// drift".
export const runCommandRaw = `metralectl run ${flagshipRecipe}`;
...
export const announcement = {
  ...
  //
  // One LINE as well as one sentence, with room to spare: tag, sentence and
  // link must fit the 1080px band with tens of pixels left over. The band first
  // paints in the metric matched fallback (src/styles/fonts.css), which runs a
  ...
  pwa: 'Metrale Engine Fleet Manager, the PWA that runs models straight from your browser, is still under development. Feedback is welcome.',
...
export const hero = {
  ...
  sub:
    'Metrale Engine is an open source LLM engine written in Rust and CUDA. One ~75 MB binary, no Python, no PyTorch. It runs on edge class accelerators today, scales across nodes with expert parallelism, and holds throughput at the concurrency a datacenter serves. What ships is what we verify, and we bench every release.',
  challenge: {
    claim: 'First token in under 90 seconds on a DGX Spark.',
    lead: 'Do not take our word for it.',
    fine:
      'Median of our GB10 runs, model cached, metrale 59616dc, Jul 2026. Same command below, run it and time it yourself.'
  },
  primaryCta: 'Star on GitHub',
  secondaryCta: 'Start Metrale Engine',
...
export const news = {
  ...
  items: [
    ...
    },
    {
      ...
      body:
        'AMD provided a Strix Halo desktop and we brought Metrale Engine to it through SCALE, custom kernels and all. That machine is the box we ran and submitted MLPerf on. One codebase now covers both vendors with no HIP port and no second kernel tree.',
      cta: 'See the code on GitHub',
      url: githubUrl
    },
    {
      tag: 'MLPerf v6.1',
      ...
      body:
        'Metrale Engine is submitted to MLPerf Inference v6.1 in the closed edge division, the same CUDA source across NVIDIA GB10 and AMD gfx1151. Results stay under embargo until MLCommons publishes.',
...
export const stars = {
  ...
  sub:
    'Metrale Engine went from one Reddit post to a community running it on their own hardware. The curve below is live, regenerated from the GitHub API on every deploy.',
  cta: 'Star the repo'
};
...
export const testimonials = [
  ...
  {
    quote:
      'Testing Metrale Engine on a DGX Spark in an agentic workflow for over an hour. Super impressed. Spark is actually awesome with Metrale Engine.',
...
export const community = {
  ...
  body:
    'Hundreds of builders are running Metrale Engine on their own hardware right now. We are in there every day, shipping fixes, taking model requests, and tuning kernels in the open. Your machine is the test fleet and your voice sets the roadmap.',
...
export const mlcommons = {
  line:
    'Metrale Engine is a member of MLCommons and sits on the Edge LLM taskforce, where we helped shape the new v6.1 edge agentic benchmark. MLCommons names Metrale Engine as a contributor in the announcement.',
...
export const models = {
  ...
  sub:
    'Pick a vendor, then a family. Every card maps to one recipe in metrale-recipes, so the site cannot list a model we do not ship. Copy the command and run it as is. Qwen3.6 leads because it is our flagship.',
  qwen: {
...
export const getRunning = {
  ...
  inspectNote:
    'Rather not pipe curl to a shell. Install metralectl from crates.io, then run the flagship recipe direct.',
  docsCta: 'Read the deployment guide',
  quickstartHint:
    'The script downloads a prebuilt metralectl, verifies its checksum, and installs it to ~/.local/bin. No Python, no Rust toolchain. Run it with --uninstall to reverse it.'
};
...
export const contribute = {
  ...
  sub:
    'Metrale Engine grows from the machines it runs on. Every path below is real and linked. Contributions ship in the Community Edition under AGPLv3, and the CLA lets us re license for the Enterprise Edition.',
  paths: [
    {
    ...
    },
    {
      ...
      body: 'Recipes are the model SSOT. Add a model, tune a quant, open a PR against metrale-recipes.',
      cta: 'metrale-recipes',
      url: recipesUrl
    },
    {
...
export const roadmap = {
  ...
  items: [
    ...
    },
    {
      ...
      status: 'In talks',
      body: 'Active conversations with Intel about bringing Metrale Engine to the Arc Pro B70. Nothing is signed yet, and this card will say so until it is.',
      cta: 'Follow along in Discord',
      url: discordUrl
    },
    {
      ...
      body: 'Native gfx1151 through SCALE. AMD provided a Strix Halo desktop and we brought Metrale Engine to it, custom kernels and all.',
      cta: 'PR #187',
      url: strixPrUrl
    },
    {
...
export const faq = {
  ...
  items: [
    ...
    },
    {
      ...
      a: 'One command: curl -fsSL https://dev.metrale.ai/install.sh | sh. It downloads a prebuilt metralectl, verifies its checksum, and installs to ~/.local/bin. If you would rather not pipe curl to a shell, cargo install metralectl does the same thing from source.'
    },
    {
      q: 'Which models can I run?',
      a: 'Every model on this page maps to a recipe in the metrale-recipes repository, which is the single source of truth — the site cannot list a model that has no recipe. Qwen3.6 is the flagship family, alongside Gemma, Nemotron, Mistral, MiniMax and DeepSeek.'
    },
    {
  ...
  cards: [
    {
      emoji: '💼',
      title: 'Business',
      body: 'Running Metrale Engine in production or evaluating the Enterprise Edition. Tell us what you need and we will scope it with you.'
    },
    {
      emoji: '🤝',
      title: 'Partnerships',
...
export const codeChat = {
  ...
  sub:
    'The whole Metrale Engine repo is embedded into a vector lattice that runs right here in your browser. Ask a question, get an answer with file and line receipts.',
  boot: [

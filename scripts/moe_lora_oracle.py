...
"""moe_lora_oracle.py — numpy delta-parity oracle for the Metrale Engine MoE/attn/overlay
...
SCALE CONVENTION (read from adapter_config.json, NEVER defaulted):
  ...
  Source: crates/metrale-core/src/config/parsers/lora.rs:85-91
...
def scaling(cfg):
    ...
    if "use_rslora" not in cfg:
        raise SystemExit(
            "REJECT(use_rslora): adapter_config.json lacks use_rslora — the Metrale Engine "
...
def main():
    ...
    ap.add_argument("--base-url", help="live mode: Metrale Engine server URL")

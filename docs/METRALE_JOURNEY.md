# Metrale Engine: From Zero to 99 tok/s — The Full Story

## What Is Metrale Engine?

Metrale Engine is a **pure Rust LLM inference server** built entirely from scratch for the **Qwen3-Next-80B-A3B-Instruct-NVFP4** model running on a single **NVIDIA DGX Spark GB10** (Blackwell SM121, 119.7 GB LPDDR5X @ 273 GB/s).
...
## Phase 1: Metrale Engine Kernel Library (Weeks 1–2)
...
- **Prefill attention**: Flash Attention v2 with `cp.async` prefetch, register-based softmax, 4-warp PV accumulation. Iterated through 47 versions (`attn_prefill_v3.cu` through `v47.cu`) to get it right.
...
32 benchmarks against PyTorch, Metrale Engine won 25/32:
...
- **Selective patching wins**: Patching individual ops (RMSNorm, SiLU, RoPE) with Metrale Engine kernels *broke* torch.compile fusion boundaries. Removing 6 of 7 patches gave **+11% throughput** — only patch attention (the one op that's SM121-specific), let vLLM handle everything else.

### Result

**40.5 tok/s** with Metrale Engine attention-only patching, FP8 KV cache, and CUDA graphs in FULL+PIECEWISE mode. Comparable to vLLM's 42 tok/s FlashInfer baseline.
...
## Phase 3: Building Metrale Engine from Scratch (Weeks 3–4)

Metrale Engine was a complete rewrite of the inference pipeline in pure Rust.

### Architecture

```
15 Rust crates:
├── metrale-core          Config, hardware detection, quantization metadata
├── metrale-kernels       29 PTX modules embedded at build time (compiled from .cu)
├── metrale-gpu-runtime       CUDA driver API wrapper, KV cache, weight loader, buffer arena
├── metrale-model-layers         Model composition, layer traits, Qwen3 attention + SSM + MoE
├── metrale-server        HTTP server, tokenizer, integration test
├── metrale-comm          gRPC stub for future multi-GPU
└── 9 other support crates (metrale-py, metrale-quant, metrale-gemm, etc.)   [historical; several no longer exist]
```
...
At this point, Metrale Engine at 41 tok/s already exceeded vLLM's 36.4 tok/s non-speculative baseline.
...
|-----------|-------------|-----------|---------------------|
| **Metrale Engine** | **NVFP4** | **99.1 tok/s** | **No** |
...
| Metrale Engine vLLM backend | NVFP4 | 40.5 tok/s | No |

Metrale Engine at 99 tok/s **without speculative decoding** is 65% faster than vLLM's best result **with** speculative decoding (59.9 tok/s with MTP).
...
**No — speculative decoding has NOT been implemented in Metrale Engine.**
...
The recommended approach for Metrale Engine is **EAGLE-2 or Medusa** with trained draft heads (not n-gram matching):
...
- SSM state checkpointing is straightforward in Metrale Engine (just memcpy ~2.3 MB of state buffers before each speculative batch)
- **Projected throughput with 3 draft tokens at 80% acceptance: ~160–200 tok/s**

This is the clear next step for Metrale Engine, and it's the one optimization that could deliver another 2x+ improvement.
...
```
Phase 1: Metrale Engine Kernel Library
  98a2913  Metrale Engine v0.4.0-sm121: 18/31 benchmark wins, 51/51 tests passing
...
Phase 2: vLLM Integration
  098c74b  Full vLLM integration: Metrale Engine attention backend with CUDA graph support
  ...
  bd88b9b  Metrale Engine attention only: 41 tok/s

Phase 3: Metrale Engine Infrastructure
  b1aa889  Metrale Engine: pure Rust inference server for Qwen3-Next-80B
...
sudo docker run -d --name metrale-build --gpus all --ipc=host --entrypoint bash \
  -v /workspace/metrale:/workspace/metrale metrale/dgx-vllm-nvfp4-kernel:v22 -c "sleep 86400"
sudo docker exec metrale-build bash -c \
  "curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y"

# Build (clear PTX cache when CUDA sources change)
sudo docker exec -w /workspace/metrale metrale-build bash -c \
  "source /root/.cargo/env && rm -rf target/release/build/metrale-kernels-* && cargo build --release"

# Unit tests (43 tests, no GPU required)
sudo docker exec -w /workspace/metrale metrale-build bash -c \
  "source /root/.cargo/env && cargo test -p metrale-model-layers -p metrale-gpu-runtime -p metrale-kernels \
   -p metrale-server -p metrale-comm --release"

# Integration benchmark (GPU + weights, 200 tokens)
sudo docker exec -w /workspace/metrale metrale-build bash -c \
  "source /root/.cargo/env && RUST_LOG=info cargo test -p metrale-server --release -- --ignored"
```

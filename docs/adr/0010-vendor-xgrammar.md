...
`crates/grammar` (workspace member, no `build.rs`, no `cxx` bridge, no C++
...
ByteLevel-BPE `VocabType` bug is what forced Metrale Engine off upstream — but every
"Decision" and "Consequences" claim about a vendored fork is historical. See
`crates/grammar/DESIGN.md` and `crates/grammar/PORT_PLAN.md` for what shipped.
**Date:** 2026-04-17
...
Metrale Engine **vendors xgrammar-rs** under `vendor/xgrammar-rs/` with a
...
Upstream-friendly changes are PR'd back; Metrale-specific glue stays

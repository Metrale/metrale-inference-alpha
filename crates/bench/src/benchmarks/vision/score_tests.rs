...
//! 2026-09-26: Tests for the vision verdict and reply matching.
//!
//! Owner: bench, vision.
...
fn a_failed_control_is_vacuous_even_when_all_green() {
    // 2026-09-26: A failed control gives Vacuous even when every other cell
    // passed.
    assert_eq!(
...
fn geometry_mismatch_fails_on_its_own() {
    // 2026-09-26: A geometry mismatch fails the run even with every probe
    // passing.
    assert_eq!(
...
fn a_run_that_asserted_nothing_is_visible_as_such() {
    // 2026-09-26: An all-Unmeasured run passes `verdict`; `asserted_cells`
    // reports that it asserted nothing.
    let cells = [um("a"), um("b")];
...
fn an_empty_want_all_is_satisfied_but_want_none_still_bites() {
    // 2026-09-26: The control's shape: nothing required, one term forbidden.
...
fn errors_count_as_failures_not_as_absences() {
    // 2026-09-26: An `Error` cell fails the run rather than being skipped.

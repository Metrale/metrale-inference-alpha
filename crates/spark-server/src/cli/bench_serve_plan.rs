...
use metrale_plugin::{gate, serve_env};
...
pub struct ServePlan {
    ...
    /// The `METRALE_*` serve levers the server runs under: the recipe's `env:`
    ...
    pub hardware: String,
    pub limits: metrale_plugin::hardware::limits::Limits,
}

impl ServePlan {
    /// The argv `met serve` is started with on `port` — the recipe's own
...
pub fn plan_serve(
    ...
    let store = metrale_plugin::ArtifactStore::discover()?;

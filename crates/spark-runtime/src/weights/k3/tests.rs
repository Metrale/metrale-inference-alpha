...
use metrale_core::config::parse_config;
...
fn fixture(dir: &Path, bad_scale: bool) -> Vec<u8> {
    ...
    for name in metrale_core::kimi_k3::weights::required_names(&c) {
        if name.contains(".block_sparse_moe.experts.") {
            ...
            let shape = if axis == TpAxis::Replicated {
                metrale_core::kimi_k3::weights_geometry::expected_replicated_shape(&name, &c)
                    .unwrap()
            } else {
                vec![n, k]
            };

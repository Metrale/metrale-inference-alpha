...
//! Metrale Engine CUDA kernel PTX modules.
//!
...
//! When `METRALE_TARGET_MODEL=*` or `METRALE_TARGET_QUANT=*`, multiple
...
// stale and `METRALE_KERNEL_SET_HASH` below already covers it.
mod target_defaults;
...
// this file (or `cargo clean -p metrale-kernels`) forces a fresh recompile
...
///
/// `{}` when the build compiled nothing (`METRALE_SKIP_BUILD=1`) or could not
...
pub struct ModelBehavior {
    ...
    /// Do not append Metrale Engine's derived `<environment>working_directory` block to
    ...
    /// especially mid-tool-call — arXiv:2603.27905 METRALE-RTC). Set `false`
    ...
    /// path that's strictly better than the previous "`[metrale]` Tool call

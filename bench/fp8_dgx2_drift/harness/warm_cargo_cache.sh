...
#     2. ${METRALE_WARM_TARGET_DIR} — COMPILED dependency rlibs (the slow part).
#   The scorer exports CARGO_TARGET_DIR=${METRALE_WARM_TARGET_DIR} so every
#   per-project build reuses the already-compiled deps and only recompiles
#   the project's own tiny crate — seconds, not minutes.
#
# SSOT
#   METRALE_WARM_TARGET_DIR is the single source of truth for the warm target

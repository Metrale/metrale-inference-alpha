...
/// (legacy `METRALE_PREFILL_CODISPATCH=1`),
/// else `None`. The window length is `METRALE_PREFILL_CODISPATCH_WINDOW_MS`
...
fn codispatch_window() -> Option<std::time::Duration> {
    ...
    let ms = std::env::var("METRALE_PREFILL_CODISPATCH_WINDOW_MS")
        .ok()
...
/// (`METRALE_PREFILL_CODISPATCH_SETTLE_MS`, default 10). The window is only
...
fn codispatch_settle() -> std::time::Duration {
    let ms = std::env::var("METRALE_PREFILL_CODISPATCH_SETTLE_MS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(10);
...
pub(super) fn drain_pending_requests(
    ...
    if active.is_empty() && prefilling.is_empty() && have_parked {
        ...
        // Co-dispatch micro-batch window (METRALE_PREFILL_CODISPATCH=1): when idle,

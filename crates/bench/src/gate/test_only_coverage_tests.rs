...
//! 2026-09-26: Tests of `coverage::TEST_ONLY_RUST_MODULES`: each entry is
//! declared once, behind `#[cfg(test)]`, by its registered parent and by no
//! other source; it re-opens no gate on its own and is not a boundary file;
//! a change to its parent still re-opens the parent's gates. Unregistered
//! look-alike paths re-open every gate.
//!
//! Owner: bench gate.
...
fn assert_guarded(root: &Path, module: &TestOnlyRustModule) {
    ...
    // 2026-09-26: No other Rust source under `crates/` may reach this file
    // through an include or a path attribute.
...
fn test_looking_neighbours_remain_fail_closed() {
    for path in [
        "crates/config/src/tests.rs.bak",
        "crates/config/src/tests/tests_b.rs",
        "crates/config/src/gguf/not_really_tests.rs",
        "crates/config/src/gguf.rs",
        "crates/config/src/lib.rs",
        "crates/bench/src/benchmarks/concurrency_tests.rs.bak",
        "crates/bench/src/benchmarks/concurrency_tests/helper.rs",
    ] {

...
Metrale Engine can build and run on Apple Silicon (M1/M2/M3/M4) under the
...
cargo build -p metrale-server --bin met

# Apple Silicon: opt out of cuda, opt into metal.
cargo build -p metrale-server --bin met --features metal --no-default-features

# Sanity check — the binary should link no libcuda / libnccl.
otool -L target/debug/met | grep -i cuda    # → no output
otool -L target/debug/met | grep -i nccl    # → no output
```
...
- **Build pipeline** — `kernels/metal/HARDWARE.toml` (`vendor =
  ...
  `include_bytes!()`. Set `METRALE_TARGET_HW=metal
  METRALE_TARGET_MODEL=qwen3-5-4b-vlm-mlx-int8 METRALE_TARGET_QUANT=mlx_int8`
...
- **Runtime backend** — `metrale_gpu_runtime::metal_backend::MetalGpuBackend`
...
- **MLX 8-bit weight format** — `metrale_gpu_runtime::mlx_int8`
...
`METRALE_KV_DTYPE={bf16,turbo8,turbo4,turbo3,turbo2}`):
...
Quality eval: `METRALE_LOGITS_OUT=path` dumps per-step bf16 logits;
...
1. `cargo check -p metrale-server --features metal --no-default-features`.
2. `cargo test -p metrale-gpu-runtime --features metal metal_backend`
   (every default-passing parity test).
3. `otool -L target/debug/met` must list **no** `libcuda` or
...
| Lift | Notes |
|---|---|
| Metal-side `Model` impl wiring metrale-model-layers's layer dispatch through the metal kernels | metrale-model-layers currently has CUDA-specific layer impls; a `MetalQwen35Model` parallel implementation is the next big lift |
...
| Token-level parity vs `mlx_lm.generate` reference | Run greedy decode of "The capital of France is" on both Metrale Engine Metal + MLX-LM; first ≥5 tokens must match |
EOF
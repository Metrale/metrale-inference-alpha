...
//!
//! Field mapping from Step 3.7 config.json → Metrale Engine ModelConfig:
...
pub(crate) fn parse_step3p7(raw: &serde_json::Value) -> Result<ModelConfig> {
    ...
    // ── MoE field mapping ───────────────────────────────────────────────
    // Step 3.7 uses different field names than Metrale Engine defaults
    ...
    // Metrale Engine ModelConfig currently supports only a single scalar for each.
    ...
    //
    // Metrale Engine's gated attention pipeline assumes Q+G are interleaved in a

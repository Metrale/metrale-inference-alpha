...
//! 2026-09-26: Tests that the card under the cursor is the recipe whose values reach the form, the preview argv and `serve_args`.
//!
//! Owner: server tui.
//! Invariants: none beyond the types.
//!
//! The two 27B fixtures differ in `max_model_len` (262144 vs 60000),
//! `kv_cache_dtype` (fp8 vs bf16) and speculation (off vs `num_drafts: 1`).

use super::*;
use crate::recipe::Recipe;
use crate::tui::data::library::LibraryEntry;

/// 2026-09-26: Loads a recipe fixture from `tests/fixtures/recipes/qwen3.6`.
...
/// 2026-09-26: The value after `name` in the form's preview argv.
...
fn several_recipes_for_one_checkpoint_are_one_row_with_several_cards() {
    // 2026-09-26: One row per model; its recipes are the cards, in id order.
...
fn the_card_the_cursor_is_on_is_the_recipe_that_reaches_the_serve_args() {
    // 2026-09-26: `--max-seq-len`, `--kv-cache-dtype` and `--speculative`
    // differ between the two cards.
...
fn the_form_rows_come_from_the_selected_card_not_the_first_one() {
    // 2026-09-26: The form rows are the selected card's `defaults`.
    let mut s = state(vec![mtp(), plain()]);
    s.open_cards().expect("cards open");
    s.card = 1;
    s.open_config().expect("form opens");
...
fn the_serve_args_a_card_validates_to_match_the_card_it_came_from() {
    // 2026-09-26: `preview_argv` renders argv; `serve_args` also parses and
    // validates it. Both must follow the selected card.
    let mut s = state(vec![mtp(), plain()]);
    s.open_cards().expect("cards open");
...
fn opening_another_card_starts_from_that_recipes_own_values() {
    // 2026-09-26: `open_config` clears the overrides, so edits do not follow
    // the cursor to another card.
    let mut s = state(vec![mtp(), plain()]);
    s.open_cards().expect("cards open");
    s.card = 0;
    s.open_config().expect("form opens");
...
fn a_model_with_no_recipe_has_no_cards_until_starting_points_are_opened() {
    ...
    // 2026-09-26: Before `open_cards` there are no cards.
    ...
    // 2026-09-26: `open_cards` builds starting points, and the form opens on
    // one.
...
#[test]
fn a_non_metrale_recipe_is_listed_and_readable_but_refuses_the_form() {
    // 2026-09-26: A recipe of another runtime is listed and its card opens,
    // but `open_config` refuses and names the runtime.
    let mut vllm = plain();
    ...
    assert!(
        ...
        "weights plus a non-metrale recipe is not runnable here"
    );
...
#[test]
fn the_row_is_described_by_its_metrale_recipe_when_it_has_a_choice() {
    // 2026-09-26: `primary()` prefers the first Metrale Engine recipe over
    // other runtimes.
    let mut vllm = mtp();
    vllm.runtime = Some("vllm".into());
...
fn an_override_for_a_key_the_recipe_does_not_have_is_visible_and_unlaunchable() {
    // 2026-09-26: An override key the recipe does not list renders as a flag
    // in the preview (the same shape as an added setting), and `serve_args`
    // fails because clap rejects the unknown flag.
...
fn a_recipe_that_needs_more_nodes_still_emits_its_world_size() {
    // 2026-09-26: `min_nodes` is not a form row, yet the argv carries
    // `--world-size` from it.
    let mut wide = plain();
    wide.min_nodes = 2;

...
//! Arguments for `met bench certify`.

use std::path::PathBuf;

/// `met bench certify` — run every required gate this commit still owes, on
...
/// `met benchmark run … --pull-request-gate` process, exactly as an operator
...
impl CertifyArgs {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for g in &self.gates {
            if !metrale_plugin::gate::REQUIRED_GATES.contains(&g.as_str()) {
                return Err(format!(
                    "--gates names {g}, which is not a required gate ({})",
                    metrale_plugin::gate::REQUIRED_GATES.join(", ")
                ));
            }
        }
        Ok(())

...
//! target verifies every draft — so the only symptom was acceptance. `METRALE_MTP_EP_PROPOSE=1`
...
pub struct Glm5NextMtpHead {
    ...
    /// the whole drafter costs (nsys 2026-08-29). Kill switch `METRALE_GLM_MTP_HEAD_FP8=0`.
...
impl Glm5NextMtpHead {
    pub fn new(
        ...
        // `METRALE_GLM_MTP_SKIP=head`: propose 8.84 -> 1.52 ms). It is a 1.27 GB BF16 sweep
    ...
    fn forward_one(
        ...
        // TIMING ARM `METRALE_GLM_MTP_SKIP=head`: everything from `shared_head.norm` on is
    ...
    fn rows_impl(
        ...
        for r in 0..rows {
            ...
            // DIAGNOSTIC ARM `METRALE_GLM_MTP_PREFILL_FULL=1`: build the row through the SAME
...
impl DraftProposer for Glm5NextMtpHead {
    ...
    fn after_verify(
        ...
        // K>=3 must run with `METRALE_MTP_CATCHUP=1`.
...
/// `METRALE_GLM_MTP_SKIP=head`: stop the drafter after the block, before `shared_head.norm`,
...
/// `METRALE_GLM_MTP_SKIP=block`: skip `layers.45` itself and run only the head. Timing arm.
fn skip_block() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();

...
| **L1 METRALE_GDN_FUSED_VERIFY** (fuse K=2 verify epilogue) | **decode** | in-tree, **default OFF** | fresh | **A/B now → flip if win** |
...
## Current best Metrale (documented, = base #356)
...
```
Build: `METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL=qwen3.6-27b cargo build --release -p spark-server --bin met --features cuda`
...
| L1 | `METRALE_GDN_FUSED_VERIFY=1` (fuse K=2 conv/norm epilogue) | in-tree, default OFF | NO (env A/B) | remove per-token epilogue at K=2 | queued |
| L2 | `METRALE_GDN_FLA` k-split chunk_delta_h (**1.75× vs wy4**) | branch `perf/gdn-fla-chunked` | yes (fold) | recurrence throughput | queued |
...
(`METRALE_MTP_REFEED_ACCEPTED`) stays OFF (refuted on GB10, Gate A 9/10 fail). NOT in scope.
...
**Conclusion:** the "fragmented un-merged wins" are mostly already in main. Genuine un-banked TPOT levers reduce to TWO:
  1. **L1 `METRALE_GDN_FUSED_VERIFY`** (fused K=2 conv/norm verify epilogue) — in main, default OFF. ← testing.
...
## Log
- L0 base: built OK, target=(gb10,qwen3.6-27b,nvfp4) 157 kernels, md5 bfb9d6e0. Serve img metrale-gb10:followups.
...
Re-scored vs REAL vLLM: Metrale golden (~5000s / ~87 / ~0.625) ≈ **PARITY** on wall/BFCL/IoU
...
grinding any decode-cost lever. dgx2 Metrale e2e gives the apples-to-apples Perf/Accuracy phases.

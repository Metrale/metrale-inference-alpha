...
#
# 1. Assumes image `metrale-gb10:minimax-ep2` exists on head.
...
#   IMAGE   — docker image tag (default metrale-gb10:minimax-ep2)
...
IMAGE="${IMAGE:-metrale-gb10:minimax-ep2}"
...
# /workspace/metrale wrote this label's result into whichever checkout happened
...
METRALE_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STARTUP_SCRIPT="$METRALE_ROOT/scripts/start-minimax-ep2.sh"

RESULT_DIR="$METRALE_ROOT/tests/all_models_results"
...
sudo docker rm -f metrale-minimax-ep0 2>/dev/null || true
ssh "$WORKER_IP" 'sudo docker rm -f metrale-minimax-ep1 2>/dev/null' || true
...
while [[ $(date +%s) -lt $DEADLINE ]]; do
  if [[ $RANK0_READY -eq 0 ]]; then
    if sudo docker logs metrale-minimax-ep0 2>&1 | grep -qE 'Listening on|Server live'; then
      RANK0_READY=1
      echo "  [rank0] listening"
    fi
  fi
  if [[ $RANK1_READY -eq 0 ]]; then
    if ssh "$WORKER_IP" "sudo docker logs metrale-minimax-ep1 2>&1 | grep -qE 'EP worker ready|Listening on|Server live|worker ready'"; then
      RANK1_READY=1
  ...
  # Detect early exit
  if ! sudo docker ps -q -f name=metrale-minimax-ep0 | grep -q .; then
    echo "[FATAL] rank 0 container exited"
    sudo docker logs --tail 100 metrale-minimax-ep0 2>&1
    exit 1
  fi
  if ! ssh "$WORKER_IP" 'sudo docker ps -q -f name=metrale-minimax-ep1' | grep -q .; then
    echo "[FATAL] rank 1 container exited"
    ssh "$WORKER_IP" 'sudo docker logs --tail 100 metrale-minimax-ep1 2>&1'
    exit 1
  fi
  sleep 15
done
...
if [[ $RANK0_READY -eq 0 || $RANK1_READY -eq 0 ]]; then
  ...
  sudo docker logs --tail 120 metrale-minimax-ep0 2>&1
  echo "--- rank 1 log tail ---"
  ssh "$WORKER_IP" 'sudo docker logs --tail 120 metrale-minimax-ep1 2>&1'
  exit 1
fi
...
set +e
python3 "$METRALE_ROOT/tests/single_gpu_suite.py" \
...
if [[ $STATUS -ne 0 ]]; then
  echo "--- rank 0 log tail ---"
  sudo docker logs --tail 80 metrale-minimax-ep0 2>&1
  echo "--- rank 1 log tail ---"
  ssh "$WORKER_IP" 'sudo docker logs --tail 80 metrale-minimax-ep1 2>&1'
fi
...
if [[ "${KEEP_RUNNING:-0}" != "1" ]]; then
  ...
  sudo docker stop metrale-minimax-ep0 2>/dev/null || true
  sudo docker rm   metrale-minimax-ep0 2>/dev/null || true
  ssh "$WORKER_IP" 'sudo docker stop metrale-minimax-ep1 2>/dev/null' || true
  ssh "$WORKER_IP" 'sudo docker rm   metrale-minimax-ep1 2>/dev/null' || true
fi

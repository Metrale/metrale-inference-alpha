    ...
    for &layer in LAYERS.iter() {
        ...
        for &(regime, t) in REGIMES.iter() {
            for &(arm, k_pre, k_post) in arms.iter() {
                // The f32 arm isolates ARITHMETIC: Metrale Engine's highway is f32, so feeding the raw f32
                ...
                for &site in SITES.iter() {
                    ...
                    // settle at `1 - O(hc_eps)`, while Metrale Engine pins them to exactly 1. If that single

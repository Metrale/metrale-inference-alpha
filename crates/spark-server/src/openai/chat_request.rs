...
pub struct ChatCompletionRequest {
    ...
    /// Metrale Engine uses `top_logprobs` for the count. Accepted for compatibility but
    ...
    /// `true`). Metrale Engine currently emits one tool call per turn regardless — the
    ...
    /// (`low | medium | high`). Metrale Engine accepts the field for compatibility
    /// but does not currently steer output length on top of `max_tokens`.
    #[serde(default)]
    pub verbosity: Option<String>,
    /// Service tier (`auto | default | flex | scale | priority`). Metrale Engine runs
    ...
    /// Metrale Engine does not currently have a completion store — field accepted, ignored.
    #[serde(default)]
    ...
    /// filtering; Metrale Engine just round-trips them.
    #[serde(default)]
    pub metadata: Option<std::collections::HashMap<String, String>>,
    /// Stable identifier for end-users (abuse detection). Metrale Engine accepts,
    /// ignores; kept for back-compat with the deprecated `user` field.
    #[serde(default)]
    pub safety_identifier: Option<String>,
    /// Key used by OpenAI to cache prompt prefixes across requests. Metrale Engine's
    ...
    /// "audio"]`, …). Metrale Engine only emits text — when audio is requested
    /// we log a warning and return text only. Accepted for compat with
    /// the gpt-4o-audio / gpt-5-audio family SDKs.
    #[serde(default)]
    pub modalities: Option<Vec<String>>,
    /// Audio-output configuration (voice + format). Metrale Engine does not
    ...
    /// Metrale Engine does not currently run speculative decoding against the
    ...
    /// Metrale Engine has no web-search backend — accepted and ignored.
    #[serde(default)]
...
pub struct StreamOptions {
    ...
    /// but not emitted on Metrale Engine — no multi-tenant side-channel risk to defend.
...
impl ChatCompletionRequest {
    ...
    /// 5. `enable_thinking` (Metrale Engine legacy) — boolean
    ///
    ...
    pub fn client_thinking_directive(&self) -> ThinkingDirective {
        ...
        // 5. Metrale Engine legacy: enable_thinking in the request body. Now Option:

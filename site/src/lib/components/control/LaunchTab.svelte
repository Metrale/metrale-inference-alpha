...
<div class="dt">
  ...
  {:else}
    ...
    {:else if recipes.length === 0}
      <p class="dt-quiet">This agent lists no recipes. Updating metralectl adds the current set.</p>
    {:else}
        ...
        {#if preview}
          ...
          {#if unapplied.length > 0}
            <p class="dt-warn">
              ...
              <code class="mono">{unapplied.join(', ')}</code>. Updating metralectl may fix this.
            </p>
          {/if}
        {/if}

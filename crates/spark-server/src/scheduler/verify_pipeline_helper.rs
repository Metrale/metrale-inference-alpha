...
// `METRALE_DISABLE_FAST_GREEDY` is now `SchedLevers::fast_greedy_grammar`,
// read off `LogitsContext::sampling` at the one site that gated on it.

// The DFlash verify statics are now `SchedLevers::dflash_*`.
// The `METRALE_NO_MTP_VERIFY_SAMPLE` kill switch is now
...
pub fn verify_pick_all_with_pipeline(
    ...
    // out-decodes Metrale Engine on tool/grammar workloads.
    //
    // But when decoding is GREEDY (temp=0 or METRALE_FORCE_TEMP_ZERO), penalties
    ...
    // Kill-switch: METRALE_DISABLE_FAST_GREEDY=1.
    //
    ...
    // Kill switch: METRALE_NO_FAST_GREEDY_CHAT=1 restores the slow path.

...
//! 2026-09-25: Correctness check of `fp8_gemm_t` (module `w4a16`), the unscaled FP8
//! (E4M3 weight) x BF16 (activation) GEMM, against a CPU reference:
//!
//!   C[M,N] = A[M,K] (BF16) · decode_e4m3(B[N,K])^T   (FP32 accumulation)
//!
//! Owner: model-arch examples.
//! Invariants:
//! - Exit 0 only when the cosine to the CPU reference is >= `COSINE_GATE` and no output
//!   is NaN or infinite; exit 1 otherwise.
//!
//! The kernel takes raw E4M3 bytes and no scale. `ops::fp8_gemm_n128` launches it, on the
//! grid used here, (ceil(N/128), ceil(M/64), 1) x (128, 1, 1), when K is not a multiple of
//! 32 or `METRALE_FP8_LDMAB=0`.
//!
//! Usage: cargo run --release -p metrale-model-arch --example fp8gemm_microtest \
//!          --features cuda,gpu-examples -- [M] [N] [K] [seed]
//! M, N and K default to 64, 2048 and 4096; the seed is hex.
...
// 2026-09-25: E4M3 (1 sign, 4 exponent, 3 mantissa bits, bias 7) decode, with the NaN
// codes decoding to 0 as `scl_fp8` does in the SCALE and HIP builds of w4a16_gemm.cu.
...
// 2026-09-25: The E4M3 code whose decoded value is nearest `v`; ties keep the lower code,
// so a NaN code (decoded as 0 above) is never chosen over 0x00.
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: With METRALE_PROBE_FP8FP8 set, time `fp8_fp8_gemm_ldmab`,
    // `fp8_fp8_gemm_t` (FP8 activation) and `fp8_gemm_t` (BF16 activation), and print
    // the cosine of ldmab against `fp8_fp8_gemm_t`.
    if std::env::var_os("METRALE_PROBE_FP8FP8").is_some() {
    ...
    // 2026-09-25: CPU reference: C[m,n] = sum_k A[m,k] * e4m3(B[n,k]) in FP32, rounded to
    // BF16.

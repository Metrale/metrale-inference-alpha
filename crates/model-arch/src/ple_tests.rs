...
//! 2026-09-25: GPU parity for the PLE kernels (`ple_gate`, `ple_conv` and the
//! n-gram row gather) against the reference.
//!
//! `ple.cu` (qwen3.8-flash-next) implements `Qwen4ExpTextPLELayer.forward`.
//! Three details in it give finite, plausible output when wrong: the
//! offset-from-1 norms, the grouped norm and the gate's signed square root,
//! so each output is compared with the reference's.
//!
//! Fixtures: `bench/qwen4_exp/ple_golden.py --bin-dir <dir>`, whose
//! intermediates come from the reference's own submodules on the checkpoint
//! weights.
//!
//! Owner: model-arch (Qwen3.8-Flash-Next PLE).
//! Invariants: none beyond the types.
//!
//! GPU test: `#[ignore]`. Run with
//! ```text
//! METRALE_PLE_TEST_DATA=/tank/metrale-testdata/qwen4exp_ple \
//!   cargo test -p metrale-model-arch --release ple_kernels -- --ignored --nocapture
//! ```
...
impl Fx {
    fn load() -> Self {
        let dir = std::env::var("METRALE_PLE_TEST_DATA").expect(
            "set METRALE_PLE_TEST_DATA — generate with \
        ...
        let m: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(format!("{dir}/meta.json")).unwrap())
                .unwrap();
        // 2026-09-25: The PLE norms are the offset-from-1 form,
        // `normed * (1.0 + weight)`; a fixture written under another convention
        // fails here instead of loosening the comparison.
        assert_eq!(
...
fn compare(label: &str, got: &[f32], want: &[f32]) {
    ...
    // 2026-09-25: A relative tolerance, each element's error divided by its
    // own magnitude floored at the reference RMS (so near-zero elements do not
    // blow the ratio up). A bound scaled to the RMS alone would ask far more
    // relative precision of a heavy-tailed tensor's peaks than of its bulk.
...
fn ple_kernels_match_reference() {
    ...
    // 2026-09-25: The worst token's max|diff|: a fault that grows with the
    // token index shows here before the whole-tensor comparison.
    {
    ...
    // 2026-09-25: The conv, SiLU and add. The conv state, `(k_size - 1) *
    // dilation` steps, starts at zero, as at a sequence start.
    ...
    // 2026-09-25: Control: the same conv at dilation 1 reads adjacent
    // timesteps instead of the dilated taps, so it must miss the reference;
    // if it matched, the comparison above would not be checking the dilation.
...
/// 2026-09-25: The segmented row cache and the gather, apart from the rest
/// of PLE: reading the wrong rows is invisible downstream, because every row
/// of the n-gram embedding table is a plausible embedding.
///
/// Needs the checkpoint (it reads real rows from it); returns without checking
/// anything when `METRALE_PLE_CKPT` is unset:
/// ```text
/// METRALE_PLE_TEST_DATA=/tank/metrale-testdata/qwen4exp_ple \
/// METRALE_PLE_CKPT=/path/to/snapshot \
///   cargo test -p metrale-model-arch --release ple_gather -- --ignored --nocapture
/// ```
#[test]
#[ignore]
fn ple_gather_reads_the_right_rows() {
    let f = Fx::load();
    let snap = match std::env::var("METRALE_PLE_CKPT") {
        Ok(s) => s,
        Err(_) => {
            println!("METRALE_PLE_CKPT unset — skipping");
            return;
        }
    };
    ...
    // 2026-09-25: The segmented cache from the checkpoint's safetensors header
    // (`ple_shard_layout`, then `open_segmented` for a BF16 table), without
    // loading the model.
    let (shards, rows_per) =
    ...
    // 2026-09-25: The CUDA backend comes first: the cache's region is pinned,
    // GPU-addressable host memory, which needs a live context.
    let set = metrale_kernels::ptx_for_exact_target("qwen3.8-flash-next", "nvfp4").unwrap();
    ...
    let mut cache = metrale_storage::NgramRowCache::open_segmented(
        &shards,
        rows_per,
        None,
        head_dim * 2,

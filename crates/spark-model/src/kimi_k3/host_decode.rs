...
impl K3BoundLayer {
    ...
    pub(super) fn decode_host(
        ...
        let dense_core = |_weights: &metrale_core::kimi_k3::cpu_weights::DenseMlp,
                          x: &[f32],
                          hidden: usize,
                          inter: usize,
                          beta: f32,
        ...
        let result = (|| -> Result<()> {
            ...
            } else if let (Some(device), metrale_core::kimi_k3::LayerCache::Kda(host)) =
            ...
            if use_cuda_mla {
                let host_seq = match &st.cache {
                    metrale_core::kimi_k3::LayerCache::Mla(kv) => kv.seq_len,
                    _ => 0,
                };
            ...
            } else if let (Some(device), metrale_core::kimi_k3::LayerCache::Mla(host)) =

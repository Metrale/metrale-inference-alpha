...
| **`METRALE_DECODE_GRAPHS_MULTISEQ`** | **DEAD — not applicable** | Advertises "the dominant lever for n>=2 decode (~1500 kernel launches/step)", but `decode_a2.rs` iterates over concurrent *sequences*, not the K verify tokens; at `--max-batch-size 1` with MTP gated to `active.len()==1` the path is never entered. Independently, the serve log shows `Captured CUDA graph for K=4 verify (slot=0)` — the verify path already graphs by default (`verify_c.rs:170`). Rejected without spending a leg. |
| **`METRALE_SSM_TAIL_MIDCHUNK` re-enable** | **DO NOT FOLD (cost/benefit)** | The `=0` in the frozen config IS a stale workaround — the 2026-07-16 fix is present verbatim in `snapshot.rs::lookup`. But N=3: warm TTFT **median unchanged** (894.1 both), mean −2.7%, sd 107.5→72.5. ~26 s = 0.6% of wall. Not worth re-opening a silent cross-request corruption path. |
...
| **`METRALE_BF16_TC_PROJ`** | **no speed case** | Monotonic warm TTFT 990.8 → 1016.5 ms (+2.6%), TPOT ~neutral, tool call fine. Its motivation is accuracy (removes FP8 E4M3 activation crushing on attention QKV/o, which we already avoid on the FFN via `METRALE_BF16_TC_PREFILL=1`), so it would need an accuracy run to justify — but it does not pay for itself on speed. |
...
- `METRALE_*` presence-flags treat `=0` as ENABLED; only `METRALE_SSM_TAIL_MIDCHUNK` and
  `METRALE_DECODE_GRAPHS_MULTISEQ` are strict-string tests. Control legs must OMIT the variable.

## RESULT: METRALE_GDN_REGRESIDENT — FOLD (full e2e, 2026-07-25)

`results/regres_e2e_20260725_071238`, frozen golden config + `METRALE_GDN_REGRESIDENT=1` only,

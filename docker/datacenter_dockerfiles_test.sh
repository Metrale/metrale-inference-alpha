...
# rented GPU:
#
#   (a) the default METRALE_TARGET_HW per file. Copy one Dockerfile from the
...
# NCCL_SOCKET_IFNAME, METRALE_TARGET_HW and the rest, and a naive grep over the
...
for pair in "hopper:$HOPPER" "b200:$B200"; do
  hw="${pair%%:*}"; f="${pair#*:}"
  got="$(instructions "$f" | sed -n 's/^[[:space:]]*ARG[[:space:]]\{1,\}METRALE_TARGET_HW=\(.*\)$/\1/p')"
  [ -n "$got" ] || fail a "$f declares no \`ARG METRALE_TARGET_HW=<default>\`"
  [ "$(printf '%s\n' "$got" | wc -l | tr -d ' ')" = 1 ] \
    || fail a "$f declares ARG METRALE_TARGET_HW more than once: $got"
  [ "$got" = "$hw" ] || fail a "$f defaults METRALE_TARGET_HW to '$got', expected '$hw'"
  ok a "$(basename "$(dirname "$f")")/Dockerfile defaults METRALE_TARGET_HW=$hw"

  # The ARG has to reach the build environment, or it is decoration.
  instructions "$f" | grep -qE '^[[:space:]]*ENV[[:space:]]+METRALE_TARGET_HW=\$\{?METRALE_TARGET_HW\}?[[:space:]]*$' \
    || fail a "$f never exports METRALE_TARGET_HW as an ENV from its ARG"
  ok a "$(basename "$(dirname "$f")")/Dockerfile exports METRALE_TARGET_HW to the build"

  # '*' = every model target under kernels/<hw>/.
  instructions "$f" | grep -qE "^[[:space:]]*ARG[[:space:]]+METRALE_TARGET_MODEL=[\"']?\*[\"']?[[:space:]]*$" \
    || fail a "$f does not default METRALE_TARGET_MODEL to '*'"
  ok a "$(basename "$(dirname "$f")")/Dockerfile defaults METRALE_TARGET_MODEL='*'"
done
...
# ── (d) the build keeps the multi-GPU feature set, and ships `met` ──────────
for f in "$HOPPER" "$B200"; do
  ...
  instructions "$f" | grep -qE '^[[:space:]]*ENTRYPOINT[[:space:]]+\["met"\][[:space:]]*$' \
    || fail d "$f does not ENTRYPOINT [\"met\"]"
  ok d "$(basename "$(dirname "$f")")/Dockerfile entrypoints met"
...
# The workflow MENTIONS METRALE_SKIP_BUILD in a comment on purpose (not set, and
...
if re.search(r"METRALE_SKIP_BUILD\s*[:=]", body):
    sys.exit("the workflow sets METRALE_SKIP_BUILD — that would ship a stub PTX registry")
...
if python3 -c 'import yaml' 2>/dev/null; then
  ...
  if sed 's/#.*$//' "$WORKFLOW" | grep -qE 'METRALE_SKIP_BUILD[[:space:]]*[:=]'; then
    fail e "the workflow sets METRALE_SKIP_BUILD — that would ship a stub PTX registry"
  fi

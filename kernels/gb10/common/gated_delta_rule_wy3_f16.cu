...
// 2026-09-25: FP16 h-state twin of gated_delta_rule_wy3 (K=3 GDN verify step).
//
// Owner: gb10 kernels.
// Invariants:
// - Apart from the FP16 round trip below, every float expression, gate clamp and
//   accumulation order is gated_delta_rule_wy3's. H, Hi0 and Hi1 are `__half` in memory,
...
//
// Qwen3SsmLayer::wy3_kernel (qwen3_ssm/trait_decode_batched_conv_gdn.rs) returns this
...
extern "C" __global__ void gated_delta_rule_wy3_f16(
    ...
    // pool's slot pitch nor the placement of intermediates, so ops::gdn_decode_wy3 accepts

...
//! 2026-09-26: The MLPerf Agentic Inference trajectory file: flat JSONL, one
//! row per message, each conversation's rows contiguous and numbered 1..=N in
//! file order.
//!
//! Teacher forcing: the prompt for a client turn is the recorded history
//! (system prompt and recorded user, tool and assistant rows, including
//! recorded `reasoning_content` and `tool_calls`) plus the current client
//! message, prebuilt at load time. The model's own output is never fed into
//! later turns, so per-turn prompts depend only on the file.
//!
//! Owner: bench, mlperf_agentic.
//! Invariants: `load` returns `Ok` only for a file whose every conversation
//! passes the id, contiguity, turn-number and role checks, and a non-empty
//! draw.
...
/// 2026-09-26: One client turn: what is sent, and the recorded assistant row
/// that scores it.
#[derive(Clone, Debug)]
pub struct ClientTurn {
    /// 2026-09-26: The client row's turn number.
    pub turn: i64,
    /// 2026-09-26: Prebuilt prompt: system + recorded history + this client
    /// message.
    pub messages: Vec<Value>,
    /// 2026-09-26: The recorded assistant row that immediately follows this
    /// client row, raw. `None` for a trailing client row with no assistant
    /// after it.
...
pub struct Conversation {
    pub id: String,
    pub domain: Domain,
    /// 2026-09-26: Tool schemas from the conversation's first `user` row,
    /// sent with every request of the conversation.
    pub tools: Option<Value>,
    pub client_turns: Vec<ClientTurn>,
}

/// 2026-09-26: How many whole trajectories to take per domain, the first K in
/// file order, with no RNG. `0` means all of that domain. Conversations are
/// drawn whole.
...
/// 2026-09-26: Load the trajectory file and apply the draw.
...
/// 2026-09-26: The draw fingerprint: SHA256 over the ordered conversations'
/// ids, tools, and each client turn's number, messages and ground truth. Two
/// runs with equal fingerprints replayed the same prompts, tools and ground
/// truth in the same order; a differing fingerprint means the numbers are not
/// comparable, whatever the sample counts say.
...
fn parse(text: &str) -> Result<Vec<Conversation>> {
    // 2026-09-26: Group contiguous rows by conversation (non-empty
    // printable-ASCII ids, rows contiguous); `build_conversation` checks the
    // turn numbers and the role grammar.
...
fn build_conversation(id: String, rows: Vec<Map<String, Value>>) -> Result<Conversation> {
    ...
    // 2026-09-26: Single pass in turn order, carrying the running history.
    // Client rows snapshot (history + current) before extending; assistant
    // rows only extend.
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        if role == "user" || role == "tool" {
            ...
            // 2026-09-26: The scorer's ground-truth pairing: the assistant row
            // that immediately follows this client row in turn order.
...
/// 2026-09-26: Format one row into OpenAI message(s): a non-empty
/// `tool_results` list expands to one tool message per result; otherwise the
/// recorded message fields pass through, with `content: null` filled in on
/// tool-calling assistant rows.
...
/// 2026-09-26: The role grammar: a conversation starts with `user`; `user` is
/// answered by `assistant`; `assistant` is followed by `tool` or `user`;
/// `tool` by `assistant` or `user`. A `tool` row that follows an assistant
/// must follow one that made tool calls, and every `tool` row must carry a
/// non-empty `tool_results` list.

...
//! Placement is exercised on a real record and its real signature (the
//! fixture corpus in `test_data/gate-records`, signed by a fixture key that
//! only that corpus registers), copied into a scratch repository that carries
//! the fixture's signer keys — so "verifies" here is the same verification CI
//! performs.
use super::*;
use std::path::Path;
...
/// fixture decode-floor record, and its git sha.

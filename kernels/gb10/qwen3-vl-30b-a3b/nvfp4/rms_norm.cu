...
// forked-from: kernels/gb10/common/rms_norm.cu (2026-09-24; 1517 of 1433 lines differ, see kernels/FORKS.md)

// 2026-09-25: RMS norm kernels for qwen3-vl-30b-a3b. They scale by the plain weight, out = x * rms * w
// with rms = 1 / sqrt(mean(x^2) + eps); gb10/common/rms_norm.cu scales by (1 + w) instead.
//
// Owner: gb10 kernels (qwen3-vl-30b-a3b).
// Invariants:
// - No kernel here applies 1 + w; sums of squares are FP32.
// - The norm kernels run one block per row and reduce through warp_sums[32], so a block has at most 1024 threads.
...
// 2026-09-25: out = x * rms * w per row. Grid (num_tokens), block min(hidden_size, 1024) (ops/norm.rs rms_norm).
...
// 2026-09-25: rms_norm that also copies the input row to residual. Grid (num_tokens), block
// min(hidden_size, 1024) (ops/norm.rs rms_norm_residual).
...
// 2026-09-25: hidden += src (stored as BF16), then out = rms_norm(hidden) and residual = the updated
// hidden. Grid (num_tokens), block min(hidden_size, 1024) (ops/norm.rs residual_add_rms_norm).
...
// 2026-09-25: out = x * rms * w * silu(gate), with the gate row at token * gate_stride. Each thread
// caches its values in x_cache[16], so hidden_size must be a multiple of 4 and at most 16 * blockDim.x.
// ops::gated_rms_norm passes a trailing group_size that this signature does not declare.
...
// 2026-09-25: In place, x /= sqrt(sum(x^2) + eps) over the head_dim values of (head, token), which
// start at token * stride + head * head_dim. Grid (num_heads, num_tokens), block min(head_dim, 1024)
// (ops/activations.rs l2_norm).
...
// 2026-09-25: gated_rms_norm with an FP32 input; gate, weight and output are BF16, and group_size is
// unused.
...
// 2026-09-25: The kernels below have the same code as their namesakes in gb10/common/rms_norm.cu,
// except that each (1 + w) there is w here. f32_residual_add, gated_rms_norm_prefill and the *_abs
// kernels are unchanged, since they have no (1 + w).
...
// 2026-09-25: residual[i] += src[i] for i < n (FP32 residual, BF16 src), one thread per element.
...
// 2026-09-25: Gated RMS norm of every (head, token) row in one launch. Row (head, token) of input and
// output starts at token * input_token_stride + head * head_dim, of gate at token * gate_token_stride
// + head * head_dim (BF16 elements). Grid (heads_per_token, num_actual_tokens), block
// min(head_dim, 1024) (ops/norm.rs gated_rms_norm_prefill).
...
// 2026-09-25: residual_add_rms_norm with an FP32 hidden: hidden (FP32) += src (BF16), out =
// rms_norm(hidden) in BF16, and residual = the updated hidden in FP32.
...
// 2026-09-25: The same arithmetic as residual_add_rms_norm_f32 above.
...
// 2026-09-25: residual_add_rms_norm that also writes the normed row in FP32 to output_f32; the BF16
// output and residual are computed as in residual_add_rms_norm. qwen3_attention's decode uses it when
// the MoE router runs in FP32 (MoeLayer::fp32_routing_active, METRALE_FP32_ROUTING), so the router
// reads unrounded normed values. Grid (num_tokens), block min(hidden_size, 1024) (ops/norm.rs).
...
// 2026-09-25: The same arithmetic as rms_norm_f32 above.
...
// 2026-09-25: rms_norm_f32 that also copies the FP32 input row to the FP32 residual.
...
// 2026-09-25: The same arithmetic as rms_norm_residual_f32 above.
...
// 2026-09-25: rms_norm over gridDim.y groups of gridDim.x rows in one launch: row x of group y starts
// at y * row_stride + x * hidden_size (elements). Per row the arithmetic is rms_norm's. Grid
// (rows_per_group, num_groups), block min(hidden_size, 1024) (ops/norm.rs rms_norm_strided).

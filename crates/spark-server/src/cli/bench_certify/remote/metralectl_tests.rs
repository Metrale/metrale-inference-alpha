...
//! The wire contract with metralectl, pinned from this side: the JSON shapes
//! `bench nodes/submit/attach/fetch --json` emit (metrale-recipes
//! `docs/BENCH.md`) parse into what the driver acts on, and every exit code
//! maps to its class.
use super::*;

/// A `bench nodes --json` row as metralectl 0.5 writes it, verbatim shape.

...
# 2026-09-26: Aggregates per-run score JSON into per-tier CSV and markdown reports.
#
...
# 2026-09-26: (JSON path, report name, kind). The kind is only a label in the
# report: "rate" for booleans (counted as 0/1), "count" for numbers.
METRICS = [
    ...
    ("metrale.ws1_mask_active_fires", "metrale_ws1_mask_fires", "count"),
    ("metrale.b1_drift_gauge_fires", "metrale_b1_drift_fires", "count"),
    ("metrale.tier_5c_retries", "metrale_tier5c_retries", "count"),
    ("metrale.a2_fuzzy_repair_fires", "metrale_a2_fuzzy_fires", "count"),
    ("metrale.tool_call_lines", "metrale_tool_call_lines", "count"),
...
def bootstrap_ci(values: list[float], n_boot: int = 10000, ci: float = 0.95) -> tuple[float, float]:
    ...
    rnd = random.Random(0xA71A5)  # 2026-09-26: fixed seed, so the interval is reproducible
    means: list[float] = []
    n = len(values)
...
def aggregate_tier(runs_dir: pathlib.Path, tier: str) -> dict[str, Any]:
    ...
    # 2026-09-26: Failure counts for the exit code: a run fails cargo when
    # cargo.cargo_toml_valid is not truthy and fails webserver when
    # webserver.webserver_ok is not truthy; a missing value counts as a failure.
    ...
    # 2026-09-26: followed_directions is informational, not part of the exit code.
    # Runs without the metric are left out of both numerator and denominator.
...
def main() -> int:
    ...
    # 2026-09-26: Exit code = total cargo plus webserver failures, capped at 255
    # because exit codes are 8-bit; the full count is printed first.

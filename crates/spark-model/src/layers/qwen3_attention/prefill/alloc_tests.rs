...
//! `METRALE_CUBLAS_GEMM=attn` arms: the cache-skip Q/K/V chain
...
//! `METRALE_CUBLAS_GEMM=ffn,ssm,attn` and arming `attn` for decode must not
...
/// `METRALE_CUBLAS_GEMM=attn` armed, which is what the decode recipe sets.

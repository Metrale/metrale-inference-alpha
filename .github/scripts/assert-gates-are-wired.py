...
# workflow-level `METRALE_SKIP_BUILD: "1"` -- which exists so the ubuntu jobs can
# type-check without nvcc. metrale-kernels' build.rs honours it before anything
...
STUB_FREE_STEPS = [
    {
        ...
        "step": "cargo test -p metrale-gpu-runtime --features metal",
        "var": "METRALE_SKIP_BUILD",
        "also": {"METRALE_TARGET_HW": "metal"},
    },
]
...
def check_stub_free(spec: dict) -> None:
    ...
    if env.get(var) in SKIP_TRUTHY:
        problems.append(
            ...
            f"{env[var]!r}, so metrale-kernels emits a stub and `metallib_modules()` is "

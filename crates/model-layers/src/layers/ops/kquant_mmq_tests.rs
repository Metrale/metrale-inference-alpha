...
//! 2026-09-25: GPU tests of the DeepSeek-V4.1 K-quant kernels against a CPU oracle, and of the fused launches against their separate launches.
//!
//! Owner: model-layers ops.
//! Invariants: none beyond the types.
//!
//! The oracle dequantizes the weights with `metrale_model_weights::weights::dequant_cpu`,
//! quantizes the activation to q8_1 on the host as the kernels do (per block `d = amax / 127`,
//! round half away from zero), and multiplies in f32. Decode (`kquant_mmvq`, M = 1 and 5) and
//! prefill (`kquant_mmq_gemm`, M = 200) are checked for Q2_K and Q3_K, and the Q6_K head GEMV at
//! M = 1 and 5.
//!
//! The tests are `#[ignore]` and need a GPU. Run with
//! ```text
//! cargo test -p metrale-model-layers --release kquant -- --ignored --nocapture
//! ```
...
const N: u32 = 320; // 2026-09-25: Not a multiple of 128, so the MMQ test takes the `_wc` entry.
const K: u32 = 1024;
const SCALES_F16: [u16; 4] = [0x3C00, 0x3800, 0x4000, 0x3400]; // 2026-09-25: 1.0, 0.5, 2.0, 0.25
...
/// 2026-09-25: `[N][K]` raw super-blocks of LCG bytes, with the fp16 scale fields at
/// `f16_offsets` overwritten from `SCALES_F16`.
...
/// 2026-09-25: Activations `[M][K]` in [-1, 1), as bf16 bits and as the f32 the kernel sees.
...
/// 2026-09-25: q8_1 emulation: per `block` values, d = amax/127, q = round(x/d), back to f32.
...
fn assert_close(label: &str, got: &[f32], want: &[f32]) {
    ...
    // 2026-09-25: `max * 2^-7` covers the bf16 rounding of the largest output; `1e-3` covers f32
...
struct Ty {
    ...
    quant: &'static str,
    smem: u32,
    /// 2026-09-25: Values per activation scale in this type's MMQ q8_1 layout (64 for D2S6, 32 for
    /// D4).
    mmq_act_block: usize,
}
...
const TYPES: [Ty; 2] = [
    Ty {
        ...
        mmq_nc: "metrale_q2_k_mmq128_nc",
        mmq_wc: "metrale_q2_k_mmq128_wc",
        quant: "metrale_q8_1_quantize_d2s6_bf16",
        smem: Q2K_MMQ_SMEM,
        mmq_act_block: 64,
    },
    Ty {
        ...
        mmq_nc: "metrale_q3_k_mmq128_nc",
        mmq_wc: "metrale_q3_k_mmq128_wc",
        quant: "metrale_q8_1_quantize_d4_bf16",
        smem: Q3K_MMQ_SMEM,
        mmq_act_block: 32,
    },
];
...
/// 2026-09-25: The Q6_K head GEMV (`kquant_mmvq_q6_k_w`, warp per row) against the CPU oracle at
/// M = 1 and 5.
#[test]
#[ignore = "requires a CUDA GB10 + the deepseek-v4-flash kernel target"]
fn kquant_q6k_head_mmvq_w_matches_cpu_oracle() {
    ...
    let g: &dyn GpuBackend = &gpu;
    let stream = g.default_stream();
    // 2026-09-25: Q6_K: { u8 ql[128]; u8 qh[64]; i8 scales[16]; f16 d } = 210 B, d at 208.
...
/// 2026-09-25: The grouped launch (`kquant_mmvq_q2_k_groups_w`) against `G` per-group
/// `kquant_mmvq_q2_k_w` launches over the column slices: the same bytes out at M = 1 and 5.
/// Weight `[G * N, K]`, activation `[M, G * K]`.
#[test]
#[ignore = "requires a CUDA GB10 + the deepseek-v4-flash kernel target"]
fn kquant_groups_w_matches_per_group_launches_bitwise() {
    ...
    for m in [1u32, 5] {
        ...
        let x_bytes: Vec<u8> = bits.iter().flat_map(|b| b.to_le_bytes()).collect();
        let x_dev = upload(g, &x_bytes);
        // 2026-09-25: Grouped: one quantize pass over the whole row, one launch.
        ...
        // 2026-09-25: Per group: slice the columns on the host, quantize, launch, place.
...
/// 2026-09-25: The paired launch (`kquant_mmvq_q2_k_pair_w`) against two `kquant_mmvq_q2_k_w`
/// launches over the same q8_1 row: the same bytes out at M = 1 and 5. The row counts differ
/// (N = 320 and N / 4 + 8 = 88), so the shorter projection's blocks past its 88 rows exit
/// early.
#[test]
#[ignore = "requires a CUDA GB10 + the deepseek-v4-flash kernel target"]

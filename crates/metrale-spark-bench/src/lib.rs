...
//! Metrale Engine Spark HTTP benchmark client.
//!
//! Shared utilities for Criterion benchmarks and correctness tests.
//! Expects a running Metrale Engine Spark server (default: `http://localhost:8888`).
...
pub fn server_url() -> String {
    std::env::var("METRALE_BENCH_URL").unwrap_or_else(|_| "http://localhost:8888".into())
}
...
pub fn require_server() -> String {
    ...
    match ureq::get(&format!("{url}/health")).call() {
        ...
        Err(e) => panic!("Server not reachable at {url}: {e}. Start Metrale Engine Spark first."),
    }
}

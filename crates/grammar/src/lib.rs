...
//
// PORT STATUS: COMPLETE (wave W7 — public API + Metrale Engine repoint). The
...
// vendored-signature façade Metrale Engine's `metrale-server` links against.
...
//
// These are the names Metrale Engine's `use xgrammar::{...}` resolves against.
...
// (`Raw`/`ByteFallback`/`ByteLevel`). Metrale Engine's `grammar/engine.rs`

...
//! 2026-09-26: Tests for the pinned script (its bytes, by SHA-256), the request
//! body, and the reference anchors.
//!
...
fn request_body_is_greedy_pinned_seed_stream() {
    ...
    // 2026-09-26: The whole object is compared, so an added field, such as a
    // `chat_template_kwargs`, fails the test; `stream_options.include_usage`
    // must stay.
    assert_eq!(
        body,
        serde_json::json!({
...
/// 2026-09-26: A reference round that satisfies every anchor.
...
fn a_reference_missing_the_ack_is_rejected() {
    // 2026-09-26: A wrong reference would make every matching replay Invariant.
    let mut reference = healthy_reference();
...
fn a_reference_with_the_wrong_section_count_is_rejected() {
    // 2026-09-26: "7741-C" carries a 7 of its own, so only the 5 counts here.
    let mut reference = healthy_reference();
...
fn a_two_line_invariant_list_is_rejected() {
    // 2026-09-26: Turn 2 demands exactly three numbered lines.
    let mut reference = healthy_reference();
...
fn a_budget_truncated_reference_is_rejected() {
    // 2026-09-26: A reference turn that hit `max_tokens` cannot anchor the
    // collapse ratios.
    let mut reference = healthy_reference();

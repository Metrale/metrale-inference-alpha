...
//! hands `metrale_kernels::metallib_modules()` to `MetalGpuBackend::new`. That
...
//!
//! That is exactly what shipped. metrale-kernels' `build.rs` honours
//! `METRALE_SKIP_BUILD` before anything else and emits a stub whose
//! `metallib_modules()` is `Vec::new()`; `ci.yml` sets `METRALE_SKIP_BUILD: "1"`

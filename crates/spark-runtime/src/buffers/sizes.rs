...
pub struct BufferSizes {
    ...
    /// FP32 gate logits [m, num_experts] for the METRALE_FP32_GATE routing path.
    ...
    /// FP32 MoE-input norm output [m, hidden] for METRALE_FP32_ROUTING — the
    ...
    /// 0 unless the model is a 128-dim-linear-head GDN model (METRALE_GDN_FLA path).
    ...
    /// (`METRALE_GGUF_NATIVE_Q2=1`). ONE persistent BF16 `[N,K]` buffer sized to
    ...
    /// Native Q2_0 MMQ prefill q8_1 activation scratch (`METRALE_GGUF_NATIVE_Q2_MMQ=1`).
    ...
    pub q2_act_q8: usize,
    /// Row-wise FP8 GDN prefill BF16-weight slab (`METRALE_FP8_ROWWISE=1`).
...
impl BufferSizes {
    ...
    pub fn from_config(
        ...
        // dispatch picks `num_splits` with (`metrale_kernels::attn_splitk`,
        ...
        // METRALE_GDN_FLA dispatch stays disabled. Layout per region:
        ...
        {
            // +margin: the batched FLA path (METRALE_GDN_BATCHED_FLA) sizes its
        ...
        // `METRALE_FP8_ROWWISE=1`. Sizing + the #917 receipt live on

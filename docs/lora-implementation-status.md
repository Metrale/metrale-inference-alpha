# Metrale Engine LoRA v0 — Implementation Status (M0 + M1-attention)
...
```
met serve Hcompany/Holo-3.1-0.8B \
...
- PEFT `adapter_config.json` parser (`metrale-core`), hard-fail with `REJECT(...)` reasons
...
- Dedicated adapter safetensors loader (`metrale-gpu-runtime`) — host F16→BF16 (the base
  `WeightDtype` whitelist rejects F16), header-only OOM preflight, named pickle reject.
- Key remap + per-`LayerType` allow-list (`metrale-model-layers`): only the **full-attention**
...
- `METRALE_LORA_EAGER=1` disables decode-graph capture (debugging hatch); deltas are
...
- **`metrale-core` parser**: 11 unit tests (accept + every named reject) green.
- **Served, live**:
  ...
  - base output `"Paris."` → adapter output **differs** (delta is applied in the live
    decode path);
  - **graph == eager** (`METRALE_LORA_EAGER=1`) — byte-identical, so deltas capture
    correctly inside CUDA graphs.

The test fixture (`test_data/lora-holo-tiny/`) is a **generated** PEFT adapter, deliberately
strong so its effect is unambiguous — no community adapter exists for Metrale Engine's custom
...
- **Runtime rotation (eager-on-rotate)** (`impl_b3.rs`, `decode_a{,2}.rs`): a
  ...
  `METRALE_LORA_ROTATE=1` / `$METRALE_LORA_PEER`) folds into the existing
...
- **Control plane**: `POST /v1/lora/active {"adapter":"NAME"}` → a dedicated
  ...
  default route).
- **RDMA slot-staging** (`metrale-storage/weight_lora_rdma.rs` +
  `metrale-model-layers/lora/rdma_stage.rs`, gated `$METRALE_LORA_PEER`): stage adapter dirs
  on `metrale-weight-peer` and RDMA-load a named adapter's A/B straight into a pool
...
- The RDMA verbs data path is gated on `metrale_rdma_verbs` (rdma-core); without it
...
`cudaMemcpy2DAsync` needs `libcudart` linked; `metrale-gpu-runtime/build.rs` now does so and adds
...
1. **Scheduler.** Under the default `--scheduler fifo`, two concurrent
   requests serialized and never decode-batched, so each fell to the single-seq
   path (= the global *active* adapter). Under `--scheduler slai` they

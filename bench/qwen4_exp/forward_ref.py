#!/usr/bin/env python3
# 2026-09-26: Runs the reference qwen4_exp decoder one layer at a time on a chat fixture
# and saves the tokens, f32 logits at every position and per-layer RMS to an npz.
#
...
METRALE_DUMP_LOGITS_PATH rows (vocab width = model.vocab_size() = 248077 —
...
def dequant_nvfp4(prefix, out=None):
    ...
    process flat."""
    # 2026-09-26: In the cached Flash-Next NVFP4 snapshot, weight is U8 [O, I/2] (two
    # e2m1 codes per byte), weight_scale F8_E4M3 [O, I/16], weight_scale_2 a 0-dim F32.
    packed = raw_tensor(prefix + '.weight')
    scale = raw_tensor(prefix + '.weight_scale')
    scale2 = raw_tensor(prefix + '.weight_scale_2').float()
    lo = packed & 0x0F
    hi = packed >> 4
    def nib(n):
...
def site_state_dict(ckpt_prefix):
    ...
    for k in IDX:
        ...
        if local.startswith('mlp.experts.'):
            continue  # 2026-09-26: fuse_experts loads these.
...
class RowLookup(torch.nn.Module):
    ...
    def __init__(self, ckpt_prefix, dim):
        ...
        assert d == dim, (d, dim)
        # 2026-09-26: The reference's n-gram lookup reads `ngram_embedding.weight.device`
        # (ref/modeling_qwen4_exp.py).
...
def main():
    ...
    # 2026-09-26: QWEN4EXP_EXTRA_IDS: comma-separated token ids appended after the prompt.
    ...
    # 2026-09-26: The QSA indexer reads the mask as `attention_mask == 0`
    # (ref/modeling_qwen4_exp.py), so a None from create_causal_mask is replaced by an
    ...
    # 2026-09-26: The PLE constructor allocates the whole n-gram nn.Embedding
    # (ref/modeling_qwen4_exp.py), so any Embedding over 10M rows is stubbed while the
    # layer is built, and RowLookup replaces it.
    ...
    for i in range(n_layers):
        ...
        # 2026-09-26: QWEN4EXP_SAVE_HIGHWAY=n saves the post-layer highway of the first n
        # layers, for comparison with the engine's METRALE_QWEN4EXP_DUMP taps.

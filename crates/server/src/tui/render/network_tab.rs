...
//! 2026-09-26: The Network section: the TP/EP topology as node cards (one
//! centred card for a single node, up to four joined by connectors), over a
//! detail pane for the selected node.
//!
//! Owner: server tui.
//! Invariants: none beyond the types.
//!
//! A worker's state and the link latency are not measured, and the pane says
//! so instead of drawing a number.
...
fn card_lines(app: &App, rank: usize) -> Vec<Line<'static>> {
    ...
    if is_head {
        ...
        // 2026-09-26: System memory from /proc/meminfo
        // (`metrics_poll::host_mem_gb`), used/total, which answers with or
        // without a model; the GPU figures stay 0.0 until a device reading
        // lands. "—" while /proc/meminfo has not been read.
...
fn draw_single(f: &mut Frame, app: &App, area: Rect) {
    ...
    // 2026-09-26: Decorative dim chevrons either side of the card.
    if card.x >= area.x + 6 {
        let deco = |x: u16| Rect {
            x,
            y: card.y + h / 2,
    ...
    // 2026-09-26: Heartbeat: the border alternates green and dim every 20
    // ticks, 2 s at the 100 ms `events::TICK`.

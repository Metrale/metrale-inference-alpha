...
//
// Hopper Nano diagnostic; FP32 logits, unchanged lower-index tie policy.
...
extern "C" __global__ void nemotron_router_topk_f32(
    const float* __restrict__ gate_logits,  // [num_tokens, num_experts] FP32
    ...
    if (blockIdx.y >= num_tokens) return;
    expert_indices += (size_t)blockIdx.y * top_k;
    expert_weights += (size_t)blockIdx.y * top_k;
    ...
    for (unsigned int i = tid; i < actual_n; i += BLOCK_SIZE) {
        float logit = gate_logits[(size_t)blockIdx.y * num_experts + i];

...
// forked-from: kernels/gb10/deepseek-v4-flash/nvfp4/mla_absorbed.cu (2026-09-24; 13 of 377 lines differ, see kernels/FORKS.md)

// 2026-09-25: MLA helper kernels for the mistral-small-4 tree: the per-head batched GEMV that does Q
// absorption and V extraction, Q rope scatter and writeback (one token and batched), K/V and cache-row
// assembly, and Q_final assembly.
//
...
// 2026-09-25: Per token, K row = [kv_latent | k_rope] and V row = [kv_latent | zeros], each
// mla_cache_dim = kv_lora + rope wide. Grid (N), block max(mla_cache_dim, 256)
...
// (rope)] from q_absorbed [N, nq * kv_lora] and q_rope [N, nq * rope]. Grid-stride; the launcher
// ops::mla_q_final_assemble_batched uses ceil(N * nq * mla_cache_dim / 256) x 256.
...
// wide; K positions past kv_lora + rope are zero. One thread per element, no stride loop
// (ops::mla_cache_assemble launches max(mla_cache_dim, 256) threads).

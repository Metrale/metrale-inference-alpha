...
There is a SECOND self-hosted pool with a different bargain. `metrale-pr-cheap`
...
# Two pools, same bargain, different capability: `metrale-pr-cheap` is every
# runner, `metrale-pr-rust` is the subset carrying a Rust toolchain (a bounded
# subset ON PURPOSE -- eight concurrent `cargo test --workspace` target
# directories would fill the disk).
PR_POOLS = (
    ("metrale-pr-cheap", "PR_CHEAP_RUNNER"),
    ("metrale-pr-rust", "PR_RUST_RUNNER"),
)
...
MAC_POOL_MARKERS = ("MACOS_RUNNER_LABEL", "metrale-macos-burst")
...
FLEET_GATE = "vars.USE_METRALE_UBUNTU_RUNNERS == '1'"
    ...
    for path in sorted(pathlib.Path(".github/workflows").glob("*.yml")):
        ...
        for name, job in (wf.get("jobs") or {}).items():
            if not isinstance(job, dict):
                continue
            pool = pr_pool_of(job)
            # Required on the two metrale Ubuntu pools, which is the hardware the

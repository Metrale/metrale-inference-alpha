...
# once with METRALE_SSM_INPLACE_VERIFY unset (baseline) and once with =1; diff
...
MODE="${2:-inplace0}"
PORT="${PORT:-8891}"
SPARK="${SPARK:-/workspace/metrale-mtp/target/release/met}"
...
if [[ "$MODE" == "inplace1" ]]; then
  export METRALE_SSM_INPLACE_VERIFY=1
else
  unset METRALE_SSM_INPLACE_VERIFY || true
fi

# Metrale Engine on DGX Spark GB10 — Deployment & Compatibility Guide

New to Metrale Engine on a DGX Spark? Start here. Read this **before** you pick a model: which
...
| Image | `metrale/metrale-inference-gb10:latest` — one **multi-model** binary; the right kernel set is auto-selected at startup from the model's `config.json` |

**Prerequisites**, in order:
1. NVIDIA driver ≥ 580 — verify: `nvidia-smi` shows CUDA 13.0+. (There is no
   driver-version gate in the engine and **no `METRALE_SKIP_DRIVER_CHECK` escape
...
3. A clean GPU before launch: `nvidia-smi` should show no other process holding
   VRAM. Metrale Engine sizes its KV cache from *free* memory at boot.
...
Every model below runs on the **same** `metrale/metrale-inference-gb10:latest` image. "Quant"
...
catalogue — a feel, not a benchmark. "Ctx" is a **practical** single-DGX-Spark ceiling
...
`metrale/metrale-inference-gb10:latest` with the recipe flags.
...
| **Biggest quality you can host** | `Qwen3.5-122B-A10B-NVFP4` on **EP=2** (2 DGX Sparks) | 122B/10B across two nodes, ~51 tok/s |
...
1. **"No MTP weights found" / missing `extra_weights.safetensors` (35B under a
   ...
   docker run ... -v /models/qwen3.5-35b:/model metrale/metrale-inference-gb10:latest \
...
4. **Native binary: driver / glibc errors.** The tarball needs driver 580
   ...
   use the Docker image. There is no bypass env var; `METRALE_SKIP_DRIVER_CHECK`
...
curl -s http://localhost:8888/v1/chat/completions -H 'Content-Type: application/json' -d \
  '{"model":"metrale","messages":[{"role":"user","content":"What is 2+2? Reply with just the number."}],"max_tokens":16,"temperature":0}'
```
...
DGX Sparks, one OpenAI endpoint on rank 0. Launch via the canonical launcher:
...
| Port already in use | A stale container/`met serve` on 8888/8889/29500. `docker ps`, stop it, retry. |
...
A Metrale Engine image is only cut after it passes the **serve matrix** — every model×quant
in §2 boots, stays coherent, and holds four quality signals. If you're evaluating
Metrale Engine or reproducing a claim, these are the signals and where they live:
...
| **KL-divergence** | Logits stay faithful to a BF16 reference (argmax agreement, KL nats) | `bench/fp8_dgx2_drift/` |
| **ST-subset accuracy** | Tool-calling accuracy on a BFCL single-turn subset | external gorilla / `spark-arena` harness (see note) |

How the release pipeline turns these into a pass/fail before an image ships is
in `tests/gate_results.py`. *Note:* the ST-subset /

...
(`libmetralegdn.so`, `gdn_holo.so`) and of `delta_rule_sm120_aot_export.patch`, plus the
...
g++ 13.3.0 (Ubuntu 24.04 aarch64) + nvcc 13.x. The committed `libmetralegdn.so`'s
...
**Target architecture of a rebuild** — the pinned FlashInfer rev compiles the kernel with
`cute.GPUArch("sm_120a")` (`delta_rule_sm120.py`, the `cached_compile` call), and that
option overrides `CUTE_DSL_ARCH`. With `nvidia-cutlass-dsl[cu13]==4.5.0` the exported
`gdn_holo_0.o` therefore embeds an sm_120a cubin only, and on GB10 (sm_121) the resulting
`libmetralegdn.so` returns 98 with `cudaErrorNoKernelImageForDevice` (verified 2026-09-24 on a
GB10 with driver 580.126.09, no cuda-13.2 compat stack; the export itself succeeds there).
The committed library embeds sm_121 cubins and runs on GB10. A rebuild meant to replace
it must compile for sm_121a (change that `GPUArch` to `sm_121a` in the FlashInfer checkout
before step 3) and be validated bit-for-bit against the committed library's output before
it is re-pinned.
...
NOT the .o that produced the committed binaries), `libmetralegdn.so` `d4f8b2bf…` vs pinned
...
Route A (integrate FlashInfer GDN into Metrale). The 11-13× scan is proven; this dir holds the AOT bridge.
...
`gdn_shim.cpp` wraps the header's static-inline funcs into extern "C" `metrale_gdn_load` + `metrale_gdn_prefill`
(shape-generic, head_dim D=128 fixed). Built into `libmetralegdn.so` (bundles gdn_holo_0.o + cute runtime).
`gdn_rs.rs` is a pure-Rust harness (raw cudart + metralegdn FFI, no cudarc) that loads ref IO, calls the kernel,
compares: **metrale_gdn_prefill ret=0, max_abs_err=0.000000, cos=1.000000.** Full chain Rust->C shim->AOT GDN proven.
Build/run (gx10):
  g++ -O2 -fPIC -shared gdn_shim.cpp gdn_holo_0.o -o libmetralegdn.so -I. -I/usr/local/cuda/include -lcudart -L<cute> -lcute_dsl_runtime -Wl,-rpath,<cute>
...
## NEXT (step 4 — wire into Metrale proper)
- Move the shim into a real Metrale crate (build.rs links gdn_holo_0.o + cute runtime; or dlopen libmetralegdn.so).
- Convention adapter: Metrale log-space cumulative gate gc -> FI linear per-token alpha=exp(gc per-token); qk-l2norm; state layout.
...
## STEP 4a DONE 2026-06-30 — Metrale-NATIVE layout adapter is BIT-EXACT ✅
KEY FINDING: Metrale `gate` is ALREADY linear α (kernel gated_delta_rule_fla.cu:16 "gate[] LINEAR decay
(NO exp)"; recompute_wu applies logf itself) == FlashInfer's alpha. NO gate-space conversion needed.
So the adapter is pure layout:
- q/k/v: pass Metrale packed QKV ([Q(key_dim)|K|V(value_dim)] bf16, row stride conv_dim) DIRECTLY via
  conv_dim strides (q/k strides{conv_dim,kd}, v{conv_dim,vd}) — NO copy.
- gate/beta: deinterleave Metrale [gate(nv)|beta(nv)] fp32 (stride 2nv) -> contiguous alpha,beta[T,nv]
  via cudaMemcpy2DAsync (in-shim).
- output: Metrale contiguous [T,value_dim] -> o strides{nv*vd, vd}.
New shim entry `metrale_gdn_prefill_packed(qkv,gate_beta,output,h_state,init_state,tensormaps,cu,
  scale,total,nk,nv,kd,vd,conv_dim,gb_stride,num_seqs,stream)` takes Metrale's EXACT native pointers.
gdn_harness_packed.cpp packs the ref IO into Metrale layout -> **bit-exact (max_abs_err=0, cos=1.0).**
=> Metrale call site becomes trivial: hand over the pointers prefill_gdn_full_inner already has
(q_ptr=gdn_bufs.qkv, gate_ptr=gdn_bufs.gate_beta, gdn_bufs.output, ssm_state.h_state, dims).

## STEP 4 remaining
- Metrale Rust binding: dlopen libmetralegdn.so (no build.rs link-time dep) OR build.rs link; call
  metrale_gdn_prefill_packed from prefill_gdn_full_inner behind METRALE_GDN_FLASHINFER=1 (FLA fallback).
- STATE-CARRY layout: validated single-call full-sequence (init_state=0). Multi-chunk prefill carries
  h_state across outer chunks -> verify FI state layout == Metrale h_state ([nv,kd,vd]) for the carry
  (FI test transposes state; check before enabling chunked).
- Ship libmetralegdn.so + libcute_dsl_runtime.so + cuda-13.2/compat in the container.
...
1. cross-impl A/B (gdn_fla_vs_fi example): Metrale FLA vs FlashInfer on identical input -> cos=0.999993,
   norm_ratio=1.0 => GDN MATH IS EQUIVALENT (garbage was NOT a math/convention bug).
2. The garbage was a DTYPE mismatch: export/harness used fp16, but Metrale GDN q/k/v/o are BF16. The fp16
...
3. Also fixed an async use-after-free (binding freed tensormaps/init/cu before the async kernel ran) ->
   managed shim entry metrale_gdn_prefill_packed_managed caches scratch internally (no per-call alloc/free/sync).
RESULT (holo35b, same binary, A/B via METRALE_GDN_FLASHINFER 1 vs 0):
...
- DECODE DRIFTS: 'The' then garbage = the known state k<->v transpose (FI writes S[v][k], Metrale decode
...
- `libmetralegdn.so`→ differed run-to-run only via linker build-id; fixed by

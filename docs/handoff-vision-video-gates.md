...
* It wrote **only** `tests/fixtures/videos/`, which nothing reads. The compiled-in copy under `crates/metrale-plugin/assets/video/` was never regenerated by it — so a fixture fix would have silently changed nothing. It now writes both from one encode.
...
* **`tokenizer::tests::qwen_dense_parity` is flaky** under `cargo test --workspace`, ~2 runs in 5, a different test each time; passes in isolation and under `-p spark-server --bin met`. The diff is JSON whitespace in the rendered tools block — two serialization paths and some shared state choosing between them under parallelism. Unrelated to vision/video.

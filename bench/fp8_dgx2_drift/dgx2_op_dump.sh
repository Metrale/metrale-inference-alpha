#!/usr/bin/env bash
# Run metrale-gb10:op-drift on dgx2 with all op-dump env vars enabled.
...
# METRALE_GDN_DUMP_LAYERS uses SSM-relative indices (0..29 for A3B).
# Want ALL 30 SSM layers covered, so list 0..29.
SSM_LAYERS=$(seq -s, 0 29)

# METRALE_OP_DUMP_LAYERS uses ABSOLUTE layer indices for full-attn (3,7,...39).
# Leaving unset means "all layers" — we want full-attn for full-attn-only ops
# but also the input/post norms for SSM layers via METRALE_OP_DUMP itself.

...
/// house convention (`METRALE_NO_GDN_WY2_RESIDENT=0` is NOT off), read once
/// per process.
...
impl Qwen3SsmLayer {
    ...
    ///
    /// Opt-in via `METRALE_GDN_FUSED_VERIFY=1` (default OFF — the per-token path
    ...
    pub(super) fn wy2_kernel(
        ...
        // METRALE_SSM_H_FP16 stage 2: under the flag the h-state in the pool is
    ...
    pub(super) fn wy3_kernel(
        ...
        static LOGGED_ENGAGED: std::sync::Once = std::sync::Once::new();
        static LOGGED_BASE: std::sync::Once = std::sync::Once::new();
        // METRALE_SSM_H_FP16 stage 2 — see `wy2_kernel` for the rationale.
        if super::ssm_h_fp16_enabled() {
    ...
    /// `METRALE_SSM_H_FP16` — its FP16 h-state twin. K=4 is the widths-1..8
    ...
    pub(super) fn decode_batched_conv_gdn(
            ...
            && !super::ssm_h_fp16_enabled()
        {
            ...
            // GDN fallback at these widths. Kill-switch: METRALE_GDN_WYN=0. ──

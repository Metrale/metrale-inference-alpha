...
//! 2026-09-25: Layer 0's MoE against the golden: routing indices exactly,
//! routing weights within `F32_TOL`, the block's FFN output within bf16.
//! Input is the `L0.ffn_in` full capture.
//!
...
fn routing_matches_exactly() {
    ...
    for r in &f.regimes {
        ...
        // 2026-09-25: Renormalised top-k times route_scale sums to route_scale
        // per token.
        for t in 0..tokens {
...
fn shared_expert_alone_is_not_the_answer() {
    // 2026-09-25: Guards against a routed path that contributes nothing.
    let f = fx();
    let p = params(&f);
    let r = &f.regimes[0];
    let x = full_f32(&f.g.tensor(r, "L0.ffn_in"), "L0.ffn_in");

...
//! 2026-09-26: Tests for configuration, planning against a fake host, and the
//! restore contract.
//!
...
async fn cleanup_restores_the_box_once_a_plan_exists() {
    ...
    configured(&mut b, |_| {}).unwrap();
    // 2026-09-26: No plan yet, so `cleanup` does not call `restore`.
...
fn without_a_host_the_benchmark_says_what_is_missing() {
    // 2026-09-26: `load` fails with this message, so it names what is absent.

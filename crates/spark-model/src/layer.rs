...
pub use transformer_layer::{
    MTP_CATCHUP_MAX, TransformerLayer, VERIFY_WY_LAYER_STRIDE_BYTES, VERIFY_WY_TABLE_SEQS,
...
/// full-attention layers (Metrale Engine #753 item B).
#[derive(Default)]
...
pub struct SsmLayerState {
    ...
    /// only on the layer that hosts a `PleLayer` (Metrale Engine #753 item B: one per
...
pub struct ForwardContext<'a> {
    ...
    /// Default-ON mid-chunk SSM tail capture (opt-out `METRALE_SSM_TAIL_MIDCHUNK=0`).
    ///

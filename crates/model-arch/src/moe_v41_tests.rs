...
//! 2026-09-25: The streamed MoE (`MoeV41::forward`) on synthetic Q2_K / Q3_K
//! experts held in a pinned, device-visible arena, against four oracles:
//!   * routing: the reference `gate`, indices exact and weights within
//!     `1e-5 * max(|w|, 1)`;
//!   * output: the production numerics emulated on the CPU (the loader's own
//!     decoders for the weights, q8_1 emulation for the activations, bf16
//!     GEMMs), within `max * 2^-7 + 1e-3`;
//!   * output: the reference's `moe` on the dequantised weights, within
//!     `max * 2^-5 + 1e-2`;
//!   * at one token: the CUDA-graph split (`stage_m1` + a captured
//!     `compute_m1`) equals the eager forward bit for bit.
//!
...
use super::*;
use crate::deepseek_v41_ref::moe::{MoeCfg, MoeWeights, gate as ref_gate, moe as ref_moe};
use metrale_model_weights::weights::dequant_cpu::{GgmlType, dequant_to_f32};
use metrale_model_weights::weights::expert_stream::{ExpertLru, ExpertSource, SlotLayout};
...
const SCALES_F16: [u16; 4] = [0x2C00, 0x2800, 0x3000, 0x2400]; // 2026-09-25: 2^-4, 2^-5, 2^-3, 2^-6
...
/// 2026-09-25: `[n][k]` raw super-blocks of random bytes, the fp16 scale
/// fields at `f16_offsets` overwritten from `SCALES_F16`.
fn build_blocks(
    n: usize,
    k: usize,
    block_bytes: usize,
...
/// 2026-09-25: `y[i][o] = bf16(sum x[i][d] * w[o][d])`, summed in f32.
...
fn run_case(tokens: usize) {
    ...
    use metrale_model_weights::weights::expert_stream::PinnedArena;
    let gpu = backend();
    ...
    // 2026-09-25: Experts: gate/up Q2_K [INTER, DIM], down Q3_K [DIM, INTER].
    let ex = Experts {
        gate: (0..N_ROUTED)
    ...
    // 2026-09-25: Oracle 1: the routing against the reference gate.
    ...
    // 2026-09-25: Oracle 2: the production numerics emulated on the CPU.
    ...
    let shared =
        crate::deepseek_v41_ref::moe::expert(&x, &s1, &s2, &s3, tokens, DIM, INTER, 10.0, None);
    ...
    // 2026-09-25: Oracle 3: the reference's own MoE on the dequantised weights, loosely.
    ...
    // 2026-09-25: Oracle 4 (one token): the CUDA-graph split of the same step.
    // The router GEMV, the host span (`stage_m1`: selection, fetch, plan
    // uploads), then `compute_m1` captured once and replayed, must be the
    // eager forward's output bit for bit.
    if tokens == 1 {
...
/// 2026-09-25: The single-token arm (`routed_m1`) adds every expert row into
/// the one token row of `acc`. This runs that arm at 6 experts a token and
/// requires the output to be the same bytes on every run.
#[test]
#[ignore = "requires a CUDA GB10 + the deepseek-v4-flash kernel target"]
fn single_token_expert_sum_is_bit_identical_across_runs() {
    use metrale_gpu_runtime::gpu::GpuBackend;
    use metrale_model_weights::weights::expert_stream::PinnedArena;
    const TOPK_MODEL: usize = 6; // 2026-09-25: deepseek41.expert_used_count

#!/usr/bin/env bash
# Launch Metrale Engine FP8-native server on dgx2 with METRALE_NEMO_DUMP enabled.
# Mirror the dgx1 metrale-qwen-final config minus the request-dump path.
set -euo pipefail

DUMP_DIR="/workspace/metrale-dumps/fp8native_dgx2"
mkdir -p "${DUMP_DIR}"

# Match dgx1 metrale-qwen-final args: max-seq-len 65536, max-batch 4, gpu-mem 0.88,

...
//! The SPILL-side cost gate (`METRALE_SSM_SPILL_MIN_TOKENS`) — the missing half
//! of the tier's cost model.
//!
//! The fault-in side has had a depth gate since task #5
//! (`METRALE_SSM_FAULT_MIN_TOKENS`, `trait_impl/ssm_fault_in.rs`). The spill side
...
/// this from a fresh `METRALE_SSM_TIER_TIMING` line if the gather changes again.
const SPILL_COST_MS: usize = 45;

/// Minimum victim depth (tokens) worth spilling rather than dropping.
/// `METRALE_SSM_SPILL_MIN_TOKENS` overrides; `0` disables the gate.
///
...
/// Pure parse of `METRALE_SSM_SPILL_MIN_TOKENS`: unset or unparseable falls back

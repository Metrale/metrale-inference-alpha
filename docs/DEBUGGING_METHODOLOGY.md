# Metrale Engine Debugging Methodology

A playbook for tracking down quality regressions in Metrale Engine — distilled from the
...
reference framework (vLLM, HF Transformers) but degrades under Metrale Engine. Same
...
much cheaper explanations. Most "Metrale Engine math is broken" reports turn out to be one
...
Metrale Engine bug.* (See `feedback_never_blame_model.md` in the memory index.)
...
|---|---|---|---|
| 1 | **Sampler/preset** is different from the reference | 5 min | Run both Metrale Engine and reference under greedy (`temperature=0`). If the gap closes, it's sampling. If not, sampling is innocent — keep going. |
| 2 | **Prompt template render** differs | 10 min | Diff the rendered prompt strings (Metrale Engine logs the Jinja-rendered output; vLLM has `print_chat_template_result`). One byte of drift is enough. |
...
| 4 | **Termination logic** (stop tokens, length cap) differs | 5 min | Look at `finish_reason`. If reference says `stop` and Metrale Engine says `length`, that's a stop-condition mismatch, not a math bug. |
...
the early bugs because Metrale Engine was terminating "cleanly" on the lazy-stub loop.
...
2. **Don't hard-code the token-ID list in the oracle script.** Read it back from
   ...
   a 52-token sequence that no longer matched what Metrale Engine was rendering.
...
magnitude ratios (Metrale/oracle) across all heads of one layer:
...
Metrale Engine supports multiple model families through largely-parallel loader trees:
...
1. **Quality gate harness** (`bench/longcode/harness.py` + `analyze.mjs`) — pays
   ...
   improved anything.
2. **Per-layer dump pattern** (env-gated `METRALE_DUMP_LAYERS` reading headerless
   bf16 `.bin` per `(layer, position, kind)`). Pattern in
   `crates/model-layers/src/layers/vision_encoder/enc_impl/utils.rs:38-64` —
...
Background reproductions on Metrale Engine range from minutes (smoke tests) to hours (a
...
env-gated `METRALE_FP8_SSM_PREFILL=1`; promoted to unconditional 2026-05-20
...
*Maintained alongside Metrale Engine. Update when you learn something this document
doesn't already say.*

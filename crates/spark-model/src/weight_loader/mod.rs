...
///
/// `METRALE_MOE_PREFILL_COPIES=0` forces the fallback — an A/B lever and an
...
/// the runtime NVFP4 format used by Metrale Engine GEMM kernels.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub trait ModelWeightLoader {
    ...
    /// Metrale Engine's rms_norm kernel). Loaders that don't need it should ignore
    /// the argument.
    fn load_final_norm(
        &self,
        store: &WeightStore,

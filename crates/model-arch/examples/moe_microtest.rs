...
//! 2026-09-25: Correctness and rough-throughput test for the routed-expert FP8 grouped GEMM
//! (`moe_fp8_grouped_gemm`): per-expert FP8 weights and 128x128 FP32 block-scale pointer tables,
//! `expert_offsets`, `sorted_token_ids` and a tile work-list, against a CPU reference with the
//! kernel's two-level FP32 accumulation (inner over a 128-wide K block, then
//! `outer += inner * block_scale`).
//!
//! Owner: model-arch examples (MoE kernels).
//! Invariants:
//! - Unless `perf` mode is selected, the run exits 1 when the cosine against the CPU reference
//!   is below `COSINE_GATE` or not finite.
//!
//! GPU and CPU are not bit-identical (the MMAs sum in a different order, and the output is
//! rounded to BF16), so the gate is a cosine, not byte equality.
//!
//! Usage:
//!   cargo run --release -p metrale-model-arch --example moe_microtest \
//!       -- `[kernel] [num_experts] [tokens_per_expert] [N] [K] [seed] [mode]`
//! Defaults: moe_fp8_grouped_gemm 4 20 256 256 0x9E3
//!
//! `[kernel]` names an entry point of the `moe_fp8_grouped_gemm` module. `[mode]` (arg 7):
//!   perf      skip the CPU reference.
//!   skew      give expert 0 seven times the average token count (clamped to the total), so it
//!             spans more than one 128-row M tile at the defaults.
//! The work-list is built on the host in the layout `moe_build_tile_worklist` writes.
...
// 2026-09-25: CUDA driver events for kernel-only timing; the wall-clock figure also includes
...
fn main() -> Result<()> {
    ...
    let tpe: usize = args.get(3).map_or(20, |s| s.parse().unwrap());
    ...
    // 2026-09-25: Arg 7 is one mode word: `perf` or `--perf`, `skew` or `--skew`.
    ...
    // 2026-09-25: A is [total, K] BF16; each sorted row is its own token (identity
    // `sorted_token_ids`).
    let a_bf16: Vec<u16> = (0..total * k)
        .map(|_| f32_to_bf16_bits(rng.uniform(-1.0, 1.0)))
        .collect();
    // 2026-09-25: Per-expert row counts, prefix-summed into `expert_offsets`. `skew` gives
    // expert 0 seven times the average (clamped to `total`) and spreads the rest over the
    // other experts; `total` is unchanged, so A and the identity ids stay valid.
    ...
    // 2026-09-25: Per-expert weights [N, K] E4M3 with exponent field at most 7, and FP32
    // scales [ceil(N/128), K/128].
    ...
    // 2026-09-25: C is [total, N] BF16, zeroed.
    let c_ptr = upload_bytes(gpu, &vec![0u8; total * n * 2])?;

    // 2026-09-25: The work-list in `moe_build_tile_worklist`'s layout: per item the expert, then
    // `(m_tile << 6) | n_tile`, over 128x64 tiles, skipping experts with no rows (no weight
    // pointer is NULL here). The tile sizes are the kernel's `PM4_M_TILE` and `PM4_N_TILE`.
    ...
    let cosine = if perf_only {
        ...
        // 2026-09-25: CPU reference per expert, with the two-level FP32 block-scale
        // accumulation.
    ...
    // 2026-09-25: Two events bracket `iters` launches with no host sync between them, so the
    // elapsed time is GPU time for the batch.

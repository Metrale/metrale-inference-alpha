...
//! 2026-09-25: Tests for the TP shard primitives on `MockGpuBackend`: the dense
//! column / row slices and their error paths, and the GDN segmented copy plans.
//!
...
// 2026-09-25: `gdn`'s plan helpers are `pub(crate)`, so `pub use gdn::*` does not
// re-export them; they are imported from the submodule.
...
// 2026-09-25: GDN segmented slices: the copy plans the device slicers execute,
// and the slicers themselves on `MockGpuBackend`.

/// 2026-09-25: Synthetic GDN dims: full_nk=4, kd=8, full_nv=8, vd=16, tp=2 → local
...
fn gdn_dims_derived() {
    ...
    assert_eq!(d.full_conv_dim(), 2 * 32 + 128);
    assert_eq!(d.local_conv_dim(), 2 * 16 + 64);
    assert_eq!(d.full_qkvz_out(), 192 + 128);
    assert_eq!(d.local_qkvz_out(), 96 + 64);
    assert_eq!(d.qkv_segments(), [32, 32, 128]);
    assert_eq!(d.qkvz_segments(), [32, 32, 128, 128]);
}

/// 2026-09-25: The QKVZ segmented plan must slice Q, K, V, Z each by the local head range
...
fn qkvz_segment_plan_is_segmented_not_contiguous() {
    ...
    assert_eq!(local_rows, d.local_qkvz_out());

    // 2026-09-25: Full segment starts (rows): Q@0, K@32, V@64, Z@192.
    ...
    // 2026-09-25: A contiguous "second half for rank 1" slice would put rank 1's
    // Q source at row 160, not 16.
    assert_ne!(ops[0].src_off, 160 * row_bytes);
}

/// 2026-09-25: Rank 0 + rank 1 slices must exactly tile the full buffer with no overlap
...
fn qkvz_two_rank_reconcat_tiles_full() {
    ...
    let full_rows: usize = segs.iter().sum();
    let h = d0.h;

    // 2026-09-25: Synthetic full weight: row r filled with value r (u16), h cols each.
    ...
    // 2026-09-25: Expected per-segment source rows for each rank.
    // Q: r0=[0..16]  r1=[16..32]
    // K: r0=[32..48] r1=[48..64]
    ...
    // 2026-09-25: Union of both ranks (per segment) == the full set of rows: no
    // overlap, no gap.
...
/// 2026-09-25: conv1d uses the same [Q|K|V] 3-segment split but with row width = d_conv.
#[test]
fn conv_segment_plan_uses_qkv_segments() {
    let d = synth_dims(0);
    let d_conv = 4usize;
    let row_bytes = d_conv * BF16_BYTES;
    let (ops, local_rows) =
        segment_copy_plan(&d.qkv_segments(), row_bytes, d.tp_rank, d.tp_size).unwrap();
    assert_eq!(local_rows, d.local_conv_dim());
    // 2026-09-25: Rank 0: Q[0..16], K[32..48], V[64..128]; packed at 0,16,32.
    let want = [
        CopyOp {
            src_off: 0,
            dst_off: 0,
...
/// 2026-09-25: BA is per-group interleaved but the rank boundary lands on a group
...
fn ba_single_segment_group_aligned() {
    ...
    assert_eq!(local_rows, 2 * d.local_nv);
    // 2026-09-25: 2*full_nv = 16 rows; rank 1 → rows [8..16).
    assert_eq!(ops.len(), 1);
    assert_eq!(ops[0].src_off, 8 * row_bytes);
    assert_eq!(ops[0].dst_off, 0);
    assert_eq!(ops[0].len, 8 * row_bytes);
    // 2026-09-25: Group size = 2*vpg = 2*(nv/nk) = 2*2 = 4 rows; 8 is a multiple → the
    // slice starts on a group boundary.
    let vpg = d.full_nv / d.full_nk;
    assert_eq!((8) % (2 * vpg), 0);
}

/// 2026-09-25: Value-vector 1D shard on the value-head axis: a BF16 vector with
/// `vd` elements per head, and an FP32 `[full_nv]` vector (`a_log` / `dt_bias`).
#[test]
...
/// 2026-09-25: Holo-3.1-0.8B dims (kernels/gb10/holo-3.1-0.8b/MODEL.toml):
/// 16 key heads x kd=128, 16 value heads x vd=128, h=1024, tp=2. The copy plan
/// at a real model's shapes, where the Q/K/V/Z segments are equally wide
/// (unlike the synthetic dims above, where V/Z are 4x wider than Q/K).
#[test]
fn qkvz_plan_holo_dense_dims() {
    ...
    // 2026-09-25: Full segment starts (rows): Q@0, K@2048, V@4096, Z@6144.
    ...
    // 2026-09-25: Rank 0 takes each segment's first half.
    let d0 = mk(0);
...
/// 2026-09-25: A segment that does not divide by `tp_size` is an error.
#[test]

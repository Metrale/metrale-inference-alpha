...
#
# ONE Metrale Engine instance at a time: --gpu-memory-utilization RESERVES its whole
...
#
# LongCat ships plain BF16 with no NVFP4/FP8 calibration metadata, so Metrale Engine
...
#
# ★ METRALE_LONGCAT_FP8_EXPERTS=1 is the one to reach for first: -26% KL for
...
#   METRALE_NVFP4_MLA=0 — better quality AND faster.
#
...
set -euo pipefail
# The repository root, where target/release/met is built.
cd "$(dirname "$0")/../.."
...
export METRALE_NGRAM_CACHE_SLOTS="${METRALE_NGRAM_CACHE_SLOTS:-65536}"

echo "LongCat-Flash-Lite  ->  port ${PORT:-8888}   (TUI: needs an interactive terminal)"
exec target/release/met serve \

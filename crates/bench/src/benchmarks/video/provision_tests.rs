...
//! 2026-09-26: Tests for the clip set and its provisioning.
//!
//! Owner: bench, video.
...
fn tmp(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("metrale-video-{name}-{}", std::process::id()));
...
/// keeps the order pair, the parity pair and the 1:2:4 durations.
#[test]
...
/// 2026-09-26: Every clip is non-empty and under 64 KiB, since each is embedded
/// in the binary.
#[test]
...
/// 2026-09-26: Each clip's magic bytes match its MIME type. The server picks
/// the GIF decoder from the bytes (`model-layers/src/video_preprocess.rs`), so
/// a mislabelled clip would test a different decoder than its name says.
#[test]

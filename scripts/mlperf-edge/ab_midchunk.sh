...
#
# THE SITUATION
# `METRALE_SSM_TAIL_MIDCHUNK` is DEFAULT-ON in the code
# (metrale-gpu-runtime/src/lib.rs: `!matches!(var.as_deref(), Ok("0"))`). The frozen
...
# METRALE_SSM_TAIL_LEASE_TTL=128 currently govern an empty set (snapshot.rs says
# so in as many words).
#
...
# `-e METRALE_SSM_TAIL_MIDCHUNK=0` in place keeps it OFF. The two legs below differ
# only by whether that -e line is emitted.
#
# Usage: ab_midchunk.sh <metrale_bin> <outdir> [bfcl_subset_n]
set -u
...
for leg in mc_off mc_on; do
  case $leg in
    mc_off) MC="-e METRALE_SSM_TAIL_MIDCHUNK=0" ;;   # today's frozen config
    mc_on)  MC="" ;;                               # the code's actual default
  esac
  sudo docker rm -f metrale-mc >/dev/null 2>&1; sleep 3
  # shellcheck disable=SC2086
  sudo docker run -d --name metrale-mc --network host --gpus all --ipc=host \
    -e METRALE_NO_FFN_NVFP4_MMQ=1 $MC -e METRALE_MTP_CATCHUP=0 \
  ...
  for _ in $(seq 1 180); do
    ...
    sudo docker ps --format '{{.Names}}' | grep -q metrale-mc || { echo "SERVE_DIED leg=$leg"; break; }
    sleep 5
  done
  [ $ok -eq 1 ] || { sudo docker logs metrale-mc 2>&1 | tail -40 > "$OUT/$leg.died.txt"; continue; }

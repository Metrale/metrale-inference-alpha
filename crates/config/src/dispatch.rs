...
//! 2026-09-26: `parse_config`: route a `config.json` to its family parser by the top-level
//! `model_type`, then fill the full stop-token set.
//!
//! Owner: config.
//! Invariants:
//! - On success, `eos_token_ids` is non-empty and its first element is `eos_token_id`.
...
/// 2026-09-26: Parse a checkpoint `config.json` into a [`ModelConfig`]: the family parse, then
...
/// 2026-09-26: Collect every declared stop-token id into `eos_token_ids`, primary first.
///
/// `eos_token_id` may be a scalar or an array, at the top level or inside `text_config`; the
/// family parsers keep one id in the `u32` `eos_token_id`, and this recovers the rest.
/// `config.eos_token_id` is always element 0, whatever the family parser chose (laguna takes
/// the first array entry, step3p7 the last). The other ids follow in config order,
/// de-duplicated. Invalid JSON leaves the config unchanged.
...
fn parse_config_dispatch(json: &str) -> Result<ModelConfig> {
    ...
    // 2026-09-26: A remote-code checkpoint may declare only `architectures` and no `model_type`.
    // The known architecture names map onto their `model_type`; the map is read only when
    // `model_type` is absent or empty.
    ...
    match top_model_type {
        "qwen3_vl_moe" | "qwen3_5_moe" | "qwen3_5" => {
            ...
            // 2026-09-26: A MoE checkpoint with interleaved MRoPE becomes `qwen3_6_moe`, the
            // `model_type` the Qwen3.6 MoE kernel targets declare. Dense `qwen3_5` keeps its
            // name and only records the MRoPE fields.
            if let Some(rope_params) = text_config.get("rope_parameters") {
            ...
            // 2026-09-26: Holo-3.1: a `qwen3_5_moe` config with a vision tower, no MTP
            // layers and `image_token_id` 248056 becomes `holo3_1_moe`. The MTP check keeps
            // Qwen3.6-35B-A3B, whose config declares `mtp_num_hidden_layers` 1 and the same
            // image token, off this path.
        ...
        "nemotron_h" | "nemotron_h_puzzle" => {
            // 2026-09-26: Puzzle configs put the hybrid schedule in `layers_block_type` /
            // `block_configs` and may leave `num_hidden_layers` null. Rewrite the JSON so serde
            // can deserialize it, then derive the per-layer schedule below.
        ...
        // 2026-09-26: GLM-5.3-Flash; its fields sit under `text_config`.
        "glm5_next" | "glm5_next_text" => parse_glm5_next(json),
        // 2026-09-26: Kimi K3: the parser unwraps `text_config` when present (a flat
        // `kimi_linear` config is read as is) and renames the family to `kimi_k3`.
...
/// 2026-09-26: Rewrite Puzzle HF JSON so serde can load it as `ModelConfig`.
///
/// - `num_hidden_layers` null, 0 or absent: set to the length of `layers_block_type`, or
///   of `block_configs` when that is missing.
/// - `moe_intermediate_size` / `num_experts_per_tok` absent or 0: set to the maximum over
///   `block_configs`, when some block declares one.
...
/// 2026-09-26: Map Puzzle `layers_block_type` onto `layer_types`, and the `moe` entries of
/// `block_configs` onto the per-layer MoE sizes (0 for other layers).

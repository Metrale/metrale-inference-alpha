...
//! 2026-09-25: `gated_delta_rule_prefill_regresident` (GDN prefill with the state in
//! registers) against `gated_delta_rule_prefill` (state in shared memory), from the same
//! initial state, at Qwen3.6-35B-A3B GDN dimensions.
//!
//! Owner: model-arch examples.
//! Invariants:
//! - Exit 1 unless the per-token output and the final state both reach cosine >=
//!   `COSINE_GATE`.
//!
//! With `METRALE_BENCH_ITERS` set, the two kernels, and
//! `gated_delta_rule_prefill_persistent_wy4` when the target has it, are also timed.
//! seq defaults to 256; the seed is hex.
//!
//! Usage: cargo run --release -p metrale-model-arch --example gdn_regresident_microtest \
...
/// 2026-09-25: Bind the GDN prefill argument list shared by the three kernels. A function
/// rather than a closure, so the returned launch can carry the lifetime `'a`.
...
fn main() -> Result<()> {
    ...
    // 2026-09-25: Small magnitudes keep the state bounded over the whole sequence: neither
    // kernel under test clamps the state norm.
    ...
    // 2026-09-25: One state buffer per kernel: each updates its state in place.
    ...
    // 2026-09-25: Reference: grid (nv, batch, 1), state plus k and q in dynamic shared memory.
    ...
    // 2026-09-25: Under test: grid (nv, batch, vd / 4), 4 warps per block, no shared memory.
    ...
    // 2026-09-25: Timing: 10 warm-up launches, then the mean over `iters` (50 when the value
    // does not parse), per kernel.

...
sudo docker rm -f metrale-deepseek-ep0 2>/dev/null || true
ssh "$WORKER_IP" "sudo docker rm -f metrale-deepseek-ep1 2>/dev/null || true"
...
echo "Monitor rank 0: sudo docker logs -f metrale-deepseek-ep0"
echo "Monitor rank 1: ssh $WORKER_IP 'sudo docker logs -f metrale-deepseek-ep1'"

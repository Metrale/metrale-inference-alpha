...
# Extra `-e K=V` flags, space separated. Used for profiling (METRALE_GLM_PROFILE=1) and for
...
FABRIC CHECK (do this before anything else):
  ssh cluster@10.10.10.1 'docker logs metrale-glm53-r0 2>&1 | grep -E "NET/IB|NET/Socket"'
    NET/IB + both rails  -> good
    NET/Socket           -> STOP

PROGRESS:
  ssh cluster@10.10.10.1 'docker logs -f metrale-glm53-r0'

...
- Prefill flags: `METRALE_FLASHINFER_PREFILL METRALE_GDN_FLASHINFER METRALE_CUBLAS_GEMM METRALE_CUTLASS_WORKSPACE_MB=512 METRALE_PREFILL_VARLEN METRALE_PREFILL_CODISPATCH(+WINDOW_MS=100) METRALE_MOE_PREFILL_EXACT_TILES METRALE_SSM_BATCHED_RECURRENT METRALE_HOLO_MOE_GROUPED_CUTLASS METRALE_HOLO_MOE_GROUPED_DOWN METRALE_HOLO_FAST_MOE_MODE=full METRALE_HOLO_FAST_MOE_LAYERS=0-39 METRALE_HOLO_NATIVE_FP8_ATTN METRALE_HOLO_NATIVE_FP8_SSM METRALE_HOLO_LOW_MEMORY_MOE METRALE_Q12_BATCHED(+_FIRST_CHUNK) METRALE_GDN_TC_VBLOCK=0 METRALE_FP8_SINGLE_SCALE` (`METRALE_KV_OVERCOMMIT` dropped — see below)
...
★ **These are bare names, but the read sites require the literal string `1`.** Verified
  strict-`"1"` reads: `METRALE_FLASHINFER_PREFILL`, `METRALE_GDN_FLASHINFER`,

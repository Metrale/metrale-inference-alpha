...
// 2026-09-25: Kernels `attn_prefill_paged_turbo4` and `_64`: paged prefill flash attention over a turbo4 KV cache.
//
// Each cache block (stride `tq4_bsb` bytes) holds 4-bit codebook indices, two per byte low nibble first, then from byte
// `tq4_dsb` one FP8 E4M3 scale per 16 values, as reshape_and_cache_flash_turbo4 writes them. That is the NVFP4 byte
// layout; the loader below is attn_prefill_paged_nvfp4.cu's, with TURBO4_CODEBOOK (reshape_and_cache_turbo.cu) in the
// LUT in place of the E2M1 values. It dequantizes to BF16 while it fills shared memory, and the body is
...
#define KERNEL_NAME attn_prefill_paged_turbo4

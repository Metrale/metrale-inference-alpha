...
//! Cosine is the robust metric (~0.9997 at that error). `METRALE_W8A8_REL_RMS_GATE`
//! overrides the bound for a measurement run; the value used is always printed.
//!
//! SCALE LAYOUT — `METRALE_CUBLAS_SCALE_LAYOUT=kmajor|rowmajor` (default
...
//!   METRALE_CUBLAS_GEMM=1 cargo run --release -p spark-model \

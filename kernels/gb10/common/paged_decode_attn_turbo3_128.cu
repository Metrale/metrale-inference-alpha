...
// 2026-09-25: Paged decode attention over a Turbo3 KV cache: 3-bit indices into an 8-entry
// codebook and one FP8 E4M3 scale per 16 elements, 3.5 bits per element.
//
// Owner: gb10 kernels.
// Invariants:
...
//
// A cache block holds block_size * num_kv_heads token-major head rows of head_dim * 3 / 8 bytes,
// then from data_section_bytes one scale byte per 16 elements. The kernel applies no WHT: Q must
// be in the basis K and V were written in, which the caller arranges (write_kv_cache.rs,
// attention_forward.rs). There is no sliding window.
//
...
// 2026-09-25: Decodes one lane's VEC_BF16 elements. Eight 3-bit indices fill 3 bytes:
// b0 = v0 | v1 << 3 | v2 << 6, b1 = v2 >> 2 | v3 << 1 | v4 << 4 | v5 << 7,
// b2 = v5 >> 1 | v6 << 2 | v7 << 5. At HDIM=256 a lane decodes its own 3 bytes; at
// HDIM=128 two lanes read the same 3 bytes, the even lane taking v0-v3 and the odd v4-v7.
// Each value is codebook[index] times the group's FP8 scale.
...
extern "C" __global__ void paged_decode_attn_turbo3(
    ...
    const unsigned int head_data_bytes = head_dim * 3 / 8;
    ...
    const unsigned int kv_data_offset = kv_head * head_data_bytes + ((lane_id / 2) * 3)  /* 2026-09-25: a lane pair shares the 3 bytes of 8 values */;
    const unsigned int kv_scale_offset = kv_head * head_scale_bytes + (lane_id / 4);  // 2026-09-25: four lanes (16 values) share one scale byte

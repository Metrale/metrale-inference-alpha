...
/// the decode-only FP8 overlay on the BF16 dense build (`METRALE_HOLO_FP8_SSM_DECODE`).
...
pub(crate) fn build_linear_attention_dense_bf16(
    ...
    // Decode-only FP8 SSM overlay (METRALE_HOLO_FP8_SSM_DECODE=1): install the
...
pub(crate) fn build_linear_attention_nvfp4(
    ...
    // Native-HIP (metrale_hip) lacks the FP8 *prefill* GEMM kernels
    ...
    // METRALE_GDN_BF16_WEIGHTS=1 extension: also install BF16 out_proj so

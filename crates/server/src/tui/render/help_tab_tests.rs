...
//! 2026-09-26: Help section render tests: every `ReportPhase` draws at
//! 160×48, 80×24 and 40×12, and the screens show the public destination, the
//! preview counts, the user code and the redaction caveat.
//!
//! Owner: server tui.
...
fn the_guide_names_the_version_the_exits_and_the_memory_only_promise() {
    ...
    assert!(has(&rows, crate::cli::METRALE_VERSION), "{rows:#?}");
...
fn done_and_failed_carry_their_glyph_twins() {
    // 2026-09-26: Under `NO_COLOR` the accents are `Color::Reset`, so the
    // ✓/✗ glyphs are what separate the outcomes.
    let mut a = help(HelpSub::Report);

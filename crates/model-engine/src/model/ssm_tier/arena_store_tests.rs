...
//! 2026-09-25: Tests for `RdmaSnapshotStore` over `MockSnapshotTransport`, and for
//! `FileSnapshotArena`.
//!
...
fn rdma_overwrite_in_place_no_slot_leak() {
    // 2026-09-25: One slot, so an overwrite that took a new slot would fail.
    let s = rdma_store(1);
    assert!(s.put(1, &[1; BLOB]).unwrap());
    assert!(
...
fn file_arena_round_trip_bit_identical() {
    let dir = std::env::temp_dir().join(format!("metrale-decode-test-{}", std::process::id()));
...
fn file_arena_write_past_capacity_errs_not_corrupts() {
    let dir = std::env::temp_dir().join(format!("metrale-decode-cap-{}", std::process::id()));
    let dir = dir.to_str().unwrap();

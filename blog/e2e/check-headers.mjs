#!/usr/bin/env bun
/**
 * Post-deploy check for blog.dev.metrale.ai.
 *
 ...
 *
 *   bun blog/e2e/check-headers.mjs https://05ca5569.metrale-blog-3ja.pages.dev
 *
 * (The earlier control, book.dev.metrale.ai with its location-level
 * add_header, is a 301 to the new domain now and no longer serves anything.)
 */

const base = (process.argv[2] ?? 'https://blog.dev.metrale.ai').replace(/\/$/, '');

...
//!
//! Initializes `MetraleRegistry`, allocates device buffers, and provides
...
use metrale_core::registry::{MetraleRegistry, RawCudaFunc};
...
/// registry per model and drops it; see `metrale_core::registry::release`.
static INIT: OnceLock<&'static MetraleRegistry> = OnceLock::new();

/// Ensure the registry is loaded (idempotent).
pub fn ensure_registry() -> &'static MetraleRegistry {
    INIT.get_or_init(|| {
        let ptx = metrale_kernels::ptx_modules();
        let registry = MetraleRegistry::load(0, &ptx)
            .expect("MetraleRegistry load failed — is GPU available?");
        Box::leak(Box::new(registry)) as &'static MetraleRegistry
    })
}
...
pub fn get_kernel(registry: &'static MetraleRegistry, module: &str, func: &str) -> RawCudaFunc {

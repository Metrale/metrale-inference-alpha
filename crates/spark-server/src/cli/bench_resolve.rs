...
///
/// * [`Self::Unknown`] — the id names no box class Metrale Engine recognises. No run
///   will ever fix it; the spelling is wrong (or the class needs registering
///   in `metrale_plugin::hardware::ids::KNOWN_HARDWARE_IDS` first).
...
impl HardwareRefusal {
    ...
    fn of(benchmark_id: &str, hardware: &str, baseline: &gate::GateBaseline) -> Self {
        let measured = baseline
            ...
            .collect::<Vec<_>>()
            .join(", ");
        if metrale_plugin::hardware::ids::is_known_hardware_id(hardware) {
            Self::NoRecordYet {
            ...
            Self::Unknown {
                benchmark_id: benchmark_id.to_string(),
                hardware: hardware.to_string(),
                registered: metrale_plugin::hardware::ids::KNOWN_HARDWARE_IDS.join(", "),
                measured,
            }
        }
    }
...
/// id Metrale Engine does not know ("fix the spelling").
///
...
pub(super) fn apply_threshold_params(
    descriptor: &metrale_plugin::BenchmarkDescriptor,
    specs: &[metrale_plugin::ParamSpec],
    values: &mut metrale_plugin::ParamValues,

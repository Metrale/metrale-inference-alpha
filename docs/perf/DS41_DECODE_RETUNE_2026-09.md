...
### S1, the misses (measured 09-19, `METRALE_DS41_ROUTE_TRACE` on the standard)
...
(`METRALE_DS41_READER_POOL=1 METRALE_DS41_PREFETCH_K=4` to enable); worth revisiting once the
...
`~/code/metrale-notes/bin/ds41_lookup_sim.py` replays the oracle texts through an n-gram
...
* **Policy** (simulated on the 100 GiB trace, then built): a random victim among the oldest 5%
  of the LRU list (`METRALE_DS41_EVICT_RANDOM_PCT`, default 5; 0 = strict LRU) breaks the
  ...
  6/6 byte-identical. Kept (`METRALE_DS41_EVICT_RANDOM_PCT`, default 5).
...
misses a step per 300-token window 17.6 (cold) / 5.4 / 4.9 / 8.7 (last 92). One DGX Spark keeps
missing on long outputs: the all-resident case is two DGX Sparks with 187.7 GB of experts split
...
* **Two GB10s, 2-way expert parallel** (each DGX Spark holds half the routed experts, the dense
  ...
  keeps one message per layer per direction. And with 187.7 GB of experts split 94 GB a DGX Spark
...
* **S2.** The per-layer CUDA graph path (`METRALE_DS41_GRAPH=1`, phase 1's segments A / host span /
  ...
  read-back a step does not pay on one DGX Spark either: 65% of decode steps miss, and a miss at
...
the oracle (6/6 on every row). Recipe from S1 on: `METRALE_DS41_EXPERT_CACHE_GIB=100
METRALE_DS41_READER_THREADS=16` on a DGX Spark running nothing else.
...
`METRALE_DS41_SHARED_SIDE=0` restores the serial order.
...
`attn_v41_sparse_attn` ran 64 blocks on 48 SMs; `gridDim.z` blocks per (token, head) each recompute the scores and softmax identically and write their own slice of `hd` by the same j-ordered sum per element (same bytes for any split). `METRALE_DS41_SPARSE_SPLIT` default 2: MinHeap 21.59 (13.73 / 21.59 / 22.26), Volvo 24.07 (15.44 / 24.07 / 24.10), 6/6 (+5.4% / +5.0%); split 4 21.31 / 23.95.
...
ae9a1dea7 -> df5ffcb4f: MinHeap 20.49 -> 21.54 (best 22.02; 22.26 in the split-2 run), Volvo 22.93 -> 24.15 (best 24.17), every text byte-identical (fourteen standards tonight, 6/6 each once M1 was right); nsys of a warm 60-token token (`ds41_nsys_decode_M4b`): GPU kernel time 42.3 -> 41.9 ms, launches 1,905 -> 1,825, syncs 186, D2H 49; sparse attention 2.10 -> 1.50 ms, the pair kernel 0.81 ms for 40 launches where `q2_k_w` lost 80 (276 -> 196). Against the 09-17 standard of 10.6 / 10.9: +103% / +122%. The segment graphs are correct and off by default; the remaining one-DGX-Spark step is the bytes the kernels read (experts 15.0 ms at 205 GB/s, attention projections 12.1, head 2.8) and the B200 carries the same kernels with 8 TB/s under them.

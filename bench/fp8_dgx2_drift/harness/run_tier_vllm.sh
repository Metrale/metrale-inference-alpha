...
# (no local docker container). Used to A/B vLLM-FP8 on dgx2 vs Metrale Engine
...
#   - per-run docker logs window (no metrale-side metrics to gather)
#
# Reads:
...
for i in $(seq 1 "${N}"); do
  ...
  : > "${EMPTY_LOG}"          # empty metrale log window — score_run.py handles it

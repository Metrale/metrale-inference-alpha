...
This crate is the only `bin` in the workspace — building `spark-server` produces the `met` executable that the Docker image ships. Everything above is `lib` — `spark-server` ties it all together.

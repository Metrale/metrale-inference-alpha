...
# 2026-09-26: Scores one agent run into one JSON record.
...
Extracts structured drift metrics from:
  ...
  - The Metrale Engine server's stderr/stdout (logged via docker logs)
...
Usage:
    python3 score_run.py --tier TIER --run N --target TARGET_DIR \
        ...
        --metrale-log-window /tmp/metrale-log-<tier>-r<N>.txt \
...
# 2026-09-26: Optional: when followed_directions cannot be imported, scoring still
# runs and records it as module-unavailable (see main).
try:
...
def count_drift_events(events: list[dict[str, Any]], target: pathlib.Path) -> dict[str, int]:
    ...
    for e in events:
        ...
        if not path:
            ...
            # 2026-09-26: A relative path is resolved against the target dir
            # (opencode's --dir), not this script's cwd.
            try:
        ...
        # 2026-09-26: In a Cargo.toml write, a section header followed by `key =`
        # on the same line.
...
def _warm_target_dir() -> str:
    ...
    Mirrors warm_cargo_cache.sh's METRALE_WARM_TARGET_DIR (same env var, same
    ...
    return os.environ.get(
        "METRALE_WARM_TARGET_DIR",
        os.path.join(os.path.expanduser("~"), ".cargo", "metrale-warm-target"),
    )
...
def _build_timeout_s() -> int:
    ...
    METRALE_WS_BUILD_TIMEOUT for de-confounding experiments.
    """
    try:
        return int(os.environ.get("METRALE_WS_BUILD_TIMEOUT", "600"))
    except ValueError:
        return 600
...
def webserver_test(target: pathlib.Path, port: int, timeout_s: int = 15) -> dict[str, Any]:
    ...
    Steps:
      ...
      3. Spawn `cargo run --release` as background process with
         METRALE_HARNESS_PORT={port} env. The prompt instructed the model
    ...
    # 2026-09-26: A fresh ephemeral port; the `port` argument is not used.
    port = _free_port()
    out["port_used"] = port

    # 2026-09-26: Build and run both use the shared warm target dir (CARGO_TARGET_DIR),
    # so `cargo run --release` reuses what the build step compiled.
    ...
    # 2026-09-26: Run the server in the background with stderr in a temp file, so a
    # bind failure ("Address already in use" / EADDRINUSE) is reported as such (see
    # the teardown below), not as a /ping timeout.
    ...
    server_err = tempfile.NamedTemporaryFile(
        mode="w+", prefix="metrale-ws-stderr-", suffix=".log", delete=False
    )
    try:
        server = subprocess.Popen(
            ...
            stderr=server_err,
            env=env,
            preexec_fn=os.setsid,  # 2026-09-26: own process group, for os.killpg below
        )
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            pass
        # 2026-09-26: Keep the stderr tail (800 chars); flag a port-in-use bind failure.
        try:
            server_err.flush()
            server_err.seek(0)
...
def metrale_log_metrics(log_text: str) -> dict[str, Any]:
    """Extract metrale-side counters from the captured docker log window.
    ...
    """
    # 2026-09-26: Counts "(length)" anywhere in the window. The scheduler logs each
    # finished sequence as `Done: <N> tokens (<finish reason>)`, and the reason is
    # length for a max_tokens or context-ceiling hit and for any guard cut except the
    # request timeout.
...
def main() -> int:
    ...
    ap.add_argument("--metrale-log-window", required=True, type=pathlib.Path)
    ...
    # 2026-09-26: Whether the agent itself ran the prompt's steps; a failure to
    # compute it is written into the record, never raised.
    try:
        followed = (
    ...
    metrale_log_text = (
        args.metrale_log_window.read_text(errors="replace") if args.metrale_log_window.exists() else ""
    )
    metrale = metrale_log_metrics(metrale_log_text)

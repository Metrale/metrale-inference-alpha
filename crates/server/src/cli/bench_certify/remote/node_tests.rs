...
//! 2026-09-26: Tests for node admission, the report fingerprint and the local node.
//!
//! Owner: server CLI (`met benchmark certify`).
//! Invariants: none beyond the types.

use super::super::metralectl::NodeRow;
use super::super::metralectl::metralectl_tests::NODE_ROW;
use super::*;
...
fn a_clean_node_is_admitted_with_its_fingerprint_and_build_state() {
    ...
    // 2026-09-26: Another anchor: not built there.
    let mut w = wanted(&signers);
    w.anchor = "0000000000000000000000000000000000000000";
    assert!(!admit(&row(), &w).unwrap().built);
}

/// 2026-09-26: Every rule refuses on its own, names its reason, and the reasons add up.
#[test]
fn each_admission_rule_refuses_with_its_reason() {
    ...
    let cases: Vec<(&str, Break)> = vec![
        ...
        (
            ...
            Box::new(|r| {
                let i = r.info.as_mut().unwrap();
                i.busy = true;
                i.busy_reason = Some("met pid 1".into());
            }),
        ),
        (
            "job(s) queued",
    ...
    // 2026-09-26: Two broken facts: both named.
    let mut r = row();
    let i = r.info.as_mut().unwrap();
    i.busy = true;
    ...
    // 2026-09-26: No info at all: the error metralectl gave.
    let e = admit(
        &NodeRow {
            node: "10.10.10.9".into(),
            ok: false,
            info: None,
            error: Some(super::super::metralectl::ErrorObj {
...
fn a_node_without_thermal_facts_has_an_undecidable_fingerprint() {
    ...
    // 2026-09-26: Such a node is still admitted.
    let signers = vec!["a27dbc8ed2fc2a31".to_string()];

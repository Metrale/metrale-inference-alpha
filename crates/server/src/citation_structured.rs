...
//! 2026-09-26: The parsers behind [`crate::citation::extract`]: markdown
//! footnotes, numeric bracket refs, and `Sources:` / `References:` /
//! `Citations:` sections. The [`crate::citation`] module docs show the
//! shapes.
//!
//! Owner: server API.
//! Invariants: every citation's URL starts with `http://` or `https://`.

use crate::citation::Citation;

/// 2026-09-26: One citation per `[^label]` reference that has a
/// `[^label]: url title` definition line; the title is optional and defaults
/// to the label.
    ...
    while i < bytes.len() {
        if let Some(rel) = content[i..].find("[^") {
            ...
            if let Some(close_rel) = after_br.find(']') {
                ...
                // 2026-09-26: A `[^x]` followed by `:` is a definition, not a
...
/// 2026-09-26: `{label: (url, optional title)}` for every line that, after
/// leading whitespace, reads `[^label]: url title`.
...
/// 2026-09-26: One citation per `[N]` reference that has a `[N] url` or
/// `[N]: url` definition line.
pub(crate) fn numeric_ref_citations(content: &str) -> Vec<Citation> {
    ...
    while i < content.len() {
        ...
        if label.bytes().all(|b| b.is_ascii_digit()) && !label.is_empty() {
            ...
            // 2026-09-26: A `[N]` at the very start of a line and followed by
            // a URL is a definition, not a reference.
...
/// 2026-09-26: For each `Sources:`, `References:` or `Citations:` heading (any
/// case) at the start of a line, one citation per following line that starts
/// with a URL, after an optional `- `, `* ` or `• `. The title is the rest of
/// the line, else the URL.
pub(crate) fn sources_block_citations(content: &str) -> Vec<Citation> {
    ...
    for marker in &markers {
        ...
        while let Some(rel) = lower[search_from..].find(marker) {
            ...
            // 2026-09-26: The first line walked is the rest of the heading
            // line. Empty lines are skipped until one has content; the
            // section then ends at the next empty line or at the first line
            // that does not start with a URL.
...
/// 2026-09-26: Split `s` into `(url, rest)` when it starts with `http://` or
/// `https://`. The URL runs to the first whitespace, `)`, `]`, `>` or
/// backtick, less trailing `.,;:!?`; `rest` starts at that stop character.
/// `None` when `s` does not start with a scheme or the URL is 8 bytes or
/// shorter.

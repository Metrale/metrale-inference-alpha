...
Metrale Engine's paged KV cache reserves a fixed fraction of GPU memory at startup
...
Metrale Engine implements **High-Speed Swap (HSS)** in `crates/spark-storage/`
...
- A single-machine `/mnt/fast-nvme/metrale-kv` directory is the canonical

# 2026-09-26: Full LongCat forward check: engine input_norm_in dumps against the golden per-layer outputs.
#
...
is the input of engine sublayer 2K+2:
  ...
  gold['input']      <-> metrale_op_L0_input_norm_in
  gold['layer0_out'] <-> metrale_op_L2_input_norm_in
  gold['layerK_out'] <-> metrale_op_L{2K+2}_input_norm_in
...
print(f'\n{"stage":16s} {"sublayer":>9s} {"|ref|":>9s} {"|metrale|":>9s} {"cos":>8s} {"relerr":>8s}')
worst = (1.0, '')
for key, sub in pairs:
    path = f'{D}/metrale_op_L{sub}_input_norm_in.bin'
    if not os.path.exists(path):
        print(f'{key:16s} {sub:9d}   MISSING  (dump layer {sub} not enabled?)')
        continue
    a = np.fromfile(path, dtype=np.float32)
    ref = gold[key][-1].astype(np.float32)     # 2026-09-26: the dumps hold the last token
    n = min(len(a), len(ref))
    a, ref = a[:n], ref[:n]

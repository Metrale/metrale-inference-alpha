...
//! History: past runs, read back from `~/.metrale/runs`.
//!
...
fn draw_list(f: &mut Frame, app: &App, area: Rect) {
        ...
        .take(visible)
    {
        ...
        // when a run was served with no overrides and when Metrale Engine did not
        ...
        let mark = match entry.frame.verdict.as_ref().map(|v| v.kind) {
            Some(metrale_plugin::VerdictKind::Pass) => Span::styled("✓", theme::brand_green()),
            Some(metrale_plugin::VerdictKind::Fail) => Span::styled("✗", theme::error()),

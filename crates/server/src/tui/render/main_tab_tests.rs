...
//! 2026-09-26: Tests for the Main section: the startup checklist, weight-load
//! panel, READY strip, chips, the log pane's line wrapping, and the Kernels
//! table.
//!
//! Owner: server tui.
...
fn logline(msg: &str) -> Line<'static> {
    Line::from(vec![
        ...
        Span::raw("metrale_core   ".to_string()),
...
fn a_long_line_wraps_instead_of_losing_its_tail() {
    ...
    // 2026-09-26: Compare words, not joined rows: the continuation indent can
    // fall between "48" and "layers".
...
fn continuations_are_indented_under_the_message() {
    ...
    assert!(rows.len() > 1);
    // 2026-09-26: `logline`'s three prefix spans are 9 + 6 + 15 = 30 chars.
    assert!(
...
fn a_server_with_no_model_refuses_to_report_a_load_that_is_not_happening() {
    // 2026-09-26: `progress.ready` with `awaiting_model` still set.
    let mut a = main_app();
...
fn the_log_filter_is_visible_while_it_is_being_typed_and_after() {
    ...
    // 2026-09-26: Committed, the cursor goes but the filter stays on the
    // title.
...
/// 2026-09-26: Main ▸ Kernels, the kernel audit table.
mod kernels {
    use super::*;
    ...
    fn a_module_reports_whether_it_was_used_never_looked_up_or_failed() {
        let a = with_kernels(KernelTableModel {
            rows: vec![
                row("metrale_gdn_decode", Some(true)),
                row("metrale_ssm_tail", Some(false)),
                row("metrale_moe_bf16", None),
            ],
            ..Default::default()
        });
        let rows = screen(&a, 160, 48);
        assert!(has(&rows, "KERNELS ─ 3 modules"), "{rows:#?}");
        assert!(has(&rows, "metrale_gdn_decode"));
    ...
    fn only_the_actionable_failures_get_a_banner() {
        // 2026-09-26: Only `missing_required` is listed in the banner;
        // `missing_expected` is only counted.
        let a = with_kernels(KernelTableModel {
            rows: vec![row("metrale_gdn_decode", Some(true))],
            missing_required: vec![MissingKernel {
                module: "metrale_ssm_tail".into(),
                func: "tail_midchunk".into(),
                site: "ops/ssm.rs:214".into(),
            }],
            missing_expected: vec![MissingKernel {
                module: "metrale_fp4_mma".into(),
                func: "mma_e2m1".into(),
        ...
        assert!(
            has(&rows, "metrale_ssm_tail::tail_midchunk  at ops/ssm.rs:214"),
            "the banner points at the dispatch site:\n{rows:#?}"
        );
        assert!(
            !has(&rows, "metrale_fp4_mma"),
    ...
    fn a_filter_narrows_the_table_and_its_own_count() {
        let mut a = with_kernels(KernelTableModel {
            rows: vec![
                row("metrale_gdn_decode", Some(true)),
                row("metrale_moe_bf16", Some(true)),
            ],
            ..Default::default()
        });
        ...
        assert!(!has(&rows, "metrale_moe_bf16"));
    }

    #[test]
    fn the_scroll_ceiling_is_published_from_what_was_actually_drawn() {
        // 2026-09-26: The ceiling counts the rows that pass the filter.
        let a = with_kernels(KernelTableModel {

...
//! §4 unification (`METRALE_SSM_TIER_UNIFIED`) store contract tests.
...
fn unified_store(slots: usize) -> UnifiedSnapshotStore {
    UnifiedSnapshotStore::new(
        Box::new(metrale_tier::VecSlotArena::new(BLOB, slots)),
        Box::new(metrale_tier::MemSwapStore::new(BLOB)),
        BLOB,
    )
    .unwrap()
}
...
// ─────────────── METRALE_SSM_TIER_DISK_GB: the bounded Marconi tier ───────────
...
fn unified_over_transport_never_drops_where_bounded_store_did() {
    ...
    let s = UnifiedSnapshotStore::new(hot, Box::new(metrale_tier::MemSwapStore::new(BLOB)), BLOB)
        .unwrap();
    let mut o = [0u8; BLOB];
    for k in 0..16u64 {
        assert!(

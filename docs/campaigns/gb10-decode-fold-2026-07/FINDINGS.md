...
**On the real gate model (centml/Qwen3.6-27B-NVFP4-W4A4-mlpinf, which is already all-NVFP4), Metrale's
...
vLLM, **Metrale already wins every reported metric.**

## Competitive picture (confirmed vLLM, same ~78 tok/turn, apples-to-apples)
| metric | confirmed vLLM | Metrale best (dpcarry) | winner |
|---|---|---|---|
| perf wall (1007) | 5361 s | 4984 s | **Metrale −7%** |
| tps | 14.6 | 15.91 | **Metrale +9%** |
| qps | 0.188 | 0.20 | **Metrale +6%** |
| 1/tps (agg ms/tok) | 68.5 | 62.9 | **Metrale −8%** |
| IoU | 0.6269 | 0.6285 | Metrale (tie+) |
| BFCL | 86.43 | 87.04 | **Metrale +0.6** |
| steady-state TPOT | ~31 (WEAK run) / unknown (confirmed) | ~40 | vLLM (weak ref only) |
Metrale wins because TTFT is ~2× better (1557 vs 2985ms), and the agentic turns are prefill-heavy

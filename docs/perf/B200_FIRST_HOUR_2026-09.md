...
each is labelled: measured on the DGX Spark (`docs/perf/DS41_DECODE_RETUNE_2026-09.md`, PR #1156
...
1. Code: the head of PR #1156 (branch `ds41-b200-prep`, 6e0f1ca04 on 2026-09-19), which contains
   ...
   `METRALE_TARGET_HW=b200 METRALE_TARGET_MODEL=deepseek-v4.1-flash` build of today's `main` panics
   in `build.rs` (`unknown_model`). Build from the #1156 head, or from `main` once the chain lands.
2. The S3 whole-step graph (`METRALE_DS41_STEP_GRAPH=1`) is **not landed**. Its estimate in section
...
3. The checkpoint: `vcruz305/DeepSeek-V4.1-Flash-GGUF`, revision
   ...
   the sha256 of each, from the DGX Spark's Hugging Face download metadata).
...
| Metrale Engine, this engine, S2 | 1x DGX Spark | **19.19 / 21.87** (MinHeap / Volvo medians; hot probe 23.91) | engram on, 100 GiB device arena, 6/6 texts byte-identical to the phase 1 oracle | #1156 S2 comment, 2026-09-19 |
| Metrale Engine, #1155 head | 1x DGX Spark | 18.57 / 21.14 | same standard, host routing | `DS41_DECODE_RETUNE_2026-09.md` phase 4 |
| llama.cpp (vcruz305 recipe) | 1x GB10, CPU backend | 2.3-2.5 | Q2_K, the recipe Turney pointed at on 09-12 | memory note queueing V4.1 Flash on a single DGX Spark |
| Metrale Engine, V4 0731 (the team) | 2x GB10 over RoCE | 15.5 | a different model, CUDA graphs off | #1141 body |
...
The standard behind the Metrale Engine rows (`docs/perf/PERFORMANCE_NUMBERS_STANDARD.md` and the retune
...
|---|---|---|---|
| GB10 (DGX Spark) | 249 GB/s device, 223 GB/s pinned (measured) | 24.5 ms; measured step 42.4 ms wall after #1155 | retune doc S0, phase 4 |
...
So the bytes stop being the step. What is left is the work the DGX Spark hides under its 24.5 ms of
reading.

### 2.2 Without the graph: the step is launch-bound

Per token after S2, measured with nsys on the DGX Spark (#1156 S2 comment): **1,905 kernel launches**
...
12.22 MiB = 196 MiB). A 150 GiB arena (`METRALE_DS41_EXPERT_CACHE_GIB=150`, section 4 step 9) is
...
misses only on r1 of each suite (the cold cache), zero on r2/r3.** That is arithmetic on DGX Spark
...
2. NVIDIA driver: the repo pins **no driver version**; it pins the toolkit at CUDA 13.0
   ...
   newer (NVIDIA's CUDA 13.0 release notes, the toolkit-to-driver table). The DGX Spark that produced
...
4. Nsight Systems for section 5 (the DGX Spark has 2025.3.2 under `/usr/local/cuda-13.0/bin/nsys`).
...
`~/models/dsv41-q2k` on the DGX Spark: 7 shards, **264,521,661,674 bytes = 264.5 GB = 246.4 GiB**
...
| sustained rate | time |
|---|---|
| 25 MB/s (a residential uplink; the DGX Spark is not the source) | 2 h 56 min |
| 100 MB/s | 44.1 min |
| 500 MB/s | 8.8 min |
| 1 GB/s | 4.4 min |
| 2 GB/s | 2.2 min |
...
3. **0:04 The oracle files and the scripts.** Copy from the DGX Spark ahead of time (they are text):
   `~/dflash-logs/ds41_retune_oracle_{minheap,volvo}_r{1,2,3}.txt` (the phase 1 oracle every
   standard since #1147 has been held to) and `~/code/metrale-notes/bin/ds41_suite.sh`,
   `ds41_smoke.sh`, `ds41_nsys_serve.sh`, `ds41_nsys_collect.sh`, `ds41_nsys_stats.sh`,
   `ds41_retune4_run.sh` (these live in metrale-notes, not in the repo). Put them in `~/dflash-logs`
...
5. **0:05 The binary, option B: build on the box** (only with the CUDA 13.0 toolkit present; the
   DGX Spark's release build of this crate set takes tens of minutes, so start it in the background
   and proceed with step 6 while it runs). Exact names from `crates/kernels/build.rs`,
   ...
   METRALE_TARGET_HW=b200 METRALE_TARGET_MODEL=deepseek-v4.1-flash METRALE_TARGET_QUANT=nvfp4 \
     cargo build --release -p metrale-server --no-default-features --features cuda,nccl 2>&1 | tail -20
   ls -l target/release/met && sha256sum target/release/met
   ```
   `deepseek-v4.1-flash`'s `MODEL.toml` declares `kernel_source = "deepseek-v4-flash"`, so the
   `.cu` set compiled is `kernels/gb10/deepseek-v4-flash/nvfp4/` plus `kernels/gb10/common/`
   (inherited through `kernels/b200/HARDWARE.toml`), exactly the 199 sources of the S1 gate. `nvfp4` is the only quant
...
6. **0:07 The ptx gate on the box** (toolkit required; skip if option A and no nvcc, the CI compile
   ...
   fails), then **199/199**, 0 rejected entry functions, as S1 measured on the DGX Spark with 13.0.88.
   ```bash
   cd ~/metrale && scripts/hopper_ptx_gate.sh --hw b200 --model deepseek-v4-flash --strict --jobs 16 \
...
7. **0:09 Check-kernels** (the Dockerfile's "FIRST BOOT SHOULD BE `--check-kernels`"): resolves
   ...
   ```bash
   export METRALE_DS41_MAX_SEQ=4096 METRALE_DS41_MAX_TOKENS=512 RUST_LOG=info
   ~/ds41_bin/met serve --model-from-path /data/dsv41-q2k --kernel-target deepseek-v4.1-flash \
...
8. **0:12 The serve.** The DGX Spark recipe is `METRALE_DS41_EXPERT_CACHE_GIB=100
   METRALE_DS41_READER_THREADS=16` (#1148) with the device arena on by default
   (`METRALE_DS41_ARENA_DEVICE`, #1155) and device routing opt-in (`METRALE_DS41_DEVICE_ROUTE=1`, S2).
   On the box the arena is device memory sized by section 2.4, `--gpu-memory-utilization` is the
   recipe's 0.90 (the DGX Spark's 0.25 is a unified-memory setting), and the readers face a datacenter
   NVMe (start at 16, it only matters on r1).
   ```bash
   env METRALE_DS41_MAX_SEQ=4096 METRALE_DS41_MAX_TOKENS=512 METRALE_DS41_EXPERT_CACHE_GIB=150 \
       METRALE_DS41_READER_THREADS=16 METRALE_DS41_DEVICE_ROUTE=1 RUST_LOG=info \
     setsid nohup ~/ds41_bin/met serve --model-from-path /data/dsv41-q2k --kernel-target deepseek-v4.1-flash \
   ...
   cold one. If the arena allocation fails, halve `METRALE_DS41_EXPERT_CACHE_GIB` and note the free
...
10. **0:18 The standard, warm pass.** Same six prompts, same settings, one serve
    ...
    print(f'oracle: {same}/6 byte-identical to the DGX Spark', diff)
    PY
    grep -c -i "miss" ~/dflash-logs/b200_serve_r0.log   # the miss counter against section 2.4
    ```
    **The oracle clause.** The six DGX Spark texts are the byte-identity reference **only if the B200
    ...
    against the DGX Spark for each of the six, and every later B200 number is held to that oracle and
...
11. **0:30 The standard, cold pass** (#1140's order: warm, then cold after a cache drop).
    ```bash
    pkill -f "ds41_bin/met serve"; sleep 5
    ...
    ```
    The cold pass measures the NVMe's expert reads (12.22 MiB each; 1.8-2.0 ms on the DGX Spark's
...
13. **0:50 Receipts off the box** (the performance-numbers standard: launch line, commit sha,
    ...
    # scp the tarball to the DGX Spark (or the .38 vault) BEFORE releasing the node; then:
    nvidia-smi --query-gpu=memory.used --format=csv && pgrep -a met || echo "no met running"
    ```
...
1. kernel launches a token (DGX Spark: 1,905 with S2) and `cuStreamSynchronize` a token (DGX Spark: 186);
2. GPU kernel time a token (DGX Spark: 44.5 ms) against wall a token (from `usage.response_token/s`);
3. `cuStreamSynchronize` **blocked** time a token (DGX Spark: 25-28 ms, most of it the GPU finishing
   queued work; on the B200 the GPU is idle in a few ms, so blocked time here is round trips);
4. `cuMemcpyHtoDAsync` a token (DGX Spark: 142 with S2, 260 without);
5. the per-kernel duration distribution: how many kernels run under 5 us.

| what the profile shows | it means | the lever |
|---|---|---|
| wall a token >> kernel time a token; gaps between kernels of 3-10 us | launch-bound, as section 2.2 predicts | the whole-step graph (S3 on #1156); until it lands, `METRALE_DS41_DEVICE_ROUTE=1` is the most the eager path can do |
...
| `cuStreamSynchronize` blocked time is the step; 186 waits at 20+ us | wait-bound; the host round trip per layer costs more than the layer | confirm `METRALE_DS41_DEVICE_ROUTE=1` was on (H2D 142, not 260); the remaining wait per layer is the header read-back S3 removes |
| step-10 miss counter non-zero on r2/r3 | the arena is smaller than section 2.4 assumed | raise `METRALE_DS41_EXPERT_CACHE_GIB` toward what `nvidia-smi` left free; two GPUs (#1141) only if the working set will not fit |
| r1 much slower than r2/r3, r2 = r3 | the cold cache, as on the DGX Spark | the NVMe read rate is the number to file; the standard's medians absorb r1 by design |
...
2. "The kernels/b200/deepseek-v4-flash target ... passes the sm_100a PTX gate": now 199/199 under
   `--strict`, zero source changes, with the ptxas resource table (S1, 316b35da6).
...
4. "The same LRU that runs on the DGX Spark" and "`METRALE_DS41_EXPERT_CACHE_GIB` sized to the box":
   ...
   pre-#1148 values; the DGX Spark recipe is 100 / 16, the B200 value is section 2.4.
5. "The single-DGX-Spark native figure from #1099": 19.19 / 21.87 (S2) and 18.57 / 21.14 (#1155).
6. "How much of the DGX Spark's disk-bound ceiling the streaming still costs": the B200 step is
...
4. Layer 3 ("tensor memory and the two-CTA MMA path"): confirmed untouched; the gate needed
   `-DMETRALE_NO_WARP_BLOCKSCALE_MMA` and nothing else.
...
6. "The team's two-DGX-Spark run of V4 0731 at 15.5 tok/s" stands as the comparison it names.
...
2. "The two-DGX-Spark EXL3 DSpark run and its accept rate in the same table": 54% draft-token accept,
...
`kernels/b200/deepseek-v4-flash/{README.md,PTXAS_RESOURCES.md}` and
`docs/perf/b200_ptx_gate_deepseek-v4-flash_2026-09-19.md`; `kernels/b200/HARDWARE.toml`;
`crates/kernels/build.rs`; `crates/core/src/arch.rs`;
...
`docs/k3/B300-PLAN.md` on the contributor branch `wip/k3-b300-integration` (#1150);
`~/code/metrale-notes/bin/ds41_{suite,smoke,nsys_serve,nsys_collect,nsys_stats,retune4_run}.sh`;
...
the note queueing V4.1 Flash on a single DGX Spark; `nvidia-smi` and `nvcc --version` on Blackbird, 2026-09-19.

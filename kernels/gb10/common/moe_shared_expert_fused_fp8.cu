...
// 2026-09-25: Single-token FP8 MoE GEMVs with the shared expert as blockIdx.y == top_k:
// gate+up, then SiLU(gate) * up and down.
// Owner: gb10 kernels.
// Invariants:
...
// - K must be a multiple of 16 in gate+up and of 8 in down, and at most 1024 in down.
...
// 2026-09-25: The E4M3 value of every byte (bias 7, range -448 to 448); the NaN bytes
// 0x7F and 0xFF map to 0.
...
// adjacent outputs. Launch: grid (ceil(N / 8), top_k + 1, 2), block 128.
...
// computes two adjacent outputs. Launch: grid (ceil(N / 8), top_k + 1, 1), block 128.

<script>
  ...
  // concurrency rung, Metrale Engine and every comparable vLLM series on one axis.
  //
  ...
  const seriesList = $derived([
    ...(cost.metrale ? [{ ...cost.metrale, role: 'metrale' }] : []),
  ...
  const metraleMeasured = $derived(new Set(cost.metrale ? measured(cost.metrale).map((p) => p.c) : []));
  const metraleAbsent = $derived(
    cost.metrale ? cost.metrale.points.filter((p) => p.energy.state !== 'measured') : []
  );
...
<figure class="gate-panel cost-chart">
      ...
      <span class="gl-group">
        {#if cost.metrale}
          <span class="gate-legend-item">
            ...
            </svg>{cost.metrale.label} · {cost.metrale.source} · {cost.metrale.build}
          </span>
        {/if}
  ...
  <svg viewBox="0 0 {W} {H}" role="img"
    aria-label="Cost per million tokens against concurrency for {subject.label}, GPU rail only, {cost.baselines.length ? 'Metrale Engine against vLLM' : 'Metrale Engine alone — no comparable vLLM energy'}{pue.band ? `, shaded between PUE ${pue.low} and ${pue.high}` : pue.low === 1 ? '' : `, at PUE ${pue.low}`}">
    {#each yTicks as t}
    ...
    {#if cost.metrale && metraleMeasured.size}
      <path d={pathOf(cost.metrale)} fill="none" stroke={color} stroke-width="2" stroke-dasharray={dash}
        stroke-linejoin="round" stroke-linecap="round" />
      {#each measured(cost.metrale) as p}
        {@const py = y(usdOf(p.energy))}
        ...
        <g
          ...
          onclick={() => cost.metrale.record && onselect([cost.metrale.record])}
          onkeydown={(e) =>
            (e.key === 'Enter' || e.key === ' ') &&
            (e.preventDefault(), cost.metrale.record && onselect([cost.metrale.record]))}
        >
          <title>{tip(cost.metrale.label, p)}{v ? ` · ${v.metraleCheaper ? `Metrale Engine cheaper ×${(1 / v.ratio).toFixed(2)}` : v.loseLabel} vs ${v.rivalLabel}` : ''}</title>

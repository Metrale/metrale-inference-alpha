...
//! 2026-09-26: Upstream-parity tests for the inline-scorer port.
//!
...
//! list in `gen_parity_fixtures.py`, so these tests show agreement with the
//! upstream scorer on every pinned case. Both files are pinned by SHA-256.
//!
...
/// 2026-09-26: The fixtures must state which upstream commit produced them; a
/// regenerated fixture or generator file fails here until the pins are moved.
#[test]
...
/// 2026-09-26: One generated case per upstream alias entry: the whole table
/// is pinned.
#[test]
...
/// 2026-09-26: The port's table must hold exactly the upstream keys: the
/// per-entry cases above cannot see an entry this port added that upstream
/// does not have.
#[test]
...
/// 2026-09-26: Denominator semantics, not covered by a fixture: a ground truth
/// with no scorable content is excluded; an issued turn whose model output is
/// missing scores 0 and stays.
#[test]
fn unscorable_ground_truth_is_excluded_but_missing_output_scores_zero() {
    ...
    // 2026-09-26: The model turn `replay_one` records for a failed request: a
    // bare assistant role with no fields.

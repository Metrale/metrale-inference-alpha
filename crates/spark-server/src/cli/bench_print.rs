...
//! JSON goes to stdout. `met benchmark run … --format json > run.json` is
//! then a clean file rather than a log with JSON in it.

use anyhow::Result;
use metrale_plugin::headless::{RunReporter, RunRequest};
use metrale_plugin::{
...
pub fn print_suite(format: OutputFormat) -> Result<()> {
        ...
        let rows: Vec<_> = all
            ...
            .map(|d| {
                serde_json::json!({
                    ...
                    "group": metrale_plugin::gate::group::find(d.id).is_some(),
                })
            })
            .collect();
    ...
    for d in all {
        ...
        let group = if metrale_plugin::gate::group::find(d.id).is_some() {
...
fn print_variants(benchmark_id: &str) {
    ...
    let Ok(baseline) = metrale_plugin::gate::read_baseline(&root, benchmark_id) else {
        return;
    };
        ...
        match event {
            ...
            PluginEvent::Log(line)
                if !self.quiet
                    || matches!(
                        line.level,
                        metrale_plugin::LogLevel::Warn | metrale_plugin::LogLevel::Error
                    ) =>
            {

...
use metrale_core::config::{ModelConfig, PeftAdapterConfig};
...
pub(crate) fn audit_adapter(
    ...
    // expert adapter is a NAMED reject unless METRALE_LORA_EXPERTS=1) + shapes.
    ...
    for t in &peft.target_modules {
        ...
        if !matched {
            // Under METRALE_LORA_ALLOW_PARTIAL the user has already been warned,
            // by name, that these modules are skipped; `validate_peft_config`
            // is the gate that decides. Bailing again here would make the
            // opt-in unusable, since a module Metrale Engine cannot apply is by

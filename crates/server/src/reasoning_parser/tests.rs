...
//! 2026-09-26: Unit tests for the reasoning parsers.
//!
//! Owner: server.
...
fn minimax_strips_model_emitted_opening_tag() {
    // 2026-09-26: A leading `<think>` is consumed even though the prompt opened
    // the block.
...
fn thinking_on_tagless_output_is_reasoning_not_content() {
    // 2026-09-26: With thinking on, tagless output from a `<think>` format is
    // unfinished reasoning, and none of it reaches the content.
    for fmt in [
...
fn thinking_off_tagless_output_is_content() {
    // 2026-09-26: With thinking off, tagless output is the answer.
    let p = parser(ReasoningFormat::DeepSeekR1);
...
fn thinking_disabled_discards_reasoning_keeps_answer() {
    // 2026-09-26: With thinking off, a closed block is still split off and
    // only the answer is returned.
    let p = parser(ReasoningFormat::Qwen);
...
fn mistral_no_tags_is_all_content() {
    // 2026-09-26: Mistral output without `[THINK]` is all content, even with
    // thinking on.
    let p = parser(ReasoningFormat::Mistral);
...
fn first_end_tag_bounds_reasoning_leftover_blocks_stripped() {
    // 2026-09-26: Reasoning ends at the first `</think>`; a later balanced
    // pair is removed from the content.
    let p = parser(ReasoningFormat::Qwen);
    let (reasoning, content) =

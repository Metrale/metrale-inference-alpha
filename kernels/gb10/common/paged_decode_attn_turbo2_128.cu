...
// 2026-09-25: Paged decode attention over a Turbo2 KV cache: 2-bit indices into a 4-entry
// codebook and one FP8 E4M3 scale per 16 elements, 2.5 bits per element.
//
// Owner: gb10 kernels.
// Invariants:
...
//
// A cache block holds block_size * num_kv_heads token-major head rows of head_dim / 4 bytes,
// four indices per byte from the low bits up, then from data_section_bytes one scale byte per 16
...
// 2026-09-25: 2-bit indices, four per byte from the low bits up, each times the FP8 scale.
...
extern "C" __global__ void paged_decode_attn_turbo2(
    ...
    const unsigned int head_data_bytes = head_dim / 4;

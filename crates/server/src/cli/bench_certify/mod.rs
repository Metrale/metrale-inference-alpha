...
//! 2026-09-26: `met bench certify`: plan, preflight, run and judge a certification campaign.
//!
//! Owner: server CLI (`met benchmark certify`).
//! - The plan comes from `gate::check_gates`, as for the gate check.
//! - Without `--with-nodes` the campaign runs one unit at a time on this box
//!   (`local::drive_local`); with it, across the admitted nodes (`remote::drive`).
//! - Unless `--no-guard`, the drift guard runs on a `GUARD_EVERY` timer while
//!   units run, and the local driver also runs it before every unit.
//! - The last word is `report::evaluate`: the gate table plus the
//!   record-agreement rule.
//!
...
use metrale_bench::{ArtifactStore, gate, history};
...
/// 2026-09-26: How often the drift guard runs while a unit is in flight.
pub const GUARD_EVERY: Duration = Duration::from_secs(60);

/// 2026-09-26: Where the campaign's words go: human lines on stderr, or (under
...
pub async fn certify_cmd(args: CertifyArgs) -> Result<i32> {
    ...
    let hardware = match &args.hardware {
        Some(h) => h.clone(),
        None => {
            let k = metrale_bench::hardware::Hardware::probe().gate_key();
            if k == "unknown" {
    ...
    // 2026-09-26: The class's limits (`kernels/<hw>/HARDWARE.toml`
    // `[benchmarks.limits]`): the thermal envelope, the equivalence spreads, the
    // memory floor and the timing allowances. A class that declares none is not
    // campaigned. `--dangerous-ignore-thermals` waives only the thermal envelope.
    ...
    let needs: Vec<&'static str> = units
        ...
        .into_iter()
        .collect();
    // 2026-09-26: A server a dead campaign left leased on this box is stopped here;
    // it would otherwise count as another `met` process in the preflight.
    ...
    emit.event(
        "preflight",
        serde_json::json!({
            "signer": facts.signer, "metrale_home": facts.metrale_home,
    ...
    // 2026-09-26: Remote units are submitted with the full sha; `anchor` may be
    // abbreviated (`gate::git_sha` runs `rev-parse --short=10`).
    let anchor_full = gate::git_rev_parse(&root, &anchor)?;
    let fleet = if args.with_nodes.is_empty() {
        None
    } else {
        let metralectl =
            remote::metralectl::SubprocessMetralectl::locate(args.metralectl.as_deref())?;
    ...
    let mut lock = lockfile::LockGuard::claim(
        ...
        lockfile::Campaign {
            ...
            metrale_home: facts.metrale_home.clone(),
    ...
    let campaign = match &fleet {
        ...
        Some(f) => {
            let metralectl: Arc<dyn remote::metralectl::Metralectl> = Arc::new(
                remote::metralectl::SubprocessMetralectl::locate(args.metralectl.as_deref())?,
            );
    ...
    // 2026-09-26: Stop whatever server the last unit left leased.
...
/// 2026-09-26: The final gate table and agreement check, and the exit code.
...
/// history, in whole-draw seconds: a shard run's time is scaled up by its count
/// (`whole_draw_secs`), because `plan::shard_secs` divides by the count the
/// campaign wants.
fn measured_secs(store: &ArtifactStore, id: &str) -> Option<(u64, u64)> {
    history::load(store, id)
        .into_iter()
        .find(|r| r.frame.status == metrale_bench::result::RunStatus::Completed)
        .map(|r| {
            (
...
/// 2026-09-26: `elapsed` of a run scaled to the whole draw: `× n` for `shard=i/n`
/// with `n > 1`, unchanged otherwise.

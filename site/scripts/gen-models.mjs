...
// -----------------------------------------------------------------------------
// SSOT: https://github.com/Metrale/metrale-recipes
//   (read-only mirror expected at /workspace/metrale-recipes/recipes on the host
//    that runs this script — that public repo is the single source of truth for
//    every supported model + its canonical `metralectl run` command).
//
...
const RECIPES_ROOT = process.env.METRALE_RECIPES_ROOT || '/workspace/metrale-recipes/recipes';
const SSOT_URL = 'https://github.com/Metrale/metrale-recipes';
...
function recipeDisplay(stem) {
  ...
  const parts = stem.replace(/-metrale$/, '').split('-');
...
for (const file of files) {
  ...
  // metralectl refuses to launch a multi-node recipe on one node rather than
  // quietly serving something smaller than the recipe describes.
  const command =
    topology === 'single'
      ? `metralectl run ${stem}`
      : `metralectl run ${stem} --rank 0 --world-size 2 --master-addr <spark-1>`;
  const recipe = {

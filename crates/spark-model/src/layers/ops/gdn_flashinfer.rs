...
//! Opt-in FlashInfer GDN prefill via `dlopen(libmetralegdn.so)` — behind `METRALE_GDN_FLASHINFER=1`.
//!
//! Bridges Metrale Engine's native packed-QKV + interleaved gate/beta buffers to the AOT-exported
...
//! (`metrale_gdn_prefill_packed`) takes Metrale Engine's exact native pointers: it deinterleaves
//! gate/beta in-shim and reads q/k/v straight out of the packed buffer via `conv_dim`
//! strides (no copy). Metrale Engine's `gate` is already linear α (the kernel does the `logf`),
//! so there is NO gate-space conversion.
//!
//! dlopen (not link-time) keeps this fully opt-in: the binary builds and runs without the
//! library; it is only loaded when the flag is set. `METRALE_GDN_LIB` overrides the path.
...
//
//   void metrale_gdn_load();
//   int  metrale_gdn_prefill_packed_managed(
...
fn lib() -> Option<&'static Lib> {
    ...
    //     at most once, so `metrale_gdn_load()` (which loads the cubin module onto
    //     the device) is called exactly once, as the shim's `g_loaded` expects.
    LIB.get_or_init(|| unsafe {
        let path =
            std::env::var("METRALE_GDN_LIB").unwrap_or_else(|_| "libmetralegdn.so".to_string());
        let cpath = std::ffi::CString::new(path.clone()).ok()?;
        let h = dlopen(cpath.as_ptr(), RTLD_NOW);
        if h.is_null() {
            tracing::warn!("METRALE_GDN_FLASHINFER: dlopen('{path}') failed — falling back to FLA");
            return None;
        }
        let load = dlsym(h, c"metrale_gdn_load".as_ptr());
        let prefill = dlsym(h, c"metrale_gdn_prefill_packed_managed".as_ptr());
        if load.is_null() || prefill.is_null() {
            tracing::warn!(
                "METRALE_GDN_FLASHINFER: symbols not found in lib — falling back to FLA"
            );
            return None;
        }
        let load: LoadFn = std::mem::transmute(load);
        load(); // load the cubin module onto the device(s) once
        tracing::info!("METRALE_GDN_FLASHINFER: FlashInfer GDN kernel loaded (opt-in)");
        Some(Lib {
...
/// True when `METRALE_GDN_FLASHINFER=1` AND the library + symbols loaded successfully.
pub fn available() -> bool {
    std::env::var("METRALE_GDN_FLASHINFER").as_deref() == Ok("1") && lib().is_some()
}

/// Run one prefill GDN scan through the FlashInfer kernel on Metrale Engine's native buffers.
///
...
pub fn flashinfer_gdn_prefill(
    ...
    if ret != 0 {
        bail!("metrale_gdn_prefill_packed_managed returned {ret}");
    }
    Ok(())
}

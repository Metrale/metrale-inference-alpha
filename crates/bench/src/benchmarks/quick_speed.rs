...
//! 2026-09-26: Quick Speed Bench: warmup plus N timed streaming runs of one
//! prompt, reporting the server's decode rate, TTFT and end-to-end latency. A
//! measurement tool, not a gate: it stores no baseline, reads no threshold,
//! and `gate::coverage::NOT_REQUIRED` lists it.
//!
//! Which tok/s to quote:
//! * Decode tok/s (server), the headline: `usage."response_token/s"`, which
//!   the server computes as `(completion_tokens - 1) / decode_time`
//!   (`ir::Usage::decode_rate_tok_s`). It excludes prefill, client buffering
//!   and the network.
//! * E2E tok/s (client): `completion_tokens / wall`, including prefill and
//!   transport.
//!
//! TPOT is derived from the server's decode rate (`1000 / server tok/s`), or
//! omitted when the server reports none; no client-clock TPOT is emitted,
//! because buffered reads and multi-token emission compress client-side
//! arrival spacing.
//!
//! Every run, warmup included, sends the same prompt, so the timed TTFT is a
//! warm figure. `ttft-warm-gate` and `ttft-cold-gate` control the prefix.
//!
//! Owner: bench, quick_speed.
//! Invariants: each average counts only the runs that reported its value.
...
pub const DESCRIPTOR: BenchmarkDescriptor = BenchmarkDescriptor {
    ...
    expected_secs: 150,
    updated: "2026-08-15",
    // 2026-09-26: A speed probe measures whatever it is pointed at; nothing
    // here compares against a checkpoint-specific number.
    intended_for: None,
    threshold_params: &[],
    needs_confirmation: false,
    // 2026-09-26: Decode tok/s, TTFT and E2E: every headline is a speed number.
    sensitivity: Sensitivity::Speed,
    ctor: || Box::new(QuickSpeed::default()),
};

/// 2026-09-26: Committed prompt fixtures, keyed by the ISL they were cut for:
/// real text rather than a looped filler corpus. Any other ISL falls back to
/// [`stats::make_prompt`]'s synthesized filler.
...
/// 2026-09-26: The counting instruction every prompt ends with (a fixture gets
/// this constant; `make_prompt`'s `PromptMode::Count` appends the same text).
/// Without it the model answers in a handful of tokens and the run measures
/// scheduling overhead rather than decode.
const COUNT_SUFFIX: &str = "Count from 1 upward, one number per line, until told to stop.";

/// 2026-09-26: The prompt for a requested input length: a committed fixture
/// when one exists for exactly that ISL, synthesized filler otherwise.
...
/// 2026-09-26: One timed run, reduced to the numbers this benchmark
/// aggregates.
...
impl RunSample {
    ...
    /// 2026-09-26: Client-observed rate, including prefill and transport.
    ...
    /// 2026-09-26: Per-token decode latency derived from the server's decode
    /// rate: the only TPOT this benchmark reports.
...
/// 2026-09-26: Cross-run averages. Each field averages only the runs that
/// reported the value, so a run without server timings never contributes a
/// zero.
#[derive(Clone, Debug, Default, PartialEq)]
...
impl QuickSpeed {
    ...
    fn summary(&self, avg: &Averages) -> Vec<Stat> {
        vec![
            // 2026-09-26: The headline; its label names the server rate so the
            // two tok/s figures cannot be confused.
            Stat::new(
                "Decode tok/s (server)",

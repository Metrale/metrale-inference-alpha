...
plugin({
  name: 'svelte-runes',
  setup(build) {
    // gates.generated.json ships EMPTY until the first certification campaign
    // banks records, and the vite build is what proves the pages render with
    // none. The unit suites assert behaviour over real records, so under test
    // the alias serves a frozen populated copy instead.
    build.onResolve({ filter: /^\$lib(\/|$)/ }, (args) => ({
      path:
        args.path === '$lib/gates.generated.json'
          ? join(LIB, 'test-fixtures', 'gates.fixture.json')
          : join(LIB, args.path.slice('$lib'.length)),
    }));

...
//! 2026-09-26: Tests of the tool-schema preparation in `grammar::schema`
//! (`sanitize_schema_for_grammar`, `enforce_min_length_on_required_strings`)
//! and of grammar compilation with schemas it has to repair.
//!
//! Owner: server (grammar).
//! Invariants: none beyond the types.

use super::*;
// 2026-09-26: These helpers are `pub(super)` in `grammar::schema`, which
// `grammar` does not re-export, so the glob import above misses them.
...
fn test_hermes_grammar_empty_enum_tool() {
    ...
    // 2026-09-26: Passes when compilation returns, `Ok` or `Err`, without
    // panicking.
...
fn test_qwen3_grammar_empty_properties() {
    ...
    // 2026-09-26: Passes when compilation returns without panicking.

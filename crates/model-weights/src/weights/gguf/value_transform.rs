...
//! with a *different encoding* than the HF checkpoint Metrale Engine's kernels expect.
...
//!
//! 1. **RMSNorm +1 offset.** llama.cpp stores the raw norm weight; Metrale Engine
...
//!
//! 2. **SSM A recovery.** llama.cpp stores `ssm_a = -exp(A_log)`; Metrale Engine's GDN
...
/// For the native keep-packed Q2_0 path (`METRALE_GGUF_NATIVE_Q2=1`): tensors whose
...
//
// The Qwen3-VL ViT tensors already land in Metrale Engine's expected HF layout under the
...
// Conv2d tensors that Metrale Engine wants fused into a single
...
/// The Metrale Engine HF name the fused patch-embed weight is stored under (the name the
...
/// Fuse the per-temporal-frame Conv2d patch-embed tensors into Metrale Engine's

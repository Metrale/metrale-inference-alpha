...
//! 2026-09-25: Hardware tests for `RdmaKvBackend`; they need a GPU and a cache peer at
//! `$METRALE_KV_PEER`, so they are ignored by default.
//!
...
// 2026-09-25: Read a pinned destination's bytes on the host; both the bounce path's
// `copy_h2d` and a zero-copy READ land there.
unsafe fn uma_bytes(buf: &PinnedBuffer, n: usize) -> &[u8] {
    unsafe { std::slice::from_raw_parts(buf.ptr as *const u8, n) }
}

#[test]
#[ignore = "requires GPU + live cache-peer at $METRALE_KV_PEER"]
fn rdma_kv_round_trip() {
    let _ctx = CudaCtx::new(0).expect("cuda init");
    let peer = std::env::var("METRALE_KV_PEER").expect("set METRALE_KV_PEER=host:port");
    ...
    // 2026-09-25: Pinned destinations, so the test runs with zero-copy on or off
    // (`METRALE_KV_ZERO_COPY`).
    let devs: Vec<_> = keys
        .iter()
...
fn rdma_kv_bandwidth() {
    ...
    // 2026-09-25: A pinned destination, so zero-copy (`METRALE_KV_ZERO_COPY=1`) can

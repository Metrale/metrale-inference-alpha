...
//! Complete per-decode-step logit dump for Metrale Engine↔vLLM divergence analysis.
//!
//! Gated by `METRALE_LOGIT_DUMP=<file>`. When set, every decode step appends
//! one JSONL record capturing the FULL pre/post-processor picture:
//!   - `raw_topk`:  top-K (id, logit) of the raw model logits, BEFORE Metrale Engine's
...
//!                  MODEL divergence.
//!   - `bias`:      every (id, delta) Metrale Engine applied this step. vLLM applies
//!                  none of these — so this list IS the Metrale-only processor
//!                  divergence, itemized.
...
//! differs) or a Metrale Engine processor (raw_topk matches but `bias` flips it).
...
/// Append one per-step record. No-op unless `METRALE_LOGIT_DUMP` is set.

...
Build with `METRALE_TARGET_HW=b200 METRALE_TARGET_MODEL=kimi-k3
METRALE_TARGET_QUANT=mxfp4 CARGO_TARGET_DIR=target/k3-b200 cargo build
--locked --release -p metrale-server --features nccl` (one shell command).
The B200 hardware declaration selects `sm_100a` and disables unsupported
warp-level blockscaled MMA. Do not reuse B300 or DGX Spark binaries.

KDA, MLA and the DeepSeek E8M0 GEMM are gb10's sources, read through
`kernels/b200/HARDWARE.toml [hardware] inherits = "gb10"` (the B200-owned
byte-identical copies were collapsed on 2026-09-24). B200 holds one source of
its own here, `bf16/dense_f32io.cu`, which `mxfp4/KERNEL.toml` `use`s; a
B200-tuned KDA or MLA kernel would be a real file in the quant directory,
declared in its `KERNEL.toml [shadow]`.

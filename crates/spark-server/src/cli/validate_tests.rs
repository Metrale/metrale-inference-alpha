...
/// Parse a `met serve ...` command line into `ServeArgs` for testing.
...
fn an_absent_lever_flag_parses_as_unspecified() {
    ...
    // write to, which turned `METRALE_SSM_TAIL_MIDCHUNK=0` and
    // `METRALE_MTP_GATE_FORCE=1` into documented, echoed, silent no-ops under
    // `met serve`. Absent has to stay absent all the way to
    // `publish_kernel_flags` for the fallback to be reachable.
    let a = parse(&[]);
    assert!(a.ssm_tail_midchunk.is_none(), "METRALE_SSM_TAIL_MIDCHUNK");
    assert!(a.mtp_gate.is_none(), "METRALE_MTP_GATE_FORCE");
    assert!(a.ssm_h_dtype.is_none(), "METRALE_SSM_H_FP16");
    assert!(a.gdn_fused_norm.is_none(), "METRALE_GDN_FUSED_NORM");
    assert!(
    ...
    assert!(a.prefill_varlen_batch.is_none(), "METRALE_PREFILL_VARLEN");
    ...
    // every boot and `METRALE_PREFILL_CODISPATCH` becomes inert while `--help`
    // still documents it.
    assert!(a.prefill_codispatch.is_none(), "METRALE_PREFILL_CODISPATCH");

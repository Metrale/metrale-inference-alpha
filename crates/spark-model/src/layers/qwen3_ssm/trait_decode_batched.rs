...
/// METRALE_K4_DIAG=1 phase checkpoint (see verify_c2.rs). Synchronizes the
...
/// (`METRALE_NO_BATCHED_BA_GATES` restores the per-token GEMV + `compute_gdn_gates`
...
/// (`METRALE_NO_BATCHED_GDN_NORM` restores the per-token loop). The batched kernel
...
/// projection — `METRALE_NO_QKVZ_NVFP4_DECODE` restores the FP8-prefill-copy
...
impl Qwen3SsmLayer {
    ...
    pub(super) fn decode_batched_inner(
        ...
        // (localized via METRALE_K4_DIAG, 2026-07-18). `w8a16_gemv_batch4`
        ...
        ) && let Some(ref nvfp4_t) = self.qkvz_nvfp4_t
        {
            ...
            // Kill switch METRALE_NO_QKVZ_NVFP4_DECODE (PRESENCE check).
            self.ms_proj_gemm(
                ctx.gpu,
                normed,
                nvfp4_t,
        ...
        match gdn {
            ...
            GdnStates::Multi {
                ...
                // METRALE_NO_VERIFY_GDN_BATCH). Fallback: the SAME per-sequence
            ...
            }
        {
            ...
            // Kill switch METRALE_NO_VERIFY_OUTPROJ_TGEMM (PRESENCE check).
            //

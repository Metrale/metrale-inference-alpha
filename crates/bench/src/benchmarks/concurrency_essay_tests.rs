...
//! 2026-09-26: Tests for the essay fixture against
//! `bench/ladder38/harness_w55_conc_ladder.py`'s own output. A child module of
//! `concurrency_verdict_tests`.
//!
...
/// 2026-09-26: The digests are of the harness's `make_prompt(isl)` after
/// `set_nonce_base(0)`, in essay mode (nonce `[req 000001]`), for isl 64, 128
/// and 512. Re-derived from the harness on 2026-09-26.
#[test]
...
fn essay_nonce_is_fixed_width_and_the_other_fixtures_keep_their_tags() {
    ...
    assert_eq!(essay_nonce_tag(1_000_007), "req 000007");
    assert_eq!(prompt_plan(2, 0, Fixture::Natural).measured, ["c0", "c1"]);
    assert_eq!(prompt_plan(2, 0, Fixture::Count).measured, ["c0", "c1"]);
}

/// 2026-09-26: Only the essay request pins the penalties, and the essay
/// fixture is chosen only by name.
#[test]

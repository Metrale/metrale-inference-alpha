...
fn parse_config_dispatch(json: &str) -> Result<ModelConfig> {
    ...
    match top_model_type {
        ...
        "nemotron_h" | "nemotron_h_puzzle" => {
            ...
            // Rewrite the JSON so serde can deserialize, then map to Metrale Engine fields.
            ...
            // Map Nemotron-H field names → Metrale Engine canonical names
...
/// Map Puzzle `layers_block_type` / `block_configs` onto Metrale Engine layer schedule.

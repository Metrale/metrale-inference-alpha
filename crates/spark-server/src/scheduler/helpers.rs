...
// 2 was tuned for Metrale Engine's pre-anchored substring-scan detector, where
...
// `METRALE_DISABLE_WATCHDOGS` is resolved once into `SchedLevers::disable_watchdogs`
...
/// thinking-loop). Cached once on first read from `METRALE_DISABLE_WATCHDOGS`.
///
...
///
/// `METRALE_DISABLE_WATCHDOGS=1`/`true` (case-insensitive) → all
...
///
/// `METRALE_DISABLE_FORCED_TOKEN=1` (or `true`) forces it off — a
...
/// the `ModelBehavior` struct lives in the `metrale-kernels` crate, which
/// this change deliberately does not touch.
/// Pure parse of the `METRALE_DISABLE_FORCED_TOKEN` env value into the
...
/// `METRALE_TOOL_RESPONSE_STOP` and `METRALE_TOOL_EOS_ESCAPE` — are now
...
/// Kill-switch: `METRALE_GRAMMAR_BUDGET_CLOSE=0`/`false` disables.
pub fn grammar_budget_close_enabled() -> bool {
    env_flag_default_on("METRALE_GRAMMAR_BUDGET_CLOSE")
}
...
pub struct WatchdogParams {
    ...
    /// (`--content-loop-min-repeats` / `METRALE_CONTENT_LOOP_MIN_REPEATS`).
...
impl WatchdogParams {
    ...
    /// (#328 family); precedence CLI → `METRALE_CONTENT_LOOP_MIN_REPEATS` →
...
/// `METRALE_CONTENT_LOOP_WATCHDOG` (env, `1`/`true`/`0`/`false`) → MODEL.toml
...
/// with no reachable off-switch short of `METRALE_DISABLE_WATCHDOGS=1`
...
/// launch next to the model) → `METRALE_MAX_INTER_TOOL_PROSE` (env) →
...
///
/// Aliases the metrale-kernels default rather than restating it: P2-1
...
/// 2026-05-24 v3: ALGORITHM REPLACE. Switched from Metrale Engine's scan-anywhere
...
///
/// **Why**: Metrale Engine's scan-anywhere algorithm fires on ANY period match
...
/// Metrale Engine would fire even though the model is no longer looping).
///
...
mod inter_tool_prose_tests {
    ...
    fn resolved_default_budget_is_plan_friendly() {
        ...
        let p =
            WatchdogParams::from_behavior(&metrale_kernels::ModelBehavior::default(), None, None);
        assert!(
...
mod content_loop_override_tests {
    ...
    fn min_repeats_cli_reaches_the_resolved_params() {
        let p = WatchdogParams::from_behavior(
            &metrale_kernels::ModelBehavior::default(),
            None,
            Some(5),
        );

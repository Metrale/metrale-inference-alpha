...
//! 2026-09-26: `impl ResponseStore`: construction from env, insert, get, delete.
//!
//! Owner: server API.
//! Invariants:
//! - Every removal from the map (capacity eviction, TTL expiry, `delete`) also
//!   calls `backend.forget` for that id.
//! - `get` and `delete` never act on an entry of the other kind.
...
impl ResponseStore {
    /// 2026-09-26: Build from env.
    /// - `METRALE_STORE_MAX_ENTRIES` (default 10 000, minimum 1)
    /// - `METRALE_STORE_TTL_SECONDS` (default 86 400, minimum 1)
    /// - `METRALE_STORE_DIR`: when set and non-empty, enable filesystem
    ///   persistence and replay the non-expired entries. If the directory cannot
    ///   be set up, a warning is logged and the store stays in memory.
    ///
    /// # Errors
    /// When `METRALE_STORE_MAX_ENTRIES` or `METRALE_STORE_TTL_SECONDS` is set to
    /// something that is not a whole number at or above its minimum.
    pub fn from_env() -> Result<Arc<Self>, String> {
        let max_entries = crate::env_config::parse_min(
            "METRALE_STORE_MAX_ENTRIES",
            std::env::var("METRALE_STORE_MAX_ENTRIES").ok().as_deref(),
            1_usize,
            "how many stored responses to keep before evicting the coldest",
        )?
        .unwrap_or(10_000);
        let ttl_secs = crate::env_config::parse_min(
            "METRALE_STORE_TTL_SECONDS",
            std::env::var("METRALE_STORE_TTL_SECONDS").ok().as_deref(),
            1_u64,
        ...
        let (backend, persistent, persist_dir): (Box<dyn StoreBackend>, bool, Option<PathBuf>) =
            match std::env::var("METRALE_STORE_DIR")
                .ok()
                .filter(|s| !s.is_empty())
            {
                Some(dir) => {
                    ...
                    match FilesystemBackend::new(&p) {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "response_store: falling back to in-memory (METRALE_STORE_DIR={dir} init failed: {e})"
                            );
    ...
    /// 2026-09-26: Insert or replace. Moves the id to the most recent end of the LRU
    /// order and persists to the backend (a no-op when persistence is disabled). A
    /// capacity eviction also removes the evicted entry from the backend.
    ...
    /// 2026-09-26: Fetch a clone of the stored body and metadata, refreshing its LRU
    /// position and idle clock. A TTL-expired entry is evicted (and removed from
    /// the backend) and treated as missing. A kind mismatch returns `None` without
    /// eviction.
    ...
    /// 2026-09-26: Remove an entry. Returns `true` when the id existed and matched
    /// `kind`, `false` otherwise. Also removes it from the backend.

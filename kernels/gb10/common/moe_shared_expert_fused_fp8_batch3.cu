...
// 2026-09-25: Three-token FP8 MoE GEMVs with the shared expert in the same launch:
// gate+up, SiLU(gate) * up then down, and the weighted sum with the sigmoid-gated shared
// expert.
//
// Owner: gb10 kernels.
// Invariants:
// - In gate+up and down, blockIdx.y < 3 * top_k is routed slot y (token y / top_k, slot
//   y % top_k, output row y); y = 3 * top_k + t is token t's shared expert.
// - Weights are row-major [N, K] FP8 E4M3 with FP32 scales [ceil(N / 128), ceil(K / 128)].
// - A null routed weight pointer writes zeros; the shared-expert pointers are not checked.
// - K must be a multiple of 8, and at most 1024 in down (the size of s_act).
// - Launch: grid (ceil(N / 8), 3 * (top_k + 1), 2 for gate+up or 1 for down), block 128.
...
// 2026-09-25: Gate+up for three tokens. blockIdx.z selects gate (0) or up (1).
...
extern "C" __global__ void moe_expert_gate_up_shared_fp8_batch3(
    ...
    // 2026-09-25: Each thread loads two of the 256 LUT entries, so blockDim.x must be 128.
...
// 2026-09-25: SiLU(gate) * up into s_act, then the down GEMV, for three tokens.
...
// Unlike moe_weighted_sum_blend_batch3, gate_weight must be non-null: it is not checked.
// Launch: grid (ceil(hidden / 256), 3, 1), block 256; the gate loop assumes 256 threads.

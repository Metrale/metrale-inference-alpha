...
| | |
|---|---|
| Branch | `feat/video-support`, pushed to `metrale` |
| PR | #516, **draft**, based on `feat/qwen3.8-27b-support` (#513) |
| Base sync | merged `metrale/feat/qwen3.8-27b-support` at 2026-08-14; **0 behind** as of the last push |
...
**Metrale Engine rendered vision markers grouped by modality, not in the order the client sent them.** A request with `video_url` first and `image_url` second rendered `<|image_pad|>` before `<|video_pad|>`.
...
1. `crates/server/src/openai/chat_message.rs` — `ParsedContent` held `images: Vec<String>` and `videos: Vec<String>`. **Order was already destroyed at the wire parse**, before any IR existed, on both the chat-completions and Responses parsers.
2. `crates/server/src/api/chat/msg_entry.rs` — `collect_message_images` walked `m.content` twice, once per modality.
3. `crates/server/src/api/chat/template.rs` — `build_json_messages_for` emitted `image_count` image markers then `video_count` video markers.
...
* **Not this checkpoint's double-quantisation.** `Qwen/Qwen3.8-27B-FP8` — the same weights in the block-scaled FP8 format Metrale Engine loads natively, with no requant — failed the same way.
...
* It wrote **only** `tests/fixtures/videos/`, which nothing reads. The compiled-in copy under `crates/bench/assets/video/` was never regenerated by it — so a fixture fix would have silently changed nothing. It now writes both from one encode.
...
* **The command in the previous handoff does not parse.** `--pull-request-gate` is mutually exclusive with `--url` / `--model` — the gate serves the benchmark's own recipe and refuses to be pointed at a server someone else started. The working form is:
  ...
  ```bash
  ./target/release/met benchmark run video-fidelity --pull-request-gate \
    --hardware gb10 --checkpoint Qwen/Qwen3.6-35B-A3B-FP8 \
    --serve-override video_allow_ffmpeg=true
  ```

* **`video-fidelity` needs that `--serve-override`.** No serve recipe enables ffmpeg, so the gate's own server refuses every MP4, the run ends Failed and **no record is written at all**. The override is recorded in the record's provenance, which is honest but means these records do not measure the recipe as pinned. The real fix is `video_allow_ffmpeg: true` in the three vision recipes upstream in `Metrale/metrale-recipes` — that is a different repo, so it was not done here.
* The qwen3.8 recipe was missing from the local recipe index (`~/.metrale/metrale-recipes/index.json`, 26 cached vs 28 upstream) and a refresh is only reachable through the interactive TUI Library. Rebuilt by hand from the tree sha; a backup of the old index is not in the repo.
...
```bash
# Build (ALWAYS all targets — see memory: metrale-build-all-targets)
export METRALE_TARGET_HW=gb10 METRALE_TARGET_MODEL='*' METRALE_TARGET_QUANT='*' \
...
./target/release/met serve --model-from-path "$SNAP" --model-name m \
...
# Run a gate
./target/release/met benchmark run vision-fidelity --url http://127.0.0.1:8897 \
...
* **`tokenizer::tests::qwen_dense_parity` is flaky** under `cargo test --workspace`, ~2 runs in 5, a different test each time; passes in isolation and under `-p metrale-server --bin met`. The diff is JSON whitespace in the rendered tools block — two serialization paths and some shared state choosing between them under parallelism. Unrelated to vision/video.
...
* **LoC cleanup** — parked until the buildout finishes. Currently over the 500-line cap and not allow-listed: `crates/bench/src/benchmarks/video/driver.rs` (917), `crates/server/src/api/chat/msg_entry.rs` (578 — down 19 from the ordering fix, which folded two copies of the URL-policy branch into one helper), plus pre-existing `crates/kernels/src/lib.rs` (501, was 484 upstream — the `min_p` field is part of that growth) and `crates/server/src/tui/bench_state.rs` (504).

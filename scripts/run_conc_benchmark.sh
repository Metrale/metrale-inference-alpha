#!/usr/bin/env bash
#
# Run smoke test + coherence + concurrency benchmark for all 6 Metrale Engine models.
...
IMAGE="${IMAGE:-metrale-gb10:latest}"
EP_IMAGE="${EP_IMAGE:-metrale-gb10:latest}"
RESULTS_DIR="/workspace/metrale/conc-bench-results"
mkdir -p "$RESULTS_DIR"
...
echo "=== Metrale Engine Concurrency Benchmark Sweep ==="
echo "Date: $(date)"
echo "Image: $IMAGE"
echo ""
...
ssh "$EP_RANK1_HOST" "sudo docker rm -f metrale-ep1 2>/dev/null" 2>/dev/null || true
sleep 2
...
if ! wait_for_server "http://localhost:8888" 600; then
    ...
    sudo docker logs metrale-ep0 2>&1 | tail -30 >> "$RESULTS_DIR/122B-EP2.txt"
    sudo docker rm -f metrale-ep0 2>/dev/null
    ssh "$EP_RANK1_HOST" "sudo docker rm -f metrale-ep1 2>/dev/null" 2>/dev/null
else
    # Smoke test
    echo "  Running smoke test..."
    smoke=$(curl -s --max-time 90 http://localhost:8888/v1/chat/completions \

...
// 2026-09-25: Kernels `attn_prefill_paged_batched` and `_64`: the attn_prefill_paged.cu kernels over several streams in one
// launch. With PREFILL_BATCHED, blockIdx.z selects the stream, which has its own block table and, when the arrays are
// passed, its own `cu_seqlens` row range and `kv_lens` entry (prefill_paged_compute.cuh).
...
#define KERNEL_NAME attn_prefill_paged_batched
